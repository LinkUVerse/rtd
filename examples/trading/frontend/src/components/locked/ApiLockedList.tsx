...
import { useRtdClient } from "@linku/dapp-kit";
...
export function LockedList({
  ...
  const { data, fetchNextPage, hasNextPage, isLoading, isFetchingNextPage } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam }) => {
         ...
         */
        const objects = await rtdClient.multiGetObjects({
        ...
        return {
          rtdObjects: objects.map((x) => x.data),
          api: data,
        };
      },
  ...
  const rtdObjects = () => {
    ...
    return data?.flatMap((x) => x.rtdObjects) || [];
  };
