...
import { fromBase64 } from '@linku/rtd/utils';
...
export const getActiveAddress = () => {
	return execSync(`${RTD_BIN} client active-address`, { encoding: 'utf8' }).trim();
};

/** Returns a signer based on the active address of system's rtd. */
export const getSigner = () => {
	const sender = getActiveAddress();

	const keystore = JSON.parse(
		readFileSync(path.join(homedir(), '.rtd', 'rtd_config', 'rtd.keystore'), 'utf8'),
	);
	...
	for (const priv of keystore) {
		...
		if (pair.getPublicKey().toRtdAddress() === sender) {
			return pair;
		}
	}
...
export const publishPackage = async ({
	...
	const { modules, dependencies } = JSON.parse(
		execSync(`${RTD_BIN} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
			encoding: 'utf-8',
		}),
	);
