...
use rtd_sdk::{
    rpc_types::{
        DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, ObjectChange, RtdData,
        RtdExecutionStatus, RtdObjectData, RtdObjectDataFilter, RtdObjectDataOptions,
        RtdObjectResponse, RtdObjectResponseQuery, RtdProtocolConfigValue,
        RtdTransactionBlockEffectsAPI, RtdTransactionBlockResponse,
    },
...
pub struct Connection {
    /// The Rtd CLI config file, (default: ~/.rtd/rtd_config/client.yaml)
    #[clap(long)]
...
impl Client {
    ...
    pub(crate) fn new(conn: Connection) -> Result<Self> {
        let Some(config) = conn.config.or_else(|| {
            ...
            default.extend([".rtd", "rtd_config", "client.yaml"]);
            Some(default)
        }) else {
            bail!(
                "Cannot find wallet config. No config was supplied, and the default path \
                 (~/.rtd/rtd_config/client.yaml) does not exist.",
            );
        };
    ...
    pub(crate) async fn turn_cap(&mut self, game: &Game) -> Result<ObjectRef> {
        ...
        let query = Some(RtdObjectResponseQuery::new(
            Some(RtdObjectDataFilter::StructType(turn_cap_type.clone())),
            Some(RtdObjectDataOptions::new().with_bcs()),
        ));
        ...
        loop {
            ...
            for RtdObjectResponse { data, error } in response.data {
    ...
    pub(crate) async fn new_shared_game(&mut self, opponent: RtdAddress) -> Result<ObjectID> {
        let player = self.wallet.active_address()?;
    ...
    pub async fn new_owned_game(&mut self, opponent_key: PublicKey) -> Result<ObjectID> {
        ...
        let opponent = RtdAddress::from(&opponent_key);
        ...
        let admin = RtdAddress::from(&admin_key);
        let admin_bytes =
    ...
    pub async fn make_owned_move(
        ...
        let data = self.build_tx_data(player, builder.finish()).await?;
        let tx = self.wallet.sign_transaction(&data).await;
        let RtdTransactionBlockResponse {
    ...
    async fn max_gas_budget(&self) -> Result<u64> {
        ...
        let Some(Some(RtdProtocolConfigValue::U64(max))) = cfg.attributes.get("max_tx_gas") else {
    ...
    async fn execute_transaction(&self, tx: Transaction) -> Result<RtdTransactionBlockResponse> {
        let response = self
            .wallet
    ...
    async fn client(&self) -> Result<RtdClient> {
        self.wallet
            .get_client()
            .await
