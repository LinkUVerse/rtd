...
import { ObjectRef, Transaction } from "@linku/rtd/transactions";
...
export class Transactions {
    ...
    newMultiSigGame(player: PublicKey, opponent: PublicKey): Transaction {
        ...
        const game = tx.moveCall({
            ...
            arguments: [
                tx.pure.address(player.toRtdAddress()),
                tx.pure.address(opponent.toRtdAddress()),
                tx.pure(bcs.vector(bcs.u8()).serialize(admin.toRawBytes()).toBytes()),
            ],
        });

        tx.transferObjects([game], admin.toRtdAddress());
