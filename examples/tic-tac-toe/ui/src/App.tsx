...
import { ConnectButton, useCurrentAccount, useRtdClientContext } from "@linku/dapp-kit";
import { isValidRtdObjectId, normalizeRtdObjectId } from "@linku/rtd/utils";
...
function Content() {
    ...
    const addr = normalizeRtdObjectId(path);
    ...
    if (packageId === null) {
        ...
        return <Root />;
    } else if (isValidRtdObjectId(addr)) {
        return <Game id={addr} />;
    } else {
        return (
            <Error title="Invalid Game ID">
                <code>"{path}"</code> is not a valid RTD object ID.
            </Error>
        );
    }
}
