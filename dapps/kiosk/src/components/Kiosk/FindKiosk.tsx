...
import { normalizeRtdAddress } from '@linku/rtd/utils';
...
export default function FindKiosk() {
	...
	const viewKiosk = (e?: FormEvent<HTMLFormElement>) => {
		...
		const id = normalizeRtdAddress(searchKiosk);
		navigate(`/kiosk/${id}`);
		setSearchKioskId(id);
	};
	...
	return (
		<form onSubmit={viewKiosk} className="text-center lg:min-w-[700px]">
            ...
            focus:border-primary block w-full p-2.5 outline-primary"
						placeholder="Enter an address or a Rtd Kiosk ID to search for a kiosk..."
						required
					/>
				</div>
				<button
