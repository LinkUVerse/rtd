...
import {
  ...
  isValidRtdObjectId,
  MIST_PER_RTD,
} from '@linku/rtd/utils';
import { bcs } from '@linku/rtd/bcs';
import { program } from 'commander';
import { KIOSK_LISTING, KioskClient, KioskTransaction, Network } from '@linku/kiosk';
import { RtdClient, getFullnodeUrl } from '@linku/rtd/client';
...
program
  .command('list')
  .description('list an item in the Kiosk for the specified amount of RTD')
  .argument('<item ID>', 'The ID of the item to list')
  .argument('<amount MIST>', 'The amount of RTD to list the item for')
  .action(listItem);
...
async function newKiosk() {
  const sender = keypair.getPublicKey().toRtdAddress();
...
async function showInventory({ address, onlyDisplay, cursor, filter }) {
  const owner = address || keypair.getPublicKey().toRtdAddress();

  if (!isValidRtdAddress(owner)) {
    throw new Error(`Invalid RTD address: "${owner}"`);
  }
...
async function showKioskContents({ id, address }) {
  ...
  if (id) {
    if (!isValidRtdObjectId(id)) {
      throw new Error(`Invalid Kiosk ID: "${id}"`);
    }

    kioskId = id;
  } else {
    const sender = address || keypair.getPublicKey().toRtdAddress();

    if (!isValidRtdAddress(sender)) {
      throw new Error(`Invalid RTD address: "${sender}"`);
    }
  ...
  const tabledItems = items
    .map((item) => ({
      ...
      'price (RTD)': item.listing ? formatAmount(item.listing.price) : 'N/A',
    }))
...
async function placeItem(itemId) {
  const kioskCap = await findKioskCap().catch(() => null);
  const owner = keypair.getPublicKey().toRtdAddress();

  if (kioskCap === null) {
    throw new Error('No Kiosk found for sender; use `new` to create one');
  }

  if (!isValidRtdObjectId(itemId)) {
...
async function takeItem(itemId, { address }) {
  const cap = await findKioskCap().catch(() => null);
  const receiver = address || keypair.getPublicKey().toRtdAddress();

  if (!isValidRtdAddress(receiver)) {
...
/**
 * Command: `list`
 * Description: Lists an item in the Kiosk for the specified amount of RTD
 */
...
async function purchaseItem(itemId, opts) {
  ...
  if (inputKioskId && !isValidRtdObjectId(inputKioskId)) {
...
function formatAmount(amount) {
  ...
  if (amount <= MIST_PER_RTD) {
    return Number(amount) / Number(MIST_PER_RTD);
  }
