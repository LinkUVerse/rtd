...
import { RtdObjectRef } from '@linku/rtd/client';
import { getFaucetHost, requestRtdFromFaucetV1 } from '@linku/rtd/faucet';
...
export async function sponsorTransaction(sender: string, transactionKindBytes: Uint8Array) {
	...
	await requestRtdFromFaucetV1({ recipient: address, host: getFaucetHost('testnet') });

	let payment: RtdObjectRef[] = [];
	let retires = 50;
	while (retires !== 0) {
