...
use rtd_types::{
    base_types::{FullObjectRef, RtdAddress, random_object_ref},
...
impl Scenario {
    ...
    fn new_outputs() -> TransactionOutputs {
        ...
        let (sender, keypair): (RtdAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
        let (receiver, _): (RtdAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    pub async fn commit(&mut self, tx: TransactionDigest) -> RtdResult {
