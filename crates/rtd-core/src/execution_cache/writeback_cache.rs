...
use crate::authority::authority_store::{
    ExecutionLockWriteGuard, LockDetailsDeprecated, ObjectLockStatus, RtdLockResult,
};
...
use rtd_types::accumulator_root::{AccumulatorObjId, AccumulatorValue};
use rtd_types::base_types::{
    EpochId, FullObjectID, ObjectID, ObjectRef, SequenceNumber, VerifiedExecutionData,
};
use rtd_types::bridge::{Bridge, get_bridge};
use rtd_types::digests::{ObjectDigest, TransactionDigest, TransactionEffectsDigest};
...
use rtd_types::transaction::{TransactionDataAPI, VerifiedSignedTransaction, VerifiedTransaction};
use tap::TapOptional;
...
impl WritebackCache {
    ...
    fn build_db_batch(&self, epoch: EpochId, digests: &[TransactionDigest]) -> Batch {
        let _metrics_guard = linku_metrics::monitored_scope("WritebackCache::build_db_batch");
    ...
    fn commit_transaction_outputs(
        ...
        let _metrics_guard =
            linku_metrics::monitored_scope("WritebackCache::commit_transaction_outputs");
        ...
        let _metrics_guard =
            linku_metrics::monitored_scope("WritebackCache::commit_transaction_outputs::flush");
...
impl ObjectCacheRead for WritebackCache {
    fn get_package_object(&self, package_id: &ObjectID) -> RtdResult<Option<PackageObject>> {
        ...
        if let Some(p) = self.get_object_impl("package", package_id) {
            if p.is_package() {
                ...
                Err(RtdErrorKind::UserInputError {
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> RtdLockResult {
        ...
        match self.get_object_by_id_cache_only("lock", &obj_ref.0) {
            ...
            CacheResult::NegativeHit => {
                Err(RtdError::from(UserInputError::ObjectNotFound {
                    object_id: obj_ref.0,
