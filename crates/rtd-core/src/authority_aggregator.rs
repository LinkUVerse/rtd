...
use linku_metrics::{GaugeGuard, MonitorCancellation, spawn_monitored_task};
use std::convert::AsRef;
use std::net::SocketAddr;
use rtd_authority_aggregation::ReduceOutput;
use rtd_authority_aggregation::quorum_map_then_reduce_with_timeout;
use rtd_config::genesis::Genesis;
use rtd_network::{
    DEFAULT_CONNECT_TIMEOUT_SEC, DEFAULT_REQUEST_TIMEOUT_SEC, default_linku_network_config,
};
use rtd_swarm_config::network_config::NetworkConfig;
use rtd_types::crypto::{AuthorityPublicKeyBytes, AuthoritySignInfo};
use rtd_types::error::{RtdErrorKind, UserInputError};
use rtd_types::message_envelope::Message;
use rtd_types::object::Object;
use rtd_types::quorum_driver_types::{GroupedErrors, QuorumDriverResponse};
...
pub fn group_errors(errors: Vec<(RtdError, Vec<AuthorityName>, StakeUnit)>) -> GroupedErrors {
...
struct ProcessTransactionState {
    ...
    errors: Vec<(RtdError, Vec<AuthorityName>, StakeUnit)>,
    // This is exclusively non-retryable stake.
    non_retryable_stake: StakeUnit,
    // This includes both object and package not found rtd errors.
...
impl ProcessTransactionState {
    pub fn record_conflicting_transaction_if_any(
        ...
        if let RtdErrorKind::ObjectLockConflict {
            obj_ref,
    ...
    pub fn check_if_error_indicates_tx_finalized_with_different_user_sig(
        ...
        // and return Rtd::FailedToVerifyTxCertWithExecutedEffects.
...
struct ProcessCertificateState {
    ...
    non_retryable_stake: StakeUnit,
    non_retryable_errors: Vec<(RtdError, Vec<AuthorityName>, StakeUnit)>,
    retryable_errors: Vec<(RtdError, Vec<AuthorityName>, StakeUnit)>,
    // As long as none of the exit criteria are met we consider the state retryable
    // 1) >= 2f+1 signatures
    // 2) >= f+1 non-retryable errors
    retryable: bool,
...
impl AuthorityAggregator<NetworkAuthorityClient> {
    ...
    pub fn new_from_epoch_start_state(
        ...
        let committee = epoch_start_state.get_rtd_committee_with_network_metadata();
    ...
    pub fn new_from_committee(
        ...
        let net_config = default_linku_network_config();
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn get_latest_object_version_for_testing(
        ...
        let result = quorum_map_then_reduce_with_timeout(
            ...
            .await.map_err(|_state| RtdError::from(UserInputError::ObjectNotFound {
                object_id,
                version: None,
            }))?;
        Ok(result.0)
    ...
    pub async fn get_latest_system_state_object_for_testing(
        &self,
    ) -> anyhow::Result<RtdSystemState> {
        #[derive(Debug, Default)]
        struct State {
            latest_system_state: Option<RtdSystemState>,
            total_weight: StakeUnit,
        }
        let initial_state = State::default();
        let result = quorum_map_then_reduce_with_timeout(
    ...
    fn record_rpc_error_maybe(metrics: Arc<AuthAggMetrics>, display_name: &str, error: &RtdError) {
        if let RtdErrorKind::RpcError(_message, code) = error.as_inner() {
            metrics
                .total_rpc_err
    ...
    fn handle_process_transaction_response(
        ...
        response: RtdResult<PlainTransactionInfoResponse>,
        name: AuthorityName,
        weight: StakeUnit,
    ) -> RtdResult<Option<ProcessTransactionResult>> {
        match response {
    ...
    pub async fn process_certificate(
        ...
        .map_err(|state| {
            ...
            for (rtd_err, _, _) in state.retryable_errors.iter().chain(state.non_retryable_errors.iter()) {
                self
                    .metrics
                    .total_aggregated_err
                    .with_label_values(&[
    ...
    fn handle_process_certificate_response(
        ...
        response: RtdResult<HandleCertificateResponseV3>,
        name: AuthorityName,
    ) -> RtdResult<Option<QuorumDriverResponse>> {
        match response {
...
impl<'a> AuthorityAggregatorBuilder<'a> {
        ...
        if let Some(genesis) = self.get_genesis() {
            ...
            state
                ...
                .map(|v| {
                    ...
                    let name = metadata.rtd_pubkey_bytes();
    ...
    pub fn build_network_clients(
        ...
        let genesis_committee = self.get_genesis().unwrap().committee().unwrap();
