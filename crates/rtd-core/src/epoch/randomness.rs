...
use rtd_types::committee::{Committee, EpochId, StakeUnit};
use rtd_types::crypto::{AuthorityKeyPair, RandomnessRound};
...
impl RandomnessManager {
    ...
    pub async fn start_dkg(&mut self) -> RtdResult {
    ...
    pub(crate) fn reserve_next_randomness(
        ...
        output: &mut ConsensusCommitOutput,
    ) -> RtdResult<Option<RandomnessRound>> {
        let epoch_store = self.epoch_store()?;
    ...
    fn epoch_store(&self) -> RtdResult<Arc<AuthorityPerEpochStore>> {
        self.epoch_store
            .upgrade()
            .ok_or(RtdErrorKind::EpochEnded(self.epoch).into())
    }
...
impl RandomnessReporter {
    ...
    /// the round.
    pub fn notify_randomness_in_checkpoint(&self, round: RandomnessRound) -> RtdResult {
        let epoch_store = self
            .epoch_store
            .upgrade()
            .ok_or(RtdErrorKind::EpochEnded(self.epoch))?;
    ...
    async fn test_dkg(version: u64) {
        ...
        for validator in network_config.validator_configs.iter() {
            ...
            let randomness_manager = RandomnessManager::try_new(
                ...
                rtd_network::randomness::Handle::new_stub(),
