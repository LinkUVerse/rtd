...
impl From<BalanceIndexInfo> for rtd_types::storage::BalanceInfo {
    fn from(index_info: BalanceIndexInfo) -> Self {
        ...
        // Anyone can call `rtd::balance::create_supply` and mint unbounded supply, potentially pushing
        ...
        rtd_types::storage::BalanceInfo { balance }
    }
}
...
impl IndexStoreTables {
    ...
    fn init(
        ...
        rpc_config: &rtd_config::RpcConfig,
    ) -> Result<(), StorageError> {
    ...
    fn extract_accumulator_version(
        &self,
        tx: &rtd_types::full_checkpoint_content::CheckpointTransaction,
    ) -> Option<u64> {
        ...
        if pt.shared_input_objects().any(|obj| {
            obj.id == RTD_ACCUMULATOR_ROOT_OBJECT_ID
                && obj.mutability == rtd_types::transaction::SharedObjectMutability::Mutable
        }) {
            return tx.output_objects.iter().find_map(|obj| {
                if obj.id() == RTD_ACCUMULATOR_ROOT_OBJECT_ID {
        ...
        for acc in acc_events {
            if let AccumulatorValue::EventDigest(event_digests) = &acc.write.value {
                ...
                if let Some(stream_id) =
                    rtd_types::accumulator_root::stream_id_from_accumulator_event(&acc)
                {
    ...
    fn initialize_current_epoch(
        ...
        let system_state = rtd_types::rtd_system_state::get_rtd_system_state(authority_store)
...
impl RpcIndexStore {
    ...
    pub fn new_without_init(dir: &Path) -> Self {
        ...
        Self {
            tables,
            pending_updates: Default::default(),
            rpc_config: rtd_config::RpcConfig::default(),
        }
    }
...
fn sparse_checkpoint_data_for_backfill(
    ...
    for ((tx, fx), ev) in transactions.into_iter().zip(effects).zip(events) {
        let input_objects =
            rtd_types::storage::get_transaction_input_objects(authority_store, &fx)?;
        let output_objects =
            rtd_types::storage::get_transaction_output_objects(authority_store, &fx)?;
...
mod tests {
    ...
    async fn test_events_compaction_filter() {
        ...
        let stream_id = RtdAddress::random_for_testing_only();
        ...
        let end_key = EventIndexKey {
            stream_id: RtdAddress::random_for_testing_only(),
