...
use rtd_types::{
    ...
    error::{RtdError, RtdErrorKind, UserInputError},
    messages_checkpoint::{
...
struct MockAuthority {
    _name: AuthorityName,
    submit_responses: Arc<StdMutex<HashMap<TransactionDigest, Result<SubmitTxResult, RtdError>>>>,
...
impl MockAuthority {
    ...
    fn set_submit_response(
        ...
        response: Result<SubmitTxResult, RtdError>,
    ) {
...
impl AuthorityAPI for MockAuthority {
    async fn submit_transaction(
        ...
        let maybe_response = match raw_request.transactions.first() {
            Some(tx_bytes) => {
                let tx: Transaction =
                    bcs::from_bytes(tx_bytes).map_err(|e| RtdErrorKind::GenericAuthorityError {
    ...
    // Test 4: Transaction with errors in submission.
    {
        ...
        for (i, mock_authority) in mock_authorities.iter().enumerate() {
            if i < 2 {
                mock_authority.set_submit_response(
                    tx_digest,
                    Err(RtdErrorKind::ValidatorOverloadedRetryAfter {
                        retry_after_secs: 1,
                    }
                    .into()),
                );
