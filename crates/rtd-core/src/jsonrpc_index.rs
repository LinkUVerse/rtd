...
//! IndexStore supports creation of various ancillary indexes of state in RtdDataStore.
...
use rtd_json_rpc_types::{RtdObjectDataFilter, TransactionFilter};
...
type OwnerIndexKey = (RtdAddress, ObjectID);
...
impl CoinIndexKey2 {
    ...
    pub fn new(owner: RtdAddress, coin_type: String, balance: u64, object_id: ObjectID) -> Self {
        Self {
            owner,
            coin_type,
...
pub struct IndexStoreCaches {
    per_coin_type_balance: ShardedLruCache<(RtdAddress, TypeTag), RtdResult<TotalBalance>>,
    all_balances: ShardedLruCache<RtdAddress, RtdResult<Arc<HashMap<TypeTag, TotalBalance>>>>,
    pub locks: MutexTable<RtdAddress>,
}

#[derive(Default)]
pub struct IndexStoreCacheUpdates {
    _locks: Vec<OwnedMutexGuard<()>>,
    per_coin_type_balance_changes: Vec<((RtdAddress, TypeTag), RtdResult<TotalBalance>)>,
    all_balance_changes: Vec<(RtdAddress, RtdResult<Arc<AllBalance>>)>,
}
...
pub struct IndexStoreTables {
    ...
    /// Index from rtd address to transactions initiated by that address.
    transactions_from_addr: DBMap<(RtdAddress, TxSequenceNumber), TransactionDigest>,

    /// Index from rtd address to transactions that were sent to that address.
    transactions_to_addr: DBMap<(RtdAddress, TxSequenceNumber), TransactionDigest>,
    ...
    /// composite key of the RtdAddress of their owner and the object ID of the object.
    ...
    event_by_sender: DBMap<(RtdAddress, EventId), EventIndex>,
...
impl IndexStore {
        ...
        let table_options = DBMapTableConfigMap::new(BTreeMap::from([
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    |(_, id): (RtdAddress, TxSequenceNumber)| id,
                ),
            ),
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, sequence_number): (RtdAddress, TxSequenceNumber)| sequence_number,
                ),
            ),
            (
            ...
            (
                ...
                compaction_filter_config_by_key(
                    ...
                    db_options.clone(),
                    pruner_watermark.clone(),
                    |(_, event_id): (RtdAddress, EventId)| event_id.0,
                ),
            ),
            (
    ...
    pub fn index_coin(
    ...
    ) -> RtdResult<IndexStoreCacheUpdates> {
        ...
        let mut addresses: HashSet<RtdAddress> = HashSet::new();
        addresses.extend(
        ...
        let mut balance_changes: HashMap<RtdAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
        // Index coin info
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, RtdError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, RtdError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn get_transactions(
        ...
        let cursor = if let Some(cursor) = cursor {
            Some(
                self.get_transaction_seq(&cursor)?
                    .ok_or(RtdErrorKind::TransactionNotFound { digest: cursor })?,
            )
        } else {
            None
        };
        match filter {
            ...
            Some(_) => Err(RtdErrorKind::UserInputError {
    ...
    pub fn get_transaction_seq(
        &self,
        digest: &TransactionDigest,
    ) -> RtdResult<Option<TxSequenceNumber>> {
    ...
    pub fn all_events(
        ...
        limit: usize,
        descending: bool,
    ) -> RtdResult<Vec<(TransactionEventsDigest, TransactionDigest, usize, u64)>> {
        Ok(if descending {
            self.tables
                .event_order
    ...
    pub fn events_by_transaction(
        ...
        let seq = self
            ...
            .ok_or(RtdErrorKind::TransactionNotFound { digest: *digest })?;
        Ok(if descending {
            self.tables
                .event_order
    ...
    pub fn prune(&self, cut_time_ms: u64) -> RtdResult<TxSequenceNumber> {
        match self
            .tables
            .event_by_time
            .reversed_safe_iter_with_bounds(
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> RtdResult {
    ...
    pub fn get_coin_object_balance(
        ...
        if force_disable_cache {
            ...
            .map_err(|e| {
                RtdErrorKind::ExecutionError(format!("Failed to read balance frm DB: {:?}", e))
                    .into()
            });
        }
    ...
    pub fn get_all_coin_object_balances(
        &self,
        owner: RtdAddress,
    ) -> RtdResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        let force_disable_cache = read_size_from_env(ENV_VAR_DISABLE_INDEX_CACHE).unwrap_or(0) > 0;
        let metrics_cloned = self.metrics.clone();
        let coin_index_cloned = self.tables.coin_index_2.clone();
        if force_disable_cache {
            return Self::get_all_balances_from_db(metrics_cloned, coin_index_cloned, owner)
                .map_err(|e| {
                    RtdErrorKind::ExecutionError(format!(
        ...
        self.caches.all_balances.get_with(owner, move || {
            Self::get_all_balances_from_db(metrics_cloned, coin_index_cloned, owner).map_err(|e| {
                RtdErrorKind::ExecutionError(format!("Failed to read all balance from DB: {:?}", e))
                    .into()
            })
        })
    }

    /// Read balance for a `RtdAddress` and `CoinType` from the backend database
    #[instrument(skip_all)]
    ...
    /// Read all balances for a `RtdAddress` from the backend database
    #[instrument(skip_all)]
    pub fn get_all_balances_from_db(
        ...
        for (coin_type, coins) in &coins {
            ...
            let coin_type =
                TypeTag::Struct(Box::new(parse_rtd_struct_tag(&coin_type).map_err(|e| {
                    RtdErrorKind::ExecutionError(format!(
    ...
    fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (RtdAddress, TypeTag)>,
    ) -> RtdResult {
    ...
    fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((RtdAddress, TypeTag), RtdResult<TotalBalance>)>,
    ) -> RtdResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &RtdResult<TotalBalance>,
        balance_delta: &RtdResult<TotalBalance>,
    ) -> RtdResult<TotalBalance> {
    ...
    fn update_all_balance_cache(
        &self,
        keys: impl IntoIterator<Item = (RtdAddress, RtdResult<Arc<HashMap<TypeTag, TotalBalance>>>)>,
    ) -> RtdResult {
        self.caches
            .all_balances
    ...
    fn merge_all_balance(
        old_balance: &RtdResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &RtdResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> RtdResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
...
mod tests {
    ...
    async fn test_index_cache() -> anyhow::Result<()> {
        ...
        let address: RtdAddress = AccountAddress::random().into();
