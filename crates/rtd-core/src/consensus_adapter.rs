...
use rtd_simulator::anemo::PeerId;
...
pub trait ConsensusOverloadChecker: Sync + Send + 'static {
    fn check_consensus_overload(&self) -> RtdResult;
}
...
pub trait ConsensusClient: Sync + Send + 'static {
    async fn submit(
        ...
        transactions: &[ConsensusTransaction],
        epoch_store: &Arc<AuthorityPerEpochStore>,
    ) -> RtdResult<(Vec<ConsensusPosition>, BlockStatusReceiver)>;
}

/// Submit Rtd certificates to the consensus.
            ...
            for transaction in &transactions[1..] {
                if is_user_tx_batch {
                    fp_ensure!(
                        ...
                        RtdErrorKind::InvalidTxKindInSoftBundle.into()
                    );
                    ...
                    return Err(RtdErrorKind::InvalidTxKindInSoftBundle.into());
                }
            }
        }
...
impl ConsensusOverloadChecker for ConsensusAdapter {
    fn check_consensus_overload(&self) -> RtdResult {
        fp_ensure!(
            self.check_limits(),
            RtdErrorKind::TooManyTransactionsPendingConsensus.into()
        );
        Ok(())
    }
}
    ...
    async fn test_await_submit_delay_user_transaction() {
        ...
        let consensus_adapter = ConsensusAdapter::new(
            ...
            *committee.authority_by_index(0).unwrap(),
            ...
            rtd_protocol_config::ProtocolConfig::get_for_max_version_UNSAFE(),
        );
