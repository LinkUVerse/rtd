...
impl<S: Clone + Eq, const STRENGTH: bool> StakeAggregator<S, STRENGTH> {
    ...
    pub fn insert_generic(
        ...
        match self.data.entry(authority) {
            Entry::Occupied(oc) => {
                return InsertResult::Failed {
                    error: RtdErrorKind::StakeAggregatorRepeatedSigner {
                        signer: authority,
        ...
        if votes > 0 {
            ...
            InsertResult::Failed {
                error: RtdErrorKind::InvalidAuthenticator.into(),
            }
        }
    }
...
mod stake_aggregator_tests {
    ...
    fn test_stake_aggregator_repeated_signer() {
        ...
        assert!(matches!(
            ...
            InsertResult::Failed {
                error
            } if matches!(error.as_inner(),  RtdErrorKind::StakeAggregatorRepeatedSigner { .. } )
        ));
        ...
        let RtdErrorKind::StakeAggregatorRepeatedSigner {
            signer,
            conflicting_sig,
        } = error.into_inner()
        else {
    ...
    fn test_stake_aggregator_from_iter_with_error() {
        ...
        let result: RtdResult<StakeAggregator<(), true>> =
