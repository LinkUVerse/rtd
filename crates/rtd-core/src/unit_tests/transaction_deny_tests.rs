...
use rtd_types::messages_grpc::HandleTransactionResponse;
...
use rtd_types::utils::get_zklogin_user_address;
use rtd_types::utils::{
...
async fn setup_test(deny_config: TransactionDenyConfig) -> (NetworkConfig, Arc<AuthorityState>) {
    let network_config =
        rtd_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir()
            .with_accounts(vec![
                AccountConfig {
                    address: None,
...
type Account = (RtdAddress, Ed25519KeyPair, Vec<ObjectRef>);
...
fn get_accounts_and_coins(
    ...
    let accounts: Vec<_> = network_config
        ...
        .iter()
        .map(|account| {
            let address: RtdAddress = account.public().into();
...
async fn transfer_with_account(
    ...
    let data = TransactionData::new_transfer_rtd_allow_sponsor(
        sender_account.0,
        sender_account.0,
        None,
        sponsor_account.2[0],
...
fn assert_denied<T: std::fmt::Debug>(result: &RtdResult<T>) {
    assert!(matches!(
...
async fn test_shared_object_transaction_disabled() {
    ...
    let tx = TestTransactionBuilder::new(account.0, account.2[0], gas_price)
        .call_staking(account.2[1], RtdAddress::default())
...
#[tokio::test]
async fn test_user_transaction_disabled_dynamic_check() {
    let program = include_str!("data/dynamic_checks/reject_all.star");
    let (network_config, state) = setup_test(
        TransactionDenyConfigBuilder::new()
...
async fn test_object_denied_dynamic_check() {
    ...
    let state = reload_state_with_new_deny_config(
        ...
        state,
        TransactionDenyConfigBuilder::new()
            .add_dynamic_transaction_checks(program)
            .unwrap()
            .build(),
    )
    .await;
...
#[tokio::test]
async fn test_shared_object_transaction_disabled_dynamic_check() {
    let program = include_str!("data/dynamic_checks/shared_object_disabled.star");
    let (network_config, state) = setup_test(
        TransactionDenyConfigBuilder::new()
            .add_dynamic_transaction_checks(program.to_string())
            .unwrap()
...
#[tokio::test]
async fn test_package_publish_disabled_dynamic_check() {
    let program = include_str!("data/dynamic_checks/publish_disabled.star");
    let (network_config, state) = setup_test(
        TransactionDenyConfigBuilder::new()
            .add_dynamic_transaction_checks(program.to_string())
            .unwrap()
