...
use rtd_json_rpc_types::{
    ...
    RtdTransactionBlockEffectsV1, RtdTypeTag,
};
...
use rtd_types::{
    MOVE_STDLIB_PACKAGE_ID, RTD_CLOCK_OBJECT_ID, RTD_FRAMEWORK_PACKAGE_ID,
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    builder.transfer_rtd(recipient, Some(amount));
...
async fn test_dev_inspect_gas_coin_argument() {
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_rtd(vec![recipient], vec![amount]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    ...
    assert_eq!(arg, &RtdArgument::GasCoin);
    check_coin_value(
        arg_value,
        arg_type,
...
fn check_coin_value(actual_value: &[u8], actual_type: &RtdTypeTag, expected_value: u64) {
...
async fn test_dry_run_dev_inspect_dynamic_field_too_new() {
    ...
    match effects {
        RtdTransactionBlockEffects::V1(RtdTransactionBlockEffectsV1 { abort_error, .. }) => {
...
#[tokio::test]
async fn test_handle_transfer_rtd_with_amount_insufficient_gas() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let authority_state = init_state_with_ids(vec![(sender, object_id)]).await;
...
async fn test_invalid_randomness_parameter() {
    ...
    assert_eq!(
        UserInputError::try_from(e).unwrap(),
        UserInputError::ImmutableParameterExpectedError {
            object_id: RTD_RANDOMNESS_STATE_OBJECT_ID
        }
    );
}
...
#[tokio::test]
async fn test_genesis_rtd_system_state_object() {
    // This test verifies that we can read the genesis RtdSystemState object.
    ...
    let _rtd_system_state =
        bcs::from_bytes::<RtdSystemStateWrapper>(move_object.contents()).unwrap();
    assert!(move_object.type_().is(&RtdSystemStateWrapper::type_()));
    let rtd_system_state = authority_state
...
#[tokio::test]
async fn test_transfer_rtd_no_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    let init_balance = rtd_types::gas::get_gas_balance(&gas_object).unwrap();
...
#[tokio::test]
async fn test_transfer_rtd_with_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
...
#[tokio::test]
async fn test_clear_cache_reverts_transfer_rtd() {
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (RtdAddress, Vec<DynamicFieldInfo>) {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let gas_object_id = ObjectID::random();
    let (authority_state, object_basics) =
        init_state_with_ids_and_object_basics(vec![(sender, gas_object_id)]).await;
...
pub async fn init_state_with_ids_and_object_basics<
    I: IntoIterator<Item = (RtdAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, ObjectRef) {
    let state = TestAuthorityBuilder::new().build().await;
...
async fn test_gas_smashing() {
    ...
    fn make_gas_coins(owner: RtdAddress, gas_amount: u64, coin_num: u64) -> Vec<Object> {
        let mut objects = vec![];
...
async fn test_publish_missing_dependency() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.publish_immutable(modules, vec![RTD_FRAMEWORK_PACKAGE_ID]);
    let kind = TransactionKind::programmable(builder.finish());
...
async fn test_publish_not_a_package_dependency() {
    ...
    deps.push(RTD_SYSTEM_STATE_OBJECT_ID);
    builder.publish_immutable(modules, deps);
    let kind = TransactionKind::programmable(builder.finish());
    ...
    assert_eq!(
        RtdErrorKind::UserInputError {
            error: UserInputError::MoveObjectAsPackage {
                object_id: RTD_SYSTEM_STATE_OBJECT_ID
            }
        },
        failure,
    )
...
pub fn create_gas_objects(num: u32, owner: RtdAddress) -> Vec<Object> {
    let mut objects = vec![];
    for _ in 0..num {
        let gas_object_id = ObjectID::random();
