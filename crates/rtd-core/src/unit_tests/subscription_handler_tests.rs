...
use rtd_types::{MOVE_STDLIB_ADDRESS, RTD_FRAMEWORK_ADDRESS};
...
fn test_to_json_value() {
    ...
    let rtd_move_struct: RtdMoveStruct =
        BoundedVisitor::deserialize_struct(&event_bytes, &TestEvent::layout())
            .unwrap()
            .into();
    let json_value = rtd_move_struct.to_json_value();
    assert_eq!(
        Some(&json!("1000000")),
...
// TODO: Do we need this in the rtd-types lib?
