...
use rtd_types::messages_grpc::HandleSoftBundleCertificatesRequestV3;
...
use rtd_types::transaction::TEST_ONLY_GAS_UNIT_FOR_OBJECT_BASICS;
...
use rtd_types::digests::ConsensusCommitDigest;
...
async fn test_handle_transfer_transaction_bad_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(err, RtdErrorKind::SignerSignatureAbsent { .. });
        },
    )
    .await;
}
...
async fn test_sender_is_not_consensus_v2_owner() {
    ...
    let err_check = |err: &RtdErrorKind| {
        assert_matches!(
            err,
            RtdErrorKind::UserInputError {
...
pub fn init_move_call_transaction(
    ...
    let mut data = TransactionData::new_move_call(
        ...
        RTD_SYSTEM_MODULE_NAME.into(),
        ident_str!("request_add_validator").to_owned(),
        vec![],
        gas_object_ref,
        vec![CallArg::RTD_SYSTEM_MUT, call_arg],
        gas_budget,
        gas_price,
    )
    .unwrap();
...
async fn do_transaction_test_skip_cert_checks(
    ...
    err_check: impl Fn(&RtdErrorKind),
) {
    let (sender1, sender_key1): (_, AccountKeyPair) = get_key_pair();
    let (sender2, sender_key2): (_, AccountKeyPair) = get_key_pair();
    do_transaction_test_impl(
...
async fn do_transaction_test_impl(
    ...
    senders: &[(RtdAddress, AccountKeyPair)],
    input_object: Object,
    pre_sign_mutations: impl Fn(&mut TransactionData),
    post_sign_mutations: impl Fn(&mut Transaction),
...
async fn test_zklogin_transfer_with_large_address_seed() {
    ...
    let sender = RtdAddress::generate(StdRng::from_seed([3; 32]));
    let recipient = dbg_addr(2);
...
async fn zklogin_test_caching_scenarios() {
    ...
    let zklogin_json_string =
        &get_one_zklogin_inputs("../rtd-types/src/unit_tests/zklogin_test_vectors.json");
    let bad_zklogin_inputs = ZkLoginInputs::from_json(zklogin_json_string, "111").unwrap();
    let sender = RtdAddress::try_from_unpadded(&bad_zklogin_inputs).unwrap();
...
async fn setup_zklogin_network(
    ...
    rtd_types::message_envelope::Envelope<SenderSignedData, rtd_types::crypto::EmptySignInfo>,
    ...
    let sender = RtdAddress::try_from_unpadded(zklogin).unwrap();
    ...
    let sender_2 = RtdAddress::from(&multisig_pk);
...
async fn zk_multisig_test() {
    ...
    .unwrap();
    let victim_addr = RtdAddress::from(&multisig_pk);
    ...
    for (kp, _pk_zklogin, inputs) in test_vectors {
        let intent_message = IntentMessage::new(Intent::rtd_transaction(), data.clone());
...
async fn test_very_large_certificate() {
    ...
    let quorum_signature = rtd_types::crypto::AuthorityQuorumSignInfo {
        epoch: 0,
        signature: rtd_types::crypto::AggregateAuthoritySignature::aggregate(&sigs)
            .map_err(|e| RtdErrorKind::InvalidSignature {
                error: e.to_string(),
            })
            .expect("Validator returned invalid signature"),
        signers_map,
    };
    let cert = rtd_types::message_envelope::Envelope::new_from_data_and_sig(
    ...
    assert!(
        matches!(err.as_inner(), RtdErrorKind::RpcError(..))
...
async fn test_handle_certificate_errors() {
    ...
    *data.tx_signatures_mut_for_testing() =
    ...
    assert_matches!(err.into_inner(), RtdErrorKind::SignerSignatureAbsent { .. });
}
...
async fn test_handle_soft_bundle_certificates_errors() {
    ...
    println!("Case 0: submit an empty soft bundle.");
    {
        ...
        assert_matches!(
            ...
            RtdErrorKind::NoCertificateProvidedError
        );
    }
...
fn sender_signed_data_serialized_intent() {
    ...
    assert_eq!(txn.intent_message().intent, Intent::rtd_transaction());
...
async fn test_shared_object_v2_denied() {
    ...
    // Test 2: Transaction with NonExclusiveWrite should be rejected during validation
    {
        ...
        if let Err(e) = result {
            ...
            match e.as_inner() {
                RtdErrorKind::UserInputError { error } => match error {
                ...
                _ => panic!("Expected RtdError::UserInputError, got: {:?}", e),
            }
        }
    }
