...
use rtd_types::message_envelope::Message as _;
...
async fn test_submit_transaction_gas_object_validation() {
    ...
    assert!(
        matches!(result, SubmitTxResult::Rejected { error } if matches!(error.as_inner(), RtdErrorKind::UserInputError {
...
async fn test_submit_batched_transactions_with_already_executed() {
    ...
    match &raw_response.results[0].inner {
        Some(rtd_types::messages_grpc::RawValidatorSubmitStatus::Executed(_)) => {
