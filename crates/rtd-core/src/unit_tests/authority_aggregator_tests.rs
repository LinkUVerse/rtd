...
use crate::test_utils::make_transfer_rtd_transaction;
...
use std::sync::Arc;
use rtd_authority_aggregation::quorum_map_then_reduce_with_timeout;
use rtd_macros::sim_test;
use rtd_move_build::BuildConfig;
use rtd_types::crypto::get_key_pair_from_rng;
use rtd_types::crypto::{AccountKeyPair, AuthorityKeyPair, get_key_pair};
use rtd_types::crypto::{AuthoritySignature, Signer};
...
async fn test_map_reducer() {
    ...
    let res = quorum_map_then_reduce_with_timeout::<_, _, _, _, _, (), _, _, _>(
        ...
        |_name, _client| {
            Box::pin(async move {
                let res: Result<usize, RtdError> = Err(RtdErrorKind::TooManyIncorrectAuthorities {
                    errors: vec![],
                    action: "".to_string(),
                }
                .into());
        ...
        |mut accumulated_state, _authority_name, _authority_weight, result| {
            Box::pin(async move {
                assert!(matches!(
                    ...
                    Err(RtdErrorKind::TooManyIncorrectAuthorities { .. })
                ));
                accumulated_state += 1;
...
async fn test_handle_transaction_fork() {
    ...
    let tx = make_transfer_rtd_transaction(
        gas_object,
        RtdAddress::default(),
        None,
        sender,
...
async fn test_handle_certificate_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let tx = VerifiedTransaction::new_unchecked(make_transfer_rtd_transaction(
        gas_object,
        RtdAddress::default(),
        None,
        sender,
    ...
    assert_resp_err(&agg, tx.clone().into(), |e| matches!(e, AggregatorProcessTransactionError::RetryableTransaction { .. }),
        |e| matches!(e, RtdErrorKind::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 1 && *actual_epoch == 0)
    ).await;
    ...
    assert_matches!(
        ...
        AggregatorProcessCertificateError::RetryableExecuteCertificate {
            retryable_errors, ..
        } if retryable_errors.iter().any(|(error, _, _)| matches!(error.as_inner(), RtdErrorKind::WrongEpoch {
...
async fn test_handle_transaction_response() {
    ...
    let tx2 = VerifiedTransaction::new_unchecked(make_transfer_rtd_transaction(
        gas_object,
        RtdAddress::default(),
        Some(1),
        sender,
    ...
    let package_not_found_error: RtdError = RtdErrorKind::UserInputError {
        error: UserInputError::DependentPackageNotFound {
            package_id: gas_object.0,
        },
    }
    .into();
    let object_not_found_error: RtdError = RtdErrorKind::UserInputError {
    ...
    assert_resp_err(
        ...
        |e| matches!(e, RtdErrorKind::Unknown(..)),
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, RtdErrorKind::MissingCommitteeAtEpoch(e) if *e == 1),
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                RtdErrorKind::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. }
    ...
    assert_resp_err(
        ...
        |e| matches!(e, RtdErrorKind::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 0 && *actual_epoch == 1)
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                RtdErrorKind::UserInputError { .. } | RtdErrorKind::RpcError(..)
            )
        },
    )
    .await;
    ...
    for (name, _) in authority_keys.iter().skip(2) {
        clients
            .get_mut(name)
            .unwrap()
            .set_tx_info_response_error(RtdErrorKind::EpochEnded(0).into());
    }
    let agg = get_genesis_agg(authorities.clone(), clients.clone());
    assert_resp_err(
        &agg,
        ...
        |e| matches!(e, RtdErrorKind::EpochEnded(0)),
    )
    .await;
...
async fn test_handle_conflicting_transaction_response() {
    ...
    let tx1 = VerifiedTransaction::new_unchecked(make_transfer_rtd_transaction(
        conflicting_object,
        RtdAddress::default(),
        Some(1),
        sender,
    ...
    let conflicting_tx2 = VerifiedTransaction::new_unchecked(make_transfer_rtd_transaction(
        conflicting_object,
        RtdAddress::default(),
        Some(2),
        sender,
    ...
    let conflicting_error: RtdError = RtdErrorKind::ObjectLockConflict {
        obj_ref: conflicting_object,
        pending_transaction: *conflicting_tx2.digest(),
    }
    .into();
    let retryable_error: RtdError = RtdErrorKind::RpcError("RPC".into(), "Error".into()).into();
    let non_retryable_error: RtdError = RtdErrorKind::ByzantineAuthoritySuspicion {
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                RtdErrorKind::ObjectLockConflict { .. } | RtdErrorKind::RpcError(..)
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, RtdErrorKind::ObjectLockConflict { .. }),
    )
    .await;
    ...
    let conflicting_tx3 = make_transfer_rtd_transaction(
        conflicting_object,
        RtdAddress::default(),
        Some(3),
        sender,
    ...
    let conflicting_error_2: RtdError = RtdErrorKind::ObjectLockConflict {
        obj_ref: conflicting_object,
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                RtdErrorKind::ObjectLockConflict { .. } | RtdErrorKind::UserInputError { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                RtdErrorKind::MissingCommitteeAtEpoch(..) | RtdErrorKind::ObjectLockConflict { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                RtdErrorKind::WrongEpoch { .. } | RtdErrorKind::ObjectLockConflict { .. }
            )
        },
    )
    .await;
...
async fn test_handle_overload_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let txn = make_transfer_rtd_transaction(
        gas_object,
        RtdAddress::default(),
        None,
        sender,
    ...
    let overload_error = RtdErrorKind::TooManyTransactionsPendingExecution {
        queue_len: 100,
        threshold: 100,
    };
    let rpc_error: RtdError = RtdErrorKind::RpcError("RPC".into(), "Error".into()).into();
...
// Tests that authority aggregator can aggregate RtdErrorKind::ValidatorOverloadedRetryAfter into
...
async fn test_handle_overload_retry_response() {
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                RtdErrorKind::ValidatorOverloadedRetryAfter { .. } | RtdErrorKind::RpcError(..)
            )
        },
    )
    .await;
    ...
    for (name, secret) in &authority_keys {
        let auth_signature = if byzantines.contains(name) {
            ...
            AuthoritySignInfo::new(
                0,
                tx.clone().data(),
                Intent::rtd_app(IntentScope::ProofOfPossession), // bad intent
                *name,
                secret,
            )
        } else {
    ...
    match agg.process_transaction(tx, Some(make_socket_addr())).await {
        Err(received_agg_err) if agg_err_checker(&received_agg_err) => match received_agg_err {
            ...
            AggregatorProcessTransactionError::FatalConflictingTransaction {
                ...
                assert!(errors.iter().map(|e| e.0.as_inner()).all(rtd_err_checker));
            }
...
fn set_cert_response_with_certified_tx(
    ...
    let effects = effects_with_tx(*cert.digest());
    for (name, secret) in authority_keys {
        let resp = rtd_types::messages_grpc::HandleCertificateResponseV2 {
...
fn set_retryable_tx_info_response_error(
    ...
    let error = RtdErrorKind::RpcError("RPC".into(), "Error".into());
