...
use rtd_types::messages_checkpoint::EndOfEpochData;
...
impl ReadStore for RocksDbStore {
            ...
            .and_then(|contents| {
                ...
                for tx in contents.iter() {
                    if let (Some(t), Some(e)) = (
                        ...
                        transactions.push(rtd_types::base_types::ExecutionData::new(
                            (*t).clone().into_inner(),
                            e,
                        ))
                    } else {
    ...
    fn get_latest_checkpoint(&self) -> rtd_types::storage::error::Result<VerifiedCheckpoint> {
        self.checkpoint_store
            ...
            .expect("db error")
            .ok_or_else(|| {
                rtd_types::storage::error::Error::missing("unable to get latest checkpoint")
            })
    }
...
impl ObjectStore for RocksDbStore {
    fn get_object(&self, object_id: &rtd_types::base_types::ObjectID) -> Option<Object> {
        self.cache_traits.object_store.get_object(object_id)
    }

    fn get_object_by_key(
        &self,
        object_id: &rtd_types::base_types::ObjectID,
...
impl WriteStore for RocksDbStore {
    ...
    ) -> Result<(), rtd_types::storage::error::Error> {
