...
use rtd_types::messages_grpc::{HandleCertificateRequestV3, TxType};
...
use linku_metrics::{GaugeGuard, spawn_monitored_task};
use std::fmt::Write;
use rtd_macros::fail_point;
use rtd_types::error::{RtdErrorKind, RtdResult};
use rtd_types::transaction::{CertifiedTransaction, Transaction};
...
impl<A: Clone> QuorumDriver<A> {
    ...
    async fn enqueue_task(&self, task: QuorumDriverTask) -> RtdResult<()> {
        self.task_sender
            .send(task.clone())
            .await
            ...
            .map_err(|e| {
                RtdErrorKind::QuorumDriverCommunicationError {
                    error: e.to_string(),
                }
                .into()
            })
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn submit_transaction(
        &self,
        request: ExecuteTransactionRequestV3,
    ) -> RtdResult<Registration<'_, TransactionDigest, QuorumDriverResult>> {
