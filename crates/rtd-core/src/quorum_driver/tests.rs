...
use linku_common::sync::notify_read::{NotifyRead, Registration};
use std::net::SocketAddr;
use std::sync::Arc;
use std::sync::atomic::{AtomicUsize, Ordering};
use std::time::Duration;
use rtd_macros::{register_fail_point, sim_test};
use rtd_types::base_types::RtdAddress;
use rtd_types::base_types::TransactionDigest;
use rtd_types::crypto::{AccountKeyPair, deterministic_random_account_key, get_key_pair};
use rtd_types::effects::TransactionEffectsAPI;
use rtd_types::object::{Object, generate_test_gas_objects};
...
async fn test_quorum_driver_object_locked() -> Result<(), anyhow::Error> {
    ...
    let (sender, keypair): (RtdAddress, AccountKeyPair) = deterministic_random_account_key();
