...
impl RtdTxValidator {
        ...
        for tx in txs.iter() {
            match tx {
                ...
                ConsensusTransactionKind::CheckpointSignatureV2(signature) => {
                    ...
                    {
                        return Err(RtdErrorKind::UnexpectedMessage(
                ...
                ConsensusTransactionKind::RandomnessDkgMessage(_, bytes) => {
                    if bytes.len() > dkg_v1::DKG_MESSAGES_MAX_SIZE {
                        ...
                        return Err(RtdErrorKind::InvalidDkgMessageSize.into());
                    }
                }
                ...
                ConsensusTransactionKind::ExecutionTimeObservation(obs) => {
                    ...
                    if obs.estimates.len()
                        > epoch_store
                            ...
                            .unwrap()
                    {
                        return Err(RtdErrorKind::UnexpectedMessage(format!(
    ...
    fn vote_transaction(
        ...
        if *verified_tx.aliases() != aliases || fail_point_always_report_aliases_changed {
            return Err(RtdErrorKind::AliasesChanged.into());
        }
...
impl TransactionVerifier for RtdTxValidator {
...
pub struct RtdTxValidatorMetrics {
...
mod tests {
    ...
    use rtd_config::transaction_deny_config::TransactionDenyConfigBuilder;
    use rtd_macros::sim_test;
    use rtd_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};
    use rtd_types::crypto::deterministic_random_account_key;
    ...
    use crate::{
        ...
        consensus_validator::{RtdTxValidator, RtdTxValidatorMetrics},
    };
        ...
        let bogus_transaction_bytes: Vec<_> = certificates
            ...
            .map(|mut cert| {
                ...
                cert.tx_signatures_mut_for_testing()[0] =
                    GenericSignature::Signature(rtd_types::crypto::Signature::Ed25519RtdSignature(
                        Ed25519RtdSignature::default(),
                    ));
                bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&name1, cert)).unwrap()
            })
            .collect();
