...
use linku_metrics::LATENCY_SEC_BUCKETS;
use prometheus::{
...
use rtd_types::base_types::{AuthorityName, TransactionDigest};
use rtd_types::transaction::VerifiedSignedTransaction;
use tokio::select;
...
mod tests {
    ...
    use rtd_types::base_types::{AuthorityName, ObjectID, RtdAddress, TransactionDigest};
    use rtd_types::committee::{CommitteeTrait, StakeUnit};
    use rtd_types::crypto::{AccountKeyPair, get_account_key_pair};
    use rtd_types::effects::{TransactionEffectsAPI, TransactionEvents};
    ...
    impl AuthorityAPI for MockAuthorityClient {
        ...
        async fn handle_transaction(
            ...
            if self.inject_fault.load(Relaxed) {
                return Err(RtdErrorKind::TimeoutError.into());
            }
