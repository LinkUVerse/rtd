...
use rtd_types::{
    RTD_ACCUMULATOR_ROOT_OBJECT_ID, TypeTag,
...
impl TestEnv {
    ...
    pub async fn fund_address(&self, address: RtdAddress, amount: u64) {
...
async fn test_object_withdraw_basic_flow() {
    ...
    let accumulator_version = env.oref(&RTD_ACCUMULATOR_ROOT_OBJECT_ID).await.1;
    let effects = env
        .authority
