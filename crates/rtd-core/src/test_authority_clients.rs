...
impl AuthorityAPI for LocalAuthorityClient {
    async fn submit_transaction(
    ...
    ) -> Result<SubmitTxResponse, RtdError> {
        if self.fault_config.fail_before_submit_transaction {
            return Err(RtdError::from("Mock error before submit_transaction"));
        }
        ...
        let deserialized_transaction = bcs::from_bytes::<Transaction>(&raw_request.transactions[0])
            .map_err(|e| RtdErrorKind::TransactionDeserializationError {
                error: e.to_string(),
            })?;
    ...
    async fn handle_transaction(
        ...
        if self.fault_config.fail_before_handle_transaction {
            return Err(RtdError::from("Mock error before handle_transaction"));
        }
        let state = self.state.clone();
        let epoch_store = self.state.load_epoch_store_one_call_per_task();
        let transaction = epoch_store
    ...
    ) -> Result<ObjectInfoResponse, RtdError> {
        let state = self.state.clone();
    ...
    ) -> Result<TransactionInfoResponse, RtdError> {
        let state = self.state.clone();
    ...
    ) -> Result<CheckpointResponse, RtdError> {
        let state = self.state.clone();
    ...
    ) -> Result<CheckpointResponseV2, RtdError> {
        let state = self.state.clone();
    ...
    ) -> Result<RtdSystemState, RtdError> {
        self.state.get_rtd_system_state_object_for_testing()
    }

    async fn validator_health(
        &self,
        _request: ValidatorHealthRequest,
    ) -> Result<ValidatorHealthResponse, RtdError> {
...
pub struct MockAuthorityApi {
    delay: Duration,
    count: Arc<Mutex<u32>>,
    handle_object_info_request_result: Option<RtdResult<ObjectInfoResponse>>,
}
...
impl MockAuthorityApi {
    ...
    pub fn set_handle_object_info_request(&mut self, result: RtdResult<ObjectInfoResponse>) {
...
impl AuthorityAPI for MockAuthorityApi {
    /// Submit a new transaction to a Rtd or Primary account.
    async fn submit_transaction(
        &self,
    ...
    /// Initiate a new transaction to a Rtd or Primary account.
    async fn handle_transaction(
        &self,
    ...
    async fn handle_transaction_info_request(
        ...
        Err(RtdErrorKind::TransactionNotFound {
...
pub struct HandleTransactionTestAuthorityClient {
    pub tx_info_resp_to_return: RtdResult<HandleTransactionResponse>,
    pub cert_resp_to_return: RtdResult<HandleCertificateResponseV2>,
...
impl HandleTransactionTestAuthorityClient {
    pub fn new() -> Self {
        Self {
            tx_info_resp_to_return: Err(RtdErrorKind::Unknown("".to_string()).into()),
            cert_resp_to_return: Err(RtdErrorKind::Unknown("".to_string()).into()),
    ...
    pub fn set_tx_info_response_error(&mut self, error: RtdError) {
        self.tx_info_resp_to_return = Err(error);
    }

    pub fn reset_tx_info_response(&mut self) {
        self.tx_info_resp_to_return = Err(RtdErrorKind::Unknown("".to_string()).into());
    }
    ...
    pub fn set_cert_resp_to_return_error(&mut self, error: impl Into<RtdError>) {
        self.cert_resp_to_return = Err(error.into());
    }

    pub fn reset_cert_response(&mut self) {
        self.cert_resp_to_return = Err(RtdErrorKind::Unknown("".to_string()).into());
    }
