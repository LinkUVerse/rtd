...
use rtd_types::full_checkpoint_content::{
...
pub(crate) fn store_checkpoint_locally(
    ...
    std::fs::create_dir_all(path).map_err(|err| {
        RtdErrorKind::FileIOError(format!(
...
pub(crate) fn load_checkpoint(
    ...
    let mut events = transaction_cache_reader
        ...
        .map(|(maybe_event, tx_digest)| {
            maybe_event
                .ok_or(RtdErrorKind::TransactionEventsNotFound { digest: tx_digest }.into())
                .map(|event| (tx_digest, event))
        })
        .collect::<RtdResult<HashMap<_, _>>>()?;
    ...
    let object_set = {
        let refs = transactions
            .iter()
            .flat_map(|tx| {
                rtd_types::storage::get_transaction_object_set(
                    &tx.transaction,
                    &tx.effects,
        ...
        for (idx, object) in objects.into_iter().enumerate() {
            object_set.insert(object.ok_or_else(|| {
                rtd_types::storage::error::Error::custom(format!(
