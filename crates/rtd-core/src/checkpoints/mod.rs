...
use linku_common::{assert_reachable, debug_fatal, fatal};
use linku_metrics::{MonitoredFutureExt, monitored_scope, spawn_monitored_task};
...
use rtd_types::base_types::{ConciseableName, SequenceNumber};
...
impl CheckpointStore {
    ...
    pub fn new_for_tests() -> Arc<Self> {
        let ckpt_dir = linku_common::tempdir().unwrap();
    ...
    fn prune_local_summaries(&self) -> RtdResult {
    ...
    pub fn reset_db_for_execution_since_genesis(&self) -> RtdResult {
...
impl<RtdError: std::error::Error + Send + Sync + 'static> From<RtdError>
...
impl CheckpointBuilder {
    ...
    async fn construct_and_execute_settlement_transactions(
        ...
        for fx in settlement_effects.iter() {
            ...
            if let Some(version) = fx
                .mutated()
                .iter()
                .find_map(|(oref, _)| (oref.0 == RTD_ACCUMULATOR_ROOT_OBJECT_ID).then_some(oref.1))
            {
                assert!(
    ...
    ) -> RtdResult<(Vec<TransactionEffects>, HashSet<TransactionDigest>)> {
    ...
    ) -> RtdResult<Option<(TransactionDigest, TransactionEffects)>> {
    ...
    ) -> CheckpointBuilderResult<RtdSystemState> {
    ...
    ) -> RtdResult<Vec<TransactionEffects>> {
...
impl CheckpointAggregator {
    ...
    async fn run_and_notify(&mut self) -> RtdResult {
    ...
    fn run_inner(&mut self) -> RtdResult<Vec<CertifiedCheckpointSummary>> {
    ...
    fn next_checkpoint_to_certify(&self) -> RtdResult<CheckpointSequenceNumber> {
        Ok(self
            .store
            .tables
            .certified_checkpoints
    ...
    pub fn try_aggregate(
        ...
        match self.signatures_by_digest.insert(their_digest, envelope) {
            ...
            InsertResult::Failed { error }
                if matches!(
                    error.as_inner(),
                    RtdErrorKind::StakeAggregatorRepeatedSigner {
...
async fn diagnose_split_brain(
    ...
    let committee = epoch_store
        .epoch_start_state()
        .get_rtd_committee_with_network_metadata();
    let network_config = default_linku_network_config();
    let network_clients =
...
pub trait CheckpointServiceNotify {
    ...
    fn notify_checkpoint(&self) -> RtdResult;
}
...
mod tests {
    ...
    use rtd_types::base_types::{SequenceNumber, TransactionEffectsDigest};
    ...
    async fn test_clear_locally_computed_checkpoints_from_deletes_inclusive_range() {
        ...
        let protocol = rtd_protocol_config::ProtocolConfig::get_for_max_version_UNSAFE();
        for seq in 70u64..=80u64 {
            let contents =
                rtd_types::messages_checkpoint::CheckpointContents::new_with_digests_only_for_tests(
                    [rtd_types::base_types::ExecutionDigests::new(
                        rtd_types::digests::TransactionDigest::random(),
                        rtd_types::digests::TransactionEffectsDigest::ZERO,
                    )],
                );
            let summary = rtd_types::messages_checkpoint::CheckpointSummary::new(
                &protocol,
                0,
                seq,
                0,
                &contents,
                None,
                rtd_types::gas::GasCostSummary::default(),
                None,
                0,
                Vec::new(),
                Vec::new(),
    ...
    pub async fn checkpoint_builder_test() {
        ...
        for digest in all_digests {
            let signature = Signature::Ed25519RtdSignature(Default::default()).into();
            state
