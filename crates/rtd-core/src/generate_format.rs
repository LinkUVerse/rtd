...
use rtd_types::effects::{AccumulatorOperation, AccumulatorValue, TransactionEvents};
...
use rtd_types::messages_checkpoint::{CertifiedCheckpointSummary, CheckpointCommitment};
use rtd_types::messages_consensus::ConsensusDeterminedVersionAssignments;
use rtd_types::messages_grpc::ObjectInfoRequestKind;
...
use rtd_types::{
    ...
    crypto::{
        ...
        KeypairTraits, Signature, RtdKeyPair, get_key_pair, get_key_pair_from_rng,
    },
...
fn get_registry() -> Result<Registry> {
    ...
    let kp1: RtdKeyPair =
        RtdKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: RtdKeyPair =
        RtdKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: RtdKeyPair =
        RtdKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    // ObjectID and RtdAddress are the same length
    ...
    let struct_tag = StructTag::from_str("0x2::coin::Coin<0x2::rtd::RTD>").unwrap();
    ...
    tracer
        .trace_type::<base_types::RtdAddress>(&samples)
        .unwrap();
...
#[derive(Debug, Parser)]
#[clap(
    name = "Rtd format generator",
    about = "Trace serde (de)serialization to generate format descriptions for Rtd types"
)]
struct Options {
...
const FILE_PATH: &str = "rtd-core/tests/staged/rtd.yaml";
