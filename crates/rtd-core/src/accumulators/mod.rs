...
use rtd_types::{
    RTD_ACCUMULATOR_ROOT_OBJECT_ID, RTD_FRAMEWORK_ADDRESS, RTD_FRAMEWORK_PACKAGE_ID, TypeTag,
};
...
impl ClassifiedType {
    fn classify(ty: &TypeTag) -> Self {
        ...
        if struct_tag.address == RTD_FRAMEWORK_ADDRESS
        ...
        match (ty, merge, split) {
            ...
            (_, MergedValue::EventDigest(digest, event_count), MergedValue::EventDigest(_, _)) => {
                ...
                builder.programmable_move_call(
                    RTD_FRAMEWORK_PACKAGE_ID,
                    ACCUMULATOR_SETTLEMENT_MODULE.into(),
                    rtd_types::accumulator_root::ACCUMULATOR_ROOT_SETTLEMENT_SETTLE_EVENTS_FUNC
                        .into(),
                    vec![],
                    args,
                );
...
struct Update {
    ...
    // Track input and output RTD for each update. Necessary so that when we construct
    // a settlement transaction from a collection of Updates, they can accurately
    // track the net RTD flows.
    input_rtd: u64,
    output_rtd: u64,
}
...
impl AccumulatorSettlementTxBuilder {
    pub fn new(
        ...
        for (tx_index, effect) in ckpt_effects.iter().enumerate() {
            ...
            for event in events {
                ...
                let (event_input_rtd, event_output_rtd) = event.total_rtd_in_event();
                ...
                entry.input_rtd += event_output_rtd;
                entry.output_rtd += event_input_rtd;
            ...
            let (total_input_rtd, total_output_rtd) =
                updates
                    ...
                    .fold((0, 0), |(acc_input, acc_output), (_, update)| {
                        (acc_input + update.input_rtd, acc_output + update.output_rtd)
                    });
    ...
    fn add_prologue(
        ...
        let total_input_rtd_arg = builder.pure(total_input_rtd).unwrap();
        let total_output_rtd_arg = builder.pure(total_output_rtd).unwrap();
    ...
    fn build_one_settlement_txn(
        ...
        let root = builder
            .input(CallArg::Object(ObjectArg::SharedObject {
                id: RTD_ACCUMULATOR_ROOT_OBJECT_ID,
