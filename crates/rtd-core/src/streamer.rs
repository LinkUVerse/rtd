...
use futures::Stream;
use linku_metrics::metered_channel::Sender;
    ...
    F: Filter<T> + Clone + Send + Sync + 'static + Clone,
{
    pub fn spawn(
        ...
        let gauge = if let Some(metrics) = linku_metrics::get_metrics() {
            metrics
                .channel_inflight
            ...
            linku_metrics::init_metrics(&Registry::default());
        ...
        let (tx, rx) = linku_metrics::metered_channel::channel(buffer, &gauge);
        let streamer = Self {
            streamer_queue: tx,
    ...
    pub fn try_send(&self, data: T) -> Result<(), RtdError> {
        self.streamer_queue.try_send(data).map_err(|e| {
            ...
            RtdErrorKind::FailedToDispatchSubscription {
                error: e.to_string(),
            }
            .into()
        })
