...
use rtd_types::{base_types::*, committee::*, fp_ensure};
use rtd_types::{
...
impl<C: Clone> SafeClient<C> {
    ...
    fn get_committee(&self, epoch_id: &EpochId) -> RtdResult<Arc<Committee>> {
        self.committee_store
            .get_committee(epoch_id)?
            .ok_or(RtdErrorKind::MissingCommitteeAtEpoch(*epoch_id).into())
    }
    ...
    ) -> RtdResult<SignedTransactionEffects> {
    ...
    ) -> RtdResult<PlainTransactionInfoResponse> {
        fp_ensure!(
    ...
    ) -> RtdResult<VerifiedObjectInfoResponse> {
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
        ...
        match (events, events_digest) {
            ...
            (Some(events), None) => {
                if !events.data.is_empty() {
                    Err(RtdErrorKind::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    fn verify_objects<I>(&self, objects: &Option<Vec<Object>>, expected_refs: I) -> RtdResult
    where
        I: IntoIterator<Item = (ObjectID, ObjectRef)>,
    {
        if let Some(objects) = objects {
            ...
            for object in objects {
                ...
                {
                    return Err(RtdErrorKind::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    pub async fn handle_object_info_request(
        &self,
        request: ObjectInfoRequest,
    ) -> Result<VerifiedObjectInfoResponse, RtdError> {
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                RtdError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                RtdError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
    ...
    #[instrument(level = "trace", skip_all, fields(authority = ?self.address.concise()))]
    pub async fn handle_system_state_object(&self) -> Result<RtdSystemState, RtdError> {
        self.authority_client
