...
pub struct AuthorityServerHandle {
    server_handle: rtd_network::validator::server::Server,
}
...
impl AuthorityServer {
    ...
    pub async fn spawn_with_bind_address_for_test(
        ...
        let config = linku_network::config::Config::new();
        let server = rtd_network::validator::server::ServerBuilder::from_config(
            &config,
            linku_network::metrics::DefaultMetricsCallbackProvider::default(),
        )
    ...
    pub fn new(registry: &Registry) -> Self {
        Self {
            ...
            handle_submit_transaction_latency: register_histogram_vec_with_registry!(
                ...
                &["req_type"],
                linku_metrics::LATENCY_SEC_BUCKETS.to_vec(),
                registry,
            )
            .unwrap(),
...
impl ValidatorService {
    ...
    async fn handle_submit_transaction(
        ...
        loop {
            ...
            match res {
                ...
                Err(err) => {
                    if max_retries > 0
                        && let RtdErrorKind::ValidatorHaltedAtEpochEnd = err.as_inner()
                    {
                        max_retries -= 1;
    ...
    async fn handle_submit_transaction_inner(
        ...
        if is_ping_request {
            fp_ensure!(
                ...
                RtdErrorKind::InvalidRequest(format!(
        ...
        for (idx, tx_bytes) in request.transactions.iter().enumerate() {
            let transaction = match bcs::from_bytes::<Transaction>(tx_bytes) {
                ...
                Err(e) => {
                    ...
                    return Err(RtdErrorKind::TransactionDeserializationError {
    ...
    ) -> Result<RawSubmitTxResponse, RtdError> {
        let mut raw_results = Vec::new();
        for (i, result) in results.into_iter().enumerate() {
            let result = result.ok_or_else(|| RtdErrorKind::GenericAuthorityError {
...
impl ValidatorService {
    ...
    async fn wait_for_effects_impl(
        ...
        let response = timeout(
            ...
            epoch_store
                ...
                .map_err(|_| RtdErrorKind::EpochEnded(epoch_store.epoch())),
        )
        .await
    ...
    async fn wait_for_effects_response(
        ...
        if request.ping_type.is_some() {
            ...
            .await
            .map_err(|_| RtdErrorKind::TimeoutError)?;
        }
    ...
    ) -> WrappedServiceResponse<RtdSystemState> {
        let response = self
            .state
    ...
    async fn validator_health_impl(
        &self,
        _request: tonic::Request<rtd_types::messages_grpc::RawValidatorHealthRequest>,
    ) -> WrappedServiceResponse<rtd_types::messages_grpc::RawValidatorHealthResponse> {
        ...
        let typed_response = rtd_types::messages_grpc::ValidatorHealthResponse {
        ...
        let raw_response = typed_response
            .try_into()
            .map_err(|e: rtd_types::error::RtdError| {
        ...
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                ...
                Err(tonic::Status::from_error(
                    RtdErrorKind::TooManyRequests.into(),
                ))
            } else {
                Ok(())
            }
    ...
    fn handle_traffic_resp<T>(
        ...
        let (error, spam_weight, unwrapped_response) = match wrapped_response {
            ...
            Err(status) => (
                Some(RtdError::from(status.clone())),
                Weight::zero(),
                Err(status.clone()),
            ),
        };
...
fn normalize(err: RtdError) -> Weight {
    match err.as_inner() {
        RtdErrorKind::UserInputError {
        ...
        | RtdErrorKind::WrongEpoch { .. } => Weight::one(),
        _ => Weight::zero(),
    }
}
