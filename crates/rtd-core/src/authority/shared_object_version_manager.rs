...
use rtd_types::executable_transaction::VerifiedExecutableTransactionWithAliases;
...
impl SharedObjVerManager {
    ...
    ) -> RtdResult<ConsensusSharedObjVerAssignment>
    where
        T: AsTx + 'a,
    {
            ...
            } in shared_input_objects.iter()
            {
                let assigned_version = match cancellation_info {
                    ...
                    Some(CancelConsensusCertificateReason::DkgFailed) => {
                        if id == &RTD_RANDOMNESS_STATE_OBJECT_ID {
...
mod tests {
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(
            epoch_store
                .get_next_object_version(&RTD_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version)
                .unwrap(),
            randomness_obj_version
        );
        ...
        assert_eq!(
            *shared_input_next_versions
                .get(&(RTD_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version))
                .unwrap(),
    ...
    async fn test_assign_versions_from_consensus_with_cancellation() {
        ...
        // transaction.
        shared_input_next_versions
            .remove(&(RTD_ACCUMULATOR_ROOT_OBJECT_ID, SequenceNumber::from_u64(1)));
        assert_eq!(
            shared_input_next_versions,
            HashMap::from([
