...
/// A cache that maintains rejection reasons (RtdError) when validators cast reject votes for transactions
...
pub(crate) struct TransactionRejectReasonCache {
    cache: RwLock<BTreeMap<ConsensusPosition, RtdError>>,
...
impl TransactionRejectReasonCache {
    ...
    /// uniquely identifies the transaction and the `reason` (RtdError) that caused the transaction to be rejected during voting
    /// should be provided.
    pub fn set_rejection_vote_reason(&self, position: ConsensusPosition, reason: &RtdError) {
    ...
    pub fn get_rejection_vote_reason(&self, position: ConsensusPosition) -> Option<RtdError> {
        debug_assert_eq!(position.epoch, self.epoch, "Epoch mismatch");
...
mod test {
    ...
    async fn test_set_rejection_vote_reason_and_get_reason() {
        ...
        {
            let reason = RtdErrorKind::ValidatorHaltedAtEpochEnd.into();
        ...
        {
            let reason = RtdErrorKind::InvalidTransactionDigest.into();
            cache.set_rejection_vote_reason(position, &reason);
            assert_eq!(cache.get_rejection_vote_reason(position), Some(reason));
        }
        ...
        for round in 0..TOTAL_ROUNDS {
            for transaction_index in 0..5 {
                cache.set_rejection_vote_reason(
                    ...
                    &RtdErrorKind::InvalidTransactionDigest.into(),
                );
            }
        }
            ...
            for transaction_index in 0..5 {
                ...
                if round <= 2 {
                    ...
                    assert_eq!(
                        ...
                        Some(RtdErrorKind::InvalidTransactionDigest.into())
                    );
                }
            }
        }
