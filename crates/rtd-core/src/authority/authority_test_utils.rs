...
use rtd_types::crypto::{AccountKeyPair, AuthorityKeyPair};
...
pub async fn certify_transaction(
    authority: &AuthorityState,
    transaction: Transaction,
) -> Result<VerifiedCertificate, RtdError> {
...
pub async fn init_state_with_ids<I: IntoIterator<Item = (RtdAddress, ObjectID)>>(
    objects: I,
) -> Arc<AuthorityState> {
...
pub async fn init_state_with_ids_and_versions<
    I: IntoIterator<Item = (RtdAddress, ObjectID, SequenceNumber)>,
>(
    objects: I,
) -> Arc<AuthorityState> {
    let state = TestAuthorityBuilder::new().build().await;
...
pub async fn init_state_with_objects<I: IntoIterator<Item = Object>>(
    ...
    let network_config = rtd_swarm_config::network_config_builder::ConfigBuilder::new(&dir).build();
...
pub async fn certify_shared_obj_transaction_no_execution(
    authority: &AuthorityState,
    transaction: Transaction,
) -> Result<(VerifiedCertificate, AssignedVersions), RtdError> {
    let epoch_store = authority.load_epoch_store_one_call_per_task();
    let transaction = epoch_store
        .verify_transaction_require_no_aliases(transaction)
        .unwrap()
...
) -> Result<Vec<TransactionEffects>, RtdError> {
