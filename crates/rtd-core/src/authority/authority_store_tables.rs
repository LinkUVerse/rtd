...
use rtd_types::storage::{FullObjectKey, MarkerValue};
use tracing::error;
...
pub struct AuthorityPerpetualTables {
    ...
    pub(crate) expected_network_rtd_amount: DBMap<(), u64>,
...
impl AuthorityPerpetualTables {
    ...
    // Constructs `rtd_types::object::Object` from `StoreObjectWrapper`.
    // Returns `None` if object was deleted/wrapped
    pub fn object(
        &self,
        object_key: &ObjectKey,
        store_object: StoreObjectWrapper,
    ) -> Result<Option<Object>, RtdError> {
    ...
    pub fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> Result<Option<(ObjectKey, StoreObjectWrapper)>, RtdError> {
        let mut iterator = self.objects.reversed_safe_iter_with_bounds(
            Some(ObjectKey::min_for_id(&object_id)),
            Some(ObjectKey::max_for_id(&object_id)),
        )?;
    ...
    ) -> RtdResult<Option<TrustedTransaction>> {
    ...
    pub fn get_effects(&self, digest: &TransactionDigest) -> RtdResult<Option<TransactionEffects>> {
    ...
    pub fn insert_object_test_only(&self, object: Object) -> RtdResult {
    ...
    // fallible get object methods for rtd-tool, which may need to attempt to read a corrupted database
    pub fn get_object_fallible(&self, object_id: &ObjectID) -> RtdResult<Option<Object>> {
        let obj_entry = self
            .objects
