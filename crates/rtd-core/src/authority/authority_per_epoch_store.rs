...
use rtd_config::node::ExpensiveSafetyCheckConfig;
use rtd_execution::{self, Executor};
use rtd_macros::fail_point;
use rtd_protocol_config::{Chain, PerObjectCongestionControlMode, ProtocolConfig, ProtocolVersion};
use rtd_storage::mutex_table::{MutexGuard, MutexTable};
...
use rtd_types::base_types::{ConciseableName, ObjectRef};
use rtd_types::committee::Committee;
use rtd_types::committee::CommitteeTrait;
use rtd_types::crypto::{AuthoritySignInfo, AuthorityStrongQuorumSignInfo, RandomnessRound};
use rtd_types::digests::{ChainIdentifier, TransactionEffectsDigest};
...
use rtd_types::execution::{ExecutionTimeObservationKey, ExecutionTiming};
use rtd_types::global_state_hash::GlobalStateHash;
use rtd_types::message_envelope::TrustedEnvelope;
...
use rtd_types::storage::{BackingPackageStore, InputKey, ObjectStore};
use rtd_types::rtd_system_state::epoch_start_rtd_system_state::{
    EpochStartSystemState, EpochStartSystemStateTrait,
};
use rtd_types::rtd_system_state::{self, RtdSystemState};
...
pub enum ConsensusCertificateResult {
    ...
    RtdTransaction(VerifiedExecutableTransaction),
...
impl AuthorityEpochTables {
    ...
    fn load_reconfig_state(&self) -> RtdResult<ReconfigState> {
        let state = self
            .reconfig_state
    ...
    pub fn get_all_pending_consensus_transactions(&self) -> RtdResult<Vec<ConsensusTransaction>> {
        Ok(self
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &TransactionDigest) -> RtdResult {
        self.executed_transactions_to_checkpoint.remove(digest)?;
        Ok(())
    }

    pub fn get_last_consensus_index(&self) -> RtdResult<Option<ExecutionIndices>> {
        Ok(self
    ...
    pub fn get_last_consensus_stats(&self) -> RtdResult<Option<ExecutionIndicesWithStats>> {
        Ok(self.last_consensus_stats.get(&LAST_CONSENSUS_STATS_ADDR)?)
    }

    pub fn get_locked_transaction(&self, obj_ref: &ObjectRef) -> RtdResult<Option<LockDetails>> {
        Ok(self
    ...
    fn get_all_deferred_transactions_v2(
        &self,
    ) -> RtdResult<BTreeMap<DeferralKey, Vec<VerifiedExecutableTransactionWithAliases>>> {
        Ok(self
...
impl AuthorityPerEpochStore {
    ...
    pub fn tables(&self) -> RtdResult<Arc<AuthorityEpochTables>> {
        match self.tables.load_full() {
            Some(tables) => Ok(tables),
            None => Err(RtdErrorKind::EpochEnded(self.epoch()).into()),
        }
    }
    ...
    ) -> RtdResult<Option<GlobalStateHash>> {
        Ok(self
            .tables()?
    ...
    > {
        ...
        // Load stored execution time observations from the RtdSystemState object.
        let system_state =
            rtd_system_state::get_rtd_system_state(object_store).expect("System state must exist");
        let system_state = match system_state {
            RtdSystemState::V2(system_state) => system_state,
            RtdSystemState::V1(_) => {
                if committee.epoch() > 1 {
                    error!(
                        "`PerObjectCongestionControlMode::ExecutionTimeEstimate` cannot load execution time observations to RtdSystemState because it has an old version. This should not happen outside tests."
                    );
                }
                return itertools::Either::Left(std::iter::empty());
            }
            #[cfg(msim)]
            RtdSystemState::SimTestV1(_)
            | RtdSystemState::SimTestShallowV2(_)
            | RtdSystemState::SimTestDeepV2(_) => {
                return itertools::Either::Left(std::iter::empty());
            }
        };
    ...
    pub fn store_reconfig_state(&self, new_state: &ReconfigState) -> RtdResult {
        self.tables()?
            .reconfig_state
    ...
    pub fn insert_signed_transaction(&self, transaction: VerifiedSignedTransaction) -> RtdResult {
        Ok(self
            .tables()?
            .signed_transactions
    ...
    pub(crate) fn insert_tx_key(
        ...
        let _metrics_scope =
            linku_metrics::monitored_scope("AuthorityPerEpochStore::insert_tx_key");
    ...
    pub fn tx_key_to_digest(&self, key: &TransactionKey) -> RtdResult<Option<TransactionDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_effects_signature(
        &self,
        tx_digest: &TransactionDigest,
    ) -> RtdResult<Option<AuthoritySignInfo>> {
        let tables = self.tables()?;
    ...
    pub fn get_signed_effects_digest(
        &self,
        tx_digest: &TransactionDigest,
    ) -> RtdResult<Option<TransactionEffectsDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_transaction_cert_sig(
        &self,
        tx_digest: &TransactionDigest,
    ) -> RtdResult<Option<AuthorityStrongQuorumSignInfo>> {
    ...
    pub fn get_last_consensus_stats(&self) -> RtdResult<ExecutionIndicesWithStats> {
        assert!(
    ...
    ) -> RtdResult<Vec<(CheckpointSequenceNumber, GlobalStateHash)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    pub fn handle_finalized_checkpoint(
        ...
        let tables = match self.tables() {
            ...
            Err(e) if matches!(e.as_inner(), RtdErrorKind::EpochEnded(_)) => return Ok(()),
            Err(e) => return Err(e),
        };
    ...
    pub fn insert_finalized_transactions(
        ...
        let _metrics_scope = linku_metrics::monitored_scope(
    ...
    ) -> RtdResult<HashMap<ConsensusObjectSequenceKey, SequenceNumber>> {
    ...
    ) -> RtdResult<AssignedTxAndVersions> {
    ...
    ) -> RtdResult<Vec<(DeferralKey, Vec<VerifiedExecutableTransactionWithAliases>)>> {
    ...
    ) -> Result<Vec<CheckpointSequenceNumber>, RtdError> {
        let tables = self.tables()?;
    ...
    pub fn clear_override_protocol_upgrade_buffer_stake(&self) -> RtdResult {
        warn!(
            epoch = ?self.epoch(),
    ...
    pub fn set_override_protocol_upgrade_buffer_stake(&self, new_stake_bps: u64) -> RtdResult {
        warn!(
            ?new_stake_bps,
            epoch = ?self.epoch(),
    ...
    pub fn record_capabilities(&self, capabilities: &AuthorityCapabilitiesV1) -> RtdResult {
    ...
    /// Record most recently advertised capabilities of all authorities
    pub fn record_capabilities_v2(&self, capabilities: &AuthorityCapabilitiesV2) -> RtdResult {
    ...
    pub fn get_capabilities_v1(&self) -> RtdResult<Vec<AuthorityCapabilitiesV1>> {
    ...
    pub fn get_capabilities_v2(&self) -> RtdResult<Vec<AuthorityCapabilitiesV2>> {
        assert!(self.protocol_config.authority_capabilities_v2());
        Ok(self
            .tables()?
            .authority_capabilities_v2
    ...
    pub(crate) fn get_new_jwks(&self, round: u64) -> RtdResult<Vec<ActiveJwk>> {
    ...
    pub(crate) fn get_randomness_last_round_timestamp(&self) -> RtdResult<Option<TimestampMs>> {
        if let Some(ts) = self
            .consensus_quarantine
            .read()
    ...
    pub fn verify_transaction_with_current_aliases(
        &self,
        tx: Transaction,
    ) -> RtdResult<VerifiedTransactionWithAliases> {
    ...
    pub(crate) fn verify_consensus_transaction(
        ...
        // Signatures are verified as part of the consensus payload verification in RtdTxValidator
    ...
    fn db_batch(&self) -> RtdResult<DBBatch> {
    ...
    ) -> RtdResult<Vec<(CheckpointHeight, PendingCheckpoint)>> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    pub fn pending_checkpoint_exists(&self, index: &CheckpointHeight) -> RtdResult<bool> {
        Ok(self
            .consensus_quarantine
            .read()
    ...
    pub fn last_built_checkpoint_builder_summary(
        &self,
    ) -> RtdResult<Option<BuilderCheckpointSummary>> {
    ...
    pub fn last_built_checkpoint_summary(
        &self,
    ) -> RtdResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
    ...
    pub fn get_built_checkpoint_summary(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> RtdResult<Option<CheckpointSummary>> {
        if let Some(BuilderCheckpointSummary { summary, .. }) =
    ...
    pub fn get_last_checkpoint_signature_index(&self) -> RtdResult<u64> {
        Ok(self
            .tables()?
    ...
    pub(crate) fn set_rejection_vote_reason(&self, position: ConsensusPosition, reason: &RtdError) {
