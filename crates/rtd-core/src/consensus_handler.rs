...
use rtd_types::{
    ...
    rtd_system_state::epoch_start_rtd_system_state::EpochStartSystemStateTrait,
    transaction::{
...
mod additional_consensus_state {
    ...
    use rtd_types::{crypto::DefaultHash, digests::Digest};
...
impl<C: CheckpointServiceNotify + Send + Sync> ConsensusHandler<C> {
    ...
    pub(crate) async fn handle_consensus_commit(
        ...
        fail_point_if!("correlated-crash-after-consensus-commit-boundary", || {
            ...
            if rtd_simulator::random::deterministic_probability_once(&key, 0.01) {
        ...
        for (txn_key, assigned_versions) in assigned_versions.0.iter() {
            ...
            if !protocol_config.include_cancelled_randomness_txns_in_prologue()
                && assigned_versions
                    ...
                    .iter()
                    .any(|((id, _), _)| *id == RTD_RANDOMNESS_STATE_OBJECT_ID)
            {
                continue;
            }
                ...
                SequencedConsensusTransactionKind::External(consensus_transaction) => {
                    match consensus_transaction.kind {
                        ...
                        ConsensusTransactionKind::CertifiedTransaction(cert) => {
                            // Safe because signatures are verified when consensus called into RtdTxValidator::validate_batch.
...
mod tests {
    ...
    use rtd_protocol_config::{ConsensusTransactionOrdering, ProtocolConfig};
    use rtd_types::{
        base_types::ExecutionDigests,
        base_types::{AuthorityName, FullObjectRef, ObjectID, RtdAddress, random_object_ref},
        committee::Committee,
    ...
    async fn test_checkpoint_signature_dedup() {
        ...
        let network_config =
            rtd_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir().build();
        let state = TestAuthorityBuilder::new()
            .with_network_config(&network_config, 0)
            .build()
            .await;
        ...
        use rtd_types::messages_consensus::ConsensusTransactionKey as CK;
    ...
    async fn test_verify_consensus_transaction_filters_mismatched_authorities() {
        ...
        let (_, wrong_keypair) = rtd_types::crypto::get_authority_key_pair();
