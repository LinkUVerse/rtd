...
use rtd_storage::write_path_pending_tx_log::WritePathPendingTransactionLog;
...
impl TransactionOrchestrator<NetworkAuthorityClient> {
    pub fn new_with_auth_aggregator(
        ...
        reconfig_channel: Receiver<RtdSystemState>,
        parent_path: &Path,
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub fn load_all_pending_transactions_in_test(&self) -> RtdResult<Vec<VerifiedTransaction>> {
    ...
    async fn wait_for_finalized_tx_executed_locally_with_timeout(
        ...
        .await
        {
            Err(_elapsed) => {
                ...
                Err(RtdErrorKind::TimeoutError.into())
            }
            Ok(_) => {
...
impl<A> rtd_types::transaction_executor::TransactionExecutor for TransactionOrchestrator<A>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
