...
use crate::authority::authority_store::{ExecutionLockWriteGuard, RtdLockResult};
...
use rtd_types::digests::{TransactionDigest, TransactionEffectsDigest};
...
use rtd_types::transaction::{VerifiedSignedTransaction, VerifiedTransaction};
use rtd_types::{
...
/// Should only be used for rtd-tool or tests. Nodes must use build_execution_cache which
...
pub trait ExecutionCacheCommit: Send + Sync {
    ...
    /// Stage a checkpoint's output watermark in the batch that persists its outputs.
...
pub trait ObjectCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> RtdResult<Option<PackageObject>>;
        ...
        for (object_opt, object_ref) in objects.into_iter().zip(object_refs) {
            match object_opt {
                None => {
                    ...
                    return Err(RtdErrorKind::UserInputError { error }.into());
                }
                Some(object) => {
                    result.push(object);
                }
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> RtdLockResult;

    // This method is considered "private" - only used by multi_get_objects_with_more_accurate_error_return
    fn _get_live_objref(&self, object_id: ObjectID) -> RtdResult<ObjectRef>;
    ...
    fn check_owned_objects_are_live(&self, owned_object_refs: &[ObjectRef]) -> RtdResult;

    fn get_rtd_system_state_object_unsafe(&self) -> RtdResult<RtdSystemState>;

    fn get_bridge_object_unsafe(&self) -> RtdResult<Bridge>;
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> RtdResult<Vec<Option<(VerifiedTransaction, usize)>>> {
...
pub trait ExecutionCacheReconfigAPI: Send + Sync {
    ...
    fn expensive_check_rtd_conservation(
        &self,
        old_epoch_store: &AuthorityPerEpochStore,
    ) -> RtdResult;

    fn checkpoint_db(&self, path: &Path) -> RtdResult;
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> RtdResult<Option<PackageObject>> {
    ...
    ($implementor: ident) => {
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn expensive_check_rtd_conservation(
                ...
                self.store
                    .expensive_check_rtd_conservation(self, old_epoch_store)
            }

            fn checkpoint_db(&self, path: &std::path::Path) -> RtdResult {
