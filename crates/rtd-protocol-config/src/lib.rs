...
// Version 3: gas model v2, including all rtd conservation fixes. Fix for loaded child object
...
//            `max_meter_ticks_per_module` limits each from 6_000_000 to 16_000_000. rtd-system
//            framework changes.
...
//             Changes to rtd-system package to add PTB-friendly unstake function, and minor cleanup.
...
//             Add `rtd::table_vec::drop` to the framework via a system package upgrade.
...
// Version 19: Changes to rtd-system package to enable liquid staking.
...
// Version 25: Add rtd::table_vec::swap and rtd::table_vec::swap_remove to system packages.
// Version 26: New gas model version.
//             Add support for receiving objects off of other objects in devnet only.
// Version 28: Add rtd::zklogin::verify_zklogin_id and related functions to rtd framework.
//             Enable transaction effects v2 in devnet.
// Version 29: Add verify_legacy_zklogin_address flag to rtd framework, this add ability to verify
...
//             Add support for getting object ID referenced by receiving object in rtd framework.
//             Create new execution layer version, and preserve previous behavior in v1.
//             Update semantics of `rtd::transfer::receive` and add `rtd::transfer::public_receive`.
// Version 32: Add delete functions for VerifiedID and VerifiedIssuer.
//             Add rtd::token module to rtd framework.
...
// Version 42: Migrate rtd framework and related code to Move 2024
...
//             Modified rtd-system package to enable withdrawal of stake before it becomes active.
//             Enable soft bundle in devnet and testnet.
//             Core macro visibility in rtd core framework.
...
//             Max gas budget moved to 50_000 RTD
//             Max gas price moved to 50 RTD
...
// Version 74: Enable load_nitro_attestation move function in rtd framework in devnet.
...
//             Enable load_nitro_attestation move function in rtd framework in testnet.
...
// Version 88: Update `rtd-system` package to use `calculate_rewards` function.
...
// Version 91: Minor changes in Rtd Framework. Include CheckpointDigest in consensus dedup key for checkpoint signatures (V2).
...
pub struct ProtocolConfig {
    ...
    /// Max size of objects a transaction can write to disk after completion. Enforce by the Rtd adapter.
    ...
    /// Max size of objects a system transaction can write to disk after completion. Enforce by the Rtd adapter.
    ...
    /// Maximum size of the `contents` part of an object, in bytes. Enforced by the Rtd adapter when effects are produced.
    ...
    /// Maximum size of a Move package object, in bytes. Enforced by the Rtd adapter at the end of a publish transaction.
    ...
    // === Execution gas costs ====
    /// Base cost for any Rtd transaction
    ...
    // === Storage gas costs ===
    /// Per-byte cost of storing an object in the Rtd global object store. Some of this cost may be refundable if the object is later freed
    obj_data_cost_refundable: Option<u64>,

    // Per-byte cost of storing an object in the Rtd transaction log (e.g., in CertifiedTransactionEffects)
...
impl ProtocolConfig {
    ...
    pub fn get_for_version(version: ProtocolVersion, chain: Chain) -> Self {
        ...
        if std::env::var("RTD_PROTOCOL_CONFIG_OVERRIDE_ENABLE").is_ok() {
            warn!(
            ...
            let overrides: ProtocolConfigOptional =
                serde_env::from_env_with_prefix("RTD_PROTOCOL_CONFIG_OVERRIDE")
        ...
        for cur in 2..=version.0 {
            match cur {
                ...
                3 => {
                    ...
                    // max gas budget is in MIST and an absolute value 50RTD
                    cfg.max_tx_gas = Some(50_000_000_000);
                    // min gas budget is in MIST and an absolute value 2000MIST or 0.000002RTD
                ...
                72 => {
                    ...
                    // max gas budget is in MIST and an absolute value 50_000 RTD
                    cfg.max_tx_gas = Some(50_000_000_000_000);
                    // max gas price is in MIST and an absolute value 50 RTD
...
macro_rules! check_limit_by_meter {
    ($is_metered:expr, $x:expr, $metered_limit:expr, $unmetered_hard_limit:expr, $metric:expr) => {{
        ...
        use rtd_protocol_config::check_limit_in_range;
