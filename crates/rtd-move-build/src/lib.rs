...
pub mod test_utils {
    ...
    pub async fn compile_managed_coin_package() -> CompiledPackage {
        compile_example_package("../../crates/rtd-core/src/unit_tests/data/managed_coin").await
    }
...
/// Wrapper around the core Move `CompiledPackage` with some Rtd-specific traits and info
#[derive(Debug, Clone)]
...
/// Wrapper around the core Move `BuildConfig` with some Rtd-specific info
#[derive(Clone)]
...
impl BuildConfig {
    ...
    fn compile_package<W: Write + Send, F: MoveFlavor>(
        ...
        if config.default_flavor.is_none() {
            config.default_flavor = Some(move_compiler::editions::Flavor::Rtd);
        }
    ...
    pub async fn build_async_from_root_pkg(
        self,
        root_pkg: &mut RootPackage<RtdFlavor>,
    ...
    fn internal_build(
        ...
        let (package, fn_info) = result.map_err(|error| {
            RtdError::from(RtdErrorKind::ModuleBuildFailure {
...
fn verify_bytecode(package: &MoveCompiledPackage, fn_info: &FnInfoMap) -> RtdResult<()> {
    ...
    for m in compiled_modules.iter_modules() {
        move_bytecode_verifier::verify_module_unmetered(m).map_err(|err| {
            RtdErrorKind::ModuleVerificationFailure {
                error: err.to_string(),
            }
        })?;
...
impl CompiledPackage {
    ...
    pub fn get_dependency_sorted_modules(
        ...
        if with_unpublished_deps {
            ...
            // sufficient as we may have modules with the same names in user code and in Rtd
    ...
    /// Get bytecode modules from the Rtd System that are used by this package
    pub fn get_rtd_system_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == RTD_SYSTEM_ADDRESS)
    }

    /// Get bytecode modules from the Rtd Framework that are used by this package
    pub fn get_rtd_framework_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == RTD_FRAMEWORK_ADDRESS)
    }
    ...
    pub fn verify_unpublished_dependencies(
        ...
        Err(RtdErrorKind::ModulePublishFailure {
