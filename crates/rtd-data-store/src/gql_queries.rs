...
//! GQL Queries
//! Interface to the rpc for the gql schema defined in `crates\rtd-indexer-alt-graphql/schema.graphql`.
...
pub(crate) mod txn_query {
    ...
    pub(crate) async fn query(
        digest: String,
        data_store: &DataStore,
    ) -> Result<Option<(TransactionData, rtd_types::effects::TransactionEffects, u64)>, Error> {
        ...
        let effects: rtd_types::effects::TransactionEffects = bcs::from_bytes(
            &CryptoBase64::decode(
                &effect_frag
                    .effects_bcs
...
pub(crate) mod object_query {
    ...
    #[derive(cynic::Scalar, Debug, Clone)]
    #[cynic(graphql_type = "RtdAddress")]
    pub(crate) struct RtdAddress(pub String);
    ...
    impl From<GqlObjectKey> for ObjectKey {
        fn from(key: GqlObjectKey) -> Self {
            ObjectKey {
                address: RtdAddress(key.object_id.to_string()),
