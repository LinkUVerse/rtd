...
pub const DATA_STORE_DIR: &str = ".rtd_data_store";
...
impl FileSystemStore {
    ...
    pub fn base_path() -> Result<PathBuf, Error> {
        let home_dir = std::env::var("RTD_DATA_STORE")
            .or_else(|_| std::env::var("RTD_CONFIG_DIR"))
            .or_else(|_| std::env::var("HOME"))
            .or_else(|_| std::env::var("USERPROFILE"))
            .map_err(|_| {
                anyhow!(
                    "Cannot determine home directory. Define a RTD_DATA_STORE environment variable"
                )
            })?;
