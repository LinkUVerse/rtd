...
use rtd_types::{RTD_FRAMEWORK_PACKAGE_ID, RTD_SYSTEM_PACKAGE_ID, coin, fp_ensure};
...
impl TransactionBuilder {
    ...
    pub async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            Err(anyhow!(
                "Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-rtd or transfer-rtd commands instead, which will use the only object as gas payment."
            ))
        }
    }
    ...
    pub async fn transfer_rtd(
        ...
        let object = self.get_object_ref(rtd_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_rtd(
    ...
    /// Construct a transaction kind for the PayRtd transaction type
    pub fn pay_rtd_tx_kind(
        &self,
        recipients: Vec<RtdAddress>,
        amounts: Vec<u64>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_rtd(recipients.clone(), amounts.clone())?;
        let pt = builder.finish();
        let tx_kind = TransactionKind::programmable(pt);
        Ok(tx_kind)
    }

    pub async fn pay_rtd(
        &self,
        signer: RtdAddress,
        input_coins: Vec<ObjectID>,
        recipients: Vec<RtdAddress>,
    ...
    pub fn pay_all_rtd_tx_kind(&self, recipient: RtdAddress) -> TransactionKind {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn pay_all_rtd(
        ...
        let gas_object_ref = coin_refs.remove(0);
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_pay_all_rtd(
            signer,
            coin_refs,
            recipient,
            gas_object_ref,
    ...
    pub async fn move_call_tx_kind(
        ...
        module: &str,
        function: &str,
        type_args: Vec<RtdTypeTag>,
        call_args: Vec<RtdJsonValue>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn request_add_stake(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            let arguments = vec![
                builder.input(CallArg::RTD_SYSTEM_MUT).unwrap(),
    ...
    pub async fn request_withdraw_stake(
        ...
        let staked_rtd = self.get_object_ref(staked_rtd).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        let gas = self
