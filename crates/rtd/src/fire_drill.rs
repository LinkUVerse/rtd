...
//!
//! Example usage:
//! rtd fire-drill metadata-rotation \
//! --rtd-node-config-path validator.yaml \
...
pub struct MetadataRotation {
    /// Path to rtd node config.
    #[clap(long = "rtd-node-config-path")]
    rtd_node_config_path: PathBuf,
...
async fn run_metadata_rotation(metadata_rotation: MetadataRotation) -> anyhow::Result<()> {
    let MetadataRotation {
        rtd_node_config_path,
        account_key_path,
        fullnode_rpc_url,
    } = metadata_rotation;
    let account_key = read_keypair_from_file(&account_key_path)?;
    let config: NodeConfig = PersistedConfig::read(&rtd_node_config_path).map_err(|err| {
        err.context(format!(
            "Cannot open Rtd Node Config file at {:?}",
            rtd_node_config_path
        ))
    })?;
    ...
    info!(
        "Running Metadata Rotation fire drill for validator address {rtd_address} in epoch {starting_epoch}."
    );

    // Prepare new metadata for next epoch
    let new_config_path =
        update_next_epoch_metadata(&rtd_node_config_path, &config, &rtd_client, &account_key)
            .await?;

    let current_epoch = current_epoch(&rtd_client).await?;
    if current_epoch > starting_epoch {
        bail!("Epoch already advanced to {current_epoch}");
    }
    let target_epoch = starting_epoch + 1;
    wait_for_next_epoch(&rtd_client, target_epoch).await?;
    info!("Just advanced to epoch {target_epoch}");

    // Replace new config
    std::fs::rename(new_config_path, rtd_node_config_path)?;
    info!("Updated Rtd Node config.");
...
async fn update_next_epoch_metadata(
    ...
    let mut backup_config_path = rtd_node_config_path.to_path_buf();
    ...
    let pop = generate_proof_of_possession(&new_protocol_key_pair, rtd_address);
    ...
    new_config.network_key_pair = KeyPairWithPath::new(RtdKeyPair::Ed25519(new_network_key_pair));
    ...
    // Save new config
    let mut new_config_path = rtd_node_config_path.to_path_buf();
    new_config_path.pop();
    new_config_path.push(
        String::from(rtd_node_config_path.file_name().unwrap().to_str().unwrap()) + ".next_epoch",
    );
...
async fn update_metadata_on_chain(
    ...
    .unwrap();
    execute_tx(account_key, rtd_client, tx_data, function).await?;
