...
async fn test_addresses_command() -> Result<(), anyhow::Error> {
    ...
    for _ in 0..3 {
        keystore
            .import(None, RtdKeyPair::Secp256k1(get_key_pair().1))
            .await?;
    }
...
async fn test_flag_in_signature_and_keypair() -> Result<(), anyhow::Error> {
    ...
    for pk in keystore.entries() {
        let pk1 = pk.clone();
        let sig = keystore
            .sign_secure(&(&pk).into(), b"hello", Intent::rtd_transaction())
            .await?;
        match sig {
            Signature::Ed25519RtdSignature(_) => {
                // signature contains corresponding flag
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Ed25519RtdSignature::SCHEME.flag()
                );
                // keystore stores pubkey with corresponding flag
                assert!(pk1.flag() == Ed25519RtdSignature::SCHEME.flag())
            }
            Signature::Secp256k1RtdSignature(_) => {
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Secp256k1RtdSignature::SCHEME.flag()
                );
                assert!(pk1.flag() == Secp256k1RtdSignature::SCHEME.flag())
            }
            Signature::Secp256r1RtdSignature(_) => {
                assert_eq!(
                    *sig.as_ref().first().unwrap(),
                    Secp256r1RtdSignature::SCHEME.flag()
                );
                assert!(pk1.flag() == Secp256r1RtdSignature::SCHEME.flag())
            }
        }
    }
    Ok(())
...
async fn test_read_write_keystore_with_flag() {
    ...
    let kp_secp = RtdKeyPair::Secp256k1(get_key_pair().1);
    let addr_secp: RtdAddress = (&kp_secp.public()).into();
    ...
    let kp_ed = RtdKeyPair::Ed25519(get_key_pair().1);
    let addr_ed: RtdAddress = (&kp_ed.public()).into();
...
async fn test_rtd_operations_config() {
    ...
    assert!(read.is_ok());
    assert_eq!(
        RtdAddress::from_str("7d20dcdb2bca4f508ea9613994683eb4e76e9c4ed371169677c1be02aaf0b58e")
            .unwrap(),
    ...
    assert_eq!(
        RtdAddress::from_str("160ef6ce4f395208a12119c5011bf8d8ceb760e3159307c819bd0197d154d384")
            .unwrap(),
        read.unwrap().addresses()[0]
    );
}
...
async fn test_private_keys_import_export() -> Result<(), anyhow::Error> {
    ...
    for (private_key, private_key_hex, private_key_base64, address) in TEST_CASES {
        ...
        let kp_from_base64 = RtdKeyPair::decode_base64(private_key_base64).unwrap();
...
async fn test_mnemonics_ed25519() -> Result<(), anyhow::Error> {
    // Test case matches with /linku/rtd/sdk/typescript/test/unit/cryptography/ed25519-keypair.test.ts
...
async fn test_mnemonics_secp256k1() -> Result<(), anyhow::Error> {
    // Test case matches with /linku/rtd/sdk/typescript/test/unit/cryptography/secp256k1-keypair.test.ts
    const TEST_CASES: [[&str; 3]; 3] = [
        [
            "film crazy soon outside stand loop subway crumble thrive popular green nuclear struggle pistol arm wife phrase warfare march wheat nephew ask sunny firm",
...
async fn test_mnemonics_secp256r1() -> Result<(), anyhow::Error> {
    // Test case matches with /linku/rtd/sdk/typescript/test/unit/cryptography/secp256r1-keypair.test.ts
    const TEST_CASES: [[&str; 3]; 3] = [
        [
...
async fn test_sign_command() -> Result<(), anyhow::Error> {
    ...
    let gas_price = 1;
    let tx_data = TransactionData::new_pay_rtd(
        *sender,
        vec![gas],
