...
use crate::validator_commands::{
    RtdValidatorCommand, RtdValidatorCommandResponse, get_validator_summary,
};
use anyhow::Ok;
...
async fn test_print_raw_rgp_txn() -> Result<(), anyhow::Error> {
    ...
    let keypair: &RtdKeyPair = test_cluster
        .swarm
        .config()
        .validator_configs
        .first()
    ...
    .execute(&mut context)
    .await?;
    let RtdValidatorCommandResponse::DisplayGasPriceUpdateRawTxn {
        data,
        serialized_data,
    } = response
    else {
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::rtd_transaction(), deserialized_data),
        keypair,
    );
