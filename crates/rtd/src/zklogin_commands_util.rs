...
pub async fn perform_zk_login_test_tx(
    ...
    let skp1 = RtdKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([1; 32])));
    ...
    let sender = if test_multisig {
        ...
        RtdAddress::from(&multisig_pk)
    } else {
        println!("Use single zklogin address as sender");
        RtdAddress::try_from_unpadded(&zk_login_inputs)?
    };
    ...
    let txb_res = rtd
        ...
        .transfer_object(
            ...
            Some(gas_coin),
            5000000,
            RtdAddress::ZERO, // as a demo, send to a dummy address
        )
        .await?;
    println!(
