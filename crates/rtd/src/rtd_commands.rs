...
use rtd_bridge::rtd_transaction_builder::build_committee_register_transaction;
...
use rtd_config::{
    RTD_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME, RTD_GENESIS_FILENAME, RTD_KEYSTORE_FILENAME,
};
use rtd_faucet::{AppState, FaucetConfig, LocalFaucet, create_wallet_context, start_faucet};
use rtd_futures::service::Service;
use rtd_indexer_alt::{config::IndexerConfig, setup_indexer};
use rtd_indexer_alt_consistent_store::{
...
use rtd_swarm_config::genesis_config::GenesisConfig;
...
use crate::validator_commands::RtdValidatorCommand;

const DEFAULT_EPOCH_DURATION_MS: u64 = 60_000;

const DEFAULT_FAUCET_MIST_AMOUNT: u64 = 200_000_000_000; // 200 RTD
...
pub struct RtdEnvConfig {
    /// Sets the file storing the state of our user accounts (an empty one will be created if missing)
    #[clap(long = "client.config")]
    config: Option<PathBuf>,
    /// The Rtd environment to use. This must be present in the current config file.
    #[clap(long = "client.env")]
    env: Option<String>,
    /// Create a new rtd config without prompting if none exists
...
pub enum RtdCommand {
    ...
    ///
    /// By default, rtd start will start a local network from the genesis blob that exists in
    /// the Rtd config default dir or in the config_dir that was passed. If the default directory
    ...
    /// follows:
    /// - RTD_PROTOCOL_CONFIG_OVERRIDE_ENABLE=1
    /// - Then, to configure an override, use the prefix `RTD_PROTOCOL_CONFIG_OVERRIDE_`
    ///   along with the parameter name. For example, to increase the interval between
    ///   checkpoint creation to >1/s, you might set:
    ///   RTD_PROTOCOL_CONFIG_OVERRIDE_min_checkpoint_interval_ms=1000
    ///
    ...
    Start {
        ...
        /// rtd genesis -f --with-faucet generates a genesis config that can be used to start this
        /// process. Use with caution as the `-f` flag will overwrite the existing config directory.
        /// We can use any config dir that is generated by the `rtd genesis`.
        ...
        /// run this command.
        ///
        /// To run with persisted state, do not pass this flag and use the `rtd genesis` command
    ...
    /// Bootstrap and initialize a new rtd network
    #[clap(name = "genesis")]
    Genesis {
        ...
        #[clap(
            long,
            help = "Creates an extra faucet configuration for rtd persisted runs."
        )]
        with_faucet: bool,
    ...
    Validator {
        ...
        config: RtdEnvConfig,
        #[clap(subcommand)]
        cmd: Option<RtdValidatorCommand>,
        /// Return command outputs in json format.
        #[clap(long, global = true)]
        json: bool,
    },
    ...
    /// Invoke Rtd's move-analyzer via CLI
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        match self {
            RtdCommand::Network {
                ...
                let config_path = config.unwrap_or(rtd_config_dir()?.join(RTD_NETWORK_CONFIG));
                let config: NetworkConfig = PersistedConfig::read(&config_path).map_err(|err| {
                    err.context(format!(
                        "Cannot open Rtd network config file at {:?}",
                        config_path
                    ))
                })?;
            ...
            RtdCommand::GenesisCeremony(cmd) => run(cmd),
            RtdCommand::KeyTool {
                keystore_path,
                json,
                cmd,
            ...
            RtdCommand::Move {
                ...
                match cmd {
                    rtd_move::Command::Summary(mut s) if s.package_id.is_some() => {
                        ...
                        if !s.summary.bytecode {
                            eprintln!("{}",
                            "[warning] `rtd move summary --package-id <object_id>` only supports bytecode summaries. \
                        ...
                        execute_move_command(
                            Some(path),
                            build_config,
                            rtd_move::Command::Summary(s),
                            Some(rtd_move::CommandMeta::Summary(package_metadata)),
                            &context,
                        )
                        .await?;
                        Ok(())
            ...
            RtdCommand::BridgeInitialize {
                ...
                // Load the config of the Rtd authority.
                let network_config_path = network_config
                    .clone()
                    .unwrap_or(rtd_config_dir()?.join(RTD_NETWORK_CONFIG));
                ...
                let rtd_bridge_client = RtdBridgeClient::new(rpc_url, bridge_metrics).await?;
                let bridge_arg = rtd_bridge_client
...
async fn start(
    ...
    if epoch_duration_ms.is_some() && genesis_blob_exists(config.clone()) && !force_regenesis {
        bail!(
            ...
            there is no genesis configuration in the default Rtd configuration folder or the given \
    ...
    let config_dir = if force_regenesis {
        ...
        linku_common::tempdir()?.keep()
    } else {
        // If the config path looks like a YAML file, it is treated as if it is the network.yaml
        // overriding the network.yaml found in the rtd config directry. Otherwise it is treated as
        // the rtd config directory for backwards compatibility with `rtd-test-validator`.
        let (network_config_path, rtd_config_path) = match config {
            Some(config)
                if config.is_file()
                    && config
                        .extension()
                ...
                if committee_size.is_some() {
                    eprintln!(
                        ...
                        "[warning] The committee-size arg wil be ignored as a network \
                            ...
                            with the desired committee size. See `rtd genesis --help` for more \
                            information."
                            .yellow()
                            .bold()
                    );
                }
                (config, rtd_config_dir()?)
            }
            ...
            Some(config) => {
                ...
                (config.join(RTD_NETWORK_CONFIG), config)
            }

            None => {
                let rtd_config = rtd_config_dir()?;
                let network_config = rtd_config.join(RTD_NETWORK_CONFIG);
                ...
                if !network_config.exists() {
                    ...
                    .map_err(|_| {
                        anyhow!(
                            "Cannot run genesis with non-empty Rtd config directory: {}.\n\n\
                                ...
                                genesis, use rtd genesis --help to see how to generate a new \
                                genesis.",
                            rtd_config.display(),
                        )
                    })?;
    ...
    if config_dir.join(RTD_CLIENT_CONFIG).exists() {
        let _ = update_wallet_config_rpc(config_dir.clone(), fullnode_rpc_url.clone())?;
    }

    if force_regenesis && rtd_config_dir()?.join(RTD_CLIENT_CONFIG).exists() {
        let _ = update_wallet_config_rpc(rtd_config_dir()?, fullnode_rpc_url.clone())?;
    }
    ...
    if let Some(input) = with_faucet {
        ...
        if force_regenesis {
            ...
            let keystore_path = config_dir.join(RTD_KEYSTORE_FILENAME);
            let mut keystore =
                Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
            let address: RtdAddress = kp.public().into();
            keystore
                .import(None, RtdKeyPair::Ed25519(kp))
                .await
                .unwrap();
            ...
            .persisted(config_dir.join(RTD_CLIENT_CONFIG).as_path())
            .save()
            .unwrap();
        }
...
async fn genesis(
    ...
    let rtd_config_dir = &match working_dir {
        // if a directory is specified, it must exist (it
        // will not be created)
        Some(v) => v,
        // create default Rtd config dir if not specified
        // on the command line and if it does not exist
        // yet
        None => {
            let config_path = rtd_config_dir()?;
            fs::create_dir_all(&config_path)?;
            config_path
        }
    };

    // if Rtd config dir is not empty then either clean it
    // up (if --force/-f option was specified or report an
    // error
    let dir = rtd_config_dir.read_dir().map_err(|err| {
        anyhow!(err).context(format!("Cannot open Rtd config dir {:?}", rtd_config_dir))
    })?;
    let files = dir.collect::<Result<Vec<_>, _>>()?;

    let client_path = rtd_config_dir.join(RTD_CLIENT_CONFIG);
    let keystore_path = rtd_config_dir.join(RTD_KEYSTORE_FILENAME);
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            let is_compatible = FileBasedKeystore::load_or_create(&keystore_path).is_ok()
                && PersistedConfig::<RtdClientConfig>::read(&client_path).is_ok();
            ...
            if is_compatible {
                ...
                fs::remove_dir_all(rtd_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot remove Rtd config dir {:?}", rtd_config_dir))
                })?;
                fs::create_dir(rtd_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot create Rtd config dir {:?}", rtd_config_dir))
                })?;
            }
        } else if files.len() != 2 || !client_path.exists() || !keystore_path.exists() {
            bail!(
                "Cannot run genesis with non-empty Rtd config directory {}, please use the --force/-f option to remove the existing configuration",
                rtd_config_dir.to_str().unwrap()
            );
        }
    }

    let network_path = rtd_config_dir.join(RTD_NETWORK_CONFIG);
    let genesis_path = rtd_config_dir.join(RTD_GENESIS_FILENAME);
    ...
    let mut genesis_conf = match from_config {
        ...
        None => {
            if let Some(ips) = benchmark_ips {
                ...
                let path = rtd_config_dir.join(RTD_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME);
    ...
    let builder = ConfigBuilder::new(rtd_config_dir);
    ...
    for validator in &mut network_config.validator_configs {
        validator.genesis = rtd_config::node::Genesis::new_from_file(&genesis_path);
    }
    ...
    fullnode_config.save(rtd_config_dir.join(RTD_FULLNODE_CONFIG))?;
    let mut ssfn_nodes = vec![];
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            let path =
                rtd_config_dir.join(rtd_config::ssfn_config_file(ssfn.p2p_address.clone(), i));
            ...
            let ssfn_config = FullnodeConfigBuilder::new()
                ...
                .with_db_path(PathBuf::from("/opt/rtd/db/authorities_db/full_node_db"))
                ...
                .with_genesis(Genesis::new_from_file("/opt/rtd/config/genesis.blob"))
                .build(&mut OsRng, &network_config);
        ...
        for (i, mut validator) in network_config
            ...
            .enumerate()
        {
            let path = rtd_config_dir.join(rtd_config::validator_config_file(
    ...
    let mut client_config = if client_path.exists() {
        ...
        RtdClientConfig::new(keystore.into())
    };
    ...
    client_config.add_env(RtdEnv::devnet());
...
async fn prompt_if_no_config(
    ...
    if !accept_defaults {
        println!(
            "No rtd config found in `{}`, create one [Y/n]?",
    ...
    // make config file
    let default_env = RtdEnv::testnet();
...
/// address
async fn create_default_keystore(keystore_file: &Path) -> anyhow::Result<(Keystore, RtdAddress)> {
...
async fn get_wallet_context(client_config: &RtdEnvConfig) -> Result<WalletContext, anyhow::Error> {
    let wallet_conf_file = client_config
        .config
        .clone()
        .unwrap_or(rtd_config_dir()?.join(RTD_CLIENT_CONFIG));
