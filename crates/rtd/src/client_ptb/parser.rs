...
impl<'a, I: Iterator<Item = &'a str>> ProgramParser<'a, I> {
    ...
    fn parse_mvr_address(
        ...
        match prefix {
            ...
            Some(sp!(_, prefix)) => {
                ...
                match self.peek() {
                    sp!(sp, L(T::Ident, s)) => {
                        if s != "rtd" {
                            error!(sp => help: { "Expected 'rtd' extension" },
                    ...
                    sp!(sp, l) => {
                        error!(
                            sp,
                            "Expected a valid MVR domain extension ('.rtd') but got {}", l
                        )
                    }
                }
            }
        ...
        self.expect(T::ForwardSlash).map_err(|e| {
            err!(e.span => help: {
                "MVR packages must contain a slash and be in the format '(@<domain>|<domain>.rtd)/<package>[/<version>]'"
...
mod tests {
    ...
    fn test_parse_types() {
        let inputs = vec![
            ...
            "rtd::object::ID",
            "0x2::object::UID",
            "3::staking_pool::StakedRtd",
            // Generic types
            "0x2::coin::Coin<2::rtd::RTD>",
            "rtd::table::Table<rtd::object::ID, vector<0x1::option::Option<u32>>>",
        ];
        let mut parsed = Vec::new();
        for input in inputs {
            let mut x = shlex::split(input).unwrap();
    ...
    fn test_parse_commands() {
        let inputs = vec![
            ...
            // Move Call
            "--move-call 0x3::rtd_system::request_add_stake system coins.0 validator",
