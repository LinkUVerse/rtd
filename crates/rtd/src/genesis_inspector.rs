...
use rtd_types::{
    base_types::ObjectID,
    coin::CoinMetadata,
    gas_coin::{GasCoin, MIST_PER_RTD, TOTAL_SUPPLY_MIST},
...
const STR_RTD: &str = "Rtd";
const STR_STAKED_RTD: &str = "StakedRtd";
const STR_PACKAGE: &str = "Package";
const STR_COIN_METADATA: &str = "CoinMetadata";
const STR_OTHER: &str = "Other";
const STR_RTD_DISTRIBUTION: &str = "Rtd Distribution";
...
pub(crate) fn examine_genesis_checkpoint(genesis: UnsignedGenesis) {
    ...
    // Prepare Rtd distribution info
    let mut rtd_distribution = BTreeMap::new();
    let entry = rtd_distribution
        .entry("Rtd System".to_string())
    ...
    entry.insert(
        ...
        (STR_RTD, system_object.stake_subsidy.balance.value()),
    );

    // Prepare Object Info
    let mut owner_map = BTreeMap::new();
    let mut package_map = BTreeMap::new();
    let mut rtd_map = BTreeMap::new();
    let mut staked_rtd_map = BTreeMap::new();
    ...
    for object in genesis.objects() {
        ...
        match &object.data {
            rtd_types::object::Data::Move(move_object) => {
                if let Ok(gas) = GasCoin::try_from(object) {
                    let entry = rtd_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str.clone(), (STR_RTD, gas.value()));
                    rtd_map.insert(object.id(), gas);
                } else if let Ok(coin_metadata) = CoinMetadata::try_from(object) {
                    coin_metadata_map.insert(object.id(), coin_metadata);
                } else if let Ok(staked_rtd) = StakedRtd::try_from(object) {
                    let entry = rtd_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str, (STR_STAKED_RTD, staked_rtd.principal()));
                    // Assert pool id is associated with a knonw validator.
                    let validator = validator_pool_id_map.get(&staked_rtd.pool_id()).unwrap();
                    assert_eq!(validator.staking_pool.id, staked_rtd.pool_id());

                    staked_rtd_map.insert(object.id(), staked_rtd);
                } else {
                    other_object_map.insert(object.id(), move_object);
                }
            }
            rtd_types::object::Data::Package(p) => {
    ...
    examine_total_supply(&rtd_distribution, false);

    // Main loop for inspection
    let main_options: Vec<&str> = vec![STR_RTD_DISTRIBUTION, STR_VALIDATORS, STR_OBJECTS, STR_EXIT];
    loop {
        let ans = Select::new(
        ...
        .prompt();
        match ans {
            Ok(name) if name == STR_RTD_DISTRIBUTION => {
                examine_total_supply(&rtd_distribution, true)
            }
...
fn examine_validators(
    ...
    validator_map: &BTreeMap<&str, &RtdValidatorGenesis>,
) {
    loop {
...
fn examine_object(
    ...
    validator_pool_id_map: &BTreeMap<ObjectID, &RtdValidatorGenesis>,
    package_map: &BTreeMap<ObjectID, &MovePackage>,
    rtd_map: &BTreeMap<ObjectID, GasCoin>,
    staked_rtd_map: &BTreeMap<ObjectID, StakedRtd>,
    ...
    loop {
        ...
        match ans {
            Ok(name) if name == STR_EXIT => break,
            Ok(name) if name == STR_RTD => {
                for gas_coin in rtd_map.values() {
                    display_rtd(gas_coin, owner_map);
                }
                print_divider("Rtd");
            }
            Ok(name) if name == STR_STAKED_RTD => {
                for staked_rtd_coin in staked_rtd_map.values() {
                    display_staked_rtd(staked_rtd_coin, validator_pool_id_map, owner_map);
                }
                print_divider(STR_STAKED_RTD);
            }
...
fn examine_total_supply(
    rtd_distribution: &BTreeMap<String, BTreeMap<String, (&str, u64)>>,
    print: bool,
) {
    let mut total_rtd = 0;
    let mut total_staked_rtd = 0;
    for (owner, coins) in rtd_distribution {
        let mut amount_sum = 0;
        for (owner, value) in coins.values() {
            amount_sum += value;
            if *owner == STR_STAKED_RTD {
                total_staked_rtd += value;
            }
        }
        total_rtd += amount_sum;
        if print {
            println!("Owner {:?}", owner);
            println!(
                "Total Amount of Rtd/StakedRtd Owned: {amount_sum} MIST or {} RTD:",
                amount_sum / MIST_PER_RTD
            );
            println!("{:#?}\n", coins);
        }
    }
    assert_eq!(total_rtd, TOTAL_SUPPLY_MIST);
    // Always print this.
    println!(
        "Total Supply of Rtd: {total_rtd} MIST or {} RTD",
        total_rtd / MIST_PER_RTD
    );
    println!(
        "Total Amount of StakedRtd: {total_staked_rtd} MIST or {} RTD\n",
        total_staked_rtd / MIST_PER_RTD
    );
    if print {
        print_divider("Rtd Distribution");
    }
}

fn display_validator(validator: &RtdValidatorGenesis) {
    ...
    println!(
        "Staking Pool Rtd Balance: {:?}",
        validator.staking_pool.rtd_balance
    );
    println!(
        "Rewards Pool: {}",
    ...
    println!(
        "Pending Total Rtd Withdraw: {}",
        validator.staking_pool.pending_total_rtd_withdraw
    );
    println!(
...
fn display_rtd(gas_coin: &GasCoin, owner_map: &BTreeMap<ObjectID, Owner>) {
    println!("ID: {}", gas_coin.id());
    println!("Balance: {}", gas_coin.value());
    println!("Owner: {}\n", owner_map.get(gas_coin.id()).unwrap());
}

fn display_staked_rtd(
    ...
    println!("Owner: {}\n", owner_map.get(&staked_rtd.id()).unwrap());
}
