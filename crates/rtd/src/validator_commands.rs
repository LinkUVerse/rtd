...
use rtd_types::{
    ...
    object::Owner,
    rtd_system_state::{
        rtd_system_state_inner_v1::{UnverifiedValidatorOperationCapV1, ValidatorV1},
...
use rtd_json_rpc_types::{
    RtdObjectDataOptions, RtdTransactionBlockResponse, RtdTransactionBlockResponseOptions,
};
use rtd_keys::{
...
const DEFAULT_GAS_BUDGET: u64 = 200_000_000; // 0.2 RTD
...
pub struct TxProcessingArgs {
    ...
    /// be used to execute transaction with `rtd client execute-signed-tx --tx-bytes <TX_BYTES>`.
    #[arg(long)]
...
pub enum RtdValidatorCommand {
    ...
    DisplayMetadata {
        ...
        validator_address: Option<RtdAddress>,
        #[clap(name = "json", long)]
        json: Option<bool>,
    },
    ...
    ReportValidator {
        ...
        #[clap(name = "operation-cap-id", long)]
        operation_cap_id: Option<ObjectID>,
        /// The Rtd Address of the validator is being reported or un-reported
        #[clap(name = "reportee-address")]
        reportee_address: RtdAddress,
    ...
    /// Rtd native bridge committee member registration
    ...
    /// Update rtd native bridge committee node url
...
#[derive(Serialize)]
#[serde(untagged)]
pub enum RtdValidatorCommandResponse {
    MakeValidatorInfo,
    DisplayMetadata,
    BecomeCandidate {
        response: Option<RtdTransactionBlockResponse>,
    ...
    RegisterBridgeCommittee {
        execution_response: Option<RtdTransactionBlockResponse>,
        serialized_unsigned_transaction: Option<String>,
    },
...
fn make_key_files(
    ...
    if file_name.exists() {
        ...
        let kp = match key {
            Some(key) => {
                println!(
                    "Generated new key file {:?} based on rtd.keystore file.",
                    file_name
                );
                key
            }
...
impl RtdValidatorCommand {
    pub async fn execute(
        self,
        context: &mut WalletContext,
    ) -> Result<RtdValidatorCommandResponse, anyhow::Error> {
        let rtd_address = context.active_address()?;

        Ok(match self {
            RtdValidatorCommand::MakeValidatorInfo {
                name,
                description,
                image_url,
                project_url,
                ...
                let account_key = match context.config.keystore.export(&rtd_address)? {
                    RtdKeyPair::Ed25519(account_key) => RtdKeyPair::Ed25519(account_key.copy()),
                    _ => panic!(
                ...
                let account_keypair: RtdKeyPair = read_keypair_from_file(account_key_file_name)?;
                ...
                let validator_info = GenesisValidatorInfo {
                    info: rtd_genesis_builder::validator_info::ValidatorInfo {
                        name,
                        protocol_key: keypair.public().into(),
                        worker_key: worker_keypair.public().clone(),
                        account_address: RtdAddress::from(&account_keypair.public()),
                        network_key: network_keypair.public().clone(),
                        gas_price,
                        commission_rate: rtd_config::node::DEFAULT_COMMISSION_RATE,
                ...
                RtdValidatorCommandResponse::MakeValidatorInfo
            }
            RtdValidatorCommand::BecomeCandidate { file, tx_args } => {
                ...
                .await?;
                RtdValidatorCommandResponse::BecomeCandidate {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            RtdValidatorCommand::JoinCommittee { tx_args } => {
                let gas_budget = tx_args.gas_budget.unwrap_or(DEFAULT_GAS_BUDGET);
                let (response, serialized_unsigned_transaction) = call_0x5(
                    context,
                ...
                .await?;
                RtdValidatorCommandResponse::JoinCommittee {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            RtdValidatorCommand::LeaveCommittee { tx_args } => {
                ...
                .await?;
                RtdValidatorCommandResponse::LeaveCommittee {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            RtdValidatorCommand::DisplayMetadata {
                validator_address,
                json,
            } => {
                ...
                display_metadata(&rtd_client, validator_address, json.unwrap_or(true)).await?;
                RtdValidatorCommandResponse::DisplayMetadata
            }

            RtdValidatorCommand::UpdateMetadata { metadata, tx_args } => {
                let gas_budget = tx_args.gas_budget.unwrap_or(DEFAULT_GAS_BUDGET);
                ...
                .await?;
                RtdValidatorCommandResponse::UpdateMetadata {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            RtdValidatorCommand::UpdateGasPrice {
                operation_cap_id,
                gas_price,
                tx_args,
            } => {
                ...
                .await?;
                RtdValidatorCommandResponse::UpdateGasPrice {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            RtdValidatorCommand::ReportValidator {
                operation_cap_id,
                reportee_address,
                undo_report,
                tx_args,
                ...
                .await?;
                RtdValidatorCommandResponse::ReportValidator {
                    response,
                    serialized_unsigned_transaction,
                }
            }

            RtdValidatorCommand::SerializePayloadForPoP {
                account_address,
                protocol_public_key,
            } => {
                ...
                let mut intent_msg_bytes = bcs::to_bytes(&IntentMessage::new(
                    Intent::rtd_app(IntentScope::ProofOfPossession),
                    msg,
                ))
                .expect("Message serialization should not fail");
                DEFAULT_EPOCH_ID.write(&mut intent_msg_bytes);
                RtdValidatorCommandResponse::SerializedPayload(Base64::encode(&intent_msg_bytes))
            }

            RtdValidatorCommand::DisplayGasPriceUpdateRawTxn {
                sender_address,
                operation_cap_id,
                new_gas_price,
                gas_budget,
                ...
                RtdValidatorCommandResponse::DisplayGasPriceUpdateRawTxn {
                    data,
                    serialized_data,
                }
            }
            RtdValidatorCommand::RegisterBridgeCommittee {
                ...
                let rtd_client = context.get_client().await?;
                let active_validators = rtd_client
                    .governance_api()
                ...
                if !active_validators
                    .into_iter()
                    .any(|s| s.rtd_address == address)
                {
                    bail!("Address {} is not in the committee", address);
                }
                println!(
                    "Starting bridge committee registration for Rtd validator: {address}, with bridge public key: {} and url: {}",
                    ecdsa_keypair.public, bridge_authority_url
                );
                let rtd_rpc_url = &context.get_active_env().unwrap().rpc;
                let bridge_metrics = Arc::new(BridgeMetrics::new_for_testing());
                let bridge_client = RtdBridgeClient::new(rtd_rpc_url, bridge_metrics).await?;
                ...
                if print_unsigned_transaction_only {
                    ...
                    RtdValidatorCommandResponse::RegisterBridgeCommittee {
            ...
            RtdValidatorCommand::UpdateBridgeCommitteeNodeUrl {
                bridge_authority_url,
                print_unsigned_transaction_only,
                validator_address,
                gas_budget,
                ...
                if !committee_members
                    .into_iter()
                    .any(|(_, m)| m.rtd_address == address)
                {
                    bail!("Address {} is not in the committee", address);
                }
                println!(
                    "Updating bridge committee node URL for Rtd validator: {address}, url: {}",
                    bridge_authority_url
                );
                ...
                if print_unsigned_transaction_only {
                    let serialized_data = Base64::encode(bcs::to_bytes(&tx_data)?);
                    RtdValidatorCommandResponse::UpdateBridgeCommitteeURL {
                        execution_response: None,
                        serialized_unsigned_transaction: Some(serialized_data),
                    }
                } else {
...
fn check_address(
    active_address: RtdAddress,
    validator_address: Option<RtdAddress>,
    print_unsigned_transaction_only: bool,
) -> Result<RtdAddress, anyhow::Error> {
    if !print_unsigned_transaction_only {
...
async fn get_cap_object_ref(
    context: &mut WalletContext,
    operation_cap_id: Option<ObjectID>,
) -> Result<(ValidatorStatus, RtdValidatorSummary, ObjectRef)> {
    let rtd_client = context.get_client().await?;
    if let Some(operation_cap_id) = operation_cap_id {
        let (status, summary) =
            get_validator_summary_from_cap_id(&rtd_client, operation_cap_id).await?;
        let cap_obj_ref = rtd_client
            .read_api()
            .get_object_with_options(
                summary.operation_cap_id,
                RtdObjectDataOptions::default().with_owner(),
            )
            .await?
            .object_ref_if_exists()
            .ok_or_else(|| anyhow!("OperationCap {} does not exist", operation_cap_id))?;
        Ok::<(ValidatorStatus, RtdValidatorSummary, ObjectRef), anyhow::Error>((
            status,
            summary,
            cap_obj_ref,
        ))
        ...
        let (status, summary) = get_validator_summary(&rtd_client, validator_address)
            .await?
        ...
        let resp = rtd_client
            .read_api()
            .get_object_with_options(cap_object_id, RtdObjectDataOptions::default().with_owner())
            .await
            .map_err(|e| anyhow!(e))?;
...
) -> Result<(Option<RtdTransactionBlockResponse>, Option<String>)> {
...
async fn report_validator(
    ...
    let validator_address = summary.rtd_address;
...
async fn get_validator_summary_from_cap_id(
    client: &RtdClient,
    operation_cap_id: ObjectID,
) -> anyhow::Result<(ValidatorStatus, RtdValidatorSummary)> {
    let resp = client
        .read_api()
        .get_object_with_options(operation_cap_id, RtdObjectDataOptions::default().with_bcs())
        .await?;
...
async fn construct_unsigned_0x5_txn(
    ...
    let gas_obj_ref = get_gas_obj_ref(sender, &rtd_client, gas_budget).await?;
    TransactionData::new_move_call(
        sender,
        RTD_SYSTEM_PACKAGE_ID,
        ident_str!("rtd_system").to_owned(),
...
async fn call_0x5(
    ...
    gas_budget: u64,
    serialize_unsigned_transaction: bool,
) -> anyhow::Result<(Option<RtdTransactionBlockResponse>, Option<String>)> {
    ...
    let signature = context
        .config
        .keystore
        .sign_secure(&sender, &tx_data, Intent::rtd_transaction())
        .await?;
    let transaction = Transaction::from_data(tx_data, vec![signature]);
    let rtd_client = context.get_client().await?;
    let response = rtd_client
        .quorum_driver_api()
...
impl Display for RtdValidatorCommandResponse {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            RtdValidatorCommandResponse::SerializedPayload(response) => {
...
impl Debug for RtdValidatorCommandResponse {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl RtdValidatorCommandResponse {
...
pub async fn get_validator_summary(
...
) -> anyhow::Result<Option<(ValidatorStatus, RtdValidatorSummary)>> {
    let RtdSystemStateSummary {
        active_validators,
    ...
    let validator_info = if active_validators.contains_key(&validator_address) {
        ...
        get_pending_candidate_summary(validator_address, client, pending_active_validators_id)
            .await?
            .map(|v| v.into_rtd_validator_summary())
...
async fn get_pending_candidate_summary(
    ...
    let pending_validators = rtd_client
        .read_api()
        ...
        .collect::<Vec<_>>();
    let resps = rtd_client
        .read_api()
        .multi_get_object_with_options(
            pending_validators,
            RtdObjectDataOptions::default().with_bcs(),
        )
        .await?;
    for resp in resps {
        ...
        if field.value.verified_metadata().rtd_address == validator_address {
...
async fn update_metadata(
    ...
    match metadata {
        ...
        MetadataUpdate::ProtocolPubKey { file } => {
            ...
            let pop = generate_proof_of_possession(&protocol_key_pair, rtd_address);
            let args = vec![
                CallArg::Pure(
                    bcs::to_bytes(&AuthorityPublicKeyBytes::from_bytes(
...
async fn check_status(
    ...
    let rtd_client = context.get_client().await?;
    let validator_address = context.active_address()?;
    let summary = get_validator_summary(&rtd_client, validator_address).await?;
    if summary.is_none() {
