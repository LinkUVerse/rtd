...
pub enum RtdClientCommands {
    ...
    Balance {
        ...
        /// Show balance for the specified coin (e.g., 0x2::rtd::RTD).
    ...
    Call {
        ...
        #[clap(
            long,
            value_parser = parse_rtd_type_tag,
            num_args(1..),
        )]
    ...
    ExecuteSignedTx {
        /// BCS serialized transaction data bytes without its type tag, as base64 encoded string. This is the output of rtd client command using --serialize-unsigned-transaction.
        #[clap(long)]
        tx_bytes: String,
    ...
    ExecuteCombinedSignedTx {
        /// BCS serialized sender signed data, as base64 encoded string. This is the output of rtd client command using --serialize-signed-transaction.
        #[clap(long)]
    ...
    Objects {
        ...
        /// objects owned by `rtd client active-address`.
        #[clap(name = "owner_address")]
        address: Option<KeyIdentity>,
    },
    ...
    /// Pay all residual RTD coins to the recipient with input coins, after deducting the gas cost.
    ...
    /// Replay a given transaction to view transaction effects (deprecated; use `rtd replay` instead)
    #[clap(name = "replay-transaction")]
    ReplayTransaction {},

    /// Replay transactions listed in a file (deprecated; use `rtd replay` instead)
    #[clap(name = "replay-batch")]
    ReplayBatch {},

    /// Replay all transactions in a range of checkpoints (deprecated; use `rtd replay` instead)
...
pub struct TxProcessingArgs {
    ...
    /// `rtd client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
    #[arg(long)]
...
impl RtdClientCommands {
    pub async fn execute(
        ...
        let ret = match self {
            RtdClientCommands::ReplayTransaction {} => {
                eprintln!("This command is deprecated. Use `rtd replay` instead.");
                RtdClientCommandResult::NoOutput
            }
            RtdClientCommands::ReplayBatch {} => {
                eprintln!("This command is deprecated. Use `rtd replay` instead.");
                RtdClientCommandResult::NoOutput
            }
            RtdClientCommands::ReplayCheckpoints {} => {
                eprintln!("This command is deprecated. Use `rtd replay` instead.");
                RtdClientCommandResult::NoOutput
            }
            RtdClientCommands::Addresses { sort_by_alias } => {
                ...
                RtdClientCommandResult::Addresses(output)
            }
            RtdClientCommands::Balance {
                address,
                coin_type,
                with_coins,
            } => {
            ...
            RtdClientCommands::VerifyBytecodeMeter {
                protocol_version,
                module_paths,
                package_path,
                build_config,
                ...
                let mut verifier = rtd_execution::verifier(
                    &protocol_config,
                    signing_limits,
            ...
            RtdClientCommands::Call {
                ...
                // without failing RtdJSON's checks.
                let args = args
                    .into_iter()
                    .map(|value| RtdJsonValue::new(convert_number_to_string(value.to_json_value())))
                    .collect::<Result<_, _>>()?;
            ...
            RtdClientCommands::TransferRtd {
                ...
                let tx_kind = client
                    .transaction_builder()
                    .transfer_rtd_tx_kind(to, amount);
            ...
            RtdClientCommands::Pay {
                ...
                let tx_kind = client
                    ...
                    "Gas coin is in input coins of Pay transaction, use PayRtd transaction instead!"
                );
            ...
            RtdClientCommands::PayRtd {
                ...
                processing,
                    ...
                    !input_coins.is_empty(),
                    "PayRtd transaction requires a non-empty list of input coins"
                );
                ensure!(
                    !recipients.is_empty(),
                    "PayRtd transaction requires a non-empty list of recipient addresses"
                );
                ensure!(
                    recipients.len() == amounts.len(),
                    format!(
                ...
                let tx_kind = client
                    .transaction_builder()
                    .pay_rtd_tx_kind(recipients, amounts)?;
            ...
            RtdClientCommands::PayAllRtd {
                ...
                processing,
                    ...
                    !input_coins.is_empty(),
                    "PayAllRtd transaction requires a non-empty list of input coins"
                );
            ...
            RtdClientCommands::NewAddress {
                ...
                RtdClientCommandResult::NewAddress(NewAddressOutput {
                    alias,
                    address,
                    key_scheme: scheme,
            ...
            RtdClientCommands::RemoveAddress { alias_or_address } => {
                let identity = KeyIdentity::from_str(&alias_or_address)
                    .map_err(|e| anyhow!("Invalid address or alias: {}", e))?;
                let address: RtdAddress = context.config.keystore.get_by_identity(&identity)?;

                context.config.keystore.remove(address).await?;

                RtdClientCommandResult::RemoveAddress(RemoveAddressOutput { alias_or_address })
            }
            ...
            RtdClientCommands::SerializedTx {
                tx_bytes,
                processing,
            } => {
            ...
            RtdClientCommands::SerializedTxKind {
                tx_bytes,
                payment,
                gas_data,
                processing,
            ...
            RtdClientCommands::Switch { address, env } => {
                let mut addr = None;
                ...
                context.config.save()?;
                RtdClientCommandResult::Switch(SwitchResponse { address: addr, env })
            }
            RtdClientCommands::ActiveAddress => {
                RtdClientCommandResult::ActiveAddress(context.active_address().ok())
            }
            ...
            RtdClientCommands::ExecuteSignedTx {
                ...
                let data = bcs::from_bytes(
                    ...
                    .to_vec()
                    .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse tx bytes, check if it matches the output of rtd client commands with --serialize-unsigned-transaction"))?;
            ...
            RtdClientCommands::ExecuteCombinedSignedTx { signed_tx_bytes } => {
                let data: SenderSignedData = bcs::from_bytes(
                    &Base64::try_from(signed_tx_bytes)
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                        .to_vec()
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse SenderSignedData bytes, check if it matches the output of rtd client commands with --serialize-signed-transaction"))?;
                let transaction = Envelope::<SenderSignedData, EmptySignInfo>::new(data);
                let response = context.execute_transaction_may_fail(transaction).await?;
                RtdClientCommandResult::TransactionBlock(response)
            }
            RtdClientCommands::NewEnv {
                alias,
                rpc,
                ws,
                basic_auth,
            ...
            RtdClientCommands::ActiveEnv => RtdClientCommandResult::ActiveEnv(
                context.get_active_env().ok().map(|e| e.alias.clone()),
            ),
            RtdClientCommands::Envs => RtdClientCommandResult::Envs(
    ...
    pub fn switch_env(config: &mut RtdClientConfig, env: &str) -> Result<(), anyhow::Error> {
        let env = Some(env.into());
            ...
            "Environment config not found for [{env:?}], add new environment config using the `rtd client new-env` command."
        );
...
impl Display for RtdClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let mut writer = String::new();
        match self {
            RtdClientCommandResult::Addresses(addresses) => {
            ...
            RtdClientCommandResult::NewAddress(new_address) => {
                let mut builder = TableBuilder::default();
            ...
            RtdClientCommandResult::RemoveAddress(remove_address) => {
                let mut builder = TableBuilder::default();
            ...
            RtdClientCommandResult::SerializedUnsignedTransaction(tx_data) => {
                writeln!(
                    writer,
                    "{}",
            ...
            RtdClientCommandResult::SerializedSignedTransaction(sender_signed_tx) => {
                writeln!(
                    writer,
                    "{}",
            ...
            RtdClientCommandResult::ActiveAddress(response) => {
                match response {
                    Some(r) => write!(writer, "{}", r)?,
                    None => write!(writer, "None")?,
                };
            }
            RtdClientCommandResult::ActiveEnv(env) => {
                write!(writer, "{}", env.as_deref().unwrap_or("None"))?;
            }
            RtdClientCommandResult::NewEnv(env) => {
                writeln!(writer, "Added new Rtd env [{}] to config.", env.alias)?;
            }
            RtdClientCommandResult::Envs(envs, active) => {
                let mut builder = TableBuilder::default();
...
impl From<&GasCoin> for GasCoinOutput {
    fn from(gas_coin: &GasCoin) -> Self {
        Self {
            ...
            rtd_balance: format_balance(gas_coin.value() as u128, 9, 2, None),
        }
    }
}
    ...
    match resp.status() {
        StatusCode::ACCEPTED | StatusCode::CREATED | StatusCode::OK => {
            ...
            if let Some(err) = faucet_resp.error {
                ...
                println!(
                    "Request successful. It can take up to 1 minute to get the coin. Run rtd client gas to check your gas coins."
                );
            }
        }
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    if serialize_unsigned_transaction {
        Ok(RtdClientCommandResult::SerializedUnsignedTransaction(
            tx_data,
        ))
    } else if tx_digest {
        ...
        if serialize_signed_transaction {
            Ok(RtdClientCommandResult::SerializedSignedTransaction(
                sender_signed_data,
            ))
        } else {
...
pub fn update_publication(
    ...
    match command {
        LockCommand::Publish => {
            ...
            Ok(Publication::<RtdFlavor> {
                chain_id: chain_id.to_string(),
                metadata: rtd_package_alt::PublishedMetadata {
                    toolchain_version: Some(env!("CARGO_PKG_VERSION").into()),
                    build_config: Some(rtd_package_alt::BuildParams::default()),
...
async fn publish_command(
    args: PublishArgs,
    root_package: &mut RootPackage<RtdFlavor>,
    context: &mut WalletContext,
) -> Result<RtdClientCommandResult, anyhow::Error> {
    let PublishArgs {
        package_path,
