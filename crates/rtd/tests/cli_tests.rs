...
use rtd_sdk::rtd_client_config::RtdClientConfig;
use rtd_sdk::wallet_context::WalletContext;
use rtd_swarm_config::genesis_config::{AccountConfig, GenesisConfig};
...
impl TreeShakingTest {
    ...
    async fn new() -> Result<Self, anyhow::Error> {
        ...
        let framework_pkgs = PathBuf::from("../rtd-framework/packages");
...
version = 1
toolchain-version = "{}"
build-config = {{ flavor = "rtd", edition = "2024" }}
upgrade-capability = "{}""#,
            chain_id,
            published_at_id,
            published_at_id,
    ...
    async fn upgrade_package(
        ...
        let RtdClientCommandResult::TransactionBlock(publish_response) = resp else {
...
async fn test_genesis() -> Result<(), anyhow::Error> {
    ...
    let config = working_dir.join(RTD_NETWORK_CONFIG);

    // Start network without authorities
    let start = RtdCommand::Start {
    ...
    assert!(files.contains(&RTD_CLIENT_CONFIG.to_string()));
    assert!(files.contains(&RTD_NETWORK_CONFIG.to_string()));
    assert!(files.contains(&RTD_FULLNODE_CONFIG.to_string()));
    assert!(files.contains(&RTD_GENESIS_FILENAME.to_string()));
    assert!(files.contains(&RTD_KEYSTORE_FILENAME.to_string()));
    assert!(files.contains(&RTD_KEYSTORE_ALIASES_FILENAME.to_string()));

    // Check network config
    let network_conf =
        PersistedConfig::<NetworkConfig>::read(&working_dir.join(RTD_NETWORK_CONFIG))?;
    assert_eq!(1, network_conf.validator_configs().len());

    // Check wallet config
    let wallet_conf =
        PersistedConfig::<RtdClientConfig>::read(&working_dir.join(RTD_CLIENT_CONFIG))?;
...
async fn test_addresses_command() -> Result<(), anyhow::Error> {
    ...
    for _ in 0..3 {
        context
            .config
            .keystore
            .import(None, RtdKeyPair::Ed25519(get_key_pair().1))
            .await?;
    }

    // Print all addresses
    RtdClientCommands::Addresses {
        sort_by_alias: true,
    }
    .execute(&mut context)
    .await
...
async fn test_ptb_publish_and_complex_arg_resolution() -> Result<(), anyhow::Error> {
    ...
    rtd::client_ptb::ptb::PTB { args: args.clone() }
        .execute(context)
        .await?;
...
async fn test_ptb_publish() -> Result<(), anyhow::Error> {
    ...
    let res = rtd::client_ptb::ptb::PTB { args: args.clone() }
        .execute(context)
        .await;
...
async fn test_gas_command() -> Result<(), anyhow::Error> {
    ...
    // Send an object
    RtdClientCommands::Transfer {
        to: KeyIdentity::Address(RtdAddress::random_for_testing_only()),
...
async fn test_move_call_args_linter_command() -> Result<(), anyhow::Error> {
    ...
    // Create the args
    let args = vec![
        RtdJsonValue::new(json!("123"))?,
        RtdJsonValue::new(json!(address1))?,
    ];

    // Test case with no gas specified
    let resp = RtdClientCommands::Call {
        package,
    ...
    assert!(RtdJsonValue::new(args_json.as_array().unwrap().first().unwrap().clone()).is_err());

    // Try a bad argument: too few args
    let args_json = json!([300usize]);
    let mut args = vec![];
    for a in args_json.as_array().unwrap() {
        args.push(RtdJsonValue::new(a.clone()).unwrap());
    }
    ...
    let args = [
        RtdJsonValue::new(json!(obj))?,
        RtdJsonValue::new(json!(address2))?,
    ];
    ...
    let args = [
        RtdJsonValue::new(json!(created_obj))?,
        RtdJsonValue::new(json!(address2))?,
    ];
    ...
    // let err_string = format!("{} ", resp.err().unwrap());
    // let framework_addr = RTD_FRAMEWORK_ADDRESS.to_hex_literal();
    // let package_addr = package.to_hex_literal();
    // assert!(err_string.contains(&format!("Expected argument of type {package_addr}::object_basics::Object, but found type {framework_addr}::coin::Coin<{framework_addr}::rtd::RTD>")));
...
async fn test_package_management_on_publish_command() -> Result<(), anyhow::Error> {
    ...
    let parsed: ParsedPublishedFile<RtdFlavor> = toml_edit::de::from_str(&pubfile_str).unwrap();
...
async fn test_delete_shared_object() -> Result<(), anyhow::Error> {
    ...
    let delete_result = RtdClientCommands::Call {
        ...
        type_args: vec![],
        args: vec![RtdJsonValue::from_str(&shared_id.to_string()).unwrap()],
        payment: PaymentArgs::default(),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_PUBLISH),
            ..Default::default()
...
async fn test_package_management_on_upgrade_command() -> Result<(), anyhow::Error> {
    ...
    let published_file: ParsedPublishedFile<RtdFlavor> =
...
async fn test_native_transfer() -> Result<(), anyhow::Error> {
    ...
    // Check the objects
    let resp = RtdClientCommands::Object {
        id: mut_obj1,
        bcs: false,
    }
    .execute(context)
    ...
    let resp2 = RtdClientCommands::Object {
        id: mut_obj2,
        bcs: false,
    }
    .execute(context)
...
async fn test_switch_command() -> Result<(), anyhow::Error> {
    ...
    let os = RtdClientCommands::Objects { address: None }
        .execute(context)
        .await?;

    let mut cmd_objs = if let RtdClientCommandResult::Objects(v) = os {
        v
    } else {
    ...
    // Switch the address
    let resp = RtdClientCommands::Switch {
    ...
    assert_eq!(
        ...
        format!(
            "{}",
            RtdClientCommandResult::Switch(SwitchResponse {
    ...
    let os = RtdClientCommands::NewAddress {
    ...
    .execute(context)
    .await?;
    let new_addr = if let RtdClientCommandResult::NewAddress(x) = os {
        x.address
    } else {
        panic!("Command failed")
    };
...
async fn test_new_address_command_by_flag() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(
        context
            ...
            .entries()
            .iter()
            .filter(|k| k.flag() == Ed25519RtdSignature::SCHEME.flag())
            .count(),
        5
    );

    RtdClientCommands::NewAddress {
    ...
    assert_eq!(
        context
            ...
            .entries()
            .iter()
            .filter(|k| k.flag() == Secp256k1RtdSignature::SCHEME.flag())
            .count(),
        1
    );
...
async fn test_remove_address_command() -> Result<(), anyhow::Error> {
    ...
    RtdClientCommands::RemoveAddress {
...
async fn test_active_address_command() -> Result<(), anyhow::Error> {
    ...
    // Run a command with address omitted
    let os = RtdClientCommands::ActiveAddress {}.execute(context).await?;

    let a = if let RtdClientCommandResult::ActiveAddress(Some(v)) = os {
        v
    } else {
        panic!("Command failed")
    };
...
async fn test_merge_coin() -> Result<(), anyhow::Error> {
    ...
    let resp = RtdClientCommands::MergeCoin {
        primary_coin,
        coin_to_merge,
        payment: PaymentArgs { gas: vec![gas] },
        gas_data: GasDataArgs {
...
async fn test_split_coin() -> Result<(), anyhow::Error> {
    ...
    // Test with gas specified
    let resp = RtdClientCommands::SplitCoin {
        coin_id: coin,
...
async fn test_execute_signed_tx() -> Result<(), anyhow::Error> {
    ...
    RtdClientCommands::ExecuteSignedTx {
...
async fn test_serialize_tx() -> Result<(), anyhow::Error> {
    ...
    let ptb = PTB { args };
    RtdClientCommands::PTB(ptb).execute(context).await.unwrap();
    let mut args = ptb_args.clone();
...
async fn test_dry_run() -> Result<(), anyhow::Error> {
    ...
    // === TRANSFER RTD === //
    let transfer_rtd_dry_run = RtdClientCommands::TransferRtd {
        to: KeyIdentity::Address(RtdAddress::random_for_testing_only()),
        rtd_coin_object_id: object_to_send,
        amount: Some(1),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
    ...
    assert_dry_run(transfer_rtd_dry_run, object_to_send, "TransferRtd");
    ...
    // === PAY RTD === //
    let pay_rtd_dry_run = RtdClientCommands::PayRtd {
        input_coins: vec![object_id],
        recipients: vec![KeyIdentity::Address(RtdAddress::random_for_testing_only())],
        amounts: vec![1],
        gas_data: GasDataArgs {
    ...
    assert_dry_run(pay_rtd_dry_run, object_id, "PayRtd");

    // === PAY ALL RTD === //
    let pay_all_rtd_dry_run = RtdClientCommands::PayAllRtd {
        input_coins: vec![object_id],
        recipient: KeyIdentity::Address(RtdAddress::random_for_testing_only()),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
        },
    ...
    assert_dry_run(pay_all_rtd_dry_run, object_id, "PayAllRtd");
...
#[sim_test]
async fn test_pay_rtd() -> Result<(), anyhow::Error> {
    let (mut test_cluster, client, rgp, objects, recipients, addresses) =
        test_cluster_helper().await;
    ...
    let pay_rtd = RtdClientCommands::PayRtd {
        input_coins: vec![object_id1, object_id2],
        recipients: vec![recipient1.clone(), recipient2.clone()],
        amounts: amounts.into(),
        gas_data: GasDataArgs {
    ...
    // pay rtd takes the input coins and transfers from each of them (in order) the amounts to the
...
#[sim_test]
async fn test_pay_all_rtd() -> Result<(), anyhow::Error> {
    let (mut test_cluster, client, rgp, objects, recipients, addresses) =
        test_cluster_helper().await;
    let (object_id1, object_id2) = (objects[0], objects[1]);
    ...
    let context = &mut test_cluster.wallet;
    let pay_all_rtd = RtdClientCommands::PayAllRtd {
        input_coins: vec![object_id1, object_id2],
    ...
    // pay all rtd will take the input coins and smash them into one coin and transfer that coin to
...
async fn test_transfer() -> Result<(), anyhow::Error> {
    ...
    let address2 = addresses[0];
    let context = &mut test_cluster.wallet;
    let transfer = RtdClientCommands::Transfer {
...
#[sim_test]
async fn test_transfer_rtd() -> Result<(), anyhow::Error> {
    let (mut test_cluster, client, rgp, objects, recipients, addresses) =
        test_cluster_helper().await;
    ...
    let context = &mut test_cluster.wallet;
    let amount = 1000;
    let transfer_rtd = RtdClientCommands::TransferRtd {
        to: KeyIdentity::Address(address2),
        rtd_coin_object_id: object_id1,
        amount: Some(amount),
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
    ...
    // transfer rtd will transfer the amount from object_id1 to address2, and use the same object
...
async fn test_transfer_gas_smash() -> Result<(), anyhow::Error> {
    ...
    let RtdClientCommandResult::TransactionBlock(response) = transfer else {
        panic!("Transfer test failed");
    };
...
async fn test_transfer_sponsored() -> Result<(), anyhow::Error> {
    ...
    // A0 sends O1 to A1
    let transfer = RtdClientCommands::TransferRtd {
        to: KeyIdentity::Address(a1),
        rtd_coin_object_id: o[1],
        amount: None,
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
            ..Default::default()
    ...
    let RtdClientCommandResult::TransactionBlock(response) = transfer_back else {
...
async fn test_transfer_serialized_data() -> Result<(), anyhow::Error> {
    ...
    let RtdClientCommandResult::SerializedUnsignedTransaction(tx_data) = transfer else {
        panic!("Expected SerializedUnsignedTransaction result");
    };

    let tx_bytes = Base64::encode(bcs::to_bytes(&tx_data)?);
    let transfer_serialized = RtdClientCommands::SerializedTx {
        tx_bytes,
        processing: TxProcessingArgs::default(),
    }
    .execute(context)
    ...
    let RtdClientCommandResult::TransactionBlock(response) = transfer_serialized else {
...
async fn test_transfer_serialized_kind() -> Result<(), anyhow::Error> {
    ...
    let transfer_serialized = RtdClientCommands::SerializedTxKind {
        tx_bytes,
        payment: PaymentArgs { gas: vec![o[1]] },
        gas_data: GasDataArgs {
            gas_budget: Some(rgp * TEST_ONLY_GAS_UNIT_FOR_TRANSFER),
...
async fn test_gas_estimation() -> Result<(), anyhow::Error> {
    ...
    let tx_kind = tx_builder.transfer_rtd_tx_kind(address2, Some(amount));
    let gas_estimate = estimate_gas_budget(context, sender, tx_kind, rgp, vec![], None).await;
    assert!(gas_estimate.is_ok());

    let transfer_rtd_cmd = RtdClientCommands::TransferRtd {
        to: KeyIdentity::Address(address2),
        rtd_coin_object_id: object_id1,
        amount: Some(amount),
        gas_data: GasDataArgs::default(),
...
async fn test_custom_sender() -> Result<(), anyhow::Error> {
    ...
    let custom_sender = RtdAddress::random_for_testing_only();
...
async fn test_tree_shaking_package_deps_on_pkg_upgrade_1() -> Result<(), anyhow::Error> {
    ...
    // the old code for publishing a package from rtd-test-transaction-builder does not know about
...
async fn test_party_transfer() -> Result<(), anyhow::Error> {
    ...
    let party_transfer = RtdClientCommands::PartyTransfer {
        to: recipient1.clone(),
        object_id: object_id1,
        payment: PaymentArgs::default(),
        gas_data: GasDataArgs {
    ...
    let RtdClientCommandResult::TransactionBlock(response) = party_transfer else {
...
fn create_temp_dir_with_framework_packages(
    ...
    copy_dir_all(
        PathBuf::from("../rtd-framework/packages"),
