...
/// - Struct tags (e.g., `0x2::coin::Coin<0x2::rtd::RTD>>`)
...
mod tests {
    ...
    #[test]
    fn test_struct_tag() {
        let tag = StructTag::from_str("0x2::table::Table<address, 0x2::coin::Coin<0x2::rtd::RTD>>")
            .unwrap();
        let (bcs, bin) = expect(tag);

        assert_eq!(
            parse("bcs(0x2::table::Table<address, 0x2::coin::Coin<0x2::rtd::RTD>>)").unwrap(),
            bcs
        );
        assert_eq!(
            parse("bin(0x2::table::Table<address, 0x2::coin::Coin<0x2::rtd::RTD>>)").unwrap(),
            bin
        );
    }
}
