...
pub struct ProgrammableTransactionCommand {
    ...
    #[clap(
        long = "inputs",
        value_parser = ParsedValue::<RtdExtraValueArgs>::parse,
        num_args(1..),
        action = clap::ArgAction::Append,
    )]
    pub inputs: Vec<ParsedValue<RtdExtraValueArgs>>,
}
...
pub struct SetAddressCommand {
    pub address: String,
    #[clap(value_parser = ParsedValue::<RtdExtraValueArgs>::parse)]
    pub input: ParsedValue<RtdExtraValueArgs>,
}
...
impl<ExtraValueArgs: ParsableValue, ExtraRunArgs: Parser> clap::FromArgMatches
    for RtdSubcommand<ExtraValueArgs, ExtraRunArgs>
{
    fn from_arg_matches(matches: &clap::ArgMatches) -> Result<Self, clap::Error> {
        Ok(match matches.subcommand() {
            ...
            Some(("consensus-commit-prologue", matches)) => RtdSubcommand::ConsensusCommitPrologue(
                ConsensusCommitPrologueCommand::from_arg_matches(matches)?,
            ),
            Some(("programmable", matches)) => RtdSubcommand::ProgrammableTransaction(
                ProgrammableTransactionCommand::from_arg_matches(matches)?,
            ),
            Some(("upgrade", matches)) => {
                RtdSubcommand::UpgradePackage(UpgradePackageCommand::from_arg_matches(matches)?)
            }
            Some(("stage-package", matches)) => {
                RtdSubcommand::StagePackage(StagePackageCommand::from_arg_matches(matches)?)
            }
            Some(("set-address", matches)) => {
                RtdSubcommand::SetAddress(SetAddressCommand::from_arg_matches(matches)?)
            }
            Some(("create-checkpoint", matches)) => {
                RtdSubcommand::CreateCheckpoint(CreateCheckpointCommand::from_arg_matches(matches)?)
            }
            Some(("advance-epoch", matches)) => {
                RtdSubcommand::AdvanceEpoch(AdvanceEpochCommand::from_arg_matches(matches)?)
            }
            Some(("advance-clock", matches)) => {
                RtdSubcommand::AdvanceClock(AdvanceClockCommand::from_arg_matches(matches)?)
            }
            Some(("set-random-state", matches)) => {
                RtdSubcommand::SetRandomState(SetRandomStateCommand::from_arg_matches(matches)?)
            }
            Some(("authenticator-state-update", matches)) => {
                RtdSubcommand::AuthenticatorStateUpdate(
...
impl RtdExtraValueArgs {
    fn parse_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(RtdExtraValueArgs::Object(fake_id, version))
    }
    ...
    fn parse_receiving_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(RtdExtraValueArgs::Receiving(fake_id, version))
    }
    ...
    fn parse_owned_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(RtdExtraValueArgs::Owned(fake_id, version))
    }
    ...
    fn parse_digest_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(RtdExtraValueArgs::Digest(package.to_owned()))
    }
    ...
    fn parse_receiving_or_object_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        let fake_id = if let Some(ValueToken::Comma) = parser.peek_tok() {
            ...
            u256_bytes.reverse();
            let address: RtdAddress = RtdAddress::from_bytes(&u256_bytes).unwrap();
...
impl RtdValue {
    fn assert_move_value(self) -> MoveValue {
        match self {
            RtdValue::MoveValue(v) => v,
            RtdValue::Object(_, _) => panic!("unexpected nested Rtd object in args"),
            RtdValue::ObjVec(_) => panic!("unexpected nested Rtd object vector in args"),
            RtdValue::Digest(_) => panic!("unexpected nested Rtd package digest in args"),
            RtdValue::Receiving(_, _) => panic!("unexpected nested Rtd receiving object in args"),
            RtdValue::Owned(_, _) => panic!("unexpected nested Rtd owned object in args"),
    ...
    fn assert_object(self) -> (FakeID, Option<SequenceNumber>) {
        match self {
            RtdValue::MoveValue(_) => panic!("unexpected nested non-object value in args"),
            RtdValue::Object(id, version) => (id, version),
            RtdValue::ObjVec(_) => panic!("unexpected nested Rtd object vector in args"),
            RtdValue::Digest(_) => panic!("unexpected nested Rtd package digest in args"),
            RtdValue::Receiving(_, _) => panic!("unexpected nested Rtd receiving object in args"),
            RtdValue::Owned(_, _) => panic!("unexpected nested Rtd owned object in args"),
    ...
    fn resolve_object(
        ...
        let obj_res = if let Some(v) = version {
            rtd_types::storage::ObjectStore::get_object_by_key(&*test_adapter.executor, &id, v)
        } else {
            rtd_types::storage::ObjectStore::get_object(&*test_adapter.executor, &id)
        };
    ...
    pub(crate) fn into_call_arg(self, test_adapter: &RtdTestAdapter) -> anyhow::Result<CallArg> {
        Ok(match self {
            ...
            RtdValue::MoveValue(v) => CallArg::Pure(v.simple_serialize().unwrap()),
            RtdValue::Receiving(fake_id, version) => {
                CallArg::Object(Self::receiving_arg(fake_id, version, test_adapter)?)
            }
            RtdValue::Owned(fake_id, version) => {
                CallArg::Object(Self::owned_arg(fake_id, version, test_adapter)?)
            }
            RtdValue::Shared(mutability, fake_id, version) => CallArg::Object(Self::shared_arg(
                fake_id,
                version,
                test_adapter,
                mutability,
            ...
            RtdValue::Withdraw(amount, type_tag) => {
...
impl ParsableValue for RtdExtraValueArgs {
    type ConcreteValue = RtdValue;
    ...
    fn move_value_into_concrete(v: MoveValue) -> anyhow::Result<Self::ConcreteValue> {
        Ok(RtdValue::MoveValue(v))
    }

    fn concrete_vector(elems: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        if !elems.is_empty() && matches!(elems[0], RtdValue::Object(_, _)) {
            Ok(RtdValue::ObjVec(
                elems.into_iter().map(RtdValue::assert_object).collect(),
            ))
        } else {
            Ok(RtdValue::MoveValue(MoveValue::Vector(
                elems.into_iter().map(RtdValue::assert_move_value).collect(),
            )))
        }
    }

    fn concrete_struct(values: Vec<Self::ConcreteValue>) -> anyhow::Result<Self::ConcreteValue> {
        Ok(RtdValue::MoveValue(MoveValue::Struct(MoveStruct(
    ...
    fn into_concrete_value(
        ...
        match self {
            RtdExtraValueArgs::Object(id, version) => Ok(RtdValue::Object(id, version)),
            RtdExtraValueArgs::Digest(pkg) => Ok(RtdValue::Digest(pkg)),
            RtdExtraValueArgs::Receiving(id, version) => Ok(RtdValue::Receiving(id, version)),
            RtdExtraValueArgs::Owned(id, version) => Ok(RtdValue::Owned(id, version)),
            RtdExtraValueArgs::Shared(mutability, id, version) => {
                Ok(RtdValue::Shared(mutability, id, version))
            }
