...
use rtd_framework::DEFAULT_FRAMEWORK_PATH;
use rtd_graphql_rpc::test_infra::cluster::{RetentionConfig, SnapshotLagConfig};
use rtd_json_rpc_api::QUERY_MAX_RESULT_LIMIT;
use rtd_json_rpc_types::{
    DevInspectResults, DryRunTransactionBlockResponse, RtdAccumulatorOperation, RtdExecutionStatus,
    RtdTransactionBlockEffects, RtdTransactionBlockEffectsAPI, RtdTransactionBlockEvents,
};
...
use rtd_swarm_config::network_config_builder::KeyPairWrapper;
use rtd_types::base_types::{SequenceNumber, VersionNumber};
...
use rtd_types::digests::{ConsensusCommitDigest, TransactionDigest};
use rtd_types::effects::{
    AccumulatorOperation, TransactionEffects, TransactionEffectsAPI, TransactionEvents,
};
use rtd_types::execution_status::ExecutionFailureStatus;
...
use rtd_types::storage::{ObjectStore, RpcStateReader};
use rtd_types::transaction::Command;
use rtd_types::transaction::ProgrammableTransaction;
use rtd_types::utils::to_sender_signed_transaction_with_multi_signers;
use rtd_types::{BRIDGE_ADDRESS, MOVE_STDLIB_PACKAGE_ID};
use rtd_types::{DEEPBOOK_ADDRESS, RTD_DENY_LIST_OBJECT_ID};
use rtd_types::{DEEPBOOK_PACKAGE_ID, RTD_RANDOMNESS_STATE_OBJECT_ID};
use rtd_types::{
    MOVE_STDLIB_ADDRESS, RTD_CLOCK_OBJECT_ID, RTD_FRAMEWORK_ADDRESS, RTD_SYSTEM_STATE_OBJECT_ID,
    base_types::{ObjectID, ObjectRef, RTD_ADDRESS_LENGTH, RtdAddress},
...
use rtd_types::{
    RTD_COIN_REGISTRY_OBJECT_ID, RTD_FRAMEWORK_PACKAGE_ID, RTD_SYSTEM_ADDRESS,
    programmable_transaction_builder::ProgrammableTransactionBuilder,
};
use rtd_types::{RTD_SYSTEM_PACKAGE_ID, utils::to_sender_signed_transaction};
use rtd_types::{execution_status::ExecutionStatus, transaction::TransactionKind};
use rtd_types::{gas::GasCostSummary, object::GAS_VALUE_FOR_TESTING};
use rtd_types::{
...
pub struct OffChainConfig {
    ...
    /// URL for the Rtd REST API. To be passed to the offchain indexer if it uses the REST API.
    pub rest_api_url: Option<String>,
}

pub struct RtdTestAdapter {
...
struct TxnSummary {
    ...
    events: Vec<Event>,
    accumulators_written: Vec<(ObjectID, RtdAddress, TypeTag, AccumulatorOperation)>,
    gas_summary: GasCostSummary,
}

impl AdapterInitConfig {
    fn from_args(init_cmd: InitCommand, rtd_args: RtdInitArgs) -> Self {
...
#[async_trait]
impl MoveTestAdapter<'_> for RtdTestAdapter {
    type ExtraPublishArgs = RtdPublishArgs;
    type ExtraRunArgs = RtdRunArgs;
    type ExtraInitArgs = RtdInitArgs;
    type ExtraValueArgs = RtdExtraValueArgs;
    type Subcommand = RtdSubcommand<Self::ExtraValueArgs, Self::ExtraRunArgs>;
    ...
    fn render_command_input(
        ...
        match &task.command {
            TaskCommand::Subcommand(RtdSubcommand::ProgrammableTransaction(..)) => {
    ...
    async fn init(
        ...
        assert!(
            ...
            "Must populate 'pre_compiled_deps' with Rtd framework"
        );
        ...
        let AdapterInitConfig {
            ...
            Some((init_cmd, rtd_args)) => AdapterInitConfig::from_args(init_cmd, rtd_args),
    ...
    async fn publish_modules(
        ...
        // we are assuming that all packages depend on Move Stdlib and Rtd Framework, so these
        // don't have to be provided explicitly as parameters
        dependencies.extend([MOVE_STDLIB_PACKAGE_ID, RTD_FRAMEWORK_PACKAGE_ID]);
    ...
    async fn call_function(
        ...
        self.next_task();
        let RtdRunArgs { summarize, .. } = extra;
    ...
    async fn handle_subcommand(
        ...
        match command {
            RtdSubcommand::RunGraphql(RunGraphqlCommand {
                show_usage,
                show_headers,
            ...
            RtdSubcommand::RunJsonRpc(RunJsonRpcCommand {
                show_headers,
                cursors,
            }) => {
            ...
            RtdSubcommand::CreateCheckpoint(CreateCheckpointCommand { count }) => {
            ...
            RtdSubcommand::AdvanceEpoch(cmd) => {
            ...
            RtdSubcommand::AdvanceClock(advance_clock_command) => {
            ...
            RtdSubcommand::SetRandomState(SetRandomStateCommand {
                randomness_round,
                random_bytes,
            ...
            RtdSubcommand::AuthenticatorStateUpdate(AuthenticatorStateUpdateCommand {
                round,
                jwk_iss,
            ...
            RtdSubcommand::ViewObject(ViewObjectCommand {
                id: fake_id,
                hide_contents,
            }) => {
            ...
            RtdSubcommand::TransferObject(TransferObjectCommand {
                id: fake_id,
                recipient,
                sender,
                gas_budget,
                ...
                let mut builder = ProgrammableTransactionBuilder::new();
                let obj_arg = RtdValue::Object(fake_id, None).into_argument(&mut builder, self)?;
                ...
                let transaction = self.sign_txn(sender, |sender, gas| {
                    ...
                    builder.command(rtd_types::transaction::Command::TransferObjects(
                        vec![obj_arg],
                        rec_arg,
                    ));
                    let pt = builder.finish();
            ...
            RtdSubcommand::ConsensusCommitPrologue(ConsensusCommitPrologueCommand {
                timestamp_ms,
            }) => {
            ...
            RtdSubcommand::ProgrammableTransaction(ProgrammableTransactionCommand {
                sender,
                sponsor,
                gas_budget,
                gas_price,
            ...
            RtdSubcommand::UpgradePackage(UpgradePackageCommand {
                package,
                upgrade_capability,
                dependencies,
                sender,
            ...
            RtdSubcommand::StagePackage(StagePackageCommand {
                syntax,
                dependencies,
            }) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
            ...
            RtdSubcommand::SetAddress(SetAddressCommand { address, input }) => {
                ...
                let (value, package) = match input {
                    ...
                    RtdValue::MoveValue(v) => {
                        let bytes = v.simple_serialize().unwrap();
                        let value: AccountAddress = bcs::from_bytes(&bytes)?;
                        (value, None)
                    }
                    RtdValue::Digest(_) => bail!("digest is not supported as an input"),
                    RtdValue::ObjVec(_) => bail!("obj vec is not supported as an input"),
                    RtdValue::Receiving(_, _) => bail!("receiving is not supported as an input"),
                    RtdValue::Owned(_, _) => bail!("owned object is not supported as an input"),
                    RtdValue::Shared(_, _, _) => {
...
impl RtdTestAdapter {
    ...
    async fn upgrade_package(
        ...
        // Argument::Input(0)
        RtdValue::Object(upgrade_capability, None).into_argument(&mut builder, self)?;
    ...
    fn tx_summary_from_effects(
        &mut self,
        effects: RtdTransactionBlockEffects,
        events: RtdTransactionBlockEvents,
    ) -> anyhow::Result<TxnSummary> {
        ...
        let mut accumulators_written: Vec<_> = accumulator_events
            ...
            .map(|event| {
                let operation = match event.operation {
                    RtdAccumulatorOperation::Merge => AccumulatorOperation::Merge,
                    RtdAccumulatorOperation::Split => AccumulatorOperation::Split,
                };
                (
    ...
    fn list_accumulator_events(
        &self,
        accumulators: &[(ObjectID, RtdAddress, TypeTag, AccumulatorOperation)],
        summarize: bool,
    ) -> String {
        if summarize {
    ...
    fn stabilize_str(&self, input: impl AsRef<str>) -> String {
        fn candidate_is_hex(s: &str) -> bool {
            const HEX_STR_LENGTH: usize = RTD_ADDRESS_LENGTH * 2;
            let n = s.len();
...
impl<'a> GetModule for &'a RtdTestAdapter {
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    // TODO fix Rtd framework constants
    map.insert(
        "rtd".to_string(),
        NumericalAddress::new(
            RTD_FRAMEWORK_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
        "rtd_system".to_string(),
        NumericalAddress::new(
            RTD_SYSTEM_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
...
pub static PRE_COMPILED: Lazy<PreCompiledProgramInfo> = Lazy::new(|| {
    ...
    let rtd_files: &Path = Path::new(DEFAULT_FRAMEWORK_PATH);
    let rtd_system_sources = {
        let mut buf = rtd_files.to_path_buf();
        buf.extend(["packages", "rtd-system", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let rtd_sources = {
        let mut buf = rtd_files.to_path_buf();
        buf.extend(["packages", "rtd-framework", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let rtd_deps = {
        let mut buf = rtd_files.to_path_buf();
    ...
    let pre_compiled_program = move_compiler::construct_pre_compiled_lib(
        vec![PackagePaths {
            name: Some(("rtd-framework".into(), config)),
            paths: vec![
                rtd_system_sources,
                rtd_sources,
                rtd_deps,
    ...
    match pre_compiled_program {
        Err((files, diags)) => {
            eprintln!("!!!Rtd framework failed to compile!!!");
...
async fn init_val_fullnode_executor(
    ...
    // For each named Rtd account without an address value, create an account with an address
    // and a gas object
...
async fn init_sim_executor(
    ...
    // For each named Rtd account without an address value, create a key pair
    for n in account_names {
    ...
    let addr_keys: Vec<_> = (0..num_custom_validator_accounts)
        ...
        .map(|_| {
            let (addr, account_key_pair): (RtdAddress, AccountKeyPair) =
...
impl ObjectStore for RtdTestAdapter {
...
impl ReadStore for RtdTestAdapter {
    ...
    fn get_committee(
        &self,
        epoch: rtd_types::committee::EpochId,
