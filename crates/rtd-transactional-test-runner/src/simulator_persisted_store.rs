...
use rtd_swarm_config::network_config_builder::{ConfigBuilder, KeyPairWrapper};
...
pub struct PersistedStoreInner {
    // Checkpoint data
    checkpoints: DBMap<CheckpointSequenceNumber, rtd_types::messages_checkpoint::TrustedCheckpoint>,
    ...
    // Transaction data
    transactions: DBMap<TransactionDigest, rtd_types::transaction::TrustedTransaction>,
...
impl SimulatorStore for PersistedStore {
    ...
    fn get_clock(&self) -> rtd_types::clock::Clock {
        SimulatorStore::get_object(self, &rtd_types::RTD_CLOCK_OBJECT_ID)
...
impl ReadStore for PersistedStoreInnerReadOnlyWrapper {
    ...
    fn get_latest_checkpoint(&self) -> rtd_types::storage::error::Result<VerifiedCheckpoint> {
        ...
        self.inner
            ...
            .map_err(rtd_types::storage::error::Error::custom)
    }
