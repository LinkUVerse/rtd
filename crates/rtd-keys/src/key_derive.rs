...
use rtd_types::{
    ...
    crypto::{SignatureScheme, RtdKeyPair},
...
) -> Result<(RtdAddress, RtdKeyPair), RtdError> {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| RtdErrorKind::SignatureKeyGenError(e.to_string()))?;
            let kp: Ed25519KeyPair = sk.into();
            Ok((kp.public().into(), RtdKeyPair::Ed25519(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), RtdKeyPair::Secp256k1(kp)))
        }
        SignatureScheme::Secp256r1 => {
            ...
            Ok((kp.public().into(), RtdKeyPair::Secp256r1(kp)))
        }
        SignatureScheme::BLS12381
        | SignatureScheme::MultiSig
        | SignatureScheme::ZkLoginAuthenticator
        | SignatureScheme::PasskeyAuthenticator => Err(RtdErrorKind::UnsupportedFeatureError {
...
) -> Result<DerivationPath, RtdError> {
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                Some(p) => {
                    ...
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                            ...
                            && address.is_hardened()
                        {
                            ...
                            Err(
                                RtdErrorKind::SignatureKeyGenError("Invalid path".to_string())
                                    .into(),
                            )
                        }
                    } else {
                        Err(RtdErrorKind::SignatureKeyGenError("Invalid path".to_string()).into())
                    }
                }
                None => Ok(format!(
                ...
                .parse()
                .map_err(|_| {
                    RtdErrorKind::SignatureKeyGenError("Cannot parse path".to_string())
                })?),
            }
        }
        SignatureScheme::Secp256k1 => {
...
) -> Result<(RtdAddress, RtdKeyPair, SignatureScheme, String), anyhow::Error> {
