...
pub trait AccountKeystore: Send + Sync {
    ...
    /// Import a keypair into the keystore from a `RtdKeyPair` and optional alias.
    async fn import(
        &mut self,
        alias: Option<String>,
        keypair: RtdKeyPair,
    ...
    async fn remove(&mut self, address: RtdAddress) -> Result<(), anyhow::Error>;
    /// Return an array of `PublicKey`, consisting of every public key in the keystore.
    fn entries(&self) -> Vec<PublicKey>;
    /// Return `RtdKeyPair` for the given address.
    fn export(&self, address: &RtdAddress) -> Result<&RtdKeyPair, anyhow::Error>;
    ...
    /// Return an array of `RtdAddress`, consisting of every address in the keystore.
    fn addresses(&self) -> Vec<RtdAddress> {
        self.entries().iter().map(|k| k.into()).collect()
    }
    /// Return an array of (&RtdAddress, &Alias), consisting of every address and its corresponding alias in the keystore.
    ...
    fn get_by_identity(&self, key_identity: &KeyIdentity) -> Result<RtdAddress, anyhow::Error> {
        match key_identity {
...
pub struct FileBasedKeystore {
    keys: BTreeMap<RtdAddress, RtdKeyPair>,
    aliases: BTreeMap<RtdAddress, Alias>,
    path: Option<PathBuf>,
}
...
impl AccountKeystore for FileBasedKeystore {
    ...
    async fn import(
        ...
        let address: RtdAddress = (&keypair.public()).into();
...
impl FileBasedKeystore {
    pub fn load_or_create(path: &PathBuf) -> Result<Self, anyhow::Error> {
        let keys = if path.exists() {
            ...
            kp_strings
                .iter()
                .map(|kpstr| {
                    let key = RtdKeyPair::decode_base64(kpstr);
                    key.map(|k| (RtdAddress::from(&k.public()), k))
                })
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<RtdAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
    ...
    /// To see Bech32 format encoding, use `rtd keytool export $RTD_ADDRESS` where
    /// $RTD_ADDRESS can be found with `rtd keytool list`. Or use `rtd keytool convert $BASE64_STR`
    ...
    pub fn key_pairs(&self) -> Vec<&RtdKeyPair> {
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, RtdKeyPair::Ed25519(k)))
            .collect::<BTreeMap<RtdAddress, RtdKeyPair>>();
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.rtd").is_ok());
        // Not allowed
