...
pub struct External {
    ...
    pub aliases: BTreeMap<RtdAddress, Alias>,
    // Holds a map of addresses to [`StoredKey`]
    pub keys: BTreeMap<RtdAddress, StoredKey>,
...
pub struct StoredKey {
    ...
    /// External signer binary (e.g., "rtd-ledger-signer")
...
impl External {
    ...
    pub fn load_or_create(path: &PathBuf) -> Result<Self, Error> {
        ...
        let aliases: BTreeMap<RtdAddress, Alias> = if aliases_store_directory.exists() {
        ...
        let keys: BTreeMap<RtdAddress, StoredKey> = if path.exists() {
    ...
    /// Add a Key ID from the given an external signer to the Rtd CLI index
    ...
    pub fn is_indexed(&self, key: &StoredKey) -> bool {
        self.keys.contains_key(&RtdAddress::from(&key.public_key))
    }
...
impl AccountKeystore for External {
    async fn generate(
        ...
        let address: RtdAddress = (&public_key).into();
    ...
    async fn import(&mut self, _alias: Option<String>, _keypair: RtdKeyPair) -> Result<(), Error> {
        Err(anyhow!("Import not supported for external keys."))
    }

    async fn remove(&mut self, address: RtdAddress) -> Result<(), Error> {
    ...
    /// Export the key pair for the given address as a `RtdKeyPair`.
    fn export(&self, _address: &RtdAddress) -> Result<&RtdKeyPair, Error> {
    ...
    async fn sign_hashed(
        ...
        let address = RtdAddress::from(&public_key);
        match &result.signature {
            Signature::Ed25519RtdSignature(s) => {
                ...
                let signature_address = RtdAddress::from(&pk);
            ...
            Signature::Secp256k1RtdSignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
            ...
            Signature::Secp256r1RtdSignature(s) => {
                let (sig, pk) = s.get_verification_inputs().map_err(|e| {
                    signature::Error::from_source(anyhow!(
                        "Failed to get verification inputs: {}",
                        e
    ...
    fn addresses_with_alias(&self) -> Vec<(&RtdAddress, &Alias)> {
    ...
    fn get_alias(&self, address: &RtdAddress) -> Result<String, anyhow::Error> {
...
mod tests {
    ...
    async fn test_serialize() {
        ...
        external.keys.insert(
            RtdAddress::from_str(ADDRESS).unwrap(),
            StoredKey {
    ...
    async fn test_exec() {
        ...
        assert!(
            external
                .exec("rtd-key-tool", "test_method", args)
                .await
                .is_ok()
        );
    }
    ...
    async fn test_create_key_success() {
        ...
        assert_eq!(address, RtdAddress::from_str(ADDRESS).unwrap());
    ...
    async fn test_add_existing_key() {
        ...
        let key = keys.get(&RtdAddress::from_str(ADDRESS).expect("Invalid address format"));
        assert!(key.is_some());
    }
    ...
    async fn test_add_keypair_not_supported() {
        ...
        let result = external
            .import(None, RtdKeyPair::Ed25519(ed25519_keypair))
            .await;
    ...
    async fn test_remove_key() {
        ...
        let address = RtdAddress::from_str(ADDRESS).unwrap();
        external.keys.insert(
            address,
            StoredKey {
                public_key: PublicKey::decode_base64(PUBLIC_KEY).unwrap(),
    ...
    fn test_get_key_fails() {
        ...
        let result = external.export(&RtdAddress::from_str(ADDRESS).unwrap());
        assert!(result.is_err())
    }

    #[tokio::test]
    async fn test_sign_hashed() {
        let skp = RtdKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    ...
    async fn test_sign_secure() {
        ...
        let intent = Intent::rtd_transaction();
