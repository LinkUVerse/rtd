...
use rtd_types::crypto::{AuthorityKeyPair, NetworkKeyPair, RtdKeyPair, ToFromBytes};
...
/// Read from file as Base64 encoded `flag || privkey` and return a RtdKeypair.
pub fn read_keypair_from_file<P: AsRef<std::path::Path>>(path: P) -> anyhow::Result<RtdKeyPair> {
    let contents = std::fs::read_to_string(path)?;
    RtdKeyPair::decode_base64(contents.as_str().trim()).map_err(|e| anyhow!(e))
}
...
pub fn read_network_keypair_from_file<P: AsRef<std::path::Path>>(
    ...
    if let RtdKeyPair::Ed25519(kp) = kp {
        Ok(kp)
    } else {
...
/// Read a RtdKeyPair from a file. The content could be any of the following:
/// - Base64 encoded `flag || privkey` for ECDSA key
/// - Base64 encoded `privkey` for Raw key
/// - Bech32 encoded private key prefixed with `rtdprivkey`
/// - Hex encoded `privkey` for Raw key
///
/// If `require_secp256k1` is true, it will return an error if the key is not Secp256k1.
pub fn read_key(path: &PathBuf, require_secp256k1: bool) -> Result<RtdKeyPair, anyhow::Error> {
    if !path.exists() {
    ...
    // Try base64 encoded RtdKeyPair `flag || privkey`
    if let Ok(key) = RtdKeyPair::decode_base64(contents) {
        if require_secp256k1 && !matches!(key, RtdKeyPair::Secp256k1(_)) {
    ...
    if let Ok(key) = Secp256k1KeyPair::decode_base64(contents) {
        return Ok(RtdKeyPair::Secp256k1(key));
    }

    // Try Bech32 encoded 33-byte `flag || private key` starting with `rtdprivkey`A prefix.
    // This is the format of a private key exported from Rtd Wallet or rtd.keystore.
    if let Ok(key) = RtdKeyPair::decode(contents) {
        if require_secp256k1 && !matches!(key, RtdKeyPair::Secp256k1(_)) {
            return Err(anyhow!("Key is not Secp256k1"));
        }
        return Ok(key);
