...
async fn alias_exists_test() {
    ...
    let keystore_path = temp_dir.path().join("rtd.keystore");
...
async fn create_alias_keystore_file_test() {
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("rtd.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
    keystore
...
async fn check_reading_aliases_file_correctly() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("rtd.keystore");
    let keystore_path_keep = temp_dir.path().join("rtd.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
...
fn keystore_no_aliases() {
    ...
    // rtd.aliases file with the existing address in the rtd.keystore,
...
async fn mnemonic_test() {
    ...
    let keystore_path_2 = temp_dir.path().join("rtd2.keystore");
    let mut keystore2 =
    ...
    let imported_address = keystore2
        ...
        .await
        .unwrap();
    assert_eq!(scheme.flag(), Ed25519RtdSignature::SCHEME.flag());
    assert_eq!(address, imported_address);
}

/// This test confirms rust's implementation of mnemonic is the same with the Rtd Wallet
#[tokio::test]
async fn rtd_wallet_address_mnemonic_test() -> Result<(), anyhow::Error> {
    let phrase = "result crisp session latin must fruit genuine question prevent start coconut brave speak student dismiss";
    let expected_address =
        RtdAddress::from_str("0x936accb491f0facaac668baaedcf4d0cfc6da1120b66f77fa6a43af718669973")?;
    ...
    assert_eq!(pubkey.flag(), Ed25519RtdSignature::SCHEME.flag());

    let mut hasher = DefaultHash::default();
    hasher.update([pubkey.flag()]);
    hasher.update(pubkey);
    let g_arr = hasher.finalize();
    let mut res = [0u8; RTD_ADDRESS_LENGTH];
    res.copy_from_slice(&AsRef::<[u8]>::as_ref(&g_arr)[..RTD_ADDRESS_LENGTH]);
    let address = RtdAddress::try_from(res.as_slice())?;
...
fn keystore_display_test() -> Result<(), anyhow::Error> {
    ...
    let keystore_path = temp_dir.path().join("rtd.keystore");
    let keystore = Keystore::from(FileBasedKeystore::load_or_create(&keystore_path).unwrap());
    assert!(keystore.to_string().contains("rtd.keystore"));
