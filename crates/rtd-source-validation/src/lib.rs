...
use rtd_sdk::error::Error as SdkError;
use rtd_sdk::rpc_types::{RtdObjectDataOptions, RtdRawData, RtdRawMovePackage};
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    async fn pkg_for_address(&self, addr: AccountAddress) -> Result<RtdRawMovePackage, Error> {
        // Move packages are specified with an AccountAddress, but are
        // fetched from a rtd network via rtd_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the Rtd object at the address specified for the package in the local resolution table
        ...
        let obj_read = self
            .rpc_client
            .get_object_with_options(obj_id, RtdObjectDataOptions::new().with_bcs())
            .await
            .map_err(Error::DependencyObjectReadFailure)?;

        let obj = obj_read
            .into_object()
            .map_err(Error::RtdObjectRefFailure)?
            .bcs
            .ok_or_else(|| {
        ...
        match obj {
            ...
            RtdRawData::MoveObject(move_obj) => {
