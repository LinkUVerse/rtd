...
// Re-export things used by rtd-macros
...
pub use linku_network;
pub use rtd_framework;
pub use rtd_move_build;
pub use rtd_types;
...
pub mod configs {
    ...
    /// Select from among a number of configs using the RTD_SIM_CONFIG env var.
    pub fn env_config(
        // Config to use when RTD_SIM_CONFIG is not set.
        default: SimConfig,
        // List of (&str, SimConfig) pairs - the SimConfig associated with the value
        // of the RTD_SIM_CONFIG var is chosen.
        ...
        if let Some(env) = std::env::var("RTD_SIM_CONFIG").ok() {
            if let Some(cfg) = env_configs.remove(env.as_str()) {
                info!("Using test config for RTD_SIM_CONFIG={}", env);
                cfg
            } else {
                panic!(
                    "No config found for RTD_SIM_CONFIG={}. Available configs are: {:?}",
                    env,
                    env_configs.keys()
                );
            }
...
pub fn has_mainnet_protocol_config_override() -> bool {
    use rtd_types::{digests::ChainIdentifier, supported_protocol_versions::Chain};
