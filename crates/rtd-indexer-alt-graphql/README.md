...
`rtd-indexer-alt`, as long as all the required tables are present (all
pipelines are enabled on the indexer). See the indexer's
[README](../rtd-indexer-alt/README.md) for details on how to set it up.
...
```sh
cargo nextest run -p rtd-indexer-alt-graphql
cargo nextest run -p rtd-indexer-alt-reader
cargo nextest run -p rtd-indexer-alt-e2e-tests -- graphql
```
...
`rtd-indexer-alt`'s stores), the final test is an end-to-end test rtdte that
...
```sh
cargo nextest run -p rtd-indexer-alt-graphql -- test_schema_sdl_export
cargo nextest run -p rtd-indexer-alt-graphql --features staging -- test_schema_sdl_export
cargo insta review
```
...
```sh
cargo run --bin rtd-indexer-alt-graphql -- generate-config > graphql_config.toml
```
...
```sh
cargo run --bin rtd-indexer-alt-graphql -- rpc \
