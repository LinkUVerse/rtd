...
impl TypeFilter {
    ...
    pub(crate) fn package(&self) -> RtdAddress {
        match self {
...
impl FromStr for TypeFilter {
    ...
    fn from_str(s: &str) -> Result<Self, TypeFilterError> {
        ...
        } else if let Ok(module) = parse_rtd_module_id(s) {
...
mod tests {
    ...
    fn test_parse_type_filter() {
        ...
        let filter = TypeFilter::from_str("0x2::coin::Coin<0x2::rtd::RTD>").unwrap();
        assert!(matches!(filter, TypeFilter::Type(ref t) if !t.type_params.is_empty()));
    }
    ...
    fn test_type_filter_intersect() {
        ...
        let type_with_params = TypeFilter::from_str("0x2::coin::Coin<0x2::rtd::RTD>").unwrap();
