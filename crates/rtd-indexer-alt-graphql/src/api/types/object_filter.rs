...
use crate::{
    api::scalars::{owner_kind::OwnerKind, rtd_address::RtdAddress, type_filter::TypeFilter},
    intersect,
};
...
mod tests {
    ...
    fn test_type_filter_intersection() {
        ...
        let rtd = ObjectFilter {
            owner_kind: None,
            owner: None,
            type_: Some("0x2::coin::Coin<0x2::rtd::RTD>".parse().unwrap()),
        };
        ...
        // intersection order.
        assert_eq!(coin.clone().intersect(rtd.clone()).unwrap(), rtd);
        assert_eq!(rtd.clone().intersect(coin.clone()).unwrap(), rtd);
    ...
    fn test_combined_intersection() {
        ...
        assert_eq!(a_coin.clone().intersect(a_rtd.clone()).unwrap(), a_rtd);
