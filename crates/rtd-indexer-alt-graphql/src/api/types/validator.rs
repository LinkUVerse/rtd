...
use rtd_types::rtd_system_state::rtd_system_state_inner_v1::ValidatorV1;

use crate::{
    api::{
        scalars::{
            base64::Base64, big_int::BigInt, cursor::JsonCursor, rtd_address::RtdAddress,
...
impl Validator {
    ...
    async fn staking_pool_rtd_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(self.validator().staking_pool.rtd_balance))
    }
    ...
    async fn pending_total_rtd_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator().staking_pool.pending_total_rtd_withdraw,
        ))
    }
    ...
    async fn at_risk(&self) -> Option<UInt53> {
        let at_risk = self
            ...
            .native
            .at_risk_validators
            .get(&self.validator().metadata.rtd_address)
...
impl Validator {
    fn super_(&self) -> Address {
        Address::with_address(
            self.contents.scope.clone(),
            self.validator().metadata.rtd_address,
        )
    }
