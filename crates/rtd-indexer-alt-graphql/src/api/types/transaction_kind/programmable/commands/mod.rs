...
use async_graphql::*;
use rtd_types::transaction::Command as NativeCommand;

use crate::api::{
    scalars::{base64::Base64, rtd_address::RtdAddress},
    ...
    pub fn from(scope: Scope, command: NativeCommand) -> Self {
        match command {
            ...
            NativeCommand::Publish(modules, dependencies) => Command::Publish(PublishCommand {
                ...
                dependencies: Some(dependencies.into_iter().map(RtdAddress::from).collect()),
            }),
            NativeCommand::Upgrade(modules, dependencies, current_package, upgrade_ticket) => {
                Command::Upgrade(UpgradeCommand {
                    modules: Some(modules.into_iter().map(Base64::from).collect()),
                    dependencies: Some(dependencies.into_iter().map(RtdAddress::from).collect()),
                    current_package: Some(RtdAddress::from(current_package)),
