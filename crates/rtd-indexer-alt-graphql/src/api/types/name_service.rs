...
use rtd_name_service::{Domain as NativeDomain, NameRecord, NameServiceConfig};
...
/// Attempt to translate the given RtdNS `domain` to its address, as long as the mapping exists,
...
/// Attempt to translate the given `address` to its default RtdNS name, as long as the reverse
...
async fn reverse_record(
    ...
    scope: Scope,
    address: RtdAddress,
) -> Result<Option<Field<RtdAddress, Domain>>, RpcError> {
    let config: &NameServiceConfig = ctx.data()?;
