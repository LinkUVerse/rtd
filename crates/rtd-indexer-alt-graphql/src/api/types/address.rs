...
use rtd_types::{base_types::RtdAddress as NativeRtdAddress, dynamic_field::DynamicFieldType};

use crate::{
    api::{
        scalars::{owner_kind::OwnerKind, rtd_address::RtdAddress, type_filter::TypeInput},
...
#[graphql(
    ...
    field(
        ...
        desc = "Fetch the total balance for coins with marker type `coinType` (e.g. `0x2::rtd::RTD`), owned by this address.\n\nIf the address does not own any coins of that type, a balance of zero is returned.",
    ),
    field(
        name = "balances",
...
impl Address {
    /// Construct an address that is represented by just its identifier (`RtdAddress`).
    /// This does not check whether the address is valid or exists in the system.
    pub(crate) fn with_address(scope: Scope, address: NativeRtdAddress) -> Self {
        Self { scope, address }
    }
}
