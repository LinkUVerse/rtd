...
use rtd_types::{
    base_types::RtdAddress as NativeRtdAddress,
    collection_types::{Entry, VecMap, VecSet},
    rtd_system_state::rtd_system_state_inner_v1::ValidatorSetV1,
};

use crate::{
    api::{
        scalars::{big_int::BigInt, cursor::JsonCursor, rtd_address::RtdAddress},
...
pub(crate) struct ValidatorSetContents {
    ...
    pub(crate) report_records: IndexMap<NativeRtdAddress, Vec<usize>>,
}
...
impl ValidatorSet {
    ...
    async fn inactive_pools_id(&self) -> Option<RtdAddress> {
    ...
    async fn pending_active_validators_id(&self) -> Option<RtdAddress> {
        Some(
            self.contents
                .native
    ...
    async fn staking_pool_mappings_id(&self) -> Option<RtdAddress> {
    ...
    async fn validator_candidates_id(&self) -> Option<RtdAddress> {
...
impl ValidatorSet {
    pub(crate) fn from_validator_set_v1(
        ...
        report_records: VecMap<NativeRtdAddress, VecSet<NativeRtdAddress>>,
    ) -> Self {
        let address_to_index: BTreeMap<_, _> = native
            .active_validators
            .iter()
            .enumerate()
            .map(|(i, v)| (v.metadata.rtd_address, i))
            .collect();
