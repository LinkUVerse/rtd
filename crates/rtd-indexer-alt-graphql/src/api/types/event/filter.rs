...
use rtd_pg_db::query::Query;
use rtd_sql_macro::query;
use rtd_types::event::Event as NativeEvent;

use crate::{
    api::{
        scalars::{
            module_filter::ModuleFilter, rtd_address::RtdAddress, type_filter::TypeFilter,
            uint53::UInt53,
        },
...
impl EventFilter {
    ...
    pub(crate) fn matches(&self, event: &NativeEvent) -> bool {
        if let Some(sender) = &self.sender
            && sender != &RtdAddress::from(event.sender)
        {
            return false;
        }

        if let Some(module) = &self.module {
            if module.package() != RtdAddress::from(event.package_id) {
                return false;
            }
        ...
        if let Some(type_) = &self.type_ {
            if type_.package() != RtdAddress::from(event.type_.address) {
                return false;
            }
