...
use rtd_indexer_alt_reader::cp_sequence_numbers::CpSequenceNumberKey;
use rtd_indexer_alt_reader::{
    epochs::{CheckpointBoundedEpochStartKey, EpochEndKey, EpochStartKey},
    pg_reader::PgReader,
};
use rtd_indexer_alt_schema::cp_sequence_numbers::StoredCpSequenceNumbers;
use rtd_indexer_alt_schema::epochs::{StoredEpochEnd, StoredEpochStart};
use rtd_types::RTD_DENY_LIST_OBJECT_ID;
use rtd_types::messages_checkpoint::CheckpointCommitment;
...
impl Epoch {
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>, RpcError> {
        ...
        let (validator_set_v1, report_records) = match system_state {
            RtdSystemState::V1(inner) => (inner.validators, inner.validator_report_records),
            RtdSystemState::V2(inner) => (inner.validators, inner.validator_report_records),
            #[cfg(msim)]
            RtdSystemState::SimTestV1(_)
            | RtdSystemState::SimTestShallowV2(_)
            | RtdSystemState::SimTestDeepV2(_) => return Ok(None),
        };
    ...
    async fn storage_fund(&self, ctx: &Context<'_>) -> Result<Option<StorageFund>, RpcError> {
        ...
        let storage_fund = match system_state {
            RtdSystemState::V1(inner) => inner.storage_fund.into(),
            RtdSystemState::V2(inner) => inner.storage_fund.into(),
            #[cfg(msim)]
            RtdSystemState::SimTestV1(_)
            | RtdSystemState::SimTestShallowV2(_)
            | RtdSystemState::SimTestDeepV2(_) => return Ok(None),
    ...
    async fn system_parameters(
        ...
        let system_parameters = match system_state {
            RtdSystemState::V1(inner) => from_system_parameters_v1(inner.parameters),
            RtdSystemState::V2(inner) => from_system_parameters_v2(inner.parameters),
            #[cfg(msim)]
            RtdSystemState::SimTestV1(_)
            | RtdSystemState::SimTestShallowV2(_)
            | RtdSystemState::SimTestDeepV2(_) => return Ok(None),
    ...
    async fn system_stake_subsidy(
        ...
        let stake_subsidy = match system_state {
            RtdSystemState::V1(inner) => from_stake_subsidy_v1(inner.stake_subsidy),
            RtdSystemState::V2(inner) => from_stake_subsidy_v1(inner.stake_subsidy),
            #[cfg(msim)]
            RtdSystemState::SimTestV1(_)
            | RtdSystemState::SimTestShallowV2(_)
            | RtdSystemState::SimTestDeepV2(_) => return Ok(None),
...
impl Epoch {
    ...
    async fn system_state(&self, ctx: &Context<'_>) -> Result<Option<RtdSystemState>, RpcError> {
        let Some(start) = self.start(ctx).await? else {
            return Ok(None);
        };

        let system_state = bcs::from_bytes::<RtdSystemState>(&start.system_state)
