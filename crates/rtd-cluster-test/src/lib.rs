...
impl TestContext {
    async fn get_rtd_from_faucet(&self, minimum_coins: Option<usize>) -> Vec<GasCoin> {
        let addr = self.get_wallet_address();

        let faucet_response = self.faucet.request_rtd_coins(addr).await;
        ...
        if gas_coins.len() < minimum_coins {
            panic!(
                "Expect to get at least {minimum_coins} Rtd Coins for address {addr}, but only got {}",
                gas_coins.len()
            )
        }
    ...
    fn get_fullnode_client(&self) -> &RtdClient {
        self.client.get_fullnode_client()
    }

    fn clone_fullnode_client(&self) -> RtdClient {
    ...
    async fn get_latest_rtd_system_state(&self) -> RtdSystemStateSummary {
        self.client
            .get_fullnode_client()
            .governance_api()
    ...
    fn get_wallet_address(&self) -> RtdAddress {
    ...
    async fn get_tx_with_retry_times(
        ...
        match self
            ...
            .get_fullnode_client()
            .read_api()
            .get_transaction_with_options(digest, RtdTransactionBlockResponseOptions::new())
            .await
        {
