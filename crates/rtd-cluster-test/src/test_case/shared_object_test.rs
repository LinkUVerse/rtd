...
use rtd_test_transaction_builder::{increment_counter, publish_basics_package_and_make_counter};
...
impl TestCaseImpl for SharedCounterTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        let rtd_objs = ctx.get_rtd_from_faucet(Some(1)).await;
        assert!(!rtd_objs.is_empty());
