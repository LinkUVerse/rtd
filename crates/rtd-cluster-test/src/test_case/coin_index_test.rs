...
use rtd_json_rpc_types::{Balance, RtdTransactionBlockResponseOptions};
...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        ctx.get_rtd_from_faucet(None).await;
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, rtd_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let args = vec![
            RtdJsonValue::from_object_id(cap.0),
            RtdJsonValue::new(json!("10000"))?,
            RtdJsonValue::new(json!(account))?,
        ];
        let txn = client
            .transaction_builder()
            .move_call(
        ...
        let rtd_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("RTD"))
            .unwrap();
        ...
        assert_eq!(rtd_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + rtd_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, rtd_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            rtd_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let args = vec![RtdJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    RtdJsonValue::from_object_id(cap.0),
                    RtdJsonValue::from_object_id(envelope.0),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    RtdJsonValue::from_object_id(cap.0),
                    RtdJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let rtd_coins = client
            .coin_read_api()
            .get_coins(account, Some(rtd_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only RTD coins at the moment
            rtd_coins,
            client
                .coin_read_api()
        ...
        let rtd_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            rtd_balance.total_balance,
            rtd_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    RtdJsonValue::from_object_id(cap.0),
                    RtdJsonValue::new(json!("5"))?,  // balance = 5
                    RtdJsonValue::new(json!("40"))?, // num = 40
                    RtdJsonValue::new(json!(account))?,
                ],
                None,
                rgp * 2_000_000,
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(rtd_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(rtd_coins.len() + managed_coins.len(), total_coins,);

        let rtd_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(rtd_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            rtd_coins_with_managed_coin_1.data.len(),
            rtd_coins.len() + 1
        );
        assert!(rtd_coins_with_managed_coin_1.has_next_page);
        let cursor = rtd_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                RtdJsonValue::from_object_id(envelope),
                RtdJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
            None,
