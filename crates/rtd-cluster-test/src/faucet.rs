...
use std::env;
use std::sync::Arc;
use rtd_faucet::{FaucetConfig, FaucetResponse, LocalFaucet, RequestStatus};
...
pub trait FaucetClient {
    async fn request_rtd_coins(&self, request_address: RtdAddress) -> FaucetResponse;
}
...
impl FaucetClient for RemoteFaucetClient {
    /// Request test RTD coins from faucet.
