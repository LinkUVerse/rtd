...
impl Cluster for LocalNewCluster {
    async fn start(options: &ClusterTestOpt) -> Result<Self, anyhow::Error> {
        ...
        if let Some(config_dir) = options.config_dir.clone() {
            ...
            let network_config_path = config_dir.join(RTD_NETWORK_CONFIG);
        ...
        let faucet_address = RtdAddress::from(faucet_key.public());
...
pub async fn new_wallet_context_from_cluster(
    ...
    let address: RtdAddress = key_pair.public().into();
    keystore
        .import(None, RtdKeyPair::Ed25519(key_pair))
        .await
        .unwrap();
    RtdClientConfig {
        keystore,
