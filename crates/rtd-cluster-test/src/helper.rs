...
use rtd_json_rpc_types::{BalanceChange, RtdData, RtdObjectData, RtdObjectDataOptions};
use rtd_sdk::RtdClient;
use rtd_types::error::RtdObjectResponseError;
use rtd_types::gas_coin::GasCoin;
use rtd_types::{base_types::ObjectID, object::Owner, parse_rtd_type_tag};
use tracing::{debug, trace};

/// A util struct that helps verify Rtd Object.
...
/// `check_into_gas_coin` expect to get a `RtdObjectData` and `GasCoin`
/// respectfully.
#[derive(Debug)]
...
impl ObjectChecker {
    ...
    pub fn is_rtd_coin(mut self, is_rtd_coin: bool) -> Self {
        self.is_rtd_coin = Some(is_rtd_coin);
        self
    }

    pub async fn check_into_gas_coin(self, client: &RtdClient) -> GasCoin {
        if self.is_rtd_coin == Some(false) {
            panic!("'check_into_gas_coin' shouldn't be called with 'is_rtd_coin' set as false");
        }
        self.is_rtd_coin(true)
            .check(client)
            .await
            .unwrap()
            .into_gas_coin()
    ...
    pub async fn check_into_object(self, client: &RtdClient) -> RtdObjectData {
        self.check(client).await.unwrap().into_object()
    }

    pub async fn check(self, client: &RtdClient) -> Result<CheckerResultObject, anyhow::Error> {
        debug!(?self);
        ...
        match (object_info.data, object_info.error) {
            (None, Some(RtdObjectResponseError::NotExists { object_id })) => {
                panic!(
            ...
            (
                None,
                Some(RtdObjectResponseError::DynamicFieldNotFound {
            ...
            (None, Some(RtdObjectResponseError::DisplayError { error })) => {
                panic!("Display Error: {error:?}");
            }
            (None, None) | (None, Some(RtdObjectResponseError::Unknown)) => {
...
pub struct CheckerResultObject {
    ...
    object: Option<RtdObjectData>,
}

impl CheckerResultObject {
    pub fn new(gas_coin: Option<GasCoin>, object: Option<RtdObjectData>) -> Self {
        Self { gas_coin, object }
    }
    pub fn into_gas_coin(self) -> GasCoin {
        self.gas_coin.unwrap()
    }
    pub fn into_object(self) -> RtdObjectData {
        self.object.unwrap()
    }
}
...
impl BalanceChangeChecker {
    ...
    pub fn coin_type(mut self, coin_type: &str) -> Self {
        self.coin_type = Some(parse_rtd_type_tag(coin_type).unwrap());
        self
    }
