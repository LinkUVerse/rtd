...
use rtd_core::authority::backpressure::BackpressureManager;
use rtd_core::authority::epoch_start_configuration::EpochFlag;
use rtd_core::authority::execution_time_estimator::ExecutionTimeObserver;
use rtd_core::authority::shared_object_version_manager::Schedulable;
use rtd_core::consensus_adapter::ConsensusClient;
use rtd_core::consensus_manager::UpdatableConsensusClient;
use rtd_core::epoch::randomness::RandomnessManager;
...
use rtd_core::storage::RestReadStore;
use rtd_json_rpc::bridge_api::BridgeReadApi;
use rtd_json_rpc_api::JsonRpcMetrics;
use rtd_network::randomness;
use rtd_rpc_api::RpcMetrics;
use rtd_rpc_api::ServerVersion;
use rtd_rpc_api::subscription::SubscriptionService;
...
pub use handle::RtdNodeHandle;
use linku_metrics::{RegistryService, spawn_monitored_task};
use linku_service::server_timing::server_timing_middleware;
use rtd_config::node::{DBCheckpointConfig, RunWithRange};
use rtd_config::node::{ForkCrashBehavior, ForkRecoveryConfig};
use rtd_config::node_config_metrics::NodeConfigMetrics;
...
use rtd_core::authority::epoch_start_configuration::EpochStartConfiguration;
use rtd_core::authority::submitted_transaction_cache::SubmittedTransactionCacheMetrics;
use rtd_core::authority_aggregator::{AuthAggMetrics, AuthorityAggregator};
use rtd_core::authority_server::{ValidatorService, ValidatorServiceMetrics};
use rtd_core::checkpoints::checkpoint_executor::metrics::CheckpointExecutorMetrics;
use rtd_core::checkpoints::checkpoint_executor::{CheckpointExecutor, StopReason};
use rtd_core::checkpoints::{
    CheckpointMetrics, CheckpointService, CheckpointStore, SendCheckpointToStateSync,
    SubmitCheckpointToConsensus,
};
use rtd_core::consensus_adapter::{
    CheckConnection, ConnectionMonitorStatus, ConsensusAdapter, ConsensusAdapterMetrics,
};
use rtd_core::consensus_manager::ConsensusManager;
use rtd_core::consensus_throughput_calculator::{
    ConsensusThroughputCalculator, ConsensusThroughputProfiler, ThroughputProfileRanges,
};
use rtd_core::consensus_validator::{RtdTxValidator, RtdTxValidatorMetrics};
use rtd_core::db_checkpoint_handler::DBCheckpointHandler;
use rtd_core::epoch::committee_store::CommitteeStore;
use rtd_core::epoch::consensus_store_pruner::ConsensusStorePruner;
use rtd_core::epoch::epoch_metrics::EpochMetrics;
use rtd_core::epoch::reconfiguration::ReconfigurationInitiator;
...
use rtd_core::storage::RocksDbStore;
use rtd_core::transaction_orchestrator::TransactionOrchestrator;
use rtd_core::{
    authority::{AuthorityState, AuthorityStore},
    authority_client::NetworkAuthorityClient,
};
use rtd_json_rpc::JsonRpcServerBuilder;
use rtd_json_rpc::coin_api::CoinReadApi;
use rtd_json_rpc::governance_api::GovernanceReadApi;
use rtd_json_rpc::indexer_api::IndexerApi;
use rtd_json_rpc::move_utils::MoveUtils;
use rtd_json_rpc::read_api::ReadApi;
use rtd_json_rpc::transaction_builder_api::TransactionBuilderApi;
use rtd_json_rpc::transaction_execution_api::TransactionExecutionApi;
use rtd_macros::fail_point;
use rtd_macros::{fail_point_async, replay_log};
use rtd_network::api::ValidatorServer;
use rtd_network::discovery;
use rtd_network::discovery::TrustedPeerChangeEvent;
use rtd_network::state_sync;
use rtd_network::validator::server::ServerBuilder;
use rtd_protocol_config::{Chain, ProtocolConfig, ProtocolVersion};
use rtd_snapshot::uploader::StateSnapshotUploader;
use rtd_storage::{
    http_key_value_store::HttpKVStore,
    key_value_store::{FallbackTransactionKVStore, TransactionKeyValueStore},
    key_value_store_metrics::KeyValueStoreMetrics,
};
use rtd_types::base_types::{AuthorityName, EpochId};
...
use crate::metrics::{GrpcMetrics, RtdNodeMetrics};
...
mod simulator {
    ...
    pub(super) struct SimState {
        pub sim_node: rtd_simulator::runtime::NodeHandle,
        pub sim_safe_mode_expected: AtomicBool,
        _leak_detector: rtd_simulator::NodeLeakDetector,
    }

    impl Default for SimState {
        fn default() -> Self {
            Self {
                sim_node: rtd_simulator::runtime::NodeHandle::current(),
                sim_safe_mode_expected: AtomicBool::new(false),
                _leak_detector: rtd_simulator::NodeLeakDetector::new(),
            }
        }
    }

    type JwkInjector = dyn Fn(AuthorityName, &OIDCProvider) -> RtdResult<Vec<(JwkId, JWK)>>
        + Send
        + Sync
        + 'static;
    ...
    ) -> RtdResult<Vec<(JwkId, JWK)>> {
        use fastcrypto_zkp::bn254::zk_login::parse_jwks;
        // Just load a default Twitch jwk for testing.
        parse_jwks(
            rtd_types::zk_login_util::DEFAULT_JWK_BYTES,
            &OIDCProvider::Twitch,
            true,
        )
        .map_err(|_| RtdErrorKind::JWKRetrievalError.into())
    }
...
pub struct RtdNode {
    ...
    metrics: Arc<RtdNodeMetrics>,
    checkpoint_metrics: Arc<CheckpointMetrics>,

    _discovery: discovery::Handle,
    _connection_monitor_handle: linku_network::anemo_connection_monitor::ConnectionMonitorHandle,
    state_sync_handle: state_sync::Handle,
    randomness_handle: randomness::Handle,
    ...
    end_of_epoch_channel: broadcast::Sender<RtdSystemState>,
    ...
    // Channel to allow signaling upstream to shutdown rtd-node
...
impl fmt::Debug for RtdNode {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("RtdNode")
...
impl RtdNode {
    ...
    pub async fn start_async(
        ...
        info!(node =? config.protocol_public_key(),
            "Initializing rtd-node listening on {}", config.network_address
        );

        // Initialize metrics to track db usage before creating any stores
        DBMetrics::init(registry_service.clone());

        // Initialize LinkU metrics.
        linku_metrics::init_metrics(&prometheus_registry);
        // Unsupported (because of the use of static variable) and unnecessary in simtests.
        #[cfg(not(msim))]
        linku_metrics::thread_stall_monitor::start_thread_stall_monitor();
        ...
        if is_genesis {
            info!("checking RTD conservation at genesis");
            // When we are opening the db table, the only time when it's safe to
            // check RTD conservation is at genesis. Otherwise we may be in the middle of
            // an epoch and the RTD conservation check will fail. This also initialize
            // the expected_network_rtd_amount table.
            cache_traits
                .reconfig_api
                .expensive_check_rtd_conservation(&epoch_store)
                .expect("RTD conservation check cannot fail at genesis");
        }
        ...
        if epoch_store.epoch() == 0 {
            ...
            let transaction =
                rtd_types::executable_transaction::VerifiedExecutableTransaction::new_unchecked(
                    rtd_types::executable_transaction::ExecutableTransaction::new_from_data_and_sig(
        ...
        {
            rtd_core::verify_indexes::verify_indexes(
        ...
        let network_connection_metrics = linku_network::quinn_metrics::QuinnConnectionMetrics::new(
            "rtd",
            &registry_service.default_registry(),
        );
        ...
        let connection_monitor_handle =
            linku_network::anemo_connection_monitor::AnemoConnectionMonitor::spawn(
        ...
        let rtd_node_metrics = Arc::new(RtdNodeMetrics::new(&registry_service.default_registry()));
    ...
    pub async fn close_epoch(&self, epoch_store: &Arc<AuthorityPerEpochStore>) -> RtdResult {
        info!("close_epoch (current epoch = {})", epoch_store.epoch());
        self.validator_components
            .lock()
            .await
            .as_ref()
            .ok_or_else(|| RtdError::from("Node is not a validator"))?
            .consensus_adapter
            .close_epoch(epoch_store);
        Ok(())
    }

    pub fn clear_override_protocol_upgrade_buffer_stake(&self, epoch: EpochId) -> RtdResult {
        self.state
    ...
    pub async fn close_epoch_for_testing(&self) -> RtdResult {
        ...
        let p2p_network = {
            ...
            let service = ServiceBuilder::new()
                ...
                .layer(CallbackLayer::new(
                    linku_network::metrics::MetricsMakeCallbackHandler::new(
            ...
            let server_name = format!("rtd-{}", chain_identifier);
    ...
    async fn construct_validator_components(
        ...
        connection_monitor_status: Arc<ConnectionMonitorStatus>,
        registry_service: &RegistryService,
        rtd_node_metrics: Arc<RtdNodeMetrics>,
        checkpoint_metrics: Arc<CheckpointMetrics>,
        ...
        let rtd_tx_validator_metrics =
            RtdTxValidatorMetrics::new(&registry_service.default_registry());
    ...
    async fn start_grpc_validator_service(
        ...
        let mut server_conf = linku_network::config::Config::new();
        ...
        let tls_config = rtd_tls::create_rustls_server_config(
            config.network_key_pair().copy().private(),
            RTD_TLS_SERVER_NAME.to_string(),
        );
        ...
        if tokio::time::timeout(
            ...
            state
                ...
                .notify_read_executed_effects_digests(
                    "RtdNode::notify_read_executed_effects_digests",
                    &digests,
                ),
        )
        .await
    ...
    pub async fn monitor_reconfiguration(
        ...
        loop {
            ...
            let latest_system_state = self
                ...
                .expect("Read Rtd System State object cannot fail");
            ...
            let next_epoch_committee = new_epoch_start_state.get_rtd_committee();
                ...
                )
            {
                self.state
                    .prune_checkpoints_for_eligible_epochs_for_testing(
                        self.config.clone(),
                        rtd_core::authority::authority_store_pruner::AuthorityStorePruningMetrics::new_for_test(),
                    )
                    .await?;
            }
    ...
    fn try_recover_checkpoint_fork(
        ...
        for (seq, expected_digest_str) in &recovery.checkpoint_overrides {
            ...
            if let Some(local_summary) = checkpoint_store.get_locally_computed_checkpoint(*seq)? {
                let local_digest = rtd_types::message_envelope::Message::digest(&local_summary);
...
#[cfg(msim)]
impl RtdNode {
    pub fn get_sim_node_id(&self) -> rtd_simulator::task::NodeId {
...
async fn build_http_servers(
    ...
    let json_rpc_router = {
        ...
        let name_service_config =
            if let (Some(package_address), Some(registry_id), Some(reverse_registry_id)) = (
                ...
                rtd_name_service::NameServiceConfig::new(
                    package_address,
                    registry_id,
                    reverse_registry_id,
                )
                ...
                match state.get_chain_identifier().chain() {
                    Chain::Mainnet => rtd_name_service::NameServiceConfig::mainnet(),
                    Chain::Testnet => rtd_name_service::NameServiceConfig::testnet(),
                    Chain::Unknown => rtd_name_service::NameServiceConfig::default(),
                }
            };
    ...
    let rpc_router = {
        let mut rpc_service =
            rtd_rpc_api::RpcService::new(Arc::new(RestReadStore::new(state.clone(), store)));
    ...
    let layers = ServiceBuilder::new()
        .map_request(|mut request: axum::http::Request<_>| {
            if let Some(connect_info) = request.extensions().get::<rtd_http::ConnectInfo>() {
    ...
    {
        let https = rtd_http::Builder::new()
...
struct HttpServers {
    #[allow(unused)]
    http: Option<rtd_http::ServerHandle>,
    #[allow(unused)]
    https: Option<rtd_http::ServerHandle>,
}
...
mod tests {
    ...
    async fn test_fork_error_and_recovery_both_paths() {
        ...
        let r = RtdNode::check_and_recover_forks(
            &checkpoint_store,
            &checkpoint_metrics,
            true,
            Some(&fork_recovery),
