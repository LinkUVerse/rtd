...
use linku_common::sync::async_once_cell::AsyncOnceCell;
use rtd_config::node::RunWithRange;
use rtd_config::{Config, NodeConfig};
use rtd_core::runtime::RtdRuntimes;
use rtd_telemetry::send_telemetry_event;
...
fn main() {
    ...
    let runtimes = RtdRuntimes::new(&config);
    let metrics_rt = runtimes.metrics.enter();
    let registry_service = linku_metrics::start_prometheus_server(config.metrics_address);
...

    {
        ...
        {
            rtd_metrics_push_client::start_metrics_push_task(
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<rtd_node::RtdNode>>::new());
    let node_once_cell_clone = node_once_cell.clone();

    // let rtd-node signal main to shutdown runtimes
    ...
    runtimes.rtd_node.spawn(async move {
        match rtd_node::RtdNode::start_async(config, registry_service, server_version).await {
            Ok(rtd_node) => node_once_cell_clone
                .set(rtd_node)
        ...
        // when we get a shutdown signal from rtd-node, forward it on to the runtime_shutdown_channel here in
        // main to signal runtimes to all shutdown.
        tokio::select! {
           _ = shutdown_rx.recv() => {
                runtime_shutdown_tx.send(()).expect("failed to forward shutdown signal from rtd-node to rtd-node main");
            }
        }
    ...
    runtimes.metrics.spawn(async move {
        ...
        info!("Rtd chain identifier: {chain_identifier}");
        prometheus_registry
