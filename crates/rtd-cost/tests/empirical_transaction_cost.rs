...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholeRtdCoin,
    TransferPortionRtdCoin,
    SharedCounterCreate,
...
// 1. Run `cargo insta test --review` under `./rtd-cost`.
...
async fn create_txes(
    ...
    //
    // Transfer Whole Rtd Coin and Transfer Portion of Rtd Coin
    //
    let whole_rtd_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_rtd(None, RtdAddress::default())
            .build();
    let partial_rtd_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_rtd(Some(10), RtdAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholeRtdCoin,
        whole_rtd_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionRtdCoin,
        partial_rtd_coin_tx,
    );
