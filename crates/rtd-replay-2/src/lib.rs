...
/// Load replay configuration from ~/.rtd/rtd_config/replay.toml file.
...
pub fn load_config_file() -> Result<ReplayConfigStable> {
    let config_dir = match rtd_config_dir() {
        Ok(dir) => dir,
        Err(e) => {
...
pub fn print_effects_or_fork<W: Write>(
    ...
    if manager.member(Artifact::ForkedTransactionEffects).exists() {
        ...
        writeln!(
            w,
            "{}",
            RtdTransactionBlockEffects::try_from(tx_effects.clone())
