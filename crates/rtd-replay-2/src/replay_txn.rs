...
pub(crate) async fn replay_transaction<S: ReadDataStore>(
    ...
    if let rtd_types::transaction::TransactionKind::ProgrammableTransaction(ptb) =
...
// version to Object.
// Returns also the packages of the type parameters instantiated (e.g. `RTD` in `Coin<RTD>`).
