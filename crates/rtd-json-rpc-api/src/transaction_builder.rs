...
use rtd_json_rpc_types::{
    RPCTransactionRequestParams, RtdTransactionBlockBuilderMode, RtdTypeTag, TransactionBlockBytes,
};
...
pub trait TransactionBuilder {
    ...
    async fn transfer_object(
        ...
        /// the recipient's Rtd address
        recipient: RtdAddress,
    ) -> RpcResult<TransactionBlockBytes>;

    /// Create an unsigned transaction to send RTD coin object to a Rtd address. The RTD object is also used as the gas object.
    #[method(name = "transferRtd")]
    async fn transfer_rtd(
        &self,
        /// the transaction signer's Rtd address
        signer: RtdAddress,
        /// the Rtd coin object to be used in this transaction
    ...
    async fn pay(
        ...
        /// the transaction signer's Rtd address
        signer: RtdAddress,
        /// the Rtd coins to be used in this transaction
    ...
    /// Send RTD coins to a list of addresses, following a list of amounts.
    /// This is for RTD coin only and does not require a separate gas coin object.
    /// Specifically, what pay_rtd does are:
    ...
    async fn pay_rtd(
        ...
        /// the transaction signer's Rtd address
        signer: RtdAddress,
        /// the Rtd coins to be used in this transaction, including the coin for gas payment.
        input_coins: Vec<ObjectID>,
        /// the recipients' addresses, the length of this vector must be the same as amounts.
        recipients: Vec<RtdAddress>,
        /// the amounts to be transferred to recipients, following the same order
    ...
    /// Send all RTD coins to one recipient.
    /// This is for RTD coin only and does not require a separate gas coin object.
    /// Specifically, what pay_all_rtd does are:
    /// 1. accumulate all RTD from input coins and deposit all RTD to the first input coin
    ...
    async fn move_call(
        ...
        /// the gas budget, the transaction will fail if the gas cost exceed the budget
        gas_budget: BigInt<u64>,
        /// Whether this is a Normal transaction or a Dev Inspect Transaction. Default to be `RtdTransactionBlockBuilderMode::Commit` when it's None.
        execution_mode: Option<RtdTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes>;
    ...
    async fn batch_transaction(
        ...
        txn_builder_mode: Option<RtdTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes>;
    ...
    async fn request_add_stake(
        ...
        /// the transaction signer's Rtd address
        signer: RtdAddress,
        /// Coin<RTD> object to stake
        coins: Vec<ObjectID>,
        /// stake amount
        amount: Option<BigInt<u64>>,
        /// the validator's Rtd address
