...
fn default_pipeline_config(pipeline: Pipeline) -> PipelineConfig {
    PipelineConfig {
        ...
        migration_id: None,
        // Schema is populated by build_pipeline_configs in indexer.rs
        schema: vec![],
    }
}
...
impl TestHarness {
    ...
    async fn read_migration_watermark(
        ...
        let registry = prometheus::Registry::new();
        let metrics = Metrics::new(&registry);
        let store = AnalyticsStore::new_migration(
        ...
        conn.committer_watermark(pipeline)
            .await
...
async fn test_smoke_all_pipelines_parquet() {
    ...
    for pipeline in pipelines_with_guaranteed_output() {
        let prefix = format!("{}/epoch_0", pipeline.name());
...
async fn test_smoke_all_pipelines_csv() {
    ...
    for pipeline in pipelines_with_guaranteed_output() {
        ...
        let expected_cols = pipeline.schema().len();
...
async fn test_checkpoint_csv_parseable() {
    ...
    let expected_cols = Pipeline::Checkpoint.schema().len();
    for (row_num, record) in records.iter().enumerate() {
        assert_eq!(
            record.len(),
            expected_cols,
