...
impl RtdFlavor {
    ...
    /// The default dependencies are `rtd` and `std`
    fn default_system_dep_names() -> BTreeSet<PackageName> {
        BTreeSet::from([
            PackageName::new("rtd").unwrap(),
...
fn validate_modern_manifest_does_not_use_legacy_system_names(
    ...
    let legacy_names = RtdFlavor::system_deps_names_map();

    for name in dep_names {
        if legacy_names.contains_key(&name) {
            return Err(format!(
                "Dependency `{name}` is a legacy system name and cannot be used. See https://docs.rtd.io/guides/developer/rtd-101/move-package-management#system-dependencies"
            ));
        }
    }
