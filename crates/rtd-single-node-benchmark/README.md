# Rtd Single Node Benchmark

This crate contains a binary for performance benchmarking a single Rtd node.
...
```
cargo run --release --bin rtd-single-node-benchmark -- ptb
```
...
```
cargo run --release --bin rtd-single-node-benchmark -- --component baseline ptb
```
...
### Profiling
If you are interested in profiling Rtd, you can start the benchmark, wait for it to print out "Started execution...", and then attach a profiler to the process.
