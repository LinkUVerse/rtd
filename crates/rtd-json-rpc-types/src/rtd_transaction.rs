...
use crate::rtd_transaction::GenericSignature::Signature;
use crate::{Filter, Page, RtdEvent, RtdMoveAbort, RtdObjectRef};

// similar to EpochId of rtd-types but BigInt
pub type RtdEpochId = BigInt<u64>;
...
pub struct RtdTransactionBlockResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<TransactionFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<RtdTransactionBlockResponseOptions>,
}

impl RtdTransactionBlockResponseQuery {
    pub fn new(
...
pub type TransactionBlocksPage = Page<RtdTransactionBlockResponse, TransactionDigest>;
...
pub struct RtdTransactionBlockResponseOptions {
...
impl RtdTransactionBlockResponseOptions {
    pub fn new() -> Self {
        Self::default()
    }
...
pub struct RtdTransactionBlockResponse {
    pub digest: TransactionDigest,
    /// Transaction input data
    #[serde(skip_serializing_if = "Option::is_none")]
    pub transaction: Option<RtdTransactionBlock>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub effects: Option<RtdTransactionBlockEffects>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub events: Option<RtdTransactionBlockEvents>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl RtdTransactionBlockResponse {
...
impl PartialEq for RtdTransactionBlockResponse {
...
impl Display for RtdTransactionBlockResponse {
...
pub enum RtdTransactionBlockKind {
    /// A system transaction that will update epoch information on-chain.
    ChangeEpoch(RtdChangeEpoch),
    /// A system transaction used for initializing the initial state of the chain.
    Genesis(RtdGenesisTransaction),
    /// A system transaction marking the start of a series of transactions scheduled as part of a
    /// checkpoint
    ConsensusCommitPrologue(RtdConsensusCommitPrologue),
    /// A series of transactions where the results of one transaction can be used in future
    /// transactions
    ProgrammableTransaction(RtdProgrammableTransactionBlock),
    /// A transaction which updates global authenticator state
    AuthenticatorStateUpdate(RtdAuthenticatorStateUpdate),
    /// A transaction which updates global randomness state
    RandomnessStateUpdate(RtdRandomnessStateUpdate),
    /// The transaction which occurs only at the end of the epoch
    EndOfEpochTransaction(RtdEndOfEpochTransaction),
    ConsensusCommitPrologueV2(RtdConsensusCommitPrologueV2),
    ConsensusCommitPrologueV3(RtdConsensusCommitPrologueV3),
    ConsensusCommitPrologueV4(RtdConsensusCommitPrologueV4),

    ProgrammableSystemTransaction(RtdProgrammableTransactionBlock),
    // .. more transaction types go here
}

impl Display for RtdTransactionBlockKind {
    ...
    fn try_from_inner(tx: TransactionKind) -> Result<Self, anyhow::Error> {
        Ok(match tx {
            ...
            TransactionKind::ConsensusCommitPrologueV2(p) => {
                Self::ConsensusCommitPrologueV2(RtdConsensusCommitPrologueV2 {
                    epoch: p.epoch,
                    round: p.round,
                    commit_timestamp_ms: p.commit_timestamp_ms,
            ...
            TransactionKind::ConsensusCommitPrologueV3(p) => {
                Self::ConsensusCommitPrologueV3(RtdConsensusCommitPrologueV3 {
                    epoch: p.epoch,
                    round: p.round,
            ...
            TransactionKind::ConsensusCommitPrologueV4(p) => {
                Self::ConsensusCommitPrologueV4(RtdConsensusCommitPrologueV4 {
                    epoch: p.epoch,
                    round: p.round,
                    sub_dag_index: p.sub_dag_index,
                    commit_timestamp_ms: p.commit_timestamp_ms,
            ...
            TransactionKind::AuthenticatorStateUpdate(update) => {
                Self::AuthenticatorStateUpdate(RtdAuthenticatorStateUpdate {
                    epoch: update.epoch,
                    round: update.round,
                    new_active_jwks: update
                        .new_active_jwks
                        .into_iter()
                        .map(RtdActiveJwk::from)
                        .collect(),
                })
            }
            TransactionKind::RandomnessStateUpdate(update) => {
                Self::RandomnessStateUpdate(RtdRandomnessStateUpdate {
                    epoch: update.epoch,
            ...
            TransactionKind::EndOfEpochTransaction(end_of_epoch_tx) => {
                Self::EndOfEpochTransaction(RtdEndOfEpochTransaction {
                    transactions: end_of_epoch_tx
                        .into_iter()
                        .map(|tx| match tx {
                            EndOfEpochTransactionKind::ChangeEpoch(e) => {
                                RtdEndOfEpochTransactionKind::ChangeEpoch(e.into())
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateCreate => {
                                RtdEndOfEpochTransactionKind::AuthenticatorStateCreate
                            }
                            EndOfEpochTransactionKind::AuthenticatorStateExpire(expire) => {
                                RtdEndOfEpochTransactionKind::AuthenticatorStateExpire(
                                    RtdAuthenticatorStateExpire {
                                        min_epoch: expire.min_epoch,
                                    },
                                )
                            }
                            EndOfEpochTransactionKind::RandomnessStateCreate => {
                                RtdEndOfEpochTransactionKind::RandomnessStateCreate
                            }
                            EndOfEpochTransactionKind::DenyListStateCreate => {
                                RtdEndOfEpochTransactionKind::CoinDenyListStateCreate
                            }
                            EndOfEpochTransactionKind::BridgeStateCreate(chain_id) => {
                                RtdEndOfEpochTransactionKind::BridgeStateCreate(
                                    (*chain_id.as_bytes()).into(),
                                )
                            }
                            EndOfEpochTransactionKind::BridgeCommitteeInit(
                                bridge_shared_version,
                            ) => RtdEndOfEpochTransactionKind::BridgeCommitteeUpdate(
                                bridge_shared_version,
                            ),
                            EndOfEpochTransactionKind::StoreExecutionTimeObservations(_) => {
                                RtdEndOfEpochTransactionKind::StoreExecutionTimeObservations
                            }
                            EndOfEpochTransactionKind::AccumulatorRootCreate => {
                                RtdEndOfEpochTransactionKind::AccumulatorRootCreate
                            }
                            EndOfEpochTransactionKind::CoinRegistryCreate => {
                                RtdEndOfEpochTransactionKind::CoinRegistryCreate
                            }
                            EndOfEpochTransactionKind::DisplayRegistryCreate => {
                                RtdEndOfEpochTransactionKind::DisplayRegistryCreate
                            }
        ...
        match tx {
            ...
            | TransactionKind::ProgrammableSystemTransaction(p) => {
                Ok(Self::ProgrammableTransaction(
                    RtdProgrammableTransactionBlock::try_from_with_module_cache(p, module_cache)?,
                ))
            }
        ...
        match tx {
            TransactionKind::ProgrammableSystemTransaction(p) => {
                Ok(Self::ProgrammableSystemTransaction(
                    RtdProgrammableTransactionBlock::try_from_with_package_resolver(
                        p,
                        package_resolver,
                    )
                    .await?,
...
impl From<ChangeEpoch> for RtdChangeEpoch {
...
#[enum_dispatch(RtdTransactionBlockEffectsAPI)]
#[serde(
    rename = "TransactionBlockEffects",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum RtdTransactionBlockEffects {
    V1(RtdTransactionBlockEffectsV1),
}

#[enum_dispatch]
pub trait RtdTransactionBlockEffectsAPI {
    fn status(&self) -> &RtdExecutionStatus;
    fn into_status(self) -> RtdExecutionStatus;
    fn shared_objects(&self) -> &[RtdObjectRef];
    fn created(&self) -> &[OwnedObjectRef];
    fn mutated(&self) -> &[OwnedObjectRef];
    fn unwrapped(&self) -> &[OwnedObjectRef];
    fn deleted(&self) -> &[RtdObjectRef];
    fn unwrapped_then_deleted(&self) -> &[RtdObjectRef];
    fn wrapped(&self) -> &[RtdObjectRef];
    ...
    fn all_deleted_objects(&self) -> Vec<(&RtdObjectRef, DeleteKind)>;

    fn accumulator_events(&self) -> Vec<RtdAccumulatorEvent>;
}
...
pub struct RtdTransactionBlockEffectsModifiedAtVersions {
    object_id: ObjectID,
...
pub struct RtdAccumulatorEvent {
    pub accumulator_obj: ObjectID,
    pub address: RtdAddress,
    pub ty: RtdTypeTag,
    pub operation: RtdAccumulatorOperation,
    pub value: RtdAccumulatorValue,
}

impl From<AccumulatorEvent> for RtdAccumulatorEvent {
...
pub enum RtdAccumulatorOperation {
    Merge,
    Split,
}

impl From<AccumulatorOperation> for RtdAccumulatorOperation {
...
impl From<AccumulatorValue> for RtdAccumulatorValue {
...
pub struct RtdTransactionBlockEffectsV1 {
    ...
    pub modified_at_versions: Vec<RtdTransactionBlockEffectsModifiedAtVersions>,
    /// The object references of the shared objects used in this transaction. Empty if no shared objects were used.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub shared_objects: Vec<RtdObjectRef>,
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub deleted: Vec<RtdObjectRef>,
    /// Object refs of objects previously wrapped in other objects but now deleted.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub unwrapped_then_deleted: Vec<RtdObjectRef>,
    /// Object refs of objects now wrapped in other objects.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub wrapped: Vec<RtdObjectRef>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub accumulator_events: Vec<RtdAccumulatorEvent>,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub abort_error: Option<RtdMoveAbort>,
}

// TODO move additional error info here

impl RtdTransactionBlockEffectsAPI for RtdTransactionBlockEffectsV1 {
    fn status(&self) -> &RtdExecutionStatus {
        &self.status
    }
    fn into_status(self) -> RtdExecutionStatus {
...
impl RtdTransactionBlockEffects {
    pub fn new_for_testing(
        transaction_digest: TransactionDigest,
        status: RtdExecutionStatus,
    ) -> Self {
        Self::V1(RtdTransactionBlockEffectsV1 {
            transaction_digest,
            status,
            gas_object: OwnedObjectRef {
                owner: Owner::AddressOwner(RtdAddress::random_for_testing_only()),
                reference: rtd_types::base_types::random_object_ref().into(),
            },
            executed_epoch: 0,
...
impl TryFrom<TransactionEffects> for RtdTransactionBlockEffects {
    type Error = RtdError;

    fn try_from(effect: TransactionEffects) -> Result<Self, Self::Error> {
        Ok(RtdTransactionBlockEffects::V1(
            RtdTransactionBlockEffectsV1 {
                ...
                modified_at_versions: effect
                    ...
                    .map(|(object_id, sequence_number)| {
                        RtdTransactionBlockEffectsModifiedAtVersions {
                            object_id,
                            sequence_number,
                        }
                    })
                ...
                abort_error: effect
                    .move_abort()
                    .map(|(abort, code)| RtdMoveAbort::new(abort, code)),
                accumulator_events: effect
                    .accumulator_events()
                    .into_iter()
                    .map(RtdAccumulatorEvent::from)
                    .collect(),
            },
        ))
    }
...
fn objref_string(obj: &RtdObjectRef) -> String {
    format!(
        " ┌──\n │ ID: {} \n │ Version: {} \n │ Digest: {}\n └──",
        obj.object_id,
...
impl Display for RtdTransactionBlockEffects {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
...
pub struct DryRunTransactionBlockResponse {
    ...
    pub input: RtdTransactionBlockData,
...
pub struct RtdTransactionBlockEvents {
    pub data: Vec<RtdEvent>,
}

impl RtdTransactionBlockEvents {
    pub fn try_from(
        ...
        Ok(Self {
            data: events
                ...
                .map(|(seq, event)| {
                    ...
                    RtdEvent::try_from(event, tx_digest, seq as u64, timestamp_ms, layout)
                })
                .collect::<Result<_, _>>()?,
        })
    }
...
impl Display for RtdTransactionBlockEvents {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
...
pub struct DevInspectArgs {
    ...
    pub gas_sponsor: Option<RtdAddress>,
...
pub struct DevInspectResults {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub results: Option<Vec<RtdExecutionResult>>,
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename = "RtdExecutionResult", rename_all = "camelCase")]
pub struct RtdExecutionResult {
    /// The value of any arguments that were mutably borrowed.
    /// Non-mut borrowed values are not included
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub mutable_reference_outputs: Vec<(/* argument */ RtdArgument, Vec<u8>, RtdTypeTag)>,
    /// The return values from the transaction
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub return_values: Vec<(Vec<u8>, RtdTypeTag)>,
}
...
impl DevInspectResults {
    pub fn new(
                    ...
                    srvs.into_iter()
                        .map(|srv| {
                            ...
                            let mutable_reference_outputs = mutable_reference_outputs
                                .into_iter()
                                .map(|(a, bytes, tag)| (a.into(), bytes, RtdTypeTag::from(tag)))
                                .collect();
                            let return_values = return_values
                                .into_iter()
                                .map(|(bytes, tag)| (bytes, RtdTypeTag::from(tag)))
                                .collect();
                            RtdExecutionResult {
        ...
        Ok(Self {
            ...
            events: RtdTransactionBlockEvents::try_from(events, tx_digest, None, resolver)?,
            results,
            error,
            raw_txn_data,
            raw_effects,
...
#[derive(Eq, PartialEq, Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub enum RtdTransactionBlockBuilderMode {
    /// Regular Rtd Transactions that are committed on chain
    Commit,
...
impl Display for RtdExecutionStatus {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
...
impl RtdExecutionStatus {
    pub fn is_ok(&self) -> bool {
        matches!(self, RtdExecutionStatus::Success)
    }
    pub fn is_err(&self) -> bool {
        matches!(self, RtdExecutionStatus::Failure { .. })
    }
}

impl From<ExecutionStatus> for RtdExecutionStatus {
...
fn to_rtd_object_ref(refs: Vec<ObjectRef>) -> Vec<RtdObjectRef> {
    refs.into_iter().map(RtdObjectRef::from).collect()
}
...
pub struct RtdGasData {
    pub payment: Vec<RtdObjectRef>,
    pub owner: RtdAddress,
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub price: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[enum_dispatch(RtdTransactionBlockDataAPI)]
#[serde(
    rename = "TransactionBlockData",
    rename_all = "camelCase",
    tag = "messageVersion"
)]
pub enum RtdTransactionBlockData {
    V1(RtdTransactionBlockDataV1),
}

#[enum_dispatch]
pub trait RtdTransactionBlockDataAPI {
    fn transaction(&self) -> &RtdTransactionBlockKind;
    fn sender(&self) -> &RtdAddress;
    fn gas_data(&self) -> &RtdGasData;
}

#[derive(Debug, Deserialize, Serialize, JsonSchema, Clone, PartialEq, Eq)]
#[serde(rename = "TransactionBlockDataV1", rename_all = "camelCase")]
pub struct RtdTransactionBlockDataV1 {
    pub transaction: RtdTransactionBlockKind,
    pub sender: RtdAddress,
    pub gas_data: RtdGasData,
}

impl RtdTransactionBlockDataAPI for RtdTransactionBlockDataV1 {
    fn transaction(&self) -> &RtdTransactionBlockKind {
        &self.transaction
    }
    fn sender(&self) -> &RtdAddress {
...
impl RtdTransactionBlockData {
    pub fn move_calls(&self) -> Vec<&RtdProgrammableMoveCall> {
        match self {
            Self::V1(data) => match &data.transaction {
                RtdTransactionBlockKind::ProgrammableTransaction(pt) => pt
                    .commands
                    .iter()
                    .filter_map(|command| match command {
                        RtdCommand::MoveCall(c) => Some(&**c),
                        _ => None,
                    })
                    .collect(),
                _ => vec![],
    ...
    fn try_from_inner(
        ...
        let gas_data = RtdGasData {
            payment: data
                .gas()
                .iter()
                .map(|obj_ref| RtdObjectRef::from(*obj_ref))
                .collect(),
        ...
        match message_version {
            1 => Ok(RtdTransactionBlockData::V1(RtdTransactionBlockDataV1 {
                transaction,
                sender,
                gas_data,
            })),
    ...
    pub fn try_from_with_module_cache(
        ...
        let transaction = RtdTransactionBlockKind::try_from_with_module_cache(
    ...
    pub async fn try_from_with_package_resolver(
        ...
        let transaction = RtdTransactionBlockKind::try_from_with_package_resolver(
            data.clone().into_kind(),
            package_resolver,
        )
        .await?;
...
impl Display for RtdTransactionBlockData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::V1(data) => {
...
pub struct RtdTransactionBlock {
    pub data: RtdTransactionBlockData,
    pub tx_signatures: Vec<GenericSignature>,
}

impl RtdTransactionBlock {
    pub fn try_from(
        data: SenderSignedData,
        module_cache: &impl GetModule,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self {
            data: RtdTransactionBlockData::try_from_with_module_cache(
    ...
    // TODO: the RtdTransactionBlock `try_from` can be removed after cleaning up indexer v1, so are the related
    // `try_from` methods for nested structs like RtdTransactionBlockData etc.
    pub async fn try_from_with_package_resolver(
        data: SenderSignedData,
        package_resolver: &Resolver<impl PackageStore>,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self {
            data: RtdTransactionBlockData::try_from_with_package_resolver(
                data.intent_message().value.clone(),
                package_resolver,
            )
            .await?,
...
impl Display for RtdTransactionBlock {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut builder = TableBuilder::default();
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdGenesisTransaction {
    pub objects: Vec<ObjectID>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdConsensusCommitPrologue {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdConsensusCommitPrologueV2 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdConsensusCommitPrologueV3 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdConsensusCommitPrologueV4 {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdAuthenticatorStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
    #[schemars(with = "BigInt<u64>")]
    ...
    pub new_active_jwks: Vec<RtdActiveJwk>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdRandomnessStateUpdate {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub epoch: u64,
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdEndOfEpochTransaction {
    pub transactions: Vec<RtdEndOfEpochTransactionKind>,
}

#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub enum RtdEndOfEpochTransactionKind {
    ChangeEpoch(RtdChangeEpoch),
...
#[serde_as]
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdAuthenticatorStateExpire {
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "BigInt<u64>")]
    pub min_epoch: u64,
}
...
impl From<ActiveJwk> for RtdActiveJwk {
...
/// commands
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdProgrammableTransactionBlock {
    /// Input objects or primitive values
    pub inputs: Vec<RtdCallArg>,
    ...
    pub commands: Vec<RtdCommand>,
}

impl Display for RtdProgrammableTransactionBlock {
...
impl RtdProgrammableTransactionBlock {
    fn try_from_with_module_cache(
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename = "RtdTransaction")]
pub enum RtdCommand {
    /// A call to either an entry or a public Move function
    MoveCall(Box<RtdProgrammableMoveCall>),
    /// `(Vec<forall T:key+store. T>, address)`
    /// It sends n-objects to the specified address. These objects must have store
    /// (public transfer) and either the previous owner must be an address or the object must
    /// be newly created.
    TransferObjects(Vec<RtdArgument>, RtdArgument),
    /// `(&mut Coin<T>, Vec<u64>)` -> `Vec<Coin<T>>`
    /// It splits off some amounts into a new coins with those amounts
    SplitCoins(RtdArgument, Vec<RtdArgument>),
    /// `(&mut Coin<T>, Vec<Coin<T>>)`
    /// It merges n-coins into the first coin
    MergeCoins(RtdArgument, Vec<RtdArgument>),
    /// Publishes a Move package. It takes the package bytes and a list of the package's transitive
    /// dependencies to link against on-chain.
    Publish(Vec<ObjectID>),
    /// Upgrades a Move package
    Upgrade(Vec<ObjectID>, ObjectID, RtdArgument),
    /// `forall T: Vec<T> -> vector<T>`
    /// Given n-values of the same type, it constructs a vector. For non objects or an empty vector,
    /// the type tag must be specified.
    MakeMoveVec(Option<String>, Vec<RtdArgument>),
}

impl Display for RtdCommand {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MoveCall(p) => {
                write!(f, "MoveCall({p})")
...
impl From<Command> for RtdCommand {
    fn from(value: Command) -> Self {
        match value {
            Command::MoveCall(m) => RtdCommand::MoveCall(Box::new((*m).into())),
            Command::TransferObjects(args, arg) => RtdCommand::TransferObjects(
                args.into_iter().map(RtdArgument::from).collect(),
                arg.into(),
            ),
            Command::SplitCoins(arg, args) => RtdCommand::SplitCoins(
                arg.into(),
                args.into_iter().map(RtdArgument::from).collect(),
            ),
            Command::MergeCoins(arg, args) => RtdCommand::MergeCoins(
                arg.into(),
                args.into_iter().map(RtdArgument::from).collect(),
            ),
            Command::Publish(_modules, dep_ids) => RtdCommand::Publish(dep_ids),
            Command::MakeMoveVec(tag_opt, args) => RtdCommand::MakeMoveVec(
                tag_opt.map(|tag| tag.to_string()),
                args.into_iter().map(RtdArgument::from).collect(),
            ),
            Command::Upgrade(_modules, dep_ids, current_package_id, ticket) => {
                RtdCommand::Upgrade(dep_ids, current_package_id, RtdArgument::from(ticket))
            }
        }
    }
}
...
impl From<Argument> for RtdArgument {
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct RtdProgrammableMoveCall {
...
impl Display for RtdProgrammableMoveCall {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let Self {
            package,
            module,
...
impl From<ProgrammableMoveCall> for RtdProgrammableMoveCall {
    fn from(value: ProgrammableMoveCall) -> Self {
        ...
        Self {
            ...
            arguments: arguments.into_iter().map(RtdArgument::from).collect(),
        }
    }
}
...
impl From<InputObjectKind> for RtdInputObjectKind {
...
impl TryInto<TypeTag> for RtdTypeTag {
    type Error = anyhow::Error;
    fn try_into(self) -> Result<TypeTag, Self::Error> {
        parse_rtd_type_tag(&self.0)
    }
}

impl From<TypeTag> for RtdTypeTag {
...
pub struct MoveCallParams {
    ...
    pub function: String,
    #[serde(default)]
    pub type_arguments: Vec<RtdTypeTag>,
    pub arguments: Vec<RtdJsonValue>,
}
...
pub struct TransactionBlockBytes {
    ...
    pub gas: Vec<RtdObjectRef>,
    /// objects to be used in this transaction
    pub input_objects: Vec<RtdInputObjectKind>,
}
...
pub enum RtdCallArg {
    ...
    FundsWithdrawal(RtdFundsWithdrawalArg),
}
...
impl RtdCallArg {
    pub fn try_from(
        ...
        Ok(match value {
            CallArg::Pure(p) => RtdCallArg::Pure(RtdPureValue {
                value_type: layout.map(|l| l.into()),
                value: RtdJsonValue::from_bcs_bytes(layout, &p)?,
            }),
            CallArg::Object(ObjectArg::ImmOrOwnedObject((id, version, digest))) => {
                RtdCallArg::Object(RtdObjectArg::ImmOrOwnedObject {
                    object_id: id,
                    version,
                    digest,
                })
    ...
    pub fn pure(&self) -> Option<&RtdJsonValue> {
        match self {
            RtdCallArg::Pure(v) => Some(&v.value),
            _ => None,
        }
    }
    ...
    pub fn object(&self) -> Option<&ObjectID> {
        match self {
            ...
            | RtdCallArg::Object(RtdObjectArg::Receiving { object_id, .. }) => Some(object_id),
            _ => None,
        }
    }
}
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct RtdPureValue {
    #[schemars(with = "Option<String>")]
    #[serde_as(as = "Option<AsRtdTypeTag>")]
    value_type: Option<TypeTag>,
    value: RtdJsonValue,
}

impl RtdPureValue {
    pub fn value(&self) -> RtdJsonValue {
        self.value.clone()
    }
...
#[derive(Eq, PartialEq, Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct RtdFundsWithdrawalArg {
    pub reservation: RtdReservation,
    pub type_arg: RtdWithdrawalTypeArg,
    pub withdraw_from: RtdWithdrawFrom,
}
...
impl From<EffectsWithInput> for RtdTransactionBlockEffects {
...
pub enum TransactionFilter {
    ...
    FromAddress(RtdAddress),
    /// Query by recipient address.
    ToAddress(RtdAddress),
    /// Query by sender and recipient address.
    FromAndToAddress { from: RtdAddress, to: RtdAddress },
    /// CURRENTLY NOT SUPPORTED. Query txs that have a given address as sender or recipient.
    FromOrToAddress { addr: RtdAddress },
