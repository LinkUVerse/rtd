...
use rtd_bridge_schema::models::RtdProgressStore;
use rtd_bridge_schema::schema::governance_actions;
use rtd_bridge_schema::schema::rtd_progress_store::txn_digest;
use rtd_bridge_schema::schema::{rtd_error_transactions, token_transfer_data};
use rtd_bridge_schema::{schema, schema::token_transfer};
...
const RTD_PROGRESS_STORE_DUMMY_KEY: i32 = 1;
        ...
        .transaction(|conn| {
            async move {
                ...
                diesel::insert_into(token_transfer::table)
                    ...
                    .execute(conn)
                    .await?;
                diesel::insert_into(rtd_error_transactions::table)
                    .values(&errors)
...
pub async fn update_rtd_progress_store(
    pool: &PgPool,
    tx_digest: TransactionDigest,
) -> Result<(), anyhow::Error> {
    let mut conn = pool.get().await?;
    diesel::insert_into(schema::rtd_progress_store::table)
        .values(&RtdProgressStore {
            id: RTD_PROGRESS_STORE_DUMMY_KEY,
            txn_digest: tx_digest.inner().to_vec(),
        })
        .on_conflict(schema::rtd_progress_store::dsl::id)
        .do_update()
...
pub async fn read_rtd_progress_store(pool: &PgPool) -> anyhow::Result<Option<TransactionDigest>> {
    let mut conn = pool.get().await?;
    let val: Option<RtdProgressStore> = schema::rtd_progress_store::dsl::rtd_progress_store
        .select(RtdProgressStore::as_select())
        .first(&mut conn)
        .await
        .optional()?;
    match val {
