...
use crate::postgres_manager::{PgPool, update_rtd_progress_store, write};
...
use linku_metrics::metered_channel::{Receiver, ReceiverStream};
use rtd_types::BRIDGE_ADDRESS;
use tracing::{error, info};

pub(crate) const COMMIT_BATCH_SIZE: usize = 10;

pub async fn handle_rtd_transactions_loop(
    pg_pool: PgPool,
    ...
    while let Some(batch) = stream.next().await {
        ...
        if !data.is_empty() {
            ...
            while let Err(err) = write(&pg_pool, data.clone()).await {
                error!("Failed to write rtd transactions to DB: {:?}", err);
                tokio::time::sleep(Duration::from_secs(5)).await;
            }
            info!("Wrote {} bridge transaction data to DB", data.len());
            metrics.last_committed_rtd_checkpoint.set(last_ckp as i64);
        }

        // update rtd progress store using the latest cursor
        if let Some(cursor) = cursor {
            while let Err(err) = update_rtd_progress_store(&pg_pool, cursor).await {
                error!("Failed to update rtd progress tore DB: {:?}", err);
                tokio::time::sleep(Duration::from_secs(5)).await;
            }
            info!("Updated rtd transaction cursor to {}", cursor);
        }
    }
...
pub fn into_token_transfers(
    ...
    for ev in tx.events.data {
        ...
        match ev.type_.name.as_str() {
            "TokenDepositedEvent" => {
                ...
                metrics.total_rtd_token_deposited.inc();
            ...
            "TokenTransferApproved" => {
                ...
                metrics.total_rtd_token_transfer_approved.inc();
            ...
            "TokenTransferClaimed" => {
                ...
                metrics.total_rtd_token_transfer_claimed.inc();
            ...
            _ => {
                metrics.total_rtd_bridge_txn_other.inc();
            }
        }
    }
    if !transfers.is_empty() {
        info!(
            ?tx_digest,
            "RTD: Extracted {} bridge token transfer data entries",
            transfers.len(),
        );
    }
    Ok(transfers)
