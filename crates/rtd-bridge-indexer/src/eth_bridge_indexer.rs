...
use rtd_indexer_builder::indexer_builder::{DataMapper, DataSender, Datasource};
...
impl EthSubscriptionDatasource {
    pub async fn new(
        eth_rtd_bridge_contract_addresses: Vec<EthAddress>,
        eth_client: Arc<EthClient<MeteredEthHttpProvider>>,
        eth_ws_url: String,
        metrics: Box<dyn IndexerMetricProvider>,
        genesis_block: u64,
        ...
        Ok(Self {
            addresses: eth_rtd_bridge_contract_addresses,
            eth_client,
            eth_ws_url,
            metrics,
            genesis_block,
...
impl EthFinalizedSyncDatasource {
    pub async fn new(
        ...
        Ok(Self {
            bridge_addresses: eth_rtd_bridge_contract_addresses,
            eth_http_url,
            eth_client,
            metrics,
            bridge_metrics,
...
impl DataMapper<RawEthData, ProcessedTxnData> for EthDataMapper {
    fn map(
        ...
        match bridge_event {
            EthBridgeEvent::EthRtdBridgeEvents(bridge_event) => match &bridge_event {
                EthRtdBridgeEvents::TokensDepositedFilter(bridge_event) => {
                    info!(
                    ...
                    processed_txn_data.push(ProcessedTxnData::TokenTransfer(TokenTransfer {
                        ...
                        data: Some(TokenTransferData {
                            ...
                            amount: bridge_event.rtd_adjusted_amount,
                            is_finalized,
                        }),
                    }));
                }
                EthRtdBridgeEvents::TokensClaimedFilter(bridge_event) => {
                    info!(
                ...
                EthRtdBridgeEvents::EmergencyOperationFilter(f) => {
                    info!(
                ...
                EthRtdBridgeEvents::ContractUpgradedFilter(f) => {
                    info!(
                        "Observed Eth RtdBridge Upgrade at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
                ...
                | EthRtdBridgeEvents::PausedFilter(_)
                | EthRtdBridgeEvents::UnpausedFilter(_)
            ...
            EthBridgeEvent::EthBridgeConfigEvents(bridge_event) => match &bridge_event {
                ...
                EthBridgeConfigEvents::TokenAddedFilter(_) => {
                    info!(
                        "Observed Eth AddRtdTokens at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
