...
use crate::rtd_bridge_indexer::RtdBridgeDataMapper;
use ethers::providers::{Http, Provider};
use ethers::types::Address as EthAddress;
use std::str::FromStr;
use std::sync::Arc;
use rtd_bridge::eth_client::EthClient;
use rtd_bridge::metered_eth_provider::MeteredEthHttpProvider;
use rtd_bridge::metrics::BridgeMetrics;
use rtd_bridge::utils::get_eth_contract_addresses;
use rtd_bridge_schema::models::{
    BridgeDataSource, GovernanceAction as DBGovernanceAction, TokenTransferStatus,
};
use rtd_bridge_schema::models::{GovernanceActionType, TokenTransferData as DBTokenTransferData};
use rtd_bridge_schema::models::{RtdErrorTransactions, TokenTransfer as DBTokenTransfer};
use rtd_data_ingestion_core::DataIngestionMetrics;
use rtd_indexer_builder::indexer_builder::{BackfillStrategy, Datasource, Indexer, IndexerBuilder};
use rtd_indexer_builder::metrics::IndexerMetricProvider;
use rtd_indexer_builder::progress::{
    OutOfOrderSaveAfterDurationPolicy, ProgressSavingPolicy, SaveAfterDurationPolicy,
};
use rtd_indexer_builder::rtd_datasource::RtdCheckpointDatasource;
...
pub mod storage;
pub mod rtd_transaction_handler;
pub mod rtd_transaction_queries;
pub mod types;

pub mod eth_bridge_indexer;
pub mod rtd_bridge_indexer;
...
impl RtdTxnError {
    fn to_db(&self) -> RtdErrorTransactions {
        RtdErrorTransactions {
...
pub async fn create_rtd_indexer(
    pool: PgPool,
    metrics: BridgeIndexerMetrics,
    ingestion_metrics: DataIngestionMetrics,
    config: &IndexerConfig,
) -> anyhow::Result<
    Indexer<PgBridgePersistent, RtdCheckpointDatasource, RtdBridgeDataMapper>,
    anyhow::Error,
> {
    ...
    let rtd_client = Arc::new(
        RtdClientBuilder::default()
            .build(config.rtd_rpc_url.clone())
            .await?,
    );

    let rtd_checkpoint_datasource = RtdCheckpointDatasource::new(
        ...
        config.rtd_bridge_genesis_checkpoint,
        ingestion_metrics,
        metrics.clone().boxed(),
    );
    ...
    Ok(IndexerBuilder::new(
        "RtdBridgeIndexer",
        rtd_checkpoint_datasource,
        RtdBridgeDataMapper { metrics },
...
async fn get_eth_bridge_contract_addresses(
    ...
    let bridge_address = EthAddress::from_str(&config.eth_rtd_bridge_contract_address)?;
