...
use rtd_bridge::e2e_tests::test_utils::{
    BridgeTestCluster, BridgeTestClusterBuilder, initiate_bridge_eth_to_rtd,
};
use rtd_bridge_indexer::config::IndexerConfig;
use rtd_bridge_indexer::create_rtd_indexer;
use rtd_bridge_indexer::metrics::BridgeIndexerMetrics;
use rtd_bridge_indexer::postgres_manager::get_connection_pool;
use rtd_bridge_indexer::storage::PgBridgePersistent;
use rtd_bridge_schema::models::{GovernanceAction, TokenTransfer, TokenTransferStatus};
use rtd_bridge_schema::{MIGRATIONS, schema};
use rtd_data_ingestion_core::DataIngestionMetrics;
use rtd_indexer::database::Connection;
use rtd_indexer_builder::indexer_builder::IndexerProgressStore;
...
async fn test_indexing_transfer() {
    ...
    let indexer = create_rtd_indexer(pool.clone(), metrics.clone(), ingestion_metrics, &config)
        .await
        .unwrap();
    ...
    // transfer eth to rtd
    initiate_bridge_eth_to_rtd(&cluster, 1000, 0).await.unwrap();
...
async fn setup_bridge_env(with_eth_env: bool) -> (IndexerConfig, BridgeTestCluster, TempDb) {
    ...
    let config = IndexerConfig {
        remote_store_url: format!("{}/rest", bridge_test_cluster.rtd_rpc_url()),
        ...
        concurrency: 10,
        rtd_bridge_genesis_checkpoint: 0,
        eth_bridge_genesis_block: 0,
        eth_rtd_bridge_contract_address: bridge_test_cluster.rtd_bridge_address(),
        metric_port: 9001,
        eth_only: false,
    };
