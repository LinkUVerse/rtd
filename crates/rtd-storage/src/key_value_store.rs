...
impl TransactionKeyValueStore {
    ...
    ) -> RtdResult<Vec<Option<CertifiedCheckpointSummary>>> {
    ...
    pub async fn multi_get_checkpoints_contents(
        &self,
        keys: &[CheckpointSequenceNumber],
    ) -> RtdResult<Vec<Option<CheckpointContents>>> {
        self.multi_get_checkpoints(&[], keys, &[])
            .await
    ...
    ) -> RtdResult<Vec<Option<Transaction>>> {
    ...
    pub async fn multi_get_fx_by_tx_digest(
        &self,
        keys: &[TransactionDigest],
    ) -> RtdResult<Vec<Option<TransactionEffects>>> {
    ...
    pub async fn get_tx(&self, digest: TransactionDigest) -> RtdResult<Transaction> {
        self.multi_get_tx(&[digest])
            .await?
            .into_iter()
            .next()
            .flatten()
            .ok_or(RtdErrorKind::TransactionNotFound { digest }.into())
    }
    ...
    ) -> RtdResult<CertifiedCheckpointSummary> {
