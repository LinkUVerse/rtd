...
trait IntoRtdResult<T> {
    fn into_rtd_result(self) -> RtdResult<T>;
}

impl<T, E> IntoRtdResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_rtd_result(self) -> RtdResult<T> {
        self.map_err(|e| RtdErrorKind::Storage(e.to_string()).into())
    }
}
...
impl HttpKVStore {
    pub fn new_kv(
        ...
        cache_size: u64,
        metrics: Arc<KeyValueStoreMetrics>,
    ) -> RtdResult<TransactionKeyValueStore> {
    ...
    pub fn new(
        ...
        let base_url = Url::parse(&base_url).into_rtd_result()?;
    ...
    fn get_url(&self, key: &Key) -> RtdResult<Url> {
        let (digest, item_type) = key.to_path_elements();
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_rtd_result()?;
        Url::from_str(joined.as_str()).into_rtd_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<RtdResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> RtdResult<Option<Bytes>> {
        ...
        if resp.status().is_success() {
            let bytes = resp.bytes().await.into_rtd_result()?;
...
fn map_fetch<'a, K>(fetch: (&'a RtdResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
