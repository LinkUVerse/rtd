...
use rtd_types::crypto::EmptySignInfo;
use rtd_types::error::{RtdError, RtdResult};
use rtd_types::message_envelope::TrustedEnvelope;
use rtd_types::transaction::{SenderSignedData, VerifiedTransaction};
...
impl WritePathPendingTransactionLog {
    ...
    pub fn finish_transaction(&self, tx: &TransactionDigest) -> RtdResult {
        let mut transactions_set = self.transactions_set.lock();
        ...
        write_batch.write().map_err(RtdError::from)?;
