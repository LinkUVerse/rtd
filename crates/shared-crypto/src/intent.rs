...
/// (i.e., Narwhal, Rtd, Ethereum etc) should never collide, so that even when a signing
...
impl Intent {
    pub fn rtd_app(scope: IntentScope) -> Self {
        Self {
...
/// be any type that implements [trait Serialize]. *ALL* signatures in Rtd must commits
...
/// A 1-byte domain separator for hashing Object ID in Rtd. It is starting from 0xf0
/// to ensure no hashing collision for any ObjectID vs RtdAddress which is derived
/// as the hash of `flag || pubkey`. See `rtd_types::crypto::SignatureScheme::flag()`.
#[derive(Serialize_repr, Deserialize_repr, Copy, Clone, PartialEq, Eq, Debug, Hash)]
#[repr(u8)]
