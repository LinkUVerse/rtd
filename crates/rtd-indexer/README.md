Rtd indexer is an off-fullnode service to serve data from Rtd protocol, including both data directly generated from chain and derivative data.

&#9888; **Warning:** Rtd indexer is still experimental and we expect occasional breaking changes that require backfills.
...
See the [docs](https://docs.rtd.io/guides/developer/getting-started/local-network) for detailed information. Below is a quick start guide:

Start a local network using the `rtd` binary:
```sh
cargo run --bin rtd -- start --with-faucet --force-regenesis
```
...
```sh
cargo run --bin rtd -- start --with-faucet --force-regenesis --with-indexer --pg-port 5432 --pg-db-name rtd_indexer_v2
```

### Running standalone indexer
1. DB setup, under `rtd/crates/rtd-indexer` run:
```sh
...
```
3. Start indexer binary, under `rtd/crates/rtd-indexer` run:
- run indexer as a writer, which pulls data from fullnode and writes data to DB
```sh
# Change the RPC_CLIENT_URL to http://0.0.0.0:9000 to run indexer against local validator & fullnode
cargo run --bin rtd-indexer -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.rtd.io:443" --fullnode-sync-worker --reset-db
```
- run indexer as a reader, which is a JSON RPC server with the [interface](https://docs.rtd.io/rtd-api-ref#rtdx_getallbalances)
```
cargo run --bin rtd-indexer -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.rtd.io:443" --rpc-server-worker
```
...
```sh
cargo run --bin rtd-indexer -- --database-url "<DATABASE_URL>" reset-database --force
```
...
3.DB setup, under `rtd/crates/rtd-indexer` run:
...
```sh
# Change the RPC_CLIENT_URL to http://0.0.0.0:9000 to run indexer against local validator & fullnode
cargo run --bin rtd-indexer --features mysql-feature --no-default-features -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.rtd.io:443" --fullnode-sync-worker --reset-db
```
