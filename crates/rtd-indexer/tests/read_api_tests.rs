...
use std::sync::Arc;
use rtd_indexer::apis::read_api::ReadApi;
use rtd_indexer::indexer_reader::IndexerReader;
use rtd_indexer::test_utils::{set_up, wait_for_checkpoint};
use rtd_json_rpc_api::ReadApiServer;
...
async fn test_checkpoint_apis() -> RpcResult<()> {
    ...
    // Test get_checkpoint
    let checkpoint_id = rtd_json_rpc_types::CheckpointId::SequenceNumber(1);
