...
use rtd_json_rpc_api::{CoinReadApiClient, IndexerApiClient, ReadApiClient};
use rtd_json_rpc_types::{
    CoinPage, EventFilter, RtdObjectDataOptions, RtdObjectResponse, RtdObjectResponseQuery,
};
use rtd_swarm_config::genesis_config::DEFAULT_GAS_AMOUNT;
use rtd_test_transaction_builder::publish_package;
use rtd_types::{event::EventID, transaction::CallArg};
...
async fn test_get_owned_objects() -> Result<(), anyhow::Error> {
    ...
    let data_option = RtdObjectDataOptions::new().with_owner();
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    // We should get all the 5 coins for RTD with the right balance.
    let result: CoinPage = http_client
