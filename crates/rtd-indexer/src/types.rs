...
use rtd_json_rpc_types::{
    ObjectChange, RtdTransactionBlockResponse, RtdTransactionBlockResponseOptions,
};
...
pub fn owner_to_owner_info(owner: &Owner) -> (OwnerType, Option<RtdAddress>) {
    match owner {
...
impl IndexedObject {
    pub fn random() -> Self {
        let mut rng = rand::thread_rng();
        let random_address = RtdAddress::random_for_testing_only();
        IndexedObject {
...
pub struct IndexedTransaction {
    ...
    pub balance_change: Vec<rtd_json_rpc_types::BalanceChange>,
    pub events: Vec<rtd_types::event::Event>,
...
pub struct TxIndex {
    ...
    pub payers: Vec<RtdAddress>,
    pub sender: RtdAddress,
    pub recipients: Vec<RtdAddress>,
    ...
    pub fn random() -> Self {
        ...
        TxIndex {
            ...
            payers: (0..rng.gen_range(0..100))
                .map(|_| RtdAddress::random_for_testing_only())
                .collect(),
            sender: RtdAddress::random_for_testing_only(),
...
// RtdTransactionBlockResponseWithOptions is only used on the reading path
pub struct RtdTransactionBlockResponseWithOptions {
    pub response: RtdTransactionBlockResponse,
    pub options: RtdTransactionBlockResponseOptions,
}

impl From<RtdTransactionBlockResponseWithOptions> for RtdTransactionBlockResponse {
    fn from(value: RtdTransactionBlockResponseWithOptions) -> Self {
        let RtdTransactionBlockResponseWithOptions { response, options } = value;
