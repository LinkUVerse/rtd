...
use linku_metrics::{get_metrics, spawn_monitored_task};
use rtd_data_ingestion_core::Worker;
use rtd_types::dynamic_field::DynamicFieldType;
use rtd_types::effects::{ObjectChange, TransactionEffectsAPI};
use rtd_types::event::SystemEpochInfoEvent;
...
use rtd_types::rtd_system_state::{RtdSystemStateTrait, get_rtd_system_state};
...
pub async fn new_handlers(
    ...
    let (indexed_checkpoint_sender, indexed_checkpoint_receiver) =
        linku_metrics::metered_channel::channel(
...
pub struct CheckpointHandler {
    state: PgIndexerStore,
    metrics: IndexerMetrics,
    indexed_checkpoint_sender: linku_metrics::metered_channel::Sender<CheckpointDataToCommit>,
}
...
impl CheckpointHandler {
    ...
    async fn index_epoch(
        ...
        if *checkpoint_summary.sequence_number() == 0 {
            ...
            let system_state_summary =
                get_rtd_system_state(&checkpoint_object_store)?.into_rtd_system_state_summary();
        ...
        if epoch_event_opt.is_none() {
            ...
            assert!(
                ...
                "Rtd is not in safe mode but no SystemEpochInfoEvent found at end of epoch {}",
                checkpoint_summary.epoch
            );
        }
    ...
    async fn index_checkpoint(
        ...
        let (checkpoint, db_transactions, db_events, db_tx_indices, db_event_indices, db_displays) = {
            ...
            (
                IndexedCheckpoint::from_rtd_checkpoint(
                    checkpoint_summary,
                    checkpoint_contents,
        ...
        checkpoint_data
            ...
            .flat_map(|data| {
                ...
                data.transactions
                    ...
                    .flat_map(|tx| &tx.output_objects)
                    .filter_map(|o| {
                        if let rtd_types::object::Data::Package(p) = &o.data {
                            Some(IndexedPackage {
                                package_id: o.id(),
