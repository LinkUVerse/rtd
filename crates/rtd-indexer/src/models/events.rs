...
use rtd_json_rpc_types::{BcsEvent, RtdEvent, type_and_fields_from_move_event_data};
...
impl StoredEvent {
    pub async fn try_into_rtd_event(
        self,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> Result<RtdEvent, IndexerError> {
        ...
        // Note: RtdEvent only has one sender today, so we always use the first one.
        let sender = {
        ...
        let sender = match sender {
            Some(s) => RtdAddress::from_bytes(s).map_err(|_e| {
                IndexerError::PersistentStorageDataCorruptionError(format!(
        ...
        let type_ = parse_rtd_struct_tag(&self.event_type)?;
