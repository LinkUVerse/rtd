...
use rtd_json_rpc::coin_api::parse_to_struct_tag;
use rtd_json_rpc_types::{Balance, Coin as RtdCoin};
...
use rtd_types::dynamic_field::{DynamicFieldType, Field};
use rtd_types::object::{Object, ObjectRead};
...
impl StoredObject {
    pub async fn try_into_object_read(
        ...
        let object: rtd_types::object::Object = self.try_into()?;
...
impl TryFrom<StoredObject> for RtdCoin {
    type Error = IndexerError;
...
mod tests {
    ...
    #[test]
    fn test_convert_stored_obj_to_rtd_coin() {
        let test_obj = Object::new_gas_for_testing();
        let indexed_obj = IndexedObject::from_object(1, test_obj, None);

        let stored_obj = StoredObject::from(indexed_obj);

        let rtd_coin = RtdCoin::try_from(stored_obj).unwrap();
        assert_eq!(rtd_coin.coin_type, "0x2::rtd::RTD");
    }
    ...
    fn test_output_format_coin_balance() {
        ...
        assert_eq!(balance.coin_type, "0x2::rtd::RTD");
    }

    #[test]
    fn test_vec_of_coin_rtd_conversion() {
        // 0xe7::vec_coin::VecCoin<vector<0x2::coin::Coin<0x2::rtd::RTD>>>
