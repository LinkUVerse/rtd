...
use rtd_json_rpc_types::{
    BalanceChange, ObjectChange, RtdEvent, RtdTransactionBlock, RtdTransactionBlockEffects,
...
impl StoredTransaction {
    ...
    pub async fn try_into_rtd_transaction_block_response(
        self,
        options: RtdTransactionBlockResponseOptions,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> IndexerResult<RtdTransactionBlockResponse> {
        ...
        let transaction = if options.show_input {
            ...
            let tx_block = RtdTransactionBlock::try_from_with_package_resolver(
                sender_signed_data,
                &package_resolver,
            )
            .await?;
        ...
        let effects = if options.show_effects {
            let effects = self.try_into_rtd_transaction_effects()?;
            Some(effects)
        } else {
            None
        };
        ...
        let events = if options.show_events {
            ...
            tx_events_to_rtd_tx_events(tx_events, package_resolver, tx_digest, timestamp).await?
        } else {
            None
        };
        ...
        Ok(RtdTransactionBlockResponse {
            digest: tx_digest,
            transaction,
            raw_transaction,
            effects,
    ...
    pub fn try_into_rtd_transaction_effects(&self) -> IndexerResult<RtdTransactionBlockEffects> {
        ...
        let effects = RtdTransactionBlockEffects::try_from(effects)?;
        Ok(effects)
    }
}
...
pub async fn tx_events_to_rtd_tx_events(
    tx_events: TransactionEvents,
    package_resolver: Arc<Resolver<impl PackageStore>>,
    tx_digest: TransactionDigest,
    timestamp: u64,
) -> Result<Option<RtdTransactionBlockEvents>, IndexerError> {
    ...
    for tx_event in tx_events.data.clone() {
        ...
        rtd_event_futures.push(tokio::task::spawn(async move {
    ...
    let event_move_type_layouts = futures::future::join_all(rtd_event_futures)
        .await
        .into_iter()
        ...
        .map_err(|e| {
            IndexerError::ResolveMoveStructError(format!(
                "Failed to convert to rtd event with Error: {e}",
            ))
        })?;
    ...
    let rtd_events = tx_events
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let rtd_tx_events = RtdTransactionBlockEvents { data: rtd_events };
    Ok(Some(rtd_tx_events))
}
