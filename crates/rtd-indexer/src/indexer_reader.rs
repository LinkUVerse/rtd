...
use rtd_json_rpc_types::{Balance, Coin as RtdCoin, RtdCoinMetadata, RtdMoveValue};
use rtd_json_rpc_types::{
    CheckpointId, EpochInfo, EventFilter, RtdEvent, RtdObjectDataFilter,
    RtdTransactionBlockResponse, TransactionFilter,
};
use rtd_package_resolver::Package;
use rtd_package_resolver::PackageStore;
use rtd_package_resolver::{PackageStoreWithLruCache, Resolver};
use rtd_types::effects::TransactionEvents;
use rtd_types::{balance::Supply, coin::TreasuryCap, dynamic_field::DynamicFieldName};
use rtd_types::{
    base_types::{ObjectID, RtdAddress, VersionNumber},
    committee::EpochId,
    digests::TransactionDigest,
    dynamic_field::DynamicFieldInfo,
    object::{Object, ObjectRead},
    rtd_system_state::{RtdSystemStateTrait, rtd_system_state_summary::RtdSystemStateSummary},
};
use rtd_types::{coin::CoinMetadata, event::EventID};
...
use crate::{
    ...
    models::{
        ...
        transactions::{StoredTransaction, tx_events_to_rtd_tx_events},
...
impl IndexerReader {
    ...
    pub async fn get_latest_rtd_system_state(&self) -> Result<RtdSystemStateSummary, IndexerError> {
        ...
        let system_state = tokio::task::spawn_blocking(move || {
            rtd_types::rtd_system_state::get_rtd_system_state(&object_store)
        })
        .await
        .unwrap()?
    ...
    pub async fn get_validator_from_table(
        &self,
        table_id: ObjectID,
        pool_id: rtd_types::id::ID,
    ) -> Result<
        rtd_types::rtd_system_state::rtd_system_state_summary::RtdValidatorSummary,
        IndexerError,
    > {
        let object_store = ConnectionAsObjectStore::from_pool(&self.pool)
            .await
        ...
        let validator = tokio::task::spawn_blocking(move || {
            rtd_types::rtd_system_state::get_validator_from_table(&object_store, table_id, &pool_id)
        })
        .await
        .unwrap()?;
        Ok(validator)
    ...
    /// `get_latest_rtd_system_state` for current epoch instead.
    pub async fn get_epoch_rtd_system_state(
        &self,
        epoch: Option<EpochId>,
    ) -> Result<RtdSystemStateSummary, IndexerError> {
        let stored_epoch = self.get_epoch_info_from_db(epoch).await?;
        let stored_epoch = match stored_epoch {
            Some(stored_epoch) => stored_epoch,
    ...
    pub async fn get_checkpoint(
        &self,
        checkpoint_id: CheckpointId,
    ) -> Result<Option<rtd_json_rpc_types::Checkpoint>, IndexerError> {
        let stored_checkpoint = match self.get_checkpoint_from_db(checkpoint_id).await? {
            Some(stored_checkpoint) => stored_checkpoint,
            None => return Ok(None),
        };

        let checkpoint = rtd_json_rpc_types::Checkpoint::try_from(stored_checkpoint)?;
        Ok(Some(checkpoint))
    }

    pub async fn get_latest_checkpoint(
        &self,
    ) -> Result<rtd_json_rpc_types::Checkpoint, IndexerError> {
        let stored_checkpoint = self.get_latest_checkpoint_from_db().await?;

        rtd_json_rpc_types::Checkpoint::try_from(stored_checkpoint)
    }
    ...
    pub async fn get_checkpoints(
        ...
        limit: usize,
        descending_order: bool,
    ) -> Result<Vec<rtd_json_rpc_types::Checkpoint>, IndexerError> {
        self.get_checkpoints_from_db(cursor, limit, descending_order)
            .await?
            .into_iter()
            .map(rtd_json_rpc_types::Checkpoint::try_from)
            .collect()
    }
    ...
    async fn stored_transaction_to_transaction_block(
        ...
        options: rtd_json_rpc_types::RtdTransactionBlockResponseOptions,
    ) -> IndexerResult<Vec<RtdTransactionBlockResponse>> {
        ...
        for stored_tx in stored_txes {
            ...
            tx_block_responses_futures.push(tokio::task::spawn(
                stored_tx
                    .try_into_rtd_transaction_block_response(options_clone, package_resolver_clone),
            ));
        }
    ...
    pub async fn get_owned_objects(
        ...
        if let Some(filter) = filter {
            match filter {
                RtdObjectDataFilter::StructType(struct_tag) => {
                ...
                RtdObjectDataFilter::MatchAny(filters) => {
                    let mut condition = "(".to_string();
                    for (i, filter) in filters.iter().enumerate() {
                        if let RtdObjectDataFilter::StructType(struct_tag) = filter {
                            let object_type =
                ...
                RtdObjectDataFilter::MatchNone(filters) => {
    ...
    async fn multi_get_transaction_block_response_in_blocking_task_impl(
        ...
        digests: &[TransactionDigest],
        options: rtd_json_rpc_types::RtdTransactionBlockResponseOptions,
    ) -> Result<Vec<rtd_json_rpc_types::RtdTransactionBlockResponse>, IndexerError> {
    ...
    pub async fn get_transaction_events(
    ...
    ) -> Result<Vec<rtd_json_rpc_types::RtdEvent>, IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        let rtd_tx_events = tx_events_to_rtd_tx_events(
            tx_events,
        ...
        .await?;
        Ok(rtd_tx_events.map_or(vec![], |ste| ste.data))
    }
    ...
    async fn query_events_by_tx_digest(
        ...
        let mut rtd_event_futures = vec![];
        for stored_event in stored_events {
            rtd_event_futures.push(tokio::task::spawn(
                stored_event.try_into_rtd_event(self.package_resolver.clone()),
            ));
        }

        let rtd_events = futures::future::join_all(rtd_event_futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to join rtd event futures: {}", e))?
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to collect rtd event futures: {}", e))?;
        Ok(rtd_events)
    }
    ...
    pub async fn bcs_name_from_dynamic_field_name(
        ...
        let rtd_json_value = rtd_json::RtdJsonValue::new(name.value.clone())?;
        let name_bcs_value = rtd_json_value.to_bcs_bytes(&move_type_layout)?;
        Ok(name_bcs_value)
    }
    ...
    pub async fn get_owned_coins(
        ...
        cursor: ObjectID,
        limit: usize,
    ) -> Result<Vec<RtdCoin>, IndexerError> {
        use diesel_async::RunQueryDsl;
...
impl rtd_types::storage::ObjectStore for ConnectionAsObjectStore {
    fn get_object(&self, object_id: &ObjectID) -> Option<rtd_types::object::Object> {
    ...
    ) -> Option<rtd_types::object::Object> {
