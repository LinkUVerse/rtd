...
use rtd_json_rpc::{RtdRpcModule, governance_api::ValidatorExchangeRates};
use rtd_json_rpc_api::GovernanceReadApiServer;
use rtd_json_rpc_types::{
    DelegatedStake, EpochInfo, StakeStatus, RtdCommittee, RtdObjectDataFilter, ValidatorApys,
};
use rtd_open_rpc::Module;
use rtd_types::{
    base_types::{MoveObjectType, ObjectID, RtdAddress},
    committee::EpochId,
    governance::StakedRtd,
    rtd_serde::BigInt,
    rtd_system_state::{PoolTokenExchangeRate, rtd_system_state_summary::RtdSystemStateSummary},
};
...
impl GovernanceReadApi {
    ...
    async fn get_latest_rtd_system_state(&self) -> Result<RtdSystemStateSummary, IndexerError> {
        self.inner.get_latest_rtd_system_state().await
    }
    ...
    async fn get_stakes_by_ids(
        ...
        for stored_object in self.inner.multi_get_objects(ids).await? {
            let object = rtd_types::object::Object::try_from(stored_object)?;
            let stake_object = StakedRtd::try_from(&object)?;
    ...
    async fn get_staked_by_owner(
        ...
        for stored_object in self
            ...
            .get_owned_objects(
                owner,
                Some(RtdObjectDataFilter::StructType(
                    MoveObjectType::staked_rtd().into(),
                )),
                None,
    ...
    pub async fn get_delegated_stakes(
        ...
        let system_state_summary = self.get_latest_rtd_system_state().await?;
        ...
        for (pool_id, stakes) in pools {
            ...
            for stake in stakes {
                ...
                delegations.push(rtd_json_rpc_types::Stake {
...
pub async fn exchange_rates(
    ...
    system_state_summary: &RtdSystemStateSummary,
    ...
    for df in state
        ...
        .await?
    {
        let pool_id: rtd_types::id::ID = bcs::from_bytes(&df.bcs_name).map_err(|e| {
            rtd_types::error::RtdErrorKind::ObjectDeserializationError {
                error: e.to_string(),
            }
        })?;
...
impl GovernanceReadApiServer for GovernanceReadApi {
    async fn get_stakes_by_ids(
        ...
        self.get_stakes_by_ids(staked_rtd_ids)
            .await
            .map_err(Into::into)
    }
    ...
    async fn get_latest_rtd_system_state(&self) -> RpcResult<RtdSystemStateSummary> {
        self.get_latest_rtd_system_state().await.map_err(Into::into)
    }
...
impl RtdRpcModule for GovernanceReadApi {
    ...
    fn rpc_doc_module() -> Module {
        rtd_json_rpc_api::GovernanceReadApiOpenRpc::module_doc()
    }
}
