...
use rtd_json_rpc_api::{IndexerApiServer, cap_page_limit};
use rtd_json_rpc_types::{
    DynamicFieldPage, EventFilter, EventPage, ObjectsPage, Page, RtdObjectResponse,
    RtdObjectResponseQuery, RtdTransactionBlockResponseQuery, TransactionBlocksPage,
    TransactionFilter,
};
use rtd_name_service::{Domain, NameRecord, NameServiceConfig, NameServiceError};
...
use rtd_types::dynamic_field::{DynamicFieldName, Field};
...
impl IndexerApi {
    ...
    async fn get_owned_objects_internal(
        ...
        let RtdObjectResponseQuery { filter, options } = query.unwrap_or_default();
...
impl IndexerApiServer for IndexerApi {
    ...
    async fn get_dynamic_field_object(
        ...
        let id = rtd_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
            &name.type_,
            &name_bcs_value,
        )
        ...
        let options = rtd_json_rpc_types::RtdObjectDataOptions::full_content();
        match self.inner.get_object_read(id).await? {
            rtd_types::object::ObjectRead::NotExists(_)
            | rtd_types::object::ObjectRead::Deleted(_) => {}
            rtd_types::object::ObjectRead::Exists(object_ref, o, layout) => {
                return Ok(RtdObjectResponse::new_with_data(
        ...
        let dynamic_object_field_struct =
            rtd_types::dynamic_field::DynamicFieldInfo::dynamic_object_field_wrapper(name.type_);
        let dynamic_object_field_type = TypeTag::Struct(Box::new(dynamic_object_field_struct));
        let dynamic_object_field_id = rtd_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
        ...
        Ok(RtdObjectResponse::new_with_error(
            rtd_types::error::RtdObjectResponseError::DynamicFieldNotFound { parent_object_id },
        ))
    }
    ...
    async fn resolve_name_service_address(&self, name: String) -> RpcResult<Option<RtdAddress>> {
        ...
        let domains: Vec<_> = self
            ...
            .await?
            .into_iter()
            .map(|o| rtd_types::object::Object::try_from(o).ok())
            .collect();
    ...
    async fn resolve_name_service_names(
        ...
        let domain = field_reverse_record_object
            .to_rust::<Field<RtdAddress, Domain>>()
            .ok_or_else(|| {
...
impl RtdRpcModule for IndexerApi {
    fn rpc(self) -> RpcModule<Self> {
        self.into_rpc()
    }

    fn rpc_doc_module() -> Module {
        rtd_json_rpc_api::IndexerApiOpenRpc::module_doc()
    }
}
