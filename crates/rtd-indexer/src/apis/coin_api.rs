...
use rtd_json_rpc::coin_api::{parse_to_struct_tag, parse_to_type_tag};
use rtd_json_rpc::error::RtdRpcInputError;
use rtd_json_rpc_api::{CoinReadApiServer, cap_page_limit};
use rtd_json_rpc_types::{Balance, CoinPage, Page, RtdCoinMetadata};
...
impl CoinReadApiServer for CoinReadApi {
    async fn get_coins(
        ...
        let cursor = match cursor {
            Some(c) => c
                .parse()
                .map_err(|e| RtdRpcInputError::GenericInvalid(format!("invalid cursor: {e}")))?,
...
impl RtdRpcModule for CoinReadApi {
    ...
    fn rpc_doc_module() -> Module {
        rtd_json_rpc_api::CoinReadApiOpenRpc::module_doc()
    }
}
