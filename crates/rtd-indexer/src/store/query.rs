...
use rtd_json_rpc_types::RtdObjectDataFilter;
...
impl DBFilter<ObjectID> for RtdObjectDataFilter {
...
fn to_latest_objects_clauses(filter: &RtdObjectDataFilter) -> Option<String> {
    match filter {
        RtdObjectDataFilter::AddressOwner(a) => Some(format!(
...
fn to_clauses(filter: &RtdObjectDataFilter) -> Option<String> {
    match filter {
        RtdObjectDataFilter::MatchAll(sub_filters) => {
        ...
        RtdObjectDataFilter::MatchAny(sub_filters) => {
            let sub_filters = sub_filters.iter().flat_map(to_clauses).collect::<Vec<_>>();
            if sub_filters.is_empty() {
        ...
        RtdObjectDataFilter::MatchNone(sub_filters) => {
            let sub_filters = sub_filters.iter().flat_map(to_clauses).collect::<Vec<_>>();
            if sub_filters.is_empty() {
                None
            } else {
        ...
        RtdObjectDataFilter::Package(p) => Some(format!("o.object_type LIKE '{}::%'", p.to_hex_literal())),
        RtdObjectDataFilter::MoveModule { package, module } => Some(format!(
            "o.object_type LIKE '{}::{}::%'",
        ...
        RtdObjectDataFilter::ObjectId(id) => {
            Some(format!("o.object_id = '{id}'"))
        }
        RtdObjectDataFilter::ObjectIds(ids) => {
            if ids.is_empty() {
                None
            } else {
                let ids = ids
        ...
        RtdObjectDataFilter::Version(v) => Some(format!("o.version = {v}")),
    }
}

fn to_outer_clauses(filter: &RtdObjectDataFilter) -> Option<String> {
    match filter {
        RtdObjectDataFilter::MatchNone(sub_filters) => {
        ...
        RtdObjectDataFilter::AddressOwner(a) => Some(format!("t1.owner_address = '{a}'")),
        _ => None,
    }
}
...
mod test {
    ...
    fn test_address_filter() {
        let address = RtdAddress::from_str(
            "0x92dd4d9b0150c251661d821583ef078024ae9e9ee11063e216500861eec7f381",
        )
        .unwrap();
        let filter = RtdObjectDataFilter::AddressOwner(address);
...
LIMIT 100;";
    ...
    fn test_move_module_filter() {
        let filter = RtdObjectDataFilter::MoveModule {
...
LIMIT 100;";
    ...
    fn test_empty_all_filter() {
        let filter = RtdObjectDataFilter::MatchAll(vec![]);
        let expected_sql = "SELECT t1.*
FROM (SELECT DISTINCT ON (o.object_id) *
      FROM objects_history o
      WHERE o.checkpoint <= $1
...
LIMIT 100;";
    ...
    fn test_empty_any_filter() {
        let filter = RtdObjectDataFilter::MatchAny(vec![]);
        let expected_sql = "SELECT t1.*
FROM (SELECT DISTINCT ON (o.object_id) *
      FROM objects_history o
      WHERE o.checkpoint <= $1
...
LIMIT 100;";
    ...
    fn test_all_filter() {
        let filter = RtdObjectDataFilter::MatchAll(vec![
            RtdObjectDataFilter::ObjectId(
                ObjectID::from_str(
                    "0xef9fb75a7b3d4cb5551ef0b08c83528b94d5f5cd8be28b1d08a87dbbf3731738",
                )
                .unwrap(),
            ),
            RtdObjectDataFilter::StructType(parse_rtd_struct_tag("0x2::test::Test").unwrap()),
        ]);
