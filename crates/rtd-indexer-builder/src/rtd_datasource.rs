...
use linku_metrics::{metered_channel, spawn_monitored_task};
...
use rtd_types::full_checkpoint_content::CheckpointData as RtdCheckpointData;
...
pub struct RtdCheckpointDatasource {
...
#[async_trait]
impl Datasource<CheckpointTxnData> for RtdCheckpointDatasource {
    async fn start_data_retrieval(
        ...
        tracing::info!(
            "Starting Rtd checkpoint data retrieval with batch size {}",
...
impl Worker for IndexerWorker<CheckpointTxnData> {
    ...
    async fn process_checkpoint(&self, checkpoint: &RtdCheckpointData) -> anyhow::Result<()> {
        tracing::trace!(
            "Received checkpoint [{}] {}: {}",
