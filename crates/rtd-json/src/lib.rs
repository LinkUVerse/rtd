...
use rtd_types::transfer::RESOLVED_RECEIVING_STRUCT;
...
pub enum RtdJsonValueErrorKind {
...
#[derive(Debug)]
pub struct RtdJsonValueError {
    kind: RtdJsonValueErrorKind,
    val: JsonValue,
}

impl RtdJsonValueError {
    pub fn new(val: &JsonValue, kind: RtdJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for RtdJsonValueError {}

impl fmt::Display for RtdJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            RtdJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            RtdJsonValueErrorKind::ArrayNotHomogeneous => {
...
pub struct RtdJsonValue(JsonValue);
impl RtdJsonValue {
    pub fn new(json_value: JsonValue) -> Result<RtdJsonValue, anyhow::Error> {
    ...
    pub fn from_object_id(id: ObjectID) -> RtdJsonValue {
    ...
    pub fn to_rtd_address(&self) -> anyhow::Result<RtdAddress> {
        json_value_to_rtd_address(&self.0)
    }
    ...
    pub fn to_move_value(
        ...
        Ok(match (val, ty) {
                ...
                if struct_layout.type_ == ID::type_() =>
            {
                ...
                let addr = RtdAddress::from_str(s)?;
            ...
            (v, MoveTypeLayout::Address) => {
                let addr = json_value_to_rtd_address(v)?;
                R::MoveValue::Address(addr.into())
            }
...
impl Debug for RtdJsonValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

fn json_value_to_rtd_address(value: &JsonValue) -> anyhow::Result<RtdAddress> {
    match value {
        JsonValue::String(s) => {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into RtdAddress."))?;
            Ok(RtdAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(RtdAddress::from(*v).to_string()),
...
impl FromStr for RtdJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        RtdJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), RtdJsonValueError> {
...
fn check_valid_homogeneous_rec(curr_q: &mut VecDeque<&JsonValue>) -> Result<(), RtdJsonValueError> {
    if curr_q.is_empty() {
        // Nothing to do
        return Ok(());
    }
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(RtdJsonValueError::new(
                    v,
                    RtdJsonValueErrorKind::ValueTypeNotAllowed,
                ));
            }
...
pub fn primitive_type(
    ...
    Some(match param {
        ...
        SignatureToken::Datatype(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_RTD_ID {
...
fn resolve_object_vec_arg(idx: usize, arg: &RtdJsonValue) -> Result<Vec<ObjectID>, anyhow::Error> {
    // Every elem has to be a string convertible to a ObjectID
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    type_args: &[TypeTag],
    combined_args_json: Vec<RtdJsonValue>,
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(rtd_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait RtdJsonArg {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue>;
        }
        // TODO: anyway to condense this?
        impl RtdJsonArg for &str {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue> {
                RtdJsonValue::from_str(self)
            }
        }
        impl RtdJsonArg for String {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue> {
                RtdJsonValue::from_str(&self)
            }
        }
        impl RtdJsonArg for rtd_types::base_types::ObjectID {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue> {
                RtdJsonValue::from_str(&self.to_string())
            }
        }
        impl RtdJsonArg for rtd_types::base_types::RtdAddress {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue> {
                RtdJsonValue::from_str(&self.to_string())
            }
        }
        impl RtdJsonArg for u64 {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue> {
                RtdJsonValue::from_bcs_bytes(
                    Some(&rtd_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl RtdJsonArg for Vec<u8> {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue> {
                RtdJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl RtdJsonArg for &[u8] {
            fn to_rtd_json(&self) -> anyhow::Result<RtdJsonValue> {
                RtdJsonValue::from_bcs_bytes(None, self)
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use rtd_json_rpc_types::RtdTypeTag;
        use rtd_types::TypeTag;
        trait RtdJsonTypeArg {
            fn to_rtd_json(&self) -> anyhow::Result<RtdTypeTag>;
        }
        impl <T: core::fmt::Display> RtdJsonTypeArg for T {
            fn to_rtd_json(&self) -> anyhow::Result<RtdTypeTag> {
                Ok(rtd_types::parse_rtd_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_rtd_json()?,)*])
    }};
    }
