...
use crate::middleware::{
    expect_content_length, expect_linku_proxy_header, expect_valid_public_key,
};
use crate::peers::{AllowedPeer, RtdNodeProvider};
use crate::var;
use anyhow::Error;
use anyhow::Result;
...
pub fn app(
    ...
    allower: Option<RtdNodeProvider>,
    ...
    let mut router = Router::new()
        ...
        .route_layer(middleware::from_fn(expect_linku_proxy_header))
...
pub fn create_server_cert_default_allow(
    hostname: String,
) -> Result<ServerConfig, rtd_tls::rustls::Error> {
    let CertKeyPair(server_certificate, _) = generate_self_cert(hostname);

    ClientCertVerifier::new(AllowAll, RTD_VALIDATOR_SERVER_NAME.to_string()).rustls_server_config(
        vec![server_certificate.rustls_certificate()],
        server_certificate.rustls_private_key(),
    )
}

/// Verify clients against rtd blockchain, clients that are not found in rtd_getValidators
/// will be rejected
pub fn create_server_cert_enforce_peer(
    ...
    let (Some(certificate_path), Some(private_key_path)) =
        ...
        return Err(rtd_tls::rustls::Error::General(
