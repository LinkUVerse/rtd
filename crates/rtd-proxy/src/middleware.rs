...
use crate::{consumer::ProtobufDecoder, peers::RtdNodeProvider};
use axum::{
    body::Body,
    body::Bytes,
...
/// we expect rtd-node to send us an http header content-length encoding.
...
/// we expect rtd-node to send us an http header content-type encoding.
pub async fn expect_linku_proxy_header(
    ...
    match format!("{content_type}").as_str() {
        ...
        ct => {
            ...
            MIDDLEWARE_OPS
                .with_label_values(&["expect_linku_proxy_header", "invalid-content-type"])
                .inc();
...
/// we expect that calling rtd-nodes are known on the blockchain and we enforce
/// their pub key tls creds here
pub async fn expect_valid_public_key(
    Extension(allower): Extension<Arc<RtdNodeProvider>>,
