...
mod tests {
    ...
    use crate::{admin::CertKeyPair, config::RemoteWriteConfig, peers::RtdNodeProvider};
    use axum::Router;
    ...
    /// rtd-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn test_axum_acceptor() {
        ...
        let CertKeyPair(client_priv_cert, client_pub_key) = admin::generate_self_cert("rtd".into());
        ...
        let mut allower = RtdNodeProvider::new("".into(), Duration::from_secs(30), vec![]);
        ...
        allower.get_rtd_mut().write().unwrap().insert(
