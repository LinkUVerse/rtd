...
use rtd_indexer_alt_schema::schema::watermarks;
use rtd_name_service::{Domain, NameRecord, NameServiceError};
use rtd_types::{base_types::RtdAddress, dynamic_field::Field};
use tokio::join;
...
/// Attempt to translate the given RtdNS `name` to its address, as long as the mapping exists,
/// and it hasn't expired.
pub(super) async fn resolved_address(
    ctx: &Context,
    name: &str,
) -> Result<Option<RtdAddress>, RpcError<Error>> {
    use Error as E;
...
/// Attempt to translate the given `address` to its RtdNS name, as long as the reverse mapping
...
pub(super) async fn resolved_name(
    ...
    let reverse_record: Field<RtdAddress, Domain> = bcs::from_bytes(
