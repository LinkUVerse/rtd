...
#[open_rpc(namespace = "rtdx", tag = "Name Service API")]
#[rpc(server, namespace = "rtdx")]
trait NameServiceApi {
    /// Resolve a RtdNS name to its address
    ...
    /// Find the RtdNS name that points to this address.
    ///
...
impl NameServiceApiServer for NameService {
    async fn resolve_name_service_address(&self, name: String) -> RpcResult<Option<RtdAddress>> {
        ...
        Ok(response::resolved_address(ctx, &name)
            .await
            .with_internal_context(|| format!("Resolving RtdNS name {name:?}"))?)
    }
