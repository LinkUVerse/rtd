...
use rtd_json_rpc_types::{
    DisplayFieldsResponse, RtdData, RtdObjectData, RtdObjectDataOptions, RtdObjectResponse,
    RtdParsedData, RtdPastObjectResponse, RtdRawData,
};
use rtd_types::{
    TypeTag,
...
) -> Result<RtdObjectResponse, RpcError> {
    let Some(object) = load_live(ctx, object_id)
        .await
        .context("Failed to load latest object")?
    else {
        return Ok(RtdObjectResponse::new_with_error(
...
pub(super) async fn past_object(
    ...
    options: &RtdObjectDataOptions,
) -> Result<RtdPastObjectResponse, RpcError> {
    let Some(object) = ctx
        .kv_loader()
        ...
        return Ok(RtdPastObjectResponse::VersionNotFound(object_id, version));
    };
...
pub(crate) async fn object_data_with_options(
    ...
    let content: OptionFuture<_> = options
        .show_content
        .then(|| object_data::<RtdParsedData>(ctx, &object))
        .into();

    let bcs: OptionFuture<_> = options
        .show_bcs
        .then(|| object_data::<RtdRawData>(ctx, &object))
        .into();
...
async fn object_data<D: RtdData>(ctx: &Context, object: &Object) -> Result<D, RpcError> {
...
async fn display(ctx: &Context, object: &Object) -> DisplayFieldsResponse {
    ...
    DisplayFieldsResponse {
        ...
        error: (!field_errors.is_empty()).then_some(RtdObjectResponseError::DisplayError {
            error: field_errors,
        }),
    }
}
