...
use rtd_json_rpc_types::{Page as PageResponse, RtdObjectDataOptions};
use rtd_sql_macro::sql;
use rtd_types::{
    Identifier, RTD_FRAMEWORK_ADDRESS, TypeTag,
    base_types::{ObjectID, RtdAddress},
    dynamic_field::{DYNAMIC_FIELD_FIELD_STRUCT_NAME, DYNAMIC_FIELD_MODULE_NAME},
    rtd_serde::RtdStructTag,
};
...
pub(crate) struct RtdObjectResponseQuery {
...
pub(crate) enum RtdObjectDataFilter {
...
/// [RtdObjectDataFilter] converted into fields that can be compared directly with values coming
/// from the database.
enum RawFilter {
...
/// all point to `rtd::dynamic_field::Field<K, V>` objects. Returns the IDs and a cursor pointing
...
pub(crate) async fn dynamic_fields(
    ...
    by_type_indices(
        ...
        &Some(RtdObjectDataFilter::StructType(StructTag {
