...
pub(super) async fn dynamic_field_object(
...
) -> Result<RtdObjectResponse, RpcError<Error>> {
    let layout = resolve_type(ctx, &name.type_).await?;
    let bytes = RtdJsonValue::new(name.value)
    ...
    let options = RtdObjectDataOptions::full_content();
...
/// `rtd::dynamic_field::Field<K, V>`, and extract the name and value from it.
