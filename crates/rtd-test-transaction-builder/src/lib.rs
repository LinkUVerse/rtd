...
impl TestTransactionBuilder {
    ...
    pub fn call_staking(self, stake_coin: ObjectRef, validator: RtdAddress) -> Self {
        self.move_call(
    ...
    pub fn transfer(mut self, object: FullObjectRef, recipient: RtdAddress) -> Self {
        self.ptb_builder.transfer_object(recipient, object).unwrap();
        self
    }

    pub fn transfer_rtd(mut self, amount: Option<u64>, recipient: RtdAddress) -> Self {
        self.ptb_builder.transfer_rtd(recipient, amount);
        self
    }

    pub fn transfer_rtd_to_address_balance(
        self,
        source: FundSource,
    ...
    pub fn build_and_sign_multisig(
        ...
        let data = self.build();
        let intent_msg = IntentMessage::new(Intent::rtd_transaction(), data.clone());
...
pub async fn make_transfer_rtd_transaction(
    context: &WalletContext,
    ...
    context
        .sign_transaction(
            &TestTransactionBuilder::new(sender, gas_object, gas_price)
                .transfer_rtd(amount, recipient.unwrap_or(sender))
                .build(),
        )
        .await
}
...
pub async fn create_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (RtdAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
