...
use linku_metrics::add_server_timing;
...
use rtd_json_rpc_types::{
    ...
    ObjectChange, ProtocolConfigResponse, RtdEvent, RtdGetPastObjectRequest, RtdObjectDataOptions,
    RtdObjectResponse, RtdPastObjectResponse, RtdTransactionBlock, RtdTransactionBlockEvents,
...
struct IntermediateTransactionResponse {
    ...
    events: Option<RtdTransactionBlockEvents>,
...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        opts: Option<RtdTransactionBlockResponseOptions>,
    ) -> Result<Vec<RtdTransactionBlockResponse>, Error> {
        trace!("start");
        ...
        if temp_response.len() < num_digests {
            Err(RtdRpcInputError::ContainsDuplicates)?
        }
            ...
            for (_, cache_entry) in temp_response.iter_mut() {
                    ...
                    cache_entry.effects.as_ref().and_then(|e| e.events_digest())
                {
                    match events.next() {
                        Some(Some(ev)) => {
                            cache_entry.events =
                                Some(to_rtd_transaction_events(self, cache_entry.digest, ev)?)
                        }
                        None | Some(None) => {
                            error!(
                    ...
                    cache_entry.events = Some(RtdTransactionBlockEvents::default());
                }
            }
        }
        ...
        if opts.show_balance_changes || opts.show_object_changes {
            ...
            for resp in temp_response.values() {
                let effects = resp.effects.as_ref().ok_or_else(|| {
                    RtdRpcInputError::GenericNotFound(
...
impl ReadApiServer for ReadApi {
                ...
                ObjectRead::Exists(object_ref, o, layout) => {
                    ...
                    if options.show_display {
                            ...
                            .await
                        {
                            ...
                            Err(e) => {
                                return Ok(RtdObjectResponse::new(
                                    Some((object_ref, o, layout, options, None).try_into()?),
                                    Some(RtdObjectResponseError::DisplayError {
                                        error: e.to_string(),
                                    }),
                                ));
                            }
    ...
    async fn multi_get_objects(
        ...
        with_tracing!(async move {
            if object_ids.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                let objects_result: Result<Vec<RtdObjectResponse>, String> = results
                    .into_iter()
        ...
        with_tracing!(async move {
            ...
            match past_read {
                PastObjectRead::ObjectNotExists(id) => {
                    Ok(RtdPastObjectResponse::ObjectNotExists(id))
                }
                PastObjectRead::VersionFound(object_ref, o, layout) => {
                    ...
                    Ok(RtdPastObjectResponse::VersionFound(
                        (object_ref, o, layout, options, display_fields).try_into()?,
                    ))
                }
                PastObjectRead::ObjectDeleted(oref) => {
                    Ok(RtdPastObjectResponse::ObjectDeleted(oref.into()))
                }
                PastObjectRead::VersionNotFound(id, seq_num) => {
                    Ok(RtdPastObjectResponse::VersionNotFound(id, seq_num))
                }
                PastObjectRead::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                } => Ok(RtdPastObjectResponse::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                }),
    ...
    async fn try_get_object_before_version(
        ...
        self.try_get_past_object(
            object_id,
            version,
            Some(RtdObjectDataOptions::bcs_lossless()),
        )
        .await
    }
            ...
            if past_objects.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                if !errors.is_empty() {
                    let error_string = errors
                        ...
                        .collect::<Vec<String>>()
                        .join("; ");
                    Err(anyhow!("{error_string}").into()) // Collects errors not related to RtdPastObjectResponse variants
                } else {
                    Ok(success)
                }
            } else {
        ...
        with_tracing!(async move {
            ...
            if opts.show_events && temp_response.effects.is_some() {
                ...
                let events = transaction_kv_store
                    ...
                    .flatten();
                match events {
                    None => temp_response.events = Some(RtdTransactionBlockEvents::default()),
                    Some(events) => match to_rtd_transaction_events(self, digest, events) {
    ...
    async fn get_latest_checkpoint_sequence_number(&self) -> RpcResult<BigInt<u64>> {
        with_tracing!(async move {
            Ok(self
                ...
                .map_err(|e| {
                    RtdRpcInputError::GenericNotFound(format!(
    ...
    async fn verify_zklogin_signature(
        ...
        let GenericSignature::ZkLoginAuthenticator(zklogin_sig) =
            ...
            return Err(RtdRpcInputError::GenericNotFound(
            ...
            ZkLoginIntentScope::TransactionData => {
                let tx_data: TransactionData =
                    bcs::from_bytes(&Base64::decode(&bytes).map_err(Error::from)?)
                        .map_err(Error::from)?;
                let intent_msg = IntentMessage::new(Intent::rtd_transaction(), tx_data.clone());
...
async fn get_display_fields(
    ...
    Ok(DisplayFieldsResponse {
        ...
        error: (!errors.is_empty()).then(|| RtdObjectResponseError::DisplayError {
            error: errors.join("; "),
        }),
    })
}
...
fn convert_to_response(
    ...
    opts: &RtdTransactionBlockResponseOptions,
    module_cache: &impl GetModule,
) -> RpcInterimResult<RtdTransactionBlockResponse> {
    let mut response = RtdTransactionBlockResponse::new(cache.digest);
    ...
    if let Some(transaction) = cache.transaction {
        ...
        if opts.show_input {
            response.transaction = Some(RtdTransactionBlock::try_from(
