...
use crate::error::{Error, RtdRpcInputError};
...
use rtd_json_rpc_types::{
    MoveFunctionArgType, ObjectValueKind, RtdMoveNormalizedFunction, RtdMoveNormalizedModule,
...
impl MoveUtilsInternalTrait for MoveUtilsInternal {
    ...
    async fn get_move_module(
        ...
        Ok(match normalized.remove(&module_name) {
            ...
            None => Err(RtdRpcInputError::GenericNotFound(format!(
        ...
        match object_read {
            ObjectRead::Exists(_obj_ref, object, _layout) => {
                match object.into_inner().data {
                    ...
                    _ => Err(RtdRpcInputError::GenericInvalid(format!(
                        "Object is not a package with ID {}",
                        package
                    )))?,
                }
            }
            _ => Err(RtdRpcInputError::GenericNotFound(format!(
...
impl RtdRpcModule for MoveUtils {
...
impl MoveUtilsServer for MoveUtils {
    ...
    async fn get_normalized_move_module(
        ...
        package: ObjectID,
        module_name: String,
    ) -> RpcResult<RtdMoveNormalizedModule> {
        with_tracing!(async move {
    ...
    async fn get_normalized_move_struct(
        ...
        module_name: String,
        struct_name: String,
    ) -> RpcResult<RtdMoveNormalizedStruct> {
        with_tracing!(async move {
            let module = self.internal.get_move_module(package, module_name).await?;
            let structs = module.structs;
            let identifier = Identifier::new(struct_name.as_str())
                .map_err(|e| RtdRpcInputError::GenericInvalid(format!("{e}")))?;
...
mod tests {
    ...
    mod get_normalized_move_module_tests {
        ...
        async fn test_success_response() {
            ...
            let expected_module: RtdMoveNormalizedModule = normalized_module.into();
        ...
        async fn test_no_module_found() {
            ...
            let expected_error =
                Error::RtdRpcInputError(RtdRpcInputError::GenericNotFound(error_string.clone()));
            mock_internal
                .expect_get_move_module()
