...
pub use rtd_config::node::ServerType;
use rtd_open_rpc::{Module, Project};
...
pub fn rtd_rpc_doc(version: &str) -> Project {
    Project::new(
        version,
        "Rtd JSON-RPC",
...
impl JsonRpcServerBuilder {
    ...
    pub fn register_module<T: RtdRpcModule>(&mut self, module: T) -> Result<(), Error> {
    ...
    pub async fn start(
        ...
        info!(local_addr =? addr, "Rtd JSON-RPC server listening on {addr}");
        Ok(handle)
    }
}
