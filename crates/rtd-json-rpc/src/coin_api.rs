...
use rtd_types::coin::{CoinMetadata, TreasuryCap};
use rtd_types::coin_registry::{Currency, SupplyState};
...
use crate::error::{Error, RpcInterimResult, RtdRpcInputError};
use crate::{RtdRpcModule, with_tracing};

pub fn parse_to_struct_tag(coin_type: &str) -> Result<StructTag, RtdRpcInputError> {
    parse_rtd_struct_tag(coin_type)
        .map_err(|e| RtdRpcInputError::CannotParseRtdStructTag(format!("{e}")))
}

pub fn parse_to_type_tag(coin_type: Option<String>) -> Result<TypeTag, RtdRpcInputError> {
...
impl RtdRpcModule for CoinReadApi {
...
impl CoinReadApiServer for CoinReadApi {
            ...
            let cursor = match cursor {
                Some(c) => {
                    let decoded = CoinCursor::decode(&c).ok_or_else(|| {
                        RtdRpcInputError::GenericInvalid("invalid cursor".to_string())
                    })?;

                    if coin_type_tag.to_string() != decoded.coin_type {
                        return Err(
                            RtdRpcInputError::GenericInvalid("invalid cursor".to_string()).into(),
                        );
                    }
                    (
                        decoded.coin_type,
    ...
    #[instrument(skip(self))]
    async fn get_coin_metadata(&self, coin_type: String) -> RpcResult<Option<RtdCoinMetadata>> {
        with_tracing!(async move {
...
async fn find_package_object_id(
    ...
    async move {
        ...
        Err(RtdRpcInputError::GenericNotFound(format!(
...
mod tests {
    ...
    use rtd_types::{TypeTag, parse_rtd_struct_tag};
    ...
    mock! {
        ...
        impl TransactionKeyValueStoreTrait for KeyValueStore {
            ...
            async fn get_object(&self, object_id: ObjectID, version: SequenceNumber) -> RtdResult<Option<Object>>;
            async fn multi_get_objects(&self, object_keys: &[rtd_types::storage::ObjectKey]) -> RtdResult<Vec<Option<Object>>>;
            ...
            async fn multi_get_events_by_tx_digests(&self,digests: &[TransactionDigest]) -> RtdResult<Vec<Option<TransactionEvents>>>;
        }
    }
    ...
    fn get_test_coin_type_tag(coin_type: String) -> TypeTag {
        TypeTag::Struct(Box::new(parse_rtd_struct_tag(&coin_type).unwrap()))
    }
    ...
    fn get_test_treasury_cap_peripherals(
        ...
        let input_coin_struct = parse_rtd_struct_tag(&coin_name).expect("should not fail");
    ...
    fn create_currency_object(currency: Currency) -> Object {
        use rtd_types::object::{Data, ObjectInner};
    ...
    mock! {
        pub ObjectStore {}
        impl rtd_types::storage::ObjectStore for ObjectStore {
    ...
    mod get_coins_tests {
        ...
        async fn test_coin_no_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_rtd_struct_tag(&coin.coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_coin_with_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_rtd_struct_tag(&coins[0].coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
            ...
            mock_state
                ...
                .returning(move |_, _, _, _| {
                    Err(StateReadError::Client(
                        RtdErrorKind::IndexStoreNotAvailable.into(),
                    ))
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
        ...
        async fn test_get_coins_iterator_typed_store_error() {
            ...
            mock_state
                .expect_get_owned_coins()
                .returning(move |_, _, _, _| {
                    Err(RtdErrorKind::Storage("mock rocksdb error".to_string()).into())
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_coins(owner, Some(coin_type.to_string()), None, None)
    ...
    mod get_balance_tests {
        ...
        async fn test_get_balance_execution_error() {
            ...
            let mut mock_state = MockStateRead::new();
            mock_state.expect_get_balance().returning(move |_, _| {
                Err(RtdErrorKind::ExecutionError("mock db error".to_string()).into())
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_balance(owner, Some(coin_type.to_string()))
    ...
    mod get_all_balances_tests {
        ...
        async fn test_index_store_not_available() {
            ...
            mock_state.expect_get_all_balance().returning(move |_| {
                Err(StateReadError::Client(
                    RtdError(Box::new(RtdErrorKind::IndexStoreNotAvailable)).into(),
                ))
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api.get_all_balances(owner).await;
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_valid_coin_metadata_object() {
            ...
            let metadata = RtdCoinMetadata::try_from(coin_metadata_object.clone()).unwrap();
        ...
        #[tokio::test]
        async fn test_find_package_object_not_rtd_coin_metadata() {
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let input_coin_struct = parse_rtd_struct_tag(&coin_name).expect("should not fail");
            let coin_metadata_struct = CoinMetadata::type_(input_coin_struct.clone());
        ...
        async fn test_coin_metadata_with_registry_data() {
            use rtd_types::coin_registry::{self, Currency, MetadataCapState, SupplyState};
            use rtd_types::collection_types::VecMap;

            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let coin_struct = parse_rtd_struct_tag(&coin_name).expect("should not fail");
