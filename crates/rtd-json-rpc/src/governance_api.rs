...
use rtd_json_rpc_api::{GovernanceReadApiOpenRpc, GovernanceReadApiServer, JsonRpcMetrics};
use rtd_json_rpc_types::{DelegatedStake, Stake, StakeStatus};
use rtd_json_rpc_types::{RtdCommittee, ValidatorApy, ValidatorApys};
...
use rtd_types::error::{RtdError, RtdErrorKind, UserInputError};
...
use rtd_types::rtd_system_state::{RtdSystemState, get_validator_from_table};

use crate::authority_state::StateRead;
use crate::error::{Error, RpcInterimResult, RtdRpcInputError};
use crate::{ObjectProvider, RtdRpcModule, with_tracing};
...
impl GovernanceReadApi {
    ...
    async fn get_staked_rtd(&self, owner: RtdAddress) -> Result<Vec<StakedRtd>, Error> {
        let state = self.state.clone();
        let result = state.get_staked_rtd(owner).await?;

        self.metrics
            .get_stake_rtd_result_size
            .observe(result.len() as f64);
        self.metrics
            .get_stake_rtd_result_size_total
    ...
    async fn get_stakes_by_ids(
        ...
        let state = self.state.clone();
        let stakes_read: Vec<_> = staked_rtd_ids
            .iter()
        ...
        let mut stakes: Vec<(StakedRtd, bool)> = vec![];
        for stake in stakes_read.into_iter() {
            match stake {
                ObjectRead::Exists(_, o, _) => stakes.push((StakedRtd::try_from(&o)?, true)),
                ObjectRead::Deleted(oref) => {
                    match self
                        ...
                        .await?
                    {
                        Some(o) => stakes.push((StakedRtd::try_from(&o)?, false)),
                        None => Err(RtdRpcInputError::UserInputError(
                ...
                ObjectRead::NotExists(id) => Err(RtdRpcInputError::UserInputError(
                    UserInputError::ObjectNotFound {
                        object_id: id,
                        version: None,
                    },
    ...
    async fn get_stakes(&self, owner: RtdAddress) -> Result<Vec<DelegatedStake>, Error> {
        let timer = self.metrics.get_stake_rtd_latency.start_timer();
        let stakes = self.get_staked_rtd(owner).await?;
        if stakes.is_empty() {
            return Ok(vec![]);
        }
        drop(timer);

        let _timer = self.metrics.get_delegated_rtd_latency.start_timer();
    ...
    async fn get_delegated_stakes(
        ...
        let system_state_summary: RtdSystemStateSummary =
            system_state.clone().into_rtd_system_state_summary();
    ...
    fn get_system_state(&self) -> Result<RtdSystemState, Error> {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    ...
    async fn get_stakes_by_ids(
        ...
        with_tracing!(async move { self.get_stakes_by_ids(staked_rtd_ids).await })
    }
    ...
    #[instrument(skip(self))]
    async fn get_committee_info(&self, epoch: Option<BigInt<u64>>) -> RpcResult<RtdCommittee> {
    ...
    async fn get_validators_apy(&self) -> RpcResult<ValidatorApys> {
        ...
        let system_state_summary: RtdSystemStateSummary =
            self.get_latest_rtd_system_state().await?;
...
async fn exchange_rates(
    ...
    let system_state_summary: RtdSystemStateSummary = system_state.into_rtd_system_state_summary();
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
        let mut rates = state
            ...
            .map(|df| {
                ...
                Ok::<_, RtdError>((epoch, exchange_rate))
            })
            .collect::<Result<Vec<_>, _>>()?;
...
impl RtdRpcModule for GovernanceReadApi {
