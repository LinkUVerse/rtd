...
pub enum Error {
    ...
    #[error(transparent)]
    RtdObjectResponseError(#[from] RtdObjectResponseError),

    #[error(transparent)]
    RtdRpcInputError(#[from] RtdRpcInputError),
...
impl From<RtdErrorKind> for Error {
    fn from(e: RtdErrorKind) -> Self {
        match e {
            RtdErrorKind::UserInputError { error } => Self::UserInputError(error),
            RtdErrorKind::RtdObjectResponseError { error } => Self::RtdObjectResponseError(error),
            RtdErrorKind::UnsupportedFeatureError { error } => Self::UnsupportedFeature(error),
            RtdErrorKind::IndexStoreNotAvailable => Self::UnsupportedFeature(
                "Required indexes are not available on this node".to_string(),
            ),
            other => Self::RtdError(RtdError(Box::new(other))),
        }
    }
}
...
impl From<Error> for ErrorObjectOwned {
    ...
    fn from(e: Error) -> ErrorObjectOwned {
        match e {
            ...
            Error::RtdObjectResponseError(err) => match err {
                RtdObjectResponseError::NotExists { .. }
                | RtdObjectResponseError::DynamicFieldNotFound { .. }
                | RtdObjectResponseError::Deleted { .. }
                | RtdObjectResponseError::DisplayError { .. } => invalid_params(err),
                _ => failed(err),
            },
            ...
            Error::RtdRpcInputError(err) => invalid_params(err),
            Error::RtdError(rtd_error) => match rtd_error.as_inner() {
                RtdErrorKind::TransactionNotFound { .. }
                | RtdErrorKind::TransactionsNotFound { .. }
                | RtdErrorKind::TransactionEventsNotFound { .. } => invalid_params(rtd_error),
                _ => failed(rtd_error),
            },
...
pub enum RtdRpcInputError {
    ...
    #[error("{0}")]
    CannotParseRtdStructTag(String),
...
impl From<RtdRpcInputError> for RpcError {
    fn from(e: RtdRpcInputError) -> Self {
        RpcError::Call(invalid_params(e))
    }
}

impl From<RtdRpcInputError> for ErrorObjectOwned {
    fn from(e: RtdRpcInputError) -> Self {
        invalid_params(e)
    }
}
...
mod tests {
    ...
    use rtd_types::crypto::AuthorityPublicKey;
    ...
    mod match_quorum_driver_error_tests {
        ...
        fn test_objects_double_used() {
            use rtd_types::crypto::VerifyingKey;
        ...
        fn test_non_recoverable_transaction_error_with_transient_errors() {
            let quorum_driver_error = QuorumDriverError::NonRecoverableTransactionError {
                errors: vec![
                    ...
                    (
                        RtdErrorKind::RpcError("Hello".to_string(), "Testing".to_string()).into(),
                        0,
                        vec![],
                    ),
                ],
