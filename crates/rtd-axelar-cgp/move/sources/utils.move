...
module axelar::utils {
    ...
    /// Prefix for Rtd Messages.
    const PREFIX: vector<u8> = b"\x19Rtd Signed Message:\n";

    /// Normalize last byte of the signature. Have it 1 or 0.
    /// See https://tech.linkulabs.com/cryptography-in-rtd-cross-chain-signature-verification/
    ...
    public fun to_rtd_signed(bytes: vector<u8>): vector<u8> {
        let mut res = vector[];
