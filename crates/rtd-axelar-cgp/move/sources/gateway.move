...
///
/// - When call approvals is sent to Rtd, it targets an object and not a module;
/// - To support cross-chain messaging, a Channel object has to be created;
/// - Channel can be either owned or shared but not frozen;
/// - Module developer on the Rtd side will have to implement a system to support messaging;
...
module axelar::gateway {
    ...
    use axelar::utils::to_rtd_signed;
    ...
    /// For when approval chainId is not RTD.
    ...
    /// Emitted when a new message is sent from the RTD network.
    ...
    entry fun process_commands(
        ...
        let mut allow_operatorship_transfer = validate_proof(validators, to_rtd_signed(*&data), proof);
    ...
    fun test_execute() {
        use rtd::test_scenario::{Self as ts, ctx};
