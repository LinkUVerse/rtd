...
use rtd_sdk::error::Error as RtdRpcError;
use rtd_types::base_types::{ObjectID, ObjectRef, SequenceNumber, RtdAddress, VersionNumber};
use rtd_types::digests::{ObjectDigest, TransactionDigest};
use rtd_types::error::{RtdError, RtdErrorKind, RtdObjectResponseError, RtdResult, UserInputError};
use rtd_types::object::Object;
use rtd_types::transaction::{InputObjectKind, SenderSignedData, TransactionKind};
use thiserror::Error;
...
pub(crate) const EPOCH_CHANGE_STRUCT_TAG: &str =
    "0x3::rtd_system_state_inner::SystemEpochInfoEvent";
...
pub struct OnChainTransactionInfo {
    ...
    #[serde(default)]
    pub gas_owner: Option<RtdAddress>,
    pub gas_budget: u64,
    pub gas_price: u64,
...
pub enum ReplayEngineError {
    #[error("RtdError: {:#?}", err)]
    RtdError { err: RtdError },

    #[error("RtdRpcError: {:#?}", err)]
    RtdRpcError { err: String },

    #[error("RtdObjectResponseError: {:#?}", err)]
    RtdObjectResponseError { err: RtdObjectResponseError },
    ...
    #[error("RtdRpcRequestTimeout")]
    RtdRpcRequestTimeout,
    ...
    EffectsForked {
        ...
        diff: String,
        on_chain: Box<RtdTransactionBlockEffects>,
        local: Box<RtdTransactionBlockEffects>,
    },
    ...
    UnexpectedEventFormat { event: RtdEvent },
...
impl From<RtdObjectResponseError> for ReplayEngineError {
    fn from(err: RtdObjectResponseError) -> Self {
        match err {
            RtdObjectResponseError::NotExists { object_id } => {
            ...
            _ => ReplayEngineError::RtdObjectResponseError { err },
        }
    }
}

impl From<ReplayEngineError> for RtdError {
    fn from(err: ReplayEngineError) -> Self {
        RtdError::from(RtdErrorKind::from(err))
    }
}

impl From<ReplayEngineError> for RtdErrorKind {
    fn from(err: ReplayEngineError) -> Self {
        RtdErrorKind::Unknown(format!("{:#?}", err))
    }
}

impl From<RtdError> for ReplayEngineError {
    fn from(err: RtdError) -> Self {
        ReplayEngineError::RtdError { err }
    }
}

impl From<RtdErrorKind> for ReplayEngineError {
    fn from(err: RtdErrorKind) -> Self {
        RtdError::from(err).into()
    }
}

impl From<RtdRpcError> for ReplayEngineError {
    fn from(err: RtdRpcError) -> Self {
        match err {
            RtdRpcError::RpcError(JsonRpseeError::RequestTimeout) => {
                ReplayEngineError::RtdRpcRequestTimeout
            }
            _ => ReplayEngineError::RtdRpcError {
                err: format!("{:?}", err),
            },
        }
    }
...
pub enum ExecutionStoreEvent {
    BackingPackageGetPackageObject {
        package_id: ObjectID,
        result: RtdResult<Option<Object>>,
    },
    ...
    ResourceResolverGetResource {
        ...
        typ: StructTag,
        result: RtdResult<Option<Vec<u8>>>,
    },
    ...
    GetModuleGetModuleByModuleId {
        id: ModuleId,
        result: RtdResult<Option<CompiledModule>>,
    },
    ReceiveObject {
        owner: ObjectID,
        receive: ObjectID,
