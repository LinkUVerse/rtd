...
use rtd_json_rpc_types::RtdObjectData;
...
use rtd_types::transaction::{EndOfEpochTransactionKind, TransactionKind};
...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<RtdTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<RtdEvent>, ReplayEngineError>;
...
impl RemoteFetcher {
    pub fn new(rpc_client: RtdClient) -> Self {
        Self {
            rpc_client,
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn multi_get_versioned(
        ...
        let options = RtdObjectDataOptions::bcs_lossless();

        let objs: Vec<_> = to_fetch
            .iter()
            .map(|(object_id, version)| RtdGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
    ...
    async fn get_child_object(
        ...
        let response = self
            ...
            .await
            .map_err(|q| ReplayEngineError::RtdRpcError { err: q.to_string() })?;
    ...
    async fn get_transaction(
        ...
        let tx_fetch_opts = RtdTransactionBlockResponseOptions::full_content();
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<RtdEvent> = vec![];
...
fn convert_past_obj_response(resp: RtdPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        RtdPastObjectResponse::VersionFound(o) => obj_from_rtd_obj_data(&o),
        RtdPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
        RtdPastObjectResponse::ObjectNotExists(id) => Err(ReplayEngineError::ObjectNotExist { id }),
        RtdPastObjectResponse::VersionNotFound(id, version) => {
            Err(ReplayEngineError::ObjectVersionNotFound { id, version })
        }
        RtdPastObjectResponse::VersionTooHigh {
            object_id,
            asked_version,
            latest_version,
...
#[allow(clippy::result_large_err)]
fn obj_from_rtd_obj_response(o: &RtdObjectResponse) -> Result<Object, ReplayEngineError> {
    let o = o.object().map_err(ReplayEngineError::from)?.clone();
    obj_from_rtd_obj_data(&o)
}

#[allow(clippy::result_large_err)]
fn obj_from_rtd_obj_data(o: &RtdObjectData) -> Result<Object, ReplayEngineError> {
...
#[allow(clippy::result_large_err)]
pub fn extract_epoch_and_version(ev: RtdEvent) -> Result<(u64, u64), ReplayEngineError> {
