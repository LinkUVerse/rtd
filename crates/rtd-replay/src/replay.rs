...
use rtd_types::storage::{PackageObject, get_module};
use rtd_types::transaction::GasData;
use rtd_types::transaction::TransactionKind::ProgrammableTransaction;
use rtd_types::{
    DEEPBOOK_PACKAGE_ID,
...
pub struct ExecutionSandboxState {
    ...
    pub local_exec_effects: RtdTransactionBlockEffects,
...
impl ExecutionSandboxState {
    ...
    pub fn check_effects(&self) -> Result<(), ReplayEngineError> {
        let RtdTransactionBlockEffects::V1(mut local_effects) = self.local_exec_effects.clone();
        let RtdTransactionBlockEffects::V1(on_chain_effects) =
        ...
        let local_effects = RtdTransactionBlockEffects::V1(local_effects);
        let on_chain_effects = RtdTransactionBlockEffects::V1(on_chain_effects);
    ...
    pub fn diff_effects(
        on_chain_effects: &RtdTransactionBlockEffects,
        local_effects: &RtdTransactionBlockEffects,
    ) -> String {
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(ReplayEngineError::RtdRpcRequestTimeout) => {
                    warn!(
    ...
    pub fn download_latest_object(
        ...
        match resp {
            ...
            Err(err) => Err(ReplayEngineError::RtdRpcError {
                err: err.to_string(),
            }),
        }
    }
    ...
    pub async fn get_checkpoint_txs(
        ...
        self.fetcher
            ...
            .await
            .map_err(|e| ReplayEngineError::RtdRpcError { err: e.to_string() })
    }
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        let effects =
            RtdTransactionBlockEffects::try_from(effects).map_err(ReplayEngineError::from)?;
    ...
    pub async fn certificate_execute_with_sandbox_state(
        ...
        let (gas_status, input_objects) = rtd_transaction_checks::check_certificate_input(
            &executable,
            input_objects,
            &protocol_config,
            reference_gas_price,
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `rtd_core::authority::AuthorityState::try_execute_immediately`
    #[allow(clippy::result_large_err)]
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `rtd_adapter::execution_engine::execute_transaction_to_effects`
    #[allow(clippy::result_large_err)]
        ...
        match parse_effect_error_for_denied_coins(status) {
            Some(coin_type) => {
                ...
                if !config_id_and_version
                    .iter()
                    .any(|(id, _)| id == &RTD_DENY_LIST_OBJECT_ID)
                {
                    let deny_list_oid_version = self.download_latest_object(&RTD_DENY_LIST_OBJECT_ID)
                        .ok()
                        .flatten()
                        .expect("Unable to download the deny list object for a transaction that requires it")
                        .version();
                    config_id_and_version.push((RTD_DENY_LIST_OBJECT_ID, deny_list_oid_version));
                }
    ...
    async fn resolve_tx_components(
        ...
        let sender = match tx_info.clone().transaction.unwrap().data {
            rtd_json_rpc_types::RtdTransactionBlockData::V1(tx) => tx.sender,
        };
        let RtdTransactionBlockEffects::V1(effects) = tx_info.clone().effects.unwrap();
        ...
        let gas_data = match tx_info.clone().transaction.unwrap().data {
            rtd_json_rpc_types::RtdTransactionBlockData::V1(tx) => tx.gas_data,
        };
        ...
        Ok(OnChainTransactionInfo {
            ...
            effects: RtdTransactionBlockEffects::V1(effects),
            receiving_objs,
            config_objects,
    ...
    async fn resolve_tx_components_from_dump(
        ...
        let effects = RtdTransactionBlockEffects::try_from(effects).unwrap();
...
impl BackingPackageStore for LocalExec {
    ...
    fn get_package_object(&self, package_id: &ObjectID) -> RtdResult<Option<PackageObject>> {
        fn inner(self_: &LocalExec, package_id: &ObjectID) -> RtdResult<Option<Object>> {
            // If package not present fetch it from the network
            self_
                .get_or_download_object(package_id, true /* we expect a Move package*/)
                .map_err(|e| RtdErrorKind::Storage(e.to_string()).into())
        }
    ...
    fn read_child_object(
        ...
        fn inner(
            ...
            if child_object.version() > child_version_upper_bound {
                return Err(RtdErrorKind::Unknown(format!(
...
impl ModuleResolver for LocalExec {
    ...
    /// We do not download
    fn get_module(&self, module_id: &ModuleId) -> RtdResult<Option<Vec<u8>>> {
        fn inner(self_: &LocalExec, module_id: &ModuleId) -> RtdResult<Option<Vec<u8>>> {
...
impl GetModule for LocalExec {
    ...
    fn get_module_by_id(&self, id: &ModuleId) -> RtdResult<Option<Self::Item>> {
...
pub fn get_executor(
    ...
    let silent = true;
    rtd_execution::executor(&protocol_config, silent)
        .expect("Creating an executor should not fail here")
}

fn parse_effect_error_for_denied_coins(status: &RtdExecutionStatus) -> Option<String> {
    let RtdExecutionStatus::Failure { error } = status else {
        return None;
    };
