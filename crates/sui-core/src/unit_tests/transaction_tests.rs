...
use crate::{
    ...
    authority_server::{AuthorityServer, AuthorityServerHandle, ValidatorService, ValidatorServiceMetrics},
    checkpoints::CheckpointStore,
    consensus_adapter::{ConsensusAdapter, ConsensusAdapterMetrics, ConnectionMonitorStatusForTests},
...
async fn do_transaction_test_impl(
    _check_forged_cert: bool, // Deprecated: certificate validation is no longer tested here
    ...
    let consensus_adapter = Arc::new(ConsensusAdapter::new(
        ...
        authority_state.epoch_store_for_testing().protocol_config().clone(),
    ));
    let metrics = Arc::new(ValidatorServiceMetrics::new_for_tests());
    let validator_service = ValidatorService::new_for_tests(
...
// test_oversized_txn moved to submit_transaction_tests.rs to use the submit_transaction endpoint

// QD-specific tests removed: test_very_large_certificate, test_handle_certificate_errors,
// test_handle_soft_bundle_certificates, test_handle_soft_bundle_certificates_errors
#[test]
