...
//!
//! # TODO: Consider moving FieldCount to rtd-pg-db
//!
//! Currently, FieldCount lives in this framework crate but is Postgres-specific. Ideally it should
//! live in rtd-pg-db. However, this creates a circular dependency:
//! - rtd-indexer-alt-framework depends on rtd-pg-db (for IndexerCluster and other utilities)
//! - This blanket impl needs FieldCount to implement concurrent::Handler for postgres indexers
//! - Moving FieldCount to rtd-pg-db would require framework to depend on rtd-pg-db (circular!)
//!
