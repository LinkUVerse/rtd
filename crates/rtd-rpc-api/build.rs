...
fn main() {
    ...
    let rtd_proto_dir = crate_dir.join("proto");
    let out_dir = crate_dir.join("src/proto/generated");

    let rtd_rpc_proto_dir = find_dependency_proto_dir("rtd-rpc", "vendored/proto");

    println!("cargo:rerun-if-changed={}", rtd_proto_dir.display());
    println!("cargo:rerun-if-changed={}", rtd_rpc_proto_dir.display());

    fs::create_dir_all(&out_dir).expect("create proto out dir");

    // Find all .proto files using walkdir (only from rtd_proto_dir, not rtd_rpc_proto_dir)
    let proto_ext = OsStr::new("proto");
    let mut proto_files = vec![];
    for entry in WalkDir::new(&rtd_proto_dir) {
    ...
    let file_descriptors = protox::compile(proto_files, [rtd_proto_dir, rtd_rpc_proto_dir])
    ...
    tonic_prost_build::configure()
        ...
        .extern_path(".rtd.rpc.v2", "::rtd_rpc::proto::rtd::rpc::v2")
        .out_dir(&out_dir)
