...
use rtd_types::messages_checkpoint::{CertifiedCheckpointSummary, CheckpointSequenceNumber};
use rtd_types::object::Object;
use rtd_types::transaction::Transaction;

pub use rtd_rpc::client::HeadersInterceptor;
pub use rtd_rpc::client::ResponseExt;
...
#[derive(Clone)]
pub struct Client(rtd_rpc::Client);
...
impl Client {
    ...
    {
        rtd_rpc::Client::new(uri).map(Self)
    }
    ...
    pub async fn get_full_checkpoint(
        ...
        let checkpoint = response
            .checkpoint
            .ok_or_else(|| tonic::Status::not_found("no checkpoint returned"))?;
        rtd_types::full_checkpoint_content::Checkpoint::try_from(&checkpoint)
            .map(Into::into)
            .map_err(|e| status_from_error_with_metadata(e, metadata))
    }
...
pub struct TransactionExecutionResponse {
    ...
    pub balance_changes: Vec<rtd_sdk_types::BalanceChange>,
...
fn certified_checkpoint_summary_try_from_proto(
    ...
    let signature = rtd_types::crypto::AuthorityStrongQuorumSignInfo::from(
        rtd_sdk_types::ValidatorAggregatedSignature::try_from(
            checkpoint
                .signature
                .as_ref()
