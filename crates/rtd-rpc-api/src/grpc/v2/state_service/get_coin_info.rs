...
use rtd_rpc::proto::rtd::rpc::v2::CoinTreasury;
use rtd_rpc::proto::rtd::rpc::v2::GetCoinInfoRequest;
use rtd_rpc::proto::rtd::rpc::v2::GetCoinInfoResponse;
use rtd_rpc::proto::rtd::rpc::v2::coin_treasury::SupplyState as RpcSupplyState;
use rtd_sdk_types::{Address, StructTag};
use rtd_types::base_types::{ObjectID as RtdObjectID, RtdAddress};
use rtd_types::coin::RegulatedCoinMetadata;
use rtd_types::coin_registry::{Currency, RegulatedState as CurrencyRegulatedState, SupplyState};
use rtd_types::object::Owner::AddressOwner;
use rtd_types::object::Owner::ConsensusAddressOwner;
use rtd_types::object::Owner::Immutable;
use rtd_types::rtd_sdk_types_conversions::struct_tag_sdk_to_core;

const RTD_COIN_TREASURY: CoinTreasury = {
    let mut treasury = CoinTreasury::const_default();
    treasury.total_supply = Some(rtd_types::gas_coin::TOTAL_SUPPLY_MIST);
...
fn get_coin_info_from_registry(
    ...
    let treasury = if rtd_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(RTD_COIN_TREASURY)
    } else {
...
fn get_coin_info_from_index(
    service: &RpcService,
    indexes: &dyn rtd_types::storage::RpcIndexes,
    coin_type: &StructTag,
    core_coin_type: &move_core_types::language_storage::StructTag,
) -> Result<Option<GetCoinInfoResponse>> {
    ...
    let metadata = if let Some(coin_metadata_object_id) = coin_info.coin_metadata_object_id {
        service
            ...
            .map(rtd_types::coin::CoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RpcError::new(
                    tonic::Code::Internal,
    ...
    } else if rtd_types::gas_coin::GAS::is_gas(core_coin_type) {
        Some(RTD_COIN_TREASURY)
    } else {
        None
    };
...
fn get_treasury_cap_info(
    service: &RpcService,
    treasury_object_id: RtdObjectID,
) -> Option<CoinTreasury> {
    ...
    let supply_state = match &obj.owner {
        ...
        AddressOwner(addr) if *addr == RtdAddress::ZERO => RpcSupplyState::Fixed,
        ConsensusAddressOwner { owner, .. } if *owner == RtdAddress::ZERO => RpcSupplyState::Fixed,
        _ => RpcSupplyState::Unknown,
    };

    rtd_types::coin::TreasuryCap::try_from(obj)
        .ok()
        .map(|treasury| {
