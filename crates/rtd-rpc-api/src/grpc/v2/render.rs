...
impl RpcService {
    ...
    pub fn render_clever_error(&self, effects: &mut TransactionEffects) {
        use rtd_rpc::proto::rtd::rpc::v2::CleverError;
        use rtd_rpc::proto::rtd::rpc::v2::MoveAbort;
        use rtd_rpc::proto::rtd::rpc::v2::clever_error;
        use rtd_rpc::proto::rtd::rpc::v2::execution_error::ErrorDetails;
        ...
        fn render(service: &RpcService, move_abort: &MoveAbort) -> Option<CleverError> {
            ...
            let package_id = location.package().parse::<rtd_sdk_types::Address>().ok()?;
            let module = location.module();

            let package = {
                let object = service.reader.inner().get_object(&package_id.into())?;
                rtd_package_resolver::Package::read_from_object(&object).ok()?
            };
            ...
            match clever_error.error_info {
                rtd_package_resolver::ErrorConstants::None => {}
                rtd_package_resolver::ErrorConstants::Rendered {
                    identifier,
                    constant,
                } => {
                ...
                rtd_package_resolver::ErrorConstants::Raw { identifier, bytes } => {
                    clever_error_message.constant_name = Some(identifier);
        ...
        if mask.contains(TransactionEffects::CHANGED_OBJECTS_FIELD) {
            for changed_object in effects.changed_objects.iter_mut() {
                let Ok(object_id) = changed_object
                    .object_id()
                    .parse::<rtd_types::base_types::ObjectID>()
                else {
                    continue;
                };
