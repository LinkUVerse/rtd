...
use rtd_rpc::proto::rtd::rpc::v2::CommandOutput;
use rtd_rpc::proto::rtd::rpc::v2::CommandResult;
use rtd_rpc::proto::rtd::rpc::v2::ExecutedTransaction;
use rtd_rpc::proto::rtd::rpc::v2::ObjectSet;
use rtd_rpc::proto::rtd::rpc::v2::SimulateTransactionRequest;
use rtd_rpc::proto::rtd::rpc::v2::SimulateTransactionResponse;
...
pub fn simulate_transaction(
    ...
    let mut transaction = match rtd_sdk_types::Transaction::try_from(transaction_proto) {
        Ok(transaction) => rtd_types::transaction::TransactionData::try_from(transaction)?,
        ...
        if transaction.gas_data().payment.is_empty() {
            let input_objects = transaction
                ...
                .flat_map(|obj| match obj {
                    rtd_types::transaction::InputObjectKind::ImmOrOwnedMoveObject((id, _, _)) => {
                        Some(*id)
                    }
                    _ => None,
                })
    ...
    let transaction = if let Some(submask) = read_mask.subtree("transaction") {
        ...
        let transaction = rtd_sdk_types::Transaction::try_from(transaction)?;
...
fn to_command_output(
    service: &RpcService,
    arg: Option<rtd_types::transaction::Argument>,
    bcs: Vec<u8>,
    ty: rtd_types::TypeTag,
) -> CommandOutput {
    let json = service
        .reader
        .inner()
        ...
        .flatten()
        .and_then(|layout| {
            rtd_types::proto_value::ProtoVisitor::new(service.config.max_json_move_value_size())
...
fn estimate_gas_budget_from_gas_cost(
    gas_cost_summary: &rtd_types::gas::GasCostSummary,
