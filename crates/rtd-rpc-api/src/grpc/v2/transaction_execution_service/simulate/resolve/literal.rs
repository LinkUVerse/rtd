...
use rtd_types::base_types::STD_UTF8_MODULE_NAME;
use rtd_types::base_types::STD_UTF8_STRUCT_NAME;
        ...
        match (command, idx) {
            ...
            (Command::MakeMoveVector(make_move_vector), Some(_)) => {
                if let Some(ty) = &make_move_vector.type_ {
                    let ty =
                        rtd_types::rtd_sdk_types_conversions::type_tag_sdk_to_core(ty.clone())?;
