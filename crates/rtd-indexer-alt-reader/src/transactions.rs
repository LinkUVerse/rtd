...
use rtd_indexer_alt_schema::{schema::kv_transactions, transactions::StoredTransaction};
use rtd_kvstore::TransactionData;
use rtd_rpc::proto::rtd::rpc::v2 as proto;
use rtd_rpc::{field::FieldMaskUtil, proto::proto_to_timestamp_ms};
    ...
    async fn load(
        ...
        for tx_result in batch_response.transactions {
            if let Some(proto::get_transaction_result::Result::Transaction(executed)) =
                tx_result.result
            {
                let full_tx: rtd_types::full_checkpoint_content::ExecutedTransaction = (&executed)
                    .try_into()
