...
use rtd_config::genesis::{TokenAllocation, TokenDistributionScheduleBuilder};
use rtd_config::node::AuthorityOverloadConfig;
#[cfg(msim)]
use rtd_config::node::ExecutionTimeObserverConfig;
use rtd_protocol_config::Chain;
use rtd_types::base_types::{AuthorityName, RtdAddress};
use rtd_types::committee::{Committee, ProtocolVersion};
...
impl ConfigBuilder {
    ...
    pub fn new_with_temp_dir() -> Self {
        Self::new(linku_common::tempdir().unwrap().keep())
    }
}
    ...
    pub fn build(self) -> NetworkConfig {
        ...
        let token_distribution_schedule = {
            ...
            for validator in &validators {
                ...
                let address = RtdAddress::from(&account_key);
        ...
        let genesis = {
            let mut builder = rtd_genesis_builder::Builder::new()
...
mod test {
    ...
    fn genesis_transaction() {
        ...
        let genesis = network_config.genesis;
        let protocol_version = ProtocolVersion::new(genesis.rtd_system_object().protocol_version());
