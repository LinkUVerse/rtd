...
use rtd_config::node::{RunWithRange, TransactionDriverConfig, default_zklogin_oauth_providers};
use rtd_config::p2p::{P2pConfig, SeedPeer, StateSyncConfig};
...
use rtd_types::crypto::{AuthorityKeyPair, AuthorityPublicKeyBytes, NetworkKeyPair, RtdKeyPair};
...
impl ValidatorConfigBuilder {
    ...
    pub fn build(
        ...
        genesis: rtd_config::genesis::Genesis,
    ) -> NodeConfig {
        let key_path = get_key_path(&validator.key_pair);
        let config_directory = self
            .config_directory
            .unwrap_or_else(|| linku_common::tempdir().unwrap().keep());
        ...
        NodeConfig {
            ...
            network_key_pair: KeyPairWithPath::new(RtdKeyPair::Ed25519(validator.network_key_pair)),
            account_key_pair: KeyPairWithPath::new(validator.account_key_pair),
            worker_key_pair: KeyPairWithPath::new(RtdKeyPair::Ed25519(validator.worker_key_pair)),
            db_path,
            network_address,
            ...
            genesis: rtd_config::node::Genesis::new(genesis),
            grpc_load_shed: None,
...
impl FullnodeConfigBuilder {
    ...
    pub fn with_rpc_config(mut self, rpc_config: rtd_config::RpcConfig) -> Self {
    ...
    pub fn with_network_key_pair(mut self, network_key_pair: Option<NetworkKeyPair>) -> Self {
        if let Some(network_key_pair) = network_key_pair {
            self.network_key_pair =
                Some(KeyPairWithPath::new(RtdKeyPair::Ed25519(network_key_pair)));
        }
        self
    }
    ...
    pub fn build<R: rand::RngCore + rand::CryptoRng>(
        ...
        NodeConfig {
            ...
            worker_key_pair: KeyPairWithPath::new(RtdKeyPair::Ed25519(
                validator_config.worker_key_pair,
            )),
            network_key_pair: self.network_key_pair.unwrap_or(KeyPairWithPath::new(
                RtdKeyPair::Ed25519(validator_config.network_key_pair),
            )),
            db_path: self
                .db_path
            ...
            remove_deprecated_tables: false,
            enable_index_processing: default_enable_index_processing(),
            genesis: self.genesis.unwrap_or(rtd_config::node::Genesis::new(
