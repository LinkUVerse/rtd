...
// This file contains tests that detect changes in Rtd configs.
...
// 1. Run `cargo insta test --review` under `./rtd-config`.
...
use rtd_types::crypto::{
    AccountKeyPair, AuthorityKeyPair, NetworkKeyPair, RtdKeyPair, generate_proof_of_possession,
...
fn populated_genesis_snapshot_matches() {
    ...
    let genesis = Builder::new()
        ...
        .build();
    assert_yaml_snapshot!(genesis.rtd_system_wrapper_object());
