...
impl Handler for CoinBalanceBuckets {
    ...
    async fn prune<'a>(
        ...
        // information. The inline compute avoids HashAggregate operations and the enrtdng
        ...
        //
        // TODO: use rtd_sql_macro's query!
        let query = format!(
            "
...
pub(crate) fn get_coin_owner(object: &Object) -> Option<(StoredCoinOwnerKind, RtdAddress)> {
    match object.owner() {
...
mod tests {
    ...
    use rtd_indexer_alt_framework::{
        Indexer,
        types::{
            base_types::{MoveObjectType, ObjectID, SequenceNumber, RtdAddress, dbg_addr},
    ...
    fn test_get_coin_balance_bucket() {
        ...
        let zero_coin = Object::with_id_owner_gas_for_testing(id, RtdAddress::ZERO, 0);
        assert_eq!(get_coin_balance_bucket(&zero_coin).unwrap(), 0);

        // Test coin with balance 1 (10^0)
        let one_coin = Object::with_id_owner_gas_for_testing(id, RtdAddress::ZERO, 1);
        assert_eq!(get_coin_balance_bucket(&one_coin).unwrap(), 0);

        // Test coin with balance 100 (10^2)
        let hundred_coin = Object::with_id_owner_gas_for_testing(id, RtdAddress::ZERO, 100);
        assert_eq!(get_coin_balance_bucket(&hundred_coin).unwrap(), 2);

        // Test coin with balance 1000000 (10^6)
        let million_coin = Object::with_id_owner_gas_for_testing(id, RtdAddress::ZERO, 1000000);
        assert_eq!(get_coin_balance_bucket(&million_coin).unwrap(), 6);

        // The type of this object is a staked RTD, not a coin.
        let invalid_coin = unsafe {
            Object::new_move(
                MoveObject::new_from_execution(
                    MoveObjectType::staked_rtd(),
                    false,
                    SequenceNumber::new(),
    ...
    fn test_get_coin_owner() {
        let id = ObjectID::random();
        let addr1 = RtdAddress::random_for_testing_only();
    ...
    #[tokio::test]
    async fn test_process_coin_balance_buckets_new_rtd_coin() {
        let (indexer, _db) = Indexer::new_for_testing(&MIGRATIONS).await;
        let mut conn = indexer.store().connect().await.unwrap();
        let mut builder = TestCheckpointBuilder::new(0);
        builder = builder
            .start_transaction(0)
            .create_rtd_object(0, 0)
            .create_rtd_object(1, 100)
            .finish_transaction();
    ...
    async fn test_process_coin_balance_buckets_out_of_order_pruning() {
        ...
        builder = builder
            .start_transaction(0)
            .create_rtd_object(0, 100)
            .create_rtd_object(1, 1000)
            .create_rtd_object(2, 10000)
            .finish_transaction();
