...
use rtd_types::crypto::{
    ...
    AuthoritySignature, DefaultHash, RtdAuthoritySignature,
};
...
impl Builder {
    ...
    pub fn add_validator_signature(mut self, keypair: &AuthorityKeyPair) -> Self {
        ...
        let checkpoint_signature = {
            let intent_msg = IntentMessage::new(
                Intent::rtd_app(IntentScope::CheckpointSummary),
                checkpoint.clone(),
            );
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.rtd_address()),
                )
            };
    ...
    fn committee(objects: &[Object]) -> Committee {
        let rtd_system_object =
            get_rtd_system_state(&objects).expect("Rtd System State object must always exist");
        rtd_system_object
    ...
    fn validate_inputs(&self) -> anyhow::Result<(), anyhow::Error> {
        ...
        if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
            ...
            token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
                self.validators.values().map(|v| v.info.rtd_address()),
            );
        }
    ...
    fn validate_output(&self) {
        ...
        let system_state = match unsigned_genesis.rtd_system_object() {
            RtdSystemState::V1(inner) => inner,
            RtdSystemState::V2(_) => unreachable!(),
            #[cfg(msim)]
            _ => {
            ...
            .zip(system_state.validators.active_validators.iter())
        {
            ...
            assert!(
                address_to_pool_id
                    .insert(metadata.rtd_address, onchain_validator.staking_pool.id)
                    .is_none()
            );
            assert_eq!(validator.info.rtd_address(), metadata.rtd_address);
            assert_eq!(validator.info.protocol_key(), metadata.rtd_pubkey_bytes());
        ...
        let mut gas_objects: BTreeMap<ObjectID, (&Object, GasCoin)> = unsigned_genesis
            ...
            .collect();
        let mut staked_rtd_objects: BTreeMap<ObjectID, (&Object, StakedRtd)> = unsigned_genesis
            .objects()
            .iter()
            .filter_map(|o| StakedRtd::try_from(o).ok().map(|s| (o.id(), (o, s))))
            .collect();
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_rtd_object_id = staked_rtd_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                ...
                let staked_rtd_object = staked_rtd_objects.remove(&staked_rtd_object_id).unwrap();
                assert_eq!(
                    staked_rtd_object.0.owner,
                    Owner::AddressOwner(allocation.recipient_address)
                );
                assert_eq!(staked_rtd_object.1.principal(), allocation.amount_mist);
                assert_eq!(staked_rtd_object.1.pool_id(), staking_pool_id);
                assert_eq!(staked_rtd_object.1.activation_epoch(), 0);
            } else {
        ...
        if !self.parameters.allow_insertion_of_extra_objects {
            ...
            assert!(staked_rtd_objects.is_empty());
        }
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.rtd_address),
    );
...
// on `rtd_system` which is mocked in some tests, but would be in the loader
    ...
    let genesis_transaction = {
        let genesis_objects = objects
            ...
            .map(|mut object| {
                ...
                rtd_types::transaction::GenesisObject::RawObject {
                    data: object.data,
                    owner: object.owner,
                }
            })
        ...
        rtd_types::transaction::VerifiedTransaction::new_genesis_transaction(genesis_objects)
            .into_inner()
    };
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        // Step 1: Create the RtdSystemState UID
        let rtd_system_state_uid = builder.programmable_move_call(
            RTD_FRAMEWORK_ADDRESS.into(),
            ident_str!("object").to_owned(),
            ident_str!("rtd_system_state").to_owned(),
            vec![],
            vec![],
        );
        ...
        if protocol_config.enable_bridge() {
            ...
            // Hardcoding chain id to RtdCustom
            let bridge_chain_id = builder.pure(BridgeChainId::RtdCustom).unwrap();
        ...
        // Step 4: Mint the supply of RTD.
        let rtd_supply = builder.programmable_move_call(
            RTD_FRAMEWORK_ADDRESS.into(),
            ident_str!("rtd").to_owned(),
        ...
        // Step 5: Run genesis.
        // The first argument is the system state uid we got from step 1 and the second one is the RTD supply we
        // got from step 3.
        let mut arguments = vec![rtd_system_state_uid, rtd_supply];
    ...
    {
        let object = written.get_mut(&rtd_types::RTD_CLOCK_OBJECT_ID).unwrap();
        object
            .data
            .try_as_move_mut()
            .unwrap()
...
mod test {
    ...
    fn allocation_csv() {
        let schedule = TokenDistributionSchedule::new_for_validators_with_default_allocation([
            RtdAddress::random_for_testing_only(),
            RtdAddress::random_for_testing_only(),
        ]);
