...
use crate::RtdClient;
use crate::rtd_client_config::{RtdClientConfig, RtdEnv};
...
use rtd_json_rpc_types::{
    RtdObjectData, RtdObjectDataFilter, RtdObjectDataOptions, RtdObjectResponse,
    RtdObjectResponseQuery, RtdTransactionBlockResponse, RtdTransactionBlockResponseOptions,
};
...
pub struct WalletContext {
    pub config: PersistedConfig<RtdClientConfig>,
    request_timeout: Option<std::time::Duration>,
    client: Arc<RwLock<Option<RtdClient>>>,
...
impl WalletContext {
    pub fn new(config_path: &Path) -> Result<Self, anyhow::Error> {
        let config: RtdClientConfig = PersistedConfig::read(config_path).map_err(|err| {
            anyhow!(
    ...
    pub fn new_for_tests(
        ...
        let mut config = RtdClientConfig::new(keystore)
    ...
    pub async fn get_client(&self) -> Result<RtdClient, anyhow::Error> {
    ...
    /// network
    pub async fn cache_chain_id(&self, client: &RtdClient) -> Result<String, anyhow::Error> {
    ...
    async fn internal_load_or_cache_chain_id(
        ...
        let mut config_result = RtdClientConfig::load_with_lock(path)?;
    ...
    pub fn get_active_env(&self) -> Result<&RtdEnv, anyhow::Error> {
    ...
    // TODO: Ger rid of mut
    pub fn active_address(&mut self) -> Result<RtdAddress, anyhow::Error> {
    ...
    pub async fn get_object_ref(&self, object_id: ObjectID) -> Result<ObjectRef, anyhow::Error> {
        ...
        Ok(client
            .read_api()
            .get_object_with_options(object_id, RtdObjectDataOptions::new())
            .await?
            .into_object()?
            .object_ref())
    }
    ...
    pub async fn gas_objects(
        &self,
        address: RtdAddress,
    ) -> Result<Vec<(u64, RtdObjectData)>, anyhow::Error> {
        let client = self.get_client().await?;
            ...
            let response = client
                ...
                .get_owned_objects(
                    ...
                    Some(RtdObjectResponseQuery::new(
                        ...
                        Some(RtdObjectDataOptions::full_content()),
                    )),
                    cursor,
                    None,
                )
    ...
    pub async fn get_object_owner(&self, id: &ObjectID) -> Result<RtdAddress, anyhow::Error> {
        let client = self.get_client().await?;
        let object = client
            .read_api()
            .get_object_with_options(*id, RtdObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
        Ok(object
            .owner
    ...
    pub async fn try_get_object_owner(
        &self,
        id: &Option<ObjectID>,
    ) -> Result<Option<RtdAddress>, anyhow::Error> {
        if let Some(id) = id {
    ...
    pub async fn infer_sender(&mut self, gas: &[ObjectID]) -> Result<RtdAddress, anyhow::Error> {
        if gas.is_empty() {
    ...
    pub async fn gas_for_owner_budget(
    ...
    ) -> Result<(u64, RtdObjectData), anyhow::Error> {
        ...
        Err(anyhow!(
            "No non-argument gas objects found for this address with value >= budget {budget}. Run rtd client gas to check for gas objects."
        ))
    }
    ...
    pub async fn get_one_account(&self) -> anyhow::Result<(RtdAddress, Vec<ObjectRef>)> {
    ...
    pub async fn get_one_gas_object(&self) -> anyhow::Result<Option<(RtdAddress, ObjectRef)>> {
    ...
    pub async fn get_all_accounts_and_gas_objects(
        &self,
    ) -> anyhow::Result<Vec<(RtdAddress, Vec<ObjectRef>)>> {
        let mut result = vec![];
        for address in self.get_addresses() {
            let objects = self
                .gas_objects(address)
    ...
    /// Add an account
    pub async fn add_account(&mut self, alias: Option<String>, keypair: RtdKeyPair) {
    ...
    pub async fn sign_transaction(&self, data: &TransactionData) -> Transaction {
        let sig = self
            .config
            .keystore
            .sign_secure(&data.sender(), data, Intent::rtd_transaction())
            .await
            .unwrap();
