...
use rtd_json_rpc_types::{
    ...
    ProtocolConfigResponse, RtdCoinMetadata, RtdCommittee, RtdEvent, RtdGetPastObjectRequest,
    RtdMoveNormalizedModule, RtdObjectDataOptions, RtdObjectResponse, RtdObjectResponseQuery,
    RtdPastObjectResponse, RtdTransactionBlockEffects, RtdTransactionBlockResponse,
    RtdTransactionBlockResponseOptions, RtdTransactionBlockResponseQuery, TransactionBlocksPage,
    TransactionFilter,
};
...
impl ReadApi {
    ...
    ///     let rtd = RtdClientBuilder::default().build_localnet().await?;
    ...
    ///         "No object data for this RtdObjectResponse {:?}",
    ///         object
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let dynamic_fields = rtd
    ///         .read_api()
    ...
    ) -> RtdRpcResult<RtdObjectResponse> {
        Ok(self
            .api
            .http
    ...
    ) -> RtdRpcResult<RtdPastObjectResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [RtdPastObjectResponse] objects, or an error upon failure.
    ///
    ...
    /// use rtd_sdk::RtdClientBuilder;
    /// use rtd_types::base_types::{ObjectID, RtdAddress};
    /// use rtd_json_rpc_types::{RtdObjectDataOptions, RtdGetPastObjectRequest};
    /// use std::str::FromStr;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), anyhow::Error> {
    ...
    ///     let multi_past_object = rtd
    ///         .read_api()
    ///         .try_multi_get_parsed_past_object(
    ///             vec![RtdGetPastObjectRequest {
    ...
    pub async fn try_multi_get_parsed_past_object(
        &self,
        past_objects: Vec<RtdGetPastObjectRequest>,
        options: RtdObjectDataOptions,
    ) -> RtdRpcResult<Vec<RtdPastObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return a [RtdObjectResponse] based on the provided [ObjectID] and [RtdObjectDataOptions], or an error upon failure.
    ///
    /// The [RtdObjectResponse] contains two fields:
    /// 1) `data` for the object's data (see [RtdObjectData](rtd_json_rpc_types::RtdObjectData)),
    /// 2) `error` for the error (if any) (see [RtdObjectResponseError](rtd_types::error::RtdObjectResponseError)).
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let object = rtd.read_api().get_object_with_options(object_id,
    ///             RtdObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    /// Return a list of [RtdObjectResponse] from the given vector of [ObjectID]s and [RtdObjectDataOptions], or an error upon failure.
    ///
    ...
    ///     let object = rtd.read_api().multi_get_object_with_options(object_ids,
    ///             RtdObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    pub async fn multi_get_object_with_options(
        ...
        options: RtdObjectDataOptions,
    ) -> RtdRpcResult<Vec<RtdObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_move_object_bcs(&self, object_id: ObjectID) -> RtdRpcResult<Vec<u8>> {
        let resp = self
            .get_object_with_options(object_id, RtdObjectDataOptions::default().with_bcs())
            .await?
            .into_object()
            .map_err(|e| {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let rtd = RtdClientBuilder::default().build_localnet().await?;
    ///     let total_transaction_blocks = rtd
    ///         .read_api()
    ...
    /// }
    /// ```
    pub async fn get_total_transaction_blocks(&self) -> RtdRpcResult<u64> {
        Ok(*self.api.http.get_total_transaction_blocks().await?)
    }

    /// Return a transaction and its effects in a [RtdTransactionBlockResponse] based on its
    ...
    /// Return a list of [RtdTransactionBlockResponse] based on the given vector of [TransactionDigest], or an error upon failure.
    ///
    ...
    pub async fn multi_get_transactions_with_options(
        ...
        options: RtdTransactionBlockResponseOptions,
    ) -> RtdRpcResult<Vec<RtdTransactionBlockResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return the [RtdCommittee] information for the provided `epoch`, or an error upon failure.
    ///
    /// The [RtdCommittee] contains the validators list and their information (name and stakes).
    ///
    ...
    ) -> RtdRpcResult<TransactionBlocksPage> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_chain_identifier(&self) -> RtdRpcResult<String> {
        Ok(self.api.http.get_chain_identifier().await?)
    }

    /// Return a checkpoint, or an error upon failure.
    ///
    /// A Rtd checkpoint is a sequence of transaction sets that a quorum of validators
    /// agree upon as having been executed within the Rtd system.
    pub async fn get_checkpoint(&self, id: CheckpointId) -> RtdRpcResult<Checkpoint> {
    ...
    pub async fn get_latest_checkpoint_sequence_number(
        &self,
    ) -> RtdRpcResult<CheckpointSequenceNumber> {
        Ok(*self
            .api
            .http
    ...
    /// Return a stream of [RtdTransactionBlockResponse], or an error upon failure.
    pub fn get_transactions_stream(
        &self,
        query: RtdTransactionBlockResponseQuery,
        cursor: Option<TransactionDigest>,
        descending_order: bool,
    ) -> impl Stream<Item = RtdTransactionBlockResponse> + '_ {
        stream::unfold(
    ...
    ) -> RtdRpcResult<impl Stream<Item = RtdRpcResult<RtdTransactionBlockEffects>>> {
        let Some(c) = &self.api.ws else {
            return Err(Error::Subscription(
                "Subscription only supported by WebSocket client.".to_string(),
            ));
        };
        let subscription: Subscription<RtdTransactionBlockEffects> =
    ...
    pub async fn get_normalized_move_modules_by_package(
        &self,
        package: ObjectID,
    ) -> RtdRpcResult<BTreeMap<String, RtdMoveNormalizedModule>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_reference_gas_price(&self) -> RtdRpcResult<u64> {
    ...
    pub async fn dry_run_transaction_block(
        &self,
        tx: TransactionData,
    ) -> RtdRpcResult<DryRunTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    ) -> RtdRpcResult<DevInspectResults> {
        Ok(self
            .api
            .http
    ...
    ) -> RtdRpcResult<ProtocolConfigResponse> {
    ...
    /// Verify a zkLogin signature against bytes that is parsed using intent_scope, and the rtd address.
    pub async fn verify_zklogin_signature(
        ...
        address: RtdAddress,
    ) -> RtdRpcResult<ZkLoginVerifyResult> {
        Ok(self
            .api
            .http
...
/// Coin Read API provides the functionality needed to get information from the Rtd network regarding the coins owned by an address.
#[derive(Debug, Clone)]
...
impl CoinReadApi {
    ...
    /// or use `None` for the default `Coin<RTD>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///
    /// The coins can be filtered by `coin_type` (e.g., 0x168da5bf1f48dafc111b0a488fa454aca95e0b5e::usdc::USDC)
    /// or use `None` to use the default `Coin<RTD>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// it will use the default `Coin<RTD>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// }
    /// ```
    pub async fn get_all_balances(&self, owner: RtdAddress) -> RtdRpcResult<Vec<Balance>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let rtd = RtdClientBuilder::default().build_localnet().await?;
    ///     let coin_metadata = rtd
    ///         .coin_read_api()
    ///         .get_coin_metadata("0x2::rtd::RTD".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_coin_metadata(
        &self,
        coin_type: String,
    ) -> RtdRpcResult<Option<RtdCoinMetadata>> {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let rtd = RtdClientBuilder::default().build_localnet().await?;
    ///     let total_supply = rtd
    ///         .coin_read_api()
    ///         .get_total_supply("0x2::rtd::RTD".to_string())
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    pub async fn get_total_supply(&self, coin_type: String) -> RtdRpcResult<Supply> {
        Ok(self.api.http.get_total_supply(coin_type).await?)
    }
}

/// Event API provides the functionality to fetch, query, or subscribe to events on the Rtd network.
#[derive(Clone)]
pub struct EventApi {
    api: Arc<RpcClient>,
}
...
impl EventApi {
    ...
    pub async fn subscribe_event(
        &self,
        filter: EventFilter,
    ) -> RtdRpcResult<impl Stream<Item = RtdRpcResult<RtdEvent>>> {
        match &self.api.ws {
            Some(c) => {
                let subscription: Subscription<RtdEvent> = c.subscribe_event(filter).await?;
                Ok(subscription.map(|item| Ok(item?)))
            }
    ...
    pub async fn get_events(&self, digest: TransactionDigest) -> RtdRpcResult<Vec<RtdEvent>> {
...
impl GovernanceApi {
    ...
    pub async fn get_stakes(&self, owner: RtdAddress) -> RtdRpcResult<Vec<DelegatedStake>> {
        Ok(self.api.http.get_stakes(owner).await?)
    }

    /// Return the [RtdCommittee] information for the given `epoch`, or an error upon failure.
    ///
    ...
    /// Return the latest RTD system state object on-chain, or an error upon failure.
    ///
    ...
    /// and much more. See the [RtdSystemStateSummary] for all the available fields.
    pub async fn get_latest_rtd_system_state(&self) -> RtdRpcResult<RtdSystemStateSummary> {
        Ok(self.api.http.get_latest_rtd_system_state().await?)
    }
