...
use rtd_config::{
    Config, PersistedConfig, RTD_CLIENT_CONFIG, RTD_KEYSTORE_FILENAME, rtd_config_dir,
};
use rtd_json_rpc_types::{Coin, RtdObjectDataOptions};
use rtd_keys::keystore::{AccountKeystore, FileBasedKeystore, GenerateOptions};
use rtd_sdk::{
...
pub async fn setup_for_write() -> Result<(RtdClient, RtdAddress, RtdAddress), anyhow::Error> {
    let (client, active_address) = setup_for_read().await?;
    // make sure we have some RTD (5_000_000 MIST) on this address
...
/// Return a rtd client to interact with the APIs and an active address from the local wallet.
///
/// This function sets up a wallet in case there is no wallet locally,
/// and ensures that the active address of the wallet has RTD on it.
/// If there is no RTD owned by the active address, then it will request
/// RTD from the faucet.
pub async fn setup_for_read() -> Result<(RtdClient, RtdAddress), anyhow::Error> {
    let client = RtdClientBuilder::default().build_testnet().await?;
    println!("Rtd testnet version is: {}", client.api_version());
...
pub async fn split_coin_digest(
    ...
    let coin = match fetch_coin(rtd, sender).await? {
        None => {
            request_tokens_from_faucet(*sender, rtd).await?;
            fetch_coin(rtd, sender)
                .await?
                .expect("Supposed to get a coin with RTD, but didn't. Aborting")
        }
        Some(c) => c,
    };
    ...
    let gas_price = rtd.read_api().get_reference_gas_price().await?;

    // now we programmatically build the transaction through several commands
    let mut ptb = ProgrammableTransactionBuilder::new();
    // first, we want to split the coin, and we specify how much RTD (in MIST) we want
    // for the new coin
...
pub async fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    let wallet_conf = rtd_config_dir()?.join(RTD_CLIENT_CONFIG);
    let keystore_path = rtd_config_dir()?.join(RTD_KEYSTORE_FILENAME);

    // check if a wallet exists and if not, create a wallet and a rtd client config
    ...
    if !wallet_conf.exists() {
        let keystore = FileBasedKeystore::load_or_create(&keystore_path)?;
        let mut client_config = RtdClientConfig::new(keystore.into());

        client_config.add_env(RtdEnv::testnet());
        client_config.add_env(RtdEnv::devnet());
        client_config.add_env(RtdEnv::localnet());
    ...
    let mut client_config: RtdClientConfig = PersistedConfig::read(&wallet_conf)?;
...
async fn just_for_clippy() -> Result<(), anyhow::Error> {
    let (rtd, sender, _recipient) = setup_for_write().await?;
    let _digest = split_coin_digest(&rtd, &sender).await?;
    Ok(())
}
