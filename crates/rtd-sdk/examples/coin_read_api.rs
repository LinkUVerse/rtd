...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<RTD>` which is represented as
    // "0x2::rtd::RTD"
    let coin_type = Some("0x2::rtd::RTD".to_string());
    let coins = rtd
        .coin_read_api()
    ...
    let coins_stream = rtd.coin_read_api().get_coins_stream(active_address, None);
    ...
    // Select coins based on the provided coin type (RTD in this example). Use `None` for the default Rtd coin
    let select_coins = rtd
        .coin_read_api()
    ...
    // or if None is passed, it will use Coin<RTD> as the coin type
    let balance = rtd
        .coin_read_api()
    ...
    let total_balance = rtd.coin_read_api().get_all_balances(active_address).await?;
    println!(" *** Balance + Total Balance *** ");
    println!("Balance: {:?}", balance);
    println!("Total Balance: {:?}", total_balance);
    println!(" *** Balance + Total Balance ***\n ");

    // Return the coin metadata for the Coin<RTD>
