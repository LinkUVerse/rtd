...
use rtd_sdk::rpc_types::{
    RtdGetPastObjectRequest, RtdObjectDataOptions, RtdTransactionBlockResponseOptions,
};
use rtd_sdk::types::base_types::ObjectID;
...
async fn main() -> Result<(), anyhow::Error> {
    let (rtd, active_address, _) = setup_for_write().await?;
    ...
    let object_data = object
        .data
        .as_ref()
        .unwrap_or_else(|| panic!("No object data for this RtdObjectResponse {:?}", object));
    let object_id = object_data.object_id;
    let version = object_data.version;

    let rtd_data_options = RtdObjectDataOptions {
        show_type: true,
        show_owner: true,
    ...
    let past_object = rtd
        .read_api()
        .try_get_parsed_past_object(object_id, version, rtd_data_options.clone())
        .await?;
    println!(" *** Past Object *** ");
    println!("{:?}", past_object);
    println!(" *** Past Object ***\n");

    let rtd_get_past_object_request = past_object.clone().into_object()?;
    let multi_past_object = rtd
        .read_api()
        .try_multi_get_parsed_past_object(
            vec![RtdGetPastObjectRequest {
                object_id: rtd_get_past_object_request.object_id,
                version: rtd_get_past_object_request.version,
            }],
            rtd_data_options.clone(),
        )
        .await?;
    ...
    // Object with options
    let object_with_options = rtd
        .read_api()
        .get_object_with_options(rtd_get_past_object_request.object_id, rtd_data_options)
        .await?;
    ...
    println!("{:?}", rtd.read_api().get_chain_identifier().await?);
    println!(" *** Chain identifier ***\n ");

    println!(" *** Protocol Config *** ");
    println!("{:?}", rtd.read_api().get_protocol_config(None).await?);
    println!(" *** Protocol Config ***\n ");

    // we make a dummy transaction which returns a transaction digest
    let tx_digest = split_coin_digest(&rtd, &active_address).await?;
    ...
    let tx_blocks = rtd.read_api().get_total_transaction_blocks().await?;
