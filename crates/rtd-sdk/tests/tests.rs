...
use rtd_sdk::{
    rtd_client_config::RtdClientConfig,
...
async fn test_get_wallet_key() {
    ...
    let signature = wallet_context
        .sign_secure(&alias_identity, &transaction, Intent::rtd_transaction())
        .await
        .unwrap();

    let intent_message = IntentMessage::new(Intent::rtd_transaction(), transaction.clone());
...
async fn test_update_env_chain_id_new_chain_id() -> Result<(), anyhow::Error> {
    ...
    let config = RtdClientConfig::load(&config_path)?;
    ...
    let reloaded_config = RtdClientConfig::load(&config_path)?;
...
async fn test_update_env_chain_id_overwrite_existing() -> Result<(), anyhow::Error> {
    ...
    let mut config = RtdClientConfig::load(&config_path)?;
    ...
    let final_config = RtdClientConfig::load(&config_path)?;
    assert_eq!(
