...
module bridge::bridge_env {
    ...
    use rtd::address;
    use rtd::clock::Clock;
    use rtd::coin::{Self, Coin, CoinMetadata, TreasuryCap};
    use rtd::ecdsa_k1::{KeyPair, secp256k1_keypair_from_seed, secp256k1_sign};
    use rtd::event;
    use rtd::package::UpgradeCap;
    ...
    use rtd_system::rtd_system::{validator_voting_powers_for_testing, RtdSystemState};
    ...
    public fun create_env(chain_id: u8): BridgeEnv {
        ...
        let mut clock = rtd::clock::create_for_testing(ctx);
    ...
    public fun setup_validators(env: &mut BridgeEnv, validators_info: vector<ValidatorInfo>) {
        ...
        create_rtd_system_state_for_testing(validators, 0, 0, ctx);
    ...
    public fun register_committee(env: &mut BridgeEnv) {
        ...
        let mut system_state = test_scenario::take_shared<RtdSystemState>(
            scenario,
        );
    ...
    fun add_default_tokens(env: &mut BridgeEnv, sender: address) {
        ...
        let add_token_message = create_add_tokens_on_rtd_message(
            env.chain_id,
            bridge.get_seq_num_for(message_types::add_tokens_on_rtd()),
            false,
    ...
    const RTD_MESSAGE_PREFIX: vector<u8> = b"RTD_BRIDGE_MESSAGE";

    fun sign_message(env: &BridgeEnv, message: BridgeMessage): vector<vector<u8>> {
        let mut message_bytes = RTD_MESSAGE_PREFIX;
    ...
    public fun bridge_to_rtd<Token>(
        env: &mut BridgeEnv,
        source_chain: u8,
        source_address: vector<u8>,
        target_address: address,
...
module bridge::test_token {
    ...
    use std::type_name;
    use rtd::address;
    use rtd::coin::{CoinMetadata, TreasuryCap, create_currency};
    use rtd::hex;
    use rtd::package::{UpgradeCap, test_publish};
    use rtd::test_utils::create_one_time_witness;
