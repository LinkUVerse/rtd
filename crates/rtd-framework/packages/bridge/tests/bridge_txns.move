...
fun test_bridge_and_claim() {
    ...
    //
    // move from eth and transfer to rtd account
    let transfer_id1 = env.bridge_to_rtd<ETH>(
        source_chain,
        eth_address,
        rtd_address,
    ...
    let transfer_id = env.bridge_to_rtd<ETH>(
        source_chain,
        eth_address,
        rtd_address,
        amount,
    );
    let token = env.claim_token<ETH>(rtd_address, source_chain, transfer_id);
    env.send_token<ETH>(
        rtd_address,
        source_chain,
        eth_address,
