...
use rtd::address;
use rtd::clock::Clock;
use rtd::coin::{Coin, TreasuryCap, CoinMetadata};
use rtd::event;
use rtd::linked_table::{Self, LinkedTable};
use rtd::package::UpgradeCap;
use rtd::vec_map::{Self, VecMap};
use rtd::versioned::{Self, Versioned};
...
const EInvariantRtdInitializedTokenTransferShouldNotBeClaimed: u64 = 10;
...
// Record bridge message approvals in Rtd, called by the bridge client
...
public fun approve_token_transfer(
    ...
    // retrieve pending message if source chain is Rtd, the initial message
    // must exist on chain
...
public fun execute_system_message(
    ...
    if (message_type == message_types::emergency_op()) {
        ...
        let payload = message.extract_add_tokens_on_rtd();
        inner.execute_add_tokens_on_rtd(payload);
    } else {
...
fun execute_add_tokens_on_rtd(inner: &mut BridgeInner, payload: AddTokenOnRtd) {
...
#[test_only]
public fun test_execute_add_tokens_on_rtd(bridge: &mut Bridge, payload: AddTokenOnRtd) {
    let inner = load_inner_mut(bridge);
    inner.execute_add_tokens_on_rtd(payload);
}
