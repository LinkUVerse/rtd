...
#[test_only]
module rtd_system::delegation_tests;

use std::unit_test::assert_eq;
use rtd::table::Table;
use rtd_system::staking_pool::{Self, StakedRtd, PoolTokenExchangeRate};
use rtd_system::test_runner;
use rtd_system::validator_builder;
use rtd_system::validator_set;
...
#[test]
// Scenario:
// 1. Stake 60 RTD to VALIDATOR_ADDR_1
// 2. Split the stake into 20 and 40
// 3. Join the 20 and 40 back together
// 4. Check that the stake is 60 again
fun split_join_staked_rtd() {
    let validator = validator_builder::new().initial_stake(100).rtd_address(VALIDATOR_ADDR_1);
    ...
    runner.owned_tx!<StakedRtd>(|mut stake| {
        stake.split_to_sender(20 * MIST_PER_RTD, runner.ctx());
        runner.keep(stake);
    });

    runner.scenario_fn!(|scenario| {
        let ids = scenario.ids_for_sender<StakedRtd>();
        assert_eq!(ids.length(), 2);

        let mut stake_1 = scenario.take_from_sender_by_id<StakedRtd>(ids[0]);
        let stake_2 = scenario.take_from_sender_by_id<StakedRtd>(ids[1]);

        assert_eq!(stake_1.amount(), 20 * MIST_PER_RTD);
        assert_eq!(stake_2.amount(), 40 * MIST_PER_RTD);

        stake_1.join(stake_2);
        runner.keep(stake_1);
    });

    runner.owned_tx!<StakedRtd>(|stake| {
        assert_eq!(stake.amount(), 60 * MIST_PER_RTD);
        runner.keep(stake);
    });
...
#[test, expected_failure(abort_code = staking_pool::EIncompatibleStakedRtd)]
fun join_different_epochs() {
    ...
    runner.scenario_fn!(|scenario| {
        let staked_rtd_ids = scenario.ids_for_sender<StakedRtd>();
        let mut part1 = scenario.take_from_sender_by_id<StakedRtd>(staked_rtd_ids[0]);
        let part2 = scenario.take_from_sender_by_id<StakedRtd>(staked_rtd_ids[1]);
...
#[test, expected_failure(abort_code = staking_pool::EStakedRtdBelowThreshold)]
fun split_below_threshold() {
    let validator = validator_builder::new().initial_stake(100).rtd_address(VALIDATOR_ADDR_1);
    let mut runner = test_runner::new().validators(vector[validator]).build();

    // Stake 2 RTD to the validator.
    runner.set_sender(STAKER_ADDR_1).stake_with(VALIDATOR_ADDR_1, 2);

    runner.owned_tx!<StakedRtd>(|mut stake| {
        stake.split_to_sender(1 * MIST_PER_RTD + 1, runner.ctx());
    });
...
fun add_remove_stake_flow() {
    let mut runner = test_runner::new()
        .validators(vector[
            ...
            validator_builder::new().initial_stake(100).rtd_address(VALIDATOR_ADDR_2),
        ])
        .build();

    // Stake 60 RTD to the validator.
    runner.set_sender(STAKER_ADDR_1).stake_with(VALIDATOR_ADDR_1, 60);

    // Check that the stake is NOT yet added to the validator.
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 100 * MIST_PER_RTD);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 100 * MIST_PER_RTD);
    });
    ...
    runner.owned_tx!<StakedRtd>(|stake| {
        runner.system_tx!(|system, ctx| {
            assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 160 * MIST_PER_RTD);
            assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 100 * MIST_PER_RTD);
...
// Scenario:
// 1. Stake 100 RTD to VALIDATOR_ADDR_1
// 2. Advance epoch
...
fun remove_stake_post_active_flow(should_distribute_rewards: bool) {
    let mut runner = test_runner::new()
        ...
        .rtd_supply_amount(300)
        .storage_fund_amount(100)
        .build();

    // Stake 100 RTD to the validator.
    ...
    let reward_amt = if (should_distribute_rewards) 15 * MIST_PER_RTD else 0;
    let validator_reward_amt = if (should_distribute_rewards) 10 * MIST_PER_RTD else 0;

    runner.set_sender(STAKER_ADDR_1);
    runner.owned_tx!<StakedRtd>(|stake| {
        assert_eq!(stake.amount(), 100 * MIST_PER_RTD);
        runner.system_tx!(|system, ctx| {
            assert!(!system.validators().is_active_validator_by_rtd_address(VALIDATOR_ADDR_1));
            system.request_withdraw_stake(stake, ctx);
        });
    });
    ...
    runner.set_sender(STAKER_ADDR_1);
    assert_eq!(runner.rtd_balance(), 100 * MIST_PER_RTD + reward_amt);
    ...
    assert_eq!(runner.rtd_balance(), 100 * MIST_PER_RTD + reward_amt + validator_reward_amt);
...
fun earns_rewards_at_last_epoch() {
    ...
    let reward_amt = 15 * MIST_PER_RTD;
    let validator_reward_amt = 10 * MIST_PER_RTD;
...
// 2. Stake 100 RTD to the validator candidate
...
fun add_preactive_remove_preactive() {
    ...
    let validator = validator_builder::preset().rtd_address(NEW_VALIDATOR_ADDR).build(runner.ctx());
    ...
    assert_eq!(runner.rtd_balance(), 100 * MIST_PER_RTD);
...
fun add_preactive_remove_pending_failure() {
    ...
    // Stake 100 RTD to the validator candidate.
    runner.set_sender(STAKER_ADDR_1).stake_with(NEW_VALIDATOR_ADDR, 100);
...
fun add_preactive_remove_active() {
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 250 * MIST_PER_RTD);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 250 * MIST_PER_RTD);
        assert_eq!(
    ...
    // Stake 50 RTD to the validator candidate from staker 2.
    runner.set_sender(STAKER_ADDR_2).stake_with(NEW_VALIDATOR_ADDR, 50);

    // Stake 100 RTD to the validator candidate from staker 3.
    ...
    runner.system_tx!(|system, _| {
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_1), 250 * MIST_PER_RTD);
        assert_eq!(system.validator_stake_amount(VALIDATOR_ADDR_2), 250 * MIST_PER_RTD);
        assert_eq!(system.validator_stake_amount(NEW_VALIDATOR_ADDR), 250 * MIST_PER_RTD);
        assert_eq!(
            system.inner_mut_for_testing().get_storage_fund_total_balance(),
            100 * MIST_PER_RTD,
        );
    ...
    // staker 1 and 3 unstake from the validator and earns about 2/5 * (85 - 10) * 1/3 = 10 RTD each.
    ...
    // So they will both get slightly more than 110 RTD in total balance.
    runner.set_sender(STAKER_ADDR_1).unstake(0);
    assert_eq!(runner.rtd_balance(), 110002000000);
    ...
    // staker 2 earns about 5 RTD from the previous epoch and 24-ish from this one
    // so in total she has about 50 + 5 + 24 = 79 RTD.
    runner.set_sender(STAKER_ADDR_2).unstake(0);
    assert_eq!(runner.rtd_balance(), 78862939078);
...
fun add_preactive_remove_post_active() {
    ...
    // staker 1 earns a bit greater than 30 RTD here. A bit greater because the new validator's voting power
    ...
    runner.set_sender(STAKER_ADDR_1).unstake(0);
    assert_eq!(runner.rtd_balance(), 130006000000);
...
fun add_remove_stake_preactive_candidate() {
    ...
    // Stake 100 RTD to the validator candidate from each of the two stakers.
    runner.set_sender(STAKER_ADDR_1).stake_with(NEW_VALIDATOR_ADDR, 100);
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(validator.total_stake(), 200 * MIST_PER_RTD);
...
// Scenario:
// 1. Add a validator candidate
// 2. Delegate 100 RTD to the validator candidate
...
fun add_preactive_candidate_drop_out() {
    ...
    let validator_address = validator.rtd_address();
    runner.add_validator_candidate(validator);
...
fun remove_inactive_stake_from_inactive_candidate() {
    ...
    assert_eq!(runner.set_sender(validator_address).rtd_balance(), 100 * MIST_PER_RTD);
...
#[test]
/// Scenario:
/// 1. Stake 100 RTD to the validator.
/// 2. Advance epoch.
...
fun staking_pool_exchange_rate_getter() {
    ...
    // Each staking pool (2 pools) gets 10 RTD of rewards.
    ...
    runner.system_tx!(|system, _| {
        ...
        rates.assert_exchange_rate_eq(1, 200, 200); // 200 RTD of self + delegate stake at epoch 1
        rates.assert_exchange_rate_eq(2, 210, 200); // 10 RTD of rewards at epoch 2
    });
...
fun assert_exchange_rate_eq(
    ...
    assert_eq!(rate.rtd_amount(), rtd_amount * MIST_PER_RTD);
    assert_eq!(rate.pool_token_amount(), pool_token_amount * MIST_PER_RTD);
}
