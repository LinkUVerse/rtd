...
// This file contains tests testing functionalities in `rtd_system` that are not
// already tested by the other more themed tests such as `stake_tests` or
// `rewards_distribution_tests`.

#[test_only]
module rtd_system::rtd_system_tests;
...
fun report_validator() {
    let mut runner = test_runner::new()
        .validators(vector[
            validator_builder::new().rtd_address(@1),
            validator_builder::new().rtd_address(@2),
            validator_builder::new().rtd_address(@3),
        ])
        .build();
...
fun report_validator_by_stakee_ok(initial_stake: u8) {
    ...
    let new_validator = validator.rtd_address();
...
#[test, expected_failure(abort_code = ::rtd_system::validator_set::EInvalidCap)]
...
#[test, expected_failure(abort_code = rtd_system::validator::EGasPriceHigherThanThreshold)]
fun set_gas_price_failure() {
    let validator = validator_builder::new().rtd_address(@1);
...
#[test, expected_failure(abort_code = rtd_system::validator::ECommissionRateTooHigh)]
...
#[test, expected_failure(abort_code = rtd_system::rtd_system_state_inner::ENotValidator)]
...
#[test, expected_failure(abort_code = rtd_system::rtd_system_state_inner::EReportRecordNotFound)]
...
#[test, expected_failure(abort_code = rtd_system::rtd_system_state_inner::ECannotReportOneself)]
...
fun staking_pool_mappings() {
    let mut runner = test_runner::new()
        .validators(vector[
            ...
            validator_builder::new().rtd_address(@2),
            validator_builder::new().rtd_address(@3),
            validator_builder::new().rtd_address(@4),
        ])
        .build();
...
#[random_test]
// Stake random amount of RTD and check that the pending and withdraw amounts are correct.
fun withdraw_inactive_stake(stake: u16) {
    let stake_amount = stake as u64;
    let validator = validator_builder::new().rtd_address(@1).initial_stake(100);
    let mut runner = test_runner::new().validators(vector[validator]).build();
    ...
    // Check that pending stake amount is 1 RTD.
    runner.system_tx!(|system, _| {
        let pool = system.active_validator_by_address(@1).get_staking_pool_ref();
        assert_eq!(pool.pending_stake_amount(), stake_amount * MIST_PER_RTD);
        assert_eq!(pool.pending_stake_withdraw_amount(), 0);
        assert_eq!(pool.rtd_balance(), 100 * MIST_PER_RTD);
    });
...
#[random_test]
// Stake random amount of RTD and check that the pending stake amount is correct.
// Convert to fungible staked RTD and redeem it.
// Check that the stake amount is correct.
fun convert_to_fungible_staked_rtd_and_redeem(stake: u16) {
    let stake_amount = stake as u64;
    let validator = validator_builder::new().rtd_address(@1).initial_stake(100);
    let mut runner = test_runner::new().validators(vector[validator]).build();
    ...
    let staked_rtd = runner.set_sender(@5).stake_with_and_take(@1, stake_amount);

    assert_eq!(staked_rtd.amount(), stake_amount * MIST_PER_RTD);
    ...
    runner.system_tx!(|system, _| {
        ...
        assert_eq!(pool.pending_stake_amount(), 0);
        assert_eq!(pool.pending_stake_withdraw_amount(), 0);
        assert_eq!(pool.rtd_balance(), (100 + stake_amount) * MIST_PER_RTD);
    });

    // Convert to fungible staked RTD.
    let fungible_staked_rtd;
    runner.system_tx!(|system, ctx| {
        fungible_staked_rtd = system.convert_to_fungible_staked_rtd(staked_rtd, ctx);
    });

    assert_eq!(fungible_staked_rtd.value(), stake_amount * MIST_PER_RTD);

    let rtd;
    runner.system_tx!(|system, ctx| {
        rtd = system.redeem_fungible_staked_rtd(fungible_staked_rtd, ctx);
    });

    assert_eq!(rtd.destroy_for_testing(), stake_amount * MIST_PER_RTD);
