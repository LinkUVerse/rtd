...
/// provides a set of convenience methods for testing the Rtd System.
module rtd_system::test_runner;
...
use rtd_system::validator_builder::{Self, ValidatorBuilder};
...
public fun build(builder: TestRunnerBuilder): TestRunner {
    ...
    let stake_subsidy = stake_subsidy::create(
        balance::create_for_testing<RTD>(rtd_supply_amount.destroy_or!(1000) * MIST_PER_RTD), // rtd_supply
    ...
    let genesis_validator_addresses = validators.map_ref!(|v| v.rtd_address());

    // create rtd system state
    rtd_system::create(
        object::new(scenario.ctx()), // it doesn't matter what ID rtd system state has in tests
        validators,
        balance::create_for_testing<RTD>(storage_fund_amount.destroy_or!(0) * MIST_PER_RTD), // storage_fund
...
public fun rtd_supply_amount(
    mut builder: TestRunnerBuilder,
    rtd_supply_amount: u64,
): TestRunnerBuilder {
    builder.rtd_supply_amount = option::some(rtd_supply_amount);
    builder
}
...
/// Mint a RTD balance for testing.
public fun mint(amount: u64): Balance<RTD> {
    balance::create_for_testing(amount * MIST_PER_RTD)
}
...
public macro fun system_tx(
    ...
    $f: |&mut RtdSystemState, &mut TxContext|,
): &mut TestRunner {
    let sender = sender($runner);
    let scenario = scenario_mut($runner);
    scenario.next_tx(sender);
    ...
    runner.system_tx!(|system, ctx| {
        ...
        storage_rebate_balance =
            system.advance_epoch_for_testing(
                ...
                storage_charge.destroy_or!(0) * MIST_PER_RTD,
                computation_charge.destroy_or!(0) * MIST_PER_RTD,
...
public fun unstake(runner: &mut TestRunner, staked_rtd_idx: u64) {
    let sender = runner.sender;
    runner.set_sender(sender);
    let stake_rtd_ids = runner.scenario.ids_for_sender<StakedRtd>();
    let staked_rtd = runner.scenario.take_from_sender_by_id(stake_rtd_ids[staked_rtd_idx]);
    runner.system_tx!(|system, ctx| {
        system.request_withdraw_stake(staked_rtd, ctx);
    });
}
...
public fun add_validator_candidate(runner: &mut TestRunner, validator: Validator) {
    let sender = runner.sender;
    runner.scenario.next_tx(validator.rtd_address());
    runner.system_tx!(|system, ctx| {
...
/// Get the sum of the balances of all the RTD coins in the sender's account.
public fun rtd_balance(runner: &mut TestRunner): u64 {
    let sender = runner.sender;
    let scenario = runner.scenario_mut();
    scenario.next_tx(sender);
    scenario.ids_for_sender<Coin<RTD>>().fold!(0, |mut sum, coin_id| {
        let coin = scenario.take_from_sender_by_id<Coin<RTD>>(coin_id);
        sum = sum + coin.value();
...
/// Get the sum of the StakedRtd objects' principal and the rewards for these objects.
public fun staking_rewards_balance(runner: &mut TestRunner): u64 {
    let sender = runner.sender;
    let scenario = runner.scenario_mut();
    let mut system = scenario.take_shared<RtdSystemState>();

    scenario.next_tx(sender);
    let total_balance = scenario.ids_for_sender<StakedRtd>().fold!(0, |mut sum, staked_rtd_id| {
        let staked_rtd = scenario.take_from_sender_by_id<StakedRtd>(staked_rtd_id);
        let rewards = system.calculate_rewards(&staked_rtd, scenario.ctx());

        sum = sum + rewards + staked_rtd.amount();
