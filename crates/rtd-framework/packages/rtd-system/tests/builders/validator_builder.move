...
module rtd_system::validator_builder;

use rtd::bag;
use rtd::balance;
use rtd::rtd::RTD;
use rtd::url;
use rtd_system::validator::{Self, Validator, ValidatorMetadata};
...
public fun preset(): ValidatorBuilder {
    ValidatorBuilder {
        rtd_address: option::some(VALID_ADDRESS),
...
public fun build(builder: ValidatorBuilder, ctx: &mut TxContext): Validator {
    ...
    validator::new_for_testing(
        rtd_address.destroy_or!(ctx.fresh_object_address()),
        ...
        initial_stake.map!(|amount| balance::create_for_testing<RTD>(amount * 1_000_000_000)),
...
public fun build_metadata(builder: ValidatorBuilder, ctx: &mut TxContext): ValidatorMetadata {
    ...
    validator::new_metadata(
        rtd_address.destroy_or!(VALID_ADDRESS),
...
public fun rtd_address(mut builder: ValidatorBuilder, rtd_address: address): ValidatorBuilder {
    builder.rtd_address = option::some(rtd_address);
    builder
}
...
public fun valid_rtd_address(): address { VALID_ADDRESS }
