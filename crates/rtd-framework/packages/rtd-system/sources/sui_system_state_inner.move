...
module rtd_system::rtd_system_state_inner;
...
use rtd::vec_set::{Self, VecSet};
use rtd_system::stake_subsidy::StakeSubsidy;
use rtd_system::staking_pool::{StakedRtd, FungibleStakedRtd, PoolTokenExchangeRate};
use rtd_system::storage_fund::{Self, StorageFund};
use rtd_system::validator::{Self, Validator};
use rtd_system::validator_cap::{UnverifiedValidatorOperationCap, ValidatorOperationCap};
use rtd_system::validator_set::{Self, ValidatorSet};
...
public struct RtdSystemStateInner has store {
    ...
    safe_mode: bool,
    safe_mode_storage_rewards: Balance<RTD>,
    safe_mode_computation_rewards: Balance<RTD>,
...
public struct RtdSystemStateInnerV2 has store {
    /// The current epoch ID, starting from 0.
    epoch: u64,
    /// The current protocol version, starting from 1.
    protocol_version: u64,
...
public(package) fun create(
    ...
    let system_state = RtdSystemStateInner {
        epoch: 0,
        protocol_version,
...
public(package) fun v1_to_v2(self: RtdSystemStateInner): RtdSystemStateInnerV2 {
    let RtdSystemStateInner {
        epoch,
        protocol_version,
...
public(package) fun request_add_validator_candidate(
    self: &mut RtdSystemStateInnerV2,
...
public(package) fun request_add_validator(self: &mut RtdSystemStateInnerV2, ctx: &TxContext) {
    assert!(
...
/// of the validator.
public(package) fun request_remove_validator(self: &mut RtdSystemStateInnerV2, ctx: &TxContext) {
...
public(package) fun request_withdraw_stake(
    ...
    self.validators.request_withdraw_stake(staked_rtd, ctx)
}
...
public(package) fun convert_to_fungible_staked_rtd(
    ...
    self.validators.convert_to_fungible_staked_rtd(staked_rtd, ctx)
}
...
public(package) fun redeem_fungible_staked_rtd(
    ...
    self.validators.redeem_fungible_staked_rtd(fungible_staked_rtd, ctx)
}
...
public(package) fun report_validator(
    ...
    assert!(self.validators.is_active_validator_by_rtd_address(reportee_addr), ENotValidator);
...
public(package) fun rotate_operation_cap(self: &mut RtdSystemStateInnerV2, ctx: &mut TxContext) {
...
public(package) fun advance_epoch(
    ...
    mut storage_reward: Balance<RTD>,
    mut computation_reward: Balance<RTD>,
...
public(package) fun epoch(self: &RtdSystemStateInnerV2): u64 {
    self.epoch
}

public(package) fun protocol_version(self: &RtdSystemStateInnerV2): u64 {
    self.protocol_version
}

public(package) fun system_state_version(self: &RtdSystemStateInnerV2): u64 {
...
public(package) fun epoch_start_timestamp_ms(self: &RtdSystemStateInnerV2): u64 {
...
public(package) fun get_reporters_of(self: &RtdSystemStateInnerV2, addr: address): VecSet<address> {
    if (self.validator_report_records.contains(&addr)) self.validator_report_records[&addr]
    else vec_set::empty()
}

public(package) fun get_storage_fund_total_balance(self: &RtdSystemStateInnerV2): u64 {
    self.storage_fund.total_balance()
}

public(package) fun get_storage_fund_object_rebates(self: &RtdSystemStateInnerV2): u64 {
...
public(package) fun active_validator_addresses(self: &RtdSystemStateInnerV2): vector<address> {
...
public(package) fun validators(self: &RtdSystemStateInnerV2): &ValidatorSet {
    &self.validators
}

public(package) fun validators_mut(self: &mut RtdSystemStateInnerV2): &mut ValidatorSet {
    &mut self.validators
}
...
#[test_only]
public(package) fun get_stake_subsidy_distribution_counter(self: &RtdSystemStateInnerV2): u64 {
    self.stake_subsidy.get_distribution_counter()
}

#[test_only]
public(package) fun set_epoch_for_testing(self: &mut RtdSystemStateInnerV2, epoch_num: u64) {
...
#[test_only]
public(package) fun epoch_duration_ms(self: &RtdSystemStateInnerV2): u64 {
