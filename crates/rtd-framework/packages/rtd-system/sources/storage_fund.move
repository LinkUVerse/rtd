...
module rtd_system::storage_fund;
...
public struct StorageFund has store {
    total_object_storage_rebates: Balance<RTD>,
    non_refundable_balance: Balance<RTD>,
}

/// Called by `rtd_system` at genesis time.
public(package) fun new(initial_fund: Balance<RTD>): StorageFund {
    StorageFund {
...
/// Called by `rtd_system` at epoch change times to process the inflows and outflows of storage fund.
