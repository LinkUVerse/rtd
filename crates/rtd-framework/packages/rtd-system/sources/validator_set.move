...
module rtd_system::validator_set;

use rtd::bag::{Self, Bag};
use rtd::balance::Balance;
use rtd::event;
use rtd::priority_queue as pq;
use rtd::rtd::RTD;
use rtd::table::{Self, Table};
use rtd::table_vec::{Self, TableVec};
use rtd::vec_map::{Self, VecMap};
use rtd::vec_set::VecSet;
use rtd_system::staking_pool::{
    PoolTokenExchangeRate,
    StakedRtd,
    pool_id,
    FungibleStakedRtd,
    fungible_staked_rtd_pool_id
};
use rtd_system::validator::{Validator, staking_pool_id, rtd_address};
use rtd_system::validator_cap::{UnverifiedValidatorOperationCap, ValidatorOperationCap};
use rtd_system::validator_wrapper::ValidatorWrapper;
use rtd_system::voting_power;
...
public(package) fun new(
    ...
    init_active_validators.do_ref!(|v| {
        staking_pool_mappings.add(v.staking_pool_id(), v.rtd_address());
    });
...
/// Called by `rtd_system` to add a new validator candidate.
public(package) fun request_add_validator_candidate(
    ...
    self.validator_candidates.add(validator.rtd_address(), validator.wrap_v1(ctx));
}

/// Called by `rtd_system` to remove a validator candidate, and move them to `inactive_validators`.
...
/// Called by `rtd_system` to add a new validator to `pending_active_validators`, which will be
...
/// Called by `rtd_system`, to remove a validator.
...
/// Called by `rtd_system`, to add a new stake to the validator.
...
public(package) fun request_add_stake(
    ...
    assert!(rtd_amount >= MIN_STAKING_THRESHOLD, EStakingBelowThreshold);
    self
        .get_candidate_or_active_validator_mut(validator_address)
        .request_add_stake(stake, ctx.sender(), ctx)
}

/// Called by `rtd_system`, to withdraw some share of a stake from the validator. The share to withdraw
/// is denoted by `principal_withdraw_amount`. One of two things occurs in this function:
/// 1. If the `staked_rtd` is staked with an active validator, the request is added to the validator's
///    staking pool's pending stake withdraw entries, processed at the end of the epoch.
/// 2. If the `staked_rtd` was staked with a validator that is no longer active,
...
public(package) fun request_withdraw_stake(
    ...
    let staking_pool_id = staked_rtd.pool_id();
    let validator = if (self.staking_pool_mappings.contains(staking_pool_id)) {
        // This is an active validator.
        let validator_address = self.staking_pool_mappings[staked_rtd.pool_id()];
    ...
    validator.request_withdraw_stake(staked_rtd, ctx)
}
...
public(package) fun convert_to_fungible_staked_rtd(
    ...
    validator.convert_to_fungible_staked_rtd(staked_rtd, ctx)
}
...
public(package) fun redeem_fungible_staked_rtd(
    ...
    let staking_pool_id = fungible_staked_rtd.pool_id();
    ...
    validator.redeem_fungible_staked_rtd(fungible_staked_rtd, ctx)
}
...
public(package) fun advance_epoch(
    self: &mut ValidatorSet,
    computation_reward: &mut Balance<RTD>,
    storage_fund_reward: &mut Balance<RTD>,
...
fun update_validator_positions_and_calculate_total_stake(
    ...
    while (i > 0) {
        ...
        let validator_address = validator_ref.rtd_address();
    ...
    pending_active_validators.do!(|mut validator| {
        ...
        if (voting_power >= min_joining_voting_power_threshold) {
            ...
            event::emit(ValidatorJoinEvent {
                epoch: ctx.epoch(),
                validator_address: validator.rtd_address(),
...
/// Called by `rtd_system` to derive reference gas price for the new epoch.
...
public(package) fun is_active_validator_by_rtd_address(
    self: &ValidatorSet,
    validator_address: address,
): bool {
...
/// It differs from `is_active_validator_by_rtd_address` in that the former checks
...
fun find_validator_from_table_vec(
    ...
    while (i < length) {
        let v = &validators[i];
        if (v.rtd_address() == validator_address) {
...
fun compute_slashed_validators(
    ...
    while (!validator_report_records.is_empty()) {
        ...
        assert!(
            self.is_active_validator_by_rtd_address(validator_address),
...
fun distribute_reward(
    ...
    staking_rewards: &mut Balance<RTD>,
    storage_fund_reward: &mut Balance<RTD>,
    ctx: &mut TxContext,
) {
    let length = validators.length();
...
fun emit_validator_epoch_events(
    ...
    length.do!(|i| {
        let v = &vs[i];
        let validator_address = v.rtd_address();
...
public(package) fun is_active_validator(self: &ValidatorSet, addr: address): bool {
    self.active_validators.any!(|v| v.rtd_address() == addr)
}
