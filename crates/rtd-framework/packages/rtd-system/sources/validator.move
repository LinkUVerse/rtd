...
use rtd_system::validator_cap::{Self, ValidatorOperationCap};

public use fun rtd_system::validator_wrapper::create_v1 as Validator.wrap_v1;
...
public struct ConvertingToFungibleStakedRtdEvent has copy, drop {
    pool_id: ID,
    stake_activation_epoch: u64,
    staked_rtd_principal_amount: u64,
    fungible_staked_rtd_amount: u64,
}

/// Event emitted when a fungible staked RTD is redeemed.
public struct RedeemingFungibleStakedRtdEvent has copy, drop {
    pool_id: ID,
    fungible_staked_rtd_amount: u64,
    rtd_amount: u64,
}
...
public(package) fun request_add_stake(
    ...
    let staked_rtd = self.staking_pool.request_add_stake(stake, stake_epoch, ctx);
    ...
    event::emit(StakingRequestEvent {
        ...
        validator_address: self.metadata.rtd_address,
        staker_address,
        epoch: ctx.epoch(),
        amount: stake_amount,
    });
...
public(package) fun convert_to_fungible_staked_rtd(
    ...
    let stake_activation_epoch = staked_rtd.activation_epoch();
    let staked_rtd_principal_amount = staked_rtd.amount();
    let fungible_staked_rtd = self.staking_pool.convert_to_fungible_staked_rtd(staked_rtd, ctx);

    event::emit(ConvertingToFungibleStakedRtdEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun redeem_fungible_staked_rtd(
    ...
    let fungible_staked_rtd_amount = fungible_staked_rtd.value();
    let rtd = self.staking_pool.redeem_fungible_staked_rtd(fungible_staked_rtd, ctx);

    self.next_epoch_stake = self.next_epoch_stake - rtd.value();

    event::emit(RedeemingFungibleStakedRtdEvent {
        pool_id: self.staking_pool_id(),
...
public(package) fun request_add_stake_at_genesis(
    ...
    // 0 = genesis epoch
    let staked_rtd = self.staking_pool.request_add_stake(stake, 0, ctx);
...
public(package) fun request_withdraw_stake(
    ...
    let principal_amount = staked_rtd.amount();
    let stake_activation_epoch = staked_rtd.activation_epoch();
    let withdrawn_stake = self.staking_pool.request_withdraw_stake(staked_rtd, ctx);
...
public(package) fun request_set_gas_price(
    ...
    assert!(validator_address == self.metadata.rtd_address, EInvalidCap);
...
public(package) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<RTD>) {
...
public fun rtd_address(self: &Validator): address {
    self.metadata.rtd_address
}
...
public fun is_duplicate(self: &Validator, other: &Validator): bool {
    ...
    self.rtd_address == other.rtd_address
...
public(package) fun new_unverified_validator_operation_cap_and_transfer(
    ...
    assert!(sender == self.metadata.rtd_address, ENewCapNotCreatedByValidatorItself);
...
fun new_from_metadata(
    ...
    let rtd_address = metadata.rtd_address;
...
// Note: `proof_of_possession` MUST be a valid signature using rtd_address and
...
public(package) fun new_for_testing(
    ...
    primary_address: vector<u8>,
    worker_address: vector<u8>,
    initial_stake_option: Option<Balance<RTD>>,
    gas_price: u64,
    commission_rate: u64,
    ...
    initial_stake_option.do!(|balance| {
        request_add_stake_at_genesis(
            &mut validator,
            balance,
            rtd_address, // give the stake to the validator
            ctx,
        );
    });
