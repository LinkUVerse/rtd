...
use rtd::event;
use rtd::package::{Self, Publisher};
use rtd::rtd::RTD;
...
public struct TransferPolicy<phantom T> has key, store {
    id: UID,
    /// The Balance of the `TransferPolicy` which collects `RTD`.
...
entry fun default<T>(pub: &Publisher, ctx: &mut TxContext) {
    ...
    rtd::transfer::share_object(policy);
    rtd::transfer::transfer(cap, ctx.sender());
}
...
/// Add some `RTD` to the balance of a `TransferPolicy`.
public fun add_to_balance<T, Rule: drop>(_: Rule, policy: &mut TransferPolicy<T>, coin: Coin<RTD>) {
