...
/// `list_with_purchase_cap` therefore performing a trade (isrtdng a
...
entry fun default(ctx: &mut TxContext) {
    ...
    rtd::transfer::share_object(kiosk);
}
...
public fun close_and_withdraw(self: Kiosk, cap: KioskOwnerCap, ctx: &mut TxContext): Coin<RTD> {
...
public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<RTD> {
    assert!(self.has_access(cap), ENotOwner);
    &mut self.profits
}
