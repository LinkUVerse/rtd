...
use rtd::balance::{Supply, Balance};
use rtd::coin::{Self, TreasuryCap, DenyCapV2, CoinMetadata, RegulatedCoinMetadata, Coin};
...
public fun new_currency_with_otw<T: drop>(
    ...
    assert!(rtd::types::is_one_time_witness(&otw), ENotOneTimeWitness);
    assert!(is_ascii_printable!(&symbol), EInvalidSymbol);
...
public fun finalize<T>(builder: CurrencyInitializer<T>, ctx: &mut TxContext): MetadataCap<T> {
    ...
    if (is_otw) transfer::transfer(currency, object::rtd_coin_registry_address())
...
fun create(ctx: &TxContext) {
    ...
    transfer::share_object(CoinRegistry {
        id: object::rtd_coin_registry_object_id(),
    });
}
