...
/// list.
module rtd::deny_list;

use rtd::bag::{Self, Bag};
use rtd::config::{Self, Config};
...
fun add_per_type_config(
    ...
    rtd::event::emit(PerTypeConfigCreated { key, config_id });
}
...
public struct PerTypeList has key, store {
    ...
    /// For example with `rtd::coin::Coin`: If addresses A and B are banned from using
...
fun create(ctx: &mut TxContext) {
    ...
    let deny_list_object = DenyList {
        id: object::rtd_deny_list_object_id(),
        lists,
    };
