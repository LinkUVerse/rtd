...
module rtd::accumulator_settlement;

use rtd::accumulator::{AccumulatorRoot, accumulator_key, U128, create_u128, destroy_u128};
use rtd::bcs;
use rtd::hash;
...
use fun rtd::accumulator_metadata::remove_accumulator_metadata as AccumulatorRoot.remove_metadata;
use fun rtd::accumulator_metadata::create_accumulator_metadata as AccumulatorRoot.create_metadata;
...
fun settlement_prologue(
    ...
    _idx: u64,
    // Total input rtd received from user transactions
    input_rtd: u64,
    // Total output rtd withdrawn by user transactions
    output_rtd: u64,
    ctx: &TxContext,
) {
    assert!(ctx.sender() == @0x0, ENotSystemAddress);
    record_settlement_rtd_conservation(input_rtd, output_rtd);
}
...
native fun record_settlement_rtd_conservation(input_rtd: u64, output_rtd: u64);
