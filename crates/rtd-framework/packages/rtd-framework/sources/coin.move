...
use std::string;
use std::type_name;
use rtd::balance::{Self, Balance, Supply};
use rtd::deny_list::DenyList;
use rtd::url::{Self, Url};

// Allows calling `.split_vec(amounts, ctx)` on `coin`
public use fun rtd::pay::split_vec as Coin.split_vec;

// Allows calling `.join_vec(coins)` on `coin`
public use fun rtd::pay::join_vec as Coin.join_vec;

// Allows calling `.split_and_transfer(amount, recipient, ctx)` on `coin`
public use fun rtd::pay::split_and_transfer as Coin.split_and_transfer;

// Allows calling `.divide_and_keep(n, ctx)` on `coin`
public use fun rtd::pay::divide_and_keep as Coin.divide_and_keep;
...
public fun redeem_funds<T>(
    withdrawal: rtd::funds_accumulator::Withdrawal<Balance<T>>,
    ctx: &mut TxContext,
): Coin<T> {
...
public fun create_currency<T: drop>(
    ...
    assert!(rtd::types::is_one_time_witness(&witness), EBadWitness);
...
/// The index into the deny list vector for the `rtd::coin::Coin` type.
