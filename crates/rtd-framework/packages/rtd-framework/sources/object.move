...
const RTD_SYSTEM_STATE_OBJECT_ID: address = @0x5;

/// The hardcoded ID for the singleton Clock Object.
const RTD_CLOCK_OBJECT_ID: address = @0x6;

/// The hardcoded ID for the singleton AuthenticatorState Object.
const RTD_AUTHENTICATOR_STATE_ID: address = @0x7;

/// The hardcoded ID for the singleton Random Object.
const RTD_RANDOM_ID: address = @0x8;

/// The hardcoded ID for the singleton DenyList.
const RTD_DENY_LIST_OBJECT_ID: address = @0x403;

/// The hardcoded ID for the singleton AccumulatorRoot Object.
const RTD_ACCUMULATOR_ROOT_OBJECT_ID: address = @0xacc;

/// The hardcoded ID for the Bridge Object.
const RTD_BRIDGE_ID: address = @0x9;

/// The hardcoded ID for the Coin Registry Object.
const RTD_COIN_REGISTRY_OBJECT_ID: address = @0xc;

/// The hardcoded ID for the AddressAliasState Object.
const RTD_ADDRESS_ALIAS_STATE_ID: address = @0xa;
...
/// Create the `UID` for the singleton `RtdSystemState` object.
/// This should only be called once from `rtd_system`.
fun rtd_system_state(ctx: &TxContext): UID {
    assert!(ctx.sender() == @0x0, ENotSystemAddress);
    UID {
        id: ID { bytes: RTD_SYSTEM_STATE_OBJECT_ID },
    }
}
...
public(package) fun clock(): UID {
    UID {
        id: ID { bytes: RTD_CLOCK_OBJECT_ID },
    }
}
...
public(package) fun authenticator_state(): UID {
    UID {
        id: ID { bytes: RTD_AUTHENTICATOR_STATE_ID },
    }
}
...
public(package) fun randomness_state(): UID {
    UID {
        id: ID { bytes: RTD_RANDOM_ID },
    }
}

/// Create the `UID` for the singleton `DenyList` object.
/// This should only be called once from `deny_list`.
public(package) fun rtd_deny_list_object_id(): UID {
    UID {
        id: ID { bytes: RTD_DENY_LIST_OBJECT_ID },
    }
}

public(package) fun rtd_accumulator_root_object_id(): UID {
    UID {
        id: ID { bytes: RTD_ACCUMULATOR_ROOT_OBJECT_ID },
    }
}

public(package) fun rtd_accumulator_root_address(): address {
...
public(package) fun rtd_coin_registry_object_id(): UID {
    UID {
        id: ID { bytes: RTD_COIN_REGISTRY_OBJECT_ID },
    }
}

public(package) fun rtd_coin_registry_address(): address {
...
public(package) fun address_alias_state(): UID {
    UID {
        id: ID { bytes: RTD_ADDRESS_ALIAS_STATE_ID },
    }
}
...
/// Create a new object. Returns the `UID` that must be stored in a Rtd object.
