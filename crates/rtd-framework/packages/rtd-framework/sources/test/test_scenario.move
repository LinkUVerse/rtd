...
/// Utility for mocking a multi-transaction Rtd execution in a single Move procedure.
...
public fun create_system_objects(scenario: &mut Scenario) {
    ...
    rtd::clock::create_for_testing(scenario.ctx()).share_for_testing();
    rtd::random::create_for_testing(scenario.ctx());
    rtd::deny_list::create_for_testing(scenario.ctx());
...
public fun return_to_address<T: key>(account: address, t: T) {
    ...
    rtd::transfer::transfer_impl(t, account)
}
...
public fun return_immutable<T: key>(t: T) {
    ...
    rtd::transfer::freeze_object_impl(t)
}
...
public fun return_shared<T: key>(t: T) {
    ...
    rtd::transfer::share_object_impl(t)
}
...
public fun most_recent_receiving_ticket<T: key>(owner: &ID): rtd::transfer::Receiving<T> {
...
public fun receiving_ticket_by_id<T: key>(object_id: ID): rtd::transfer::Receiving<T> {
    let version = allocate_receiving_ticket_for_object<T>(object_id);
    rtd::transfer::make_receiver(object_id, version)
}
...
/// test scenario.
public fun return_receiving_ticket<T: key>(ticket: rtd::transfer::Receiving<T>) {
    let id = rtd::transfer::receiving_id(&ticket);
