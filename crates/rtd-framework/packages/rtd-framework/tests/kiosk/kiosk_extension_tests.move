...
#[test_only]
module rtd::kiosk_marketplace_ext {
    use rtd::coin::Coin;
    use rtd::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use rtd::kiosk_extension as ext;
    use rtd::rtd::RTD;
    use rtd::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of RTD.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in RTD) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<RTD>) {
    ...
    public fun accept_bid<Market, T: key + store>(
        ...
        let bid: Coin<RTD> = ext::storage_mut(Ext<Market> {}, destination).remove(Bid<T> {});
...
#[test_only]
module rtd::kiosk_extensions_tests {
    use rtd::kiosk_extension as ext;
    use rtd::kiosk_test_utils as test;
    ...
    #[test, expected_failure(abort_code = rtd::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = rtd::kiosk_extension::EExtensionNotInstalled)]
