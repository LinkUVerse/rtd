...
async fn build_system_packages() {
    ...
    let rtd_system_path = packages_path.join("rtd-system");
    let rtd_framework_path = packages_path.join("rtd-framework");
...
fn check_diff(checked_in: &Path, built: &Path) {
    for path in [COMPILED_PACKAGES_DIR, DOCS_DIR, PUBLISHED_API_FILE] {
        ...
        if !output.status.success() {
            let header = "Generated and checked-in rtd-framework packages and/or docs do not match.\n\
...
async fn build_packages(
    bridge_path: &Path,
    deepbook_path: &Path,
    rtd_system_path: &Path,
    rtd_framework_path: &Path,
    stdlib_path: &Path,
    out_dir: &Path,
) {
    let config = MoveBuildConfig {
        ...
        default_flavor: Some(Flavor::Rtd),
        ..Default::default()
    };
...
async fn build_packages_with_move_config(
    ...
    let rtd_system = system_pkg.get_rtd_system_modules();
    let rtd_framework = framework_pkg.get_rtd_framework_modules();
    ...
    let rtd_system_members =
        serialize_modules_to_file(rtd_system, &compiled_packages_dir.join(system_dir)).unwrap();
    let rtd_framework_members =
        serialize_modules_to_file(rtd_framework, &compiled_packages_dir.join(framework_dir))
            .unwrap();
    ...
    let published_api = [
        rtd_system_members.join("\n"),
        rtd_framework_members.join("\n"),
