...
use rtd_types::{
    MOVE_STDLIB_PACKAGE_ID, RTD_FRAMEWORK_PACKAGE_ID, RTD_SYSTEM_PACKAGE_ID,
    base_types::ObjectID,
...
pub struct SystemPackageMetadata {
    ...
    pub name: String,
    /// The path within the repo to the source (e.g. "crates/rtd-framework/packages/move-stdlib")
    pub path: String,
        ...
        static PACKAGES: LazyLock<Vec<SystemPackageMetadata>> = LazyLock::new(|| {
            vec![
                $(SystemPackageMetadata::new(
                    $name,
                    concat!("crates/rtd-framework/packages/", $path),
                    $id,
