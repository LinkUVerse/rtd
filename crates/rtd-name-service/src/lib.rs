...
const DEFAULT_TLD: &str = "rtd";
const ACCEPTED_SEPARATORS: [char; 2] = ['.', '*'];
const RTD_NEW_FORMAT_SEPARATOR: char = '@';
...
pub struct Registry {
    ...
    reverse_registry: Table<RtdAddress, Domain>,
}
...
pub struct NameRecord {
    ...
    pub target_address: Option<RtdAddress>,
    /// Additional data which may be stored in a record
    pub data: VecMap<String, String>,
}

/// A RtdnsRegistration object to manage an SLD
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct RtdnsRegistration {
    pub id: UID,
    pub domain: Domain,
...
pub struct SubDomainRegistration {
    pub id: UID,
    pub nft: RtdnsRegistration,
}

/// Two different view options for a domain.
/// `At` -> `test@example` | `Dot` -> `test.example.rtd`
...
pub struct NameServiceConfig {
    pub package_address: RtdAddress,
    pub registry_id: ObjectID,
    pub reverse_registry_id: ObjectID,
}

/// Rust version of the Move rtd::table::Table type.
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
impl Domain {
    pub fn type_(package_address: RtdAddress) -> StructTag {
        StructTag {
    ...
    /// E.g. `test.example.rtd` -> `example.rtd`
    ///
    ...
    pub fn format(&self, format: DomainFormat) -> String {
        ...
        format!("{}{}{}", labels.join(sep), RTD_NEW_FORMAT_SEPARATOR, sld)
    }
}

impl NameServiceConfig {
    pub fn new(
        package_address: RtdAddress,
    ...
    pub fn record_field_id(&self, domain: &Domain) -> ObjectID {
        ...
        rtd_types::dynamic_field::derive_dynamic_field_id(
            self.registry_id,
    ...
    pub fn mainnet() -> Self {
        ...
        let package_address = RtdAddress::from_str(MAINNET_NS_PACKAGE_ADDRESS).unwrap();
    ...
    pub fn testnet() -> Self {
        ...
        let package_address = RtdAddress::from_str(TESTNET_NS_PACKAGE_ADDRESS).unwrap();
...
/// E.g.  `example.rtd` -> `.` | example*rtd -> `@` | `example*rtd` -> `*`
...
/// Converts @label ending to label{separator}rtd ending.
///
/// E.g. `@example` -> `example.rtd` | `test@example` -> `test.example.rtd`
fn convert_from_new_format(s: &str, separator: &char) -> Result<String, NameServiceError> {
    let mut splits = s.split(RTD_NEW_FORMAT_SEPARATOR);
...
mod tests {
    ...
    fn test_parent_extraction() {
        let mut name = Domain::from_str("leaf.node.test.rtd").unwrap();

        assert_eq!(name.parent().to_string(), "node.test.rtd");

        name = Domain::from_str("node.test.rtd").unwrap();

        assert_eq!(name.parent().to_string(), "test.rtd");
    }
    ...
    fn test_expirations() {
        ...
        let mut name = NameRecord {
            nft_id: rtd_types::id::ID::new(ObjectID::random()),
            data: VecMap { contents: vec![] },
            target_address: Some(RtdAddress::random_for_testing_only()),
    ...
    fn test_name_service_outputs() {
        assert_eq!("@test".parse::<Domain>().unwrap().to_string(), "test.rtd");
        assert_eq!(
            "test.rtd".parse::<Domain>().unwrap().to_string(),
            "test.rtd"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "test.test.example.rtd"
        );
        assert_eq!(
            "rtd@rtd".parse::<Domain>().unwrap().to_string(),
            "rtd.rtd.rtd"
        );

        assert_eq!("@rtd".parse::<Domain>().unwrap().to_string(), "rtd.rtd");
        ...
        assert_eq!(
            "test.test.rtd".parse::<Domain>().unwrap().to_string(),
            "test.test.rtd"
        );
        assert_eq!(
            "test.test.test.rtd".parse::<Domain>().unwrap().to_string(),
            "test.test.test.rtd"
        );
    }
    ...
    fn test_different_wildcard() {
        assert_eq!("test.rtd".parse::<Domain>(), "test*rtd".parse::<Domain>(),);

        assert_eq!("@test".parse::<Domain>(), "test*rtd".parse::<Domain>(),);
    }
    ...
    fn test_invalid_inputs() {
        ...
        assert!(".".parse::<Domain>().is_err());
        assert!("@".parse::<Domain>().is_err());
        assert!("@inner.rtd".parse::<Domain>().is_err());
        assert!("@inner*rtd".parse::<Domain>().is_err());
        assert!("test@".parse::<Domain>().is_err());
        assert!("rtd".parse::<Domain>().is_err());
        assert!("test.test@example.rtd".parse::<Domain>().is_err());
        assert!("test@test@example".parse::<Domain>().is_err());
    }

    #[test]
    fn output_tests() {
        let mut domain = "test.rtd".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.rtd");
        assert!(domain.format(DomainFormat::At) == "@test");

        domain = "test.test.rtd".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.rtd");
        assert!(domain.format(DomainFormat::At) == "test@test");

        domain = "test.test.test.rtd".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.test.rtd");
        assert!(domain.format(DomainFormat::At) == "test.test@test");

        domain = "test.test.test.test.rtd".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.test.test.rtd");
