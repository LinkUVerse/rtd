...
/// Parse the owner's `RtdAddress` from a string. Addresses must start with `0x` followed by
...
/// new SDK.
fn owner(input: &str) -> Result<RtdAddress, Error> {
    ...
    RtdAddress::from_str(s.as_ref()).map_err(|_| Error::InvalidOwner(s.to_string()))
}
