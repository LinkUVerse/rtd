...
use rtd_rosetta::types::{CurveType, PrefundedAccount, RtdEnv};
use rtd_rosetta::{RosettaOfflineServer, RosettaOnlineServer, RTD};
use rtd_rpc::client::Client as GrpcClient;
use rtd_types::base_types::RtdAddress;
use rtd_types::crypto::{KeypairTraits, RtdKeyPair, ToFromBytes};
use tracing::info;

#[derive(Parser)]
#[clap(name = "rtd-rosetta", rename_all = "kebab-case", author, version)]
        ...
        match self {
            RosettaServerCommand::GenerateRosettaCLIConfig {
                ...
                let path = keystore_path
                    .unwrap_or_else(|| rtd_config_dir().unwrap().join(RTD_KEYSTORE_FILENAME));

                let prefunded_accounts = read_prefunded_account(&path)?;

                info!(
                    "Retrieved {} Rtd address from keystore file {:?}",
                ...
                let dsl_path = PathBuf::from(".").join("rtd.ros");
                let dsl = include_str!("../resources/rtd.ros");
                fs::write(
                    &dsl_path,
                    dsl.replace("{{rtd.env}}", json!(env).as_str().unwrap()),
                )?;
            ...
            RosettaServerCommand::StartOnlineRemoteServer {
                ...
                info!(
                    "Starting Rosetta Online Server with remote Rtd full node [{full_node_url}]."
                );
...
/// This method reads the keypairs from the Rtd keystore to create the PrefundedAccount objects,
