...
use rtd_rpc::proto::rtd::rpc::v2::BalanceChange;
use rtd_rpc::proto::rtd::rpc::v2::ExecutedTransaction;
use rtd_rpc::proto::rtd::rpc::v2::Input;
use rtd_rpc::proto::rtd::rpc::v2::MoveCall;
use rtd_rpc::proto::rtd::rpc::v2::ProgrammableTransaction;
use rtd_rpc::proto::rtd::rpc::v2::TransactionKind;
use rtd_rpc::proto::rtd::rpc::v2::argument::ArgumentKind;
use rtd_rpc::proto::rtd::rpc::v2::command::Command;
...
use rtd_types::{RTD_SYSTEM_ADDRESS, RTD_SYSTEM_PACKAGE_ID};

use crate::types::internal_operation::{PayCoin, PayRtd, Stake, WithdrawStake};
use crate::types::{
...
use crate::{CoinMetadataCache, Error, RTD};
...
impl Operations {
    ...
    pub fn into_internal(self) -> Result<InternalOperation, Error> {
        ...
        match type_ {
            OperationType::PayRtd => self.pay_rtd_ops_to_internal(),
    ...
    fn pay_rtd_ops_to_internal(self) -> Result<InternalOperation, Error> {
        ...
        Ok(InternalOperation::PayRtd(PayRtd {
            sender,
            recipients,
            amounts,
        }))
    ...
    fn parse_programmable_transaction(
        ...
        fn transfer_object(
            aggregated_recipients: &mut HashMap<RtdAddress, u64>,
            inputs: &[Input],
            known_results: &[Vec<KnownValue>],
            objs: &[Argument],
            recipient: &Argument,
            ...
            let addr = match recipient.kind() {
                ArgumentKind::Input => {
                    ...
                    match input.kind() {
                        InputKind::Pure => {
                            let bytes = input.pure();
                            bcs::from_bytes::<RtdAddress>(bytes).ok()?
                        }
                        _ => return None,
                    }
                }
        ...
        fn stake_call(
            ...
            known_results: &[Vec<KnownValue>],
            call: &MoveCall,
        ) -> Result<Option<(Option<u64>, RtdAddress)>, Error> {
                        ...
                        ArgumentKind::Input => {
                            ...
                            let validator_addr = match inputs.get(i as usize) {
                                Some(input) if input.kind() == InputKind::Pure => {
                                    bcs::from_bytes::<RtdAddress>(input.pure()).ok()
                                }
                                _ => None,
                            };
        ...
        let mut aggregated_recipients: HashMap<RtdAddress, u64> = HashMap::new();
                ...
                aggregated_recipients
                    ...
                    .map(|(recipient, amount)| {
                        ...
                        match currency {
                            ...
                            None => Operation::pay_rtd(status, recipient, amount.into()),
                        }
                    }),
            );
            match currency {
                ...
                _ => operations.push(Operation::pay_rtd(status, sender, -(total_paid as i128))),
            }
    ...
    fn is_stake_call(tx: &MoveCall) -> bool {
        ...
        package_id == RTD_SYSTEM_PACKAGE_ID
            && tx.module() == RTD_SYSTEM_MODULE_NAME.as_str()
    ...
    fn process_balance_change(
        ...
        status: Option<OperationStatus>,
        balances: HashMap<(RtdAddress, Currency), i128>,
                ...
                .fold(balances, |mut balances, (balance_change, ccy)| {
                    if let (Some(addr_str), Some(amount_str)) =
                        ...
                        && let (Ok(owner), Ok(amount)) =
                            (RtdAddress::from_str(addr_str), i128::from_str(amount_str))
                    {
        ...
        *balances.entry((gas_owner, RTD.clone())).or_default() -= gas_used;
    ...
    fn add_missing_gas_owner(operations: &mut Vec<Operation>, gas_owner: RtdAddress) {
    ...
    fn validate_operations(
        ...
        let balances: HashMap<(RtdAddress, Currency), i128> = HashMap::new();
    ...
    fn process_gascoin_transfer(
        ...
        prev_gas_owner: RtdAddress,
        new_gas_owner: RtdAddress,
        gas_used: i128,
        initial_balance_changes: &[(BalanceChange, Currency)],
        ...
        if is_gascoin_transfer && prev_gas_owner != new_gas_owner {
            ...
            for operation in &mut operations {
                match operation.type_ {
                    ...
                    OperationType::RtdBalanceChange => {
                        ...
                        if account.address == prev_gas_owner && amount.currency == *RTD {
                            amount.value -= gas_used;
                        } else if account.address == new_gas_owner && amount.currency == *RTD {
                            amount.value += gas_used;
                        }
                    }
                    _ => {
...
impl Operations {
    pub async fn try_from_executed_transaction(
        ...
        let sender = RtdAddress::from_str(transaction.sender())?;

        let gas_owner = if effects.gas_object.is_some() {
            let gas_object = effects.gas_object();
            let owner = gas_object.output_owner();
            RtdAddress::from_str(owner.address())?
        } else if sender == RtdAddress::ZERO {
        ...
        let prev_gas_owner = RtdAddress::from_str(transaction.gas_payment().owner())?;
        ...
        let staking_balance = if principal_amounts != 0 {
            *accounted_balances.entry((sender, RTD.clone())).or_default() -= principal_amounts;
            *accounted_balances.entry((sender, RTD.clone())).or_default() -= reward_amounts;
            vec![
        ...
        for balance_change in &balance_changes {
            ...
            let type_tag = rtd_types::TypeTag::from_str(coin_type)
        ...
        // and convert BalanceChange to PayRtd when GasCoin is transferred
        ...
        // This is a workaround for the payCoin cases that are mistakenly considered to be payRtd operations
        // In this case we remove any irrelevant, RTD specific operation entries that sum up to 0 balance changes per address
        ...
        let mutually_cancelling_balances: HashMap<_, _> = ops
            ...
            .fold(
                HashMap::new(),
                |mut balances: HashMap<(RtdAddress, Currency), i128>, op| {
                    if let (Some(acc), Some(amount), Some(OperationStatus::Success)) =
                        (&op.account, &op.amount, &op.status)
...
fn is_unstake_event(tag: &StructTag) -> bool {
    tag.address == RTD_SYSTEM_ADDRESS
...
impl Operation {
    ...
    pub fn genesis(index: u64, sender: RtdAddress, coin: GasCoin) -> Self {
        Operation {
    ...
    fn pay_rtd(status: Option<OperationStatus>, address: RtdAddress, amount: i128) -> Self {
        Operation {
            operation_identifier: Default::default(),
            type_: OperationType::PayRtd,
            status,
    ...
    fn balance_change(
        ...
        Self {
            operation_identifier: Default::default(),
            type_: OperationType::RtdBalanceChange,
            status,
    ...
    fn gas(addr: RtdAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_reward(status: Option<OperationStatus>, addr: RtdAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_principle(status: Option<OperationStatus>, addr: RtdAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
...
mod tests {
    ...
    use rtd_types::base_types::{ObjectDigest, ObjectID, SequenceNumber, RtdAddress};
    ...
    #[tokio::test]
    async fn test_operation_data_parsing_pay_rtd() -> Result<(), anyhow::Error> {
        let gas = (
            ObjectID::random(),
            SequenceNumber::new(),
            ObjectDigest::random(),
        ...
        let pt = {
            ...
            builder
                .pay_rtd(vec![RtdAddress::random_for_testing_only()], vec![10000])
                .unwrap();
            builder.finish()
        };
        let gas_price = 10;
        ...
        ops.0
            .iter()
            .for_each(|op| assert_eq!(op.type_, OperationType::PayRtd));
