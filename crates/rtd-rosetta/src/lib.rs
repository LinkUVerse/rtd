...
use rtd_sdk_types::{StructTag, TypeTag as SDKTypeTag};
...
use crate::types::{Currency, CurrencyMetadata, RtdEnv};
...
pub static RTD: Lazy<Currency> = Lazy::new(|| Currency {
    symbol: "RTD".to_string(),
    decimals: 9,
    metadata: CurrencyMetadata {
        coin_type: SDKTypeTag::from(StructTag::rtd()).to_string(),
    },
});
...
impl RosettaOnlineServer {
    pub fn new(env: RtdEnv, client: Client) -> Self {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Rtd Rosetta online server listening on {}",
...
impl RosettaOfflineServer {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Rtd Rosetta offline server listening on {}",
            listener.local_addr().unwrap()
        );
        axum::serve(listener, app).await.unwrap();
    }
