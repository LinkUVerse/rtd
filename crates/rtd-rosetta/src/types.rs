...
use rtd_rpc::proto::rtd::rpc::v2::ExecutionStatus;
...
impl RtdEnv {
    pub fn check_network_identifier(
        ...
        if &network_identifier.blockchain != "rtd" {
...
impl From<RtdAddress> for AccountIdentifier {
...
impl From<RtdPublicKey> for PublicKey {
    fn from(pk: RtdPublicKey) -> Self {
        match pk {
            RtdPublicKey::Ed25519(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Edwards25519,
            },
            RtdPublicKey::Secp256k1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256k1,
            },
            RtdPublicKey::Secp256r1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
            RtdPublicKey::ZkLogin(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::ZkLogin, // inaccurate but added for completeness.
            },
            RtdPublicKey::Passkey(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
        }
...
impl TryInto<RtdAddress> for PublicKey {
    type Error = Error;

    fn try_into(self) -> Result<RtdAddress, Self::Error> {
        let key_bytes = self.hex_bytes.to_vec()?;
        let pub_key = RtdPublicKey::try_from_bytes(self.curve_type.into(), &key_bytes)?;
        Ok((&pub_key).into())
    }
}
...
pub enum OperationType {
    ...
    StakeReward,
    StakePrinciple,
    // rtd-rosetta supported operation type
    PayRtd,
    PayCoin,
    Stake,
    WithdrawStake,
    // All other Rtd transaction types, readonly
    EpochChange,
    Genesis,
...
pub struct ConstructionMetadata {
    ...
    /// Always refers to RTD balance used
    #[serde(with = "str_format")]
