...
Rtd-Rosetta is an implementation of the Mesh API (f.k.a. Rosetta API) for the Rtd network, the Rtd-Rosetta server uses the Rtd Full Node to
read and write transactions to the Rtd network.
...
`./rtd genesis -f`  
The Rtd genesis process will create configs and coins for testing, the config files are stored in `~/.rtd/rtd_config` by
default.
...
`./rtd-rosetta start-online-server`
...
`./rtd-rosetta generate-rosetta-cli-config`
This will generate the `rosetta-cli.json` and `rtd.ros` file to be used by the [Rosetta-CLI](https://github.com/coinbase/rosetta-cli)
...
```shell
cd <rtd project directory>/crate/rtd-rosetta/docker/rtd-rosetta-local
```   
#### 2. Build the image
```shell
./build.sh
...
```shell
docker run linku/rtd-rosetta-devnet rtd-rosetta start-online-server
```
Alternatively, to start the rosetta-offline server, run
```shell
docker run linku/rtd-rosetta-devnet rtd-rosetta start-offline-server
```
...
| Method | Endpoint         | Description                    | Rtd Supported? |  Server Type  |
|--------|------------------|--------------------------------|:--------------:|:-------------:|
...
| Method | Endpoint           | Description             |                                      Rtd Supported?                                       |  Server Type  |
|--------|--------------------|-------------------------|:-----------------------------------------------------------------------------------------:|:-------------:|
| POST   | /block             | Get a Block             | Yes (One transaction per block in phase 1, will be replaced by Rtd checkpoint in phase 2) |    Online     |
...
## Rtd transaction <> Rosetta Operation conversion explained
There are 2 places we convert Rtd's transaction to Rosetta's operations, 
...
the `/construction/parse` endpoint is used for checking transaction correctness during transaction construction, in our case we only support `TransferRtd` for now, the operations created looks like this (negative amount indicate sender):
```json
{
...
Since the intent operations of TransferRtd contains all the balance change information(amount field) already, 
