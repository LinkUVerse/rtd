...
use rtd_rpc::proto::rtd::rpc::v2::changed_object::IdOperation;
use rtd_rpc::proto::rtd::rpc::v2::get_object_result;
use rtd_rpc::proto::rtd::rpc::v2::{BatchGetObjectsRequest, ExecutedTransaction, GetObjectRequest};
...
pub async fn split_coins(
    ...
    let sig = keystore
        .sign_secure(&tx_data.sender(), &tx_data, Intent::rtd_transaction())
        .await?;
...
async fn test_make_change_exact_div() -> Result<()> {
    const RTD_100: u64 = 100_000_000_000;
    ...
    if get_coin_value(&new_coin) != RTD_100 {
        return Err(anyhow!(
    ...
    assert!(
        new_coins_count == (RTD_100 / RTD_10) as usize - 1,
...
async fn test_make_change_remainder_div() -> Result<()> {
    ...
    assert!(
        new_coins.len() == (RTD_100 / RTD_12) as usize,
        "Expected {} new coins. Got: {}",
        RTD_100 / RTD_12,
        new_coins.len()
    );
    ...
    for result in batch_response.objects {
        ...
        match b {
            ...
            b => {
                return Err(anyhow!(
                    "Did not expect anything else other than 12 or 4 RTD. Found {b}"
                ));
            }
        }
    }

    assert!(twelve_count == 8, "Expected 8 coins with 12 RTD");
    assert!(four_count == 1, "Expected 1 coin with 4 RTD");
