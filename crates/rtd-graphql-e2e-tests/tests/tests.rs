...
use rtd_graphql_rpc::test_infra::cluster::{ExecutorCluster, serve_executor};
use rtd_transactional_test_runner::{
    args::RtdInitArgs,
    create_adapter,
    offchain_state::{OffchainStateReader, TestResponse},
    run_tasks_with_adapter,
    test_adapter::{PRE_COMPILED, RtdTestAdapter},
};
...
async fn run_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !cfg!(msim) {
        ...
        // In another crate like `rtd-mvr-graphql-e2e-tests`, this would be the place to translate
