...
//# run-graphql --cursors bcs(bcs(@{obj_3_0}),@{highest_checkpoint})
{
...
//# run-graphql --cursors bcs(bcs(@{obj_3_0}),1)
...
//# run-graphql --cursors bcs(bcs(@{obj_3_0}),2)
{
  address(address: "@{A}") {
...
//# run-graphql --cursors bcs(bcs(@{obj_3_0}),3)
{
  address(address: "@{A}") {
    objects(filter: {type: "@{Test}"}) {
      nodes {
