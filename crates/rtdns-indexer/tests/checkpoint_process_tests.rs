...
use rtdns_indexer::indexer::RtdnsIndexer;
...
/// Checkpoint 22279496: Replaces the name added on `22279365` (new.test.rtd) by removing it and then adding it as a node name.
/// Checkpoint 22279944: Adds `remove.test.rtd`.
/// Checkpoint 22280030: Adds `remove.test.rtd` as a replacement (the previous one expired!).
...
fn process_22279187_checkpoint() {
    ...
    assert!(names.contains(&"leaf.test.rtd".to_string()));
    assert!(names.contains(&"node.test.rtd".to_string()));
    assert!(names.contains(&"test.rtd".to_string()));
}
...
fn process_22279365_checkpoint() {
    ...
    assert_eq!(addition.name, "new.test.rtd".to_string());
    assert_eq!(addition.parent, "test.rtd".to_string());
    assert_eq!(
        addition.nft_id,
...
fn process_22279944_checkpoint() {
    ...
    let addition = updates.first().unwrap();
    assert_eq!(addition.name, "remove.test.rtd".to_string());
    assert_eq!(
        addition.nft_id,
...
/// Return a rtdns_indexer instance for testing.
/// Uses the testnet demo we used to extract the checkpoints being processed.
fn get_test_indexer() -> RtdnsIndexer {
    RtdnsIndexer::new(
