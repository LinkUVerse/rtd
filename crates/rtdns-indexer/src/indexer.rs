...
use rtd_name_service::{Domain, NameRecord, SubDomainRegistration};
use rtd_types::{
...
pub struct RtdnsIndexer {
    registry_table_id: RtdAddress,
    subdomain_wrapper_type: StructTag,
    name_record_type: StructTag,
}

impl std::default::Default for RtdnsIndexer {
    fn default() -> Self {
        Self::new(
...
impl RtdnsIndexer {
    ...
    pub fn new(registry_address: String, wrapper_type: String, record_type: String) -> Self {
        let registry_table_id = RtdAddress::from_str(&registry_address).unwrap();
    ...
    pub fn process_checkpoint(&self, data: &CheckpointData) -> (Vec<VerifiedDomain>, Vec<String>) {
        let mut checkpoint = RtdnsIndexerCheckpoint::new(data.checkpoint_summary.sequence_number);
...
pub struct RtdnsIndexerCheckpoint {
...
impl RtdnsIndexerCheckpoint {
    ...
    pub fn parse_record_changes(&mut self, config: &RtdnsIndexer, objects: &[Object]) {
...
/// Allows us to format a RtdNS specific query for updating the DB entries
