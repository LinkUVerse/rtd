...
use dotenvy::dotenv;
use linku_service::metrics::start_basic_prometheus_server;
use prometheus::Registry;
use std::env;
use std::path::PathBuf;
use rtd_data_ingestion_core::{
...
use rtdns_indexer::{
    ...
    indexer::{RtdnsIndexer, format_update_field_query, format_update_subdomain_wrapper_query},
    models::VerifiedDomain,
    schema::domains,
};

struct RtdnsIndexerWorker {
    pg_pool: PgConnectionPool,
    indexer: RtdnsIndexer,
}

impl RtdnsIndexerWorker {
...
#[async_trait]
impl Worker for RtdnsIndexerWorker {
    type Result = ();
...
async fn main() -> Result<()> {
    let _guard = linku_service::logging::init();
    dotenv().ok();
    ...
    let indexer_setup =
        ...
        {
            RtdnsIndexer::new(registry_id, subdomain_wrapper_type, name_record_type)
        } else {
            RtdnsIndexer::default()
        };
    ...
    let worker_pool = WorkerPool::new(
        ...
        "rtdns_indexing".to_string(), /* task name used as a key in the progress store */
        100,                          /* concurrency */
    );
