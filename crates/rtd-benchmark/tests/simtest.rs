...
mod test {
    ...
    use rtd_benchmark::bank::BenchmarkBank;
    use rtd_benchmark::system_state_observer::SystemStateObserver;
    use rtd_benchmark::workloads::adversarial::AdversarialPayloadCfg;
    use rtd_benchmark::workloads::benchmark_move_base_dir;
    use rtd_benchmark::workloads::expected_failure::ExpectedFailurePayloadCfg;
    use rtd_benchmark::workloads::workload::ExpectedFailureType;
    use rtd_benchmark::workloads::workload_configuration::{
    ...
    use rtd_config::node::{AuthorityOverloadConfig, ForkCrashBehavior, ForkRecoveryConfig};
    use rtd_config::{AUTHORITIES_DB_NAME, RTD_KEYSTORE_FILENAME};
    ...
    use rtd_core::checkpoints::{CheckpointStore, CheckpointWatermark};
    ...
    use rtd_simulator::tempfile::TempDir;
    use rtd_simulator::{SimConfig, configs::*};
    use rtd_storage::blob::Blob;
    use rtd_surfer::surf_strategy::SurfStrategy;
    use rtd_swarm_config::network_config_builder::ConfigBuilder;
    use rtd_types::base_types::{AuthorityName, ConciseableName, ObjectID, SequenceNumber};
    ...
    use rtd_types::traffic_control::{FreqThresholdConfig, PolicyConfig, PolicyType};
    ...
    async fn test_simulated_load_with_reconfig() {
        rtd_protocol_config::ProtocolConfig::poison_get_for_min_version();
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<rtd_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(rtd_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<rtd_simulator::task::NodeId>,
        ...
        let cur_node = rtd_simulator::current_simnode_id();
        ...
        if rng.gen_range(0.0..1.0) < probability {
            ...
            *dead_validator = Some(DeadValidator {
            ...
            rtd_simulator::task::kill_current_node(Some(restart_after));
        }
    }
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.rtd_node.clone().state();
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = rtd_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            rtd_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        let test_cluster = Arc::new(
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = rtd_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    fn handle_bool_failpoint(
        eligible_nodes: &HashSet<rtd_simulator::task::NodeId>, // only given eligible nodes may fail
        probability: f64,
    ) -> bool {
        if !eligible_nodes.contains(&rtd_simulator::current_simnode_id()) {
        ...
        Fut: std::future::Future<Output = ()> + Send,
    {
        ...
        let keystore_path = test_cluster.swarm.dir().join(RTD_KEYSTORE_FILENAME);
        ...
        if enable_surfer {
            let surfer_task = tokio::spawn(async move {
                // now do a rtd-surfer test
                let mut test_packages_dir = benchmark_move_base_dir();
                test_packages_dir.extend(["..", "..", "crates", "rtd-surfer", "tests"]);
                let test_package_paths: Vec<PathBuf> = std::fs::read_dir(test_packages_dir)
                    ...
                    .collect();
                info!("using rtd_surfer test packages: {test_package_paths:?}");

                let surf_strategy = SurfStrategy::new(Duration::from_millis(400));
                let results = rtd_surfer::run_with_test_cluster_and_strategy(
                    surf_strategy,
                    test_duration,
                    test_package_paths,
                    test_cluster,
                ...
                .await;
                info!("rtd_surfer test complete with results: {results:?}");
                ...
                move |_cluster: Arc<TestCluster>| async move {
                    ...
                    register_fail_point_arg("kill_checkpoint_fork_node", {
                        let forked_validators: Arc<
                            Mutex<HashSet<rtd_types::crypto::AuthorityPublicKeyBytes>>,
                        > = forked_validators.clone();
                        let checkpoint_overrides = checkpoint_overrides.clone();
                        let node_to_authority_map = node_to_authority_map.clone();
                        move || {
                            let current_node_id = rtd_simulator::current_simnode_id();
                            let authority_name =
                        ...
                        move || {
                            forked_validators.lock().unwrap().contains(
                                node_to_authority_map
                                    .get(&rtd_simulator::current_simnode_id())
                                    .unwrap(),
                            )
                        }
                    });
