...
/// A Rtd account and all of the objects it owns
#[derive(Debug)]
pub struct RtdAccount {
...
pub struct InMemoryWallet {
    accounts: BTreeMap<RtdAddress, RtdAccount>, // TODO: track shared and immutable objects as well
}
...
impl InMemoryWallet {
    ...
    pub fn add_account(
        ...
        self.accounts.insert(addr, RtdAccount::new(key, gas, objs));
    }
    ...
    pub fn account_mut(&mut self, addr: &RtdAddress) -> Option<&mut RtdAccount> {
        self.accounts.get_mut(addr)
    }

    pub fn account(&self, addr: &RtdAddress) -> Option<&RtdAccount> {
        self.accounts.get(addr)
    }

    pub fn accounts(&self) -> impl Iterator<Item = &RtdAddress> {
        self.accounts.keys()
    }

    pub fn gas(&self, addr: &RtdAddress) -> Option<&ObjectRef> {
        self.accounts.get(addr).map(|a| &a.gas)
    }

    pub fn owned_object(&self, addr: &RtdAddress, id: &ObjectID) -> Option<&ObjectRef> {
        self.accounts.get(addr).and_then(|a| a.owned.get(id))
    }

    pub fn owned_objects(&self, addr: &RtdAddress) -> Option<impl Iterator<Item = &ObjectRef>> {
    ...
    pub fn keypair(&self, addr: &RtdAddress) -> Option<Arc<AccountKeyPair>> {
    ...
    pub fn addresses(&self) -> impl Iterator<Item = &RtdAddress> {
        self.accounts.keys()
    }
