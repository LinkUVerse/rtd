...
impl BenchmarkBank {
    ...
    pub async fn generate(
        ...
        for chunk in chunked_coin_configs {
            let gas_coins = self.pay_rtd(chunk, &mut init_coin, gas_price).await?;
    ...
    async fn pay_rtd(
        ...
        let recipient_addresses: Vec<RtdAddress> = coin_configs.iter().map(|g| g.address).collect();
        ...
        let tx = make_pay_rtd_transaction(
            init_coin.0,
            vec![],
            recipient_addresses,
            amounts,
        ...
        let address_map: HashMap<RtdAddress, Arc<AccountKeyPair>> = coin_configs
            .iter()
