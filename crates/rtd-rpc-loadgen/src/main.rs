...
#[derive(Parser)]
#[clap(
    name = "Rtd RPC Load Generator",
    version = "0.1",
    about = "A load test application for Rtd RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.rtd/rtd_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.rtd/loadgen/data")]
...
fn get_keypair() -> Result<SignerInfo> {
    ...
    // Load keystore from ~/.rtd/rtd_config/rtd.keystore
    let keystore_path = get_rtd_config_directory().join("rtd.keystore");
    ...
    let keypair: &RtdKeyPair = keystore.export(&active_address)?;
    println!("using address {active_address} for signing");
    Ok(SignerInfo::new(keypair.encode_base64()))
}

fn get_rtd_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".rtd").join("rtd_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("rtd-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayRtd { common } => (Command::new_pay_rtd(), common, true),
