...
pub struct RpcCommandProcessor {
    clients: Arc<RwLock<Vec<RtdClient>>>,
    ...
    addresses: Arc<DashSet<RtdAddress>>,
    data_dir: String,
}
...
impl RpcCommandProcessor {
    ...
    async fn process_command_data(
        ...
        match command {
            ...
            CommandData::PayRtd(v) => self.process(v, signer_info).await,
    ...
    pub(crate) async fn get_clients(&self) -> Result<Vec<RtdClient>> {
    ...
    pub(crate) fn add_addresses_from_response(&self, responses: &[RtdTransactionBlockResponse]) {
    ...
    pub(crate) fn add_object_ids_from_response(&self, responses: &[RtdTransactionBlockResponse]) {
        for response in responses {
    ...
    pub(crate) fn dump_cache_to_file(&self) {
        ...
        let addresses: Vec<RtdAddress> = self.addresses.iter().map(|x| *x).collect();
        if !addresses.is_empty() {
            debug!("dumping addresses to file {:?}", addresses.len());
            write_data_to_file(
                &addresses,
                &format!("{}/{}", &self.data_dir, CacheType::RtdAddress),
            )
            .unwrap();
        }
...
impl fmt::Display for CacheType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CacheType::RtdAddress => write!(f, "RtdAddress"),
...
pub fn load_addresses_from_file(filepath: String) -> Vec<RtdAddress> {
    let path = format!("{}/{}", filepath, CacheType::RtdAddress);
    let addresses: Vec<RtdAddress> = read_data_from_file(&path).expect("Failed to read addresses");
    addresses
}
...
async fn prepare_new_signer_and_coins(
    ...
    let (gas_fee_for_split, gas_fee_for_pay_rtd) = (
        DEFAULT_LARGE_GAS_BUDGET * num_split_txns as u64,
        DEFAULT_GAS_BUDGET,
    );

    let primary_keypair = RtdKeyPair::decode_base64(&signer_info.encoded_keypair)
        .expect("Decoding keypair should not fail");
    let sender = RtdAddress::from(&primary_keypair.public());
    let (coin, balance) = get_coin_with_max_balance(client, sender).await;
    // The balance needs to cover `pay_amount` plus
    // 1. gas fee for pay_rtd from the primary address to the burner address
    // 2. gas fee for splitting the primary coin into `num_coins`
    let required_balance = pay_amount + gas_fee_for_split + gas_fee_for_pay_rtd;
    ...
    let burner_keypair = RtdKeyPair::Ed25519(burner_keypair);
    ...
    let coins = get_rtd_coin_ids(client, burner_address).await;
...
async fn get_coin_with_max_balance(client: &RtdClient, address: RtdAddress) -> (ObjectID, u64) {
    let coins = get_rtd_coin_ids(client, address).await;
    assert!(!coins.is_empty());
...
async fn get_rtd_coin_ids(client: &RtdClient, address: RtdAddress) -> Vec<(ObjectID, u64)> {
    match client
        .coin_read_api()
        ...
        .await
    {
        Ok(page) => page
            ...
            .collect::<Vec<_>>(),
        Err(e) => {
            panic!("get_rtd_coin_ids error for address {address} {e}")
        }
    }
...
async fn pay_rtd(
    ...
    let sender = RtdAddress::from(&keypair.public());
    let tx = client
        .transaction_builder()
        .pay(sender, input_coins, recipients, amounts, None, gas_budget)
        .await
        .expect("Failed to construct pay rtd transaction");
    sign_and_execute(
        client,
        keypair,
        tx,
...
pub(crate) async fn sign_and_execute(
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::rtd_transaction(), &txn_data),
        keypair,
    );
