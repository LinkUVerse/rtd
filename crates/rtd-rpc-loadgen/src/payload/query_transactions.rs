...
use rtd_json_rpc_types::{
    Page, RtdTransactionBlockResponse, RtdTransactionBlockResponseOptions,
...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<RtdTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| RtdTransactionBlockResponseQuery {
                filter,
...
) -> Result<Page<RtdTransactionBlockResponse, TransactionDigest>> {
