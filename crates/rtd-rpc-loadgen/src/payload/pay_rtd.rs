...
use crate::payload::{PayRtd, ProcessPayload, RpcCommandProcessor, SignerInfo};
...
#[async_trait]
impl<'a> ProcessPayload<'a, &'a PayRtd> for RpcCommandProcessor {
    async fn process(
        &'a self,
        _op: &'a PayRtd,
        ...
        let keypair =
            RtdKeyPair::decode_base64(&encoded_keypair).expect("Decoding keypair should not fail");

        debug!(
            "Transfer Rtd {} time to {recipient} with {amount} MIST with {gas_payments:?}",
            gas_payments.len()
        );
        ...
        join_all(gas_payments.iter().map(|gas| async {
            let tx = TransactionData::new_transfer_rtd(
                recipient,
                sender,
                Some(amount),
