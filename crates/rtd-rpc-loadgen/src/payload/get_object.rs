...
use rtd_json_rpc_types::{RtdObjectDataOptions, RtdObjectResponse};
...
pub(crate) async fn get_object(
    client: &RtdClient,
    object_id: ObjectID,
) -> Result<RtdObjectResponse> {
    let result = client
        .read_api()
        .get_object_with_options(object_id, RtdObjectDataOptions::full_content())
        .await
        .unwrap();
    Ok(result)
}
