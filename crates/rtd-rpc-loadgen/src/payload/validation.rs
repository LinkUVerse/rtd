...
pub(crate) async fn check_transactions(
    ...
    cross_validate: bool,
    verify_objects: bool,
) -> Vec<Vec<RtdTransactionBlockResponse>> {
    let transactions: Vec<Vec<RtdTransactionBlockResponse>> =
        join_all(clients.iter().map(|client| async move {
            client
                .read_api()
                .multi_get_transactions_with_options(
                    digests.to_vec(),
                    RtdTransactionBlockResponseOptions::full_content(), // todo(Will) support options for this
                )
                .await
        }))
        .await
...
pub(crate) fn get_all_object_ids(response: &RtdTransactionBlockResponse) -> Vec<ObjectID> {
...
pub(crate) async fn multi_get_object(
    clients: &[RtdClient],
    object_ids: &[ObjectID],
) -> Vec<Vec<RtdObjectResponse>> {
    let objects: Vec<Vec<RtdObjectResponse>> = join_all(clients.iter().map(|client| async move {
        ...
        client
            ...
            .multi_get_object_with_options(
                object_ids.to_vec(),
                RtdObjectDataOptions::full_content(), // todo(Will) support options for this
            )
            .await
    }))
    .await
