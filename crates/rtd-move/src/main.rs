...
use rtd_config::{RTD_CLIENT_CONFIG, rtd_config_dir};
use rtd_move::execute_move_command;
use rtd_sdk::wallet_context::WalletContext;
use rtd_types::exit_main;
use tracing::debug;
...
async fn main() {
    ...
    //     RtdCommand::Console { .. } | RtdCommand::Client { .. } => {
    ...
    let _guard = telemetry_subscribers::TelemetryConfig::new()
        ...
        .with_env()
        .init();
    debug!("Rtd-Move CLI version: {VERSION}");

    exit_main!(execute(args).await);
}

async fn execute(args: Args) -> anyhow::Result<()> {
    let config = rtd_config_dir()?.join(RTD_CLIENT_CONFIG);
