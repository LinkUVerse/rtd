...
use rtd_json_rpc_types::{
    RtdObjectDataOptions, RtdTransactionBlockEffects, RtdTransactionBlockEffectsAPI,
    RtdTransactionBlockResponseOptions,
};
use rtd_sdk::RtdClient;
use rtd_sdk::apis::ReadApi;
use rtd_sdk::rpc_types::RtdObjectResponse;
...
use rtd_types::quorum_driver_types::NON_RECOVERABLE_ERROR_MSG;
use rtd_types::transaction::{Argument, Transaction};
use rtd_types::transaction::{Command, ObjectArg, SharedObjectMutability};
use rtd_types::{
...
use rtd_types::base_types::{ObjectID, ObjectRef, random_object_ref};
...
impl DataProviderRunner {
    pub async fn new(
        ...
        client: Arc<RtdClient>,
        signer_address: RtdAddress,
        metrics: Arc<OracleMetrics>,
    ) -> Self {
...
async fn get_gas_obj_ref(
    read_api: &ReadApi,
    gas_obj_id: ObjectID,
    owner_address: RtdAddress,
) -> ObjectRef {
    loop {
        match read_api
            .get_object_with_options(gas_obj_id, RtdObjectDataOptions::default().with_owner())
            .await
            .map(|resp| resp.data)
        {
            Ok(Some(gas_obj)) => {
...
impl OnChainDataUploader {
    ...
    async fn upload(
    ...
    ) -> anyhow::Result<RtdTransactionBlockEffects> {
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_rtd_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<RtdTransactionBlockResponse> {
        ...
        loop {
            match self
                ...
                Err(rtd_sdk::error::Error::RpcError(err)) => {
                    // jsonrpsee translate every RtdError into jsonrpsee::core::Error, so we need to further distinguish 
        ...
        loop {
            ...
            for (feed_name, object_id) in &self.read_configs {
                match self
                    .client
                    .read_api()
                    .get_object_with_options(*object_id, RtdObjectDataOptions::default())
                    .await
                {
                    Ok(RtdObjectResponse {
                        data: Some(_data), ..
                    }) => {
