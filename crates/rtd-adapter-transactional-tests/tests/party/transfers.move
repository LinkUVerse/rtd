...
public fun create_party(ctx: &mut TxContext) {
    let p = Pub { id: object::new(ctx) };
    transfer::public_party_transfer(p, rtd::party::single_owner(@0))
}

public fun pub_party(obj: Pub, p: rtd::party::Party) {
    transfer::public_party_transfer(obj, p)
}

public fun priv_party(obj: Priv, p: rtd::party::Party) {
...
//> 0: rtd::party::single_owner(Input(1));
//> rtd::transfer::public_party_transfer<ex::m::Pub>(Input(0), Result(0))
