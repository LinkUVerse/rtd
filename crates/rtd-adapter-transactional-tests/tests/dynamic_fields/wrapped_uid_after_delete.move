...
use rtd::dynamic_field::{add, exists_, borrow, borrow_mut};
...
entry fun t0(ctx: &mut TxContext) {
    ...
    rtd::transfer::transfer(Obj { id }, ctx.sender())
}
...
entry fun t3(obj: Obj, ctx: &mut TxContext) {
    let Obj { id } = obj;
    assert!(count(borrow(&id, 0u64)) == 1u64, 0);
    let wrapper = Wrapper { id: object::new(ctx), old: id };
    rtd::transfer::transfer(wrapper, ctx.sender())
}
