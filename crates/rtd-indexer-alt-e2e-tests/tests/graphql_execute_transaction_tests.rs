...
use rtd_types::{gas_coin::GasCoin, transaction::SharedObjectMutability};

use rtd_futures::service::Service;
use url::Url;

use rtd_types::{base_types::RtdAddress, transaction::ObjectArg};
...
async fn test_execute_transaction_grpc_errors() {
    ...
    let recipient1 = RtdAddress::random_for_testing_only();
    let recipient2 = RtdAddress::random_for_testing_only();

    let signed_tx1 =
        make_transfer_rtd_transaction(&validator_cluster.wallet, Some(recipient1), Some(1_000_000))
            .await;
    let signed_tx2 =
        make_transfer_rtd_transaction(&validator_cluster.wallet, Some(recipient2), Some(2_000_000))
            .await;
...
async fn test_execute_transaction_unchanged_consensus_objects() {
    ...
    let tx_data = {
        ...
        let clock_arg = ptb
            .obj(ObjectArg::SharedObject {
                id: rtd_types::RTD_CLOCK_OBJECT_ID,
                initial_shared_version: rtd_types::base_types::SequenceNumber::from_u64(1),
    ...
    assert_eq!(object.address, rtd_types::RTD_CLOCK_OBJECT_ID.to_string());
...
async fn test_execute_transaction_object_changes_input_output() {
    ...
    let output_type = output_move_obj
        ...
        .as_str()
        .unwrap();
    let rtd_coin_type = GasCoin::type_().to_canonical_string(true);

    // Gas coin versions: 1 → 2 (modified for gas payment)
    assert_eq!(input_state.version, 1);
    assert_eq!(output_state.version, 2);

    // Both should be RTD coins
    assert_eq!(input_type, rtd_coin_type);
    assert_eq!(output_type, rtd_coin_type);
    ...
    // Created object should be RTD coins
    ...
    let created_type = created_move_obj
        ...
        .as_str()
        .unwrap();
    assert_eq!(created_type, rtd_coin_type);
}
