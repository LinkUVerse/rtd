...
query GetObjects($owner: RtdAddress!, $after: String) {
...
#[tokio::test]
async fn test_rtd() {
    // RTD coin is available from genesis, no need to publish
    let mut cluster = FullCluster::new().await.unwrap();
    cluster.create_checkpoint().await;

    let metadata = query_metadata(&cluster, "0x2::rtd::RTD").await;
...
async fn query_owned_outputs(cluster: &FullCluster, owner: RtdAddress) -> LegacyCoinOutputs {
...
async fn query_objects(cluster: &FullCluster, owner: RtdAddress) -> Vec<(StructTag, ObjectRef)> {
    #[derive(Deserialize)]
    #[serde(rename_all = "camelCase")]
    struct PageInfo {
