...
use rtd_indexer_alt_schema::{checkpoints::StoredGenesis, epochs::StoredEpochStart};
use rtd_types::{
    balance::Balance,
    digests::Digest,
    messages_checkpoint::{CheckpointCommitment, ECMHLiveObjectSetDigest},
    rtd_system_state::{
        RtdSystemState, mock, rtd_system_state_inner_v1::RtdSystemStateInnerV1,
...
async fn safe_mode_system_state_v1() -> Result<(), anyhow::Error> {
    let rtd_system_state = RtdSystemState::V1(RtdSystemStateInnerV1 {
        safe_mode: ENABLED,
        ...
        ..mock::rtd_system_state_inner_v1()
    });
    let SafeModeEpoch {
        safe_mode:
            SafeMode {
        ...
        SAFE_MODE_QUERY,
        rtd_system_state.clone(),
        AdvanceEpochConfig {
            output_objects: mock::system_state_output_objects(rtd_system_state),
            ..Default::default()
        },
    )
    .await?;
...
async fn safe_mode_system_state_v2() -> Result<(), anyhow::Error> {
    let rtd_system_state = RtdSystemState::V2(RtdSystemStateInnerV2 {
        safe_mode: ENABLED,
        safe_mode_computation_rewards: Balance::new(COMPUTATION_COST),
        safe_mode_storage_rewards: Balance::new(STORAGE_COST),
        safe_mode_storage_rebates: STORAGE_REBATE,
        safe_mode_non_refundable_storage_fee: NON_REFUNDABLE_STORAGE_FEE,
        ..mock::rtd_system_state_inner_v2()
    });
    let SafeModeEpoch {
        safe_mode:
            SafeMode {
...
async fn live_object_set_digest() -> Result<(), anyhow::Error> {
    let rtd_system_state = RtdSystemState::V2(mock::rtd_system_state_inner_v2());
...
async fn test_graphql<T: DeserializeOwned>(
    query: &str,
    genesis_rtd_system_state: RtdSystemState,
        ...
        OffchainClusterConfig {
            ...
            bootstrap_genesis: Some(BootstrapGenesis {
                ...
                stored_epoch_start: StoredEpochStart {
                    ...
                    system_state: bcs::to_bytes(&genesis_rtd_system_state)?,
                },
            }),
            ..Default::default()
        },
