...
/// Tests happy path for RtdNS resolution (forward lookup).
...
/// Tests RtdNS resolution response when the domain doesn't exist or has expired.
...
async fn test_resolve_domain_expiry() {
    ...
    assert_not_resolved!(c.resolve_address("foo.rtd").await.unwrap());
...
async fn test_resolve_subdomain_parent_expiry() {
    ...
    assert_not_resolved!(c.resolve_address("bar.foo.rtd").await.unwrap());
    assert_no_reverse!(c.resolve_name(target).await.unwrap());
}
...
impl RtdNSCluster {
    /// Sets up a full cluster with a mock RtdNS package and registries. GraphQL is configured to read
    ...
    async fn new() -> Self {
        ...
        // (7) Configure the GraphQL RPC to read from the mock RtdNS package.
