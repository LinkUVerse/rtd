...
async fn test_aggregation() {
    ...
    // Create multiple RTD coins for address A with different amounts
    create_coin(&mut cluster, a, 1);
    create_coin(&mut cluster, a, 2);
    create_coin(&mut cluster, a, 3);

    // Create RTD coins for address B
...
async fn test_multiple_coin_types() {
    ...
    let rtd_balance = DEFAULT_GAS_BUDGET as i64 - fx.gas_cost_summary().net_gas_usage();
    ...
    // P's balances should include RTD (left over from the gas coin) and MY_COIN (1000 + 200 + 30)
    // = 1230 from minting during publish and init.
    let mut balances = vec![
        (gas_type.clone(), rtd_balance as u64),
    ...
    assert_eq!(
        ...
        vec![
            ...
            (p.to_string(), gas_type.clone(), rtd_balance as u64),
        ]
    );
}
...
async fn test_transfers() {
    ...
    builder.transfer_rtd(b, Some(1000));
...
async fn test_edge_cases() {
    ...
    request
        .metadata_mut()
        .insert("x-rtd-checkpoint", "10".parse().unwrap());
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: RtdAddress, amount: u64) -> ObjectRef {
    let (sender, kp, gas) = cluster
        .funded_account(DEFAULT_GAS_BUDGET + amount)
        .expect("Failed to fund account");

    let mut builder = ProgrammableTransactionBuilder::new();
    builder.transfer_rtd(owner, Some(amount));
...
async fn batch_get_balances(
    cluster: &FullCluster,
    requests: Vec<(RtdAddress, String)>,
    checkpoint: Option<u64>,
