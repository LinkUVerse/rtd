...
/// Deserialized successful JSON-RPC response for `rtdx_getOwnedObjects`.
#[derive(Deserialize)]
struct Response {
...
/// `amount` to `owner`.
fn create_coin(cluster: &mut FullCluster, owner: RtdAddress, amount: u64) -> ObjectID {
...
/// isn't a coin.
fn create_bag(cluster: &mut FullCluster, owner: RtdAddress, size: u64) -> ObjectID {
    let (sender, kp, gas) = cluster
...
/// Make a call to `rtdx_getOwnedObjects` to the RPC server running on `cluster`.
