// Copyright (c) LinkU Labs, Inc.
// SPDX-License-Identifier: Apache-2.0

//# init --protocol-version 70 --accounts A --addresses P=0x0 --simulator

// 1. Fetching a freshly created object
// 2. ...the same object after it has been modified
// 3. ...after it has been wrapped
// 4. ...after it has been unwrapped
// 5. ...after it has been deleted
// 6. Fetching an object that just doesn't exist
// 7. Fetching a newly created package
// 8. Fetching an upgraded package

//# programmable --sender A --inputs @A
//> 0: rtd::table::new<u64, rtd::coin::Coin<rtd::rtd::RTD>>();
//> 1: TransferObjects([Result(0)], Input(0))

//# programmable --sender A --inputs 42 @A
//> 0: SplitCoins(Gas, [Input(0)]);
//> 1: TransferObjects([Result(0)], Input(1))

//# create-checkpoint

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["@{obj_2_0}", { "showContent": true }]
}

//# programmable --sender A --inputs object(2,0) 21
//> 0: SplitCoins(Input(0), [Input(1)]);
//> 1: MergeCoins(Gas, [Result(0)])

//# create-checkpoint

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["@{obj_2_0}", { "showContent": true }]
}

//# programmable --sender A --inputs object(1,0) 0 object(2,0)
//> 0: rtd::table::add<u64, rtd::coin::Coin<rtd::rtd::RTD>>(Input(0), Input(1), Input(2));

//# create-checkpoint

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["@{obj_2_0}", { "showContent": true }]
}

//# programmable --sender A --inputs object(1,0) 0 @A
//> 0: rtd::table::remove<u64, rtd::coin::Coin<rtd::rtd::RTD>>(Input(0), Input(1));
//> 1: TransferObjects([Result(0)], Input(2))

//# create-checkpoint

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["@{obj_2_0}", { "showContent": true }]
}

//# programmable --sender A --inputs object(2,0)
//> 0: MergeCoins(Gas, [Input(0)])

//# create-checkpoint

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["@{obj_2_0}", { "showContent": true }]
}

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"]
}

//# publish --sender A --upgradeable
module P::M {
  public fun foo(): u64 { 42 }
}

//# upgrade --sender A --package P --upgrade-capability 18,1
module P::M {
  public fun foo(): u64 { 43 }
}

//# create-checkpoint

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["@{obj_18_0}", { "showType": true }]
}

//# run-jsonrpc
{
  "method": "rtd_getObject",
  "params": ["@{obj_19_0}", { "showType": true }]
}
