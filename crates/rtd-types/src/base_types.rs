...
use crate::governance::StakedRtd;
use crate::id::RESOLVED_RTD_ID;
...
pub enum MoveObjectType_ {
    ...
    Other(StructTag),
    /// A RTD coin (i.e., `0x2::coin::Coin<0x2::rtd::RTD>`)
    GasCoin,
    /// A record of a staked RTD coin (i.e., `0x3::staking_pool::StakedRtd`)
    StakedRtd,
...
impl MoveObjectType {
    ...
    pub fn staked_rtd() -> Self {
        Self(MoveObjectType_::StakedRtd)
    }
    ...
    pub fn balance_accumulator_field_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            MoveObjectType_::RtdBalanceAccumulatorField => Some(GAS::type_tag()),
    ...
    pub fn is_rtd_balance_accumulator_field(&self) -> bool {
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedRtd => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::RtdBalanceAccumulatorField => 1,
            MoveObjectType_::BalanceAccumulatorField(inner) => {
    ...
    /// Return true if `self` is `0x2::coin::Coin<T>` for some T (note: T can be RTD)
    ...
    /// Return true if `self` is 0x2::coin::Coin<0x2::rtd::RTD>
    ...
    pub fn is_staked_rtd(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedRtd => true,
            MoveObjectType_::GasCoin
            | MoveObjectType_::Coin(_)
            | MoveObjectType_::RtdBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_)
    ...
    pub fn is_upgrade_cap(&self) -> bool {
        self.address() == RTD_FRAMEWORK_ADDRESS
    ...
    pub fn is_dynamic_field(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedRtd | MoveObjectType_::Coin(_) => {
                false
            }
            MoveObjectType_::RtdBalanceAccumulatorField
            | MoveObjectType_::BalanceAccumulatorField(_)
    ...
    pub fn try_extract_field_name(&self, type_: &DynamicFieldType) -> RtdResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedRtd | MoveObjectType_::Coin(_) => {
    ...
    pub fn try_extract_field_value(&self) -> RtdResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedRtd | MoveObjectType_::Coin(_) => {
                Err(RtdErrorKind::ObjectDeserializationError {
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedRtd => StakedRtd::is_staked_rtd(s),
            MoveObjectType_::Coin(inner) => {
                Coin::is_coin(s) && s.type_params.len() == 1 && inner == &s.type_params[0]
            }
            MoveObjectType_::RtdBalanceAccumulatorField => accumulator_value_balance_type_maybe(s)
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedRtd::is_staked_rtd(&s) {
            MoveObjectType_::StakedRtd
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedRtd => StakedRtd::type_(),
...
pub fn is_primitive_type_tag(t: &TypeTag) -> bool {
    ...
    match t {
        ...
        T::Struct(st) => {
            ...
            // is id or..
            if resolved_struct == RESOLVED_RTD_ID {
                return true;
            }
            // is utf8 string
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_rtd_struct_tag(s)?;
...
pub const RTD_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct RtdAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; RTD_ADDRESS_LENGTH],
);

impl RtdAddress {
    pub const ZERO: Self = Self([0u8; RTD_ADDRESS_LENGTH]);
    ...
    /// Deserialize into an `Option<RtdAddress>`.
    pub fn optional_address_from_hex<'de, D>(
        deserializer: D,
    ) -> Result<Option<RtdAddress>, D::Error>
    where
    ...
    /// Return the underlying byte array of a RtdAddress.
    pub fn to_inner(self) -> [u8; RTD_ADDRESS_LENGTH] {
        self.0
    }

    /// Parse a RtdAddress from a byte array or buffer.
    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, RtdError> {
        <[u8; RTD_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| RtdErrorKind::InvalidAddress.into())
            .map(RtdAddress)
    }
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> RtdResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> RtdResult<Self> {
        ...
        Ok(RtdAddress(hasher.finalize().digest))
    }
}

impl From<ObjectID> for RtdAddress {
    fn from(object_id: ObjectID) -> RtdAddress {
        Self(object_id.into_bytes())
    }
}

impl From<AccountAddress> for RtdAddress {
    fn from(address: AccountAddress) -> RtdAddress {
        Self(address.into_bytes())
    }
}

impl TryFrom<&[u8]> for RtdAddress {
    type Error = RtdError;

    /// Tries to convert the provided byte array into a RtdAddress.
    fn try_from(bytes: &[u8]) -> Result<Self, RtdError> {
...
impl TryFrom<Vec<u8>> for RtdAddress {
    ...
    /// Tries to convert the provided byte buffer into a RtdAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, RtdError> {
        Self::from_bytes(bytes)
    }
}

impl AsRef<[u8]> for RtdAddress {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}
...
impl<T: RtdPublicKey> From<&T> for RtdAddress {
    fn from(pk: &T) -> Self {
        let mut hasher = DefaultHash::default();
        hasher.update([T::SIGNATURE_SCHEME.flag()]);
        hasher.update(pk);
        let g_arr = hasher.finalize();
        RtdAddress(g_arr.digest)
    }
}

impl From<&PublicKey> for RtdAddress {
...
impl From<&MultiSigPublicKey> for RtdAddress {
    /// Derive a RtdAddress from [struct MultiSigPublicKey]. A MultiSig address
    ...
    fn from(multisig_pk: &MultiSigPublicKey) -> Self {
        ...
        RtdAddress(hasher.finalize().digest)
    }
}

/// Rtd address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || unpadded_address_seed_in_bytes].
impl TryFrom<&ZkLoginAuthenticator> for RtdAddress {
    type Error = RtdError;
    fn try_from(authenticator: &ZkLoginAuthenticator) -> RtdResult<Self> {
        RtdAddress::try_from_unpadded(&authenticator.inputs)
    }
}

impl TryFrom<&GenericSignature> for RtdAddress {
    type Error = RtdError;
    /// Derive a RtdAddress from a serialized signature in Rtd [GenericSignature].
    fn try_from(sig: &GenericSignature) -> RtdResult<Self> {
        match sig {
            ...
            GenericSignature::ZkLoginAuthenticator(zklogin) => {
                RtdAddress::try_from_unpadded(&zklogin.inputs)
            }
            GenericSignature::PasskeyAuthenticator(s) => Ok(RtdAddress::from(&s.get_pk()?)),
        }
    }
}
...
impl fmt::Debug for RtdAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

/// Generate a fake RtdAddress with repeated one byte.
pub fn dbg_addr(name: u8) -> RtdAddress {
    let addr = [name; RTD_ADDRESS_LENGTH];
    RtdAddress(addr)
}
...
impl TxContext {
    ...
    pub fn sponsor(&self) -> Option<RtdAddress> {
        self.sponsor.map(RtdAddress::from)
    }
...
impl From<RtdAddress> for ObjectID {
    fn from(address: RtdAddress) -> ObjectID {
...
impl From<RtdAddress> for AccountAddress {
    fn from(address: RtdAddress) -> Self {
        Self::new(address.0)
    }
}
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_rtd_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
