...
/// The value should be the same in Rtd committee.
...
impl ConsensusPosition {
    pub fn into_raw(self) -> Result<Bytes, RtdError> {
        bcs::to_bytes(&self)
            .map_err(|e| {
...
pub struct ConsensusTransaction {
    /// Encodes an u64 unique tracking id to allow us trace a message between Rtd and consensus.
...
impl AuthorityCapabilitiesV1 {
    pub fn new(
        ...
        let generation = SystemTime::now()
            ...
            .expect("Rtd did not exist prior to 1970")
            .as_millis()
            .try_into()
            .expect("This build of rtd is not supported in the year 500,000,000");
        Self {
            authority,
            generation,
            supported_protocol_versions,
