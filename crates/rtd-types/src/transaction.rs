...
use crate::{
    RTD_AUTHENTICATOR_STATE_OBJECT_ID, RTD_CLOCK_OBJECT_ID, RTD_CLOCK_OBJECT_SHARED_VERSION,
    RTD_FRAMEWORK_PACKAGE_ID, RTD_RANDOMNESS_STATE_OBJECT_ID, RTD_SYSTEM_STATE_OBJECT_ID,
...
pub enum CallArg {
    ...
    // `rtd::funds_accumulator::Withdrawal` struct and passed into Move.
    // It is allowed to have multiple withdraw arguments even for the same funds type.
    FundsWithdrawal(FundsWithdrawalArg),
}

impl CallArg {
    pub const RTD_SYSTEM_MUT: Self = Self::Object(ObjectArg::RTD_SYSTEM_MUT);
    ...
    fn input_objects(&self) -> Vec<InputObjectKind> {
        match self {
            ...
            Self::AuthenticatorStateExpire(expire) => {
                vec![InputObjectKind::SharedMoveObject {
                    id: RTD_AUTHENTICATOR_STATE_OBJECT_ID,
        ...
        match self {
            ...
            Self::BridgeCommitteeInit(bridge_version) => Either::Left(
                vec![
                    ...
                    SharedInputObject::RTD_SYSTEM_OBJ,
                ]
                .into_iter(),
            ),
            Self::StoreExecutionTimeObservations(_) => {
...
impl ObjectArg {
    pub const RTD_SYSTEM_MUT: Self = Self::SharedObject {
        id: RTD_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: RTD_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutability: SharedObjectMutability::Mutable,
    };
...
impl ProgrammableTransaction {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(
                obj,
                CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == RTD_RANDOMNESS_STATE_OBJECT_ID
            )
        }) {
            fp_ensure!(
...
impl SharedInputObject {
    pub const RTD_SYSTEM_OBJ: Self = Self {
        id: RTD_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: RTD_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutability: SharedObjectMutability::Mutable,
    };
...
impl TransactionKind {
    ...
    pub fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match &self {
            Self::ChangeEpoch(_) => {
                Either::Left(Either::Left(iter::once(SharedInputObject::RTD_SYSTEM_OBJ)))
            }
            ...
            Self::RandomnessStateUpdate(update) => {
                Either::Left(Either::Left(iter::once(SharedInputObject {
                    id: RTD_RANDOMNESS_STATE_OBJECT_ID,
...
impl TransactionData {
    ...
    pub fn new_with_gas_data(kind: TransactionKind, sender: RtdAddress, gas_data: GasData) -> Self {
        TransactionData::V1(TransactionDataV1 {
            kind,
            sender,
            gas_data,
    ...
    pub fn new_transfer_rtd(
        ...
        Self::new_transfer_rtd_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_rtd_allow_sponsor(
        recipient: RtdAddress,
        sender: RtdAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_rtd(recipient, amount);
            builder.finish()
        };
    ...
    pub fn new_pay_rtd(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.pay_rtd(recipients, amounts)?;
            builder.finish()
        };
        Ok(Self::new_programmable(
    ...
    pub fn execution_parts(&self) -> (TransactionKind, RtdAddress, GasData) {
    ...
    pub fn uses_randomness(&self) -> bool {
        self.kind()
            ...
            .any(|obj| obj.id() == RTD_RANDOMNESS_STATE_OBJECT_ID)
    }
...
pub trait TransactionDataAPI {
    ...
    fn required_signers(&self) -> NonEmpty<RtdAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> RtdAddress;
...
impl Serialize for SenderSignedTransaction {
        ...
        S: serde::Serializer,
    {
        ...
        if self.intent_message().intent != Intent::rtd_transaction() {
...
impl<'de> Deserialize<'de> for SenderSignedTransaction {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if intent_message.intent != Intent::rtd_transaction() {
...
impl SenderSignedData {
    pub fn new(tx_data: TransactionData, tx_signatures: Vec<GenericSignature>) -> Self {
        Self(SizeOneVec::new(SenderSignedTransaction {
            intent_message: IntentMessage::new(Intent::rtd_transaction(), tx_data),
            tx_signatures,
        }))
    }
    ...
    pub fn serialized_size(&self) -> RtdResult<usize> {
    ...
    fn check_user_signature_protocol_compatibility(&self, config: &ProtocolConfig) -> RtdResult {
    ...
    pub fn validity_check(&self, context: &TxValidityCheckContext<'_>) -> Result<usize, RtdError> {
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> RtdAddress {
        self.data().intent_message().value.sender()
    }

    pub fn gas_owner(&self) -> RtdAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::rtd_transaction(), &data);
            signers
                .into_iter()
...
impl SignedTransaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> RtdResult<VerifiedSignedTransaction> {
...
//
// CheckedInputObjects should really be defined in rtd-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by rtd-transaction-checks.
