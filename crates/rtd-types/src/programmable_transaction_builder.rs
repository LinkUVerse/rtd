...
use crate::{
    ...
    base_types::{FullObjectID, FullObjectRef, ObjectID, ObjectRef, RtdAddress},
...
impl ProgrammableTransactionBuilder {
    ...
    pub fn transfer_arg(&mut self, recipient: RtdAddress, arg: Argument) {
        self.transfer_args(recipient, vec![arg])
    }

    pub fn transfer_args(&mut self, recipient: RtdAddress, args: Vec<Argument>) {
    ...
    pub fn transfer_rtd(&mut self, recipient: RtdAddress, amount: Option<u64>) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn pay_all_rtd(&mut self, recipient: RtdAddress) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn split_coin(&mut self, recipient: RtdAddress, coin: ObjectRef, amounts: Vec<u64>) {
    ...
    fn pay_impl(
        ...
        let mut recipient_map: IndexMap<RtdAddress, Vec<usize>> = IndexMap::new();
