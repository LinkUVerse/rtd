...
use crate::RTD_BRIDGE_OBJECT_ID;
...
pub const BRIDGE_CREATE_ADD_TOKEN_ON_RTD_MESSAGE_FUNCTION_NAME: &IdentStr =
    ident_str!("create_add_tokens_on_rtd_message");
...
impl BridgeChainId {
    pub fn is_rtd_chain(&self) -> bool {
        matches!(
            self,
            BridgeChainId::RtdMainnet | BridgeChainId::RtdTestnet | BridgeChainId::RtdCustom
        )
    }
}
...
pub fn get_bridge_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&RTD_BRIDGE_OBJECT_ID)
...
/// Rust version of the Move rtd::bridge::Bridge type
...
pub trait BridgeTrait {
    ...
    fn frozen(&self) -> bool;
    fn try_into_bridge_summary(self) -> RtdResult<BridgeSummary>;
}
...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, RtdError> {
    let wrapper = object_store
        .get_object(&RTD_BRIDGE_OBJECT_ID)
        // Don't panic here on None because object_store is a generic store.
        .ok_or_else(|| {
            RtdErrorKind::RtdBridgeReadError("BridgeWrapper object not found".to_owned())
        })?;
    let move_object = wrapper.data.try_as_move().ok_or_else(|| {
        RtdErrorKind::RtdBridgeReadError("BridgeWrapper object must be a Move object".to_owned())
    })?;
    let result = bcs::from_bytes::<BridgeWrapper>(move_object.contents())
        .map_err(|err| RtdErrorKind::RtdBridgeReadError(err.to_string()))?;
    Ok(result)
}

pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, RtdError> {
    ...
    match version {
        1 => {
            let result: BridgeInnerV1 = get_dynamic_field_from_store(object_store, id, &version)
                .map_err(|err| {
                    RtdErrorKind::RtdBridgeReadError(format!(
        ...
        _ => Err(RtdErrorKind::RtdBridgeReadError(format!(
            "Unsupported RtdBridge version: {}",
            version
        ))
        .into()),
    }
        ...
        let transfer_limit = self
            ...
            .map(|e| {
                let source = BridgeChainId::try_from(e.key.source).map_err(|_e| {
                    RtdErrorKind::GenericBridgeError {
...
pub struct MoveTypeBridgeCommittee {
    ...
    pub member_registrations: VecMap<RtdAddress, MoveTypeCommitteeMemberRegistration>,
...
pub struct BridgeCommitteeSummary {
    ...
    pub member_registration: Vec<(RtdAddress, MoveTypeCommitteeMemberRegistration)>,
    pub last_committee_update_epoch: u64,
}
...
pub fn is_bridge_committee_initiated(object_store: &dyn ObjectStore) -> RtdResult<bool> {
    match get_bridge(object_store).map_err(|e| *e.0) {
        Ok(bridge) => Ok(!bridge.committee().members.contents.is_empty()),
        Err(RtdErrorKind::RtdBridgeReadError(..)) => Ok(false),
