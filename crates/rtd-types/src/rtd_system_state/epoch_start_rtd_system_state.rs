...
use crate::base_types::{AuthorityName, EpochId, RtdAddress};
...
pub trait EpochStartSystemStateTrait {
    ...
    fn get_validator_addresses(&self) -> Vec<RtdAddress>;
...
/// This type captures the minimum amount of information from RtdSystemState needed by a validator
/// to run the protocol. This allows us to decouple from the actual RtdSystemState type, and hence
/// do not need to evolve it when we upgrade the RtdSystemState type.
...
impl EpochStartSystemStateTrait for EpochStartSystemStateV1 {
    ...
    fn get_validator_addresses(&self) -> Vec<RtdAddress> {
        self.active_validators
            .iter()
            .map(|validator| validator.rtd_address)
            .collect()
    }
            ...
            .map(|validator| {
                (
                    ...
                    (
                        ...
                        NetworkMetadata {
                            network_address: validator.rtd_net_address.clone(),
...
mod test {
    ...
    #[test]
    fn test_rtd_and_mysticeti_committee_are_same() {
        // GIVEN
        let mut active_validators = vec![];

        for i in 0..10 {
            let (rtd_address, protocol_key): (RtdAddress, AuthorityKeyPair) = get_key_pair();
            ...
            active_validators.push(EpochStartValidatorInfoV1 {
                ...
                rtd_net_address: Multiaddr::empty(),
        ...
        // WHEN
        let rtd_committee = state.get_rtd_committee();
        let consensus_committee = state.get_consensus_committee();

        // THEN
        // assert the validators details
        assert_eq!(rtd_committee.num_members(), 10);
        assert_eq!(rtd_committee.num_members(), consensus_committee.size());
        assert_eq!(
            rtd_committee.validity_threshold(),
            consensus_committee.validity_threshold()
        );
        assert_eq!(
            rtd_committee.quorum_threshold(),
        ...
        for (authority_index, consensus_authority) in consensus_committee.authorities() {
            let rtd_authority_name = rtd_committee
            ...
            assert_eq!(
                ...
                rtd_committee.weight(rtd_authority_name),
                "LinkU & RTD committee member stake differs"
            );
        }
    }
}
