...
use super::{RtdSystemState, RtdSystemStateTrait};
use crate::base_types::{AuthorityName, ObjectID, RtdAddress};
...
use crate::rtd_system_state::get_validator_from_table;
...
/// This is the JSON-RPC type for the RTD system state object.
/// It flattens all fields to make them top-level fields such that it as minimum
/// dependencies to the internal data structures of the RTD system state type.

#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct RtdSystemStateSummary {
    ...
    pub active_validators: Vec<RtdValidatorSummary>,
    ...
    #[schemars(with = "Vec<(RtdAddress, BigInt<u64>)>")]
    #[serde_as(as = "Vec<(_, Readable<BigInt<u64>, _>)>")]
    pub at_risk_validators: Vec<(RtdAddress, u64)>,
    /// A map storing the records of validator reporting each other.
    pub validator_report_records: Vec<(RtdAddress, Vec<RtdAddress>)>,
}

impl RtdSystemStateSummary {
    pub fn get_rtd_committee_for_benchmarking(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            .active_validators
            .iter()
            .map(|validator| {
...
/// This is the JSON-RPC type for the RTD validator. It flattens all inner structures
/// to top-level fields so that they are decoupled from the internal definitions.
#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct RtdValidatorSummary {
    // Metadata
    ...
    #[schemars(with = "BigInt<u64>")]
    #[serde_as(as = "Readable<BigInt<u64>, _>")]
    pub staking_pool_rtd_balance: u64,
...
impl Default for RtdSystemStateSummary {
    fn default() -> Self {
        Self {
            epoch: 0,
            protocol_version: 1,
...
impl Default for RtdValidatorSummary {
    fn default() -> Self {
        Self {
            rtd_address: RtdAddress::default(),
...
/// Given the staking pool id of a validator, return the validator's `RtdValidatorSummary`,
    ...
    S: ObjectStore + ?Sized,
{
    ...
    let candidate_address: RtdAddress = get_dynamic_field_from_store(
        &object_store,
