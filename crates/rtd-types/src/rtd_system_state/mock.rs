...
use crate::rtd_system_state::{RtdSystemState, RtdSystemStateWrapper};
use crate::{MoveTypeTagTrait, RTD_SYSTEM_STATE_OBJECT_ID};
...
pub fn rtd_system_state_inner_v1() -> RtdSystemStateInnerV1 {
    RtdSystemStateInnerV1 {
        epoch: 0,
        protocol_version: 0,
        // must be 1
...
pub fn rtd_system_state_inner_v2() -> RtdSystemStateInnerV2 {
...
pub fn system_state_output_objects(rtd_system_state: RtdSystemState) -> Vec<Object> {
    let version = rtd_system_state.version();
    let system_state_wrapper_object = Object::new_move(
        unsafe {
            MoveObject::new_from_execution(
                ...
                true,
                0.into(),
                bcs::to_bytes(&RtdSystemStateWrapper {
                    // must be RTD_SYSTEM_STATE_OBJECT_ID
                    id: UID::new(RTD_SYSTEM_STATE_OBJECT_ID),
                    version,
                })
                .unwrap(),
    ...
    let object_bytes = match rtd_system_state {
        RtdSystemState::V1(inner) => serialize_dynamic_field(&field_id, &dynamic_field_key, inner),
        RtdSystemState::V2(inner) => serialize_dynamic_field(&field_id, &dynamic_field_key, inner),
        #[cfg(msim)]
        RtdSystemState::SimTestV1(_)
        | RtdSystemState::SimTestShallowV2(_)
        | RtdSystemState::SimTestDeepV2(_) => unimplemented!(),
    }
    .unwrap();
