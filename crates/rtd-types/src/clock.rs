...
pub const RESOLVED_RTD_CLOCK: (&AccountAddress, &IdentStr, &IdentStr) =
    (&RTD_FRAMEWORK_ADDRESS, CLOCK_MODULE_NAME, CLOCK_STRUCT_NAME);
...
impl Clock {
    ...
    /// Detects a `&mut rtd::clock::Clock` or `rtd::clock::Clock` in the signature.
    pub fn is_mutable(view: &CompiledModule, s: &SignatureToken) -> bool {
        use SignatureToken as S;
        match s {
            S::MutableReference(inner) => Self::is_mutable(view, inner),
            S::Datatype(idx) => resolve_struct(view, *idx) == RESOLVED_RTD_CLOCK,
            _ => false,
        }
    }
}
