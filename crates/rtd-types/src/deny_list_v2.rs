...
use crate::base_types::{EpochId, RtdAddress};
...
use crate::{
    MoveTypeTagTrait, RTD_DENY_LIST_OBJECT_ID, RTD_FRAMEWORK_ADDRESS, RTD_FRAMEWORK_PACKAGE_ID,
};
...
impl DenyCapV2 {
    pub fn is_deny_cap_v2(other: &StructTag) -> bool {
        other.address == RTD_FRAMEWORK_ADDRESS
...
fn check_new_regulated_coin_owners(
    new_regulated_coin_owners: BTreeMap<String, (Config, BTreeSet<RtdAddress>)>,
    cur_epoch: EpochId,
    object_store: &dyn ObjectStore,
...
pub fn get_per_type_coin_deny_list_v2(
    ...
    let config: Config =
        get_dynamic_field_from_store(object_store, RTD_DENY_LIST_OBJECT_ID, &config_key).ok()?;
    Some(config)
}
