...
impl From<NitroAttestationVerifyError> for RtdError {
    fn from(err: NitroAttestationVerifyError) -> Self {
        RtdErrorKind::NitroAttestationFailedToVerify(err.to_string()).into()
    }
}
...
) -> RtdResult<(Vec<u8>, Vec<u8>, AttestationDocument)> {
...
impl CoseSign1 {
    ...
    fn to_signed_message(&self) -> RtdResult<Vec<u8>> {
        ...
        ciborium::ser::into_writer(&value, &mut bytes).map_err(|_| {
            RtdErrorKind::NitroAttestationFailedToVerify("cannot parse message".to_string())
        })?;
        Ok(bytes)
    }
}
