...
use crate::error::{ErrorCategory, RtdError};
...
pub enum QuorumDriverError {
    ...
    QuorumDriverInternalError(RtdError),
    #[error("Invalid user signature: {0}.")]
    InvalidUserSignature(RtdError),
    #[error(
...
pub type GroupedErrors = Vec<(RtdError, StakeUnit, Vec<ConciseAuthorityPublicKeyBytes>)>;
