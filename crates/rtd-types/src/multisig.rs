...
impl AuthenticatorTrait for MultiSig {
        ...
        T: Serialize,
    {
        ...
        if RtdAddress::from(&self.multisig_pk) != multisig_address {
        ...
        for (sig, i) in self.sigs.iter().zip(as_indices(self.bitmap)?) {
            let (subsig_pubkey, weight) =
                self.multisig_pk
                    .pk_map
                    .get(i as usize)
                    .ok_or(RtdErrorKind::InvalidSignature {
            ...
            let res = match sig {
                CompressedSignature::Ed25519(s) => {
                    ...
                    pk.verify(
                        &digest,
                        &s.try_into().map_err(|_| RtdErrorKind::InvalidSignature {
        ...
        if weight_sum >= self.multisig_pk.threshold {
            ...
            Err(RtdErrorKind::InvalidSignature {
                error: format!(
...
pub fn as_indices(bitmap: u16) -> Result<Vec<u8>, RtdError> {
...
impl MultiSig {
    ...
    pub fn combine(
        ...
        for s in full_sigs {
            ...
            let index = multisig_pk
                .get_index(&pk)
                .ok_or(RtdErrorKind::IncorrectSigner {
    ...
    pub fn get_zklogin_sigs(&self) -> Result<Vec<ZkLoginAuthenticator>, RtdError> {
    ...
    pub fn get_indices(&self) -> Result<Vec<u8>, RtdError> {
...
impl FromStr for MultiSig {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let bytes = Base64::decode(s).map_err(|_| RtdErrorKind::InvalidSignature {
            error: "Invalid base64 string".to_string(),
        })?;
        let sig = MultiSig::from_bytes(&bytes).map_err(|_| RtdErrorKind::InvalidSignature {
