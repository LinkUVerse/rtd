...
use crate::{RTD_AUTHENTICATOR_STATE_OBJECT_ID, RTD_FRAMEWORK_ADDRESS, id::UID};
...
pub const RESOLVED_RTD_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> RtdResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&RTD_AUTHENTICATOR_STATE_OBJECT_ID);
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&RTD_AUTHENTICATOR_STATE_OBJECT_ID)
