...
use crate::gas_coin::MIST_PER_RTD;
use crate::id::{ID, UID};
...
///
/// 30 million RTD
pub const MIN_VALIDATOR_JOINING_STAKE_MIST: u64 = 30_000_000 * MIST_PER_RTD;
...
///
/// 20 million RTD
pub const VALIDATOR_LOW_STAKE_THRESHOLD_MIST: u64 = 20_000_000 * MIST_PER_RTD;
...
///
/// 15 million RTD
pub const VALIDATOR_VERY_LOW_STAKE_THRESHOLD_MIST: u64 = 15_000_000 * MIST_PER_RTD;
...
pub const STAKED_RTD_STRUCT_NAME: &IdentStr = ident_str!("StakedRtd");
...
impl StakedRtd {
    ...
    pub fn is_staked_rtd(s: &StructTag) -> bool {
        s.address == RTD_SYSTEM_ADDRESS
            && s.module.as_ident_str() == STAKING_POOL_MODULE_NAME
            && s.name.as_ident_str() == STAKED_RTD_STRUCT_NAME
...
impl TryFrom<&Object> for StakedRtd {
    type Error = RtdError;
    fn try_from(object: &Object) -> Result<Self, Self::Error> {
        match &object.data {
            Data::Move(o) => {
                if o.type_().is_staked_rtd() {
                    return bcs::from_bytes(o.contents()).map_err(|err| {
                        RtdErrorKind::TypeError {
                            error: format!("Unable to deserialize StakedRtd object: {:?}", err),
                        }
                        .into()
                    });
                }
        ...
        Err(RtdErrorKind::TypeError {
            error: format!("Object type is not a StakedRtd: {:?}", object),
        }
        .into())
    }
}
