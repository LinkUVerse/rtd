...
use crate::{
    base_types::{RtdAddress, VersionDigest},
...
impl AccumulatorAddress {
    pub fn new(address: RtdAddress, ty: TypeTag) -> Self {
        Self { address, ty }
    }
}
...
impl AccumulatorWriteV1 {
    pub fn merge(mut writes: Vec<Self>) -> Self {
        ...
        if linku_common::in_test_configuration() {
            ...
            AccumulatorValue::EventDigest(first_digests) => {
                ...
                for write in &writes[1..] {
                    if let AccumulatorValue::EventDigest(digests) = &write.value {
                        ...
                        linku_common::fatal!("mismatched accumulator value types for same object");
                    }
                }
                (
...
mod tests {
    ...
    fn test_accumulator_address() -> AccumulatorAddress {
        AccumulatorAddress::new(
            ...
            "0x2::balance::Balance<0x2::rtd::RTD>".parse().unwrap(),
        )
    }
    ...
    fn test_merge_mismatched_types() {
        let addr = RtdAddress::random_for_testing_only();
