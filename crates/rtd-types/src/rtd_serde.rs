...
pub struct RtdStructTag;

impl SerializeAs<StructTag> for RtdStructTag {
    fn serialize_as<S>(value: &StructTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let f = to_rtd_struct_tag_string(value).map_err(S::Error::custom)?;
        f.serialize(serializer)
    }
}

const RTD_ADDRESSES: [AccountAddress; 7] = [
    AccountAddress::ZERO,
    AccountAddress::ONE,
    RTD_FRAMEWORK_ADDRESS,
    RTD_SYSTEM_ADDRESS,
    DEEPBOOK_ADDRESS,
    RTD_SYSTEM_STATE_ADDRESS,
    RTD_CLOCK_ADDRESS,
];
/// Serialize StructTag as a string, retaining the leading zeros in the address.
pub fn to_rtd_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    let mut f = String::new();
    // trim leading zeros if address is in RTD_ADDRESSES
    let address = if RTD_ADDRESSES.contains(&value.address) {
    ...
    if let Some(first_ty) = value.type_params.first() {
        write!(f, "<")?;
        write!(f, "{}", to_rtd_type_tag_string(first_ty)?)?;
        for ty in value.type_params.iter().skip(1) {
            write!(f, ", {}", to_rtd_type_tag_string(ty)?)?;
        }
        write!(f, ">")?;
    }
    Ok(f)
...
fn to_rtd_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    match value {
        TypeTag::Vector(t) => Ok(format!("vector<{}>", to_rtd_type_tag_string(t)?)),
        TypeTag::Struct(s) => to_rtd_struct_tag_string(s),
        _ => Ok(value.to_string()),
    }
}

impl<'de> DeserializeAs<'de, StructTag> for RtdStructTag {
    fn deserialize_as<D>(deserializer: D) -> Result<StructTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_rtd_struct_tag(&s).map_err(D::Error::custom)
    }
}

pub struct RtdTypeTag;

impl SerializeAs<TypeTag> for RtdTypeTag {
    fn serialize_as<S>(value: &TypeTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s = to_rtd_type_tag_string(value).map_err(S::Error::custom)?;
        s.serialize(serializer)
    }
}

impl<'de> DeserializeAs<'de, TypeTag> for RtdTypeTag {
    fn deserialize_as<D>(deserializer: D) -> Result<TypeTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_rtd_type_tag(&s).map_err(D::Error::custom)
    }
}
...
pub(crate) struct RtdBitmap;

impl SerializeAs<roaring::RoaringBitmap> for RtdBitmap {
...
impl<'de> DeserializeAs<'de, roaring::RoaringBitmap> for RtdBitmap {
    fn deserialize_as<D>(deserializer: D) -> Result<roaring::RoaringBitmap, D::Error>
    where
        D: Deserializer<'de>,
    {
        let bytes: Vec<u8> = Bytes::deserialize_as(deserializer)?;
        deserialize_rtd_bitmap(&bytes).map_err(to_custom_deser_error::<'de, D, _>)
    }
}
