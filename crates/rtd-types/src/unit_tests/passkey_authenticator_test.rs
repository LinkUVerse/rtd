...
use crate::{
    base_types::{ObjectID, RtdAddress, dbg_addr},
...
async fn create_credential_and_sign_test_tx(
    ...
    // Derives its rtd address and make a test transaction with it as sender.
...
async fn test_passkey_fails_invalid_json() {
    ...
    let res: Result<PasskeyAuthenticator, RtdError> = raw.try_into();
    ...
    let res_2: Result<PasskeyAuthenticator, RtdError> = raw_2.try_into();
    assert!(res_2.is_err());
    ...
    let res_3: Result<PasskeyAuthenticator, RtdError> = raw_3.try_into();
    assert!(res_3.is_ok());
}
