...
use crate::RtdAddress;
use crate::crypto::{Signer, RtdKeyPair};
...
pub fn make_transaction_data(sender: RtdAddress) -> TransactionData {
...
pub fn make_transaction(sender: RtdAddress, kp: &RtdKeyPair) -> Transaction {
...
mod zk_login {
    ...
    pub fn load_test_vectors(path: &str) -> Vec<(RtdKeyPair, PublicKey, ZkLoginInputs)> {
        ...
        for test in test_datum {
            let kp = RtdKeyPair::decode(&test.kp).unwrap();
            let inputs =
    ...
    pub fn get_zklogin_user_address() -> RtdAddress {
        thread_local! {
            static USER_ADDRESS: RtdAddress = {
                ...
                RtdAddress::from_bytes(hasher.finalize().digest).unwrap()
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    fn get_zklogin_user_key() -> RtdKeyPair {
    ...
    pub fn get_legacy_zklogin_user_address() -> RtdAddress {
        thread_local! {
            static USER_ADDRESS: RtdAddress = {
                let inputs = get_inputs_with_bad_address_seed();
                RtdAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap())
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    pub fn sign_zklogin_personal_msg(data: PersonalMessage) -> (RtdAddress, GenericSignature) {
        let inputs = get_zklogin_inputs();
    ...
    pub fn sign_zklogin_tx_with_default_proof(
        data: TransactionData,
        legacy: bool,
    ) -> (RtdAddress, Transaction, GenericSignature) {
    ...
    pub fn keys() -> Vec<RtdKeyPair> {
        let mut seed = StdRng::from_seed([0; 32]);
        let kp1: RtdKeyPair = RtdKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        let kp2: RtdKeyPair = RtdKeyPair::Secp256k1(get_key_pair_from_rng(&mut seed).1);
        let kp3: RtdKeyPair = RtdKeyPair::Secp256r1(get_key_pair_from_rng(&mut seed).1);
        vec![kp1, kp2, kp3]
    }
    ...
    pub fn make_upgraded_multisig_tx() -> Transaction {
        ...
        .unwrap();
        let addr = RtdAddress::from(&multisig_pk);
        let tx = make_transaction(addr, &keys[0]);

        let msg = IntentMessage::new(Intent::rtd_transaction(), tx.transaction_data().clone());
