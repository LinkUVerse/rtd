...
use crate::{
    MoveTypeTagTrait, MoveTypeTagTraitGeneric, RTD_ACCUMULATOR_ROOT_OBJECT_ID,
    RTD_FRAMEWORK_ADDRESS,
};
use move_core_types::language_storage::{StructTag, TypeTag};

#[test]
fn test_rtd_balance_accumulator_field_recognition() {
    // Create a Field<Key<Balance<RTD>>, U128> type
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[rtd_balance]);
    ...
    assert!(move_type.is_rtd_balance_accumulator_field());
...
#[test]
fn test_non_rtd_balance_accumulator_field_recognition() {
    ...
    // Should be recognized as a balance accumulator field, but not RTD
    assert!(move_type.is_balance_accumulator_field());
    assert!(!move_type.is_rtd_balance_accumulator_field());
...
fn test_accumulator_storage_savings() {
    let owner = RtdAddress::random_for_testing_only();
    let balance = 1000u64;

    // Create the field struct tag for Field<Key<Balance<RTD>>, U128>
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[rtd_balance]);
    let u128_type = U128::get_type_tag();
    ...
    assert!(
        efficient_move_object
            .type_()
            .is_rtd_balance_accumulator_field()
    );
}
...
#[test]
fn test_rtd_balance_accumulator_metadata_field_recognition() {
    // Create a Field<MetadataKey<Balance<RTD>>, Metadata<Balance<RTD>>> type
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    let key_type = MetadataKey::get_type_tag(std::slice::from_ref(&rtd_balance));
    let metadata_type = AccumulatorMetadata::get_type_tag(&[rtd_balance]);
    ...
    // Should extract the correct balance type (which should be RTD)
    let balance_type = move_type
        .balance_accumulator_metadata_field_type_maybe()
        .expect("Should have balance type");
    assert_eq!(balance_type, GAS::type_tag());

    // Since the balance type is RTD, this should be a RtdBalanceAccumulatorMetadataField
    assert!(
        matches!(
            move_type.0,
            crate::base_types::MoveObjectType_::RtdBalanceAccumulatorMetadataField
        ),
        "Should be RtdBalanceAccumulatorMetadataField variant"
    );
}

#[test]
fn test_non_rtd_balance_accumulator_metadata_field_recognition() {
    // Create a custom token type
    let custom_token = TypeTag::Struct(Box::new(StructTag {
        address: RTD_FRAMEWORK_ADDRESS,
        module: "test".parse().unwrap(),
    ...
    // Should be recognized as a balance accumulator metadata field, but not RTD
    assert!(move_type.is_balance_accumulator_metadata_field());
    ...
    // Since the balance type is not RTD, this should be a BalanceAccumulatorMetadataField
    assert!(
        matches!(
            move_type.0,
...
fn test_accumulator_metadata_storage_savings() {
    // Create the field struct tag for Field<MetadataKey<Balance<RTD>>, Metadata<Balance<RTD>>>
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    let key_type = MetadataKey::get_type_tag(std::slice::from_ref(&rtd_balance));
    let metadata_type = AccumulatorMetadata::get_type_tag(&[rtd_balance]);
    ...
    // Specifically verify it's the RTD variant
    assert!(
        matches!(
...
fn test_accumulator_field_wrong_value_type_not_u128() {
    // Field<Key<Balance<RTD>>, SomeOtherType> should NOT be recognized
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[rtd_balance]);
...
fn test_accumulator_field_key_with_multiple_type_params() {
    // Key<Balance<RTD>, ExtraParam> should not be recognized
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    ...
    let key_type = TypeTag::Struct(Box::new(StructTag {
        ...
        module: "accumulator".parse().unwrap(),
        name: "Key".parse().unwrap(),
        type_params: vec![rtd_balance, extra_param],
    }));
    let u128_type = U128::get_type_tag();
...
fn test_accumulator_field_with_nested_balance_type() {
    // Balance<Balance<RTD>> - nested balance should still work correctly
    ...
    // Should recognize as accumulator field with Balance<Balance<RTD>> as the extracted type
    assert!(move_type.is_balance_accumulator_field());
    let extracted = move_type.balance_accumulator_field_type_maybe().unwrap();
    // The extracted type should be Balance<RTD>, which is the inner type of Balance<Balance<RTD>>
...
fn test_metadata_field_mismatched_key_value_types() {
    // Field<MetadataKey<Balance<RTD>>, Metadata<Balance<CustomToken>>> should NOT be recognized
...
fn test_metadata_field_wrong_key_module() {
    ...
    let wrong_key_type = TypeTag::Struct(Box::new(StructTag {
        ...
        type_params: vec![rtd_balance.clone()],
    }));
    let metadata_type = AccumulatorMetadata::get_type_tag(&[rtd_balance]);
...
#[test]
fn test_is_method_rtd_balance_accumulator_field() {
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    let key_type = AccumulatorKey::get_type_tag(&[rtd_balance]);
    let u128_type = U128::get_type_tag();
    let field_type = DynamicFieldInfo::dynamic_field_type(key_type, u128_type);
...
#[test]
fn test_is_method_non_rtd_balance_accumulator_field() {
    let custom_token = TypeTag::Struct(Box::new(StructTag {
        address: RTD_FRAMEWORK_ADDRESS,
        module: "test".parse().unwrap(),
        name: "CustomToken".parse().unwrap(),
    ...
    // Should not match RTD accumulator field
    let rtd_balance = Balance::type_tag(GAS::type_tag());
    let rtd_key_type = AccumulatorKey::get_type_tag(&[rtd_balance]);
    let rtd_field_type = DynamicFieldInfo::dynamic_field_type(rtd_key_type, U128::get_type_tag());
    assert!(!move_type.is(&rtd_field_type));
