...
use crate::{
    base_types::RtdAddress, signature::GenericSignature, zk_login_util::DEFAULT_JWK_BYTES,
};
...
fn test_serde_zk_login_signature() {
    ...
    let addr: RtdAddress = (&authenticator).try_into().unwrap();
...
fn test_serde_zk_public_identifier() {
    ...
    assert_eq!(
        RtdAddress::try_from_unpadded(&modified_inputs).unwrap(),
        RtdAddress::from(&pk1)
    );
    ...
    assert_eq!(
        RtdAddress::try_from_padded(&modified_inputs).unwrap(),
        RtdAddress::from(&pk2)
    );
