...
pub enum UserInputError {
    ...
    #[error(
        "RTD payment transactions use first input coin for gas payment, but found a different gas object"
    )]
...
pub enum RtdObjectResponseError {
    ...
    // TODO: also integrate RtdPastObjectResponse (VersionNotFound,  VersionTooHigh)
}

/// Custom error type for Rtd.
#[derive(Eq, PartialEq, Clone, Serialize, Deserialize, Error, Hash)]
#[error(transparent)]
pub struct RtdError(#[from] pub Box<RtdErrorKind>);

/// Custom error type for Rtd.
#[derive(
    Eq, PartialEq, Clone, Debug, Serialize, Deserialize, Error, Hash, AsRefStr, IntoStaticStr,
)]
pub enum RtdErrorKind {
    ...
    RtdObjectResponseError { error: RtdObjectResponseError },
    ...
    TooManyIncorrectAuthorities {
        errors: Vec<(AuthorityName, RtdError)>,
        action: String,
    },
    ...
    #[error("Failed to execute handle_consensus_transaction on Rtd: {0}")]
...
impl From<RtdErrorKind> for RtdError {
    fn from(error: RtdErrorKind) -> Self {
        RtdError(Box::new(error))
    }
}

impl std::ops::Deref for RtdError {
    type Target = RtdErrorKind;
...
impl From<rtd_protocol_config::Error> for RtdError {
    fn from(error: rtd_protocol_config::Error) -> Self {
        RtdErrorKind::WrongMessageVersion { error: error.0 }.into()
    }
}

impl From<ExecutionError> for RtdError {
    fn from(error: ExecutionError) -> Self {
        RtdErrorKind::ExecutionError(error.to_string()).into()
    }
}

impl From<Status> for RtdError {
    fn from(status: Status) -> Self {
        if status.message() == "Too many requests" {
            return RtdErrorKind::TooManyRequests.into();
        }

        let result = bcs::from_bytes::<RtdError>(status.details());
        if let Ok(rtd_error) = result {
            rtd_error
        } else {
            RtdErrorKind::RpcError(
...
impl From<TypedStoreError> for RtdError {
    fn from(e: TypedStoreError) -> Self {
        RtdErrorKind::Storage(e.to_string()).into()
    }
}

impl From<crate::storage::error::Error> for RtdError {
    fn from(e: crate::storage::error::Error) -> Self {
        RtdErrorKind::Storage(e.to_string()).into()
    }
}

impl From<RtdErrorKind> for Status {
    fn from(error: RtdErrorKind) -> Self {
...
impl From<RtdError> for Status {
    fn from(error: RtdError) -> Self {
        Status::from(error.into_inner())
    }
}

impl From<ExecutionErrorKind> for RtdError {
    fn from(kind: ExecutionErrorKind) -> Self {
        ExecutionError::from_kind(kind).into()
    }
}

impl From<&str> for RtdError {
...
impl From<String> for RtdError {
    fn from(error: String) -> Self {
        RtdErrorKind::GenericAuthorityError { error }.into()
    }
}

impl TryFrom<RtdErrorKind> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: RtdErrorKind) -> Result<Self, Self::Error> {
        match err {
            RtdErrorKind::UserInputError { error } => Ok(error),
            other => anyhow::bail!("error {:?} is not UserInputError", other),
        }
    }
}

impl TryFrom<RtdError> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: RtdError) -> Result<Self, Self::Error> {
        err.into_inner().try_into()
    }
}

impl From<UserInputError> for RtdError {
    fn from(error: UserInputError) -> Self {
        RtdErrorKind::UserInputError { error }.into()
    }
}

impl From<RtdObjectResponseError> for RtdError {
    fn from(error: RtdObjectResponseError) -> Self {
        RtdErrorKind::RtdObjectResponseError { error }.into()
    }
}

impl PartialEq<RtdErrorKind> for RtdError {
    fn eq(&self, other: &RtdErrorKind) -> bool {
        &*self.0 == other
    }
}

impl PartialEq<RtdError> for RtdErrorKind {
    fn eq(&self, other: &RtdError) -> bool {
        self == &*other.0
    }
}

impl RtdError {
    pub fn as_inner(&self) -> &RtdErrorKind {
        &self.0
    }

    pub fn into_inner(self) -> RtdErrorKind {
        *self.0
    }
}
...
impl RtdErrorKind {
    ...
    pub fn to_variant_name(&self) -> &'static str {
        match &self {
            RtdErrorKind::UserInputError { error } => error.into(),
            _ => self.into(),
        }
    }
    ...
    pub fn individual_error_indicates_epoch_change(&self) -> bool {
        matches!(
            self,
            RtdErrorKind::ValidatorHaltedAtEpochEnd | RtdErrorKind::MissingCommitteeAtEpoch(_)
        )
    }
    ...
    pub fn is_retryable(&self) -> (bool, bool) {
        let retryable = match self {
            // Network error
            RtdErrorKind::RpcError { .. } => true,

            // Reconfig error
            RtdErrorKind::ValidatorHaltedAtEpochEnd => true,
            RtdErrorKind::MissingCommitteeAtEpoch(..) => true,
            RtdErrorKind::WrongEpoch { .. } => true,
            RtdErrorKind::EpochEnded(..) => true,

            RtdErrorKind::UserInputError { error } => {
                match error {
            ...
            RtdErrorKind::PotentiallyTemporarilyInvalidSignature { .. } => true,

            // Overload errors
            RtdErrorKind::TooManyTransactionsPendingExecution { .. } => true,
            ...
            RtdErrorKind::ByzantineAuthoritySuspicion { .. } => false,
            RtdErrorKind::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. } => false,
            RtdErrorKind::TxAlreadyFinalizedWithDifferentUserSigs => false,
            RtdErrorKind::FailedToVerifyTxCertWithExecutedEffects { .. } => false,
            RtdErrorKind::ObjectLockConflict { .. } => false,
            ...
            RtdErrorKind::TooManyRequests => false,
    ...
    pub fn is_overload(&self) -> bool {
        matches!(
            self,
            RtdErrorKind::TooManyTransactionsPendingExecution { .. }
    ...
    pub fn is_retryable_overload(&self) -> bool {
        matches!(self, RtdErrorKind::ValidatorOverloadedRetryAfter { .. })
    }

    pub fn retry_after_secs(&self) -> u64 {
        match self {
            RtdErrorKind::ValidatorOverloadedRetryAfter { retry_after_secs } => *retry_after_secs,
            _ => 0,
        }
    }

    /// Categorizes RtdError into ErrorCategory.
    pub fn categorize(&self) -> ErrorCategory {
        match self {
            ...
            | RtdErrorKind::SignerSignatureAbsent { .. }
            | RtdErrorKind::SignerSignatureNumberMismatch { .. }
            | RtdErrorKind::IncorrectSigner { .. }
            | RtdErrorKind::UnknownSigner { .. }
            | RtdErrorKind::TransactionExpired => ErrorCategory::InvalidTransaction,

            RtdErrorKind::ObjectLockConflict { .. } => ErrorCategory::LockConflict,

            RtdErrorKind::Unknown { .. }
            | RtdErrorKind::GrpcMessageSerializeError { .. }
            | RtdErrorKind::GrpcMessageDeserializeError { .. }
            | RtdErrorKind::ByzantineAuthoritySuspicion { .. }
            | RtdErrorKind::InvalidTxKindInSoftBundle
            | RtdErrorKind::UnsupportedFeatureError { .. }
            | RtdErrorKind::InvalidRequest { .. } => ErrorCategory::Internal,
            ...
            RtdErrorKind::TimeoutError => ErrorCategory::Unavailable,
...
impl Ord for RtdError {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        Ord::cmp(self.as_ref(), other.as_ref())
    }
}

impl PartialOrd for RtdError {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl std::fmt::Debug for RtdError {
