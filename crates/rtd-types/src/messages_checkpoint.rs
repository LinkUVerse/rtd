...
impl CheckpointArtifact {
    pub fn digest(&self) -> RtdResult<Digest> {
        match self {
            Self::ObjectStates(object_states) => {
                ...
                .map_err(|e| RtdErrorKind::GenericAuthorityError {
...
impl CheckpointArtifacts {
    ...
    pub fn add_artifact(&mut self, artifact: CheckpointArtifact) -> RtdResult<()> {
        if self
            .artifacts
            .iter()
    ...
    pub fn object_states(&self) -> RtdResult<&BTreeMap<ObjectID, (SequenceNumber, ObjectDigest)>> {
        self.artifacts
            .iter()
    ...
    pub fn digest(&self) -> RtdResult<CheckpointArtifactsDigest> {
...
impl CheckpointSummary {
    ...
    pub fn verify_epoch(&self, epoch: EpochId) -> RtdResult {
        fp_ensure!(
            self.epoch == epoch,
    ...
    pub fn report_checkpoint_age(&self, metrics: &Histogram, metrics_deprecated: &LinkUHistogram) {
        SystemTime::now()
    ...
    pub fn checkpoint_artifacts_digest(&self) -> RtdResult<&CheckpointArtifactsDigest> {
...
impl CertifiedCheckpointSummary {
    pub fn verify_authority_signatures(&self, committee: &Committee) -> RtdResult {
    ...
    pub fn try_into_verified(self, committee: &Committee) -> RtdResult<VerifiedCheckpoint> {
    ...
    pub fn verify_with_contents(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                RtdErrorKind::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, received content digest {:?}, received {} transactions", self.data(), content_digest, contents.size())}.into()
            );
        }
...
impl SignedCheckpointSummary {
    ...
    pub fn try_into_verified(
        self,
        committee: &Committee,
    ) -> RtdResult<VerifiedEnvelope<CheckpointSummary, AuthoritySignInfo>> {
        self.verify_authority_signatures(committee)?;
...
impl CheckpointSignatureMessage {
    pub fn verify(&self, committee: &Committee) -> RtdResult {
