...
use crate::error::{
    ExecutionError, ExecutionErrorKind, RtdErrorKind, UserInputError, UserInputResult,
};
use crate::error::{RtdError, RtdResult};
...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, RtdError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, RtdError> {
    ...
    /// Get the total amount of RTD embedded in `self`. Intended for testing purposes
    pub fn get_total_rtd(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, RtdError> {
        if self.type_.is_gas_coin() {
            let balance = self.get_coin_value_unsafe();
            Ok(balance)
        } else if self.type_.coin_type_maybe().is_some() {
            // It's a coin, but its not RTD
            Ok(0)
        } else if self.type_.is_rtd_balance_accumulator_field() {
            ...
            // anywhere, which is 10B for RTD.
            assert!(
                v.value <= u64::MAX as u128,
                "RTD balance cannot exceed u64::MAX"
            );
            Ok(v.value as u64)
        } else {
...
pub enum Owner {
    ...
    /// The object ID is converted to RtdAddress as RtdAddress is universal.
    ObjectOwner(RtdAddress),
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> RtdResult<RtdAddress> {
        match self {
            Self::AddressOwner(address) => Ok(*address),
            Self::Shared { .. }
            | Self::Immutable
            | Self::ObjectOwner(_)
            | Self::ConsensusAddressOwner { .. } => Err(RtdErrorKind::UnexpectedOwnerType.into()),
        }
    }
    ...
    // type is RtdAddress.
    pub fn get_owner_address(&self) -> RtdResult<RtdAddress> {
        match self {
            Self::AddressOwner(address)
            | Self::ObjectOwner(address)
            | Self::ConsensusAddressOwner { owner: address, .. } => Ok(*address),
            Self::Shared { .. } | Self::Immutable => Err(RtdErrorKind::UnexpectedOwnerType.into()),
        }
    }
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: RtdAddress = (*other).into();
        match self {
...
fn is_object_debug_verbose() -> bool {
    static RTD_OBJECT_DEBUG_VERBOSE: Lazy<bool> =
        Lazy::new(|| std::env::var("RTD_OBJECT_DEBUG_VERBOSE").is_ok());
    *RTD_OBJECT_DEBUG_VERBOSE
}
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<RtdAddress> {
    ...
    // context: https://github.com/LinkUVerse/rtd/pull/10679#discussion_r1165877816
    ...
    pub fn transfer(&mut self, new_owner: RtdAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, RtdError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> RtdResult<TypeTag> {
        let move_struct = self
            .data
            .struct_tag()
            .ok_or_else(|| RtdErrorKind::TypeError {
                error: "Object must be a Move object".to_owned(),
            })?;
        fp_ensure!(
            move_struct.type_params.len() == 1,
            RtdErrorKind::TypeError {
...
impl Object {
    /// Get the total amount of RTD embedded in `self`, including both Move objects and the storage rebate
    pub fn get_total_rtd(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, RtdError> {
        Ok(self.storage_rebate
            + match &self.data {
                Data::Move(m) => m.get_total_rtd(layout_resolver)?,
                Data::Package(_) => 0,
            })
    }
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: RtdAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: RtdAddress) -> Self {
    ...
    pub fn with_owner_for_testing(owner: RtdAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: RtdAddress) -> Self {
...
mod tests {
    ...
    use crate::{
        base_types::{ObjectID, RtdAddress, TransactionDigest},
        gas_coin::GasCoin,
    };
    ...
    fn test_object_digest_and_serialized_format() {
        ...
        let o = Object::new_move(
            g,
            Owner::AddressOwner(RtdAddress::ZERO),
