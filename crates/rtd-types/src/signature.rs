...
use crate::{base_types::RtdAddress, crypto::Signature, error::RtdResult, multisig::MultiSig};
...
impl GenericSignature {
    ...
    /// Parse [enum CompressedSignature] from trait RtdSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to combine partial signature into a MultiSig public key.
    pub fn to_compressed(&self) -> Result<CompressedSignature, RtdError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    ...
                    _ => Err(RtdErrorKind::UnsupportedFeatureError {
    ...
    /// Parse [struct PublicKey] from trait RtdSignature `flag || sig || pk`.
    /// This is useful for the MultiSig to construct the bitmap in [struct MultiPublicKey].
    pub fn to_public_key(&self) -> Result<PublicKey, RtdError> {
        match self {
            GenericSignature::Signature(s) => {
                ...
                match s.scheme() {
                    SignatureScheme::ED25519 => Ok(PublicKey::Ed25519(
                        (&Ed25519PublicKey::from_bytes(bytes).map_err(|_| {
                            RtdErrorKind::KeyConversionError("Cannot parse ed25519 pk".to_string())
                        })?)
                            .into(),
                    )),
...
impl AuthenticatorTrait for Signature {
    fn verify_user_authenticator_epoch(&self, _: EpochId, _: Option<EpochId>) -> RtdResult {
        Ok(())
    }
