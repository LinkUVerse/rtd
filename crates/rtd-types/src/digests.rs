...
impl TryFrom<Vec<u8>> for Digest {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self, RtdError> {
        let bytes: [u8; 32] =
            <[u8; 32]>::try_from(&bytes[..]).map_err(|_| RtdErrorKind::InvalidDigestLength {
                expected: 32,
                actual: bytes.len(),
            })?;
...
pub const RTD_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME: &str =
    "RTD_PROTOCOL_CONFIG_CHAIN_OVERRIDE";

static RTD_PROTOCOL_CONFIG_CHAIN_OVERRIDE: Lazy<Option<Chain>> = Lazy::new(|| {
    if let Ok(s) = env::var(RTD_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME) {
        info!("RTD_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {:?}", s);
        match s.as_str() {
            "mainnet" => Some(Chain::Mainnet),
            "testnet" => Some(Chain::Testnet),
            "" => None,
            _ => panic!("unrecognized RTD_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {s:?}"),
        }
    } else {
        None
    }
...
impl TryFrom<&[u8]> for TransactionDigest {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, crate::error::RtdError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::error::RtdErrorKind::InvalidTransactionDigest)?;
        Ok(Self::new(arr))
    }
}
...
impl CheckpointArtifactsDigest {
    ...
    pub fn from_artifact_digests(digests: Vec<Digest>) -> RtdResult<Self> {
        let bytes =
            bcs::to_bytes(&digests).map_err(|e| RtdError::from(format!("BCS error: {}", e)))?;
