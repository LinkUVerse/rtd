...
use crate::base_types::{MoveObjectType, ObjectDigest, RtdAddress};
...
/// Rust version of the Move rtd::dynamic_field::Field type
...
/// Rust version of the Move rtd::dynamic_object_field::Wrapper type
...
pub struct DynamicFieldName {
    ...
    #[serde_as(as = "Readable<RtdTypeTag, _>")]
    pub type_: TypeTag,
...
impl DynamicFieldInfo {
    pub fn is_dynamic_field(tag: &StructTag) -> bool {
        tag.address == RTD_FRAMEWORK_ADDRESS
    ...
    pub fn try_extract_field_name(tag: &StructTag, type_: &DynamicFieldType) -> RtdResult<TypeTag> {
        match (type_, tag.type_params.first()) {
            ...
            (DynamicFieldType::DynamicObject, Some(TypeTag::Struct(s))) => Ok(s
                .type_params
                .first()
                .ok_or_else(|| RtdErrorKind::ObjectDeserializationError {
    ...
    pub fn try_extract_field_value(tag: &StructTag) -> RtdResult<TypeTag> {
        match tag.type_params.last() {
            Some(value_type) => Ok(value_type.clone()),
            None => Err(RtdErrorKind::ObjectDeserializationError {
    ...
    pub fn parse_move_object(
    ...
    ) -> RtdResult<(MoveValue, DynamicFieldType, ObjectID)> {
        let name = extract_field_from_move_struct(move_struct, "name").ok_or_else(|| {
            RtdErrorKind::ObjectDeserializationError {
                error: "Cannot extract [name] field from rtd::dynamic_field::Field".to_string(),
            }
        })?;

        let value = extract_field_from_move_struct(move_struct, "value").ok_or_else(|| {
            RtdErrorKind::ObjectDeserializationError {
                error: "Cannot extract [value] field from rtd::dynamic_field::Field".to_string(),
            }
        })?;
        ...
        Ok(if is_dynamic_object(move_struct) {
            ...
            .ok_or_else(|| RtdErrorKind::ObjectDeserializationError {
                error: "Cannot extract [name] field from rtd::dynamic_object_field::Wrapper."
                    .to_string(),
            })?;
            ...
            let object_id = extract_object_id(move_struct).ok_or_else(|| {
                RtdErrorKind::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        rtd::dynamic_field::Field, {move_struct:?}",
                    ),
                }
            })?;
...
pub fn serialize_dynamic_field<K, V>(id: &UID, name: &K, value: V) -> Result<Vec<u8>, RtdError>
where
    K: Serialize + Clone,
    V: Serialize,
{
    ...
    K: Serialize + fmt::Debug,
{
    ...
    ) -> Result<BoundedDynamicFieldID<K>, RtdError> {
        let id = self.object_id()?;
    ...
    pub fn into_field<V>(self, value: V) -> Result<DynamicField<K, V>, RtdError>
    where
    ...
    V: Serialize,
{
    ...
    pub fn into_move_object_unsafe_for_testing(
        ...
        let field_bytes =
            bcs::to_bytes(&field).map_err(|e| RtdErrorKind::ObjectSerializationError {
                error: e.to_string(),
            })?;
        Ok(unsafe {
    ...
    K: Serialize + std::fmt::Debug,
{
    ...
    pub fn expect_object(
        ...
        key: &K,
        object_store: &dyn ObjectStore,
    ) -> Result<DynamicFieldObject<K>, RtdError> {
        let parent = self.0;
        let id = self.1;
    ...
    K: Serialize + DeserializeOwned,
{
    ...
    pub fn load_value<V>(self) -> Result<V, RtdError>
    where
        V: Serialize + DeserializeOwned,
    {
    ...
    /// Deserialize the field value from the object. Requires that the value type is known.
    pub fn load_field<V>(self) -> Result<Field<K, V>, RtdError>
    where
        V: Serialize + DeserializeOwned,
    {
        let object = self.0;
        ...
        bcs::from_bytes::<Field<K, V>>(move_object.contents())
            .map_err(|err| RtdErrorKind::DynamicFieldReadError(err.to_string()).into())
    }
}
