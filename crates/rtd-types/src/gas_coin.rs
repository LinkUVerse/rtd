...
/// The number of Mist per Rtd token
pub const MIST_PER_RTD: u64 = 1_000_000_000;

/// Total supply denominated in Rtd
...
pub const GAS_MODULE_NAME: &IdentStr = ident_str!("rtd");
pub const GAS_STRUCT_NAME: &IdentStr = ident_str!("RTD");
...
mod checked {
    ...
    /// Rust version of the Move rtd::coin::Coin<Rtd::rtd::RTD> type
    ...
    impl GasCoin {
        ...
        /// Return `true` if `s` is the type of a gas coin (i.e., 0x2::coin::Coin<0x2::rtd::RTD>)
        pub fn is_gas_coin(s: &StructTag) -> bool {
            Coin::is_coin(s) && s.type_params.len() == 1 && GAS::is_gas_type(&s.type_params[0])
        }

        /// Return `true` if `s` is the type of a gas balance (i.e., 0x2::balance::Balance<0x2::rtd::RTD>)
