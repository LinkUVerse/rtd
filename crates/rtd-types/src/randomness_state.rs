...
use crate::{
    RTD_FRAMEWORK_ADDRESS, RTD_RANDOMNESS_STATE_OBJECT_ID, error::RtdResult, object::Owner,
    storage::ObjectStore,
};
...
pub const RESOLVED_RTD_RANDOMNESS_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
pub fn is_mutable_random(view: &CompiledModule, s: &SignatureToken) -> bool {
    match s {
        ...
        SignatureToken::Datatype(idx) => {
            resolve_struct(view, *idx) == RESOLVED_RTD_RANDOMNESS_STATE
        }
        _ => false,
    }
}
