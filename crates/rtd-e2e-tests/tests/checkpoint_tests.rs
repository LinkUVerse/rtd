...
async fn test_checkpoint_split_brain() {
    #[cfg(msim)]
    {
        ...
        // loss of liveness
        use rtd_core::authority::{CheckpointTimeoutConfig, init_checkpoint_timeout_config};
...
async fn test_checkpoint_contents_v2_alias_versions() {
    ...
    let test_cluster = TestClusterBuilder::new()
        ...
        .with_state_sync_config(rtd_config::p2p::StateSyncConfig {
            ...
            &TestTransactionBuilder::new(account, gas_objects[0], gas_price)
                .move_call(
                    ...
                    vec![CallArg::Object(ObjectArg::SharedObject {
                        id: RTD_ADDRESS_ALIAS_STATE_OBJECT_ID,
    ...
    let transfer_tx = {
        ...
        ptb.input(CallArg::Object(ObjectArg::SharedObject {
            id: RTD_ADDRESS_ALIAS_STATE_OBJECT_ID,
            initial_shared_version: address_alias_state_initial_shared_version,
            mutability: rtd_types::transaction::SharedObjectMutability::Immutable,
        }))
        .unwrap();
        let recipient = ptb
    ...
    let mut client = test_cluster
        ...
        .unwrap();
    let request = rtd_types::messages_grpc::RawSubmitTxRequest {
        transactions: vec![
            bcs::to_bytes(&enable_tx).unwrap().into(),
            bcs::to_bytes(&transfer_tx).unwrap().into(),
        ],
        submit_type: rtd_types::messages_grpc::SubmitTxType::SoftBundle.into(),
    };
    let result = client
    ...
    let post_enable_tx = test_cluster
        ...
        .sign_transaction(
            &TestTransactionBuilder::new(account, gas_objects[2], gas_price)
                .transfer_rtd(None, account)
                .build(),
        )
        .await;
    let post_enable_digest = *post_enable_tx.digest();
    let result = client
        .submit_transaction(rtd_types::messages_grpc::RawSubmitTxRequest {
            transactions: vec![bcs::to_bytes(&post_enable_tx).unwrap().into()],
            submit_type: rtd_types::messages_grpc::SubmitTxType::Default.into(),
        })
        .await
        .unwrap()
        .into_inner();
