...
async fn test_sponsored_address_balance_storage_rebates() {
    ...
    let sponsor_sig = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &create_txn, Intent::rtd_transaction())
        .await
        .unwrap();
    ...
    let sponsor_delete_sig = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &delete_txn, Intent::rtd_transaction())
        .await
        .unwrap();
...
fn create_withdraw_balance_transaction(
    ...
    let split_withdraw_arg = builder.programmable_move_call(
        ...
        vec!["0x2::balance::Balance<0x2::rtd::RTD>".parse().unwrap()],
...
fn create_address_balance_transaction_with_expiration(
    ...
    use rtd_types::transaction::{GasData, TransactionDataV1, TransactionExpiration};
...
async fn test_sponsor_insufficient_balance_charges_zero_gas() {
    ...
    let sponsor_sig1 = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx1, Intent::rtd_transaction())
        .await
        .unwrap();
    ...
    let sponsor_sig2 = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx2, Intent::rtd_transaction())
        .await
        .unwrap();
...
async fn test_address_balance_large_rebate() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let object_arg = builder
        .obj(rtd_types::transaction::ObjectArg::ImmOrOwnedObject(
            created_object_ref,
        ))
        .unwrap();
    builder.programmable_move_call(
...
async fn test_sponsored_address_balance_storage_oog() {
    ...
    let sponsor_sig = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &tx, Intent::rtd_transaction())
        .await
        .unwrap();
