...
use rtd_rpc::proto::rtd::rpc::v2::Command;
use rtd_rpc::proto::rtd::rpc::v2::ExecuteTransactionRequest;
use rtd_rpc::proto::rtd::rpc::v2::GasPayment;
...
fn proto_to_response(
    proto: rtd_rpc::proto::rtd::rpc::v2::SimulateTransactionResponse,
) -> (
    rtd_types::transaction::TransactionData,
...
async fn resolve_transaction_transfer_with_sponsor() {
    ...
    let sender_sig = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sender, &transaction, Intent::rtd_transaction())
        .await
        .unwrap();
    let sponsor_sig = test_cluster
        ...
        .config
        .keystore
        .sign_secure(&sponsor, &transaction, Intent::rtd_transaction())
        .await
        .unwrap();

    let signed_transaction =
        rtd_types::transaction::Transaction::from_data(transaction, vec![sender_sig, sponsor_sig]);
    let effects = client
        .execute_transaction(&signed_transaction)
        .await
        .unwrap()
...
async fn resolve_transaction_mutable_shared_object() {
    ...
    let validator_address = test_cluster.swarm.config().validator_configs()[0].rtd_address();
    ...
    unresolved_transaction.kind = Some(TransactionKind::from({
        ...
        ptb.commands = vec![Command::from({
            ...
            message.module = Some("rtd_system".to_owned());
...
async fn resolve_transaction_insufficient_gas() {
    ...
    unresolved_transaction.sender = Some(RtdAddress::random_for_testing_only().to_string()); // random account with no
...
async fn resolve_transaction_insufficient_gas_with_payment_objects() {
    ...
    let split_result = builder.command(RtdCommand::SplitCoins(coin_arg, vec![amt_arg]));

    let split_coin = match split_result {
        rtd_types::transaction::Argument::Result(idx) => {
            rtd_types::transaction::Argument::NestedResult(idx, 0)
        }
    ...
    let mut transaction = rtd_rpc::proto::rtd::rpc::v2::Transaction::default();
    ...
    let effects: rtd_types::effects::TransactionEffects =
...
async fn resolve_transaction_shared_object_with_generic_type_parameter() {
    ...
    let inputs = match transaction.kind() {
        rtd_types::transaction::TransactionKind::ProgrammableTransaction(ptb) => &ptb.inputs,
        _ => panic!("Expected ProgrammableTransaction"),
    };
    assert_eq!(inputs.len(), 2);

    match &inputs[0] {
        rtd_types::transaction::CallArg::Object(
            rtd_types::transaction::ObjectArg::SharedObject { id, mutability, .. },
        ) => {
            assert_eq!(*id, party_object.0);
            assert_eq!(
                mutability,
                &rtd_types::transaction::SharedObjectMutability::Mutable,
