...
use rtd_sdk_types::BalanceChange;
...
async fn execute_transaction_transfer() {
    ...
    let txn =
        make_transfer_rtd_transaction(&test_cluster.wallet, Some(address), Some(amount)).await;
    ...
    let gas_summary =
        rtd_sdk_types::GasCostSummary::try_from(&transaction.effects.unwrap().gas_used.unwrap())
            .unwrap();
    let gas = gas_summary.net_gas_usage();

    let coin_type = rtd_types::rtd_sdk_types_conversions::type_tag_core_to_sdk(
