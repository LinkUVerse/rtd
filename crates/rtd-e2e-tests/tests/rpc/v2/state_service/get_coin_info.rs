...
use rtd_rpc::proto::rtd::rpc::v2::coin_metadata::MetadataCapState;
use rtd_rpc::proto::rtd::rpc::v2::coin_treasury::SupplyState;
use rtd_rpc::proto::rtd::rpc::v2::regulated_coin_metadata::CoinRegulatedState;
use rtd_rpc::proto::rtd::rpc::v2::state_service_client::StateServiceClient;
use rtd_types::base_types::{ObjectID, RtdAddress};
use rtd_types::coin_registry::Currency;
...
// not been migrated.
#[sim_test]
async fn get_coin_info_rtd() {
    let test_cluster = TestClusterBuilder::new().build().await;

    let mut grpc_client = get_grpc_client(&test_cluster).await;

    let coin_type_sdk: TypeTag = "0x2::rtd::RTD".parse().unwrap();
    ...
    assert_eq!(metadata.symbol, Some("RTD".to_owned()));
    assert_eq!(metadata.name, Some("Rtd".to_string()));
    ...
    assert_eq!(
        ...
        Some(rtd_types::gas_coin::TOTAL_SUPPLY_MIST)
    );
    assert_eq!(
        treasury.supply_state,
        Some(SupplyState::Fixed as i32),
        "RTD should have Fixed supply state"
    );
...
async fn publish_non_otw_coin(
    ...
    let registry_obj_response = {
        ...
        ledger_client
            .get_object({
                ...
                request.object_id = Some(RTD_COIN_REGISTRY_OBJECT_ID.to_string());
    ...
    let registry_initial_version = registry_obj_response
        ...
        .and_then(|owner| {
            if owner.kind == Some(OwnerKind::Shared as i32) {
                owner
                    .version
                    .map(rtd_types::base_types::SequenceNumber::from)
            } else {
                None
            }
        })
    ...
    let registry_arg = ptb
        .obj(ObjectArg::SharedObject {
            id: RTD_COIN_REGISTRY_OBJECT_ID,
...
async fn finalize_registration(
    ...
    let registry_owned = grpc_client
        .list_owned_objects({
            ...
            request.owner = Some(RtdAddress::from(RTD_COIN_REGISTRY_OBJECT_ID).to_string());
        ...
        .find(|obj| {
            ...
            obj_type
                .and_then(|t| {
                    ...
                    match rtd_types::parse_rtd_struct_tag(t) {
                        Ok(struct_tag) => {
    ...
    let mut client = rtd_rpc::Client::new(test_cluster.rpc_url().to_owned()).unwrap();
