...
use rtd_rpc::proto::rtd::rpc::v2::changed_object::{IdOperation, OutputObjectState};
...
async fn test_filter_by_type() {
    ...
    // We start with some RTD coins
    assert!(!objects.is_empty());
    assert!(objects.iter().all(|o| o.object_type() == rtd));
    ...
    let treasury_cap_type =
        rtd_types::coin::TreasuryCap::type_(rtd_types::parse_rtd_struct_tag(&trusted).unwrap())
    ...
    assert_eq!(objects.iter().filter(|o| o.object_type() == rtd).count(), 5);
}
...
async fn test_reverse_sorted_coins_by_balance() {
    ...
    let address = rtd_types::base_types::RtdAddress::random_for_testing_only();
