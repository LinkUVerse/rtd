...
use rtd_rpc::Client;
use rtd_rpc::proto::rtd::rpc::v2::{ExecutedTransaction, GasCostSummary};
use rtd_rpc::proto::rtd::rpc::v2::{GetBalanceRequest, ListBalancesRequest};
...
const INITIAL_RTD_BALANCE: u64 = 150000000000000000;
...
async fn test_balance_apis() {
    ...
    verify_balances(
        &mut grpc_client,
        address,
        &[(RTD_COIN_TYPE, INITIAL_RTD_BALANCE)],
    )
    .await;
}
...
async fn test_balance_changes_on_transfer() {
    ...
    let txn = rtd_test_transaction_builder::make_transfer_rtd_transaction(
        &test_cluster.wallet,
        Some(receiver),
    ...
    verify_balances(
        ...
        &[(
            RTD_COIN_TYPE,
            INITIAL_RTD_BALANCE - transfer_amount - gas_used,
        )],
    )
    .await;
    ...
    verify_balances(
        &mut grpc_client,
        receiver,
        &[(RTD_COIN_TYPE, INITIAL_RTD_BALANCE + transfer_amount)],
    )
    .await;
}
...
async fn test_custom_coin_balance() {
    ...
    verify_balances(
        ...
        &[
            (
                RTD_COIN_TYPE,
                INITIAL_RTD_BALANCE - publish_gas_used - mint_gas_used,
            ),
    ...
    let rtd_client = test_cluster.rtd_client();
    ...
    verify_balances(
        ...
        &[
            (
                RTD_COIN_TYPE,
                INITIAL_RTD_BALANCE - publish_gas_used - mint_gas_used - transfer_gas_used,
            ),
...
async fn test_multiple_concurrent_balance_changes() {
    ...
    verify_balances(
        ...
        &[(
            RTD_COIN_TYPE,
            INITIAL_RTD_BALANCE - transfer_0_to_1 - gas_used_0,
        )],
    )
    .await;
    ...
    verify_balances(
        ...
        &[(
            RTD_COIN_TYPE,
            INITIAL_RTD_BALANCE + transfer_0_to_1 - transfer_1_to_2 + transfer_2_to_1 - gas_used_1,
        )],
    )
    .await;
    ...
    verify_balances(
        ...
        &[(
            RTD_COIN_TYPE,
            INITIAL_RTD_BALANCE + transfer_1_to_2 - transfer_2_to_1 - gas_used_2,
        )],
    )
    .await;
}
...
async fn test_fresh_address_with_no_coins() {
    ...
    let fresh_address = RtdAddress::random_for_testing_only();
    ...
    let response = grpc_client
        ...
        .get_balance({
            ...
            message.coin_type = Some(RTD_COIN_TYPE.to_string());
            message
        })
        .await
        .unwrap()
...
async fn test_invalid_requests() {
    ...
    request.coin_type = Some(RTD_COIN_TYPE.to_string());
    let result = grpc_client.state_client().get_balance(request).await;
...
async fn execute_transaction(
    ...
    txn: &rtd_types::transaction::Transaction,
...
/// Build a transaction that splits and transfers RTD
async fn build_split_and_transfer_transaction(
    ...
    let coins = rtd_client
        .coin_read_api()
        .get_coins(sender, Some(RTD_COIN_TYPE.to_string()), None, None)
        .await
        .unwrap();
...
async fn split_and_transfer_coin(
    ...
    recipient: RtdAddress,
    gas_object: (
        rtd_types::base_types::ObjectID,
        rtd_types::base_types::SequenceNumber,
        rtd_types::base_types::ObjectDigest,
    ),
    coin: (
        rtd_types::base_types::ObjectID,
        rtd_types::base_types::SequenceNumber,
