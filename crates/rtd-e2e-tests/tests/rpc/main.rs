...
async fn transfer_coin(context: &rtd_sdk::wallet_context::WalletContext) -> rtd_sdk_types::Digest {
    ...
    let txn = context
        .sign_transaction(
            &rtd_test_transaction_builder::TestTransactionBuilder::new(
                sender, gas_object, gas_price,
            )
            .transfer(
                rtd_types::base_types::FullObjectRef::from_fastpath_ref(object_to_send),
                receiver,
            )
            .build(),
        )
...
async fn stake_with_validator(cluster: &test_cluster::TestCluster) -> rtd_sdk_types::Digest {
    ...
    let validator_address = cluster.swarm.config().validator_configs()[0].rtd_address();
    let txn = context
        .sign_transaction(
            &rtd_test_transaction_builder::TestTransactionBuilder::new(
                sender, gas_object, gas_price,
