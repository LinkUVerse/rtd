...
async fn execute_tx(tx: Transaction, test_cluster: &TestCluster) -> RtdResult {
    test_cluster
...
async fn test_passkey_fails_wrong_author() {
    ...
    let response =
        create_credential_and_sign_test_tx(&test_cluster, Some(RtdAddress::ZERO), false, false)
            .await;
    let sig = GenericSignature::PasskeyAuthenticator(
        PasskeyAuthenticator::new_for_testing(
            response.authenticator_data,
