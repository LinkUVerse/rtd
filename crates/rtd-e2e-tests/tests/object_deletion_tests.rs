...
mod sim_only_tests {
    ...
    async fn object_pruning_test() {
        let test_cluster = TestClusterBuilder::new()
            ...
            .build()
            .await;
        let fullnode = &test_cluster.fullnode_handle.rtd_node;
    ...
    async fn publish_package_and_create_parent_object(
        ...
        let package_id = publish_package(
            ...
            PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("../rtd-surfer/tests/move_building_blocks"),
        )
        .await
        .0;
    ...
    async fn wait_until_checkpoint_pruned(node: &RtdNode, checkpoint: CheckpointSequenceNumber) {
        loop {
            if node
                .state()
