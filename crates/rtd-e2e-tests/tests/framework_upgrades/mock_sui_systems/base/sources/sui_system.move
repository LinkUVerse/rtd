...
use rtd_system::rtd_system_state_inner::{Self, RtdSystemStateInner};
...
fun load_inner_maybe_upgrade(self: &mut RtdSystemState): &mut RtdSystemStateInner {
    let version = self.version;
    // TODO: This is where we check the version and perform upgrade if necessary.

    let inner: &mut RtdSystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
