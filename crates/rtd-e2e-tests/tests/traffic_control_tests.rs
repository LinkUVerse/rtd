...
use rtd_types::traffic_control::TrafficControlReconfigParams;
use rtd_types::{
...
async fn test_fullnode_traffic_control_dry_run() -> Result<(), anyhow::Error> {
    ...
    let response: RtdTransactionBlockResponse = jsonrpc_client
        .request("rtd_executeTransactionBlock", params.clone())
        .await
        .unwrap();
    ...
    for _ in 0..txn_count {
        let response: Result<RtdTransactionBlockResponse, _> = jsonrpc_client
...
async fn test_validator_traffic_control_error_blocked() -> Result<(), anyhow::Error> {
    ...
    signatures.push(GenericSignature::Signature(
        rtd_types::crypto::Signature::Ed25519RtdSignature(Ed25519RtdSignature::default()),
    ));
