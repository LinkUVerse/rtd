...
//     let client = RtdClientBuilder::default().build(rpc_url).await?;
//     let txs = client
//         .read_api()
//         .get_transactions_stream(RtdTransactionBlockResponseQuery::default(), None, true)
//         .collect::<Vec<_>>()
//         .await;
...
//     RtdClientCommands::CreateExampleNFT {
//         name: None,
//         description: None,
//         url: None,
//         gas: None,
...
async fn test_coins_stream() -> Result<(), anyhow::Error> {
    ...
    let coins = client
        .coin_read_api()
        .get_coins_stream(address, Some(RTD_COIN_TYPE.to_string()))
        .collect::<Vec<_>>()
        .await;
