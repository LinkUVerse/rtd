...
use rtd_types::base_types::{FullObjectRef, RtdAddress};
...
async fn party_object_transfer_multi_certs() {
    ...
    rtd_macros::register_fail_point_async("transaction_execution_delay", move || async move {
        let delay = {
...
async fn party_object_grpc() {
    ...
    assert_eq!(new_owner.address, Some(RtdAddress::ZERO.to_string()));
...
async fn party_coin_grpc() {
    ...
    builder.programmable_move_call(
        ...
        vec!["0x2::coin::Coin<0x2::rtd::RTD>".parse().unwrap()],
...
async fn party_object_jsonrpc() {
    ...
    let object = client
        ...
        .get_object_with_options(
            object_id,
            rtd_json_rpc_types::RtdObjectDataOptions::new().with_owner(),
        )
        .await
        .unwrap()
        .data
    ...
    assert_eq!(new_owner_address, RtdAddress::ZERO);
