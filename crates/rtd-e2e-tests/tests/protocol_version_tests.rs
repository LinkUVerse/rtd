...
mod sim_only_tests {
    ...
    use rtd_types::base_types::{FullObjectID, FullObjectRef, ObjectID, ObjectRef};
    use rtd_types::effects::{TransactionEffects, TransactionEffectsAPI};
    use rtd_types::id::ID;
    use rtd_types::object::Owner;
    use rtd_types::rtd_system_state::{
        RTD_SYSTEM_STATE_SIM_TEST_DEEP_V2, RTD_SYSTEM_STATE_SIM_TEST_SHALLOW_V2,
        RTD_SYSTEM_STATE_SIM_TEST_V1, RtdSystemState, RtdSystemStateTrait,
        epoch_start_rtd_system_state::EpochStartSystemStateTrait, get_validator_from_table,
    };
    use rtd_types::supported_protocol_versions::SupportedProtocolVersions;
    ...
    enum BuildKind {
        /// Extend the RtdSystem package with a module. This is for tests that live in
        /// `/framework_upgrades/extend` folder.
        ExtendRtdSystem,
        /// In place builds happen when we want to build the rtd framework directly
        /// in its root folder.
        InPlace,
    ...
    impl From<&str> for BuildKind {
        fn from(value: &str) -> Self {
            ...
            // we want to extend the rtd system module with extra code
            if value.starts_with("extend/") {
                return Self::ExtendRtdSystem;
            }

            // If we are dealing with in-place framework, we need to build in place.
            if value.starts_with("../../../rtd-framework/") {
                return Self::InPlace;
            }
    ...
    async fn test_framework_add_struct_ability() {
        ...
        transfer_obj(&cluster, RtdAddress::ZERO, to_transfer0).await;
        wrap_obj(&cluster, to_wrap1.as_object_ref()).await;
        transfer_obj(&cluster, RtdAddress::ZERO, to_transfer1).await;
    }
    ...
    async fn test_new_framework_package() {
        ...
        let rtd_extra = ObjectID::from_single_byte(0x42);
        framework_injection::set_override(rtd_extra, fixture_modules("extra_package").await);
        ...
        // initializer
        let effects = get_framework_upgrade_effects(&cluster, &rtd_extra).await;
    ...
    async fn run_framework_upgrade(from: &str, to: &str) -> TestCluster {
        ...
        override_rtd_system_modules(to).await;
        TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([rtd_system_package_object(from).await])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn get_framework_upgrade_versions(
        ...
        let effects = get_framework_upgrade_effects(cluster, &RTD_SYSTEM_PACKAGE_ID).await;

        let modified_at = effects
            .modified_at_versions()
            .iter()
            .find_map(|(id, v)| (id == &RTD_SYSTEM_PACKAGE_ID).then_some(*v));

        let mutated_to = effects
            .mutated()
            .iter()
            .find_map(|((id, v, _), _)| (id == &RTD_SYSTEM_PACKAGE_ID).then_some(*v));
    ...
    async fn get_framework_upgrade_effects(
        ...
        let node_handle = &cluster.fullnode_handle.rtd_node;
    ...
    async fn test_framework_compatible_upgrade_no_protocol_version() {
        ...
        override_rtd_system_modules("extend/compatible").await;
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([rtd_system_package_object("extend/base").await])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, START,
            ))
            .build()
    ...
    async fn test_framework_upgrade_conflicting_versions() {
        ...
        // Gotta do both
        let base_modules = rtd_system_modules("extend/base").await;
        let compatible_modules = rtd_system_modules("extend/compatible").await;

        override_rtd_system_modules_cb(Box::new(move |name| {
            if name == first_name {
    ...
    async fn test_safe_mode_recovery() {
        ...
        override_rtd_system_modules("mock_rtd_systems/base").await;
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            // Overrides with a rtd system package that would abort during epoch change txn
            .with_objects([rtd_system_package_object("mock_rtd_systems/safe_mode").await])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn rtd_system_mock_smoke_test() {
        ...
        let test_cluster = TestClusterBuilder::new()
            ...
            .with_objects([rtd_system_package_object("mock_rtd_systems/base").await])
            .build()
            .await;
    ...
    async fn rtd_system_state_shallow_upgrade_test() {
        ...
        override_rtd_system_modules("mock_rtd_systems/shallow_upgrade").await;
        ...
        assert!(matches!(system_state, RtdSystemState::SimTestV1(_)));
        ...
        assert!(matches!(system_state, RtdSystemState::SimTestShallowV2(_)));
    }
    ...
    async fn rtd_system_state_deep_upgrade_test() {
        ...
        override_rtd_system_modules("mock_rtd_systems/deep_upgrade").await;
        ...
        if let RtdSystemState::SimTestV1(inner) = system_state {
        ...
        if let RtdSystemState::SimTestDeepV2(inner) = system_state {
            // Make sure we have 1 inactive validator for latter testing.
            assert_eq!(inner.validators.inactive_validators.size, 1);
            get_validator_from_table(
                test_cluster
    ...
    #[sim_test]
    async fn rtd_system_state_production_upgrade_test() {
        // Use this test to test a real rtd system state upgrade. To make this test work,
        // put the new rtd system in a new path and point to it in the override.
        ...
        override_rtd_system_modules("../../../rtd-framework/packages/rtd-system").await;
        // Wait for the upgrade to finish. After the upgrade, the new framework will be installed,
        // but the system state object hasn't been upgraded yet.
        let system_state = test_cluster.wait_for_epoch(Some(1)).await;
        assert_eq!(system_state.protocol_version(), FINISH);
        ...
        // at epoch boundary.
        let system_state = test_cluster.wait_for_epoch(Some(2)).await;
        if let RtdSystemState::V2(inner) = system_state {
            assert_eq!(inner.parameters.min_validator_count, 4);
        } else {
            unreachable!("Unexpected rtd system state version");
        }
    }
    ...
    async fn override_rtd_system_modules(path: &str) {
        framework_injection::set_override(RTD_SYSTEM_PACKAGE_ID, rtd_system_modules(path).await);
    }

    fn override_rtd_system_modules_cb(f: framework_injection::PackageUpgradeCallback) {
        framework_injection::set_override_cb(RTD_SYSTEM_PACKAGE_ID, f);
    }

    /// Get compiled modules for Rtd System, built from fixture `fixture` in the
    /// `framework_upgrades` directory.
    async fn rtd_system_modules(fixture: &str) -> Vec<CompiledModule> {
        fixture_package(fixture)
            .await
            .get_rtd_system_modules()
            .cloned()
            .collect()
    }

    /// Like `rtd_system_modules`, but package the modules in an `Object`.
    async fn rtd_system_package_object(fixture: &str) -> Object {
        Object::new_package(
            &rtd_system_modules(fixture).await,
            ...
            &[
                ...
                BuiltInFramework::get_package_by_id(&RTD_FRAMEWORK_PACKAGE_ID)
    ...
    async fn fixture_package(fixture: &str) -> CompiledPackage {
        ...
        let project_path = match build_kind {
            BuildKind::ExtendRtdSystem | BuildKind::TempDir => {
                let tempdir_path = tempdir.path();

                let rtd_packages_path = root.join("../rtd-framework/packages");

                // Copy the whole `rtd-framework/packages` into the tempdir under `/system-packages/*`
                copy_dir_all(&rtd_packages_path, &tempdir_path.join("system-packages")).unwrap();

                let project_path = tempdir_path.join("tmp-test");

                // Copy `rtd_system` into the project (we copy that first, before we paste the actual project).
                // The project lives in `/${fixture}/`, so we copy rtd-system move files into `/fixture/sources/`
                if build_kind == BuildKind::ExtendRtdSystem {
                    copy_dir_all(
                        &rtd_packages_path.join("rtd-system/sources"),
    ...
    // Copied from `rtd/tests/cli_tests.rs`
