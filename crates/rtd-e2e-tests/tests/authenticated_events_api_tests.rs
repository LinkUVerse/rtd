...
use rtd_light_client::mmr::apply_stream_updates;
use rtd_light_client::proof::base::{Proof, ProofContents, ProofTarget, ProofVerifier};
use rtd_light_client::proof::committee::extract_new_committee_info;
use rtd_light_client::proof::ocs::{OCSInclusionProof, OCSProof, OCSTarget};
...
use rtd_rpc::proto::rtd::rpc::v2::{Event, GetCheckpointRequest, GetEpochRequest};
use rtd_rpc_api::grpc::alpha::event_service_proto::event_service_client::EventServiceClient;
use rtd_rpc_api::grpc::alpha::event_service_proto::{
    AuthenticatedEvent, ListAuthenticatedEventsRequest,
};
use rtd_rpc_api::grpc::alpha::proof_service_proto::GetObjectInclusionProofRequest;
use rtd_rpc_api::grpc::alpha::proof_service_proto::proof_service_client::ProofServiceClient;
use rtd_sdk_types::ValidatorCommittee;
use rtd_types::accumulator_root as ar;
use rtd_types::accumulator_root::EventCommitment;
use rtd_types::base_types::{ObjectID, SequenceNumber, RtdAddress};
use rtd_types::committee::Committee;
use rtd_types::digests::{Digest, ObjectDigest};
use rtd_types::dynamic_field::{DynamicFieldKey, Field};
...
async fn publish_test_package(test_cluster: &TestCluster) -> ObjectID {
    ...
    let txn = test_cluster
        .wallet
        .sign_transaction(
            &rtd_test_transaction_builder::TestTransactionBuilder::new(sender, gas_object, 1000)
...
async fn emit_test_event(
    ...
    let tx_data = TransactionData::new(
        rtd_types::transaction::TransactionKind::ProgrammableTransaction(ptb.finish()),
        sender,
        gas_object,
        50_000_000_000,
        rgp,
...
async fn emit_multiple_test_events(
    ...
    start_value: u64,
    count: u64,
) -> rtd_json_rpc_types::RtdTransactionBlockResponse {
    let rgp = test_cluster.get_reference_gas_price().await;
    let mut ptb = ProgrammableTransactionBuilder::new();
...
async fn query_authenticated_events(
    ...
    rtd_rpc_api::grpc::alpha::event_service_proto::ListAuthenticatedEventsResponse,
    tonic::Status,
> {
...
async fn verify_events_with_stream_head(
    ...
    let stream_id = rtd_types::base_types::RtdAddress::from(package_id);
...
fn proto_object_ref_to_rtd_object_ref(
    object_ref_proto: &rtd_rpc::proto::rtd::rpc::v2::ObjectReference,
...
fn proto_ocs_inclusion_proof_to_light_client_proof(
    grpc_proof: &rtd_rpc_api::grpc::alpha::proof_service_proto::OcsInclusionProof,
...
fn convert_grpc_event_to_commitment(
    ...
    let package_id = rtd_types::base_types::ObjectID::from_hex_literal(package_id)
    ...
    let sender = rtd_types::base_types::RtdAddress::from_str(sender)
    ...
    let rtd_event = rtd_types::event::Event {
        package_id,
...
fn get_event_stream_head_object_id(
    stream_id: rtd_types::base_types::RtdAddress,
) -> Result<rtd_types::base_types::ObjectID, String> {
    ...
    let field_id = DynamicFieldKey(RTD_ACCUMULATOR_ROOT_OBJECT_ID, key, key_type_tag)
        .into_unbounded_id()
...
async fn get_committee_for_epoch_via_api(
    ...
    epoch: u64,
) -> Result<rtd_types::committee::Committee, String> {
    ...
    Ok(rtd_types::committee::Committee::from(sdk_committee))
}
...
async fn build_epoch_cache(
    ...
    for epoch in 0..current_epoch {
        ...
        let checkpoint: rtd_types::full_checkpoint_content::Checkpoint = (&proto_checkpoint)
            .try_into()
...
async fn verify_ocs_inclusion_proof(
    ...
    checkpoint_summary: &rtd_types::messages_checkpoint::CertifiedCheckpointSummary,
    object_ref_proto: &rtd_rpc::proto::rtd::rpc::v2::ObjectReference,
    grpc_proof: &rtd_rpc_api::grpc::alpha::proof_service_proto::OcsInclusionProof,
    checkpoint_seq: u64,
) -> Result<(), String> {
    let object_ref = proto_object_ref_to_rtd_object_ref(object_ref_proto)?;
...
async fn fetch_and_verify_event_stream_head(
    ...
    let checkpoint_data: rtd_types::full_checkpoint_content::Checkpoint = (&proto_checkpoint)
        .try_into()
...
async fn authenticated_events_backfill_test() {
    ...
    let rpc_config_without_indexing = rtd_config::RpcConfig {
...
async fn test_object_inclusion_proof_error_code() {
    ...
    let highest_checkpoint = test_cluster.fullnode_handle.rtd_node.with(|node| {
        node.state()
...
async fn authenticated_events_multiple_commits_per_checkpoint() {
    use rtd_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
    use rtd_types::transaction::{Argument, Command};
    ...
    deposit_builder.programmable_move_call(
        rtd_types::RTD_FRAMEWORK_PACKAGE_ID,
    ...
    let deposit_tx = TransactionData::new(
        rtd_types::transaction::TransactionKind::ProgrammableTransaction(deposit_builder.finish()),
        sender,
        gas_for_deposit,
        10_000_000,
        rgp,
    ...
    let tx_data_vec: Vec<_> = (0..event_count)
        .map(|i| {
            ...
            rtd_types::transaction::TransactionData::V1(rtd_types::transaction::TransactionDataV1 {
                kind: rtd_types::transaction::TransactionKind::ProgrammableTransaction(
                    ptb.finish(),
                ),
                sender,
                gas_data: rtd_types::transaction::GasData {
                    payment: vec![],
                    owner: sender,
                    price: rgp,
                    budget: 50_000_000_000,
                },
                expiration: rtd_types::transaction::TransactionExpiration::ValidDuring {
                    min_epoch: Some(0),
                    max_epoch: Some(0),
                    min_timestamp: None,
                    max_timestamp: None,
