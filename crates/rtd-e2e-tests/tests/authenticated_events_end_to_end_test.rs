...
async fn load_event_stream_head_by_object_id(
    state: &rtd_core::authority::AuthorityState,
    object_id: ObjectID,
...
async fn authenticated_events_single_event_test() {
    ...
    assert_eq!(
        event.address,
        RtdAddress::from(AccountAddress::from(package_id))
    );

    let state = test_cluster.fullnode_handle.rtd_node.state();
...
async fn authenticated_events_mmr_validation_test() {
    ...
    let rtd_stream_head_x = load_event_stream_head_by_object_id(&state, accumulator_obj_id)
        .await
    ...
    assert_eq!(
        calculated_stream_head_x.num_events, rtd_stream_head_x.num_events,
        "Event count should match at checkpoint X"
    );

    assert_eq!(
        calculated_stream_head_x.mmr, rtd_stream_head_x.mmr,
        "MMR should match at checkpoint X"
    );

    let stream_head_x = &rtd_stream_head_x;
