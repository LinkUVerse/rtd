...
use rtd_types::committee::TOTAL_VOTING_POWER;
    ...
    match args.command {
        ...
        BridgeCommand::Governance {
            ...
            let rtd_bridge_client =
                RtdBridgeClient::new(&config.rtd_rpc_url, metrics.clone()).await?;

            let (rtd_key, rtd_address, gas_object_ref) = config
                .get_rtd_account_info()
                .await
                .expect("Failed to get rtd account info");
            let bridge_summary = rtd_bridge_client
                .get_bridge_summary()
                .await
            ...
            if chain_id.is_rtd_chain() {
                let rtd_chain_id = BridgeChainId::try_from(bridge_summary.chain_id).unwrap();
                assert_eq!(
                    rtd_chain_id, chain_id,
                    "Chain ID mismatch, expected: {:?}, got from url: {:?}",
                    chain_id, rtd_chain_id
                );
                // Create BridgeAction
                let rtd_action = make_action(rtd_chain_id, &cmd);
                println!("Action to execute on Rtd: {:?}", rtd_action);
                ...
                let rgp = rtd_bridge_client
                    ...
                    .await;
                let id_token_map = rtd_bridge_client.get_token_id_map().await.unwrap();
                let tx = build_rtd_transaction(
                    rtd_address,
                    &gas_object_ref,
                    certified_action,
                    bridge_arg,
                ...
                .expect("Failed to build rtd transaction");
                let rtd_sig = Signature::new_secure(
                    &IntentMessage::new(Intent::rtd_transaction(), tx.clone()),
                    &rtd_key,
                );
                let tx = Transaction::from_data(tx, vec![rtd_sig]);
                ...
                match &resp.status {
                    rtd_json_rpc_types::RtdExecutionStatus::Success => {
                        println!("Rtd Transaction succeeded");
                    }
                    rtd_json_rpc_types::RtdExecutionStatus::Failure { error } => {
                        println!("Rtd Transaction failed: {:?}", error);
                    }
                }
                return Ok(());
            }
        ...
        BridgeCommand::ViewBridgeRegistration { rtd_rpc_url } => {
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let rtd_bridge_client = RtdBridgeClient::new(&rtd_rpc_url, metrics).await?;
            let bridge_summary = rtd_bridge_client
                .get_bridge_summary()
                .await
                ...
                .map(|summary| {
                    let protocol_key =
                        AuthorityPublicKeyBytes::from_bytes(&summary.protocol_pubkey_bytes)
                            .unwrap();
                    (summary.rtd_address, (protocol_key, summary.name))
                })
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            let mut output_wrapper = Output::<OutputRtdBridgeRegistration>::default();
            for (_, member) in move_type_bridge_committee.member_registration {
                ...
                let Ok(pubkey) = BridgeAuthorityPublicKey::from_bytes(&bridge_pubkey_bytes) else {
                    output_wrapper.add_error(format!(
                        ...
                        rtd_address, bridge_pubkey_bytes
                    ));
                    continue;
                };
                ...
                let Ok(url) = from_utf8(&http_rest_url) else {
                    output_wrapper.add_error(format!(
                        ...
                        rtd_address, http_rest_url
                    ));
                    continue;
                };
                let url = url.to_string();

                let (protocol_key, name) = names.get(&rtd_address).unwrap();
                let stake = stakes.get(protocol_key).unwrap();
                authorities.push((name, rtd_address, pubkey, eth_address, url, stake));
            }
            let total_stake = authorities
                .iter()
                .map(|(_, _, _, _, _, stake)| **stake)
                .sum::<u64>();
            let mut output = OutputRtdBridgeRegistration {
                total_registered_stake: total_stake as f32 / TOTAL_VOTING_POWER as f32 * 100.0,
                ..Default::default()
            };
            for (name, rtd_address, pubkey, eth_address, url, stake) in authorities {
        ...
        BridgeCommand::ViewRtdBridge {
            ...
            let names = rtd_client
                ...
                .active_validators
                .into_iter()
                .map(|summary| (summary.rtd_address, summary.name))
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            ...
            let client = reqwest::Client::builder()
                ...
                .build()
                .unwrap();
            let mut output_wrapper = Output::<OutputRtdBridge>::default();
            for (_, member) in move_type_bridge_committee.members {
                ...
                let name = names.get(&rtd_address).cloned().unwrap_or(url.clone());
            ...
            let total_stake = authorities
                ...
                .sum::<u64>();
            let mut output = OutputRtdBridge {
        ...
        BridgeCommand::Client { config_path, cmd } => {
            ...
            let config = LoadedBridgeCliConfig::load(config).await?;
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let rtd_bridge_client = RtdBridgeClient::new(&config.rtd_rpc_url, metrics).await?;
            cmd.handle(&config, rtd_bridge_client).await?;
            return Ok(());
        }
    }
...
#[derive(serde::Serialize, Default)]
struct OutputRtdBridgeRegistration {
