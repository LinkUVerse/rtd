...
const RTD_DIR: &str = ".rtd";
pub const RTD_CONFIG_DIR: &str = "rtd_config";
pub const RTD_NETWORK_CONFIG: &str = "network.yaml";
pub const RTD_FULLNODE_CONFIG: &str = "fullnode.yaml";
pub const RTD_CLIENT_CONFIG: &str = "client.yaml";
pub const RTD_KEYSTORE_FILENAME: &str = "rtd.keystore";
...
pub fn rtd_config_dir() -> Result<PathBuf, anyhow::Error> {
    match std::env::var_os("RTD_CONFIG_DIR") {
        Some(config_env) => Ok(config_env.into()),
        None => match dirs::home_dir() {
            Some(v) => Ok(v.join(RTD_DIR).join(RTD_CONFIG_DIR)),
...
pub fn genesis_blob_exists(config_dir: Option<PathBuf>) -> bool {
    if let Some(dir) = config_dir {
        dir.join(RTD_GENESIS_FILENAME).exists()
    } else if let Some(config_env) = std::env::var_os("RTD_CONFIG_DIR") {
        Path::new(&config_env).join(RTD_GENESIS_FILENAME).exists()
    } else if let Some(home) = dirs::home_dir() {
        let mut config = PathBuf::new();
        config.push(&home);
        config.extend([RTD_DIR, RTD_CONFIG_DIR, RTD_GENESIS_FILENAME]);
        config.exists()
    } else {
        false
    }
