...
use crate::types::AddTokensOnRtdAction;
...
use crate::types::EthToRtdBridgeAction;
...
use rtd_types::base_types::RTD_ADDRESS_LENGTH;
...
pub const ADD_TOKENS_ON_RTD_MESSAGE_VERSION: u8 = 1;
pub const ADD_TOKENS_ON_EVM_MESSAGE_VERSION: u8 = 1;

pub const BRIDGE_MESSAGE_PREFIX: &[u8] = b"RTD_BRIDGE_MESSAGE";
...
impl BridgeMessageEncoding for RtdToEthBridgeAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        let e = &self.rtd_bridge_event;
        // Add message type
        ...
        // Add source chain id
        bytes.push(e.rtd_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        bytes.push(RTD_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&e.rtd_address.to_vec());
        // Add dest chain id
        ...
        // Add token amount
        bytes.extend_from_slice(&e.amount_rtd_adjusted.to_be_bytes());

        Ok(bytes)
    }
}

impl BridgeMessageEncoding for RtdToEthTokenTransfer {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::TokenTransfer as u8);
        ...
        // Add source chain id
        bytes.push(self.rtd_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        bytes.push(RTD_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&self.rtd_address.to_vec());
        // Add dest chain id
...
impl BridgeMessageEncoding for EthToRtdBridgeAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        // Add token amount
        bytes.extend_from_slice(&e.rtd_adjusted_amount.to_be_bytes());
...
impl BridgeMessageEncoding for AddTokensOnRtdAction {
    fn as_bytes(&self) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::AddTokensOnRtd as u8);
        // Add message version
        bytes.push(ADD_TOKENS_ON_RTD_MESSAGE_VERSION);
        // Add nonce
        bytes.extend_from_slice(&self.nonce.to_be_bytes());
        // Add chain id
        bytes.push(self.chain_id as u8);
...
impl BridgeMessageEncoding for AddTokensOnEvmAction {
    ...
    fn as_payload_bytes(&self) -> Result<Vec<u8>> {
        ...
        // Add token rtd decimals
        bytes.push(u8::try_from(self.token_rtd_decimals.len())?);
        for token_rtd_decimal in &self.token_rtd_decimals {
            bytes.push(*token_rtd_decimal);
        }
...
mod tests {
    ...
    fn test_bridge_message_encoding() -> anyhow::Result<()> {
        ...
        let rtd_chain_id = BridgeChainId::RtdTestnet;
        let rtd_tx_event_index = 1u16;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let rtd_address = RtdAddress::random_for_testing_only();
        let eth_address = EthAddress::random();
        let token_id = TOKEN_ID_USDC;
        let amount_rtd_adjusted = 1_000_000;

        let rtd_bridge_event = EmittedRtdToEthTokenBridgeV1 {
            nonce,
            rtd_chain_id,
            eth_chain_id,
            rtd_address,
        ...
        let encoded_bytes = BridgeAction::RtdToEthBridgeAction(RtdToEthBridgeAction {
            rtd_tx_digest,
            rtd_tx_event_index,
            rtd_bridge_event,
        })
        ...
        let source_chain_id_bytes = vec![rtd_chain_id as u8]; // len: 1

        let rtd_address_length_bytes = vec![RTD_ADDRESS_LENGTH as u8]; // len: 1
        let rtd_address_bytes = rtd_address.to_vec(); // len: 32
        ...
        let token_amount_bytes = amount_rtd_adjusted.to_be_bytes().to_vec(); // len: 8
        ...
        combined_bytes.extend_from_slice(&rtd_address_length_bytes);
        combined_bytes.extend_from_slice(&rtd_address_bytes);
    ...
    #[test]
    fn test_bridge_message_encoding_regression_emitted_rtd_to_eth_token_bridge_v1()
    -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        linku_metrics::init_metrics(&registry);
        ...
        let rtd_chain_id = BridgeChainId::RtdTestnet;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let rtd_address = RtdAddress::from_str(
    ...
    fn test_bridge_message_encoding_blocklist_update_v1() {
        ...
        let blocklist_action = BridgeAction::BlocklistCommitteeAction(BlocklistCommitteeAction {
            nonce: 129,
            chain_id: BridgeChainId::RtdCustom,
    ...
    #[test]
    fn test_bridge_message_encoding_regression_eth_to_rtd_token_bridge_v1() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        linku_metrics::init_metrics(&registry);
        ...
        let eth_address =
            EthAddress::from_str("0x00000000000000000000000000000000000000c8").unwrap();
        let token_id = TOKEN_ID_USDC;
        let rtd_adjusted_amount = 12345;

        let eth_bridge_event = EthToRtdTokenBridgeV1 {
            nonce,
            rtd_chain_id,
            eth_chain_id,
            rtd_address,
        ...
        let encoded_bytes = BridgeAction::EthToRtdBridgeAction(EthToRtdBridgeAction {
            eth_tx_hash,
            eth_event_index,
            eth_bridge_event,
        })
    ...
    #[test]
    fn test_bridge_message_encoding_regression_add_coins_on_rtd() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();

        let action = BridgeAction::AddTokensOnRtdAction(AddTokensOnRtdAction {
            nonce: 0,
            chain_id: BridgeChainId::RtdCustom,
            native: false,
