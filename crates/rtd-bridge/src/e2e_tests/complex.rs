...
use crate::e2e_tests::test_utils::{
    BridgeTestClusterBuilder, initiate_bridge_eth_to_rtd, initiate_bridge_rtd_to_eth,
};
...
async fn test_rtd_bridge_paused() {
    ...
    let rtd_token_type_tags = bridge_client.get_token_id_map().await.unwrap();

    // verify bridge are not paused
    assert!(!bridge_client.get_bridge_summary().await.unwrap().is_frozen);

    // try bridge from eth and verify it works on rtd
    initiate_bridge_eth_to_rtd(&bridge_test_cluster, 10, 0)
        .await
        .unwrap();
    // verify Eth was transferred to Rtd address
    let eth_coin_type = rtd_token_type_tags.get(&TOKEN_ID_ETH).unwrap();
    let eth_coin = bridge_client
        .jsonrpc_client()
        .coin_read_api()
        .get_coins(rtd_address, Some(eth_coin_type.to_string()), None, None)
        .await
        .unwrap()
        .data;
    ...
    // Transfer from eth to rtd should fail on Rtd
    let eth_to_rtd_bridge_action = initiate_bridge_eth_to_rtd(&bridge_test_cluster, 10, 1).await;
    assert!(eth_to_rtd_bridge_action.is_err());
    // message should not be recorded on Rtd when the bridge is paused
    ...
    // Transfer from Rtd to eth should fail
    ...
    .await;
    assert!(rtd_to_eth_bridge_action.is_err())
}
