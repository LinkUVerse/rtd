...
impl BridgeTestClusterBuilder {
    ...
    pub fn with_rtd_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.rtd_chain_id = chain_id;
        self
    }
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &RtdBridgeClient {
        &self.bridge_client
    }
    ...
    pub fn rtd_chain_id(&self) -> BridgeChainId {
        self.rtd_chain_id
    }
    ...
    pub fn rtd_bridge_address(&self) -> String {
        self.eth_environment.contracts().rtd_bridge_addrress_hex()
    }
    ...
    pub fn rtd_rpc_url(&self) -> String {
...
impl DeployedSolContracts {
    ...
    pub fn rtd_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.rtd_bridge)
    }
}
...
pub(crate) async fn deploy_sol_contract(
    ...
    let deploy_config = SolDeployConfig {
        ...
        rtd_decimals: vec![],     // this is set up in the deploy script
    ...
    let contracts = DeployedSolContracts {
        rtd_bridge: deployed_contracts.remove(RTD_BRIDGE_NAME).unwrap(),
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            rtd: RtdConfig {
                rtd_rpc_url: test_cluster.inner.fullnode_handle.rpc_url.clone(),
...
pub async fn get_signatures(
    ...
    let sigs = rtd_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(rtd_chain_id, nonce)
        .await
        .unwrap();
...
impl TestClusterWrapperBuilder {
    ...
    pub async fn build(self) -> TestClusterWrapper {
        ...
        let gas_objects_for_authority_keys = self
            ...
            .iter()
            .map(|k| {
                let address = RtdAddress::from(k.public());
        ...
        if self.deploy_tokens {
            ...
            let action = publish_and_register_coins_return_add_coins_on_rtd_action(
                test_cluster.wallet(),
                bridge_arg,
                vec![
...
pub async fn initiate_bridge_eth_to_rtd(
    ...
    .await
    .tap_ok(|_| {
        info!("Eth to Rtd bridge transfer claimed");
    })
}

pub async fn initiate_bridge_rtd_to_eth(
    bridge_test_cluster: &BridgeTestCluster,
    ...
    let resp = match deposit_eth_to_rtd_package(
        rtd_client,
        rtd_address,
    ...
    let rtd_events = resp.events.unwrap().data;
    ...
    .await
    .unwrap();
    info!("Rtd to Eth bridge transfer approved.");
...
pub async fn initiate_bridge_erc20_to_rtd(
    bridge_test_cluster: &BridgeTestCluster,
    amount_u64: u64,
