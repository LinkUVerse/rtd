...
use crate::abi::{
    ...
    eth_committee_upgradeable_contract, eth_rtd_bridge,
};
...
pub async fn build_eth_transaction(
    ...
    match action.data() {
        ...
        BridgeAction::AddTokensOnRtdAction(_) => {
            unreachable!();
        }
...
pub async fn build_emergency_op_approve_transaction(
    ...
    let contract = EthRtdBridge::new(contract_address, signer.into());

    let message: eth_rtd_bridge::Message = action.clone().try_into()?;
    let signatures = sigs
        .signatures
        .values()
