...
pub struct BridgeOrchestratorTables {
    ...
    pub(crate) rtd_syncer_cursors: DBMap<Identifier, EventID>,
...
impl BridgeOrchestratorTables {
    ...
    pub(crate) fn update_rtd_event_cursor(
        &self,
        module: Identifier,
        cursor: EventID,
    ) -> BridgeResult<()> {
        let mut batch = self.rtd_syncer_cursors.batch();

        batch
            .insert_batch(&self.rtd_syncer_cursors, [(module, cursor)])
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Coudln't insert into rtd_syncer_cursors: {:?}",
                    e
                ))
            })?;
        batch
    ...
    pub fn get_rtd_event_cursors(
        &self,
        identifiers: &[Identifier],
    ) -> BridgeResult<Vec<Option<EventID>>> {
        self.rtd_syncer_cursors.multi_get(identifiers).map_err(|e| {
            BridgeError::StorageError(format!("Couldn't get rtd_syncer_cursors: {:?}", e))
        })
    }
...
mod tests {
    ...
    async fn test_bridge_storage_basic() {
        ...
        // update rtd event cursor
        let rtd_module = Identifier::from_str("test").unwrap();
        let rtd_cursor = EventID {
            tx_digest: TransactionDigest::random(),
            event_seq: 1,
        };
        assert!(
            store
                .get_rtd_event_cursors(std::slice::from_ref(&rtd_module))
                .unwrap()[0]
                .is_none()
        );
        store
            .update_rtd_event_cursor(rtd_module.clone(), rtd_cursor)
            .unwrap();
        assert_eq!(
            store
                .get_rtd_event_cursors(std::slice::from_ref(&rtd_module))
