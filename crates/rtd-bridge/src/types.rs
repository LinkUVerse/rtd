...
use rtd_types::bridge::{
    APPROVAL_THRESHOLD_ADD_TOKENS_ON_EVM, APPROVAL_THRESHOLD_ADD_TOKENS_ON_RTD,
...
pub struct RtdToEthBridgeAction {
    // Digest of the transaction where the event was emitted
    pub rtd_tx_digest: TransactionDigest,
    // The index of the event in the transaction
    pub rtd_tx_event_index: u16,
    pub rtd_bridge_event: EmittedRtdToEthTokenBridgeV1,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct RtdToEthTokenTransfer {
    pub nonce: u64,
...
pub struct LimitUpdateAction {
    ...
    // for the limit update. For example, if chain_id is EthMainnet and sending_chain_id is RtdMainnet,
    // it means we want to update the limit for the RtdMainnet to EthMainnet route.
    pub chain_id: BridgeChainId,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct AddTokensOnRtdAction {
    pub nonce: u64,
    pub chain_id: BridgeChainId,
    pub native: bool,
...
pub enum BridgeAction {
    ...
    RtdToEthBridgeAction(RtdToEthBridgeAction),
    /// Eth to rtd bridge action
    EthToRtdBridgeAction(EthToRtdBridgeAction),
    ...
    AddTokensOnRtdAction(AddTokensOnRtdAction),
...
impl BridgeAction {
    ...
    pub fn chain_id(&self) -> BridgeChainId {
        match self {
            ...
            BridgeAction::AddTokensOnRtdAction(a) => a.chain_id,
    ...
    pub fn is_governace_action(&self) -> bool {
        match self.action_type() {
            ...
            BridgeActionType::AddTokensOnRtd => true,
    ...
    pub fn action_type(&self) -> BridgeActionType {
        match self {
            ...
            BridgeAction::AddTokensOnRtdAction(_) => BridgeActionType::AddTokensOnRtd,
    ...
    pub fn seq_number(&self) -> u64 {
        match self {
            ...
            BridgeAction::AddTokensOnRtdAction(a) => a.nonce,
    ...
    pub fn approval_threshold(&self) -> u64 {
        match self {
            ...
            BridgeAction::AddTokensOnRtdAction(_) => APPROVAL_THRESHOLD_ADD_TOKENS_ON_RTD,
    ...
    pub fn update_to_token_transfer(self) -> Self {
        match self {
            BridgeAction::RtdToEthBridgeAction(a) => {
                BridgeAction::RtdToEthTokenTransfer(RtdToEthTokenTransfer {
                    nonce: a.rtd_bridge_event.nonce,
                    rtd_chain_id: a.rtd_bridge_event.rtd_chain_id,
                    eth_chain_id: a.rtd_bridge_event.eth_chain_id,
                    rtd_address: a.rtd_bridge_event.rtd_address,
                    eth_address: a.rtd_bridge_event.eth_address,
                    token_id: a.rtd_bridge_event.token_id,
                    amount_adjusted: a.rtd_bridge_event.amount_rtd_adjusted,
                })
            }
    ...
    pub fn try_from_bridge_record(record: &MoveTypeBridgeRecord) -> BridgeResult<Self> {
        ...
        struct RtdToEthOnChainBcsPayload {
            rtd_address: Vec<u8>,
            target_chain: u8,
            eth_address: Vec<u8>,
            token_type: u8,
...
pub fn is_route_valid(one: BridgeChainId, other: BridgeChainId) -> bool {
    if one.is_rtd_chain() && other.is_rtd_chain() {
        return false;
    }
    if !one.is_rtd_chain() && !other.is_rtd_chain() {
        return false;
    ...
    if one == BridgeChainId::EthMainnet {
        return other == BridgeChainId::RtdMainnet;
    }
    if one == BridgeChainId::RtdMainnet {
        return other == BridgeChainId::EthMainnet;
    }
    if other == BridgeChainId::EthMainnet {
        return one == BridgeChainId::RtdMainnet;
    }
    if other == BridgeChainId::RtdMainnet {
...
pub struct RtdEvents {
    pub transaction_digest: TransactionDigest,
    pub checkpoint: Option<u64>,
    pub timestamp_ms: Option<u64>,
    pub events: Vec<rtd_json_rpc_types::RtdEvent>,
}
...
mod tests {
    ...
    fn test_bridge_action_approval_threshold_regression_test() -> anyhow::Result<()> {
        let action = get_test_rtd_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);

        let action = get_test_eth_to_rtd_bridge_action(None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);
