...
pub struct RtdConfig {
    ...
    /// The expected BridgeChainId on Rtd side.
    pub rtd_bridge_chain_id: u8,
    /// Path of the file where bridge client key (any RtdKeyPair) is stored.
    ...
    /// When set, RtdSyncer will start from this cursor (exclusively) instead of the one in storage.
    /// If the cursor is not found in storage or override, the query will start from genesis.
    /// Key: rtd module, Value: last processed EventID (tx_digest, event_seq).
...
pub struct BridgeNodeConfig {
    ...
    /// Whether to run client. If true, `rtd.bridge_client_key_path`
...
pub struct WatchdogConfig {
    /// Total supplies to watch on Rtd. Mapping from coin name to coin type tag
...
impl BridgeNodeConfig {
    pub async fn validate(
        ...
        if !is_route_valid(
            BridgeChainId::try_from(self.rtd.rtd_bridge_chain_id)?,
            BridgeChainId::try_from(self.eth.eth_bridge_chain_id)?,
        ) {
            return Err(anyhow!(
                "Route between Rtd chain id {} and Eth chain id {} is not valid",
        ...
        // we do this check here instead of `prepare_for_rtd` below because
        // that is only called when `run_client` is true.
        let rtd_client =
            Arc::new(RtdBridgeClient::new(&self.rtd.rtd_rpc_url, metrics.clone()).await?);
        let bridge_committee = rtd_client
        ...
        let bridge_summary = rtd_client
            .get_bridge_summary()
            .await
            .map_err(|e| anyhow!("Error getting bridge summary: {:?}", e))?;
        if bridge_summary.chain_id != self.rtd.rtd_bridge_chain_id {
            anyhow::bail!(
        ...
        let (bridge_client_key, client_rtd_address, gas_object_ref) =
            self.prepare_for_rtd(rtd_client.clone(), metrics).await?;
        ...
        let bridge_client_config = BridgeClientConfig {
            rtd_address: client_rtd_address,
    ...
    async fn prepare_for_rtd(
        ...
        let bridge_client_key = match &self.rtd.bridge_client_key_path {
            None => read_key(&self.bridge_authority_key_path, true),
            Some(path) => read_key(path, false),
        }?;

        // If bridge chain id is Rtd Mainent or Testnet, we expect to see chain
        // identifier to match accordingly.
        let rtd_identifier = rtd_client
        ...
        info!(
            "Connected to Rtd chain: {}, Bridge chain id: {}",
            rtd_identifier, self.rtd.rtd_bridge_chain_id,
        );

        let client_rtd_address = RtdAddress::from(&bridge_client_key.public());

        let gas_object_id = match self.rtd.bridge_client_gas_object {
            Some(id) => id,
            None => {
        ...
        let (gas_coin, gas_object_ref, owner) = rtd_client
            ...
            .await;
        if owner != Owner::AddressOwner(client_rtd_address) {
            return Err(anyhow!(
                "Gas object {:?} is not owned by bridge client key's associated rtd address {:?}, but {:?}",
                gas_object_id,
                client_rtd_address,
                owner
            ));
        ...
        info!("Rtd client setup complete");
        Ok((bridge_client_key, client_rtd_address, gas_object_ref))
    }
}
