...
pub const ETH_TO_RTD_TX_PATH: &str = "/sign/bridge_tx/eth/rtd/{tx_hash}/{event_index}";
pub const RTD_TO_ETH_TX_PATH: &str = "/sign/bridge_tx/rtd/eth/{tx_digest}/{event_index}";
pub const RTD_TO_ETH_TRANSFER_PATH: &str =
...
pub const ADD_TOKENS_ON_RTD_PATH: &str = "/sign/add_tokens_on_rtd/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}";
pub const ADD_TOKENS_ON_EVM_PATH: &str = "/sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_rtd_decimals}/{token_prices}";
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(ETH_TO_RTD_TX_PATH, get(handle_eth_tx_hash))
        .route(RTD_TO_ETH_TX_PATH, get(handle_rtd_tx_digest))
        .route(RTD_TO_ETH_TRANSFER_PATH, get(handle_rtd_token_transfer))
        .route(
        ...
        .route(ADD_TOKENS_ON_RTD_PATH, get(handle_add_tokens_on_rtd))
    ...
    fn into_response(self) -> axum::response::Response {
        ...
        let sanitized_error = match self {
            ...
            BridgeError::BridgeEventInUnrecognizedRtdPackage => {
                "BridgeEventInUnrecognizedRtdPackage"
            }
...
async fn handle_rtd_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_rtd_tx_digest(tx_digest_base58, event_idx)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_rtd_tx_digest", future).await
}
...
async fn handle_rtd_token_transfer(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_rtd_token_transfer(source_chain, message_type, bridge_seq_num)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_rtd_token_transfer", future).await
}
...
async fn handle_add_tokens_on_rtd(
    ...
    let future = async {
        ...
        if !chain_id.is_rtd_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_rtd only expects Rtd chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_rtd", future).await
}

#[instrument(level = "error", skip_all, fields(chain_id=chain_id, nonce=nonce, native=native, token_ids=token_ids, token_addresses=token_addresses, token_rtd_decimals=token_rtd_decimals, token_prices=token_prices))]
async fn handle_add_tokens_on_evm(
    Path((chain_id, nonce, native, token_ids, token_addresses, token_rtd_decimals, token_prices)): Path<(
        u8,
        u64,
        u8,
        String,
    ...
    let future = async {
        ...
        if chain_id.is_rtd_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect Rtd chain id".to_string(),
            ));
        }
        ...
        validate_list_size(&token_rtd_decimals, "token_rtd_decimals")?;
        validate_list_size(&token_prices, "token_prices")?;
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_rtd_decimals = token_rtd_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token rtd decimals: {:?}",
                        err
                    ))
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bridge_server_handle_add_tokens_on_rtd_action_path() {
        let client = setup();
