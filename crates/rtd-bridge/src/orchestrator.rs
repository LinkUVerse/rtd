...
//! 1. monitors Rtd and Ethereum events with the help of `RtdSyncer` and `EthSyncer`
...
mod tests {
    ...
    fn setup() -> (
        ...
        let (eth_events_tx, eth_events_rx) = linku_metrics::metered_channel::channel(
            100,
    ...
    impl BridgeActionExecutorTrait for MockExecutor {
        fn run(
            ...
            let (tx, mut rx) =
                linku_metrics::metered_channel::channel::<BridgeActionExecutionWrapper>(
                    100,
                    &linku_metrics::get_metrics()
                        .unwrap()
                        .channel_inflight
