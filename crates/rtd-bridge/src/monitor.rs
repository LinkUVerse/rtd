...
pub struct BridgeMonitor<C> {
    ...
    rtd_monitor_rx: linku_metrics::metered_channel::Receiver<RtdBridgeEvent>,
    eth_monitor_rx: linku_metrics::metered_channel::Receiver<EthBridgeEvent>,
    ...
    C: RtdClientInner + 'static,
{
    ...
    pub async fn run(self) {
        ...
        let mut latest_token_config = (*rtd_token_type_tags.load().clone()).clone();

        loop {
            tokio::select! {
                rtd_event = rtd_monitor_rx.recv() => {
                    if let Some(rtd_event) = rtd_event {
                        Self::handle_rtd_events(
                            rtd_event,
                            &rtd_client,
                            &bridge_auth_agg,
                            &bridge_paused_watch_tx,
                        ...
                        panic!("BridgeMonitor rtd events channel was closed unexpectedly");
                    }
                }
    ...
    async fn handle_rtd_events(
        event: RtdBridgeEvent,
        rtd_client: &Arc<RtdClient<C>>,
        bridge_auth_agg: &Arc<ArcSwap<BridgeAuthorityAggregator>>,
        bridge_paused_watch_tx: &tokio::sync::watch::Sender<IsBridgePaused>,
        rtd_token_type_tags: &Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        ...
        info!("Received RtdBridgeEvent: {:?}", event);
        macro_rules! bump_rtd_counter {
            ($action:expr) => {
                bridge_metrics
                    .observed_governance_actions
                    .with_label_values(&[$action, "rtd"])
                    .inc();
            };
        }
        ...
        match event {
            ...
            RtdBridgeEvent::TokenTransferAlreadyClaimed(_) => (),
            RtdBridgeEvent::TokenTransferLimitExceed(_) => {
                // TODO do we want to do anything here?
            }

            RtdBridgeEvent::EmergencyOpEvent(event) => {
                bump_rtd_counter!(if event.frozen {
                    "bridge_paused"
                } else {
                    "bridge_unpaused"
                });
            ...
            RtdBridgeEvent::CommitteeMemberRegistration(_) => {
                bump_rtd_counter!("committee_member_registered");
            }
            RtdBridgeEvent::CommitteeUpdateEvent(_) => {
                bump_rtd_counter!("committee_updated");
            }

            RtdBridgeEvent::CommitteeMemberUrlUpdateEvent(event) => {
                bump_rtd_counter!("committee_member_url_updated");
            ...
            RtdBridgeEvent::BlocklistValidatorEvent(event) => {
                bump_rtd_counter!(if event.blocklisted {
            ...
            RtdBridgeEvent::TokenRegistrationEvent(_) => {
                bump_rtd_counter!("new_token_registered");
            }

            RtdBridgeEvent::NewTokenEvent(event) => {
                bump_rtd_counter!("new_token_added");
                    ...
                    latest_token_config.entry(event.token_id)
                {
                    ...
                    rtd_token_type_tags.store(Arc::new(latest_token_config.clone()));
                } else {
                    // invariant
            ...
            RtdBridgeEvent::UpdateTokenPriceEvent(_event) => {
                bump_rtd_counter!("token_price_updated");
            }

            RtdBridgeEvent::UpdateRouteLimitEvent(_event) => {
                bump_rtd_counter!("limit_updated");
            }
        }
    }
...
async fn get_latest_bridge_committee_with_url_update_event<C: RtdClientInner>(
    rtd_client: Arc<RtdClient<C>>,
    ...
    loop {
        let Ok(Ok(committee)) = retry_with_max_elapsed_time!(
            rtd_client.get_bridge_committee(),
...
async fn get_latest_bridge_committee_with_blocklist_event<C: RtdClientInner>(
    rtd_client: Arc<RtdClient<C>>,
...
async fn get_latest_bridge_pause_status_with_emergency_event<C: RtdClientInner>(
    rtd_client: Arc<RtdClient<C>>,
    ...
    loop {
        let Ok(Ok(summary)) =
            retry_with_max_elapsed_time!(rtd_client.get_bridge_summary(), Duration::from_secs(600))
        else {
...
mod tests {
    ...
    use rtd_types::bridge::BridgeCommitteeSummary;
    use rtd_types::bridge::MoveTypeCommitteeMember;
    use rtd_types::crypto::get_key_pair;

    use crate::{rtd_mock_client::RtdMockClient, types::BridgeCommittee};
    ...
    async fn test_get_latest_bridge_committee_with_url_update_event() {
        ...
        rtd_client_mock.set_bridge_committee(summary.clone());
        ...
        rtd_client_mock.set_bridge_committee(old_summary.clone());
        let timer = std::time::Instant::now();
        // update the url to "http://new.url" in 1 second
        let rtd_client_mock_clone = rtd_client_mock.clone();
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            rtd_client_mock_clone.set_bridge_committee(summary.clone());
        });
        let committee = get_latest_bridge_committee_with_url_update_event(
            rtd_client.clone(),
            event.clone(),
        ...
        rtd_client_mock.set_bridge_committee(newer_summary.clone());
        let timer = std::time::Instant::now();
        let committee = get_latest_bridge_committee_with_url_update_event(
            rtd_client.clone(),
            event.clone(),
    ...
    async fn test_get_bridge_pause_status_with_emergency_event() {
        ...
        rtd_client_mock.set_is_bridge_paused(BRIDGE_PAUSED);
        let timer = std::time::Instant::now();
        assert!(
        ...
        rtd_client_mock.set_is_bridge_paused(BRIDGE_UNPAUSED);
        let timer = std::time::Instant::now();
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                rtd_client.clone(),
        ...
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            rtd_client_mock_clone.set_is_bridge_paused(BRIDGE_UNPAUSED);
        });
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                rtd_client.clone(),
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        ...
        let rtd_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        ...
        rtd_client_mock.set_bridge_committee(new_committee_summary.clone());
        rtd_monitor_tx
            .send(RtdBridgeEvent::CommitteeMemberUrlUpdateEvent(
    ...
    async fn test_update_bridge_authority_aggregation_with_blocklist_event() {
        ...
        rtd_client_mock.set_bridge_committee(new_committee_summary.clone());
        rtd_monitor_tx
            .send(RtdBridgeEvent::BlocklistValidatorEvent(
    ...
    async fn test_update_bridge_pause_status_with_emergency_event() {
        ...
        rtd_client_mock.set_is_bridge_paused(event.frozen);
        rtd_monitor_tx
            .send(RtdBridgeEvent::EmergencyOpEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
    ...
    #[tokio::test]
    async fn test_update_rtd_token_type_tags() {
        let (
            rtd_monitor_tx,
            rtd_monitor_rx,
            _eth_monitor_tx,
        ...
        let rtd_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        let rtd_token_type_tags_clone = rtd_token_type_tags.clone();
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(
                rtd_client.clone(),
                rtd_monitor_rx,
        ...
        rtd_monitor_tx
            .send(RtdBridgeEvent::NewTokenEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Now expect new token type tags to appear in rtd_token_type_tags
        rtd_token_type_tags
            .load()
            .clone()
            .get(&event.token_id)
