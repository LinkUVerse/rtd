...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            BridgeAction::RtdToEthBridgeAction(e) => format!(
            ...
            BridgeAction::AddTokensOnRtdAction(a) => {
                let chain_id = (a.chain_id as u8).to_string();
                let nonce = a.nonce.to_string();
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_rtd/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}"
                )
            }
            BridgeAction::AddTokensOnEvmAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_rtd_decimals}/{token_prices}"
                )
            }
        }
    }
...
mod tests {
    ...
    use rtd_types::bridge::{BridgeChainId, TOKEN_ID_BTC, TOKEN_ID_USDT};
    ...
    async fn test_bridge_client() {
        ...
        let action =
            get_test_rtd_to_eth_bridge_action(None, Some(1), Some(1), Some(100), None, None, None);
    ...
    async fn test_bridge_client_request_sign_action() {
        ...
        mock_handler.add_rtd_event_response(tx_digest, event_idx, Ok(signed_event.clone()), None);
        ...
        let action2 = get_test_rtd_to_eth_bridge_action(
            Some(tx_digest),
            Some(event_idx),
            Some(2),
            Some(200),
        ...
        mock_handler.add_rtd_event_response(tx_digest, event_idx, Ok(wrong_signed_action), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_rtd_event_response(tx_digest, event_idx, Ok(signed_event), None);
        ...
        mock_handler.add_rtd_event_response(tx_digest, event_idx, Ok(signed_event2), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_rtd_event_response(tx_digest, event_idx, Ok(signed_event3), None);
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
    ...
    fn test_bridge_action_path_regression_tests() {
        ...
        let rtd_tx_event_index = 5;
        let action = BridgeAction::RtdToEthBridgeAction(crate::types::RtdToEthBridgeAction {
            rtd_tx_digest,
            rtd_tx_event_index,
        ...
        assert_eq!(
            ...
            format!(
                "sign/bridge_tx/rtd/eth/{}/{}",
                rtd_tx_digest, rtd_tx_event_index
            )
        );

        let eth_tx_hash = TxHash::random();
        let eth_event_index = 6;
        let action = BridgeAction::EthToRtdBridgeAction(crate::types::EthToRtdBridgeAction {
            eth_tx_hash,
            eth_event_index,
        ...
        let action = BridgeAction::AddTokensOnRtdAction(crate::types::AddTokensOnRtdAction {
            nonce: 3,
            chain_id: BridgeChainId::RtdCustom,
            native: false,
