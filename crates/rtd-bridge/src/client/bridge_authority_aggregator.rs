...
use rtd_authority_aggregation::{SigRequestPrefs, quorum_map_then_reduce_with_timeout_and_prefs};
...
impl GetSigsState {
    ...
    fn handle_verified_signed_action(
        ...
        if self.total_ok_stake >= self.validity_threshold {
            ...
            let certified_action: rtd_types::message_envelope::Envelope<
                BridgeAction,
...
async fn request_sign_bridge_action_into_certification(
    ...
    let preference = match action {
        BridgeAction::RtdToEthBridgeAction(_) => Some(SigRequestPrefs {
            ordering_pref: BTreeSet::new(),
            prefetch_timeout,
        }),
        BridgeAction::EthToRtdBridgeAction(_) => None,
        _ => {
            if action.chain_id().is_rtd_chain() {
                None
            } else {
...
mod tests {
    ...
    use rtd_types::committee::VALIDITY_THRESHOLD;
