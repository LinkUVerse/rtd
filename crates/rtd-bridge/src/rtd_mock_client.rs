...
//! A mock implementation of Rtd JSON-RPC client.
...
impl RtdMockClient {
    ...
    pub fn add_events_by_tx_digest_error(&self, tx_digest: TransactionDigest) {
        self.events_by_tx_digest.lock().unwrap().insert(
            tx_digest,
            Err(rtd_sdk::error::Error::DataError("".to_string())),
        );
    }
...
impl RtdClientInner for RtdMockClient {
    ...
    async fn get_events_by_tx_digest(
        ...
        {
            Ok(events) => Ok(RtdEvents {
                transaction_digest: tx_digest,
                checkpoint: None,
                timestamp_ms: None,
                events: events.clone(),
            }),
            // rtd_sdk::error::Error is not Clone
            Err(_) => Err(rtd_sdk::error::Error::DataError("".to_string()).into()),
        }
    }
