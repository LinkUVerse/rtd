...
pub trait BridgeActionExecutorTrait {
    fn run(
        ...
        linku_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
    );
}
...
pub struct BridgeActionExecutor<C> {
    ...
    rtd_token_type_tags: Arc<ArcSwap<HashMap<u8, TypeTag>>>,
    ...
    C: RtdClientInner + 'static,
{
    ...
    fn run_inner(
        ...
        Vec<tokio::task::JoinHandle<()>>,
        linku_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
        linku_metrics::metered_channel::Sender<CertifiedBridgeActionExecutionWrapper>,
    ) {
        let key = self.key;

        let (sender, receiver) = linku_metrics::metered_channel::channel(
            CHANNEL_SIZE,
        ...
        let (execution_tx, execution_rx) = linku_metrics::metered_channel::channel(
            CHANNEL_SIZE,
            &linku_metrics::get_metrics()
                .unwrap()
                .channel_inflight
        ...
        let client_clone = self.rtd_client.clone();
        let mut tasks = vec![];
    ...
    async fn run_signature_aggregation_loop(
        ...
        mut signing_queue_receiver: linku_metrics::metered_channel::Receiver<
    ...
    async fn should_proceed_signing(rtd_client: &Arc<RtdClient<C>>) -> bool {
        let Ok(Ok(is_paused)) =
            retry_with_max_elapsed_time!(rtd_client.is_bridge_paused(), Duration::from_secs(600))
        else {
    ...
    async fn handle_signing_task(
        ...
        let should_proceed = Self::should_proceed_signing(rtd_client).await;
        if !should_proceed {
    ...
    async fn handle_already_processed_token_transfer_action_maybe(
        ...
        match status {
            ...
            // Although theoretically a legit RtdToEthBridgeAction should not have
            // status `NotFound`
    ...
    // Before calling this function, `key` and `rtd_address` need to be
    // verified to match.
    async fn run_onchain_execution_loop(
        ...
        execution_queue_sender: linku_metrics::metered_channel::Sender<
            CertifiedBridgeActionExecutionWrapper,
        >,
        mut execution_queue_receiver: linku_metrics::metered_channel::Receiver<
            CertifiedBridgeActionExecutionWrapper,
        >,
        bridge_object_arg: ObjectArg,
        rtd_token_type_tags: Arc<ArcSwap<HashMap<u8, TypeTag>>>,
    ...
    async fn handle_execution_task(
        ...
        >,
        bridge_object_arg: &ObjectArg,
        rtd_token_type_tags: &ArcSwap<HashMap<u8, TypeTag>>,
        metrics: &Arc<BridgeMetrics>,
    ) {
        metrics
        ...
        let (gas_coin, gas_object_ref) =
            Self::get_gas_data_assert_ownership(*rtd_address, gas_object_id, rtd_client).await;
        ...
        if Self::handle_already_processed_token_transfer_action_maybe(
            rtd_client, action, store, metrics,
        )
        .await
        {
        ...
        info!("Building Rtd transaction");
        let rgp = rtd_client.get_reference_gas_price_until_success().await;
        ...
        info!(?tx_digest, ?gas_object_ref, "Sending transaction to Rtd");
        match rtd_client
            ...
            .await
        {
            ...
            Err(err) => {
                error!(
                    ?action_key,
                    ?tx_digest,
                    "Rtd transaction failed at signing: {err:?}"
                );
                metrics.err_rtd_transaction_submission.inc();
                let metrics_clone = metrics.clone();
                ...
                spawn_logged_monitored_task!(async move {
                    ...
                    if attempt_times >= MAX_EXECUTION_ATTEMPTS {
                        metrics_clone
                            .err_rtd_transaction_submission_too_many_failures
                            .inc();
    ...
    async fn handle_execution_effects(
        ...
        match &response.status {
            RtdExecutionStatus::Success => {
                ...
                info!(?tx_digest, "Rtd transaction executed successfully");
            ...
            RtdExecutionStatus::Failure { error } => {
                ...
                metrics.err_rtd_transaction_execution.inc();
                error!(
                    ?tx_digest,
                    "Manual intervention is needed. Rtd transaction executed and failed with error: {error:?}"
                );
            }
        }
    }
    ...
    async fn get_gas_data_assert_ownership(
        rtd_address: RtdAddress,
        gas_object_id: ObjectID,
        rtd_client: &RtdClient<C>,
    ) -> (GasCoin, ObjectRef) {
        let (gas_coin, gas_obj_ref, owner) = rtd_client
        ...
        assert_eq!(
            owner,
            Owner::AddressOwner(rtd_address),
...
pub async fn submit_to_executor(
    tx: &linku_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
    action: BridgeAction,
) -> Result<(), BridgeError> {
...
mod tests {
    ...
    async fn test_onchain_execution_loop() {
        ...
        let id_token_map = (*rtd_token_type_tags.load().clone()).clone();
        let tx_data = build_rtd_transaction(
            rtd_address,
            &gas_object_ref,
            action_certificate,
        ...
        let tx_digest = get_tx_digest(tx_data, &dummy_rtd_key);

        let gas_coin = GasCoin::new_for_testing(1_000_000_000_000); // dummy gas coin
        rtd_client_mock.add_gas_object_info(
            gas_coin.clone(),
            gas_object_ref,
            Owner::AddressOwner(rtd_address),
        );
        ...
        let mut event = RtdEvent::random_for_testing();
    ...
    async fn test_signature_aggregation_loop() {
        ...
        let id_token_map = (*rtd_token_type_tags.load().clone()).clone();
        let (action_certificate, rtd_tx_digest, rtd_tx_event_index) =
        ...
        loop {
            let requested_times =
                mock0.get_rtd_token_events_requested(rtd_tx_digest, rtd_tx_event_index);
    ...
    async fn test_skip_request_signature_if_already_processed_on_chain() {
        ...
        rtd_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Approved);
    ...
    async fn test_skip_tx_submission_if_already_processed_on_chain() {
        ...
        rtd_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Pending);
    ...
    async fn test_skip_tx_submission_if_bridge_is_paused() {
        ...
        let id_token_map: HashMap<u8, TypeTag> = (*rtd_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            None,
    ...
    async fn test_action_executor_handle_new_token() {
        ...
        let mut id_token_map: HashMap<u8, TypeTag> = (*rtd_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            Some(new_token_id),
            false, // we need an eth -> rtd action that entails the new token type tag in transaction building
        );
        ...
        rtd_token_type_tags.store(Arc::new(id_token_map));
    ...
    fn mock_bridge_authority_sigs(
        ...
        rtd_tx_digest: TransactionDigest,
        rtd_tx_event_index: u16,
    ...
    fn get_bridge_authority_approved_action(
        ...
        let rtd_tx_digest = TransactionDigest::random();
        let rtd_tx_event_index = 1;
        let action = if rtd_to_eth {
            ...
            get_test_eth_to_rtd_bridge_action(None, None, None, token_id)
        };

        let sigs =
            mock_bridge_authority_sigs(mocks, &action, secrets, rtd_tx_digest, rtd_tx_event_index);
        let certified_action = CertifiedBridgeAction::new_from_data_and_sig(
            action,
            BridgeCommitteeValiditySignInfo { signatures: sigs },
        );
    ...
    fn get_tx_digest(tx_data: TransactionData, dummy_rtd_key: &RtdKeyPair) -> TransactionDigest {
        let sig = Signature::new_secure(
            &IntentMessage::new(Intent::rtd_transaction(), &tx_data),
            dummy_rtd_key,
        );
    ...
    fn mock_transaction_response(
        rtd_client_mock: &RtdMockClient,
        tx_digest: TransactionDigest,
        status: RtdExecutionStatus,
        events: Option<Vec<RtdEvent>>,
        wildcard: bool,
    ) {
        ...
        if wildcard {
            rtd_client_mock.set_wildcard_transaction_response(Ok(response));
        } else {
            rtd_client_mock.add_transaction_response(tx_digest, Ok(response));
        }
    }
    ...
    fn mock_transaction_error(
        ...
        if wildcard {
            rtd_client_mock.set_wildcard_transaction_response(Err(error));
        } else {
            rtd_client_mock.add_transaction_response(tx_digest, Err(error));
        }
    }
    ...
    async fn setup() -> (
        ...
        let (rtd_address, kp): (_, fastcrypto::secp256k1::Secp256k1KeyPair) = get_key_pair();
        let rtd_key = RtdKeyPair::from(kp);
        let gas_object_ref = random_object_ref();
        let temp_dir = tempfile::tempdir().unwrap();
        let store = BridgeOrchestratorTables::new(temp_dir.path());
        let rtd_client_mock = RtdMockClient::default();
        let tx_subscription = rtd_client_mock.subscribe_to_requested_transactions();
        let rtd_client = Arc::new(RtdClient::new_for_testing(rtd_client_mock.clone()));
        ...
        let dummy_rtd_key = RtdKeyPair::from(dummy_kp);
        ...
        let rtd_token_type_tags = Arc::new(ArcSwap::new(Arc::new(rtd_token_type_tags)));
