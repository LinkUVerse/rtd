...
use crate::rtd_bridge_watchdog::rtd_bridge_status::RtdBridgeStatus;
...
pub async fn run_bridge_node(
    ...
    let rtd_chain_identifier = server_config
        .rtd_client
        .get_chain_identifier()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to get rtd chain identifier: {:?}", e))?;
    ...
    prometheus_registry
        .register(linku_metrics::bridge_uptime_metric(
            "bridge",
            metadata.version,
            &rtd_chain_identifier,
    ...
    if let Some(client_config) = client_config {
        let committee_keys_to_names =
            Arc::new(get_validator_names_by_pub_keys(&committee, &rtd_system).await);
...
async fn start_watchdog(
    ...
    let rtd_bridge_status = RtdBridgeStatus::new(
        rtd_client.clone(),
...
fn get_rtd_modules_to_watch(
    ...
    rtd_bridge_module_last_processed_event_id_override: Option<EventID>,
) -> HashMap<Identifier, Option<EventID>> {
    let rtd_bridge_modules = vec![
        BRIDGE_MODULE_NAME.to_owned(),
        BRIDGE_COMMITTEE_MODULE_NAME.to_owned(),
        BRIDGE_TREASURY_MODULE_NAME.to_owned(),
        BRIDGE_LIMITER_MODULE_NAME.to_owned(),
    ];
    if let Some(cursor) = rtd_bridge_module_last_processed_event_id_override {
        info!("Overriding cursor for rtd bridge modules to {:?}", cursor);
    ...
    let rtd_bridge_module_stored_cursor = store
        .get_rtd_event_cursors(&rtd_bridge_modules)
        .expect("Failed to get eth rtd event cursors from storage");
        ...
        .zip(rtd_bridge_module_stored_cursor)
    {
        if cursor.is_none() {
            info!(
                "No cursor found for rtd bridge module {} in storage or config override, query start from the beginning.",
                module_identifier
            );
        }
        rtd_modules_to_watch.insert(module_identifier.clone(), cursor);
    }
    rtd_modules_to_watch
}
...
mod tests {
    ...
    async fn test_starting_bridge_node() {
        ...
        let config = BridgeNodeConfig {
            ...
            rtd: RtdConfig {
                rtd_rpc_url: bridge_test_cluster.rtd_rpc_url(),
                rtd_bridge_chain_id: BridgeChainId::RtdCustom as u8,
    ...
    async fn test_starting_bridge_node_with_client() {
        ...
        let client_rtd_address = RtdAddress::from(kp.public());
        let sender_address = bridge_test_cluster.rtd_user_address();
        // send some gas to this address
        bridge_test_cluster
            .test_cluster
            .inner
            .transfer_rtd_must_exceed(sender_address, client_rtd_address, 1000000000)
            .await;
        ...
        let config = BridgeNodeConfig {
            ...
            rtd: RtdConfig {
                ...
                bridge_client_key_path: None,
                bridge_client_gas_object: None,
                rtd_bridge_module_last_processed_event_id_override: Some(EventID {
                    tx_digest: TransactionDigest::random(),
                    event_seq: 0,
                }),
            },
