...
use crate::types::{
    ...
    BridgeActionType, EmergencyAction, EthLog, EthToRtdBridgeAction, EvmContractUpgradeAction,
    LimitUpdateAction, RtdToEthBridgeAction, RtdToEthTokenTransfer,
};
...
#[rustfmt::skip]
gen_eth_events!(
    EthRtdBridge, EthRtdBridgeEvents, "abi/rtd_bridge.json",
...
impl EthBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthRtdBridgeEvents(event) => {
                match event {
                    EthRtdBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToRtdTokenBridgeV1::try_from(&event) {
                            Ok(bridge_event) => {
                                if bridge_event.rtd_adjusted_amount == 0 {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!(
                                    "Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToRtdTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}",
                                    event, e
                                )));
                            }
                        };

                        Some(BridgeAction::EthToRtdBridgeAction(EthToRtdBridgeAction {
                            eth_tx_hash,
                            eth_event_index,
                            eth_bridge_event: bridge_event,
                        }))
                    }
                    EthRtdBridgeEvents::TokensClaimedFilter(_event) => None,
                    EthRtdBridgeEvents::PausedFilter(_event) => None,
                    EthRtdBridgeEvents::UnpausedFilter(_event) => None,
                    EthRtdBridgeEvents::UpgradedFilter(_event) => None,
                    EthRtdBridgeEvents::InitializedFilter(_event) => None,
                    EthRtdBridgeEvents::ContractUpgradedFilter(_event) => None,
                    EthRtdBridgeEvents::EmergencyOperationFilter(_event) => None,
                }
            }
...
pub struct EthToRtdTokenBridgeV1 {
    pub nonce: u64,
    ...
    pub token_id: u8,
    pub rtd_adjusted_amount: u64,
}

impl TryFrom<&TokensDepositedFilter> for EthToRtdTokenBridgeV1 {
    type Error = BridgeError;
    fn try_from(event: &TokensDepositedFilter) -> BridgeResult<Self> {
        Ok(Self {
            nonce: event.nonce,
            rtd_chain_id: BridgeChainId::try_from(event.destination_chain_id)?,
            eth_chain_id: BridgeChainId::try_from(event.source_chain_id)?,
            rtd_address: RtdAddress::from_bytes(event.recipient_address.as_ref())?,
            eth_address: event.sender_address,
            token_id: event.token_id,
            rtd_adjusted_amount: event.rtd_adjusted_amount,
        })
    }
}
...
impl TryFrom<RtdToEthBridgeAction> for eth_rtd_bridge::Message {
    type Error = BridgeError;

    fn try_from(action: RtdToEthBridgeAction) -> BridgeResult<Self> {
        Ok(eth_rtd_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
            version: TOKEN_TRANSFER_MESSAGE_VERSION,
            nonce: action.rtd_bridge_event.nonce,
            chain_id: action.rtd_bridge_event.rtd_chain_id as u8,
            payload: action
                .as_payload_bytes()
...
impl TryFrom<RtdToEthTokenTransfer> for eth_rtd_bridge::Message {
    type Error = BridgeError;

    fn try_from(action: RtdToEthTokenTransfer) -> BridgeResult<Self> {
        Ok(eth_rtd_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
            version: TOKEN_TRANSFER_MESSAGE_VERSION,
            nonce: action.nonce,
            chain_id: action.rtd_chain_id as u8,
            payload: action
                .as_payload_bytes()
                .map_err(|e| BridgeError::Generic(format!("Failed to encode payload: {}", e)))?
                .into(),
...
impl From<ParsedTokenTransferMessage> for eth_rtd_bridge::Message {
    fn from(parsed_message: ParsedTokenTransferMessage) -> Self {
        eth_rtd_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
...
impl TryFrom<EmergencyAction> for eth_rtd_bridge::Message {
    type Error = BridgeError;
...
mod tests {
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_rtd_bridge::Message = action.try_into().unwrap();
        assert_eq!(
            message,
            eth_rtd_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_eth_message_conversion_update_limit_action_regression() -> anyhow::Result<()> {
        ...
        let action = LimitUpdateAction {
            nonce: 2,
            chain_id: BridgeChainId::EthSepolia,
            sending_chain_id: BridgeChainId::RtdTestnet,
    ...
    #[test]
    fn test_token_deposit_eth_log_to_rtd_bridge_event_regression() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        ...
        assert_eq!(
            event,
            EthBridgeEvent::EthRtdBridgeEvents(EthRtdBridgeEvents::TokensDepositedFilter(
                TokensDepositedFilter {
                    source_chain_id: 12,
                    nonce: 0,
                    destination_chain_id: 2,
                    token_id: 2,
                    rtd_adjusted_amount: 4200000000,
    ...
    #[test]
    fn test_0_rtd_amount_conversion_for_eth_event() {
        let e = EthBridgeEvent::EthRtdBridgeEvents(EthRtdBridgeEvents::TokensDepositedFilter(
            TokensDepositedFilter {
                source_chain_id: BridgeChainId::EthSepolia as u8,
                nonce: 0,
                destination_chain_id: BridgeChainId::RtdTestnet as u8,
                token_id: 2,
                rtd_adjusted_amount: 1,
                sender_address: EthAddress::random(),
                recipient_address: ethers::types::Bytes::from(
                    RtdAddress::random_for_testing_only().to_vec(),
                ),
            },
        ));
        assert!(
