    ...
    C: RtdClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        let (events_tx, events_rx) = linku_metrics::metered_channel::channel(
            ...
            &linku_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["rtd_events_queue"]),
        );

        let mut task_handles = vec![];
        for (module, cursor) in self.cursors {
            let metrics = self.metrics.clone();
            let events_rx_clone: linku_metrics::metered_channel::Sender<(
                Identifier,
                Vec<RtdEvent>,
            )> = events_tx.clone();
            let rtd_client_clone = self.rtd_client.clone();
    ...
    async fn run_event_listening_task(
        ...
        events_sender: linku_metrics::metered_channel::Sender<(Identifier, Vec<RtdEvent>)>,
        rtd_client: Arc<RtdClient<C>>,
        query_interval: Duration,
        metrics: Arc<BridgeMetrics>,
    ) {
        tracing::info!(?module, ?cursor, "Starting rtd events listening task");
        ...
        loop {
            ...
            let Ok(Ok(events)) = retry_with_max_elapsed_time!(
                rtd_client.query_events_by_module(BRIDGE_PACKAGE_ID, module.clone(), cursor),
                Duration::from_secs(120)
            ) else {
                tracing::error!("Failed to query events from rtd client after retry");
                continue;
            };
            ...
            if len != 0 {
                ...
                events_sender
                    ...
                    .await
                    .expect("All Rtd event channel receivers are closed");
                if let Some(next) = events.next_cursor {
                    cursor = Some(next);
                }
                tracing::info!(?module, ?cursor, "Observed {len} new Rtd events");
            }
        }
    }
}
...
mod tests {
    ...
    use rtd_json_rpc_types::EventPage;
    use rtd_types::{Identifier, digests::TransactionDigest, event::EventID};
    use tokio::time::timeout;

    #[tokio::test]
    async fn test_rtd_syncer_basic() -> anyhow::Result<()> {
        ...
        let (_handles, mut events_rx) = RtdSyncer::new(client, target_modules, metrics.clone())
            .run(interval)
            .await
            .unwrap();
        ...
        let mut event_1: RtdEvent = RtdEvent::random_for_testing();
        let package_id = BRIDGE_PACKAGE_ID;
        event_1.type_.address = package_id.into();
        event_1.type_.module = module_foo.clone();
        let module_foo_events_1: rtd_json_rpc_types::Page<RtdEvent, EventID> = EventPage {
        ...
        let mut event_2: RtdEvent = RtdEvent::random_for_testing();
    ...
    async fn assert_no_more_events(
        interval: Duration,
        events_rx: &mut linku_metrics::metered_channel::Receiver<(Identifier, Vec<RtdEvent>)>,
    ) {
