...
use rtd_json_rpc_types::RtdTransactionBlockDataAPI;
use rtd_json_rpc_types::RtdTransactionBlockResponseQuery;
use rtd_json_rpc_types::TransactionFilter;
use rtd_json_rpc_types::{
    RtdObjectDataOptions, RtdObjectResponseQuery, RtdTransactionBlockResponse,
    RtdTransactionBlockResponseOptions, TransactionBlockBytes,
};
...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    // let batch_responses: Vec<RtdTransactionBlockResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(RtdTransactionBlockResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, RtdTransactionBlockResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: RtdTransactionBlockResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(RtdTransactionBlockResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, RtdTransactionBlockResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let query = RtdTransactionBlockResponseQuery {
        options: Some(RtdTransactionBlockResponseOptions {
            show_input: true,
            show_effects: true,
            show_events: true,
            ..Default::default()
    ...
    let first_page = client
        ...
        .query_transaction_blocks(
            RtdTransactionBlockResponseQuery::default(),
            None,
            Some(5),
            false,
        )
    ...
    let address_txs_asc = client
        .read_api()
        .query_transaction_blocks(
            RtdTransactionBlockResponseQuery::new_with_filter(TransactionFilter::FromAddress(
    ...
    for tx_resp in tx.data {
        let response: RtdTransactionBlockResponse = client
            .read_api()
            .get_transaction_with_options(tx_resp.digest, RtdTransactionBlockResponseOptions::new())
            .await
            .unwrap();
...
async fn test_query_transaction_blocks() -> Result<(), anyhow::Error> {
    ...
    let rtd_type_args = type_args![GAS::type_tag()]?;
    let type_args = rtd_type_args
        .into_iter()
        .map(|ty| ty.try_into())
        .collect::<Result<Vec<_>, _>>()?;

    let rtd_call_args_1 = call_args!(coin.data.clone().unwrap().object_id, 10)?;
    ...
    let rtd_call_args_2 = call_args!(coin_2.data.clone().unwrap().object_id, 10)?;
    ...
    let move_call_query = RtdTransactionBlockResponseQuery::new_with_filter(filter);
    let tx = client
        .read_api()
        .query_transaction_blocks(move_call_query, None, Some(20), true)
        .await
        .unwrap();
    // verify that only 1 tx is returned and no RtdRpcInputError::ContainsDuplicates error
...
async fn test_display_transaction_block_with_empty_balance_changes() {
    ...
    let tx_block = client
        .get_transaction_block(
            *digest,
            Some(RtdTransactionBlockResponseOptions::full_content()),
        )
        .await
        .unwrap();
