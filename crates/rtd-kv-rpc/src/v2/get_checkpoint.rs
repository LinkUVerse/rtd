...
use rtd_rpc::proto::rtd::rpc::v2::{Checkpoint, GetCheckpointRequest, GetCheckpointResponse};
use rtd_rpc_api::{
...
pub async fn get_checkpoint(
    ...
    let summary: rtd_sdk_types::CheckpointSummary = checkpoint.summary.try_into()?;
    let signatures: rtd_sdk_types::ValidatorAggregatedSignature = checkpoint.signatures.into();
    message.merge(&summary, &read_mask);
    message.merge(signatures, &read_mask);

    if read_mask.contains(Checkpoint::CONTENTS_FIELD.name) {
        message.merge(
            rtd_sdk_types::CheckpointContents::try_from(checkpoint.contents)?,
            &read_mask,
        );
    }
        ...
        && let Some(url) = checkpoint_bucket
    {
        ...
        let checkpoint = rtd_types::full_checkpoint_content::Checkpoint::from(
