...
```cargo run --features "pg_backend" --bin rtd-graphql-rpc start-server --db-url <DB_URL>```

The order is important:
1. --features "pg_backend": This part tells Cargo to enable the pg_backend feature.
2. --bin rtd-graphql-rpc: This specifies which binary to run.
...
The graphql service is backed by a db based on the db schema in [rtd-indexer](../rtd-indexer/src/schema.rs). To spin up a local db, follow the instructions at [rtd-indexer](../rtd-indexer/README.md) until "Running standalone indexer".

If you have not created a db yet, you can do so as follows:
```sh
psql -U postgres
CREATE DATABASE rtd_indexer_v2;
```

You should be able to refer to the db url now:
`psql postgres://postgres:postgrespw@localhost:5432/rtd_indexer_v2`

With the new db, run the following commands (also under `rtd/crates/rtd-indexer`):
...
This will build rtd-graphql-rpc and start an IDE:
```
cargo run --bin rtd-graphql-rpc start-server [--rpc-url] [--db-url] [--port] [--host] [--config]
```

### Launching the server w/ indexer
For local dev, it might be useful to spin up an indexer as well. Instructions are at [Running standalone indexer](../rtd-indexer/README.md#running-standalone-indexer).

## Compatibility with json-rpc

`cargo run --bin rtd -- start --with-faucet --force-regenesis --with-indexer --pg-port 5432 --pg-db-name rtd_indexer_v2 --with-graphql`

`pnpm --filter @linku/graphql-transport test:e2e`

## Testing
The full gamut of graphql-specific tests are listed in the [rust.yml](../../.github/workflows/rust.yml).

To run the tests in `rtd-graphql-rpc`, you will need to have postgres running locally.
