...
use rtd_graphql_rpc::client::simple_client::GraphqlQueryVariable;
use rtd_graphql_rpc::client::ClientError;
use rtd_graphql_rpc::config::Limits;
use rtd_graphql_rpc::config::ServiceConfig;
use rtd_graphql_rpc::test_infra::cluster::prep_executor_cluster;
use rtd_graphql_rpc::test_infra::cluster::start_cluster;
...
use rtd_types::DEEPBOOK_ADDRESS;
...
async fn test_simple_client_simulator_cluster() {
    ...
    let cluster = rtd_graphql_rpc::test_infra::cluster::serve_executor(
        Arc::new(sim),
        None,
        None,
...
async fn test_graphql_client_variables() {
    ...
    assert_eq!(
        data.get("obj1")
            ...
            .as_str()
            .unwrap(),
        RTD_FRAMEWORK_ADDRESS.to_canonical_string(true)
    );
    assert_eq!(
        data.get("obj2")
            .unwrap()
...
async fn test_payload_using_vars_mutation_passes() {
    telemetry_subscribers::init_for_testing();
    let cluster = rtd_graphql_rpc::test_infra::cluster::start_cluster(ServiceConfig {
        limits: Limits {
