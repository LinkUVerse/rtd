...
use crate::{
    ...
    types::query::{Query, RtdGraphQLSchema},
};
...
use linku_network::callback::{CallbackLayer, MakeCallbackHandler, ResponseHandler};
use std::convert::Infallible;
use std::net::TcpStream;
use std::sync::Arc;
use std::time::Duration;
use std::{any::Any, net::SocketAddr, time::Instant};
use rtd_graphql_rpc_headers::LIMITS_HEADER;
use rtd_indexer::db::check_db_migration_consistency;
...
impl ServerBuilder {
    ...
    pub async fn from_config(
        ...
        let registry_service = linku_metrics::start_prometheus_server(prom_addr);
        info!("Starting Prometheus HTTP endpoint at {}", prom_addr);
        let registry = registry_service.default_registry();
        registry
            .register(linku_metrics::uptime_metric(
                "graphql",
                version.full,
                "unknown",
            ))
        ...
        let rtd_sdk_client = if let Some(url) = &config.tx_exec_full_node.node_rpc_url {
            Some(
                RtdClientBuilder::default()
                    ...
                    .build(url)
                    .await
                    .map_err(|e| Error::Internal(format!("Failed to create RtdClient: {}", e)))?,
            )
        } else {
...
async fn graphql_handler(
    ...
    schema: Extension<RtdGraphQLSchema>,
...
pub mod tests {
    ...
    use rtd_types::digests::get_mainnet_chain_identifier;
    ...
    fn metrics() -> Metrics {
        let binding_address: SocketAddr = format!("127.0.0.1:{}", get_available_port())
            .parse()
            .unwrap();
        let registry = linku_metrics::start_prometheus_server(binding_address).default_registry();
    ...
    async fn test_timeout() {
        ...
        cluster
            ...
            .await;
        // timeout test includes mutation timeout, which requires a [RtdClient] to be able to run
        ...
        async fn test_timeout(
            ...
            let schema = prep_schema(db_url, Some(cfg))
                .await
                .context_data(Some(rtd_client.clone()))
                .extension(Timeout)
        ...
        let rtd_client = wallet.get_client().await.unwrap();

        test_timeout(delay, timeout, query, &rtd_client, db_url.clone())
            .await
            .into_result()
            .expect("Should complete successfully");

        // Should timeout
        let errs: Vec<_> = test_timeout(delay, delay, query, &rtd_client, db_url.clone())
            .await
            .into_result()
            .unwrap_err()
            .into_iter()
