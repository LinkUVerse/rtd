...
use rtd_graphql_rpc_client::simple_client::SimpleClient;
pub use rtd_indexer::config::RetentionConfig;
pub use rtd_indexer::config::SnapshotLagConfig;
use rtd_indexer::errors::IndexerError;
use rtd_indexer::store::PgIndexerStore;
use rtd_indexer::test_utils::start_indexer_writer_for_testing;
use rtd_pg_db::temp::{get_available_port, TempDb};
...
/// Takes in a simulated instantiation of a Rtd blockchain and builds a cluster around it. This
...
pub async fn serve_executor(
    ...
    let executor_server_handle = tokio::spawn(async move {
        rtd_rpc_api::RpcService::new(executor)
