...
use super::stake::StakedRtdDowncastError;
use super::rtd_address::RtdAddress;
use super::rtdns_registration::{DomainFormat, RtdnsRegistration, RtdnsRegistrationDowncastError};
...
use crate::types::stake::StakedRtd;
use async_graphql::connection::Connection;
use async_graphql::*;
use rtd_name_service::NameServiceConfig;
use rtd_types::object::{Data, MoveObject as NativeMoveObject};
...
#[graphql(
    ...
    field(
        ...
        desc = "Determines whether a transaction can transfer this object, using the \
                TransferObjects transaction command or `rtd::transfer::public_transfer`, both of \
...
pub(crate) enum IMoveObject {
    ...
    StakedRtd(StakedRtd),
    RtdnsRegistration(RtdnsRegistration),
}
...
impl MoveObject {
    ...
    /// Attempts to convert the Move object into a `0x3::staking_pool::StakedRtd`.
    async fn as_staked_rtd(&self) -> Result<Option<StakedRtd>> {
        match StakedRtd::try_from(self) {
            Ok(coin) => Ok(Some(coin)),
            Err(StakedRtdDowncastError::NotAStakedRtd) => Ok(None),
            Err(StakedRtdDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize StakedRtd: {e}"
            )))
            .extend(),
        }
    }
    ...
    /// Attempts to convert the Move object into a `RtdnsRegistration` object.
    async fn as_rtdns_registration(&self, ctx: &Context<'_>) -> Result<Option<RtdnsRegistration>> {
        let cfg: &NameServiceConfig = ctx.data_unchecked();
        let tag = RtdnsRegistration::type_(cfg.package_address.into());

        match RtdnsRegistration::try_from(self, &tag) {
            Ok(registration) => Ok(Some(registration)),
            Err(RtdnsRegistrationDowncastError::NotARtdnsRegistration) => Ok(None),
            Err(RtdnsRegistrationDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize RtdnsRegistration: {e}",
            )))
            .extend(),
        }
    }
