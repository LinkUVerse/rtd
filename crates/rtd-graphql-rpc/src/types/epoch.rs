...
use rtd_indexer::models::epoch::QueryableEpochInfo;
use rtd_indexer::schema::epochs;
use rtd_types::messages_checkpoint::CheckpointCommitment as EpochCommitment;
...
/// Operation of the Rtd network is temporally partitioned into non-overlapping epochs,
...
impl Epoch {
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>> {
        let system_state = ctx
            ...
            .fetch_rtd_system_state(Some(self.stored.epoch as u64))
            .await?;
