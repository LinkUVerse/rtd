...
use rtd_indexer::models::{events::StoredEvent, transactions::StoredTransaction};
use rtd_indexer::schema::{checkpoints, events};
use rtd_types::base_types::ObjectID;
use rtd_types::Identifier;
use rtd_types::{
    base_types::RtdAddress as NativeRtdAddress, event::Event as NativeEvent, parse_rtd_struct_tag,
};
...
/// A Rtd node emits one of the following events:
/// Move event
/// Publish event
...
impl Event {
    ...
    async fn sender(&self) -> Result<Option<Address>> {
        if self.native.sender == NativeRtdAddress::ZERO {
            return Ok(None);
        }
...
impl Event {
    ...
    fn try_from_stored_event(
        ...
        let sender = NativeRtdAddress::from_bytes(sender_bytes)
            .map_err(|e| Error::Internal(e.to_string()))?;
        let package_id =
            ObjectID::from_bytes(&stored.package).map_err(|e| Error::Internal(e.to_string()))?;
        let type_ =
            parse_rtd_struct_tag(&stored.event_type).map_err(|e| Error::Internal(e.to_string()))?;
