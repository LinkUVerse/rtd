...
use super::rtdns_registration::{DomainFormat, RtdnsRegistration};
use super::transaction_block::{self, TransactionBlock, TransactionBlockFilter};
use super::type_filter::ExactTypeFilter;
use super::uint53::UInt53;
use super::{
    big_int::BigInt, epoch::Epoch, move_object::MoveObject, object, rtd_address::RtdAddress,
};
use async_graphql::connection::Connection;
use async_graphql::*;
use move_core_types::language_storage::StructTag;
use rtd_json_rpc_types::{Stake as RpcStakedRtd, StakeStatus as RpcStakeStatus};
use rtd_types::base_types::MoveObjectType;
use rtd_types::governance::StakedRtd as NativeStakedRtd;
...
pub(crate) enum StakedRtdDowncastError {
    NotAStakedRtd,
    Bcs(bcs::Error),
}

#[derive(Clone)]
pub(crate) struct StakedRtd {
    /// Representation of this StakedRtd as a generic Move Object.
    pub super_: MoveObject,

    /// Deserialized representation of the Move Object's contents as a
    /// `0x3::staking_pool::StakedRtd`.
    pub native: NativeStakedRtd,
}

/// Represents a `0x3::staking_pool::StakedRtd` Move object on-chain.
#[Object]
impl StakedRtd {
    ...
    async fn pool_id(&self) -> Option<RtdAddress> {
        Some(self.native.pool_id().into())
    }

    /// The RTD that was initially staked.
...
impl StakedRtd {
    /// Query the database for a `page` of Staked RTD. The page uses the same cursor type as is used
    ...
    ) -> Result<Connection<String, StakedRtd>, Error> {
        let type_: StructTag = MoveObjectType::staked_rtd().into();
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedRtd, but it's not a Move Object.",
                ))
            })?;

            StakedRtd::try_from(&move_object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedRtd, but it is not."
                ))
            })
        })
        .await
    ...
    /// The JSON-RPC representation of a StakedRtd so that we can "cheat" to implement fields that
    /// are not yet implemented directly for GraphQL.
    ///
    /// TODO: Make this obsolete
    async fn rpc_stake(&self, ctx: &Context<'_>) -> Result<RpcStakedRtd, Error> {
        ctx.data_unchecked::<PgManager>()
            .fetch_rpc_staked_rtd(self.native.clone())
            .await
    }
}

impl TryFrom<&MoveObject> for StakedRtd {
    type Error = StakedRtdDowncastError;

    fn try_from(move_object: &MoveObject) -> Result<Self, Self::Error> {
        if !move_object.native.is_staked_rtd() {
            return Err(StakedRtdDowncastError::NotAStakedRtd);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(StakedRtdDowncastError::Bcs)?,
        })
    }
}
