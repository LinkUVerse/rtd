...
use rtd_types::dynamic_field::visitor::{Field, FieldVisitor};
use rtd_types::dynamic_field::{derive_dynamic_field_id, DynamicFieldInfo, DynamicFieldType};
...
use super::{
    base64::Base64, move_object::MoveObject, move_value::MoveValue, rtd_address::RtdAddress,
};
...
/// 2) Dynamic Object Fields values must be Rtd objects (have the `key` and `store`
...
impl DynamicField {
    ...
    async fn value(&self, ctx: &Context<'_>) -> Result<Option<DynamicFieldValue>> {
        ...
        if kind == DynamicFieldType::DynamicObject {
            let df_object_id: RtdAddress = bcs::from_bytes(value_bytes)
...
fn apply_filter(query: RawQuery, parent: RtdAddress, parent_version: Option<u64>) -> RawQuery {
    let query = filter!(
        query,
        format!(
