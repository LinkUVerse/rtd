...
use rtd_name_service::{validate_label, Domain};
use rtd_types::{
...
pub(crate) struct AppInfo {
    ...
    pub(crate) package_address: Option<RtdAddress>,
...
impl Name {
    ...
    pub(crate) fn type_(package_address: RtdAddress) -> StructTag {
        StructTag {
    ...
    pub(crate) fn to_dynamic_field_id(
        ...
        rtd_types::dynamic_field::derive_dynamic_field_id(
            config.registry_id,
            &rtd_types::TypeTag::Struct(Box::new(domain_type_tag)),
            &self.to_bytes(),
        )
    }
}
