...
use crate::types::rtd_address::addr;
...
use rtd_indexer::models::objects::StoredFullHistoryObject;
use rtd_indexer::schema::packages;
use rtd_package_resolver::{error::Error as PackageCacheError, Package as ParsedMovePackage};
use rtd_types::is_system_package;
use rtd_types::{move_package::MovePackage as NativeMovePackage, object::Data};
...
impl MovePackage {
    ...
    /// The `0x3::staking_pool::StakedRtd` objects owned by this package.
    ///
    /// Note that objects owned by a package are inaccessible, because packages are immutable and
    /// cannot be owned by an address.
    ...
    /// The RtdnsRegistration NFTs owned by this package. These grant the owner the capability to
...
impl MovePackage {
    ...
    pub(crate) fn try_from_serialized(
        ...
        let object = Object::new_serialized(
            RtdAddress::from_bytes(&history_object.object_id)
...
impl Loader<PackageVersionKey> for Db {
    type Value = RtdAddress;
    type Error = Error;

    async fn load(
        &self,
        keys: &[PackageVersionKey],
    ) -> Result<HashMap<PackageVersionKey, RtdAddress>, Error> {
        use packages::dsl;
...
impl Loader<LatestKey> for Db {
    ...
    async fn load(&self, keys: &[LatestKey]) -> Result<HashMap<LatestKey, RtdAddress>, Error> {
        use packages::dsl;
        let other = diesel::alias!(packages as other);
