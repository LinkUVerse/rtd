...
use rtd_indexer::models::objects::StoredHistoryObject;
use rtd_name_service::{Domain as NativeDomain, NameRecord, NameServiceConfig, NameServiceError};
use rtd_types::{base_types::RtdAddress as NativeRtdAddress, dynamic_field::Field, id::UID};

const MOD_REGISTRATION: &IdentStr = ident_str!("rtdns_registration");
const TYP_REGISTRATION: &IdentStr = ident_str!("RtdnsRegistration");
...
/// Wrap RtdNS Domain type to expose as a string scalar in GraphQL.
#[derive(Debug)]
pub(crate) struct Domain(NativeDomain);

#[derive(Enum, Copy, Clone, Eq, PartialEq)]
#[graphql(remote = "rtd_name_service::DomainFormat")]
...
pub(crate) struct NativeRtdnsRegistration {
    pub id: UID,
...
#[derive(Clone)]
pub(crate) struct RtdnsRegistration {
    /// Representation of this RtdnsRegistration as a generic Move object.
    pub super_: MoveObject,

    /// The deserialized representation of the Move object's contents.
    pub native: NativeRtdnsRegistration,
}
...
pub(crate) enum RtdnsRegistrationDowncastError {
    NotARtdnsRegistration,
    Bcs(bcs::Error),
}

#[Object]
impl RtdnsRegistration {
    pub(crate) async fn address(&self) -> RtdAddress {
    ...
    ///
    ///`type` is a filter on the coin's type parameter, defaulting to `0x2::rtd::RTD`.
    ...
    /// The `0x3::staking_pool::StakedRtd` objects owned by this object.
    pub(crate) async fn staked_rtds(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, StakedRtd>> {
        OwnerImpl::from(&self.super_.super_)
            .staked_rtds(ctx, first, after, last, before)
            .await
    }
    ...
    pub(crate) async fn default_rtdns_name(
        ...
        OwnerImpl::from(&self.super_.super_)
            .default_rtdns_name(ctx, format)
            .await
    }

    /// The RtdnsRegistration NFTs owned by this object. These grant the owner the capability to
    /// manage the associated domain.
    pub(crate) async fn rtdns_registrations(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, RtdnsRegistration>> {
        OwnerImpl::from(&self.super_.super_)
            .rtdns_registrations(ctx, first, after, last, before)
            .await
    }
    ...
    /// The amount of RTD we would rebate if this object gets deleted or mutated. This number is
    ...
    /// transaction command or `rtd::transfer::public_transfer`, both of which require the object to
    ...
    /// Domain name of the RtdnsRegistration object
...
impl NameService {
    /// Lookup the RtdNS NameRecord for the given `domain` name. `config` specifies where to find
    ...
    /// Lookup the RtdNS Domain for the given `address`. `config` specifies where to find the domain
    ...
    pub(crate) async fn reverse_resolve_to_name(
        ...
        let field: Field<NativeRtdAddress, NativeDomain> = object
            .native
            .to_rust()
            .ok_or_else(|| Error::Internal("Malformed Rtdns Domain".to_string()))?;
    ...
    async fn query_domain_expiration(
        ...
        // parent's `NameRecord`.
        let mut object_ids = vec![RtdAddress::from(config.record_field_id(&domain.0))];
        if domain.0.is_subdomain() {
            object_ids.push(RtdAddress::from(config.record_field_id(&domain.0.parent())));
        }
...
impl RtdnsRegistration {
    /// Query the database for a `page` of RtdNS registrations. The page uses the same cursor type
    ...
    pub(crate) async fn paginate(
        ...
        owner: RtdAddress,
        checkpoint_viewed_at: u64,
    ) -> Result<Connection<String, RtdnsRegistration>, Error> {
        let type_ = RtdnsRegistration::type_(config.package_address.into());
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a RtdnsRegistration, but it's not a Move Object.",
                ))
            })?;

            RtdnsRegistration::try_from(&move_object, &type_).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a RtdnsRegistration, but it is not."
                ))
            })
        })
        .await
    ...
    /// Return the type representing a `RtdnsRegistration` on chain. This can change from chain to
    /// chain (mainnet, testnet, devnet etc).
    pub(crate) fn type_(package: RtdAddress) -> StructTag {
        StructTag {
    ...
    // Because the type of the RtdnsRegistration object is not constant,
    // we need to take it in as a param.
    pub(crate) fn try_from(
        move_object: &MoveObject,
        tag: &StructTag,
    ) -> Result<Self, RtdnsRegistrationDowncastError> {
        if !move_object.native.is_type(tag) {
            return Err(RtdnsRegistrationDowncastError::NotARtdnsRegistration);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(RtdnsRegistrationDowncastError::Bcs)?,
        })
    }
}
