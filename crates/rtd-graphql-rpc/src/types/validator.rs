...
use rtd_indexer::apis::GovernanceReadApi;
...
use async_graphql::*;
use rtd_indexer::apis::governance_api::exchange_rates;
use rtd_types::rtd_system_state::rtd_system_state_summary::RtdValidatorSummary as NativeRtdValidatorSummary;
#[derive(Clone, Debug)]
pub(crate) struct Validator {
    pub validator_summary: NativeRtdValidatorSummary,
...
impl Loader<u64> for Db {
    type Value = (
        ...
        BTreeMap<NativeRtdAddress, Vec<(EpochId, PoolTokenExchangeRate)>>,
    );
    type Error = Error;
    ...
    async fn load(
        ...
        Error,
    > {
        let latest_rtd_system_state = self
            .inner
            .get_latest_rtd_system_state()
            .await
            .map_err(|_| Error::Internal("Failed to fetch latest Rtd system state".to_string()))?;
        let governance_api = GovernanceReadApi::new(self.inner.clone());
        let exchange_rates = exchange_rates(&governance_api, &latest_rtd_system_state)
            .await
        ...
        let epoch_to_filter_out = if let Some(epoch) = keys.first() {
            if epoch == &latest_rtd_system_state.epoch {
                *epoch - 1
            } else {
                *epoch
            }
        } else {
            latest_rtd_system_state.epoch - 1
        };
        ...
        let requested_epoch = match keys.first() {
            Some(x) => *x,
            None => latest_rtd_system_state.epoch,
        };

        let mut r = HashMap::new();
        r.insert(
            requested_epoch,
            (latest_rtd_system_state.stake_subsidy_start_epoch, results),
        );
...
impl Validator {
    ...
    async fn address(&self) -> Address {
        Address {
            address: RtdAddress::from(self.validator_summary.rtd_address),
    ...
    async fn staking_pool_id(&self) -> RtdAddress {
    ...
    /// the amount of RTD tokens that each past RTD staker can withdraw in the future.
    #[graphql(
    ...
    /// of RTD tokens that each past RTD staker can withdraw in the future.
    ...
    async fn staking_pool_rtd_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.staking_pool_rtd_balance,
        ))
    }
    ...
    async fn pending_total_rtd_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.pending_total_rtd_withdraw,
        ))
    }
    ...
    /// The total number of RTD tokens in this pool plus
    ...
    async fn apy(&self, ctx: &Context<'_>) -> Result<Option<u64>, Error> {
        ...
        let rates = exchange_rates
            .get(&self.validator_summary.rtd_address)
            .ok_or_else(|| {
                Error::Internal(format!(
                    "Failed to get the exchange rate for this validator address {} for requested epoch {}",
                    self.validator_summary.rtd_address, self.requested_for_epoch
                ))
            })?;
...
impl Validator {
    pub fn operation_cap_id(&self) -> RtdAddress {
        RtdAddress::from_array(**self.validator_summary.operation_cap_id)
    }
}
