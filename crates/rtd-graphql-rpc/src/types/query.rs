...
use rtd_types::{gas_coin::GAS, transaction::TransactionDataAPI, TypeTag};
...
use super::rtdns_registration::NameService;
...
pub(crate) type RtdGraphQLSchema = async_graphql::Schema<Query, Mutation, EmptySubscription>;
...
impl Query {
    ...
    async fn dry_run_transaction_block(
        ...
        let rtd_sdk_client: &Option<RtdClient> = ctx
            .data()
            .map_err(|_| Error::Internal("Unable to fetch Rtd SDK client".to_string()))
            .extend()?;
        let rtd_sdk_client = rtd_sdk_client
            .as_ref()
            .ok_or_else(|| Error::Internal("Rtd SDK client not initialized".to_string()))
            .extend()?;
    ...
    /// Look up an Owner by its RtdAddress.
    ///
    ...
    ///
    /// If `rootVersion` is omitted, dynamic fields will be from a consistent snapshot of the Rtd
    ...
    async fn address(&self, ctx: &Context<'_>, address: RtdAddress) -> Result<Option<Address>> {
        let Watermark { hi_cp, .. } = *ctx.data()?;
    ...
    /// `0x2::rtd::RTD`). If no type is provided, it will default to `0x2::rtd::RTD`.
    async fn coins(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
    ...
    /// Resolves a RtdNS `domain` name to an address, if it has been bound.
    async fn resolve_rtdns_address(
        &self,
        ctx: &Context<'_>,
        domain: Domain,
