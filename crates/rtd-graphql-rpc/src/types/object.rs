...
use super::stake::StakedRtd;
use super::rtd_address::addr;
...
use super::{owner::Owner, rtd_address::RtdAddress, transaction_block::TransactionBlock};
...
use rtd_indexer::models::obj_indices::StoredObjectVersion;
use rtd_indexer::models::objects::{StoredFullHistoryObject, StoredHistoryObject};
use rtd_indexer::schema::{full_objects_history, objects_version};
use rtd_indexer::types::ObjectStatus as NativeObjectStatus;
use rtd_indexer::types::OwnerType;
use rtd_types::object::{Object as NativeObject, Owner as NativeOwner};
...
pub(crate) struct ObjectFilter {
    ...
    /// the full type name, such as `0x2::coin::Coin<0x2::rtd::RTD>`.
    pub type_: Option<TypeFilter>,

    /// Filter for live objects by their current owners.
    pub owner: Option<RtdAddress>,

    /// Filter for live objects by their IDs.
    pub object_ids: Option<Vec<RtdAddress>>,
}

#[derive(InputObject, Debug, Clone, Eq, PartialEq)]
pub(crate) struct ObjectKey {
    pub object_id: RtdAddress,
    pub version: UInt53,
}
...
/// An object in Rtd is a package (set of Move bytecode modules) or object (typed data structure
...
impl Object {
    ...
    pub(crate) async fn default_rtdns_name(
        ...
        OwnerImpl::from(self).default_rtdns_name(ctx, format).await
    }
    ...
    pub(crate) async fn owner(&self) -> Option<ObjectOwner> {
        ...
        match &native.owner {
            O::AddressOwner(address) => {
                let address = RtdAddress::from(*address);
...
mod tests {
    ...
    fn test_owner_filter_intersection() {
        let f0 = ObjectFilter {
            owner: Some(RtdAddress::from_str("0x1").unwrap()),
            ..Default::default()
        };

        let f1 = ObjectFilter {
            owner: Some(RtdAddress::from_str("0x2").unwrap()),
            ..Default::default()
        };
    ...
    fn test_object_filter_intersection() {
        let i1 = RtdAddress::from_str("0x1").unwrap();
        let i2 = RtdAddress::from_str("0x2").unwrap();
        let i3 = RtdAddress::from_str("0x3").unwrap();
