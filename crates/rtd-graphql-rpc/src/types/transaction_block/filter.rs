...
use crate::types::{digest::Digest, rtd_address::RtdAddress, type_filter::FqNameFilter};
...
pub(crate) struct TransactionBlockFilter {
    ...
    pub affected_address: Option<RtdAddress>,
    ...
    #[cfg(feature = "staging")]
    pub affected_object: Option<RtdAddress>,

    /// Limit to transactions that were sent by the given address.
    pub sent_address: Option<RtdAddress>,
    ...
    pub input_object: Option<RtdAddress>,
    ...
    /// This filter will be removed with 1.36.0 (2024-10-14), or at least one release after
    /// `affectedObject` is introduced, whichever is later.
    pub changed_object: Option<RtdAddress>,
...
impl TransactionBlockFilter {
    ...
    pub(crate) fn explicit_sender(&self) -> Option<RtdAddress> {
            ...
            || matches!(
                ...
                (Some(kind), Some(signer))
                    if (kind == TransactionBlockKindInput::SystemTx)
                        != (signer == RtdAddress::from(NativeRtdAddress::ZERO))
            )
    }
}
