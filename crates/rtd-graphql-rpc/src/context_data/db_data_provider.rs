...
use crate::{
    error::Error,
    types::{address::Address, rtd_address::RtdAddress, validator::Validator},
};
use std::{collections::BTreeMap, time::Duration};
use rtd_indexer::db::ConnectionPoolConfig;
use rtd_indexer::{apis::GovernanceReadApi, indexer_reader::IndexerReader};
use rtd_json_rpc_types::Stake as RpcStakedRtd;
use rtd_types::{
    governance::StakedRtd as NativeStakedRtd,
    rtd_system_state::rtd_system_state_summary::RtdSystemStateSummary as NativeRtdSystemStateSummary,
};
...
impl PgManager {
    ...
    /// returns the latest rtd system state.
    pub(crate) async fn fetch_rtd_system_state(
        &self,
        epoch_id: Option<u64>,
    ) -> Result<NativeRtdSystemStateSummary, Error> {
        let latest_rtd_system_state = self.inner.get_latest_rtd_system_state().await?;

        if let Some(epoch_id) = epoch_id {
            if epoch_id == latest_rtd_system_state.epoch {
                Ok(latest_rtd_system_state)
            } else {
                Ok(self
                    .inner
                    .get_epoch_rtd_system_state(Some(epoch_id))
                    .await?)
            }
        } else {
            Ok(latest_rtd_system_state)
    ...
    /// Make a request to the RPC for its representations of the staked rtd we parsed out of the
    /// object.  Used to implement fields that are implemented in JSON-RPC but not GraphQL (yet).
    pub(crate) async fn fetch_rpc_staked_rtd(
        &self,
        stake: NativeStakedRtd,
    ) -> Result<RpcStakedRtd, Error> {
...
/// `RtdValidatorSummary` was queried for. Each `Validator` will inherit this checkpoint, so that
/// when viewing the `Validator`'s state, it will be as if it was read at the same checkpoint.
pub(crate) fn convert_to_validators(
    system_state_at_requested_epoch: NativeRtdSystemStateSummary,
    ...
    system_state_at_requested_epoch
        ...
        .map(move |validator_summary| {
            let at_risk = at_risk.get(&validator_summary.rtd_address).copied();
            let report_records = reports.get(&validator_summary.rtd_address).map(|addrs| {
                addrs
                    .iter()
                    .cloned()
                    .map(|a| Address {
                        address: RtdAddress::from(a),
