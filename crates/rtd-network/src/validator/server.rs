    ...
    pub async fn bind(self, addr: &Multiaddr, tls_config: Option<ServerConfig>) -> Result<Server> {
        ...
        let route_layers = ServiceBuilder::new()
            ...
            .layer_fn(move |service| {
                linku_network::grpc_timeout::GrpcTimeout::new(service, request_timeout)
            });

        let mut builder = rtd_http::Builder::new().config(http_config);
...
/// TLS server name to use for the public Rtd validator interface.
pub const RTD_TLS_SERVER_NAME: &str = "rtd";

pub struct Server {
    server: rtd_http::ServerHandle,
...
impl Server {
    ...
    pub fn handle(&self) -> &rtd_http::ServerHandle {
        &self.server
    }
}
...
mod test {
    ...
    use linku_network::metrics::MetricsCallbackProvider;
    use std::ops::Deref;
    ...
    async fn test_metrics_layer_successful() {
        ...
        let server = super::ServerBuilder::from_config(&config, metrics.clone())
            .bind(
                &address,
                Some(rtd_tls::create_rustls_server_config(
