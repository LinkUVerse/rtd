...
use rtd_types::crypto::{AccountKeyPair, AggregateAuthoritySignature, get_key_pair_from_rng};
use rtd_types::digests::TransactionEventsDigest;
use rtd_types::dynamic_field::{DynamicFieldInfo, DynamicFieldName, DynamicFieldType};
...
use rtd_types::transaction::{CallArg, TransactionData};
use rtd_types::utils::to_sender_signed_transaction;
use rtd_types::{RTD_FRAMEWORK_PACKAGE_ID, parse_rtd_struct_tag};
...
impl RpcExampleProvider {
    ...
    pub fn examples(&mut self) -> BTreeMap<String, Vec<ExamplePairing>> {
        [
            ...
            self.rtd_get_committee_info(),
            self.rtd_get_reference_gas_price(),
            self.rtdx_get_all_balances(),
            self.rtdx_get_all_coins(),
            self.rtdx_get_balance(),
            self.rtdx_get_coin_metadata(),
            self.rtd_get_latest_checkpoint_sequence_number(),
            self.rtdx_get_coins(),
            self.rtdx_get_total_supply(),
            self.rtdx_get_dynamic_fields(),
            self.rtdx_get_dynamic_field_object(),
            self.rtdx_get_owned_objects(),
            self.rtd_get_move_function_arg_types(),
            self.rtd_get_normalized_move_function(),
            self.rtd_get_normalized_move_module(),
            self.rtd_get_normalized_move_modules_by_package(),
            self.rtd_get_normalized_move_struct(),
            self.multi_get_objects_example(),
            self.multi_get_transaction_blocks(),
            self.rtdx_get_validators_apy(),
            self.rtdx_get_dynamic_fields(),
            self.rtdx_get_dynamic_field_object(),
            self.rtdx_get_owned_objects(),
            self.rtdx_query_events(),
            ...
            self.rtdx_resolve_name_service_names(),
            self.rtd_try_multi_get_past_objects(),
        ]
        .into_iter()
    ...
    fn batch_transaction_examples(&mut self) -> Examples {
        let signer = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
        let recipient = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
        ...
        let tx_params = vec![
            RPCTransactionRequestParams::MoveCallRequestParams(MoveCallParams {
                package_object_id: RTD_FRAMEWORK_PACKAGE_ID,
                module: "pay".to_string(),
                function: "split".to_string(),
                type_arguments: vec![RtdTypeTag::new("0x2::rtd::RTD".to_string())],
                arguments: vec![
                    RtdJsonValue::new(json!(coin_ref.0)).unwrap(),
                    RtdJsonValue::new(json!(random_amount)).unwrap(),
                ],
            }),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "options",
                        json!(RtdTransactionBlockResponseOptions::full_content()),
                    ),
                    (
                        "request_type",
    ...
    fn dev_inspect_transaction_block(&mut self) -> Examples {
        ...
        let dev_inspect_results = DevInspectResults {
            ...
            events: RtdTransactionBlockEvents { data: vec![] },
            results: None,
            error: None,
            raw_txn_data: vec![],
            raw_effects: vec![],
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    (
                        "sender_address",
                        json!(RtdAddress::from(ObjectID::new(self.rng.r#gen()))),
                    ),
                    ("tx_bytes", json!(tx_bytes.tx_bytes)),
                    ("gas_price", json!(1000)),
                    ("epoch", json!(8888)),
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    ("options", json!(RtdObjectDataOptions::full_content())),
                ],
                json!(objects),
            )],
        )
    ...
    fn get_object_responses(&mut self, object_count: usize) -> Vec<RtdObjectResponse> {
        (0..object_count)
            .map(|_| {
                let object_id = ObjectID::new(self.rng.r#gen());
                let coin = GasCoin::new(object_id, 100000000);

                RtdObjectResponse::new_with_data(RtdObjectData {
                    content: Some(
                        RtdParsedData::try_from_object(
                            coin.to_object(SequenceNumber::from_u64(1)),
                            GasCoin::layout(),
                        )
                        .unwrap(),
                    ),
                    owner: Some(Owner::AddressOwner(RtdAddress::from(ObjectID::new(
                        self.rng.r#gen(),
                    )))),
    ...
    fn get_past_object_example(&mut self) -> Examples {
        ...
        let result = RtdPastObjectResponse::VersionFound(RtdObjectData {
            content: Some(
                RtdParsedData::try_from_object(
                    coin.to_object(SequenceNumber::from_u64(1)),
                    GasCoin::layout(),
    ...
    fn get_owned_objects(&mut self) -> Examples {
        let owner = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
        let result = (0..4)
            .map(|_| RtdObjectData {
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    ...
                    (
                        "query",
                        json!(RtdObjectResponseQuery {
                            filter: Some(RtdObjectDataFilter::StructType(
                                StructTag::from_str("0x2::coin::Coin<0x2::rtd::RTD>").unwrap()
                            )),
                            options: Some(
    ...
    fn query_transaction_blocks(&mut self) -> Examples {
        ...
        let data = data
            .into_iter()
            .map(RtdTransactionBlockResponse::new)
            .collect();
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                ...
                vec![
                    (
                        "query",
                        json!(RtdTransactionBlockResponseQuery {
    ...
    fn get_transfer_data_response(
        ...
        let object_change = ObjectChange::Transferred {
            ...
            object_type: parse_rtd_struct_tag("0x2::example::Object").unwrap(),
        ...
        let result = RtdTransactionBlockResponse {
            digest: *tx_digest,
            effects: Some(RtdTransactionBlockEffects::V1(
                RtdTransactionBlockEffectsV1 {
                    status: RtdExecutionStatus::Success,
                    executed_epoch: 0,
                    ...
                    gas_object: OwnedObjectRef {
                        ...
                        reference: RtdObjectRef::from(gas_ref),
                    },
            ...
            timestamp_ms: None,
            transaction: Some(RtdTransactionBlock {
    ...
    fn get_events(&mut self) -> Examples {
        ...
        let event = RtdEvent {
            ...
            sender: RtdAddress::from(ObjectID::new(self.rng.r#gen())),
            type_: parse_rtd_struct_tag("0x9::test::TestEvent").unwrap(),
    ...
    fn rtd_get_reference_gas_price(&mut self) -> Examples {
        let result = 1000;
        Examples::new(
            "rtdx_getReferenceGasPrice",
            vec![ExamplePairing::new(
    ...
    fn rtdx_get_all_balances(&mut self) -> Examples {
        let address = RtdAddress::from(ObjectID::new(self.rng.r#gen()));

        let result = Balance {
            coin_type: "0x2::rtd::RTD".to_string(),
    ...
    fn rtdx_get_all_coins(&mut self) -> Examples {
        let limit = 3;
        let owner = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
    ...
    fn rtdx_get_balance(&mut self) -> Examples {
        let owner = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
    ...
    fn rtdx_get_coin_metadata(&mut self) -> Examples {
        let result = RtdCoinMetadata {
            decimals: 9,
            name: "Usdc".to_string(),
    ...
    fn rtd_get_latest_checkpoint_sequence_number(&mut self) -> Examples {
        let result = "507021";
        Examples::new(
    ...
    fn rtdx_get_coins(&mut self) -> Examples {
        let coin_type = "0x2::rtd::RTD".to_string();
        let owner = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
        let coins = (0..3)
            .map(|_| Coin {
                coin_type: coin_type.clone(),
        ...
        Examples::new(
            "rtdx_getCoins",
            vec![ExamplePairing::new(
                "Gets all RTD coins owned by the address provided. Return a paginated list of `limit` results per page. Similar to `rtdx_getAllCoins`, but provides a way to filter by coin type.",
                vec![
                    ("owner", json!(owner)),
    ...
    fn rtdx_get_total_supply(&mut self) -> Examples {
    ...
    fn rtd_get_move_function_arg_types(&mut self) -> Examples {
        let result = vec![
    ...
    fn rtd_get_normalized_move_function(&mut self) -> Examples {
        let ability_set = RtdMoveAbilitySet {
            abilities: vec![RtdMoveAbility::Store, RtdMoveAbility::Key],
        };

        let result = RtdMoveNormalizedFunction {
            is_entry: false,
            type_parameters: vec![ability_set],
            parameters: vec![RtdMoveNormalizedType::U64],
            visibility: RtdMoveVisibility::Public,
            return_: vec![RtdMoveNormalizedType::U64],
        };
    ...
    fn rtd_get_normalized_move_module(&mut self) -> Examples {
        let result = RtdMoveNormalizedModule {
    ...
    fn rtd_get_normalized_move_modules_by_package(&mut self) -> Examples {
        let result = RtdMoveNormalizedModule {
            address: ObjectID::new(self.rng.r#gen()).to_string(),
            exposed_functions: BTreeMap::new(),
            file_format_version: 6,
        ...
        Examples::new(
            "rtd_getNormalizedMoveModulesByPackage",
            vec![ExamplePairing::new(
    ...
    fn rtd_get_normalized_move_struct(&mut self) -> Examples {
        let abilities = RtdMoveAbilitySet {
            abilities: vec![RtdMoveAbility::Store, RtdMoveAbility::Key],
        };
        let fields = vec![].into_iter().collect::<Vec<_>>();
        let type_parameters = vec![].into_iter().collect::<Vec<_>>();
        let result = RtdMoveNormalizedStruct {
            abilities,
            fields,
            type_parameters,
        };
    ...
    fn rtdx_get_validators_apy(&mut self) -> Examples {
        let result = vec![
            ValidatorApy {
                address: RtdAddress::from(ObjectID::new(self.rng.r#gen())),
                apy: 0.06,
            },
            ValidatorApy {
                address: RtdAddress::from(ObjectID::new(self.rng.r#gen())),
    ...
    fn rtdx_get_dynamic_fields(&mut self) -> Examples {
        let object_id = ObjectID::new(self.rng.r#gen());
    ...
    fn rtdx_get_dynamic_field_object(&mut self) -> Examples {
        ...
        let struct_tag = parse_rtd_struct_tag("0x9::test::TestField").unwrap();
        let resp = RtdObjectResponse::new_with_data(RtdObjectData {
            content: Some(
                RtdParsedData::try_from_object(
                    unsafe {
            ...
            type_: Some(ObjectType::Struct(MoveObjectType::from(
                parse_rtd_struct_tag("0x9::test::TestField").unwrap(),
            ))),
            bcs: None,
            display: None,
        });
    ...
    fn rtdx_get_owned_objects(&mut self) -> Examples {
        let owner = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
        ...
        let filter = Some(RtdObjectDataFilter::MatchAll(vec![
            RtdObjectDataFilter::StructType(
                StructTag::from_str("0x2::coin::Coin<0x2::rtd::RTD>").unwrap(),
            ),
            RtdObjectDataFilter::AddressOwner(owner),
            RtdObjectDataFilter::Version(version),
        ]));
        let query = json!(RtdObjectResponseQuery { filter, options });
        let object_id = ObjectID::new(self.rng.r#gen());
    ...
    fn rtdx_query_events(&mut self) -> Examples {
    ...
    fn rtdx_get_latest_rtd_system_state(&mut self) -> Examples {
        let result = "some_system_state";
        Examples::new(
            "rtdx_getLatestRtdSystemState",
            vec![ExamplePairing::new(
                "Gets objects owned by the address in the request.",
                vec![],
                json!(result),
    ...
    fn rtd_get_chain_identifier(&mut self) -> Examples {
    ...
    fn rtdx_get_stakes(&mut self) -> Examples {
        let principal = 200000000000;
        let owner = RtdAddress::from(ObjectID::new(self.rng.r#gen()));
        let result = vec![
            DelegatedStake {
                validator_address: RtdAddress::from(ObjectID::new(self.rng.r#gen())),
                staking_pool: ObjectID::new(self.rng.r#gen()),
                stakes: vec![
                    Stake {
                        staked_rtd_id: ObjectID::new(self.rng.r#gen()),
    ...
    fn rtdx_get_stakes_by_ids(&mut self) -> Examples {
        let principal = 200000000000;
        ...
        Examples::new(
            ...
            vec![ExamplePairing::new(
                "Returns the staking information for the address the request provides.",
                vec![("staked_rtd_ids", json!(vec![stake1, stake2]))],
                json!(result),
            )],
        )
    }

    fn rtdx_resolve_name_service_address(&mut self) -> Examples {
        let result = ObjectID::new(self.rng.r#gen());
        Examples::new(
            "rtdx_resolveNameServiceAddress",
            vec![ExamplePairing::new(
                "Returns the resolved address for the name the request provides.",
                vec![("name", json!("example.rtd".to_string()))],
                json!(result),
            )],
        )
    }

    fn rtdx_resolve_name_service_names(&mut self) -> Examples {
        let next_cursor = Some(ObjectID::new(self.rng.r#gen()));
        let object_id = ObjectID::new(self.rng.r#gen());
        let result = Page {
            data: vec!["example.rtd".to_string()],
            next_cursor,
            has_next_page: false,
        };
        Examples::new(
            "rtdx_resolveNameServiceNames",
            vec![ExamplePairing::new(
                "Returns the RtdNS name for the address the request provides. Currently, the API returns only the first name in cases where there are multiple. Future support will use the cursor ID and limit values in the request to control pagination of the response for addresses with multiple names.",
                vec![
    ...
    fn rtd_try_multi_get_past_objects(&mut self) -> Examples {
        ...
        let result = vec![
            RtdPastObjectResponse::VersionFound(RtdObjectData {
                content: Some(
                    RtdParsedData::try_from_object(
                        coin.to_object(SequenceNumber::from_u64(1)),
                        GasCoin::layout(),
