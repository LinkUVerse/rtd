...
use rtd_indexer_builder::progress::ProgressSavingPolicy;
use rtd_types::base_types::ObjectID;
use rtd_types::transaction::{Command, TransactionDataAPI};
use tracing::info;

use rtd_indexer_builder::indexer_builder::{DataMapper, IndexerProgressStore, Persistent};
use rtd_indexer_builder::rtd_datasource::CheckpointTxnData;
use rtd_indexer_builder::{LIVE_TASK_TARGET_CHECKPOINT, Task, Tasks};
...
use crate::schema::{
    ...
    rtd_error_transactions, trade_params_update, votes,
};
use crate::types::{
    Balances, Flashloan, OrderFill, OrderUpdate, OrderUpdateStatus, PoolPrice, ProcessedTxnData,
    Proposals, Rebates, Stakes, RtdTxnError, TradeParamsUpdate, Votes,
};
...
/// Data mapper impl
#[derive(Clone)]
pub struct RtdDeepBookDataMapper {
    pub metrics: DeepBookIndexerMetrics,
    pub package_id: ObjectID,
}

impl DataMapper<CheckpointTxnData, ProcessedTxnData> for RtdDeepBookDataMapper {
    fn map(
        &self,
        ...
        match &data.events {
            Some(events) => {
                let processed_rtd_events =
                    events
                        .data
                        .iter()
                        .enumerate()
                        .try_fold(vec![], |mut result, (i, ev)| {
                            if let Some(data) = process_rtd_event(
                                ev,
                                i,
                                &data,
                                checkpoint_num,
                ...
                if !processed_rtd_events.is_empty() {
                    info!(
                        "RTD: Extracted {} deepbook data entries for tx {}.",
                        processed_rtd_events.len(),
                        data.transaction.digest()
                    );
                }
                Ok(processed_rtd_events)
            }
            None => {
