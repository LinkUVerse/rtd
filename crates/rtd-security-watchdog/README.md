...
```shell
cargo run --release -p rtd-security-watchdog
```
## Usage
...
The config file allows setting up time based schedule for expected results. For example, when periodically checking total rtd in the network we want it to be an exact value i.e 10B whereas when periodically checking balance of an account
...
[
  {
    ...
    "sql_query": "SELECT total_rtd FROM total_rtd_mainnet ORDER BY epoch DESC LIMIT 1",
    "metric_name": "total_rtd_10B",
    "timed_exact_limits": {
      // total rtd should always be exact 10B since
      // the dawn of time
  ...
  },
  {
    ...
    "timed_lower_limits": {
      ...
      // i.e. balance should not drop below 50 RTD before 1/1/2024,
      // balance should not drop below 100RTD before 2/1/2024
      // and it should not drop below 150RTD before 3/1/2024
