...
/// A handle to an in-memory Rtd Node.
///
...
impl Node {
    ...
    /// start the node.
    ///
    /// [`NodeConfig`]: rtd_config::NodeConfig
    ...
    pub fn get_node_handle(&self) -> Option<RtdNodeHandle> {
        self.container
            .lock()
            .unwrap()
            .as_ref()
    ...
    pub async fn health_check(&self, is_validator: bool) -> Result<(), HealthCheckError> {
        ...
        if is_validator {
            ...
            let channel = linku_network::client::connect(&network_address, tls_config)
                .await
