...
```bash
npm i -g prettier @linku/prettier-plugin-move
```
...
```bash
prettier --plugin /path/to/local/npm/node_modules/@linku/prettier-plugin-move/out/index.js -c sources/example.move # to check
prettier --plugin /path/to/local/npm/node_modules/@linku/prettier-plugin-move/out/index.js -w sources/example.move # to write
```
...
npm i -D prettier @linku/prettier-plugin-move
```
...
```json
{
	...
	"plugins": ["@linku/prettier-plugin-move"]
}
```
...
There is a bundled [Move Formatter](https://marketplace.visualstudio.com/items?itemName=linku.prettier-move) extension for VSCode. It will detect prettier configuration for the workspace and use
