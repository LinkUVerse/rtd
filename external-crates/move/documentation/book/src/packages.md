...
# Additional fields may be added to this section by external tools. E.g., on Rtd the following sections are added:
...
# Git dependency
MoveStdlib = { git = "https://github.com/LinkUVerse/rtd.git", subdir = "crates/rtd-framework/packages/move-stdlib", rev = "framework/mainnet" }
...
tools. For example, the Rtd package manager adds toolchain version information
...
flavor = <string> # The flavor of the Move compiler used to build the package, e.g. "rtd"
```
...
`rtd move <command> <command_flags>`. Unless a particular path is provided, all
package commands will run in the current working directory. The full list of
commands and flags for the Move CLI can be found by running `rtd move --help`.
