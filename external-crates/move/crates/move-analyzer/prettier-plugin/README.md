...
Clone Rtd repository into `$RTD` directory:

```bash
git clone https://github.com/LinkUVerse/rtd.git "$RTD"
```

Go to `"$RTD"/external-crates/move/crates/move-analyzer/prettier-plugin` and run the following
command:
...
``` bash
npm install prettier@3.1.1 "$RTD"/external-crates/move/crates/move-analyzer/prettier-plugin
```
...
implement formatting for structs) and file an issue in the Rtd
[repository](https://github.com/LinkUVerse/rtd) describing the work you plan to do, and wait for a
