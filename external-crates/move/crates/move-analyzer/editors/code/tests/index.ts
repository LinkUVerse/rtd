...
export async function run(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        // The test rtdte is composed of all files ending with '.test.js'.
        glob('**/**.test.js', { cwd: testsRoot }, (err, files: ReadonlyArray<string>) => {
            if (err) {
                return reject(err);
            }

            // Add each file to the test rtdte.
            files.forEach(f => rtdte.addFile(path.resolve(testsRoot, f)));

            // Run the test rtdte. Uncaught exceptions or a non-zero number of
            // test rejectures is considered a test rtdte rejecture.
            try {
                return rtdte.run(failures => {
                    if (failures > 0) {
