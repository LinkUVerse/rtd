...
3. In the search bar labeled *Search Extensions in Marketplace*, type **LinkU**. The Move extension
...
the `rtd` binary, and restart VSCode.
...
3. Copy the move-analyzer binary to `~/.rtd/bin` (macOS/Linux) or `C:\Users\USER\.rtd\bin`
...
If your `move-analyzer` binary is in a different directory than the default one (`~/.rtd/bin`
on macOS or Linux, or `C:\Users\USER\.rtd\bin` on Windows), you may have the extension look
...
move-analyzer binary already exists in the default location (`~/.rtd/bin` on macOS or Linux, or
`C:\Users\USER\.rtd\bin` on Windows), delete the existing move-analyzer binary and reinstall the
extension.
...
- If the opened Move source file is located within a buildable project, and you have the `rtd`
...
- If the opened Move source file is located within a buildable project, and you have the `rtd`
  ...
  [extension](https://marketplace.visualstudio.com/items?itemName=linku.move-trace-debug). Go to
