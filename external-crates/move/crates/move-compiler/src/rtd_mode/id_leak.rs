...
use crate::{
    ...
    rtd_mode::{
        ...
        REGISTRY_CREATE_FUNCTION_NAME, RTD_ADDR_NAME, RTD_ADDR_VALUE, RTD_CLOCK_CREATE,
...
pub const FRESH_ID_FUNCTIONS: &[(AccountAddress, Symbol, Symbol)] = &[
    (RTD_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW),
    (RTD_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW_UID_FROM_HASH),
    (
        RTD_ADDR_VALUE,
        DERIVED_OBJECT_MODULE_NAME,
        DERIVED_OBJECT_CLAIM,
    ),
    (RTD_ADDR_VALUE, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(AccountAddress, Symbol, Symbol)] = &[
    (
        RTD_SYSTEM_ADDR_VALUE,
        RTD_SYSTEM_MODULE_NAME,
        RTD_SYSTEM_CREATE,
    ),
    (RTD_ADDR_VALUE, CLOCK_MODULE_NAME, RTD_CLOCK_CREATE),
    (
        RTD_ADDR_VALUE,
    ...
    (RTD_ADDR_VALUE, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
    (BRIDGE_ADDR_VALUE, BRIDGE_MODULE_NAME, BRIDGE_CREATE),
    (RTD_ADDR_VALUE, ACCUMULATOR_MODULE_NAME, ACCUMULATOR_CREATE),
    (
        RTD_ADDR_VALUE,
...
impl SimpleAbsInt for IDLeakVerifierAI<'_> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from `{rtd}::{object}::{new}`, or `rtd::{derived}::{claim}`. \
                For tests, it can come from `{rtd}::{ts}::{ts_new}`",
                rtd = RTD_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(&RTD_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
