...
//! ProgramInfo extension for Rtd Flavor
//! Contains information that may be expensive to compute and is needed only for Rtd
...
pub enum TransferKind {
    ...
    PublicTransfer(Loc),
    /// transferred within the module to an address vis `rtd::transfer::transfer`
    PrivateTransfer(Loc),
}

/// Rtd info for the entire program (mostly used during translation ).
#[derive(Debug, Clone)]
pub struct RtdInfo {
...
/// Same as `RtdInfo` but for a given module (store per-module in `ProgramInfo`).
#[derive(Debug, Clone)]
pub struct RtdModInfo {
    ...
    fn visit_ty(
        ...
        match ty_.inner() {
            ...
            TI::Apply(_, sp!(_, tn_), _)
                if tn_.is(&RTD_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
