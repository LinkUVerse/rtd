...
const SHARE_FUNCTIONS: &[(AccountAddress, &str, &str)] = &[
    (RTD_ADDR_VALUE, TRANSFER_MOD_NAME, PUBLIC_SHARE_FUN),
    (RTD_ADDR_VALUE, TRANSFER_MOD_NAME, SHARE_FUN),
];
...
impl ShareOwnedVerifierAI<'_> {
    ...
    /// Gets RTD info from either the current program or precompiled library
    fn get_rtd_info(&self, mident: &ModuleIdent) -> &crate::rtd_mode::info::RtdModInfo {
        self.info
            .modules
            .get(mident)
            .or_else(|| {
            ...
            .rtd_info
            .as_ref()
            .expect("ICE typing module info expected to contain Rtd info")
    }
    ...
    fn transfer_kind(
        ...
        self.get_rtd_info(mident).transferred.get(datatype_name)
    }
}
