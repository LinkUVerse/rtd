...
The `rtd-execution` crate is responsible for abstracting access to the
...
- The adapter, that integrates Move into Rtd.
...
code must be via `rtd-execution`, and not by directly depending a
constituent crate.
...
and elsewhere, **default to accessing execution via `rtd-execution`.**
...
`rtd-execution tests::test_encapsulation` is a test that detects
...
`./crates/rtd-types` are shared across all versions.  Changes to
...
## `rtd-execution/src/lib.rs`

The entry-point to the execution crate -- `rtd-execution/src/lib.rs`
...
- `rtd-execution/src/lib.template.rs` -- a template file with
