...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<RtdAddress, BTreeMap<Type, Set<ObjectID>>>,
...
impl<'a> ObjectRuntime<'a> {
    ...
    pub fn transfer(
        ...
        //   child object
        let is_framework_obj = [RTD_SYSTEM_STATE_OBJECT_ID, RTD_CLOCK_OBJECT_ID].contains(&id);
            ...
            .map(|(id, (owner, type_, value))| {
                let write_kind =
                    if input_objects.contains_key(&id) || loaded_child_objects.contains_key(&id) {
                        ...
                        WriteKind::Mutate
                    } else if id == RTD_SYSTEM_STATE_OBJECT_ID || new_ids.contains_key(&id) {
                        // RTD_SYSTEM_STATE_OBJECT_ID is only transferred during genesis
