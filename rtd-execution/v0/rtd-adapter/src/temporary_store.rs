...
use rtd_types::execution::{DynamicallyLoadedObjectMetadata, ExecutionResults, SharedInput};
...
impl<'backing> TemporaryStore<'backing> {
    ...
    pub fn to_effects(
        ...
        let updated_gas_object_info = if let Some(coin_id) = gas_charger.gas_coin() {
            ...
            (
                ...
                Owner::AddressOwner(RtdAddress::default()),
            )
        };
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&RTD_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping RTD on the floor by overriding it.
...
impl TemporaryStore<'_> {
    ...
    ) -> RtdResult<(Vec<ObjectID>, HashSet<ObjectID>)> {
            ...
            match kind {
                WriteKind::Mutate => {
                    ...
                    match &old_obj.owner {
                        ...
                        Owner::Immutable => {
                            assert!(is_epoch_change, "Immutable objects cannot be written, except for Rtd Framework/Move stdlib upgrades at epoch change boundaries");
...
impl TemporaryStore<'_> {
    ...
    /// `RtdGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl TemporaryStore<'_> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_rtd_system_state_wrapper(self.store.as_object_store())
...
impl TemporaryStore<'_> {
    fn get_input_rtd(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_rtd(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input RTD in RTD conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Some(obj) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in RTD conservation checking"
                );
            };
            obj.get_total_rtd(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input RTD in RTD conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys RTD. This should hold for all txes
    ...
    /// rebate:
    ///
    /// 1. all RTD in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all RTD charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
    /// If `do_expensive_checks` is true, this will also check a third invariant:
    ///
    /// 3. all RTD in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of RTD, and burn `epoch_rebates`
    /// amount of RTD. We need these information for conservation check.
    pub fn check_rtd_conserved(
        &self,
        ...
        // total amount of RTD in storage rebate of input objects
        let mut total_input_rebate = 0;
        // total amount of RTD in storage rebate of output objects
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, storage_rebate)) = input {
                ...
                if do_expensive_checks {
                    total_input_rtd += self.get_input_rtd(&id, version, layout_resolver)?;
                }
            }
            if let Some(object) = output {
                ...
                if do_expensive_checks {
                    total_output_rtd += object.get_total_rtd(layout_resolver).map_err(|e| {
                        make_invariant_violation!(
                            "Failed looking up output RTD in RTD conservation checking for \
                             mutated type {:?}: {e:#?}",
                            object.struct_tag(),
                        )
                    })?;
        ...
        if do_expensive_checks {
            ...
            if total_input_rtd != total_output_rtd {
                ...
                format!("RTD conservation failed: input={}, output={}, this transaction either mints or burns RTD",
                total_input_rtd,
                total_output_rtd))
            );
            }
        ...
        // all RTD in storage rebate fields of input objects should flow either to the transaction storage rebate, or the non-refundable
        // storage rebate pool
        if total_input_rebate != gas_summary.storage_rebate + gas_summary.non_refundable_storage_fee
        {
            ...
            /*return Err(ExecutionError::invariant_violation(
                format!("RTD conservation failed--{} RTD in storage rebate field of input objects, {} RTD in tx storage rebate or tx non-refundable storage rebate",
                total_input_rebate,
                gas_summary.non_refundable_storage_fee))
            );*/
        }

        // all RTD charged for storage should flow into the storage rebate field of some output object
        if gas_summary.storage_cost != total_output_rebate {
            // TODO: re-enable once we fix the edge case with OOG, gas smashing, and storage rebate
            /*return Err(ExecutionError::invariant_violation(
                format!("RTD conservation failed--{} RTD charged for storage, {} RTD in storage rebate field of output objects",
...
impl Storage for TemporaryStore<'_> {
    ...
    fn record_execution_results(
        ...
        let ExecutionResults::V1(results) = results else {
            panic!("ExecutionResults::V1 expected in rtd-execution v0");
        };
    ...
    fn check_coin_deny_list(
        &self,
        _receiving_funds_type_and_owners: BTreeMap<TypeTag, BTreeSet<RtdAddress>>,
    ) -> DenyListResult {
        unreachable!("Coin denylist v2 is not supported in rtd-execution v0");
    }

    fn record_generated_object_ids(&mut self, _generated_ids: BTreeSet<ObjectID>) {
        unreachable!(
            "Generated object IDs are not recorded in ExecutionResults in rtd-execution v0"
        );
    }
}
