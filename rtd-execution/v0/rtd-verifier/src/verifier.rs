...
use rtd_types::{error::ExecutionError, move_package::FnInfoMap};
...
pub fn rtd_verify_module_metered(
...
/// Runs the Rtd verifier and checks if the error counts as a Rtd verifier timeout
/// NOTE: this function only check if the verifier error is a timeout
/// All other errors are ignored
pub fn rtd_verify_module_metered_check_timeout_only(
    config: &ProtocolConfig,
    module: &CompiledModule,
    fn_info_map: &FnInfoMap,
    meter: &mut (impl Meter + ?Sized),
) -> Result<(), ExecutionError> {
    // Checks if the error counts as a Rtd verifier timeout
    if let Err(error) = rtd_verify_module_metered(config, module, fn_info_map, meter) {
        if matches!(
            error.kind(),
...
pub fn rtd_verify_module_unmetered(
    config: &ProtocolConfig,
    module: &CompiledModule,
    fn_info_map: &FnInfoMap,
) -> Result<(), ExecutionError> {
    rtd_verify_module_metered(config, module, fn_info_map, &mut DummyMeter).inspect_err(|err| {
