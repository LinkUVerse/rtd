...
#[cfg(msim)]
const RTD_DENY_LIST_OBJECT_CREATE: FunctionIdent = (
    &RTD_FRAMEWORK_ADDRESS,
    COIN_MODULE_NAME,
...
#[cfg(not(msim))]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[RTD_SYSTEM_CREATE, RTD_CLOCK_CREATE];
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from rtd::{}::{}. \
                Or for tests, it can come from rtd::{}::{}",
