...
use rtd_types::storage::{get_module, PackageObject};
use rtd_types::{
    base_types::ObjectID,
...
pub struct LinkageView<'state> {
    ...
    resolver: Box<dyn RtdResolver + 'state>,
...
impl<'state> LinkageView<'state> {
    pub fn new(resolver: Box<dyn RtdResolver + 'state>) -> Self {
        Self {
            resolver,
            linkage_info: None,
    ...
    pub(crate) fn relocate(&self, module_id: &ModuleId) -> Result<ModuleId, RtdError> {
