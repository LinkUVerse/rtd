...
mod checked {
    ...
    use crate::{adapter::new_native_extensions, gas_meter::RtdGasMeter};
    ...
    use rtd_types::execution::ExecutionResults;
    ...
    use rtd_types::{error::command_argument_error, execution_status::CommandArgumentError};
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub fn new(
            ...
            let mut linkage_view = LinkageView::new(Box::new(state_view.as_rtd_resolver()));
        ...
        pub fn finish<Mode: ExecutionMode>(self) -> Result<ExecutionResults, ExecutionError> {
            ...
            Ok(ExecutionResults::V2(ExecutionResultsV2 {
                ...
                settlement_input_rtd: 0,
                settlement_output_rtd: 0,
            }))
        }
    ...
    pub(crate) struct RtdDataStore<'state, 'a> {
        linkage_view: &'a LinkageView<'state>,
        new_packages: &'a [MovePackage],
    }

    impl<'state, 'a> RtdDataStore<'state, 'a> {
        pub(crate) fn new(
            linkage_view: &'a LinkageView<'state>,
            new_packages: &'a [MovePackage],
        ) -> Self {
    ...
    impl DataStore for RtdDataStore<'_, '_> {
