...
mod checked {
    ...
    use rtd_move_natives::{object_runtime::ObjectRuntime, NativesCostTable};
            ...
            if let Err(e) = verify_module_with_config_metered(verifier_config, module, meter) {
                ...
                if check_for_verifier_timeout(&e.major_status()) {
                    ...
                    metrics
                        ...
                        .inc();
                    return Err(RtdErrorKind::ModuleVerificationFailure {
                        error: format!("Verification timedout: {}", e),
                    }
                    .into());
                };
            } else if let Err(err) =
                rtd_verify_module_metered_check_timeout_only(module, &BTreeMap::new(), meter)
            {
                ...
                metrics
                    .verifier_timeout_metrics
                    .with_label_values(&[
                        BytecodeVerifierMetrics::RTD_VERIFIER_TAG,
                        BytecodeVerifierMetrics::TIMEOUT_TAG,
                    ])
                    .inc();
                return Err(err.into());
