...
use rtd_types::randomness_state::is_mutable_random;
use rtd_types::{
...
fn verify_param_type(
    ...
    // Only `rtd::rtd_system` is allowed to expose entry functions that accept a mutable clock
    // parameter.
    ...
    // Only `rtd::rtd_system` is allowed to expose entry functions that accept a mutable Random
    // parameter.
