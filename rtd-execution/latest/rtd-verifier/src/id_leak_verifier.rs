...
//! Objects whose struct type has key ability represent Rtd objects.
//! They have unique IDs that should never be reused. This verifier makes
//! sure that the id field of Rtd objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a Rtd object. From there, we track the flow of the value and make
...
const OBJECT_NEW: FunctionIdent = (RTD_FRAMEWORK_ADDRESS, OBJECT_MODULE_NAME, ident_str!("new"));
...
const RTD_SYSTEM_CREATE: FunctionIdent = (
    RTD_SYSTEM_ADDRESS,
    RTD_SYSTEM_MODULE_NAME,
    ident_str!("create"),
);
const RTD_CLOCK_CREATE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
const RTD_AUTHENTICATOR_STATE_CREATE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    AUTHENTICATOR_STATE_MODULE_NAME,
    ident_str!("create"),
);
const RTD_RANDOMNESS_STATE_CREATE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    RANDOMNESS_MODULE_NAME,
    ident_str!("create"),
);
const RTD_DENY_LIST_CREATE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    DENY_LIST_MODULE,
    DENY_LIST_CREATE_FUNC,
);

const RTD_BRIDGE_CREATE: FunctionIdent = (BRIDGE_ADDRESS, BRIDGE_MODULE_NAME, ident_str!("create"));
const RTD_ACCUMULATOR_CREATE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    ACCUMULATOR_MODULE_NAME,
    ident_str!("create"),
);
const RTD_COIN_REGISTRY_CREATE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
...
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[
    RTD_SYSTEM_CREATE,
    RTD_CLOCK_CREATE,
    RTD_AUTHENTICATOR_STATE_CREATE,
    RTD_RANDOMNESS_STATE_CREATE,
    RTD_DENY_LIST_CREATE,
    RTD_BRIDGE_CREATE,
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from `rtd::{}::{}`, or `rtd::{}::{}`. \
                For tests, it can also come from `rtd::{}::{}`",
            OBJECT_NEW.1,
            OBJECT_NEW.2,
