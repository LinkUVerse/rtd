...
fn verification_failure(error: String) -> ExecutionError {
    ExecutionError::new_with_source(ExecutionErrorKind::RtdMoveVerificationError, error)
}

fn to_verification_timeout_error(error: String) -> ExecutionError {
    ExecutionError::new_with_source(ExecutionErrorKind::RtdMoveVerificationTimedout, error)
}
