...
use rtd_types::{RTD_FRAMEWORK_ADDRESS, error::ExecutionError};
...
/// All transfer functions (the functions in `rtd::transfer`) are "private" in that they are
...
pub fn verify_module(
    ...
    if *module.address() == RTD_FRAMEWORK_ADDRESS
    ...
    // do not need to check the rtd::transfer module itself
...
fn verify_function(
    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            if ident == (RTD_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
                verify_private_transfer(view, fhandle, type_arguments, allow_receiving_object_id)?
            } else if ident == (RTD_FRAMEWORK_ADDRESS, EVENT_MODULE) {
                verify_private_event_emit(view, fhandle, type_arguments)?
            } else if ident == (RTD_FRAMEWORK_ADDRESS, COIN_REGISTRY_MODULE) {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (RTD_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
        return Ok(());
    }
    ...
    if !is_defined_in_current_module(view, type_arg) {
        return Err(format!(
            "Invalid call to '{rtd}::transfer::{f}' on an object of type '{t}'. \
            ...
            instead, i.e. '{rtd}::transfer::public_{f}'",
            rtd = RTD_FRAMEWORK_ADDRESS,
            f = fident,
...
fn verify_private_event_emit(
    ...
    if fident == EMIT_AUTHENTICATED_IMPL_FUNCTION {
        ...
        if (module_id.address(), module_id.name()) != (&RTD_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            debug_assert!(
                false,
