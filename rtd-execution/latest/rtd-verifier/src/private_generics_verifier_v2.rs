...
// Event function
pub const RTD_EVENT_EMIT_EVENT: FunctionIdent =
    (RTD_FRAMEWORK_ADDRESS, EVENT_MODULE, ident_str!("emit"));
pub const RTD_EVENT_EMIT_AUTHENTICATED: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    EVENT_MODULE,
    ident_str!("emit_authenticated"),
);
pub const RTD_EVENT_NUM_EVENTS: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    EVENT_MODULE,
    ident_str!("num_events"),
);
pub const RTD_EVENT_EVENTS_BY_TYPE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    EVENT_MODULE,
...
pub const RTD_TRANSFER_PUBLIC_TRANSFER: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_transfer"),
);
pub const RTD_TRANSFER_PUBLIC_FREEZE_OBJECT: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_freeze_object"),
);
pub const RTD_TRANSFER_PUBLIC_SHARE_OBJECT: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_share_object"),
);
pub const RTD_TRANSFER_PUBLIC_RECEIVE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("public_receive"),
);
pub const RTD_TRANSFER_RECEIVING_OBJECT_ID: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("receiving_object_id"),
);
pub const RTD_TRANSFER_PUBLIC_PARTY_TRANSFER: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
...
pub const RTD_TRANSFER_TRANSFER: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("transfer"),
);
pub const RTD_TRANSFER_FREEZE_OBJECT: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("freeze_object"),
);
pub const RTD_TRANSFER_SHARE_OBJECT: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("share_object"),
);
pub const RTD_TRANSFER_RECEIVE: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
    ident_str!("receive"),
);
pub const RTD_TRANSFER_PARTY_TRANSFER: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
    TRANSFER_MODULE,
...
pub const RTD_COIN_REGISTRY_NEW_CURRENCY: FunctionIdent = (
    RTD_FRAMEWORK_ADDRESS,
...
pub const EXHAUSTIVE_MODULES: &[(AccountAddress, &IdentStr)] = &[
    (RTD_FRAMEWORK_ADDRESS, EVENT_MODULE),
    (RTD_FRAMEWORK_ADDRESS, TRANSFER_MODULE),
];
...
pub const FUNCTIONS_TO_CHECK: &[(FunctionIdent, &[/* is internal */ bool])] = &[
    ...
    // event functions
    (RTD_EVENT_EMIT_EVENT, &[true]),
    (RTD_EVENT_EMIT_AUTHENTICATED, &[true]),
    (RTD_EVENT_NUM_EVENTS, &[]),
    (RTD_EVENT_EVENTS_BY_TYPE, &[false]),
    // public transfer functions
    (RTD_TRANSFER_PUBLIC_TRANSFER, &[false]),
    (RTD_TRANSFER_PUBLIC_FREEZE_OBJECT, &[false]),
    (RTD_TRANSFER_PUBLIC_SHARE_OBJECT, &[false]),
    (RTD_TRANSFER_PUBLIC_RECEIVE, &[false]),
    (RTD_TRANSFER_RECEIVING_OBJECT_ID, &[false]),
    (RTD_TRANSFER_PUBLIC_PARTY_TRANSFER, &[false]),
    // private transfer functions
    (RTD_TRANSFER_TRANSFER, &[true]),
    (RTD_TRANSFER_FREEZE_OBJECT, &[true]),
    (RTD_TRANSFER_SHARE_OBJECT, &[true]),
    (RTD_TRANSFER_RECEIVE, &[true]),
    (RTD_TRANSFER_PARTY_TRANSFER, &[true]),
    // coin registry functions
    (RTD_COIN_REGISTRY_NEW_CURRENCY, &[true]),
];
...
/// Several functions in the Rtd Framework have `internal` type parameters, whose arguments must be
...
/// To avoid, issues, all `su::transfer` and `rtd::event` functions must be configured in `INTERNAL_FUNCTIONS`.
pub fn verify_module(
    ...
    // Skip rtd::test_scenario
    if module_address == RTD_FRAMEWORK_ADDRESS && module_name.as_str() == TEST_SCENARIO_MODULE_NAME
    {
        // exclude test_module which is a test-only module in the Rtd framework which "emulates"
...
pub fn callee_package_name(callee_addr: &AccountAddress) -> Cow<'static, str> {
    match *callee_addr {
        RTD_FRAMEWORK_ADDRESS => Cow::Borrowed("rtd"),
...
pub fn help_message(
    ...
    if *callee_addr == RTD_FRAMEWORK_ADDRESS && callee_module == TRANSFER_MODULE {
        format!(
            " If the type has the 'store' ability, use the public variant instead: 'rtd::transfer::public_{}'.",
            callee_function
        )
    } else {
        String::new()
