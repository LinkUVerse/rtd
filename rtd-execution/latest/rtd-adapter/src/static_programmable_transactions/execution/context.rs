...
macro_rules! charge_gas_ {
    ($gas_charger:expr, $env:expr, $call:ident($($args:expr),*)) => {{
        RtdGasMeter($gas_charger.move_gas_status_mut())
            .$call($($args),*)
...
impl<'env, 'pc, 'vm, 'state, 'linkage, 'gas> Context<'env, 'pc, 'vm, 'state, 'linkage, 'gas> {
    ...
    pub fn finish<Mode: ExecutionMode>(mut self) -> Result<ExecutionResults, ExecutionError> {
        ...
        let RuntimeResults {
            ...
            deleted_object_ids,
            accumulator_events,
            settlement_input_rtd,
            settlement_output_rtd,
    ...
    fn publish_and_verify_modules(
        ...
        self.env
            ...
            .publish_module_bundle(
                ...
                &mut data_store,
                &mut RtdGasMeter(self.gas_charger.move_gas_status_mut()),
            )
            .map_err(|e| self.env.convert_linked_vm_error(e, linkage))?;
    ...
    pub fn argument_updates(
        &mut self,
        args: Vec<T::Argument>,
    ) -> Result<Vec<(rtd_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        args.into_iter()
    ...
    ) -> Result<Option<(rtd_types::transaction::Argument, Vec<u8>, TypeTag)>, ExecutionError> {
        use rtd_types::transaction::Argument as TxArgument;
        let ty = match ty {
...
fn load_object_arg_impl(
    ...
    let rtd_types::object::ObjectInner {
        data: rtd_types::object::Data::Move(move_obj),
        ..
    } = obj.as_inner()
    else {
