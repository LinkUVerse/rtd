...
impl PackageStore for RtdDataStore<'_, '_> {
    fn get_package(&self, id: &ObjectID) -> RtdResult<Option<Rc<MovePackage>>> {
        for package in self.new_packages {
            if package.id() == *id {
    ...
    fn resolve_type_to_defining_id(
        ...
        unimplemented!(
            "resolve_type_to_defining_id is not implemented for legacy::RtdDataStore and should never be called"
        )
    }
}
