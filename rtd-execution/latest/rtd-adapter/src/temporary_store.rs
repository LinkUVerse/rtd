...
impl Storage for TemporaryStore<'_> {
    ...
    fn check_coin_deny_list(
        &self,
        receiving_funds_type_and_owners: BTreeMap<TypeTag, BTreeSet<RtdAddress>>,
    ) -> DenyListResult {
        ...
        if result.num_non_gas_coin_owners > 0
            && !self.input_objects.contains_key(&RTD_DENY_LIST_OBJECT_ID)
        {
            self.loaded_per_epoch_config_objects
                .write()
                .insert(RTD_DENY_LIST_OBJECT_ID);
        }
        result
    }
