...
pub mod checked {
    ...
    use rtd_types::deny_list_v2::CONFIG_SETTING_DYNAMIC_FIELD_SIZE_FOR_GAS;
    use rtd_types::gas::{GasCostSummary, RtdGasStatus, deduct_gas};
    use rtd_types::gas_model::gas_predicates::{
    ...
    pub struct GasCharger {
        ...
        address_balance_gas_payer: Option<RtdAddress>,
    }
    ...
    impl GasCharger {
        ...
        pub fn new_unmetered(tx_digest: TransactionDigest) -> Self {
            Self {
                ...
                gas_coins: vec![],
                smashed_gas_coin: None,
                gas_status: RtdGasStatus::new_unmetered(),
        ...
        pub fn into_gas_status(self) -> RtdGasStatus {
            self.gas_status
        }
        ...
        pub fn charge_input_objects(
            ...
            let total_size = temporary_store
                .objects()
                .iter()
                // don't charge for loading Rtd Framework or Move stdlib
            ...
            if self.smashed_gas_coin.is_some() || self.address_balance_gas_payer.is_some() {
                ...
                let is_move_abort = execution_result
                    ...
                    .map(|err| {
                        matches!(
                            err.kind(),
                            rtd_types::execution_status::ExecutionFailureStatus::MoveAbort(_, _)
                        )
                    })
                    .unwrap_or(false);
                // bucketize computation cost
            ...
            if let Some(payer_address) = self.address_balance_gas_payer {
                let is_insufficient_balance_error = execution_result
                    ...
                    .map(|err| {
                        matches!(
                            err.kind(),
                            rtd_types::execution_status::ExecutionFailureStatus::InsufficientFundsForWithdraw
                        )
                    })
                    .unwrap_or(false);
                ...
                if is_insufficient_balance_error {
                    ...
                    if net_change != 0 {
                        let balance_type = rtd_types::balance::Balance::type_tag(
