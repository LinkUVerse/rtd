...
pub trait RtdResolver: BackingPackageStore {
    fn as_backing_package_store(&self) -> &dyn BackingPackageStore;
}

impl<T> RtdResolver for T
where
...
pub trait ExecutionState: StorageView + RtdResolver {
    fn as_rtd_resolver(&self) -> &dyn RtdResolver;
...
impl InputValue {
    ...
    pub fn withdrawal(withdrawal_ty: RawValueType, owner: RtdAddress, limit: U256) -> Self {
        let value = Value::Raw(
            withdrawal_ty,
...
impl TryFromValue for RtdAddress {
    fn try_from_value(value: Value) -> Result<Self, CommandArgumentError> {
