...
pub(crate) struct TestInventories {
    ...
    pub(crate) address_inventories: BTreeMap<RtdAddress, BTreeMap<MoveObjectType, Set<ObjectID>>>,
...
pub struct RuntimeResults {
    ...
    pub settlement_input_rtd: u64,
    pub settlement_output_rtd: u64,
}
...
pub(crate) struct ObjectRuntimeState {
    ...
    // Used to track RTD conservation in settlement transactions. Settlement transactions
    // gather up withdraws and deposits from other transactions, and record them to accumulator
    // fields. The settlement transaction records the total amount of RTD being disbursed here,
...
impl<'a> ObjectRuntime<'a> {
    ...
    pub fn record_settlement_rtd_conservation(&mut self, input_rtd: u64, output_rtd: u64) {
        self.state.settlement_input_rtd += input_rtd;
        self.state.settlement_output_rtd += output_rtd;
    }
