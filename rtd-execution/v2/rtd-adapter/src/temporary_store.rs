                ...
                match &old_obj.owner {
                    ...
                    Owner::Immutable => {
                        assert!(
                            ...
                            Rtd Framework/Move stdlib upgrades at epoch change boundaries"
                        );
...
impl Storage for TemporaryStore<'_> {
    ...
    fn check_coin_deny_list(
        ...
        unreachable!("Coin denylist v2 is not supported in rtd-execution v2");
    }

    fn record_generated_object_ids(&mut self, _generated_ids: BTreeSet<ObjectID>) {
        unreachable!(
            "Generated object IDs are not recorded in ExecutionResults in rtd-execution v2"
        );
    }
}
