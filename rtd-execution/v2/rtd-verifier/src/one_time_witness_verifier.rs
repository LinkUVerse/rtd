...
pub fn verify_module(
    ...
    // In Rtd's framework code there is an exception to the one-time witness type rule - we have a
    // RTD type in the rtd module but it is instantiated outside of the module initializer (in fact,
    // the module has no initializer). The reason for it is that the RTD coin is only instantiated
    ...
    if ModuleId::new(RTD_FRAMEWORK_ADDRESS, ident_str!("rtd").to_owned()) == self_id {
        return Ok(());
    }
