...
def parse_args():
    ...
    generate_lib = subparsers.add_parser(
        "generate-lib",
        help=(
            "Generate `rtd-execution/src/lib.rs` based on the current set of "
...
def do_cut(args):
    ...
    update_toml(args.feature, Path() / "rtd-execution" / "Cargo.toml")
    generate_impls(args.feature, impl_module)

    with open(Path() / "rtd-execution" / "src" / "lib.rs", mode="w") as lib:
        generate_lib(lib)
...
def do_generate_lib(args):
    if args.dry_run:
        ...
        lib_path = Path() / "rtd-execution" / "src" / "lib.rs"
...
def impl(feature):
    ...
    return Path() / "rtd-execution" / "src" / (feature.replace("-", "_") + ".rs")
...
def update_toml(feature, toml_path):
    """Add dependencies for 'feature' to rtd-execution's manifest."""
...
def generate_impls(feature, copy):
    ...
    orig = Path() / "rtd-execution" / "src" / "latest.rs"
...
def generate_lib(output_file: TextIO):
    ...
    Generates the contents of rtd-execution/src/lib.rs to assign a numeric
    ...
    template_path = Path() / "rtd-execution" / "src" / "lib.template.rs"
    cuts = discover_cuts()
...
# Modules in `rtd-execution` that don't count as "cuts" (they are
...
def discover_cuts():
    ...
    Finds all modules within the `rtd-execution` crate that count as
