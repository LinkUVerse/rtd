...
if [[ "$PR_TYPE" == *snapshot* ]]; then
  ...
  IFS=. read -r major minor patch <<<"$RTD_VERSION"; NEW_RTD_VERSION="$major.$((minor+1)).$patch"

  # Setup new branch for staging
  BRANCH="${GITHUB_ACTOR}/rtd-v${NEW_RTD_VERSION}-version-bump-${STAMP}"
  git checkout -b "$BRANCH"

  # Update the version in Cargo.toml and openrpc.json
  sed -i -E "s/^(version = \")[0-9]+\.[0-9]+\.[0-9]+(\"$)/\1${NEW_RTD_VERSION}\2/" Cargo.toml
  sed -i -E "s/(\"version\": \")([0-9]+\.[0-9]+\.[0-9]+)(\")/\1${NEW_RTD_VERSION}\3/" crates/rtd-open-rpc/spec/openrpc.json
  ...
  # Generate PR body
  BODY="Rtd v${NEW_RTD_VERSION} Version Bump"
  ...
  echo "Pull request for Rtd v${NEW_RTD_VERSION} Version Bump created: $PR_URL"
else
