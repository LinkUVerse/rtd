...
description: Each Rtd release provides a set of binaries for several operating systems. You can download these binaries from GitHub and use them to install Rtd.
keywords: [ install rtd, install rtd binaries, binaries ]
---
...
1. Go to https://github.com/LinkUVerse/rtd.
1. In the right pane, find the **Releases** section.

    ![Rtd releases in GitHub](./images/releases.png)
...
```sh
$ cargo build --profile release --bin rtd
```
...
If you previously installed the Rtd binaries, you can update them to the most recent release with the same command you used to install them (changing `testnet` to the desired branch):
...
<ImportContent source="install-rtd-node.mdx" mode="snippet" />
