...
description: zkLogin can be integrated into applications deployed on Rtd.
keywords: [ zklogin, how to use zklogin, generate ephemeral key pair, fetch JWT, decode JWT, generate salt, generate user Rtd address, fetch ZK proof, assemble zkLogin signature, oauth, openid ]
---
...
1. The wallet obtains a unique user salt based on a JWT. Use the OAuth subject identifier and salt to compute the zkLogin Rtd address.
...
```sh npm2yarn
$ npm install @linku/rtd
```

If you want to use the latest experimental version:

```sh npm2yarn
$ npm install @linku/rtd@experimental
```

## Get JWT

1. Generate an ephemeral key pair. Follow the same process as you would generating a key pair in a traditional wallet. See [Rtd SDK](https://sdk.linkulabs.com/typescript/cryptography/keypairs) for details.
...
```typescript
import { generateNonce, generateRandomness } from '@linku/rtd/zklogin';

const FULLNODE_URL = 'https://fullnode.devnet.rtd.io'; // replace with the RPC URL you want to use
const rtdClient = new RtdClient({ url: FULLNODE_URL });
const { epoch, epochDurationMs, epochStartTimestampMs } = await rtdClient.getLatestRtdSystemState();
...
zkLogin uses the user salt to compute the zkLogin Rtd address (see [definition](../../../concepts/cryptography/zklogin.mdx#address-definition)). The salt must be a 16-byte value or a integer smaller than `2n**128n`. There are several options for the application to maintain the user salt:
...
2. Backend service that exposes an endpoint that returns a unique salt for each user consistently.
   ...
   - Option 4: Implement a service that keeps a master seed value, and derive a user salt with key derivation by validating and parsing the JWT. For example, use `HKDF(ikm = seed, salt = iss || aud, info = sub)` defined [here](https://github.com/LinkUVerse/fastcrypto/blob/e6161f9279510e89bd9e9089a09edc018b30fbfe/fastcrypto/src/hmac.rs#L121). Note that this option does not allow any rotation on master seed or change in client ID (i.e. aud), otherwise a different user address will be derived and will result in loss of funds.

Here's an example request and response for the LinkU Labs-maintained salt server (using option 4). If you want to use the LinkU Labs salt server, please refer to [Enoki docs](https://docs.enoki.linkulabs.com/) and contact us. Only valid JWT authenticated with whitelisted client IDs are accepted.

```sh
$ curl -X POST https://salt.api.linkulabs.com/get_salt -H 'Content-Type: application/json' -d '{"token": "$JWT_TOKEN"}'
```
...
```typescript
import { jwtToAddress } from '@linku/rtd/zklogin';
...
```typescript
import { getExtendedEphemeralPublicKey } from '@linku/rtd/zklogin';
...
1. Call the LinkU Labs-maintained proving service.
1. Run the proving service in your backend using the provided Docker images.

### Call the LinkU Labs-maintained proving service

If you want to use the LinkU hosted ZK Proving Service for Mainnet, please refer to [Enoki docs](https://docs.enoki.linkulabs.com/) and contact us for accessing it.
...
1. Download the [Groth16 proving key zkey file](https://docs.circom.io/getting-started/proving-circuits/). There are zkeys available for all Rtd networks. See [the Ceremony section](#ceremony) for more details on how the main proving key is generated.

   - Main zkey (for Mainnet and Testnet)
     ```sh
     $ wget -O - https://raw.githubusercontent.com/rtd-foundation/zklogin-ceremony-contributions/main/download-main-zkey.sh | bash
     ```
   - Test zkey (for Devnet)

     ```sh
     $ wget -O - https://raw.githubusercontent.com/rtd-foundation/zklogin-ceremony-contributions/main/download-test-zkey.sh | bash
     ```
...
1. For the next step, you need two Docker images from the [linku/zklogin repository](https://hub.docker.com/r/linku/zklogin) (tagged as `prover` and `prover-fe`). To simplify, a docker compose file is available that automates this process. Run `docker compose` with the downloaded zkey from the same directory as the YAML file.

```yaml
services:
  backend:
    image: linku/zklogin:prover-stable
    volumes:
  ...
  frontend:
    image: linku/zklogin:prover-fe-stable
    command: '8080'
    ports:
...
1. To call the service, the following two endpoints are supported:
   ...
   - `/v1`: The request and response are the same as the LinkU Labs maintained service.

A few important things to note:

- The backend service (linku/zklogin:prover-stable) is compute-heavy. Use at least the minimum recommended 16 cores and 16GB RAM. Using weaker instances can lead to timeout errors with the message "Call to rapidsnark service took longer than 15s". You can adjust the environment variable `PROVER_TIMEOUT` to set a different timeout value, for example, `PROVER_TIMEOUT=30` for a timeout of 30 seconds.

- If you want to compile the prover from scratch (for performance reasons), please see our fork of [rapidsnark](https://github.com/LinkUVerse/rapidsnark#compile-prover-in-server-mode). You'd need to compile and launch the prover in server mode.
...
First, sign the transaction bytes with the ephemeral private key using the key pair generated previously. This is the same as [traditional key pair signing](https://sdk.linkulabs.com/typescript/cryptography/keypairs). Make sure that the transaction `sender` is also defined.

```typescript
const ephemeralKeyPair = new Ed25519Keypair();

const client = new RtdClient({ url: '<YOUR_RPC_URL>' });
...
```typescript
import { genAddressSeed, getZkLoginSignature } from '@linku/rtd/zklogin';
...
Compared to traditional signatures, zkLogin signatures take a longer time to generate. For example, the prover that LinkU Labs maintains typically takes about three seconds to return a proof, which runs on a machine with 16 vCPUs and 64 GB RAM. Using more powerful machines, such as those with physical CPUs or graphics processing units (GPUs), can reduce the proving time further.
...
The prover that LinkU Labs maintains is set to auto-scale to handle traffic surges. If you are not sure whether LinkU Labs can handle a specific number of requests or expect a sudden spike in the number of prover requests your application needs to make, please reach out to us on [Discord](https://discord.gg/rtd). Our plan is to horizontally scale the prover to handle any RPS you require.
