---
title: Rtd On-Chain Signatures Verification in Move
description: Rtd supports verification within Move smart contracts through several signature schemes. Signature schemes include Ed25519, Secp256k1 recoverable, Secp256k1 non-recoverable, Secp256r1 non-recoverable, Secp256r1 recoverable, BLS G1, and BLS G2.
...
Move contracts in Rtd support verifications for several signature schemes on-chain. Not all signatures supported in on-chain verification are supported as user signature verification. See [Rtd Signatures](/concepts/transactions/transaction-auth.mdx#user-signature) for valid signature schemes for transaction authorization.

This topic covers:
1. How to use [fastcrypto](https://github.com/LinkUVerse/fastcrypto)'s CLI tool to create a signature of a given scheme. For testing and debugging only, DO NOT use in production.
...
```sh
git@github.com:LinkUVerse/fastcrypto.git
cd fastcrypto/
