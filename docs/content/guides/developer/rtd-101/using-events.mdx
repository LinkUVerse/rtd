...
keywords: [ using events, notify events, notify assets, notify on-chain assets, activity events, events for activities, move events, events in move, query events, rtd RPC, graphql, move, filter events, rust, typescript ]
---

The Rtd network stores countless objects on chain where Move code can perform actions using those objects. Tracking this activity is often desired, for example, to discover how many times a module mints an NFT or to tally the amount of RTD in transactions that a smart contract generates.

To support activity monitoring, Move provides a structure to emit events on the Rtd network. You can then leverage a custom indexer to process checkpoint data that includes events that have been emitted. See the [custom indexer](../advanced/custom-indexer.mdx) topic in the Advanced section to learn how to stream checkpoints and filter events continuously. 

If you don't want to run a custom indexer, you can poll the Rtd network to query for emitted events instead. This approach typically includes a database to store the data retrieved from these calls. The [Poll events](#poll-events) section provides an example of using this method.

## Move event structure

An event object in Rtd consists of the following attributes:
...
- `sender`: The Rtd network address that triggered the event.
...
To create an event in your Move modules, add the `rtd::event` dependency.
...
The Rtd RPC provides a [queryEvents](/rtd-api-ref#rtdx_queryEvents) method to query on-chain packages and return available events. As an example, the following `curl` command queries the Deepbook package on Mainnet for a specific type of event:

```sh
$ curl -X POST https://fullnode.mainnet.rtd.io:443 \
	...
	"result": {
		"data": [
			...
			},
			{
				...
				"transactionModule": "deepbook_utils",
				"sender": "0x8b35e67a519fffa11a9c74f169228ff1aa085f3a3d57710af08baab8c02211b9",
				"type": "0xdee9::clob_v2::OrderFilled<0x2::rtd::RTD, 0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>",
				"parsedJson": {
...
The TypeScript SDK provides a wrapper for the `rtdx_queryEvents` method: [`client.queryEvents`](https://sdk.linkulabs.com/typedoc/classes/_linku_rtd.client.RtdClient.html#queryEvents).
...
```ts
import { useCurrentAccount, useSignAndExecuteTransaction, useRtdClient } from "@linku/dapp-kit";
...
| ----------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `All`             | All events                                               | `{"All": []}`                                                                                         |
| `Any`             | Events emitted from any of the given filter              | `{"Any": RtdEventFilter[]}`                                                                           |
...
The [Rtd by Example](https://github.com/gdanezis/rtd-by-example/blob/main/src/05_reading_events/bin/main.rs) repo on GitHub contains a code sample that demonstrates how to query events using the `query_events` function. The package that `PACKAGE_ID_CONST` points to exists on Mainnet, so you can test this code using Cargo. To do so, clone the `rtd-by-example` repo locally and follow the [Example 05 directions](https://github.com/gdanezis/rtd-by-example/tree/main/src/05_reading_events). 

```rust
use rtd_sdk::{rpc_types::EventFilter, types::Identifier, RtdClientBuilder};

const PACKAGE_ID_CONST: &str = "0x279525274aa623ef31a25ad90e3b99f27c8dbbad636a6454918855c81d625abc";

#[tokio::main]
async fn main() -> Result<(), anyhow::Error> {
    let rtd_mainnet = RtdClientBuilder::default()
        .build("https://fullnode.mainnet.rtd.io:443")
        .await?;

    let events = rtd_mainnet
        .event_api()
        .query_events(
...
You can use GraphQL to query events instead of JSON RPC. The following example queries are in the [`rtd-graphql-rpc` crate](https://github.com/LinkUVerse/rtd/tree/main/crates/rtd-graphql-rpc/examples/event_connection) in the Rtd repo. 
...
<ImportContent source="crates/rtd-graphql-rpc/examples/event_connection/event_connection.graphql" mode="code" />
...
<ImportContent source="crates/rtd-graphql-rpc/examples/event_connection/filter_by_sender.graphql" mode="code" />

</details>

The [TypeScript SDK](https://sdk.linkulabs.com/typedoc/modules/_linku_rtd.graphql.html) provides functionality to interact with the Rtd GraphQL service.
...
- Incorporate a [custom indexer](/guides/developer/advanced/custom-indexer.mdx) to take advantage of Rtd's micro-data ingestion framework.
- Poll the Rtd network on a schedule to query events.
...
To monitor events, you need a database to store checkpoint data. The [Trustless Swap](/guides/developer/app-examples/trustless-swap.mdx) example uses a Prisma database to store checkpoint data from the Rtd network. The database is populated from polling the network to retrieve emitted events. 
