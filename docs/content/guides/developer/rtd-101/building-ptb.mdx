...
description: Using the Rtd TypeScript SDK, you can create programmable transaction blocks to perform multiple commands in a single transaction.
...
This guide explores creating a programmable transaction block (PTB) on Rtd using the TypeScript SDK. For an overview of what a PTB is, see [Programmable Transaction Blocks](/concepts/transactions/prog-txn-blocks.mdx) in the Concepts section. If you don't already have the Rtd TypeScript SDK, follow the [install instructions](https://sdk.linkulabs.com/typescript/install) on the Rtd TypeScript SDK site.

This example starts by constructing a PTB to send Rtd. If you are familiar with the legacy Rtd transaction types, this is similar to a `payRtd` transaction. To construct transactions, import the `Transaction` class, and construct it:
...
tx.transferObjects([coin], tx.pure('address', '0xSomeRtdAddress'));
```
...
// Procure a list of some Rtd transfers to make:
...
After you have the Transaction defined, you can directly execute it with a `RtdClient` and `KeyPair` using `client.signAndExecuteTransaction`.
...
Inputs are how you provide external values to PTBs. For example, defining an amount of Rtd to transfer, or which object to pass into a Move call, or a shared object.
...
- For pure values: the `tx.pure(type, value)` function is used to construct an input for a non-object input.
  ...
  - In the new version, a more intuitive way of writing is provided, for example: `tx.pure.u64(100)`, `tx.pure.string('SomeString')`, `tx.pure.address('0xSomeRtdAddress')`, `tx.pure.vector('bool', [true, false])`...

## Available transactions

Rtd supports the following transaction commands:
...
- `tx.moveCall({ target, arguments, typeArguments })`: Executes a Move call. Returns whatever the Rtd Move call returns.
...
```ts
import { Inputs } from '@linku/rtd/transactions';
...
Rtd represents the gas budget in Rtd, and you should take the gas price of the PTB into account.
...
To serialize a PTB for sending to a wallet, Rtd recommends using the `tx.serialize()` function, which returns an opaque string representation of the PTB that can be passed from the Wallet Standard dApp context to your wallet. You can then convert this back into a `Transaction` using `Transaction.from()`.
