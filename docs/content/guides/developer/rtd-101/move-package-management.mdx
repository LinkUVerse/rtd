...
The Move package system supports 4 types of dependencies, each rtdted for different use cases:
 ...
 - System dependencies are used for built-in packages that are part of Rtd.
...
```toml
[dependencies]
usdc = { git = "https://github.com/circlefin/stablecoin-rtd.git", subdir = "packages/usdc", rev = "master" }
```
...
```toml
[dependencies.usdc]
git = "https://github.com/circlefin/stablecoin-rtd.git"
...
Several packages are built into Rtd. The `system` dependency type can be used to depend on these packages. The available system packages are `std`, `rtd`, `rtd_system`, `bridge`, and `deepbook`.

However:

 - The `std` and `rtd` packages are implicitly included unless you write `implicit-dependencies = false` in your `[package]` section. You do not need to explicitly include them.
...
To include a system dependency, write `{ system = "<name>" }`. For example, to use `rtd_system`, add `rtd_system = { system = "rtd_system" }` to your `[dependencies]` section.
...
This dependency is included whenever you run `rtd move test` or pass `-m test` to any `rtd move` command.
...
The right-hand side of the entry is the chain identifier for the network. You can find the chain ID using `rtd client chain-identifier`. The chain identifier is used to ensure dependencies agree on the meaning of the environment name, and to reset package addresses if the network is wiped and restarted.
...
```toml
[dependencies]
codec = { git = "https://github.com/rtd-potatoes/app.git", subdir = "packages/codec", rev = "codec@testnet-v2" }

[dep-replacements.mainnet]
codec = { git = "https://github.com/rtd-potatoes/app.git", subdir = "packages/codec", rev = "codec@mainnet-v2" }
```
...
Whenever you run `rtd move build` or `rtd move test`, the system ensures that you have all dependencies cached in your `~/.move` directory. It first checks whether the lockfile is current, and if it is not, it repins all of the dependencies for your build environment. It then checks whether the packages are cached, and if not it downloads them.
...
Run `rtd move update-deps` to repin all of your dependencies. This does not change your manifest file, it only redownloads the latest versions that are compatible with what you have written.

### Publishing and upgrading

During normal package publication and upgrades (`rtd client publish` or `rtd client upgrade`), the system rechecks that the dependencies are published on the given network and that the relevant chain IDs are consistent. The system also ensures that the additional on-chain linkage requirements are satisfied. For example, on-chain packages can have extra dependencies in their linkage that they do not actually use, so the system needs to include the on-chain linkages.
...
You can also publish or upgrade using the `rtd client ptb` command. See [Building Programmable Transaction Blocks](/guides/developer/rtd-101/building-ptb.mdx) for details.
...
Instead, the package manager supports ephemeral publication of a package and its dependencies for testing. The `rtd client test-publish` command works like a normal publish, except that the system reads the publication addresses for packages and writes them to a separate file.
...
The command `rtd client test-publish <pubfile> --build-env <env>` builds the root package for the `<env>` environment and then publishes it to a chain using the addresses from the ephemeral publication file `<pubfile>`.
...
| **Created by** | `rtd client publish` | `rtd client test-publish` |

## Register your package on `mvr`

Once you have published your package, you should consider adding it to the Move registry so that others can easily depend on it. See [the Move registry documentation](https://docs.rtdns.io/move-registry/managing-package-info) for more information.
