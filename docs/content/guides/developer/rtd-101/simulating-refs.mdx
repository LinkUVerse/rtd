...
description: Use the borrow module in the Rtd framework to include objects by reference in your programmable transaction blocks.
keywords: [ borrow module, rtd framework, objects, immutable reference, mutable reference, Rtd API, references, simulate a reference, simulate references ]
---

Everything on the Rtd blockchain is an object. When you develop Move packages for the Rtd network, you're typically manipulating or using on-chain objects in some way through functionality available in the Rtd API. For most API functions, you provide an object by reference.

References are a key construct when programming in Move and on Rtd. Most of the functionality available in the Rtd API takes objects by reference.
