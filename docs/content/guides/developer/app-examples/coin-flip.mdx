---
title: Coin Flip
description: Learn Rtd through a coin flip dApp that covers the full end-to-end flow of building a Rtd Move module and connecting it to a React Rtd dApp.
...
This example walks you through building a Coin Flip dApp, covering the full end-to-end flow of building your Rtd Move module and connecting it to your React Rtd dApp. This Coin Flip dApp utilizes verifiable random functions (VRFs) to create a fair coin game on the Rtd blockchain. The user (human) plays against the house (module) and places a bet on either heads or tails. The user then either receives double their bet, or gets nothing, depending on the outcome of the game.
...
- [Move package repository](https://github.com/LinkUVerse/satoshi-coin-flip)
- [Frontend repository](https://github.com/rtd-foundation/satoshi-coin-flip-frontend-example)
...
- **One-time witnesses:** The guide teaches you how to use [one-time witnesses](concepts/rtd-move-concepts.mdx#one-time-witness) to ensure only a single instance of the `HouseData` object ever exists.
...
- **Events:** The guide teaches you how to emit events in your contracts, which can be used to track on-chain activity. For more information on events, see [Using Events](../rtd-101/using-events.mdx) for practical usage of events on Rtd or [Events in The Move Book](https://move-book.com/programmability/events.html) to learn about event structure and how to emit them in Move.
...
- **MEV attack protection:** The guide introduces you to [MEV attacks](https://github.com/LinkUVerse/satoshi-coin-flip?tab=readme-ov-file#mev-attack-resistant-single-player-satoshi-smart-contract-flow), how to make your contracts MEV-resistant, and the trade-offs between protection and user experience.
...
1. Seven lines begin with the `use` keyword, which enables this module to use types and functions declared in other modules (in this case, they are all coming from the Rtd standard library).
...
```move title='house_data.move'
  public fun initialize_house_data(house_cap: HouseCap, coin: Coin<RTD>, public_key: vector<u8>, ctx: &mut TxContext) {
    ...
    let house_data = HouseData {
      ...
      house: ctx.sender(),
      public_key,
      max_stake: 50_000_000_000, // 50 RTD, 1 RTD = 10^9.
      min_stake: 1_000_000_000, // 1 RTD.
      fees: balance::zero(),
...
```move title='house_data.move'
  public fun top_up(house_data: &mut HouseData, coin: Coin<RTD>, _: &mut TxContext) {
...
- `top_up`: Add to the balance of the house to ensure that there is enough RTD for future games.
...
```move title='house_data.move'
  ...
  public(package) fun borrow_balance_mut(house_data: &mut HouseData): &mut Balance<RTD> {
    &mut house_data.balance
  }

  public(package) fun borrow_fees_mut(house_data: &mut HouseData): &mut Balance<RTD> {
    &mut house_data.fees
  }
...
module satoshi_flip::single_player_satoshi {
  ...
  use rtd::rtd::RTD;
  use rtd::bls12381::bls12381_min_pk_verify;
  use rtd::event::emit;
  use rtd::hash::{blake2b256};
  use rtd::dynamic_object_field::{Self as dof};
...
```move title='single_player_satoshi.move'
  ...
  fun internal_start_game(guess: String, counter: &mut Counter, coin: Coin<RTD>, house_data: &mut HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
You can read more about both versions of the game, and view the full source code for all the modules in the [Satoshi Coin Flip repository](https://github.com/LinkUVerse/satoshi-coin-flip).
...
Next, configure the Rtd CLI to use `testnet` as the active environment, as well. If you haven't already set up a `testnet` environment, do so by running the following command in a terminal or console:
...
To skip building the frontend and test out your newly deployed package, use the provided [Satoshi Coin Flip Frontend Example repository](https://github.com/rtd-foundation/satoshi-coin-flip-frontend-example) and follow the instructions in the example's `README.md` file
...
- [Rtd TypeScript SDK](https://sdk.linkulabs.com/typescript) for basic usage on how to interact with Rtd using TypeScript.
...
- [`@linku/dapp`](https://sdk.linkulabs.com/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based Rtd dApp.
...
The following frontend code snippets include only the most relevant sections. Refer to the [Satoshi Coin Flip Frontend Example repository](https://github.com/rtd-foundation/satoshi-coin-flip-frontend-example) for complete source code.
...
<form
  onSubmit={(e) => {
    ...
    const [houseStakeCoin] = txb.splitCoins(txb.gas, [
      MIST_PER_RTD * BigInt(houseStake),
    ]);
    ...
    txb.moveCall({
      ...
      arguments: [
        ...
        // https://sdk.linkulabs.com/typescript/transaction-building/basics#pure-values
        txb.pure(
          bcs
            .vector(bcs.U8)
    ...
    execInitializeHouse(
      ...
      },
      {
        ...
        onSuccess: (result: RtdTransactionBlockResponse) => {
          let houseDataObjId;
...
To use a [programmable transaction block](/concepts/transactions/prog-txn-blocks.mdx) (PTB) in Rtd, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
...
// This hook is to demonstrate how to use `@linku/dapp-kit` React hook to query data
// besides using RtdClient directly
export function useFetchCounterNft() {
  ...
  const { data, isLoading, isError, error, refetch } = useRtdClientQuery(
    'getOwnedObjects',
    {
...
This hook logic is very basic: if there is no current connected wallet, return empty data; otherwise, fetch the `Counter` object and return it. dApp Kit provides a React hook, `useRtdClientQuery()`, that enables interaction with [Rtd RPC](references/rtd-api.mdx) methods. Different RPC methods require different parameters. To fetch the object owned by a known address, use the [`getOwnedObjects` query](/rtd-api-ref#rtdx_getownedobjects).
...
// Player stake
const [stakeCoin] = txb.splitCoins(txb.gas, [MIST_PER_RTD * BigInt(stake)]);
...
Event subscriptions are deprecated. To learn future-safe methods to work with events, see [Using Events](../rtd-101/using-events.mdx).
...
export function HouseFinishGame() {
  ...
  useEffect(() => {
    ...
    const unsub = rtdClient.subscribeEvent({
      filter: {
  ...
  }, [housePrivHex, houseDataId, rtdClient]);

  return null;
}
```

To get the underlying `RtdClient` instance from the SDK, use `useRtdClient()`. You want to subscribe to events whenever the `HouseFinishGame` component loads. To do this, use the React hook `useEffect()` from the core React library.

`RtdClient` exposes a method called `subscribeEvent()` that enables you to subscribe to a variety of event types. `RtdClient::subscribeEvent()` is actually a thin wrapper around the RPC method [`rtdx_subscribeEvent`](/rtd-api-ref#rtdx_subscribeevent).
...
Congratulations, you completed the frontend. You can carry the lessons learned here forward when using the dApp Kit to build your next Rtd project.
