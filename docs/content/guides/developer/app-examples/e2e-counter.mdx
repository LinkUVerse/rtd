...
This example walks you through building a basic distributed counter app, covering the full end-to-end flow of building your Rtd Move module and connecting it to your React Rtd dApp. The app allows users to create counters that anyone can increment, but only the owner can reset. 
...
[Example source code](https://github.com/LinkUVerse/rtd/blob/60bb8bdc274b9e5706fd916cd84c13f81e832529/sdk/create-dapp/templates/react-e2e-counter)
...
- Your active address has RTD. Run `rtd client balance` in your terminal or console. If there is no balance, [acquire RTD](../getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/Move.toml" mode="code" org="LinkUVerse" repo="ts-sdks" />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" struct="Counter" org="LinkUVerse" repo="ts-sdks" noComments />

- The `Counter` type stores the address of its `owner`, its current `value`, and its own `id`.

### Creating `Counter`

<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" fun="create" org="LinkUVerse" repo="ts-sdks" noComments />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" fun="increment" org="LinkUVerse" repo="ts-sdks" noComments />

The `increment` function accepts a mutable reference to any shared `Counter` object and increments its `value` field.

<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" fun="set_value" org="LinkUVerse" repo="ts-sdks" noComments />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/move/counter/sources/counter.move" mode="code" org="LinkUVerse" repo="ts-sdks" noComments />

:::checkpoint

Your smart contract is complete. You should be able to run the `rtd move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `rtd move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
...
```sh
$ pnpm create @linku/dapp --template react-e2e-counter
```
...
```sh
$ yarn create @linku/dapp --template react-e2e-counter
```
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/networkConfig.ts" mode="code" org="LinkUVerse" repo="ts-sdks" noComments />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/CreateCounter.tsx" mode="code" org="LinkUVerse" fun="create" repo="ts-sdks" noComments />

</details>

The `create` function now creates a new Rtd `Transaction` and calls the `create` function from your Move module. The PTB is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
...
```tsx
  ...
  import { isValidRtdObjectId } from "@linku/rtd/utils";
  ...
  function App() {
    ...
    const [counterId, setCounter] = useState(() => {
      ...
      return isValidRtdObjectId(hash) ? hash : null;
    });
...
Add the following code to your `src/Counter.tsx` file:
  ...
  ```tsx
    ...
    export function Counter({ id }: { id: string }) {
      const currentAccount = useCurrentAccount();
      const { data, isPending, error, refetch } = useRtdClientQuery("getObject", {
        id,
        options: {
          showContent: true,
          showOwner: true,
    ...
    function getCounterFields(data: RtdObjectData) {
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/Counter.tsx" mode="code" org="LinkUVerse" repo="ts-sdks" noComments />
...
<ImportContent source="packages/create-dapp/templates/react-e2e-counter/src/App.tsx" mode="code" org="LinkUVerse" repo="ts-sdks" noComments />
