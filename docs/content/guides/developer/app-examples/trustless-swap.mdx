...
description: An app that performs atomic swaps on Rtd. Atomic swaps are similar to escrows but without requiring a trusted third party.
effort: large
...
This guide demonstrates how to make an app that performs atomic swaps on Rtd. Atomic swaps are similar to escrows but without requiring a trusted third party.
...
- You have the latest version of Rtd installed. If you run `rtd --version` in your terminal or console, it responds with the currently installed version.
- Your active environment is pointing to the expected network. Run `rtd client active-env` to make sure. If you receive a warning about a client and server API version mismatch, update Rtd using the version in the relevant branch (`mainnet`, `testnet`, `devent`) of the Rtd repo.
- Your active address has RTD. Run `rtd client balance` in your terminal or console. If there is no balance, [acquire RTD](/guides/developer/getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
In this part of the guide, you write the Move contracts that perform the trustless swaps. The guide describes how to create the package from scratch, but you can use a fork or copy of the example code in the Rtd repo to follow along instead. See [Hello, World!](/guides/developer/getting-started/hello-world.mdx) to learn more about package structure and how to use the Rtd CLI to scaffold a new project.
...
If you are targeting a network other than Testnet, be sure to update the `rev` value for the Rtd dependency.
...
The test starts with a helper function for creating an object. The object type does not matter, as long as it has the `store` ability. The test uses `Coin<RTD>`, because it comes with a `#[test_only]` function for minting:
...
- The `test_scenario` module is used to provide access to a `&mut TxContext` in the test (necessary for creating new objects). Tests that do not need to simulate multiple transactions but still need access to a `TxContext` can use `rtd::tx_context::dummy` to create a test context instead.
...
From your `escrow` folder, run `rtd move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass:
...
- [Full source code](https://github.com/LinkUVerse/rtd/blob/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/contracts/escrow/sources/shared.move)
- Concepts: [Shared Objects](/guides/developer/objects/object-ownership/shared)
- Concepts: [Shared Object Deletion](https://blog.rtd.io/ephemeral-shared-objects/)
...
`Locked` and `Key` objects are typically owned by the transaction sender, and so can be queried through the Rtd RPC, but `Escrow` objects are shared, and it is useful to be able to query them by their sender and recipient (so that users can see the trades they have offered and received).
...
<ImportContent source="examples/trading/contracts/escrow/sources/shared.move" mode="code" dep="rtd::event"  />
...
- Guide: [Using Events](../rtd-101/using-events.mdx)

:::

:::checkpoint

You now have `shared.move` and `locked.move` files in your `sources` folder. From the parent `escrow` folder, run `rtd move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass: 
...
- [Full source code](https://github.com/LinkUVerse/rtd/tree/705ee1ed3ce8cfadc4597c6facb6769d7dfb5896/examples/trading/api)
- Reference: [JSON-RPC](https://docs.rtd.io/rtd-api-ref)
...
<ImportContent source="initialize-rtd-client-cli.mdx" mode="snippet" />

Next, configure the Rtd CLI to use `testnet` as the active environment. 
...
```sh
$ rtd client new-env --alias testnet --rpc https://fullnode.testnet.rtd.io:443
```
...
Before being able to publish your package to Testnet, you need Testnet RTD tokens. To get some, run the following command: 

```sh
$ rtd client faucet
```

For other ways to get RTD in your Testnet account, see [Get RTD Tokens](/guides/developer/getting-started/get-coins).

Now that you have an account with some Testnet RTD, you can deploy your contracts. 

There are some helper functions to publish the smart contracts so you can create some demo data (for Testnet). The helper function to publish the smart contracts expects built smart contracts in both the `escrow` and `demo` directories. Run `rtd move build` in both directories, if necessary. Be sure to update the Rtd dependency in the manifest to point to the correct source based on your environment. 
...
- [Rtd TypeScript SDK](https://sdk.linkulabs.com/typescript). For basic usage on how to interact with Rtd with TypeScript.
- [Rtd dApp Kit](https://sdk.linkulabs.com/dapp-kit). To learn basic building blocks for developing a dApp in the Rtd ecosystem with React.js.
- [`@linku/dapp`](https://sdk.linkulabs.com/dapp-kit/create-dapp). This is used within this project to quickly scaffold a React-based Rtd dApp.
...
`RtdObjectDisplay.tsx`

</summary>

<ImportContent source="examples/trading/frontend/src/components/RtdObjectDisplay.tsx" mode="code"  />
...
The dApp Kit provides a set of hooks for making query and mutation calls to the Rtd blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`.
...
Fetch the owned objects directly from Rtd blockchain using the `useRtdClientInfiniteQuery()` hook from dApp Kit. This hook is a thin wrapper around Rtd blockchain RPC calls. Reference the documentation to learn more about these [RPC hooks](https://sdk.linkulabs.com/dapp-kit/rpc-hooks). Supply the RPC endpoint you want to execute, in this case the [`getOwnedObjects` endpoint](https://docs.rtd.io/rtd-api-ref#rtdx_getownedobjects). Supply the connected wallet account as the `owner`. The returned data is stored inside the cache at query key `getOwnedObjects`. In a future step you invalidate this cache after a mutation succeeds, so the data is re-fetched automatically.
...
Use the `useRtdClient()` hook from dApp Kit to retrieve the Rtd client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from dApp kit that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, use the `executeTransaction()` on the Rtd client instance of the Rtd TypeScript SDK.

### Generate demo data

:::info

The full source code of the demo bear smart contract is available at [Trading Contracts Demo directory](https://github.com/LinkUVerse/rtd/tree/main/examples/trading/contracts/demo)
...
As previously mentioned, this example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from Rtd blockchain RPC.

When you execute a transaction call to mutate data on the Rtd blockchain, use the `useMutation()` hook. The `useMutation()` hook accepts several inputs. However, you only need 2 of them for this example. The first parameter, `mutationFn`, accepts the function to execute the main mutating logic, while the second parameter, `onSuccess`, is a callback that runs when the mutating logic succeeds.
...
This component uses tanstack's `useInfiniteQuery` instead of `useRtdClientInfiniteQuery` since the data is being fetched from the example's API rather than Rtd. 
