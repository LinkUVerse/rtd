...
Objects on Rtd can either be immutable or mutable. An immutable object cannot be mutated, transferred, or deleted. These objects have no owner and are freely accessible to everyone.

## Create immutable objects

To make an object immutable, call the [`rtd::transfer::public_freeze_object`](/references/framework/rtd_rtd/transfer#rtd_transfer_public_freeze_object) function from the [transfer module](https://github.com/LinkUVerse/rtd/blob/main/crates/rtd-framework/packages/rtd-framework/sources/transfer.move):
...
You can see this function used in the [`color_object` example module](https://github.com/LinkUVerse/rtd/blob/main/examples/move/color_object/sources/example.move) tests. The test creates a new address-owned `ColorObject` object, then calls `public_freeze_object` to turn it into an immutable object.
...
The [`rtd::transfer::public_freeze_object`](/references/framework/rtd_rtd/transfer#rtd_transfer_public_freeze_object) function requires that you pass the object by value. If you are allowed to pass the object by a mutable reference, you could still mutate the object after the [`rtd::transfer::public_freeze_object`](/references/framework/rtd_rtd/transfer#rtd_transfer_public_freeze_object) call. This contradicts the fact that it should have become immutable.
...
After an object becomes immutable, the rules of who can use this object in Rtd Move calls change:

1. You can only pass an immutable object as a read-only, immutable reference to Rtd Move entry functions as `&T`.
...
```sh
$ export ADDR=`rtd client active-address`
```
```sh
$ rtd client objects $ADDR
```

Publish the `ColorObject` code on-chain using the Rtd CLI:

<ImportContent source="info-gas-budget.mdx" mode="snippet" />

```sh
$ rtd client publish $ROOT/examples/move/color_object --gas-budget <GAS-AMOUNT>
```

Set the package object ID to the `$PACKAGE` environment variable, if you have it set. Then create a new `ColorObject`:

```sh
$ rtd client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "create" --args 0 255 0
```
...
```sh
$ rtd client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "freeze_object" --args \"$OBJECT\"
```
...
```sh
$ rtd client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "update" --args \"$OBJECT\" 0 0 0
```
