...
`SwapRequest` defines a Rtd object type, wraps the `object` to swap, and tracks the original `owner` of the object. To define an interface to request a swap by someone who owns an `Object`:
...
View the full source code in the [`trusted_swap`](https://github.com/LinkUVerse/rtd/blob/main/examples/move/trusted_swap) example.
