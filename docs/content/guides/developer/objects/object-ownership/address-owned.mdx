...
description: Address-owned objects are owned by a Rtd 32-byte address, which can either be an account address or an object ID. Learn how to create and access these objects.
...
Use these [`transfer` module](https://github.com/LinkUVerse/rtd/blob/main/crates/rtd-framework/packages/rtd-framework/sources/transfer.move) functions to create address-owned objects:

- Use the [`rtd::transfer::transfer`](/references/framework/rtd_rtd/transfer#rtd_transfer_transfer) function if you are defining [custom transfer rules](/guides/developer/objects/transfers/custom-rules.mdx) for the object. 

- Use the [`rtd::transfer::public_transfer`](/references/framework/rtd_rtd/transfer#rtd_transfer_public_transfer) function to create an address-owned object if the object has the `store` capability.
...
You can see [`rtd::transfer::public_transfer`](/references/framework/rtd_rtd/transfer#rtd_transfer_public_transfer) function used in the [`color_object` example module](https://github.com/LinkUVerse/rtd/blob/main/examples/move/color_object/sources/example.move) tests. The test creates a new address-owned `ColorObject` object, then calls `public_transfer` to transfer it to the owner's address. 

Save the [`color_object` example code](https://github.com/LinkUVerse/rtd/blob/main/examples/move/color_object/sources/example.move), then publish the `ColorObject` code on-chain using the Rtd CLI:
