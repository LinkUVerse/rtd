...
description: Learn about transfer policies on Rtd.
keywords: [ transfer, policy, transferpolicy, transfer receipt, transfer request, transferrequest ]
---

A [`TransferPolicy`](https://docs.rtd.io/references/framework/rtd_rtd/transfer_policy) on Rtd is a customizable primitive that enables the owner of a type to create custom rules that define how the type can be transferred. You can set any number of rules, such as paying a royalty or commission, and all must be satisfied in a single transaction for the transfer to succeed. A `TransferPolicy` can be used within a Rtd Kiosk marketplace or any other system that integrates the `TransferPolicy` primitive. 
...
module examples::dummy_rule {
    ...
    ///
    /// This example also illustrates that rules can add Coin<RTD>
...
This example module has no configuration and accepts a `Coin<RTD>` of any value.
...
The `rtd::transfer_policy` module provides public functions to access the fields `paid()`, `item()`, and `from()`.
