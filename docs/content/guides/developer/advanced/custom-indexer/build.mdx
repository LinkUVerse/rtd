...
description: Build a custom indexer using the `rtd-indexer-alt-framework` module. The example indexer demonstrates a sequential pipeline that extracts transaction digests from Rtd checkpoints and stores them in a local PostgreSQL.
...
- Connects to Rtd Testnet: Uses the remote checkpoint store at https://checkpoints.testnet.rtd.io.
...
Rtd provides checkpoint stores for both Mainnet and Testnet.

- Testnet: `https://checkpoints.testnet.rtd.io`
- Mainnet: `https://checkpoints.mainnet.rtd.io`
...
```sh
$ cargo new simple-rtd-indexer
$ cd simple-rtd-indexer
```
...
<ImportContent source="examples/rust/basic-rtd-indexer/Cargo.toml" mode="code" />

The manifest now includes the following dependencies:

- `rtd-indexer-alt-framework`: Core framework providing pipeline infrastructure.
...
```sh
$ createdb rtd_indexer
```

Get your connection details:

```sh
$ psql rtd_indexer -c "\conninfo"
```

If successful, your console should display a message similar to the following:

```sh
You are connected to database "rtd_indexer" as user "username" via socket in "/tmp" at port "5432".
```
...
```sh
$ PSQL_URL=postgres://username@localhost:5432/rtd_indexer
```
...
<ImportContent source="examples/rust/basic-rtd-indexer/diesel.toml" mode="code" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/up.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/migrations/YYYY-MM-DD-HHMMSS_transaction_digests/down.sql" mode="code" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/src/schema.rs" mode="code" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/src/models.rs" mode="code" />

Key annotations:

- `FieldCount`: Required by `rtd-indexer-alt-framework` for memory optimization and batch processing efficiency. It is used to limit the max size of a batch so that we don't exceed the postgres limit on the number of bind parameters a single SQL statement can have.
...
<ImportContent source="examples/rust/basic-rtd-indexer/src/handlers.rs" mode="code" struct="TransactionDigestHandler" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/src/handlers.rs" mode="code" tag="processordeps" />

After the `TransactionDigestHandler` struct, add the `Processor` code:

<ImportContent source="examples/rust/basic-rtd-indexer/src/handlers.rs" mode="code" tag="processor" />
...
<ImportContent source="crates/rtd-indexer-alt-framework/src/pipeline/processor.rs" mode="code" trait="Processor" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/src/handlers.rs" mode="code" tag="handlerdeps" />

Add the logic for `Handler` after the `Processor` code. The complete code is available at the end of this step.

<ImportContent source="examples/rust/basic-rtd-indexer/src/handlers.rs" mode="code" tag="handler" />
...
<ImportContent source="crates/rtd-indexer-alt-framework/src/pipeline/sequential/committer.rs" mode="code" tag="batch" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/src/handlers.rs" mode="code" />
...
<ImportContent source="crates/rtd-indexer-alt-framework/src/pipeline/sequential/committer.rs" mode="code" trait="Handler" />
...
<ImportContent source="examples/rust/basic-rtd-indexer/src/main.rs" mode="code" />
...
```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.rtd.io
```

**Using gRPC streaming from a full node (with remote checkpoint store fallback):**

```sh
$ cargo run -- --remote-store-url https://checkpoints.testnet.rtd.io --streaming-url https://fullnode.testnet.rtd.io:443
```

:::info

Allow incoming network requests if your operating system requests it for the `basic-rtd-indexer` application.
...
To confirm your data is accurate, copy any transaction digest from your database and verify it on RtdScan: [https://rtdscan.xyz/testnet/home](https://rtdscan.xyz/testnet/home)
...
<RelatedLink href="https://github.com/LinkUVerse/rtd/tree/main/crates/rtd-indexer-alt" label="Rtd Indexer Alt" desc="The `rtd-indexer-alt` crate in the Rtd repo." />
<RelatedLink href="https://github.com/LinkUVerse/mvr/tree/main/crates/mvr-indexer" label="Move Registry" desc="The indexer that the Move Registry (MVR) implements." />
<RelatedLink href="https://github.com/LinkUVerse/deepbookv3/tree/main/crates/indexer" label="DeepBook Indexer" desc="The indexer that DeepBook implements." />
