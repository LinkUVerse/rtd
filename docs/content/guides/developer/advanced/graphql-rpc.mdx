---
title: Querying Rtd RPC with GraphQL
description: Introductory guide to making queries of the Rtd RPC using the GraphQL service.
keywords: [ rpc, rtd rpc, graphql, how to use graphql, schema, graphql schema, query transactions, query blocks, query block info, query dynamic field, execute transaction ]
---
...
- Mainnet: https://graphql.mainnet.rtd.io/graphql
- Testnet: https://graphql.testnet.rtd.io/graphql
- Devnet: https://graphql.devnet.rtd.io/graphql

:::caution

Switch any apps that still use the GraphQL Alpha endpoints (`https://rtd-mainnet.linkulabs.com/graphql`, `https://rtd-testnet.linkulabs.com/graphql`, and `https://rtd-devnet.linkulabs.com/graphql`) as soon as possible. Migrate your apps to the GraphQL Beta endpoints to avoid disruption of service.

:::

For more details about some concepts used in the following examples, see the [GraphQL concepts](/concepts/data-access/graphql-rpc.mdx) page. Consult the [GraphQL reference](../../../references/rtd-graphql.mdx) for full documentation on the supported schema.
...
- `address.coins` and `address.stakedRtds` are being deprecated in favor of `address.objects` with the corresponding `type` filter.
...
Dry-run now supports the same input schema as `gRPC`'s [`SimulateTransaction`](/references/fullnode-protocol#rtd-rpc-v2-Transaction) in JSON form. This means that it can also accept transactions where the inputs have not been fully resolved or serialized, perform gas estimation, and coin selection. The feature is now called `simulateTransaction`.
...
Find all the transactions that touched (modified/transferred/deleted) a given object. Useful when you want to trace the flow of a `Coin`, `StakeRtd`, `NFT`, or similar object.
...
query ($address: RtdAddress!) {
  transactions(filter: {
    function: "0x3::rtd_system::request_add_stake"
    sentAddress: $address
  }) {
    nodes {
      digest
...
Paginate over the dynamic fields of an object. This works even when the object in question is [wrapped](/guides/developer/objects/object-ownership/wrapped.mdx) by using the <a href="/references/rtd-api/rtd-graphql/alpha/reference/operations/queries/owner.mdx" data-noBrokenLinkCheck='true'>owner query</a>. It can be used for iterating over the elements of on-chain data structures, like tables and bags. See [The Move Book](https://move-book.com/programmability/dynamic-collections.html) to learn more about dynamic collections available in Move.
...
Transaction execution takes 2 arguments, `transactionDataBcs` and `signatures`. `transactionDataBcs` is the serialized unsigned transaction data, which the [Rtd Client CLI](/references/cli/client.mdx) `client call` command can generate. Pass the `--serialize-unsigned-transaction` flag to the command to call a Move function. The [Rtd Keytool CLI](/references/cli/keytool.mdx) command `rtd keytool sign` can generate the `signatures`.
...
The examples in the following sections demonstrate the typical API calls. The calls are tabbed to show both the JSON-RPC call and the equivalent GraphQL structure. For a comprehensive list of all available GraphQL features, consult the [reference](/references/rtd-graphql).
...
Return all `Coin<0x2::rtd::RTD>` objects that an address owns.
...
query {
  address(address: "0x5094652429957619e6efa79a404a6714d1126e63f551f4b6c7fb76440f8118c9") {
    objects(
      ...
      filter: { type: "0x2::coin::Coin<0x2::rtd::RTD>" }
    ) {
      nodes {
        address
      }
...
Find all versions of the Rtd framework and list their modules:
...
<RelatedLink to="/references/rtd-graphql.mdx" />
<RelatedLink to="/concepts/data-access/graphql-indexer" />
<RelatedLink href="https://graphql.testnet.rtd.io/graphql" label="Rtd Testnet GraphiQL" desc="Rtd GraphiQL IDE for Testnet." />
<RelatedLink href="https://graphql.mainnet.rtd.io/graphql" label="Rtd Mainnet GraphiQL" desc="Rtd GraphiQL IDE for Mainnet." />