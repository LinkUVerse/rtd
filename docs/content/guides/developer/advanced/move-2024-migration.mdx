...
Provide any feedback or report any issues you encounter through [GitHub](https://github.com/LinkUVerse/rtd/issues/new/choose), [Discord](https://discord.gg/Rtd), or [Telegram](https://t.me/RtdTokenNetwork).
...
1. Delete your existing `Move.lock` file (if it exists) to make sure you're using the newest `rtd-framework` version.
1. Perform 1 of the following:
   - Run `rtd move migrate` in the root of your Move project. See [Automatic migration](#automatic-migration).
...
Use the new [VSCode Move extension](https://marketplace.visualstudio.com/items?itemName=linku.move) to get support for Move 2024 features. The new extension has a number of improvements over the original [move-analyzer extension](https://marketplace.visualstudio.com/items?itemName=move.move-analyzer), but if you would like to keep using the original, be sure to rebuild and reinstall the `move-analyzer` binary to get 2024 support:

```sh
$ cargo install --git https://github.com/LinkUVerse/rtd.git move-analyzer
```

See the getting started guide on [Move IDEs and plugins](/references/contribute/rtd-environment.mdx#move-ides-and-plugins) for more information.
...
```move
use rtd::{balance, coin::{Self, Coin}};
```
...
Move 2024 revises how paths and namespaces work compared to legacy Move, toward easing `enum` aliasing in the future. Consider the following snippet from a test annotation in the `rtd_system` library:

```move
use rtd_system::rtd_system;
...
#[expected_failure(abort_code = rtd_system::validator_set::EInvalidCap)]
```

Legacy Move always treats a 3-part name as an address(`rtd_system`), module(`validator_set`), and module member (`EInvalidCap`). Move 2024 respects scope for `use`, so `rtd_system` in the attribute resolves to the module, producing a name resolution error overall.
