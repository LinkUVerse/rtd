...
`Random` has a reserved address `0x8`. See <UnsafeLink href="/references/framework/rtd-framework/rtd_rtd/random">random.move</UnsafeLink> for the Move APIs for accessing randomness on Rtd.
...
entry fun insecure_play(r: &Random, payment: Coin<RTD>, ...) {
  ...
...
<ImportContent source="crates/rtd-protocol-config/src/lib.rs" mode="code" struct="ProtocolConfig" />
...
module games::dice {
  ...
  public fun play_dice(guess: u8, fee: Coin<RTD>, r: &Random, ctx: &mut TxContext): Ticket {
    // Pay for the turn
...
For example, consider the `entry play_dice(guess: u8, fee: Coin<RTD>, r: &Random, ctx: &mut TxContext): Ticket { … }` function defined earlier, the attacker can publish the function
...
To protect against PTB-based composition attacks, Rtd rejects PTBs that have commands that are not `TransferObjects` or `MergeCoins` following a `MoveCall` command that uses `Random` as an input.
...
<RelatedLink href="https://docs.rtd.io/references/framework/rtd_rtd/random" label="random.move" desc="Documentation for the `random` module." />
<RelatedLink href="https://github.com/LinkUVerse/rtd/blob/main/examples/move/random/raffles" label="Raffle example" desc="GitHub repo containing a raffle smart contract demonstrating randomness." />
