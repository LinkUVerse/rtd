...
On Rtd, you can interact with various stablecoins such as USDC, USDT, Agora, and Ondo USDY.
...
For more detailed information on how to interact with USDC on Rtd, refer to the [USDC guide](#usdc-guide).
...
USDT is currently not issued natively on Rtd. For more information on bridging USDT to Rtd, refer to [RTD Bridging](../../concepts/tokenomics/rtd-bridging.mdx).
...
## How to use USDC on Rtd {#usdc-guide}

:::info
While this example uses USDC, the same principles can be applied to any asset on Rtd that uses the [Rtd Currency Standard](/standards/currency.mdx).
:::
...
The USDC stablecoin source code is available in the [circlefin/stablecoin-rtd](https://github.com/circlefin/stablecoin-rtd/blob/master/packages/usdc/sources/usdc.move) repository.
...
:::warning
The `usdc` package uses a specific version of the `rtd` package, which causes a version conflict with the `Rtd` package in the Rtd framework. You can override the version of the `Rtd` package in your `Move.toml` file to use a different version. Add the `override = true` flag to the `Rtd` package in your `Move.toml` file.
:::

### Using USDC in Move

USDC uses the [Rtd Currency Standard](/standards/currency.mdx) and can be used just like any other coin type in the Rtd framework.
...
Then use the `USDC` type just as you would use the `RTD` type when accepting a `Coin<RTD>` parameter.

<ImportContent source="examples/move/usdc_usage/sources/example.move" mode="code" fun="buy_sword_with_usdc" />

The following example demonstrates how to use the USDC stablecoin in a Move package and how it relates to using the `RTD` type as well as any generic coin types.

<ImportContent source="examples/move/usdc_usage/sources/example.move" mode="code" />

### Using USDC in PTBs

Use USDC in your [PTBs](/guides/developer/rtd-101/building-ptb) just like any other coin type.
