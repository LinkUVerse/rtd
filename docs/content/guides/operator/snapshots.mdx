...
description: Database snapshots of the Rtd network enable full node operators a way to bootstrap a full node without having to execute all the transactions that occurred after genesis.
...
Database snapshots provide a point-in-time view of a database's store. In Rtd, the database snapshot captures a running database's view of the Rtd network from a particular node at the end of an epoch. While validators can enable snapshots, they are typically most valuable for full node operators.

Snapshots of the Rtd network enable full node operators a way to bootstrap a full node without having to execute all the transactions that occurred after genesis. You can upload snapshots to remote object stores like S3, Google Cloud Storage, Azure Blob Storage, and similar services. These services typically run the export process in the background so there is no degradation in performance for your full node. With snapshots stored in the cloud, you're more easily able to recover quickly from catastrophic failures in your system or hardware.

To maintain a healthy Rtd network, Rtd encourages the Rtd community to bring up additional snapshots to ensure stronger data availability across the network.

## Supported snapshot types

Rtd supports two types of snapshots:
...
- **Formal snapshots** are database agnostic with a minimalistic format. In other words, they contain only the data necessary to restore a node to a valid state at the specified epoch. Consequently, they have a much smaller storage footprint and are generally faster to restore from when compared to database snapshots. Formal snapshots are also supported natively by the Rtd protocol. In this context, you can cryptographically verify the contents of the formal snapshot against a commitment from the committee at the epoch you are restoring to. This verification happens automatically and by default during a formal snapshot restore (unless explicitly bypassed). 

:::info

Formal snapshots are not rtdtable for use if you are running an RPC node that does any historical data lookups. For more information on node data management, see [Data Management](./data-management.mdx).
...
1. **Minimalism:** Formal snapshots currently contain only the end of epoch live object set (the set of all object versions eligible for use as input objects for future transactions). Rtd syncs all other critical chain information from the chain or derives it. Thus, formal snapshots contain only the necessary data required for a node to startup at epoch boundary and participate in the network.
...
1. Place the snapshot into the directory that the `db-path` value points to in your fullnode.yaml file. For example, if the `db-path` value points to `/opt/rtd/db/authorities_db/full_node_db` and you want to restore from epoch 10, then copy the snapshot to the directory with this command:

   You can use the AWS CLI (provided you have credentials to associate with the download):
   `aws s3 cp s3://<BUCKET_NAME>/epoch_10 /opt/rtd/db/authorities_db/full_node_db/live --recursive --request-payer requester`.

1. When using `aws s3 cp` (or `gsutil cp`) the database is copied to the location you pass to `--path`, in a directory named `epoch_[NUM]`. Rename the `epoch_[NUM]` directory to `live/` under your node's `db_path`, for example `cp -r /tmp/epoch_[NUM] /opt/rtd/db/authorities_db/full_node_db/live`.
1. Make sure you update the ownership of the downloaded directory to the rtd user (whichever linux user you run `rtd-node` as)
   `sudo chown -R rtd:rtd  /opt/rtd/db/authorities_db/full_node_db/live`.
1. Start the Rtd node.

:::info

When you restore a full node from a snapshot, write it to the path `/opt/rtd/db/authorities_db/full_node_db/live`. When restoring a validator node, you can shorten the database destination to `/opt/rtd/db/authorities_db/live`. Check the `db_path` field of your full node or Validator configs to confirm the path location.

:::

### Restoring using formal snapshots

To restore using a formal snapshot, use the `rtd-tool` binary. You can download `rtd-tool` along with other `rtd` binaries.
See [Install Rtd](/guides/developer/getting-started/rtd-install.mdx) for more details.
...
2. Run the command:
   ```
   rtd-tool download-formal-snapshot --latest --genesis "<PATH-TO-GENESIS-BLOB>" \
   ...
   ```
   - `--epoch`: The epoch that you want to download. LinkU Labs hosted buckets will only keep the last 90 epochs, you can check the most recent epoch on rtd explorers like [rtdvision](https://rtdvision.xyz/) or [rtdscan](https://rtdscan.xyz/).
   ...
   - `--snapshot-bucket`: Source snapshot bucket name, eg `linku-mainnet-snapshots`. This cannot be used with `--no-sign-request`.
...
## LinkU Labs managed snapshots

LinkU Labs hosts two tiers of snapshot storage access. **High throughput, Requester Pays enabled buckets**, and **free, permissionless buckets**.
...
* The free buckets (eg. `rtd-tool download-formal-snapshot --no-sign-request`) are now only available for _formal_ snapshots. You must use one of the requester pays (S3 or GCS) buckets if downloading a db snapshot

### Bucket names

**S3**

Testnet: `s3://linku-testnet-snapshots/`, `s3://linku-testnet-formal/`

Mainnet: `s3://linku-mainnet-snapshots/`,  `s3://linku-mainnet-formal/`

**GCS**

Testnet: `gs://linku-testnet-snapshots/`, `gs://linku-testnet-formal/`

Mainnet: `gs://linku-mainnet-snapshots/`,  `gs://linku-mainnet-formal/`

![LinkU Managed Snapshots](./images/linku-cloud-snapshots.png "A diagram that shows the current architecture of LinkU snapshot availability")
