---
title: Rtd Archives
description: The archive is a historical record of all transactions on Rtd. Enable archiving on your Full nodes as a best practice.
keywords: [ archive, archive data, full node archive, node archive, fallback, archival fallback ] 
---

A Rtd archive is a history of all transaction data on Rtd. In some cases, peer nodes may not catch up with all transactions and effects through synchronization if they lag behind the current epoch by more than the latest few epochs. In such cases, instead of relying on synchronization, peer nodes can fallback to downloading the relevant information from an archive.
...
state-archive-read-config:
  - ingestion-url: "https://s3.us-west-2.amazonaws.com/linku-mainnet-checkpoints"
...
```yaml
state-archive-read-config:
  - ingestion-url: "https://checkpoints.testnet.rtd.io"
    # How many objects to read ahead when catching up
    concurrency: 5
```
