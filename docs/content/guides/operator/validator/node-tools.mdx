...
description: Use the Rtd CLI to interact with a full node's validator commands. 
keywords: [ rtd cli, full node, delegator, validator, operation cap, report validators, join a committee, leave a committee, report a validator, proof of possession, generate proof of possession ]
sidebar_position: 5
sidebar_label: Tools
---

This guide focuses on using the Rtd CLI `validator` commands. 
...
- [x] Complete all [Rtd installation prerequisites](/guides/developer/getting-started/rtd-install.mdx#prerequisites).

- [x] Build the `rtd` binary, which you need for the genesis ceremony. You can do this on any computer.

<details className="nudge-details">

<summary>

View `rtd` binary installation details.
...
View Rtd account and CLI environment instructions.

</summary>

    - If this is the first time running this program, it asks you to provide a Rtd full node server URL and a meaningful environment alias. It also generates a random key pair in `rtd.keystore` and a config `client.yaml`. Swap in your validator account key if you already have one.
    ...
    - If you already set it up, just make sure:
      ...
      - `rtd.keystore` contains your account key pair.
...
```sh
$ rtd validator display-metadata
```
...
## Using Rtd CLI 
Use the Rtd CLI to perform validator tasks.
...
```sh
$ rtd validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```
...
```sh
$ rtd validator become-candidate path/to/validator.info
```
...
Then, you must acquire 30M RTD and stake it to the validator staking pool. You can add Rtd to the validator staking pool with the command:

```sh
$ rtd client call --package 0x3 --module rtd_system --function request_add_stake --args 0x5 {rtd_object_id} {validator_address}
```

This creates a stake object (of type `0x3::staking_pool::StakedRtd`). This object can be withdrawn with the following command:

```sh
$ rtd client call --package 0x3 --module rtd_system --function request_withdraw_stake --args 0x5 {stake_object_id}
```
...
```sh
$ rtd validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
```
...
```sh
$ rtd validator display-metadata <validator-address>
```
...
```sh
$ rtd validator update-metadata --help
```
...
```sh
$ rtd validator update-gas-price <gas-price>
```

If the account is a delegatee:

```sh
$ rtd validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```sh
$ rtd validator report-validator <reportee-address>
```
