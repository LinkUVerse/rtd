---
title: Rtd Bridge Validator Node Configuration
sidebar_label: Rtd Bridge Node Configuration
...
- [x] Install `rtd` and `rtd-bridge-cli`.
...
```sh
$ cargo install --locked --git "https://github.com/LinkUVerse/rtd.git" rtd rtd-bridge-cli
```

Install with a commit sha:

```sh
$ cargo install --locked --git "https://github.com/LinkUVerse/rtd.git" --rev {SHA} rtd rtd-bridge-cli
```
...
- A REST API URL where the bridge node listens to and serves requests. Example: `https://bridge.example-rtd-validator.io:443`. Make sure the port is correct and the URL does not contain any invalid characters, like quotes for example.

To create a `BridgeAuthorityKey`, run
```sh
$ rtd-bridge-cli create-bridge-validator-key <PATH-TO-WRITE>
```
...
After you have both authority key file and REST API URL ready, you can register them by using Rtd CLI:

```sh
$ rtd validator register-bridge-committee --bridge-authority-key-path <BRIDGE-AUTHORITY-KEY-PATH> --bridge-authority-url <BRIDGE-AUTHORITY-URL>
```
...
```sh
$ rtd client execute-signed-tx
```

#### Update metadata (before committee is finalized)

Both key and URL are changeable **before the committee is finalized**. If you wish to update metadata, simply rerun `rtd validator register-bridge-committee`.
...
```sh
$ rtd-bridge-cli view-bridge-registration --rtd-rpc-url {RTD-FULLNODE-URL}
```
...
```sh
$ rtd validator update-bridge-committee-node-url
```
...
Use `rtd-bridge-cli` command to create a template. If you want to run `BridgeClient` (see the following section), pass `--run-client` as a parameter.

```sh
$ rtd-bridge-cli create-bridge-node-config-template {PATH}
$ rtd-bridge-cli create-bridge-node-config-template --run-client {PATH}
```
...
| `bridge-authority-key-path` | The path to the Bridge Validator key, generated from `rtd-bridge-cli create-bridge-validator-key` command referenced previously. |
...
| `rtd:rtd-rpc-url` | Rtd RPC URL. |
| `rtd:rtd-bridge-chain-id` | `0` for Rtd Mainnet, `1` for Rtd Testnet. |
...
| `metrics:push-url` | The url of the remote Rtd metrics pipeline (for example, `https://metrics-proxy.[testnet_OR_mainnet].rtd.io:8443/publish/metrics`). See the [metrics push section](#metrics-push) that follows for more details. | 
...
| `rtd:bridge-client-key-path` | The file path of Bridge Client key. This key can be generated with `rtd-bridge-cli create-bridge-client-key` as previously shown. When `run-client` is true but you do not provide `rtd:bridge-client-key-path`, it defaults to use the Bridge Validator key to submit transactions on Rtd. This is not recommended for the sake of key separation. | 

### BridgeClient

`BridgeClient` orchestrates bridge transfer requests. It is **optional** to run for a `BridgeNode`. `BridgeClient` submits transaction on the Rtd network. Thus when it's enabled, you need a Rtd account key with enough RTD balance.
...
```sh
$ rtd-bridge-cli create-bridge-client-key <PATH_TO_BRIDGE_CLIENT_KEY>
```
This prints the newly created Rtd Address. Then we need to fund this address with some RTD for operations.
...
- Use `cargo install`.
    ```sh
    $ cargo install --locked --git "https://github.com/LinkUVerse/rtd.git" --branch {BRANCH-NAME} rtd-bridge
    ```
    Or
    ```sh
    $ cargo install --locked --git "https://github.com/LinkUVerse/rtd.git" --rev {SHA-NAME} rtd-bridge
    ```
- Compile from source code
    ...
    ```
    ```sh
    $ cargo build --release --bin rtd-bridge
    ```
- Use `curl`/`wget` pre-built binaries (for Linux/AMD64 only).
    ```sh
    $ curl https://rtd-releases.s3.us-east-1.amazonaws.com/{SHA}/rtd-bridge -o rtd-bridge
    ```
- Use pre-built Docker image. Pull from Docker Hub: `linku/rtd-tools:{SHA}`


### Run bridge node

Running bridge node is similar to running a Rtd node using systemd or Ansible. The command to start the bridge node is:

```sh
$ RUST_LOG=info,rtd_bridge=debug rtd-bridge --config-path {BRIDGE-NODE-CONFIG-PATH}
```
...
You can find a full list of bridge node metrics and their descriptions in the [`rtd-bridge` crate](https://github.com/LinkUVerse/rtd/blob/main/crates/rtd-bridge/src/metrics.rs).
