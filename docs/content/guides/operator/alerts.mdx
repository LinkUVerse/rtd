---
title: Rtd Validator Alert Reference
...
When running a Rtd Validator node or Full node, you may want to configure alerting based off some or all of the following metrics.
...
| Key          | Value                                     |
| ------------ | ----------------------------------------- |
| **Name**     | `LinkU validators are not upgraded`      |
| **Summary**  | Validators are behind on protocol version |
| **Duration** | `1h`                                      |

```sh
min(rtd_configured_max_protocol_version{network="$network", host=~"LinkU-.*"})
  < quantile(0.34, rtd_configured_max_protocol_version{network="$network"})
```
