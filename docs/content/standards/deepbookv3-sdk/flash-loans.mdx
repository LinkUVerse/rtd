...
// Borrow 1 DEEP from DEEP_RTD pool
// Swap 0.5 DBUSDC for RTD in RTD_DBUSDC pool, pay with deep borrowed
// Swap RTD back to DEEP
// Return 1 DEEP to DEEP_RTD pool
flashLoanExample = async (tx: Transaction) => {
  const borrowAmount = 1;
  const [deepCoin, flashLoan] = tx.add(this.flashLoans.borrowBaseAsset('DEEP_RTD', borrowAmount));
  ...
  const loanRemain = tx.add(
    this.flashLoans.returnBaseAsset('DEEP_RTD', borrowAmount, baseOut2, flashLoan),
  );
