...
To use the SDK in your projects, install the `@linku/deepbook-v3` package, which includes the margin trading functionality.
...
class DeepBookMarginTrader {
	...
	getMarginManagers(): { [key: string]: MarginManager } {
		...
		const poolKey = process.env.POOL_KEY || 'RTD_DBUSDC';
...
(async () => {
	...
	traderClient.marginManager.borrowBase('MARGIN_MANAGER_1', 'RTD_DBUSDC', 100)(tx);
