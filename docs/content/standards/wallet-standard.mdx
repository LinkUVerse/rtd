...
keywords: [ linku/wallet-standard, wallet standards, Wallet Standard, browser extensions, exposing accounts, wallet events, register in window, wallet best practices, efficient transaction execution, wallet data, managing wallets, wallet-standard ]
pagination_prev: null
---

Browser extension wallets built for Rtd use the [Wallet Standard](https://github.com/wallet-standard/wallet-standard/). This is a cross-chain standard that defines how dApps can automatically discover and interact with wallets.

If you are building a wallet, the helper library `@linku/wallet-standard` provides types and utilities to help get started.
...
`@linku/wallet-standard` to help ensure your class adheres to the standard.

```tsx
import { RTD_DEVNET_CHAIN, Wallet } from '@linku/wallet-standard';
...
class YourWallet implements Wallet {
	...
	// Return the Rtd chains that your wallet supports.
	get chains() {
		return [RTD_DEVNET_CHAIN];
	}
}
```
...
Current methods:
- `rtd:signTransaction` - Use to prompt the user to sign a transaction and return the serialized
  transaction and signature back to the dApp. This method does not submit the transaction for
  execution.
- `rtd:signAndExecuteTransaction` - Use to prompt the user to sign a transaction, then submit it
  for execution to the blockchain.

Legacy methods:
- `rtd:signTransactionBlock` - The legacy version of `rtd:signTransaction`. Many existing dApps still rely on this method.
- `rtd:signAndExecuteTransactionBlock` - The legacy version of `rtd:signAndExecuteTransaction`. Many existing dApps still rely on this method.

**Personal message signing**
- `rtd:signPersonalMessage` - Use to prompt the user to sign a personal message and return the
...
} from "@linku/wallet-standard";

class YourWallet implements Wallet {
	/* ... existing code from above ... */

  get features(): ConnectFeature & EventsFeature & RtdFeatures {
    return {
      [StandardConnect]: {
        version: "1.0.0",
...
```tsx
import { ReadonlyWalletAccount } from '@linku/wallet-standard';
		...
		return someWalletAccounts.map(
			(walletAccount) =>
				new ReadonlyWalletAccount({
					address: walletAccount.rtdAddress,
					publicKey: walletAccount.pubkey,
					// The Rtd chains that this account supports. This can be a subset of the wallet's supported chains.
          // These chains must exist on the wallet as well.
					chains: [RTD_DEVNET_CHAIN],
...
```tsx
import { registerWallet } from '@linku/wallet-standard';
...
```tsx
import { getWallets } from '@linku/wallet-standard';
...
Construct the transaction separately with the `@linku/rtd` library and then sign it with the private key of the user. Use the `rtd:signTransaction` feature to achieve this:

```tsx
wallet.features['rtd:signTransaction'].signTransaction({
...
To execute the transaction, use `RtdClient` from `@linku/rtd`:

```tsx
const client: RtdClient
...
Many wallets abstract the above flow into one feature: `rtd:signAndExecuteTransaction`. The required arguments for this feature are the raw transaction and the options with the desired information to be included in the response:
...
LinkU Labs offers a bare bones scaffold for React-based applications called `@linku/dapp-kit`. See the [dApp Kit documentation](https://sdk.linkulabs.com/dapp-kit) for more information.
