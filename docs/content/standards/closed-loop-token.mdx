...
Using the Closed-Loop Token standard, you can limit the applications that can use the token and set up custom policies for transfers, spends, and conversions. The [`rtd::token` module](https://github.com/LinkUVerse/rtd/blob/main/crates/rtd-framework/docs/rtd/token.md) in the Rtd framework defines the standard.

## Background and use cases

The [Currency Standard](/standards/currency.mdx) on Rtd is an example of an open-loop system. Coins are free-flowing, [wrappable](/guides/developer/objects/object-ownership/wrapped.mdx), [freely transferable](/guides/developer/objects/transfers/custom-rules.mdx#the-store-ability-and-transfer-rules) and you can store them in any application. 
