...
keywords: [ pool, create pool, permissionlessly, permissionless, RTD, USDC, RTD/USDC, DEEP price point, MIST ]
---
...
<ImportContent source="packages/deepbook/sources/pool.move" mode="code" org="LinkUVerse" repo="deepbookv3" fun="create_permissionless_pool" noComments />

Tick size should be 10^(9 - base_decimals + quote_decimals - decimal_desired). For example, if creating a RTD(9 decimals)/USDC(6 decimals) pool, with a desired decimal of 3 for tick size (0.001), tick size should be 10^(9 - 9 + 6 - 3) = 10^(3) = 1000.
...
<ImportContent source="packages/deepbook/sources/pool.move" mode="code" org="LinkUVerse" repo="deepbookv3" fun="add_deep_price_point" noComments />
...
1. Either the base or quote asset must be `USDC` or `RTD`.
...
For a pool with `RTD` as an asset, use the `DEEP/RTD` pool at `0xb663828d6217467c8a1838a03793da896cbe745b150ebd57d82f814ca579fc22` as the reference pool.
...
<ImportContent source="packages/deepbook/sources/pool.move" mode="code" org="LinkUVerse" repo="deepbookv3" fun="update_allowed_versions" noComments />
