...
keywords: [ deepbook, deep book, deepbook v3, typescript, typescript SDK, linku/deepbook, deepbookV3 SDK, Coin, Pool, Manager ] 
---
...
- [SDK repository](https://github.com/LinkUVerse/ts-sdks/tree/main/packages/deepbook-v3)
- [NPM version](https://www.npmjs.com/package/@linku/deepbook-v3)

## Install

To use the SDK in your projects, install the `@linku/deepbook` package.

```sh npm2yarn
npm install @linku/deepbook-v3
```
...
```ts reference
https://github.com/LinkUVerse/ts-sdks/blob/main/packages/deepbook-v3/src/utils/constants.ts
```
...
To work with DeepBookV3, you must create a `DeepBookClient`. To construct the `DeepBookClient`, pass in a `RtdClient`, the sender address, and environment. The [Rtd TypeScript SDK](https://sdk.linkulabs.com/typescript) provides the `RtdClient` and key functionality necessary to process transactions. The following example imports those libraries, as well.

```tsx
import { DeepBookClient } from '@linku/deepbook-v3';
...
class DeepBookMarketMaker {
	...
	rtdClient: RtdClient; // For executing transactions
	...
	getActiveAddress() {
		return this.keypair.toRtdAddress();
	}
}
```
...
class DeepBookMarketMaker {
	...
	async createBalanceManagerAndReinitialize() {
		...
		const res = await this.rtdClient.signAndExecuteTransaction({
			transaction: tx,
			signer: this.keypair,
			options: {
				showEffects: true,
...
```tsx
import { decodeRtdPrivateKey } from '@linku/rtd.js/cryptography';
import { getFullnodeUrl, RtdClient } from '@linku/rtd/client';
...
export class DeepBookMarketMaker extends DeepBookClient {
	...
	constructor(
		...
		const address = resolvedKeypair.toRtdAddress();

		super({
			address: address,
			env: env,
			client: new RtdClient({
				url: getFullnodeUrl(env),
			}),
			balanceManagers: balanceManagers,
			adminCap: adminCap,
	...
	signAndExecute = async (tx: Transaction) => {
		// remove arguments
		return this.rtdClient.signAndExecuteTransaction({
			transaction: tx,
			signer: this.keypair,
			options: {
				showEffects: true,
...
(async () => {
	...
	// Read only call
	console.log(await mmClient.checkManagerBalance('MANAGER_1', 'RTD'));
	console.log(await mmClient.getLevel2Range('RTD_DBUSDC', 0.1, 100, true));
...
<RelatedLink href="https://github.com/LinkUVerse/ts-sdks/tree/main/packages/deepbook-v3" label="DeepBookV3 Indexer repository" desc="The DeepBookV3 SDK repository on GitHub." />
<RelatedLink href="https://www.npmjs.com/package/@linku/deepbook-v3" label="DeepBookV3 SDK node package" desc="The DeepBookV3 SDK node package on NPM." />
