...
<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/coin.move" mode="code" fun="deny_list_v2_add" noComments />
...
<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/coin.move" mode="code" fun="deny_list_v2_remove" noComments />
...
You can use either the TypeScript or Rust SDK to manipulate the addresses held in the `DenyList` for your coin. The following examples are based on the [regulated coin sample](https://github.com/LinkUVerse/regulated-coin-sample).
...
tx.moveCall({
    ...
    arguments: [
        tx.object(<RTD-DENY-LIST-OBJECT-ID>),
...
- `<RTD-DENY-LIST-OBJECT-ID>` is `"0x403"`.
...
- `cmd.address()` returns the address to ban as a `RtdAddress`.
...
<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/coin.move" mode="code" fun="deny_list_v2_enable_global_pause" noComments signatureOnly />
...
<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/coin.move" mode="code" fun="deny_list_v2_disable_global_pause" noComments signatureOnly  />