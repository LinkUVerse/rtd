...
1. Search for `linku.move`.
1. Click **Install** on the Move extension by LinkU Labs.

Alternative install methods include:

  - Use <kbd>Ctrl</kbd> + <kbd>P</kbd> or <kbd>⌘</kbd> + <kbd>P</kbd> and type `ext install linku.move`.
  - Use the command line: 

      ```sh
      $ code --install-extension linku.move
      ```
...
```sh
$ cargo install --git https://github.com/LinkUVerse/rtd.git rtd-move-lsp
```

By default, the Move extension expects to find the `move-analyzer` binary in `~/.rtd/bin`. You can either copy the binary to this location, or configure the extension to use a different path. 
...
These commands find the `Move.toml` file for the open Move source file and open a terminal to run the appropriate `rtd move` command.

To execute these commands you must have `rtd` binary installed, in particular, to generate a trace, the `rtd` binary version must be trace-generation capable. See [Debugger](./debugger.mdx#install) for installation instructions. The extension will search for the `rtd` binary on a system path but you can configure the extension to use a different path.
