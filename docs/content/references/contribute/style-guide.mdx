...
title: Style Guide
slug: /style-guide
description: All contributions to the Rtd documentation must adhere to the style guide.
...
This document defines the styles, vocabulary usage, and content formatting for Rtd documentation. Entries are in alphabetical order. A style guide is never finished; expect continued iterations to add additional styles, additional information to existing styles, and infrequently a change to an existing style.
...
> ✅ You can view the transaction history in the Rtd Explorer.

> ❌ We can view the transaction history in the Rtd Explorer.
...
> ✅ You must install Cargo, Rust, Docker, and the Rtd CLI to create a Rtd node.
...
- Capitalize _Rtd Full Node_ when referring to the official software binary/package that Rtd distributes.
    “Download and run the Rtd Full Node from GitHub to connect to Mainnet.”
...
Product names are proper nouns. Capitalize all words of a product name. When referring to a product, use only the product name without "the". When referring specifically to a Rtd wallet, use Rtd Wallet or Ethos Wallet and not just wallet. Users likely have multiple wallets, and we want to make it clear which wallet. Use wallet generically when referring to the concept of a wallet.

> There are several types of wallets to choose from.

> Never share the recovery passphrase for your wallet with anyone.

> The Rtd network supports the following wallets:
> - Rtd Wallet
> - Ethos Wallet
> - Coinbase Wallet
...
> Example: You can mint non-fungible tokens (NFTs) using your Rtd Wallet. To view an NFT after you mint it, click the NFTs tab of your wallet.
...
A better option is **Get Started with Rtd**, but even this is still too broad. Readers want guidance for a specific task or journey. Instead, use precise titles such as **Create a Rtd Full Node** or **Install Rtd Tooling**. These tell the reader exactly what they will learn or accomplish.
...
> ✅ Correct usage: 
> Page 1: Page title "Rtd Gas Profiling" with heading "Environment configuration"
> Page 2: Page title "Rtd Indexing" with heading "Environment configuration"

> ❌ Incorrect usage: 
> Page 1: Page title "Rtd Gas Profiling" with heading "Environment configuration"
> Page 2: Page title: "Rtd Features" with heading "Rtd gas profiling"
...
> ##### Section heading: Install `rtdup`

> Body content: Install `rtdup` using the command: ...
...
Rtd Explorer supports the following browsers:
...
- CLI commands when used within a sentence: If using the suggested location, type `export PATH=$PATH:~/rtd` and press Enter.
...
To get the latest version of the Rtd Wallet extension:

1. Open Google Chrome.
1. Click **Extensions**, then click **Manage Extensions**.
1. Click **Details** for the Rtd Wallet extension, then click **View in Chrome Web Store**.
...
Always use full, relative links when linking to topics on [docs.rtd.io].
...
> To learn more, see [Examples of Rtd Smart Contracts](/guides/developer/app-examples.mdx).
...
> Before you install Rtd, make sure to install the [prerequisites](/guides/developer/getting-started/rtd-install.mdx#prerequisites).
...
You can also just refer to a topic by title where it makes sense. See Installing Rtd for more information.
...
Running `target/release/rtd-node`
...
INFO: Rtd node is now running
```
...
You must install Rust before you can build Rtd from source.
