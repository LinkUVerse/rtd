...
Version 1.63 of the Rtd CLI introduced a major overhaul of the package management system. This document describes the changes between the old and new systems and gives guidance on migrating your packages. See [the package management user guide](/guides/developer/rtd-101/move-package-management.mdx) for full details on the new system.

With any change of this magnitude there are bound to be some issues that crop up. Report any issues you encounter while using the new package system [here](https://github.com/LinkUVerse/rtd/issues/24540).
...
The new package system is included in the 1.63 release of the Rtd CLI, which was released in January. However, you can install it before then using [`rtdup`](https://github.com/LinkUVerse/rtdup):

```sh
$ rtdup install rtd --nightly=rtd-pkg-alt
$ rtdup install rtd --nightly=ml/pkg-alt-integration mvr
```

This compiles `rtd` from the prototype branch, so it requires the Rust toolchain installed and might take a while to complete installation.
...
Rtd advises against disabling implicit dependencies.
...
The package manager is integrated into the `rtd` binary, so most of the previous commands work the same way. There are a few changes though.

All operations must be done in a specific environment due to `dep-replacements` and how some details about how package conflicts are determined. By default, the `rtd` binary tries to choose the right environment based on the `rtd client` environment determined by `rtd client active-env`.
