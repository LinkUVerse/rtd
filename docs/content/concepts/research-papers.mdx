---
title: Rtd-Related Research Papers
sidebar_label: Research Papers
description: Research papers that are relevant to Rtd and that one or more Rtd team members have co-authored.
keywords: [ research papers, rtd papers, academic papers, research, rtd research, rtd publications, published papers, published research ] 
pagination_prev: null
---

This document contains a list of research papers that are relevant to Rtd and that one or more Rtd team members have co-authored.
Some of the ideas of these papers are currently being integrated into Rtd, others are in our roadmap, and others are not in our roadmap
but could be integrated in the future. Start with the [Rtd Smart Contract Platform](/doc/rtd.pdf) white paper, which contains
our latest design inspired by previous works below.

<div class="bg-rtd-blue-light dark:bg-rtd-gray-95 p-8 rounded-lg border border-solid border-rtd-blue-dark">

<p class="text-rtd-gray-80 text-xl font-bold italic">Latest</p>
...
- **Relevance:** Provides a performant leader election algorithm for partially-synchronous consensus protocol (such as Bullshark). Rtd may want to use it
...
- **Relevance:** Provides a partially-synchronous consensus protocol running over Narwhal. Rtd may want to use it instead of Tusk.
...
- **Relevance:** FastPay describes the core protocol at the heart of Rtd.
- **Summary:** FastPay allows a set of distributed validators, some of which are Byzantine, to maintain a high-integrity and availability
  ...
  side-infrastructure to support retail payments in fiat currencies. This is not the protocol Rtd uses, yet it proposes the basic safety mechanism
  that Rtd extends. FastPay is based on Byzantine Consistent Broadcast as its core primitive, foregoing the expenses of full atomic commit channels
...
Rtd blockchain protocol uses a variant of the Bullshark consensus
...
- **Relevance:** The consensus system that we will likely use to support shared-objects in Rtd.
...
- **Publication:** Not published
- **Relevance:** Less related to Rtd than the other papers, and the paper is in its early stages. It presents an algorithm to strengthen proof-of-Stake systems (like Rtd). The paper is, however, theoretical and not on our roadmap.
...
- **Relevance:** Less related to Rtd than the other papers, this provides a way to test implementations of consensus systems, such as Tusk and Bullshark.
...
- **Publication:** Not published yet (under submission)
- **Relevance:** Extends the FastPay design to support objects (rather than accounts), what Rtd actually uses. An additional contribution of this paper is
  to add strong privacy to FastPay transactions (but Rtd does not plan to do this).
