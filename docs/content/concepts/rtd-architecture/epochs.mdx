...
description: Epochs define time periods on Rtd where the validator set remains unchanged. Equivocation occurs when objects are used incorrectly across transactions. Reconfiguration adjusts network parameters at epoch boundaries.
keywords: [ epochs, equivocation, reconfiguration, validators, object versioning ]
---

On Rtd, an _epoch_ is a period of time defined by the network. During this time, the Rtd [validator](/guides/operator/validator/validator-config.mdx) set and their stakes remain unchanged. On both [Mainnet](/concepts/rtd-architecture/networks.mdx) and [Testnet](/concepts/rtd-architecture/networks.mdx), an epoch is about 24 hours. Epochs on Devnet are 1 hour. This timeframe allows validators to process transactions efficiently without worrying about ad hoc validator changes.
...
- Inadvertent logic traps in [smart contract](/guides/developer/getting-started/hello-world.mdx) code, for example, when handling [sponsored transactions](/guides/developer/rtd-101/sponsor-txn.mdx#client-equivocation-risk).
...
1. **Distribution of gas rewards:** Computation gas fees are distributed to the validator staking reward pool, from which stakers can withdraw. Storage fees are allocated to a storage fund, playing a vital role in the [Rtd tokenomics](/concepts/tokenomics.mdx).
