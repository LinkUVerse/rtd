...
description: On the Rtd network, checkpoints define the history of the blockchain. Checkpoint verification is how full nodes and other clients guarantee their state is exactly the same as the Rtd network.
---

Unlike traditional blockchains that create blocks before execution, Rtd creates checkpoints after transaction execution to provide a certified record of chain history.
...
Full nodes and validators verify a checkpoint to trust it. This verification confirms that the Rtd validator committee created it and that it is authentic.
...
This process creates an apparent circular dependency. The client needs to know the committee to verify checkpoints, but also learns the committee through checkpoint validation. The process is bootstrapped from the genesis checkpoint, the earliest checkpoint in a Rtd network. The genesis checkpoint contains the initial validator committee, which allows the client to verify all checkpoints in history by following this process:
...
1. The client uses the state sync network or Rtd archive to obtain the next checkpoint.
...
By executing checkpoints and verifying transaction outputs, a full node can build the entire state of the Rtd network (the collection of objects in the network) and trust that every byte of every object is correct.