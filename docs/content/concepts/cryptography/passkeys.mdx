---
title: Passkey
description: Rtd supports the passkey signature scheme that enables you to sign in to apps and sign transactions using a private key stored securely on a passkey authenticator. It uses the WebAuthn standard.
---

Passkey provides a secure and user-friendly alternative for submitting transactions to Rtd. Built on the **WebAuthn standard**, passkey allows users to authenticate and sign transactions using:
...
By supporting the passkey signature scheme, Rtd improves security and accessibility, making it easier for users to manage their accounts with hardened security. Passkey-based wallets are tied to the origin, meaning they cannot be phished or used on a different site. This makes passkey a more secure authentication option.

See the [TypeScript SDK documentation](https://sdk.linkulabs.com/typescript/cryptography/passkey) to learn how to add passkey support to your application. For the product specification, see [SIP-9](https://github.com/rtd-foundation/sips/blob/main/sips/sip-9.md).

:::info
Passkey support is available in beta in Rtd Devnet and Testnet. Mainnet release is not yet scheduled.  
:::

## Benefits of using passkey

- **Sign transactions securely:**  
  Users can sign transactions in Rtd using passkey, while the private key stays securely stored within the authenticator. This reduces the risk of key extraction attacks.  
