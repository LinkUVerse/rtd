...
description: Use GraphQL to make Rtd RPC calls. This feature is currently in Beta. 
keywords: [ graphql, graphql headers, x-rtd-rpc-version, x-rtd-rpc-show-usage, variables, fragments, pagination, graphql limits, make rpc call, rtd rpc calls ]
---
...
Switch any apps that still use the GraphQL Alpha endpoints (`https://rtd-mainnet.linkulabs.com/graphql`, `https://rtd-testnet.linkulabs.com/graphql`, and `https://rtd-devnet.linkulabs.com/graphql`) to the GraphQL Beta endpoints as soon as possible to avoid disruption of service.

- GraphQL Beta Mainnet: https://graphql.mainnet.rtd.io/graphql
- GraphQL Beta Testnet: https://graphql.testnet.rtd.io/graphql
- GraphQL Beta Devnet: https://graphql.devnet.rtd.io/graphql
...
- `x-rtd-rpc-version` to specify which RPC version to use (currently only one version is supported),
- `x-rtd-rpc-show-usage` returns the response with extra query complexity information.

By default, each response contains the following HTTP response headers:
- `x-rtd-rpc-request-id` a unique identifier for the request; appears in application logs for debugging.
- `x-rtd-rpc-version` the version of the service that handled the request.

```sh
$ curl -i -X POST https://graphql.testnet.rtd.io/graphql\
     --header 'x-rtd-rpc-show-usage: true'                 \
...
content-length: 179
x-rtd-rpc-request-id: f5442058-47ab-4360-8295-61c009f38516
x-rtd-rpc-version: 1.56.1-
...
```sh
$ curl -X POST https://rtd-testnet.linkulabs.com/graphql \
    --header 'Content-Type: application/json' \
    --data '{
...
- `pageInfo`, of type <UnsafeLink href="/references/rtd-api/rtd-graphql/beta/reference/types/objects/page-info">PageInfo</UnsafeLink>, which indicates whether there are more pages before or after the page returned.
...
The GraphQL service for Rtd RPC is rate-limited on all available instances to keep network throughput optimized and to protect against excessive or abusive calls to the service.
