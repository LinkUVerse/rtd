...
description: Overview of the Archival Store and Service to access historical Rtd network data.
beta: testnet, mainnet
keywords: [ grpc, g rpc, RPC, rtd full node grpc, full node rpc, what is grpc, grpc on rtd, graphql, graph ql, what is graphql, graphql rpc ]
---

The Archival Store and Service provide a scalable, consistent foundation for accessing historical data on Rtd beyond what Full nodes or indexer databases typically retain. They power rich developer experiences through [gRPC](./grpc-overview.mdx) and [GraphQL](/concepts/data-access/graphql-indexer.mdx) APIs while keeping storage and operations manageable.
...
- **Archival Store:** A long-term storage system (such as [Bigtable](https://cloud.google.com/bigtable)) that holds checkpoint-indexed Rtd data.
...
Access the Archival Service hosted by any participating provider. Rtd Foundation offers a public good service on a Bigtable-based store. This service has strict rate limits:

- Testnet: archive.testnet.rtd.io
- Mainnet: archive.mainnet.rtd.io
...
Run an indexer pipeline to keep the Archival Store updated. Rtd Foundation provides an ingestion mechanism for Bigtable:

- Tool: [rtd-kv-store](https://github.com/LinkUVerse/rtd/tree/main/crates/rtd-kvstore)
- Run the following script to initialize the Bigtable cluster:
```shell
$ ./crates/rtd-kvstore/src/bigtable/init.sh <name_of_bigtable_cluster>
```
- Run the ingestion daemon with:
```shell
$ cargo run --bin rtd-kvstore <name_of_your_bigtable_cluster> ingestion <mainnet|testnet>
```
...
- Reference implementation for Bigtable: [rtd-kv-rpc](https://github.com/LinkUVerse/rtd/tree/main/crates/rtd-kv-rpc).
