...
description: Overview of the types of data access mechanisms available in Rtd.
...
You can access Rtd network data like [Transactions](/concepts/transactions.mdx), [Checkpoints](/concepts/cryptography/system/checkpoint-verification.mdx), [Objects](/guides/developer/objects/object-model.mdx), [Events](/guides/developer/rtd-101/using-events.mdx), and more through the available interfaces. You can use this data in your application workflows, to analyze network behavior across applications or protocols of interest, or to perform audits on parts or the whole of the network.

This document outlines the interfaces that are currently available to access the Rtd network data, along with an overview of how that's gradually evolving. Refer to the following definitions for release stages mentioned in this document:
...
<div class="bg-rtd-ghost-white">
![Future state data serving stack](./images/dataservingstack.png)
</div>

Primary interfaces to access Rtd data include:
...
:::info
View the video below for a comparison of the latest and legacy Rtd data stacks. 
...
General-purpose Indexer is declarative in the sense that you can seamlessly configure it to load different kinds of Rtd network data into Postgres relational tables in parallel. This improves the performance of the data ingestion into the Postgres-compatible database. In addition, you can configure pruning for different tables in the Postgres-compatible database, allowing you to tune it for the desired combination of performance and cost characteristics.
...
| Tentative time | Milestone | Description |
| -------- | ------- | ------- |
| :heavy_check_mark: September 2025 | Beta release of GraphQL RPC Server and General-purpose Indexer. | You can start validating the setup of General-purpose Indexer, along with testing the GraphQL RPC Server to access the indexed Rtd data. You can also start migrating your application in the non-production environments, and share feedback on the improvements you want to see. |
...
If you need more control over the types, granularity, and retention period of the data that you need in your application, or if you have specific query patterns that are not served with gRPC or GraphQL RPC, then you can set up your own [custom indexer](/guides/developer/advanced/custom-indexer/build.mdx) or reach out to a [Data indexer operator](/references/awesome-rtd.mdx#indexers--data-services) that might already have set one up.
...
You can use the high-level criteria mentioned in the following table to determine whether gRPC API or GraphQL RPC with General-purpose Indexer would better serve your use case. It's not an exhaustive list and it's expected that either of the options could work rtdtably for some of the use cases.
...
Directly connect to [JSON-RPC](/references/rtd-api.mdx) hosted on Rtd [full nodes](guides/operator/rtd-full-node.mdx) that are operated by [RPC providers](https://rtd.io/developers#dev-tools) (filter by `RPC`) or [data indexer operators](https://github.com/rtd-foundation/awesome-rtd?tab=readme-ov-file#indexers--data-services).
  - The [Mainnet](https://fullnode.mainnet.rtd.io:443), [Testnet](https://fullnode.testnet.rtd.io:443), or [Devnet](https://fullnode.devnet.rtd.io:443) load balancer URLs abstract the Rtd Foundation-managed full nodes. Those are not recommended for production use.

You can get real-time or historical data using JSON-RPC. Retention period for historical data depends on the [pruning strategy](/guides/operator/data-management.mdx#rtd-full-node-pruning-policies) that node operators implement, though the default configuration for all full nodes is to implicitly fall back on the [Archival Store](#archival-store-and-service) managed by the Rtd Foundation.

:::caution

WebSocket-based JSON RPCs `rtdx_subscribeEvent` and `rtdx_subscribeTransaction` were deprecated in July 2024. Do not rely on those RPCs in your applications. Refer to [Future data access interfaces](#future-state-data-interfaces) to learn about the `gRPC subscriptions` alternative.
...
<RelatedLink href="/references/fullnode-protocol" label="Rtd Full Node gRPC" desc="Generated documentation from the Rtd gRPC schema." />
<RelatedLink href="/references/rtd-api/rtd-graphql/beta/reference" label="GraphQL Beta schema" desc="Schema documentation for GraphQL Beta." />