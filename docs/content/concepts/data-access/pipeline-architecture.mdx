...
description: The `rtd-indexer-alt-framework` provides two distinct pipeline architectures. Understand the differences between the sequential and concurrent pipelines that the `rtd-indexer-alt-framework` provides to decide which best rtdts your project needs.
keywords: [ indexer, index data, rtd-indexer, custom indexer, sequential pipeline, concurrent pipeline ]
---

The `rtd-indexer-alt-framework` provides 2 distinct pipeline architectures. Understanding their differences is crucial for choosing the right approach.
...
<li><span className="text-rtd-success-dark">✓</span> You want straightforward implementation with direct commits and simple queries.</li>
<li><span className="text-rtd-success-dark">✓</span> Team prefers predictable, easy-to-debug behavior.</li>
<li><span className="text-rtd-success-dark">✓</span> Current performance meets your requirements.</li>
<li><span className="text-rtd-success-dark">✓</span> Operational simplicity is valued.</li>
</ul>
...
<ul className="list-none pl-2">
<li><span className="text-rtd-success-dark">✓</span> Performance optimization is essential.</li>
<li><span className="text-rtd-success-dark">✓</span> Sequential processing cannot keep up with your data volume.</li>
<li><span className="text-rtd-success-dark">✓</span> Your team is willing to handle the additional implementation complexity for the performance benefits.</li>
</ul>
...
The `Broadcaster` and `Processor` components use identical backpressure mechanisms, `FANOUT` parallel processing, and `processor()` implementations to the concurrent pipeline.
...
<ImportContent source="crates/rtd-indexer-alt-framework/src/pipeline/sequential/mod.rs" mode="code" fun="batch" noComments />

`commit()`: Database write logic.

<ImportContent source="crates/rtd-indexer-alt-framework/src/pipeline/sequential/mod.rs" mode="code" fun="commit" noComments />
...
To avoid this pitfall, you can use watermark progress reporting. After successful commits, instruct the pipelines to send `(pipeline_name, highest_committed_checkpoint)` to the broadcaster via the `commit_hi_tx` channel.

<ImportContent source="crates/rtd-indexer-alt-framework/src/pipeline/sequential/committer.rs" tag="send" />
...
<ImportContent source="crates/rtd-indexer-alt-framework/src/ingestion/broadcaster.rs" tag="regulator" />
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MIN_EAGER_ROWS" />
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="MAX_PENDING_ROWS" />
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/mod.rs" struct="CommitterConfig" />
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/mod.rs" struct="WatermarkPart"/>
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/mod.rs" struct="CommitterConfig" highlight="watermark_interval_ms" />
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/concurrent/mod.rs" struct="PrunerConfig" highlight="delay_ms" />
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/concurrent/mod.rs" struct="PrunerConfig" highlight="prune_concurrency" />
...
<ImportContent mode="code" source="crates/rtd-pg-db/src/schema.rs" />
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/ingestion/mod.rs" struct="IngestionConfig" highlight="checkpoint_buffer_size" />   
    
**`Processor` to `Collector`**: `FANOUT + PIPELINE_BUFFER` slots → All workers block on `send()`.

<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/concurrent/mod.rs" variable="processor_tx" />
    
**`Collector` to `Committer`**: `write_concurrency + PIPELINE_BUFFER` slots → `Collector` stops accepting.

<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/concurrent/mod.rs" tag="buff" /> 
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/pipeline/concurrent/collector.rs" tag="collector" /> 
...
<ImportContent mode="code" source="crates/rtd-indexer-alt-framework/src/ingestion/broadcaster.rs" tag="bound" /> 
