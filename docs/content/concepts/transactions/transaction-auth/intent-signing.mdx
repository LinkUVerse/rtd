...
description: Intent signing provides a compact domain separator to data signed by both user and authority signatures. Data that each signature commits to is called an intent message, which all signatures on Rtd must include. 
...
In Rtd, an intent is a compact struct that serves as the domain separator for a message that a signature commits to. The data that the signature commits to is an intent message. All signatures in Rtd must commit to an intent message instead of the message itself.
...
1.  Passes the digest to the signing API for each corresponding scheme of the signer. The supported signature schemes are pure Ed25519, ECDSA Secp256k1, and ECDSA Secp256r1. See [Rtd Signatures](/concepts/transactions/transaction-auth.mdx#signature-requirements) for requirements of each scheme.

## Authority signature

The authority signature is created using the protocol key. The data that it commits to is also an intent message `intent || message`. See all available intent scopes [in the source code](https://github.com/LinkUVerse/rtd/blob/0dc1a38f800fc2d8fabe11477fdef702058cf00d/crates/rtd-types/src/intent.rs#L66).
...
The proof of possession is a BLS signature created using the authority protocol private key, committed over the following message: `intent || pubkey || address || epoch`. Here `intent` is serialized to `[5, 0, 0]` representing an intent with scope as `Proof of Possession`, `version` as `V0`, and `app_id` as `Rtd`. The `pubkey` value is the serialized public key bytes of the authority BLS protocol key. The `address` value is the account address associated with the authority account key. The `epoch` value is serialized to `[0, 0, 0, 0, 0, 0, 0, 0]`.
