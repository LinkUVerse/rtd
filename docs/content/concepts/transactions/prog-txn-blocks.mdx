...
description: Programmable transaction blocks are a group of commands that complete a transaction on Rtd.
...
The remainder of this topic covers the semantics of the execution of the transaction commands. It assumes familiarity with the Rtd object model and the [Move language](/concepts/rtd-move-concepts.mdx). 
...
- The `commands` value is a vector of commands, `[Command]`. The possible commands are:
  ...
  - [`SplitCoins`](#splitcoins) splits off one or more coins from a single coin. It can be any `rtd::coin::Coin<_>` object.

  - [`MergeCoins`](#mergecoins) merges one or more coins into a single coin. Any `rtd::coin::Coin<_>` objects can be merged, as long as they are all of the same type.
  ...
  - [`Upgrade`](#upgrade) upgrades an existing package. The upgrade is gated by the `rtd::package::UpgradeCap` for that package.
...
- If the object is owned by another object, as in it was sent to an object's ID through the `TransferObjects` command or the `rtd::transfer::transfer` function, then use `ObjectArg::Receiving(ObjectID, SequenceNumber, ObjectDigest)`. The object data is the same as for the `ImmOrOwnedObject` case.

For pure inputs, the only data provided is the [BCS](https://github.com/LinkUVerse/rtd/blob/main/crates/rtd-framework/docs/std/bcs.md) bytes, which are deserialized to construct Move values. Not all Move values can be constructed from BCS bytes. The following types are allowed to be used with pure values:
...
- `SplitCoins`: Produces one or more coins from a single coin. The type of each coin is `rtd::coin::Coin<T>` where the specific coin type `T` matches the coin being split.

- `Publish`: Returns the upgrade capability `rtd::package::UpgradeCap` for the newly published package.

- `Upgrade`: Returns the upgrade receipt `rtd::package::UpgradeReceipt` for the upgraded package.
...
The most important thing to note at this stage is the effects on the gas coin. At the beginning of execution, the maximum gas budget (in terms of `RTD`) is withdrawn from the gas coin. Any unused gas is returned to the gas coin at the end of execution, even if the coin has changed owners.
...
- If the signature expects a `T`, the runtime checks the argument has type `T` and it is copied if `T: copy` and moved otherwise. No object in Rtd has `copy` because the unique ID field `rtd::object::UID` present in all objects does not have the `copy` ability.
...
Object inputs have the type of their object `T` as you might expect. However, for `ObjectArg::Receiving` inputs, the object type `T` is instead wrapped as `rtd::transfer::Receiving<T>`. This is because the object is not owned by the sender, but instead by another object. To prove ownership with that parent object, you call the `rtd::transfer::receive` function to remove the wrapper.
...
- `CoinArg: Argument`: Coin of type `rtd::coin::Coin<T>` (any coin type). Taken by mutable reference.

- `AmountArgs: [Argument]`: `u64` values for split amounts. Taken by value (copied).

**Returns:** Vector of coins `rtd::coin::Coin<T>` matching the number of amounts.

**Signature:** `<T: key + store>(coin: &mut rtd::coin::Coin<T>, amounts: vector<u64>): vector<rtd::coin::Coin<T>>`
...
- `CoinArg: Argument`: Target coin of type `rtd::coin::Coin<T>` (any coin type). Taken by mutable reference.

- `ToMergeArgs: [Argument]`: Coins of type `rtd::coin::Coin<T>` to merge. Taken by value (moved).

**Returns:** Empty result vector.

**Signature:** `<T: key + store>(coin: &mut rtd::coin::Coin<T>, to_merge: vector<rtd::coin::Coin<T>>): ()`
...
**Returns:** Single result of type `rtd::package::UpgradeCap` for the newly published package.
...
- `UpgradeTicket: rtd::package::UpgradeTicket`: Upgrade ticket generated from `rtd::package::UpgradeCap`. Taken by value (moved).

**Returns:** Single result of type `rtd::package::UpgradeReceipt` providing proof of upgrade.

For more details on upgrades, see [Upgrading Packages](/concepts/rtd-move-concepts/packages/upgrade.mdx).
...
Any remaining RTD deducted from the gas coin at the beginning of execution is returned to the coin, even if the owner has changed. The maximum possible gas is deducted at the beginning of execution, and unused gas is returned at the end (all in RTD). Because the gas coin can only be taken by-value with `TransferObjects`, it has not been wrapped or deleted.

The total effects (created, mutated, and deleted objects) are then passed out of the execution layer and applied by the Rtd network.
...
```rust
{
  ...
  commands: [
    ...
    MoveCall("rtd", "tx_context", "sender", [], []),
    TransferObjects([NestedResult(1, 1)], NestedResult(3, 0)),
  ]
}
```

The inputs include the friend's address, the marketplace object, and the coin split value. The commands split off the coin, call the marketplace function, send the gas coin and one object, grab your address (through `rtd::tx_context::sender`), and send the remaining object to yourself.

#### Initial state

```rust
Gas Coin: Coin<RTD> { id: gas_coin, balance: 1_000_000u64 }
...
```rust
Gas Coin: Coin<RTD> { id: gas_coin, balance: 500_000u64 }
```
...
**Changes:** Gas coin balance decreases to `499_900u64`. New result: `[Coin<RTD> { id: new_coin, value: 100u64 }]`

#### Command 1: `MoveCall("some_package", "some_marketplace", "buy_two", ...)`

Calls `buy_two(marketplace: &mut Marketplace, coin: Coin<Rtd>, ctx: &mut TxContext): (Item, Item)`
...
#### Command 3: `MoveCall("rtd", "tx_context", "sender", [], [])`
