...
description: Understanding cryptographic keys, addresses, and signatures on Rtd. Rtd supports multiple cryptography algorithms and primitives with rapid switching between them.
...
Transaction authentication on Rtd involves three core concepts: cryptographic keys that you control, addresses that identify accounts on-chain, and signatures that prove ownership. Rtd implements these concepts using widely accepted wallet specifications and multiple signature schemes.

## Keys and addresses

Rtd adheres to widely accepted wallet specifications in the cryptocurrency industry, including [BIP-32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki) and its variation [SLIP-0010](https://github.com/satoshilabs/slips/blob/master/slip-0010), [BIP-44](https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki), and [BIP-39](https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki), to facilitate key management for users. Rtd supports pure Ed25519, ECDSA Secp256k1, ECDSA Secp256r1, and multisig for signed transactions.

### Key derivation schemes

For managing wallets that support the Ed25519 (EdDSA) signing scheme, Rtd follows [SLIP-0010](https://github.com/satoshilabs/slips/blob/master/slip-0010), which enforces wallets to always derive child private keys from parent private keys using the hardened key path.

Rtd follows BIP-32 for managing wallets that support the ECDSA Secp256k1 and ECDSA Secp256r1 signing schemes.
...
BIP-49 and BIP-84, for example, are used to identify script types in Bitcoin. Rtd chose `54` to indicate ECDSA Secp256k1 because there is no existing BIP under `54`, avoiding confusion with any Bitcoin standard.

The `coin_type` value is managed with a repository of all other cryptocurrencies. Both signature schemes use Rtd's registered `coin_type`: `784`.

The `account` level is used for logically separating user accounts and creating specific account categories.

Account-based currencies define only the first 3 levels, while UTXO-based currencies add `change` and `address` level definitions. Because Rtd's object-oriented data model is neither UTXO or account-based (it combines both), it employs all 5 levels for maximum compatibility.
...
After Rtd defines the deterministic way to derive the master key from a seed, BIP-39 is introduced to make the seed more human-readable and memorable using mnemonics. Rtd accepts 12, 15, 18, 21, and 24 words from the BIP-39 word list that is properly checksummed, corresponding to 128, 160, 192, 224, and 256 bits of entropy.

### Address format

For deriving a 32-byte Rtd address, Rtd hashes the signature scheme flag 1-byte concatenated with public key bytes using the [BLAKE2b](https://www.blake2.net/) (256 bits output) hashing function. Rtd addresses currently support pure Ed25519, Secp256k1, Secp256r1, and [multisig](/concepts/transactions/transaction-auth/multisig.mdx) with corresponding flag bytes of 0x00, 0x01, 0x02, and 0x03, respectively.
...
A Rtd signature consists of three components concatenated together:
...
- **Passkey**: See [SIP-8](https://github.com/rtd-foundation/sips/blob/main/sips/sip-9.md) for passkey implementation details.
...
Rtd's [collection of validators](/guides/operator/validator-index.mdx) holds 3 distinctive key pairs for different purposes.

### Protocol key pair {#protocol-pair}

The protocol key pair provides authority signatures on user-signed transactions if they are verified. When a stake of the authorities that provide signatures on user transactions passes the required 2/3 threshold, Rtd executes the transaction. Rtd uses the BLS12381 scheme for its fast verification on aggregated signatures for a given number of authorities. In particular, Rtd uses the minSig BLS mode, where each individual public key is 96 bytes, while the signature is 48 bytes. The latter is important as typically validators register their keys once at the beginning of each epoch and then they continuously sign transactions; thus, Rtd optimizes on minimum signature size.

As with the BLS scheme, you can aggregate independent signatures resulting in a single BLS signature payload. Rtd also accompanies the aggregated signature with a bitmap to denote which of the validators signed. This effectively reduces the authorities signature size from (2f + 1) × `BLS_sig` size to just 1 `BLS_sig` payload. This has significant network cost benefits resulting in compressed transaction certificates independently on the validators set size.

To counter potential rogue key attacks on BLS12381 aggregated signatures, proof of knowledge of the secret key (KOSK) is used during authority registration. When an authority requests to be added to the validator set, a proof of possession is submitted and verified. See [Intent Signing](/concepts/transactions/transaction-auth/intent-signing.mdx) on how to create a proof of possession. Unlike most standards, the Rtd proof of knowledge scheme commits to the address as well, which offers an extra protection against adversarial reuse of a validator BLS key from another malicious validator.

### Account key pair {#account-pair}

The account that the authority uses to receive payments on staking rewards is secured by the account key pair. Rtd uses pure Ed25519 as the signing scheme.

### Network key pair {#network-pair}

The private key is used to perform the TLS handshake required for [consensus](/concepts/rtd-architecture/consensus.mdx) networking. The public key is used for validator identity. Pure Ed25519 is used as the scheme.

See more authority key toolings in [Validator Tool](https://github.com/LinkUVerse/rtd/blob/f8b5ad9aaecc3c4b30a060ec5e00bdad9ba75a93/nre/validator_tool.md).

## Examples

The [Rtd CLI tool](/guides/developer/getting-started/rtd-install.mdx) and the [Rtd SDKs](https://sdk.linkulabs.com/typescript) provide a flexible interface to sign transactions with various signing schemes.

<Tabs>
<TabItem value="cli" label="CLI">
```sh
$ rtd keytool import "TEST_MNEMONIC" ed25519 "m/44'/784'/0'/0'/0'"
```
```sh
$ rtd client new-address ed25519 "m/44'/784'/0'/0'/0'"
```
...
See more test vectors for [pure Ed25519](https://github.com/LinkUVerse/rtd/blob/f3b5fdd73da64a0df65fb4323471512b0f57ec4d/sdk/typescript/test/unit/cryptography/ed25519-keypair.test.ts) or [ECDSA Secp256k1](https://github.com/LinkUVerse/rtd/blob/199f06d25ce85f0270a1a5a0396156bb2b83122c/sdk/typescript/test/unit/cryptography/secp256k1-keypair.test.ts).