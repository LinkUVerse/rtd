...
description: Recommended Move 2024 best practices for Rtd development.
---

This guide outlines recommended conventions and best practices for writing Move smart contracts on Rtd. Following these guidelines helps create more maintainable, secure, and composable code that aligns with ecosystem standards.

While these conventions are recommendations rather than strict rules, they represent patterns that have proven effective across many Rtd projects. They help create consistency across the ecosystem and make code easier to understand and maintain.

## Organization principles

### Package

A Rtd package consists of:
...
- `Move.lock` file that the Rtd Move toolchain automatically generates to lock the versions of the dependencies and track the different published and upgraded versions of the package that exist on the different networks.

For this reason, the `Move.lock` file should always be part of the package (don't add it to the `.gitignore` file). Use the [automated address management](/concepts/rtd-move-concepts/packages/automated-address-management.mdx) instead of the old `published-at` field in the manifest file.
...
The following section covers common patterns and best practices specific to Move development on Rtd, including object ownership models and function design principles.
...
public fun withdraw(state: &mut State, account: &mut Account, ctx: &mut TxContext): Coin<RTD> {
...
public fun wrong_withdraw(state: &mut State, ctx: &mut TxContext): Coin<RTD> {
...
public fun new(deposit: Coin<RTD>, ctx: &mut TxContext): OwnedWallet {
    // Implementation omitted.
    abort(0)
}
...
public fun new_shared(deposit: Coin<RTD>, ctx: &mut TxContext) {
    // Implementation omitted.
