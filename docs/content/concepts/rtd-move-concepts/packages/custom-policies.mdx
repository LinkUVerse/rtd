...
To address the security risk of single-key upgrade ownership while still providing the opportunity to [upgrade live packages](./upgrade.mdx), Rtd offers _custom upgrade policies_. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.

## Compatibility {#compatibility}

Rtd comes with a set of built-in package compatibility policies, listed here from most to least strict:
...
You can change the current policy by calling one of the functions in `rtd::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `rtd::package::only_dep_upgrades` to restrict the policy to become dependency-only, calling `rtd::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
While step 2 is a built-in command, steps 1 and 3 are implemented as Move functions. The Rtd framework provides their most basic implementation:

<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/package.move" mode="" fun="authorize_upgrade,commit_upgrade" noComments />

These are the functions that `rtd client upgrade` calls for authorization and commit. Custom upgrade policies work by guarding
...
<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/package.move" mode="code" struct="UpgradeCap" noComments />
...
<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/package.move" mode="code" struct="UpgradeTicket" noComments />
...
bytecode or source code for ahead of time. Rtd calculates the digest as follows:
...
Refer to the [implementation for digest calculation](https://github.com/LinkUVerse/rtd/blob/d8cb153d886d54752763fbdab631b062da7d894b/crates/rtd-types/src/move_package.rs#L232-L251) for more information, but in most cases, you can rely on the Move toolchain to output the digest as part of the build, when passing the `--dump-bytecode-as-base64` flag:

```sh
$ rtd move build --dump-bytecode-as-base64
```
```sh
FETCHING GIT DEPENDENCY https://github.com/LinkUVerse/rtd.git
...
<ImportContent source="crates/rtd-framework/packages/rtd-framework/sources/package.move" mode="code" struct="UpgradeReceipt" noComments />

The `UpgradeReceipt` is proof that the `Upgrade` command ran successfully, and Rtd added the new package to the set of created
...
After Rtd creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction (you cannot store it for later, drop it, or burn it), or the transaction fails.
...
The signature of a custom `authorize_upgrade` can be different from the signature of `rtd::package::authorize_upgrade` as long as it returns an `UpgradeTicket`.

Finally, provide implementations of `commit_upgrade` and `make_immutable` that delegate to their respective functions in `rtd::package`:
...
Use the `rtd client publish` command to publish the policy.
...
If you don't have a package available, use the `rtd move new` command to create the template for a new package called `example`.

```sh
$ rtd move new example
```
...
Open a terminal or console to the root of your Node.js project. Run the following command to add the Rtd TypeScript SDK as a dependency:
...
- `RTD`: the location of the `rtd` CLI binary.
...
Next, add boilerplate code to get the signer key pair for the currently active address in the Rtd Client CLI:
...
const sender = execSync(`${RTD} client active-address`, { encoding: 'utf8' }).trim();
const signer = (() => {
...
const { modules, dependencies } = JSON.parse(
	execSync(`${RTD} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
		encoding: 'utf-8',
	}),
);
```
...
```sh
$ rtd client call --gas-budget 10000000 \
...
Array [
    Object {
        ...
        "objectType": String("0x2::coin::Coin<0x2::rtd::RTD>"),
        "objectId": String("<GAS>"),
        "version": String("12"),
...
import { getFullnodeUrl, RtdClient } from '@linku/rtd/client';
import { Ed25519Keypair } from '@linku/rtd/keypairs/ed25519';
import { Transaction, UpgradePolicy } from '@linku/rtd/transactions';
import { fromBase64 } from '@linku/rtd/utils';
...
Use the Rtd Client CLI to test the upgraded package (the package ID is **different** from the original version of your example package):
