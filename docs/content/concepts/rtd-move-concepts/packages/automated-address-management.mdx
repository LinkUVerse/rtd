...
When you publish or upgrade your package on multiple Rtd networks (Mainnet, Testnet, Devnet), automated address management tracks separate addresses for each chain. Tracking is based on your active environment (run `rtd client active-env` if unsure). For example, if your active environment is set to an RPC connected to `testnet` and you publish a package, the `Move.lock` records the address for that package and associates it with `testnet`.

Automated address management works for one package published to one or more Rtd networks. If a package is republished to a network, tracked addresses for that network are overwritten.
...
1. Switch to your active environment for the chain where your package is published:
   ...
   ```sh
   rtd client --switch --env <YOUR-CHAIN-ENVIRONMENT>
   ```

2. Run the `manage-package` command with details of your published package:

   ```sh
   rtd move manage-package --environment "$(rtd client active-env)" \
                           --network-id "$(rtd client chain-identifier)" \
...
- Key-value pairs correspond to the data described in [adopting automated address management](#adopting-automated-address-management). Use the `rtd move manage-package` command as the frontend to modify these values.
