...
/// @title BridgeConfig
/// @notice This contract manages a registry of supported tokens and supported chain IDs for the RtdBridge.
/// It also provides functions to convert token amounts to Rtd decimal adjusted amounts and vice versa.
contract BridgeConfig is IBridgeConfig, CommitteeUpgradeable {
    ...
    function initialize(
        ...
        require(
            _supportedTokens.length == _rtdDecimals.length, "BridgeConfig: Invalid Rtd decimals"
        );
        ...
        for (uint8 i; i < _tokenIds.length; i++) {
            ...
            supportedTokens[_tokenIds[i]] = Token(_supportedTokens[i], _rtdDecimals[i], true);
        }
    ...
    /// @notice Returns the rtd decimal places of the token with the given ID.
    /// @param tokenID The ID of the token.
    /// @return amount of rtd decimal places of the provided token.
    function tokenRtdDecimalOf(uint8 tokenID) public view override returns (uint8) {
        return supportedTokens[tokenID].rtdDecimal;
    }
    ...
    /// @notice Returns whether a token is supported in RtdBridge with the given ID.
    /// @param tokenID The ID of the token.
    ...
    /// @notice Returns whether a chain is supported in RtdBridge with the given ID.
        ...
        verifyMessageAndSignatures(message, signatures, BridgeUtils.ADD_EVM_TOKENS)
    {
        ...
        for (uint8 i; i < tokenIDs.length; i++) {
            _addToken(tokenIDs[i], tokenAddresses[i], rtdDecimals[i], _tokenPrices[i], native);
        }

        emit TokensAddedV2(message.nonce, tokenIDs, tokenAddresses, rtdDecimals, _tokenPrices);
    }
    ...
    /// @param rtdDecimal The decimal places of the token.
    /// @param tokenPrice The price of the token.
    ...
    function _addToken(
        ...
        require(rtdDecimal > 0, "BridgeConfig: Invalid Rtd decimal");
        require(tokenPrice > 0, "BridgeConfig: Invalid token price");

        uint8 erc20Decimals = IERC20Metadata(tokenAddress).decimals();
        require(erc20Decimals >= rtdDecimal, "BridgeConfig: Invalid Rtd decimal");

        supportedTokens[tokenID] = Token(tokenAddress, rtdDecimal, native);
        tokenPrices[tokenID] = tokenPrice;
    }
