...
/// @title BridgeUtils
/// @notice This library defines the message format and constants for the Rtd native bridge. It also
...
library BridgeUtils {
    ...
    /// @notice Converts the provided token amount to the Rtd decimal adjusted amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param rtdDecimal The rtd decimal value for the token.
    /// @param amount The ERC20 amount of the tokens to convert to Rtd.
    /// @return Rtd converted amount.
    function convertERC20ToRtdDecimal(uint8 erc20Decimal, uint8 rtdDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == rtdDecimal) {
        ...
        require(erc20Decimal > rtdDecimal, "BridgeUtils: Invalid Rtd decimal");

        // Difference in decimal places
        uint256 factor = 10 ** (erc20Decimal - rtdDecimal);
    ...
    /// @notice Converts the provided Rtd decimal adjusted amount to the ERC20 token amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param rtdDecimal The rtd decimal value for the token.
    /// @param amount The Rtd amount of the tokens to convert to ERC20.
    /// @return ERC20 converted amount.
    function convertRtdToERC20Decimal(uint8 erc20Decimal, uint8 rtdDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (rtdDecimal == erc20Decimal) {
    ...
    ///     bytes 1-32   : sender address (as we only support Rtd now, it has to be 32 bytes long)
        ...
        returns (TokenTransferPayload memory)
    {
        ...
        require(
            ...
            "BridgeUtils: Invalid sender address length, Rtd address must be 32 bytes"
        );
    ...
    ///     byte m + 1       : number of rtd decimals
    ///     bytes m + 2 -> i : rtd decimals
    ...
    /// @return rtdDecimals the Rtd decimal places of the tokens to be added.
        ...
        )
    {
        ...
        rtdDecimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            rtdDecimals[i] = uint8(_payload[offset++]);
        }
