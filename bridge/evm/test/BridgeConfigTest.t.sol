...
contract BridgeConfigTest is BridgeBaseTest {
    ...
    function testBridgeConfigInitialization() public {
        ...
        assertEq(config.tokenRtdDecimalOf(0), 9);
        assertEq(config.tokenRtdDecimalOf(1), 8);
        assertEq(config.tokenRtdDecimalOf(2), 8);
        assertEq(config.tokenRtdDecimalOf(3), 6);
        assertEq(config.tokenRtdDecimalOf(4), 6);
        assertEq(config.tokenPriceOf(0), RTD_PRICE);
    ...
    function testTokenRtdDecimalOf() public {
        assertEq(config.tokenRtdDecimalOf(1), 8);
    }
    ...
    function testAddTokensWithSignatures() public {
        ...
        assertEq(config.tokenRtdDecimalOf(10), 6);
    ...
    function testAddTokensRtdDecimalFailure() public {
        MockUSDC _newToken = new MockUSDC();
        ...
        // add token shoudl fail because the rtd decimal is greater than the eth decimal
        vm.expectRevert(bytes("BridgeConfig: Invalid Rtd decimal"));
        config.addTokensWithSignatures(signatures, message);
    }
    ...
    function testUpdateTokenPriceRegressionTestWithSigVerficiation() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (address(committee), chainID, supportedTokens, tokenPrices, tokenIds, rtdDecimals, _supportedDestinationChains)
            ),
            opts
        );
    ...
    function testAddTokensRegressionTest() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                BridgeConfig.initialize,
                (address(committee), 12, supportedTokens, tokenPrices, tokenIds, rtdDecimals, _supportedDestinationChains)
            ),
            opts
        );
        config = BridgeConfig(_config);
        ...
        assertEq(rtdDecimals.length, 3);
        assertEq(rtdDecimals[0], 5);
        assertEq(rtdDecimals[1], 6);
        assertEq(rtdDecimals[2], 7);
        ...
        assertEq(config.tokenRtdDecimalOf(99), 5);
        assertEq(config.tokenRtdDecimalOf(100), 6);
        assertEq(config.tokenRtdDecimalOf(101), 7);
