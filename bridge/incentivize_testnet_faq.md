---
title: Rtd Bridge Testnet Incentive Program FAQ
---

# Rtd Bridge Testnet Incentive Program FAQ
...
**A:** After the program ends, the rewards will be sent to eligible participants' Rtd addresses on Mainnet that they used to test Rtd Bridge on testnet.

**Q: Am I eligible for rewards if I test the bridge by directly calling the contract/package?**\
**A:** No, to be eligible, users must directly interact with the [Rtd Bridge web interface on Testnet](https://bridge.testnet.rtd.io/?ref=blog.rtd.io)

**Q: Can I use a bot to bulk test the [Rtd Bridge Testnet](https://bridge.testnet.rtd.io/)?**\
...
**A:** Refer to the ["How to Guide" found in our latest blog post](https://blog.rtd.io/rtd-bridge-live-on-testnet-with-incentives/).

**Q: What is a roundtrip bridge? Do I have to do a round trip bridge to be eligible for rewards? What if I do three Ethereum to Rtd transfers and one Rtd to Ethereum transfer?**\
**A:** A roundtrip consists of bridging assets from Sepolia to Rtd Testnet, then bridging from Rtd Testnet back to Sepolia. Only a round trip is eligible for rewards. If an address does three Ethereum to Rtd transfers and one Rtd to Ethereum transfer, it will be counted as one eligible test activity.

**Q: If I use different Ethereum addresses, does it still count for a roundtrip?**\
**A:** Yes. We track eligible test activities by Rtd address.

**Q: Does a roundtrip require the same token and amount?**\
**A:** No it doesn't. Namely transferring 1 Native Sepolia ETH from Ethereum to Rtd and 500 USDC from Rtd to Ethereum is considered an eligible test activity.

**Q: I see my transfer is "delayed". What does it mean?**\
**A:** Check the "What is the limiter?" on the [FAQ section of the Rtd Bridge Website] (https://bridge.testnet.rtd.io/#faq). On Mainnet we expect the limiter to be hit rarely. However during the incentivize program we may intentionally trigger this scenario more often to thoroughly test it.
...
**A:** Prepare a video clip or screenshots to clearly reproduce the issue. Go to the ["rtd-bridge-forum" channel](https://discord.com/channels/916379725201563759/1249826301972316190) and make a post. Don't forget to provide your rtd address in the report.
