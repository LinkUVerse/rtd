# Rtd Native Bridge Primer for Agents

This document is a fast-start artifact for any agent (human or AI) that needs to reason about the Rtd Native Bridge. It maps the three primary code surfaces—the Ethereum smart contracts, the Rtd Move package, and the Rust bridge node—so you can quickly answer “where does this live?” and “who is responsible for what?” before making changes or asking follow-up questions.

## Purpose & Scope

- Clarify the ownership boundaries between the EVM contracts, the Move bridge module, and the Rust bridge node.
- Highlight the critical data flows for Rtd→EVM and EVM→Rtd transfers, plus emergency/governance operations.
...
| --------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| EVM `RtdBridge`       | `bridge/evm/contracts/RtdBridge.sol`                                                          | Custody vault orchestrator on Ethereum: collects deposits, enforces limits, releases assets after committee-signed messages.                                                                | `CommitteeUpgradeable`, `IBridgeVault`, `IBridgeLimiter`, `IBridgeConfig`, OpenZeppelin upgradeable stack.                    |
| Move `bridge::bridge` | `crates/rtd-framework/packages/bridge/sources/bridge.move`                                    | On-chain Rtd contract that mints/burns bridged assets, stores bridge messages, enforces limiter + committee approvals, handles governance ops.                                              | `bridge::committee`, `bridge::limiter`, `bridge::treasury`, `rtd::event`, `rtd_system::rtd_system`.                           |
| Rtd Bridge Node       | `crates/rtd-bridge/src` (`main.rs`, `node.rs`, `server`, `client`, `orchestrator`, `syncers`) | Off-chain daemon that (a) exposes a signing server for bridge actions, (b) optionally runs a client/orchestrator that watches both chains, aggregates signatures, and submits transactions. | Axum server, Reqwest client, `BridgeAuthorityAggregator`, `EthSyncer`, `RtdSyncer`, `BridgeOrchestrator`, Prometheus metrics. |

## EVM Contract: `bridge/evm/contracts/RtdBridge.sol`

### State & Initialization

`RtdBridge` keeps track of processed transfer nonces, references to the vault and limiter contracts, and enforces a fixed Rtd address length. Deployment follows the OpenZeppelin upgradeable pattern via `initialize`, which wires in the committee contract and pause controller:

```24:45:bridge/evm/contracts/RtdBridge.sol
mapping(uint64 nonce => bool isProcessed) public isTransferProcessed;
IBridgeVault public vault;
IBridgeLimiter public limiter;

uint8 constant RTD_ADDRESS_LENGTH = 32;
...
`transferBridgedTokensWithSignatures` is invoked by the bridge node once it has gathered quorum signatures for a Rtd→EVM transfer message. It verifies the message, ensures the nonce was not processed, decodes the payload, checks the target chain, converts Rtd decimals into ERC20 decimals, pulls tokens from the vault, and marks the nonce consumed:

```55:103:bridge/evm/contracts/RtdBridge.sol
...
{
    require(!isTransferProcessed[message.nonce], "RtdBridge: Message already processed");
    ...
    require(
        tokenTransferPayload.targetChain == config.chainID(), "RtdBridge: Invalid target chain"
    );
    uint256 erc20AdjustedAmount = BridgeUtils.convertRtdToERC20Decimal(
        IERC20Metadata(config.tokenAddressOf(tokenTransferPayload.tokenID)).decimals(),
        config.tokenRtdDecimalOf(tokenTransferPayload.tokenID),
...
Users call `bridgeERC20` or `bridgeETH` to lock assets into the vault. The contract checks the destination route, token support, allowance, and recipient length. It measures the vault delta, converts ERC20 decimals back into Rtd decimals, and emits `TokensDeposited` with the current nonce before incrementing it:

```135:189:bridge/evm/contracts/RtdBridge.sol
function bridgeERC20(
    ...
    require(recipientAddress.length == RTD_ADDRESS_LENGTH, "RtdBridge: Invalid recipient address length");
    IBridgeConfig config = committee.config();
    require(config.isTokenSupported(tokenID), "RtdBridge: Unsupported token");
    address tokenAddress = config.tokenAddressOf(tokenID);
    require(
        IERC20(tokenAddress).allowance(msg.sender, address(this)) >= amount,
        "RtdBridge: Insufficient allowance"
    );
    ...
    uint64 rtdAdjustedAmount = BridgeUtils.convertERC20ToRtdDecimal(
        IERC20Metadata(tokenAddress).decimals(),
        config.tokenRtdDecimalOf(tokenID),
        amountTransfered
    );
...
```244:265:bridge/evm/contracts/RtdBridge.sol
function _transferTokensFromVault(
    ...
    require(tokenAddress != address(0), "RtdBridge: Unsupported token");
...
```105:125:bridge/evm/contracts/RtdBridge.sol
...
- Decimal conversions happen _twice_ (ERC20→Rtd during deposit, Rtd→ERC20 during withdrawal); mismatched decimals lead to silent value drift.
...
- Registry for supported tokens (`tokenAddressOf`, `tokenRtdDecimalOf`, `tokenPriceOf`) and supported chain IDs. The contract also stores per-token USD prices (8 decimal precision) and can add new ERC20s via committee-signed `ADD_EVM_TOKENS` messages.
- `updateTokenPriceWithSignatures` and `addTokensWithSignatures` mirror the governance actions emitted on Rtd; both routes end with dedicated events so the off-chain client can pick them up.
- When adding a token, `_addToken` cross-checks ERC20 decimals against the provided Rtd decimals to prevent overflows during conversion.
...
- `recordBridgeTransfers` is `onlyOwner` (the RtdBridge proxy) and converts each transfer into USD using `BridgeConfig.tokenPriceOf`. If the USD total for the past 24 hours would exceed `chainLimits[chainID]`, it reverts with `"BridgeLimiter: amount exceeds rolling window limit"`.
...
## Move Package: `crates/rtd-framework/packages/bridge/sources/bridge.move`
...
```46:87:crates/rtd-framework/packages/bridge/sources/bridge.move
...
When a Rtd user bridges out, the module ensures the bridge is not paused, validates the route and EVM address length, increments the per-message-type sequence number, builds a `BridgeMessage`, burns the coins in the treasury, stores the record, and emits an event:

```218:333:crates/rtd-framework/packages/bridge/sources/bridge.move
...
Bridge authorities submit committee-signed `BridgeMessage`s. The Move module verifies signatures, ensures message type/version/chain constraints, and either attaches verified signatures to an existing record (Rtd-originated) or inserts a new record (foreign-originated). Duplicate approvals short-circuit with events:

```338:395:crates/rtd-framework/packages/bridge/sources/bridge.move
public fun approve_token_transfer(
    ...
    if (message.source_chain() == inner.chain_id) {
        ...
        assert!(!record.claimed, EInvariantRtdInitializedTokenTransferShouldNotBeClaimed);
...
```436:473:crates/rtd-framework/packages/bridge/sources/bridge.move
public fun execute_system_message(
    ...
    else if (message_type == message_types::add_tokens_on_rtd()) { /* ... */ }
    else {
...
- Stores the bridge committee members, their voting power snapshot, URLs, and blocklist state. Registration is limited to validators (sender must appear in `RtdSystemState.active_validator_addresses`), and the final committee is instantiated via `try_create_next_committee` during end-of-epoch.
- `verify_signatures` recreates the same prefix-hash scheme as the EVM contracts (`"RTD_BRIDGE_MESSAGE"` prefix + BCS payload) and enforces that the accumulated voting power across non-blocklisted members meets `BridgeMessage.required_voting_power()`:

```89:120:crates/rtd-framework/packages/bridge/sources/committee.move
...
- `register_foreign_token` accepts a `TreasuryCap<T>` and `UpgradeCap`, ensures the cap has zero supply, drops the metadata into a “waiting room,” and emits `TokenRegistrationEvent`. Only after governance approves the token via `AddTokenOnRtd` does `add_new_token` move it into `supported_tokens`, freeze the UpgradeCap, and emit `NewTokenEvent`.
...
- `create_token_bridge_message` encodes sender bytes, target chain/address, token type, and amount into a canonical BCS layout that both Rtd and Ethereum verify.
...
## Rtd Bridge Node (`crates/rtd-bridge`)
...
```53:150:crates/rtd-bridge/src/node.rs
pub async fn run_bridge_node(
    ...
    let rtd_chain_identifier = server_config.rtd_client.get_chain_identifier().await?;
    let eth_chain_identifier = server_config.eth_client.get_chain_id().await?;
    prometheus_registry.register(linku_metrics::bridge_uptime_metric(/* ... */))?;
    let committee = Arc::new(server_config.rtd_client.get_bridge_committee().await?);
    let mut handles = vec![];
    let eth_provider = server_config.eth_client.provider();
    let eth_bridge_proxy_address = server_config.eth_bridge_proxy_address;
    let rtd_client = server_config.rtd_client.clone();
    handles.push(spawn_logged_monitored_task!(start_watchdog(/* ... */)));
    let rtd_system = server_config.rtd_client.jsonrpc_client().governance_api().get_latest_rtd_system_state().await?;
    if let Some(client_config) = client_config {
        let committee_keys_to_names = Arc::new(get_validator_names_by_pub_keys(&committee, &rtd_system).await);
        let client_components = start_client_components(/* ... */).await?;
        handles.extend(client_components);
    }
    let committee_name_mapping = get_committee_voting_power_by_name(&committee, &rtd_system).await;
    // expose Axum server
...
```91:139:crates/rtd-bridge/src/server/mod.rs
pub fn run_server(
...
If `run_client` is true in the config, `start_client_components` spins up the full cross-chain pipeline: persistent storage (`BridgeOrchestratorTables`), `EthSyncer` + `RtdSyncer` watchers, `BridgeAuthorityAggregator` (fetches signatures from peer bridge nodes), `BridgeActionExecutor` (submits transactions to Rtd/EVM), `BridgeMonitor`, and orchestrator channels. It also queries token metadata, checks the pause flag, and seeds watch cursors:

```258:360:crates/rtd-bridge/src/node.rs
async fn start_client_components(
    ...
    let rtd_modules_to_watch = get_rtd_modules_to_watch(/* ... */);
    ...
    let (task_handles, rtd_events_rx) = RtdSyncer::new(client_config.rtd_client, rtd_modules_to_watch, metrics.clone())
    ...
    let rtd_token_type_tags = rtd_client.get_token_id_map().await.unwrap();
    let is_bridge_paused = rtd_client.is_bridge_paused().await.unwrap();
    let (bridge_pause_tx, bridge_pause_rx) = tokio::sync::watch::channel(is_bridge_paused);
    let (rtd_monitor_tx, rtd_monitor_rx) = linku_metrics::metered_channel::channel(10000, /* ... */);
    let (eth_monitor_tx, eth_monitor_rx) = linku_metrics::metered_channel::channel(10000, /* ... */);
    let rtd_token_type_tags = Arc::new(ArcSwap::from(Arc::new(rtd_token_type_tags)));
...
```4:105:crates/rtd-bridge/src/orchestrator.rs
//! `BridgeOrchestrator` monitors Rtd & Ethereum events, updates WAL/cursors, hands actions to the executor.
...
- `start_watchdog` (see `node.rs`) monitors Ethereum vault balances for WETH/USDT/WBTC/LBTC, ETH & Rtd pause status, and optional total supply invariants, exporting Prometheus gauges.
- `metrics.rs` and `linku_metrics::bridge_uptime_metric` standardize dashboards; if you add new long-running tasks, wrap them with `spawn_logged_monitored_task!`.

## End-to-End Flows

Bridge clients (the optional `run_client` portion of `rtd-bridge`) always submit an **approval transaction** before any assets are released or minted. On both chains this approval is the `bridge::approve_token_transfer` Move call—which either attaches signatures to an existing Rtd-originated record or inserts a new record for EVM-originated transfers. Only after this approval lands do claims/unlocks occur. Keep that invariant in mind when reasoning about replay protection or idempotency.

### Rtd → Ethereum (Mint on Rtd, Unlock on EVM)

1. User calls `bridge::send_token` to burn/lock the Rtd asset and emit `TokenDepositedEvent` (`bridge.move` lines 218–333).
2. `RtdSyncer` (client-side) tails the `bridge`, `bridge_committee`, `bridge_treasury`, and `bridge_limiter` modules, forwarding every deposit event into the orchestrator channel.
3. `BridgeOrchestrator` converts the raw event into `BridgeAction::RtdToEthTokenTransfer` and persists it in the WAL to guarantee retries.
4. `BridgeAuthorityAggregator` gathers the validator signatures needed for quorum by calling each Axum `/sign/bridge_action/rtd/eth/...` endpoint.
5. **Approval:** The bridge client sends `bridge::approve_token_transfer` on Rtd. Because the message originated on Rtd, this transaction simply attaches the aggregated signatures to the existing `BridgeRecord` (`bridge.move` lines 338–372). This is the first on-chain transaction the client executes in this direction.
6. After approval is on-chain, the executor calls `RtdBridge.transferBridgedTokensWithSignatures` on Ethereum, which verifies the same signatures, withdraws from the vault, and pays the EVM recipient (`RtdBridge.sol` lines 55–103).

```mermaid title="Rtd → Ethereum Flow" type="diagram"
flowchart LR
    ...
    F --> G["RtdBridge.sol transferBridgedTokensWithSignatures\n(EVM unlock)"]
    G --> H["EVM Recipient"]
```

### Ethereum → Rtd (Lock on EVM, Mint on Rtd)

1. User calls `RtdBridge.bridgeERC20`/`bridgeETH` to send funds into the EVM vault and emit `TokensDeposited` (`RtdBridge.sol` lines 135–235).
...
3. `BridgeOrchestrator` maps logs into `BridgeAction::EthToRtd*` records, writes them into the WAL, and forwards them to the executor.
4. `BridgeAuthorityAggregator` again fans out to Axum peers for signatures covering the decoded `BridgeMessage`.
5. **Approval:** The bridge client now calls `bridge::approve_token_transfer` on Rtd. Because the message originated on Ethereum, this transaction inserts a brand-new `BridgeRecord` containing the message + signatures (`bridge.move` lines 374–395). This is the first client-submitted transaction in this direction and unlocks the subsequent claim path.
6. Recipients (or relayers via `claim_and_transfer_token`) call `bridge::claim_token`/`claim_and_transfer_token`, which runs limiter checks, mints from the treasury, and transfers the Rtd coins to the target address (`bridge.move` lines 398–434 & 548–634).

```mermaid title="Ethereum → Rtd Flow" type="diagram"
flowchart LR
    A["EVM User\nbridgeERC20/ETH"] -->|TokensDeposited event| B["RtdBridge.sol + Vault"]
    B --> C["EthSyncer\n(node client)"]
    C --> D["BridgeOrchestrator\nWAL"]
    D --> E["BridgeAuthorityAggregator\n(sigs)"]
    E --> F["bridge::approve_token_transfer\n(first client tx)"
...
- Emergency pause/unpause: bridge node hits `/sign/emergency_button/...`, collects signatures, then calls `bridge::execute_system_message` (Move) or `RtdBridge.executeEmergencyOpWithSignatures` (EVM) to toggle `paused`.
...
- Adding tokens: `/sign/add_tokens_on_rtd` or `/sign/add_tokens_on_evm` feeds into corresponding Move/EVM modules; keep token IDs, type names, and prices arrays length-matched (see Move code lines 676–693).

```mermaid title="Rtd Native Bridge Data Flow" type="diagram"
flowchart LR
    RtdUser["Rtd User\nsend_token"] -->|burn & emit event| RtdBridge["Move bridge::bridge"]
    RtdBridge -->|TokenDepositedEvent| RtdSyncer["RtdSyncer\n(node client)"]
    RtdSyncer --> Orchestrator["BridgeOrchestrator\nWAL + actions"]
    ...
    AxumServer --> EvmBridge["EVM RtdBridge.sol"]
    ...
    MoveBridgeOps -->|mint/claim| RtdRecipient["Rtd Recipient"]
...
```67:86:crates/rtd-framework/packages/bridge/sources/bridge.move
...
```275:333:crates/rtd-framework/packages/bridge/sources/bridge.move
...
```548:582:crates/rtd-framework/packages/bridge/sources/bridge.move
...
- `RtdBridgeV2.sol` duplicates the deposit/claim entry points with timestamp-aware variants. Deposits emit `TokensDepositedV2` (including `block.timestamp` in seconds), while withdrawals call `_transferTokensFromVault` guarded by `limitNotExceededV2`:
...
// RtdBridgeV2.sol - limitNotExceededV2 modifier
...
- **Rtd/Move**: Uses milliseconds (`timestamp_ms`)
...
- **Rust bridge node**: Converts ETH seconds → Rtd milliseconds (`* 1000`) when building Rtd transactions

### Bridge Node Updates

- New bridge actions (`RtdToEthTokenTransferV2`, `EthToRtdTokenTransferV2`) propagate the timestamp through the orchestrator WAL, and the authority aggregator requests signatures against the V2 payload format (`BridgeUtilsV2`).
...
- Watchers must track both the legacy and V2 events. `get_rtd_modules_to_watch` and `get_eth_contracts_to_watch` remain the same; the differentiator is the event decoding logic in `RtdBridgeEvent` / `EthBridgeEvent`.
...
- **Explain the nonce story:** Can you point to `BridgeInner.sequence_nums` and `RtdBridge.nonces` and describe how they prevent replays across message types?
- **Describe limiter enforcement on both sides:** On EVM it’s `limitNotExceeded` + `limiter.recordBridgeTransfers` (`RtdBridge.sol` lines 244–265); on Rtd it’s `limiter.check_and_record_sending_transfer` inside `claim_token_internal` (`bridge.move` lines 603–624). Make sure you can articulate when the 48h bypass kicks in.
...
- **Know where to adjust watchers:** `get_rtd_modules_to_watch` & `get_eth_contracts_to_watch` in `node.rs` (lines 362–404) determine cursor overrides. If you change event structure, update these lists.
- **Be ready to run tests:** `forge test` in `bridge/evm`, `cargo test -p rtd-bridge` for Rust, `rtd move test` for the Move package. Long-running tasks need ≥10m timeouts per repo guidelines.
...
- **Bridge node:** `cargo run -p rtd-bridge -- --config-path <path>`; ensure your config passes `BridgeNodeConfig::validate` (see `crates/rtd-bridge/src/config.rs`).
- **Move package:** `rtd move test --path crates/rtd-framework/packages/bridge`.
- **Metrics:** Run `cargo run -p rtd-bridge ...` and hit `http://<host>:<metrics_port>/metrics` or `/ping` to confirm the node advertises its version and metrics key.

## Running Rust Tests for rtd-bridge

The `rtd-bridge` crate has specific test configuration requirements that differ from typical Cargo test invocations.
...
```toml
# From crates/rtd-bridge/Cargo.toml
[features]
...
```bash
cargo check -p rtd-bridge
```

**Unit tests only (fast, no external dependencies):**

```bash
cargo nextest run -p rtd-bridge --features test-utils --lib
```

**Full test rtdte including e2e tests:**

```bash
cargo nextest run -p rtd-bridge --features test-utils
```

**Run specific e2e tests:**

```bash
cargo nextest run -p rtd-bridge --features test-utils e2e_tests::basic
```

### E2E Test Considerations

E2E tests spin up local Rtd clusters and anvil (Ethereum simulator). They require:
...
| Integration  | `action_executor::`, `monitor::`, `rtd_client::` | 1–10s each       | Uses mock clients        |
...
With this primer in hand, you—and future agents—should be able to navigate the Rtd Native Bridge code paths, justify design decisions, and verify understanding before proposing changes.
